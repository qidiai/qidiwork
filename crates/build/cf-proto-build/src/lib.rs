...
impl XaiProtoBuilder {
    ...
    fn emit_rerun_if_changed<'a>(
        ...
        // On Unix we use /dev/null for descriptor output and /dev/stdout for
        // dependency output. On Windows, /dev/stdout doesn't exist, so we use
        // a temp file for dependency output and NUL for descriptor output.
        let null_device = if cfg!(windows) { "NUL" } else { "/dev/null" };

        for proto in protos {
            let mut command = Command::new(protoc.unwrap_or(Path::new("protoc")));

            // Use a temp file for dependency output (cross-platform).
            let dep_file = std::env::temp_dir().join(format!(
                "protoc_dep_{}.dep",
                Path::new(proto).file_stem().unwrap_or_default().to_string_lossy()
            ));
            ...
            // protoc's dependency output format is: <descriptor_set_out_path>: <deps>
            let prefix = format!("{}:", null_device);
            let rem = first_line.strip_prefix(&prefix).with_context(|| {
                format!("protoc command output must start with {}: {output:?}", null_device)
            })?;
