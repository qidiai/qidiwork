...
use std::sync::Arc;
use cf_tools::bridge::ToolBridge;
...
/// tool registry in `qidi-code` (also used by the hook matcher).
fn claude_tool_kind(name: &str) -> Option<ToolKind> {
    cf_tools::types::kind_for(name)
}
...
pub struct AgentBuilder {
    ...
    /// path (e.g., `~/.qidi/worktrees/project/fork-...-overlay`) that must stay
    ...
    memory_backend: Option<Arc<dyn cf_tools::types::memory_backend::MemoryBackend>>,
    web_search_config: cf_tools::implementations::web_search::WebSearchConfig,
    ...
    compat: cf_tools::types::compat::CompatConfig,
    ...
    attribution_callback: Option<cf_tools::SharedAttributionCallback>,
    ...
    /// `resolve_max_mcp_output_bytes_for_cwd` in qidi-code).
...
fn merge_tool_params(
    tool_config: &mut cf_tools::registry::types::ToolServerConfig,
    ids: &[&str],
...
impl AgentBuilder {
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<cf_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn cf_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: cf_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn cf_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: cf_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    /// Without this, only auto-discovered skills (cwd/.qidi/skills, ~/.qidi/skills)
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        if definition.inject_default_tools {
            ...
            if self.write_file_enabled && !has_write_tool {
                tool_config
                    .tools
                    .push((&cf_tools::implementations::opencode::OpenCodeWriteTool).into());
            }
        ...
        if task_stripped {
            use cf_tools::types::tool::ToolNamespace;
        ...
        if !definition.tools.is_empty() {
            ...
            if unresolved.is_empty() {
                ...
                tracing::warn!(
                    ...
                    definition.tools,
                    "tools allowlist had unmappable entries; keeping full toolset"
                );
            }
        }
        tool_config
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                cf_tools::types::resources::TruncationCfg(
                    cf_tools::types::context::TruncationConfig {
        ...
        if use_backend_search {
            if web_search_enabled && definition.hosted_tool_allowed("web_search") {
                hosted_tools.push(cf_sampling_types::HostedTool::WebSearch {
                    allowed_domains: None,
                });
            }
            if definition.hosted_tool_allowed("x_search") {
                hosted_tools.push(cf_sampling_types::HostedTool::XSearch);
            }
        }
...
/// CLI naming for the shared [`cf_tool_types::build_task_description`] builder.
const TASK_TOOL_NAMING: cf_tool_types::TaskToolNaming<'static> = cf_tool_types::TaskToolNaming {
...
/// CLI [`cf_tool_types::SubagentToolNaming`]: each kind maps to its
...
const SUBAGENT_TOOL_NAMING: cf_tool_types::SubagentToolNaming<'static> =
    cf_tool_types::SubagentToolNaming {
...
/// shared [`cf_tool_types`] catalog and rendered with [`SUBAGENT_TOOL_NAMING`]
...
fn builtin_tools_fragment(name: BuiltinAgentName) -> String {
    let subagent = match name {
        BuiltinAgentName::GeneralPurpose => cf_tool_types::GENERAL_PURPOSE_SUBAGENT,
        BuiltinAgentName::Explore => cf_tool_types::EXPLORE_SUBAGENT,
        BuiltinAgentName::Plan => cf_tool_types::PLAN_SUBAGENT,
        _ => return String::new(),
    };
...
/// [`cf_tool_types::SubagentDescriptor`] and defers to
/// [`cf_tool_types::build_task_description`] so the CLI and the prod chat
...
pub(crate) fn build_task_description(
    ...
    let descriptors: Vec<cf_tool_types::SubagentDescriptor> = subagents
        .iter()
        .map(|entry| {
            ...
            cf_tool_types::SubagentDescriptor {
        ...
        .collect();
    let mut description = cf_tool_types::build_task_description(&descriptors, &TASK_TOOL_NAMING);
...
fn resolve_shell_for_prompt() -> String {
    ...
    #[cfg(not(unix))]
    {
        cf_config::shell::detect_windows_shell()
            .name()
            .to_string()
    }
}
...
mod tests {
    ...
    fn entry(name: &str, desc: &str, source: SubagentSource) -> SubagentEntry {
        SubagentEntry {
            ...
            config_source: cf_tools::types::config_source::ConfigSource::Builtin,
        }
    }
    #[test]
    fn build_task_description_builtin_includes_tools() {
        ...
        assert!(
            desc.contains(cf_tool_types::GENERAL_PURPOSE_SUBAGENT.tools_template),
            "should include general-purpose tool names"
        );
        assert!(
            desc.contains(cf_tool_types::EXPLORE_SUBAGENT.tools_template),
    ...
    fn build_task_description_shadowed_builtin_uses_user_desc() {
        let subagents = vec![SubagentEntry {
            ...
            config_source: cf_tools::types::config_source::ConfigSource::Project {
    ...
    fn task_model_guidance_resolves_model_param_override() {
        use cf_tools::types::template_renderer::TemplateRenderer;
        use cf_tools::types::tool::ToolKind;
    ...
    /// cf-tools toolsets have no Skill tool — skills are read from
    ...
    async fn disallowed_web_search_strips_function_and_hosted_tools() {
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, cf_sampling_types::HostedTool::WebSearch { .. })),
            "hosted WebSearch must be removed when web_search is disallowed, got: {hosted:?}"
        );
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, cf_sampling_types::HostedTool::XSearch)),
