...
//!
//! Searches `.qidi/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.qidi/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.qidi/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".qidi/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.qidi/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.qidi/agents` / `.claude/agents`) under each
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.qidi/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.qidi/agents/` walking from `cwd` up to repo root
/// 2. `~/.qidi/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.qidi/bundled/agents/` (bundled, lowest priority)
///
...
/// (QIDI_HOME-aware) plus the legacy literal `~/.qidi` when QIDI_HOME points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    grok_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.qidi, included only when it differs from grok_home
    // (i.e. QIDI_HOME points elsewhere) so agents left in the old location are
...
///
/// Project-level `.qidi/agents/` has highest priority, then falls back
...
/// Load project agent definitions from every `.qidi/agents` / `.claude/agents`
...
mod tests {
    ...
    fn user_agent_dirs_dedups_legacy_when_grok_home_is_dot_grok() {
        ...
        assert_eq!(
            count, 1,
            "no duplicate ~/.qidi/agents when grok_home == ~/.qidi"
        );
    }
    ...
    fn test_by_name_in_cwd_project_shadows_builtin() {
        ...
        // Create a project-level "cf-tools" that shadows the built-in
        write_agent_file(
            &agents_dir,
            "cf-tools.md",
            "cf-tools",
        ...
        let def = by_name_in_cwd("cf-tools", tmp.path());
        assert!(def.is_some());
        let def = def.unwrap();
        assert_eq!(def.name, "cf-tools");
        assert_eq!(def.description, "Custom cf-tools");
    }

    #[test]
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .qidi/agents/ directory — should fall back to built-in
    ...
    fn test_orchestrator_by_name_in_cwd() {
        let tmp = tempfile::tempdir().unwrap();
        let def = by_name_in_cwd("cf-tools-orchestrator", tmp.path())
            .expect("by_name_in_cwd must find cf-tools-orchestrator");
        assert_eq!(def.name, "cf-tools-orchestrator");
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.qidi/agents/) agent named "explore" should NOT shadow
