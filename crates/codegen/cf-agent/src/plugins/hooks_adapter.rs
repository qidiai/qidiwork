...
use cf_hooks::config::{HookSpec, parse_hook_file};
...
fn process_hooks_content(
    ...
    for spec in &mut specs {
        ...
        if let Some(cmd) = &spec.command {
            ...
            let expanded = cf_config::expand_env_vars_in_string(&substituted);
            if expanded != cmd_str {
...
mod tests {
    ...
    fn parse_plugin_hooks_from_file() {
        ...
        assert_eq!(
            specs[0].extra_env.get("QIDI_PLUGIN_ROOT").unwrap(),
            "/path/to/plugin"
        );
        assert_eq!(
        ...
        assert_eq!(
            specs[0].extra_env.get("QIDI_PLUGIN_DATA").unwrap(),
            "/path/to/data"
        );
    ...
    /// `${CLAUDE_PLUGIN_ROOT}` (or its `QIDI_PLUGIN_ROOT` alias) must be
    ...
    fn parse_plugin_hooks_substitutes_plugin_root_in_command() {
            ...
            "hooks": {
                "PreToolUse": [
                    {"hooks": [
                        ...
                        {"type": "command", "command": "${QIDI_PLUGIN_ROOT}/hooks/alias.sh"},
        ...
        assert!(
            raws.contains(&"${QIDI_PLUGIN_ROOT}/hooks/alias.sh"),
            "command_raw must preserve the source string verbatim, got {raws:?}"
        );
        assert!(
    ...
    /// (CLAUDE_PLUGIN_ROOT, QIDI_PLUGIN_ROOT, CLAUDE_PLUGIN_DATA,
    /// QIDI_PLUGIN_DATA). This preserves the plugin contract while still
    ...
    fn parse_plugin_hooks_user_env_merged_with_plugin_precedence() {
                    ...
                    {"hooks": [
                        {
                            ...
                            "env": {
                                ...
                                "QIDI_PLUGIN_ROOT": "/user/wins?",
                                "CLAUDE_PLUGIN_DATA": "/user/wins?",
                                "QIDI_PLUGIN_DATA": "/user/wins?"
                            }
                        }
                    ]}
                ]
        ...
        // overridden. CLAUDE_PLUGIN_ROOT and QIDI_PLUGIN_ROOT both map
        // to plugin_root; CLAUDE_PLUGIN_DATA and QIDI_PLUGIN_DATA both
        // map to plugin_data.
        for (key, expected) in [
            ("CLAUDE_PLUGIN_ROOT", "/actual/plugin/root"),
            ("QIDI_PLUGIN_ROOT", "/actual/plugin/root"),
            ("CLAUDE_PLUGIN_DATA", "/actual/plugin/data"),
            ("QIDI_PLUGIN_DATA", "/actual/plugin/data"),
        ] {
            assert_eq!(
