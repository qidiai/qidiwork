...
/// Default install directory name under `~/.qidi/`.
...
impl InstallRegistry {
    ...
    pub fn save_atomic(&self) -> Result<(), InstallError> {
        ...
        if std::env::var_os("XAI_QIDI_TEST_FAIL_REGISTRY_SAVE_AFTER_SERIALIZE").is_some() {
