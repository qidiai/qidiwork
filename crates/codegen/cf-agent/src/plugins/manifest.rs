...
///
/// Replaces `${QIDI_PLUGIN_ROOT}`, `${CLAUDE_PLUGIN_ROOT}`,
/// `${QIDI_PLUGIN_DATA}`, and `${CLAUDE_PLUGIN_DATA}` with the provided values.
///
/// Delegates to [`cf_tools::util::substitute_plugin_tokens`], the single
...
pub fn substitute_env_vars(s: &str, plugin_root: &str, plugin_data: &str) -> String {
    cf_tools::util::substitute_plugin_tokens(s, Some(plugin_root), Some(plugin_data))
}
...
mod tests {
    ...
    fn substitute_env_vars_replaces_all() {
        let input = "${QIDI_PLUGIN_ROOT}/bin:${CLAUDE_PLUGIN_ROOT}/lib:${QIDI_PLUGIN_DATA}/cache";
