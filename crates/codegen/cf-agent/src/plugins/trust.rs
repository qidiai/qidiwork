...
//!
//! Plugins from project directories (`.qidi/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.qidi/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.qidi/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.qidi/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_grok_home() so a project's `.qidi/trusted-plugins` is never
        // read as the user trust store when neither QIDI_HOME nor a home dir resolves.
        let Some(grok) = cf_config::user_grok_home() else {
            return Self {
                trusted: HashSet::new(),
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.qidi/trusted-plugins`.
    ...
    /// rewrites `~/.qidi/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.qidi/trusted-plugins`.
