...
//!
//! Searches from cwd to repo root, plus `~/.qidi/`. Also discovers
//! `*.md` files in `.qidi/rules/` and `.claude/rules/` directories.
...
/// Find `*.md` files in `.qidi/rules/`, `.claude/rules/`, and `.cursor/rules/`,
...
/// Read Agents.md from ~/.qidi/, git repo root, and session cwd.
...
async fn read_agents_config_with_options(
    ...
    let global_dir = cf_tools::util::grok_home::grok_home();
    ...
    // Always include grok_home (~/.qidi/) first, then ~/.claude/ and ~/.cursor/
...
/// Used by `qidi-code` to structurally detect legacy untagged AGENTS.md
...
fn render_agents_md(configs: &[AgentConfigFile]) -> Option<String> {
    ...
    for config in configs {
        ...
        // .qidi/rules/*.md) so globs/paths metadata doesn't leak into the
        // system prompt as raw YAML.
        let is_rules_file = config.file_path.contains("/.qidi/rules/")
            || config.file_path.contains("/.claude/rules/");
        let content = if is_rules_file {
            cf_tools::implementations::skills::skill::extract_skill_body(&config.content)
        } else {
