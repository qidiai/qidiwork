...
//! `TemplateRenderer` in `qidi-code`. This struct does NOT own a
...
pub struct PromptContext {
    ...
    /// Identity in the primary cf-tools system prompt (`You are <label>…`).
    /// Not the UI picker name. Defaults to [`DEFAULT_SYSTEM_PROMPT_LABEL`].
    #[serde(default = "default_system_prompt_label")]
    pub system_prompt_label: String,
}
/// Default identity on trim-tool-descriptions (`You are QIDI Code`).
pub const DEFAULT_SYSTEM_PROMPT_LABEL: &str = "QIDI Code";
...
mod tests {
    ...
    fn test_agents_md_paths_use_display_cwd_in_rendered_section() {
        ...
        let overlay_path = "/root/.qidi/worktrees/my-project/ab-123-a-overlay";
    ...
    fn trimmed_prompts_no_redundant_identity() {
        ...
        for (name, prompt) in [
            ...
            assert!(
                !prompt.contains("You are a QIDI Code agent"),
