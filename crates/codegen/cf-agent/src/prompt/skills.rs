...
//! live in `cf_tools::implementations::skills::discovery`.

use std::collections::{HashMap, HashSet};
use std::path::{Path, PathBuf};

use crate::plugins::discovery::PluginScope;
use cf_tools::implementations::skills::types::skill_name_from_path;
pub use cf_tools::implementations::skills::types::{SkillInfo, SkillScope};
/// Re-export so agent-side discovery (and the shell) can name the resolved
/// vendor-compat config without reaching into `cf_tools` directly.
pub use cf_tools::types::compat::CompatConfig;

use cf_tools::implementations::skills::discovery::{
...
///
/// Priority order: Local (cwd/.qidi/skills, cwd/.agents/skills, cwd/.claude/skills) → Intermediate dirs →
/// Repo (repo_root/.qidi/skills, repo_root/.agents/skills, repo_root/.claude/skills) → User (~/.qidi/skills, ~/.agents/skills, ~/.claude/skills)
/// → additional paths from `config.paths`
/// → Server (injected `config.server_skill_dirs`)
/// → Bundled (injected `config.bundled_skill_dirs` + `~/.qidi/bundled`; lowest precedence).
///
...
/// (e.g. `Repo` for `.qidi/plugins/`). Native skills always win bare-name
...
pub async fn list_skills_with_plugins(
    ...
    let mut skills = list_skills_with_options(
        ...
        &cf_tools::util::grok_home::grok_home(),
        compat,
    )
    .await;
...
fn scope_for_config_dir(dir: &Path, cwd: Option<&Path>, git_root: Option<&Path>) -> SkillScope {
    // Home-level dirs (e.g. ~/.qidi/, ~/.agents/, ~/.claude/) are User scope.
    #[allow(deprecated)]
    if let Some(home) = std::env::home_dir()
...
fn collect_config_skills(config_paths: &[String], git_root: Option<&Path>) -> Vec<SkillInfo> {
    ...
    for skill in &mut skills {
        skill.config_source = Some(
            cf_tools::types::config_source::ConfigSource::ConfigToml {
...
pub(crate) fn format_skill_for_injection(skill: &SkillInfo) -> Option<String> {
    skill.body.as_ref().filter(|b| !b.is_empty()).map(|body| {
        cf_tools::implementations::skills::skill::build_skill_message(skill, body)
    })
}
    ...
    for name in names {
        ...
        let skill = discovered.iter().find(|s| {
            s.name.eq_ignore_ascii_case(name)
                || cf_tools::implementations::skills::skill::format_skill_name(s)
        ...
        match cf_tools::implementations::skills::skill::load_skill_with_body(skill).await {
...
mod tests {
    ...
    fn collect_config_skills_stamps_config_toml_source() {
        ...
        match &skills[0].config_source {
            Some(cf_tools::types::config_source::ConfigSource::ConfigToml { path }) => {
    ...
    async fn list_skills_auto_and_config_overlap_keeps_config_toml_source() {
        ...
        assert!(
            matches!(
                ...
                Some(cf_tools::types::config_source::ConfigSource::ConfigToml { .. })
            ),
    ...
    // these functions now live in qidi-code::implementations::skills::discovery
    // and qidi-code::types::skill_discovery_tracker, tested there.
    ...
    fn dedupe_frontmatter_owner_evicts_rekeyed_squatter() {
        ...
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/u/.qidi/skills/japandi/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "japandi",
                "/u/.qidi/skills/japandi2/SKILL.md",
                SkillScope::User,
            ),
            named_skill(
                "japandi2",
    ...
    fn dedupe_cross_scope_shadowing_unchanged() {
        ...
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/repo/.qidi/skills/japandi/SKILL.md",
                SkillScope::Repo,
            ),
            named_skill("japandi", "/u/skills/japandi2/SKILL.md", SkillScope::User),
        ]);
    ...
    fn dedupe_same_scope_same_basename_still_drops() {
        ...
        // (e.g. ~/.qidi/skills and ~/.agents/skills): first-seen wins.
        let out = dedupe_skills(vec![
            named_skill(
                "japandi",
                "/u/.qidi/skills/japandi/SKILL.md",
