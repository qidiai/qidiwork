...
//! - `Default` -- the legacy QIDI Code prefix (built by the shell layer).
...
use cf_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
pub enum UserMessageTemplate {
    /// Legacy QIDI Code prefix: `<user_info>` + optional `<git_status>`.
...
/// One discovered rule file (AGENTS.md / Claude.md / .qidi/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `cf_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// User-scoped rule files (~/.qidi/, ~/.claude/).
    ...
    /// (`~/.qidi/projects/<encoded-cwd>/mcps`). Surfaced in
