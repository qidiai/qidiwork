...
//! primitive consumed cross-crate by `qidi-code`, not agent-definition
//! discovery.
...
/// code-exec markers (`.mcp.json`, `.qidi/config.toml`, `.claude/settings.json`,
...
/// from `qidi-code`, which depends on THIS crate) to keep the dep edge
