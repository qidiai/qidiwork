...
//!
//! This crate provides the common logic used by `qidi-code` and
//! `qidi-code` for handling announcements (banner notifications).
...
// Types
// ─────────────────────────────────────────────────────────────────────────────
// RemoteAnnouncement is defined in cf-config and re-exported here for compatibility.
pub use cf_config::types::RemoteAnnouncement;


// AnnouncementCta is defined in cf-config and re-exported here for client crates.
pub use cf_config::types::AnnouncementCta;
...
/// Read hidden announcement ids from `~/.qidi/announcements.json`.
...
/// Write hidden announcement ids to `~/.qidi/announcements.json`.
...
fn announcements_state_path() -> PathBuf {
    cf_config::grok_home().join("announcements.json")
}
...
///
/// Precedence: `QIDI_ANNOUNCEMENTS_OVERRIDE` env var (JSON override) → remote announcements.
...
pub fn resolve_startup(
    ...
    if let Ok(raw) = std::env::var("QIDI_ANNOUNCEMENTS_OVERRIDE") {
        match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
            Ok(list) => return Some(list),
            Err(e) => {
                tracing::warn!(error = %e, "invalid QIDI_ANNOUNCEMENTS_OVERRIDE JSON; ignoring override");
            }
        }
    }
...
mod tests {
    ...
    fn resolve_startup_env_override() {
        ...
        unsafe {
            std::env::set_var("QIDI_ANNOUNCEMENTS_OVERRIDE", r#"[{"id":"test"}]"#);
        }
        ...
        // SAFETY: test-only
        unsafe {
            std::env::remove_var("QIDI_ANNOUNCEMENTS_OVERRIDE");
        }
    }
