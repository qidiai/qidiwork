...
pub fn estimate_tool_definition_tokens(td: &cf_sampling_types::ToolDefinition) -> u64 {
    ...
    ((name_len + desc_len + params_len) as u64) / cf_token_estimation::BYTES_PER_TOKEN
}

/// Sum [`estimate_tool_definition_tokens`] across a slice.
pub fn estimate_tool_definitions_tokens(tds: &[cf_sampling_types::ToolDefinition]) -> u64 {
...
/// constant defined by [`cf_token_estimation::IMAGE_TOKEN_ESTIMATE`].
pub fn estimate_conversation_tokens(items: &[ConversationItem]) -> u64 {
    items.iter().map(estimate_item_tokens).sum()
}

/// cf-tools's [`ItemTokenCounter`](cf_compaction::ItemTokenCounter)
/// for the shared compaction engine: the bytes/4 estimate cf-tools already
...
/// cf-tools estimates instead, reusing [`estimate_item_tokens`] so the
/// per-variant arithmetic (images, reasoning blobs, tool-call args) stays in
/// one place.
pub struct EstimatedItemTokenCounter;

impl cf_compaction::ItemTokenCounter<ConversationItem> for EstimatedItemTokenCounter {
...
mod tests {
    ...
    fn estimated_item_token_counter_matches_estimate_item_tokens() {
        use cf_compaction::ItemTokenCounter;
    ...
    fn estimate_tool_definition_tokens_counts_name_desc_params() {
        ...
        let td = cf_sampling_types::ToolDefinition::function(
            "search",
            Some("find a file"),
            serde_json::json!({}),
        );
    ...
    fn estimate_tool_definitions_tokens_sums_across_slice() {
        let a = cf_sampling_types::ToolDefinition::function(
            "a",
            None::<&str>,
            serde_json::json!({}),
        );
        let b = cf_sampling_types::ToolDefinition::function(
            "b",
            None::<&str>,
            serde_json::json!({}),
        );
