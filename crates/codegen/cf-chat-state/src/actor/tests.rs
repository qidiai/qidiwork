...
async fn record_last_turn_usage_round_trip() {
    use cf_sampling_types::TokenUsage;
...
async fn build_request_with_tool_definitions() {
    use cf_sampling_types::ToolSpec;
...
/// Simulates the exact sequence that `qidi-code`'s `execute_tool_calls`
...
async fn parallel_tool_calls_accept_first_reject_second_skip_third() {
    ...
    let assistant_with_tools =
        ConversationItem::Assistant(cf_sampling_types::AssistantItem {
...
async fn get_first_user_text_image_first_returns_none() {
    use cf_sampling_types::{ContentPart, UserItem};
...
async fn sampling_config_survives_compaction_replacement() {
    use cf_sampling_types::ApiBackend;
...
/// If that model slug is e.g. "cf-tools" and not in the ModelState
...
async fn model_metadata_lost_after_compaction_then_recovered_on_next_turn() {
    ...
    h.handle
        .push_assistant_response(ConversationItem::Assistant(
            cf_sampling_types::AssistantItem {
...
fn serialize_via_public_api(
    req: &cf_sampling_types::ConversationRequest,
) -> serde_json::Value {
    ...
    cf_sampling_types::patch_reasoning_text_types(&mut body);
...
fn assert_prefix_stable_pair(
    base: &cf_sampling_types::ConversationRequest,
    extended: &cf_sampling_types::ConversationRequest,
    label: &str,
) {
...
async fn prefix_stable_after_image_pruning() {
    ...
    let h = TestHarness::with_conversation(vec![
        ConversationItem::system("sys"),
        ConversationItem::User(cf_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
    ...
    h.handle
        .push_assistant_response(ConversationItem::assistant("a2"));
    h.handle
        .push_user_message(ConversationItem::User(cf_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
                    text: "new image".into(),
                },
...
async fn prefix_stable_with_backend_tool_calls() {
    use cf_sampling_types::{BackendToolCallItem, BackendToolKind, rs};
