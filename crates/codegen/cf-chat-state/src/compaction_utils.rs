...
//! this crate and `qidi-code` can share them without duplication.
use std::collections::BTreeSet;
use cf_sampling_types::{ContentPart, ConversationItem, ToolResultItem};
...
///
/// Stored here (rather than only in `qidi-code`) so that query-extraction
...
/// `ActiveSubagentSummary` in qidi-code. The mapping between them
/// happens in `run_compact_inner()` (qidi-code).
#[derive(Clone)]
...
/// A dependency-free mirror of `TodoStatus` (qidi-code), kept here so
...
impl TodoSummaryStatus {
    ...
    /// Mirrors `TodoStatus::tag()` in qidi-code.
...
/// handled by the consumer (e.g. `qidi-code`), which has access to
...
impl CompactionStateContext {
    ...
    /// model to re-read the same files. cf-tools retains
...
/// `xai-chat-state` and `qidi-code` share the same format.
...
pub struct CompactedHistoryInput<'a> {
    ...
    /// When `false` (the default), recent messages come first (cf-tools
    /// ordering).
    ...
    /// carrier and the cf-tools summary.
    ...
    /// "Total summaries generated so far …" footer. Ignored by the cf-tools
...
/// `run_compact_inner` in `qidi-code` assembles inline, but is
...
pub fn repair_history(items: &mut Vec<ConversationItem>) -> HistoryRepairReport {
    let duplicates_removed = cf_sampling_types::dedup_duplicate_tool_results(items);
    let stripped_tool_result_ids = strip_displaced_tool_results(items);
    let synthetic_results_inserted = cf_sampling_types::repair_dangling_tool_calls(
        items,
        cf_sampling_types::DanglingToolCallReason::HarnessHalted {
...
mod tests {
    use super::*;
    use cf_sampling_types::SyntheticReason;
    #[test]
...
actual user question";
    ...
    fn call(id: &str) -> cf_sampling_types::ToolCall {
        cf_sampling_types::ToolCall {
            id: id.into(),
            name: "read_file".to_string(),
            arguments: "{}".into(),
        }
    ...
    async fn build_compacted_history_transcript_hint() {
        ...
        let summary = build_compacted_history(input(Some(
            "/home/user/.qidi/sessions/abc/updates.jsonl".to_string(),
        )))
        .last()
        .unwrap()
        .text_content();
        assert!(summary.contains("/home/user/.qidi/sessions/abc/updates.jsonl"));
    ...
    ///
    /// Simulates the exact conversation shape produced by qidi-code:
    ///
    ...
    async fn build_compacted_history_multi_turn_with_parallel_tool_calls() {
        use cf_sampling_types::{AssistantItem, ToolCall};
        let conversation = vec![
...
The user asked to read main.rs and lib.rs. main.rs prints hello world, lib.rs has an add function. The user then asked to fix a typo in main.rs and run tests. The typo was fixed and tests passed.";
    ...
    fn conversation_item_preserves_reasoning_siblings() {
        use cf_sampling_types::{AssistantItem, rs};
        let result = strip_tool_messages_for_conversation_item(vec![
            ConversationItem::system("system"),
    ...
    fn prepare_for_summarization_drops_reasoning_sibling_on_mutated_assistant() {
        use cf_sampling_types::{AssistantItem, ToolCall, rs};
        let mk_reasoning = || {
            ConversationItem::Reasoning(rs::ReasoningItem {
                id: "r_123".to_string(),
                summary: vec![rs::SummaryPart::SummaryText(rs::SummaryTextContent {
    ...
    fn verbatim_reasoning_kept_unless_messages_backend() {
        use cf_sampling_types::{ToolCall, rs};
        let mk = || {
            vec![
                ConversationItem::system("sys"),
                ConversationItem::Reasoning(rs::ReasoningItem {
