...
/// `.qidi/<filename>` (the cwd-fallback would silently promote an untrusted
...
impl ConfigLayers {
    ...
    /// wraps this with the `QIDI_CAMPAIGNS_OVERRIDE` env layer.
...
/// `QIDI_CAMPAIGNS=0` or `[features] campaigns = false` on pre-campaign base.
pub fn campaigns_application_disabled(base_effective: &toml::Value) -> bool {
    if crate::env_bool("QIDI_CAMPAIGNS") == Some(false) {
        return true;
    }
    base_effective
        .get("features")
...
/// (`cf_shell::util::config::load_effective_config`) when remote campaigns
/// or `QIDI_CAMPAIGNS_OVERRIDE` must be honored. The name mirrors the
...
/// Path to `$QIDI_HOME/campaigns_state.json` under `home`.
pub fn campaigns_state_path(home: &std::path::Path) -> std::path::PathBuf {
    home.join(CAMPAIGNS_STATE_FILE)
}

/// Fail-open dismissed ids from `$QIDI_HOME/campaigns_state.json`.
...
/// can't be parsed (broken `QIDI_TEST_VERSION` in dev), silently strips
...
pub fn apply_version_overrides_with_registered(value: &mut toml::Value) -> std::io::Result<()> {
    match cf_version::installed_semver() {
...
mod tests {
    ...
    /// `QIDI_CAMPAIGNS=0` disables campaign application regardless of config.
    /// `QIDI_CAMPAIGNS` is process-global, so this test serializes itself with a
    ...
    fn kill_switch_env_var_disables() {
        ...
        let prior = std::env::var_os("QIDI_CAMPAIGNS");
        let empty = toml::Value::Table(Default::default());

        // SAFETY: ENV_GUARD serializes this against itself; no other test in the
        // crate mutates or reads QIDI_CAMPAIGNS concurrently.
        unsafe { std::env::set_var("QIDI_CAMPAIGNS", "0") };
        assert!(campaigns_application_disabled(&empty));

        unsafe { std::env::remove_var("QIDI_CAMPAIGNS") };
        assert!(!campaigns_application_disabled(&empty));

        match prior {
            Some(v) => unsafe { std::env::set_var("QIDI_CAMPAIGNS", v) },
            None => unsafe { std::env::remove_var("QIDI_CAMPAIGNS") },
        }
    }
}
