...
mod tests {
    ...
    fn forced_payload_is_not_env_expanded() {
        ...
        let prior = std::env::var("QIDI_MDM_NO_EXPAND_TEST").ok();
        unsafe { std::env::set_var("QIDI_MDM_NO_EXPAND_TEST", "attacker") };
        let decoded = decode_managed_toml(&b64("base_url = \"${QIDI_MDM_NO_EXPAND_TEST}/v1\"\n"));
        unsafe {
            match prior {
                Some(p) => std::env::set_var("QIDI_MDM_NO_EXPAND_TEST", p),
                None => std::env::remove_var("QIDI_MDM_NO_EXPAND_TEST"),
            }
        }
        assert_eq!(
            decoded
                ...
                .and_then(|v| v.as_str()),
            Some("${QIDI_MDM_NO_EXPAND_TEST}/v1"),
