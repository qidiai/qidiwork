...
//! C++/C#/.NET builds.
//!
//! Set `QIDI_SHELL` to override auto-detection: `pwsh`, `powershell`,
...
///
/// If `QIDI_SHELL` is set, it takes precedence over auto-detection.
...
pub fn detect_windows_shell() -> &'static WindowsShell {
    ...
    CACHED.get_or_init(|| {
        // Explicit override via QIDI_SHELL.
        if let Ok(val) = std::env::var("QIDI_SHELL") {
            match val.trim().to_ascii_lowercase().as_str() {
                "pwsh" => {
                    tracing::info!("Windows shell (QIDI_SHELL override): pwsh");
                    return WindowsShell::Pwsh;
                }
                "powershell" => {
                    tracing::info!("Windows shell (QIDI_SHELL override): powershell.exe");
                ...
                "bash" | "gitbash" | "git-bash" => {
                    if let Some(path) = find_git_bash() {
                        tracing::info!(
                            shell = path,
                            "Windows shell (QIDI_SHELL override): Git Bash"
                        );
                        return WindowsShell::GitBash(path);
                    }
                    tracing::warn!(
                        "QIDI_SHELL={val} but Git Bash not found; falling through to auto-detect"
                    );
                }
                "cmd" | "cmd.exe" => {
                    tracing::info!("Windows shell (QIDI_SHELL override): cmd.exe");
                    return WindowsShell::Cmd;
                }
                other => {
                    tracing::warn!(
                        "QIDI_SHELL={other} is not recognized \
...
//
//   1. `$QIDI_SHELL` override, if it names the requested kind and is runnable.
...
fn resolve_unix_shell_path(kind: UnixShellKind) -> String {
    ...
    // 1) Explicit override via $QIDI_SHELL.
    if let Ok(s) = std::env::var("QIDI_SHELL") {
...
///
/// The probe is spawned via `cf_tty_utils::detach_std_command` so that
...
fn is_executable(path: &std::path::Path) -> bool {
    ...
    // Nix fallback. Detach from the controlling TTY via cf_tty_utils so
