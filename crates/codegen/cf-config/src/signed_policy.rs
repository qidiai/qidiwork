...
// SECURITY (B4): this fork ships an empty trusted-key set on purpose. QIDI Code
// has no provisioned signing keypair, so signature verification stays inert:
// [`verification_active`] returns false and the on-disk sidecar cache marker
// remains the (best-effort) authority. To turn verification on, populate this
// table with a real 32-byte Ed25519 public key; it is keyed by `key_id`, so keys
// can rotate without touching anything but this list.
