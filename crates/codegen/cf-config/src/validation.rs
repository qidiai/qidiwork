...
/// [`fail_closed_flag_from_str`]. Env tightening (file vs `QIDI_MANAGED_CONFIG_FAIL_CLOSED`)
...
/// with `QIDI_MANAGED_CONFIG_URL`; only applies to `requirements.toml`.
pub(crate) const FAIL_CLOSED_ENV: &str = "QIDI_MANAGED_CONFIG_FAIL_CLOSED";
...
pub struct RequirementsLayer {
    ...
    /// not re-derive from the source (`QIDI_HOME`-influenced, could carry `..`).
    pub is_system: bool,
}
...
fn validate_requirements_value(
    ...
    let Ok(version) = cf_version::installed_semver() else {
        return Ok(());
    };
...
/// no-op (no cwd-relative `.qidi/requirements.toml` is read or enforced).
