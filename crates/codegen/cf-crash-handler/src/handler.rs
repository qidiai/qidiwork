...
mod imp {
    ...
    pub fn install(crash_dir: &Path, qidi_version: &str) -> bool {
        ...
        unsafe {
            ...
            version.fill(0);
            let copy_len = qidi_version.len().min(format::VERSION_STRING_LEN);
            version[..copy_len].copy_from_slice(&qidi_version.as_bytes()[..copy_len]);
        }
