...
fn subprocess_entry() {
    ...
    let config = cf_crash_handler::CrashHandlerConfig {
        app_version: "0.0.0-test".to_string(),
        crash_dir,
    };
    cf_crash_handler::install(config);
...
fn sigbus_produces_valid_crash_blob() {
    ...
    let blob = cf_crash_handler::format::CrashBlob::parse(&data).expect("crash blob should parse");
...
fn clean_exit_does_not_produce_crash_report() {
    ...
    let report = cf_crash_handler::check_previous_crash(tmp.path());
