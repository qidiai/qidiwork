...
impl WorktreeDb {
    ...
    /// Open the default DB at `~/.qidi/worktrees.db`.
    ///
    /// Discovers grok home via `$QIDI_HOME`, falling back to the canonicalized
    /// `$HOME/.grok` (matching `cf_config::grok_home`).
...
pub fn resolve_grok_home() -> Result<PathBuf> {
    if let Ok(v) = std::env::var("QIDI_HOME") {
        return Ok(PathBuf::from(v));
    }
    let home = PathBuf::from(std::env::var("HOME").context("neither $QIDI_HOME nor $HOME is set")?);
...
#[cfg(test)]
static QIDI_HOME_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// Test-only isolation for code that resolves the DB via `open_default()`.
///
/// Holds [`QIDI_HOME_ENV_LOCK`] (serializing concurrent setters), points
/// `QIDI_HOME` at a fresh private tmp dir, and restores the prior value on drop.
/// Use instead of hand-rolling the lock + restore guard + tmp dir per test.
///
/// `Drop` restores `QIDI_HOME` before `_lock` releases, so the env is correct
...
impl GrokHomeFixture {
    pub(crate) fn new() -> Self {
        let lock = QIDI_HOME_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let tmp = tempfile::TempDir::new().unwrap();
        let home = tmp.path().join("grok-home");
        std::fs::create_dir_all(&home).unwrap();
        // Warm up the DB (journal-mode conversion + schema) before exposing it
        // via QIDI_HOME, sparing the test hot loop set_journal_mode's retry
        // sleeps. This open has exclusive access (nothing reaches the path
        // until QIDI_HOME points here); set_journal_mode's retry is the actual
        // race fix.
        let _ = WorktreeDb::open(&home);
        let prev = std::env::var_os("QIDI_HOME");
        unsafe { std::env::set_var("QIDI_HOME", &home) };
        Self {
            _lock: lock,
            prev,
            home,
    ...
    fn drop(&mut self) {
        unsafe {
            match self.prev.take() {
                Some(p) => std::env::set_var("QIDI_HOME", p),
                None => std::env::remove_var("QIDI_HOME"),
            }
        }
    }
}
