...
pub enum Event {
    ...
    /// `TODO_GATE_*` discriminator constant in `qidi-code::session::events`.
    TodoGateFired {
        fires: u32,
        pending: usize,
        in_progress: usize,
    ...
    /// constants in `qidi-code::session::events`.
    ...
    /// discriminator constants in `qidi-code::session::events`.
    ...
    /// `qidi-code::session::events`. Fail-open never pauses the goal.
    ...
    /// enumerated by
    /// `qidi-code::session::goal_stop_detector::PATTERN_LABELS`;
...
/// Telemetry mirror of `qidi-code`'s `GoalClassifierVerdict`. Two
/// crates due to the orphan rule; the conversion lives in
/// `qidi-code/src/session/events.rs`.
#[derive(Debug, Clone, Copy, Serialize)]
#[serde(rename_all = "snake_case")]
...
/// Telemetry mirror of `qidi-code`'s `GoalPauseReason`. The two types
