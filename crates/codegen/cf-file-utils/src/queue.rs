...
use cf_circuit_breaker::{Disposition, RetryPolicy};
...
/// Resolve a `QIDI_UPLOAD_QUEUE_AUTH_PROBE_SECS` override (seconds) into a probe
...
/// `session_id`, no GCS path). Read by `cf_workspace::recovery`.
...
impl UploadQueue {
    ...
    pub fn spawn_with_concurrency(
        ...
        if let Some(raw_secs) = std::env::var("QIDI_UPLOAD_QUEUE_AUTH_PROBE_SECS")
            .ok()
            .and_then(|s| s.parse::<u64>().ok())
        {
            match auth_park_probe_override(raw_secs) {
                ...
                None => {
                    tracing::warn!(
                        "Ignoring QIDI_UPLOAD_QUEUE_AUTH_PROBE_SECS={raw_secs}: a zero probe \
        ...
        let max_queue_bytes = std::env::var("QIDI_UPLOAD_QUEUE_MAX_BYTES")
            .ok()
