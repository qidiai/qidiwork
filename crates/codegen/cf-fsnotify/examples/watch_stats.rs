...
//! startup latency, under either strategy (`QIDI_FSNOTIFY_PER_DIR=0|1`).
//!
//! ```bash
...
//! QIDI_FSNOTIFY_PER_DIR=0 cargo run --release -p xai-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! QIDI_FSNOTIFY_PER_DIR=1 cargo run --release -p xai-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! ```
//!
...
use cf_fsnotify::{FsConfig, FsEventSource};
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("run") => {
            ...
            let strategy = std::env::var("QIDI_FSNOTIFY_PER_DIR").unwrap_or_default();
