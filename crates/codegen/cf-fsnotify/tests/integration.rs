...
use cf_fsnotify::{FsConfig, FsEvent, FsEventKind, FsEventSource};
...
async fn shared_dedupes_by_directory() {
    ...
    let Ok(a) = cf_fsnotify::shared(path.clone(), FsConfig::default()) else {
        eprintln!("skipping: OS watcher unavailable (resource limit?)");
        return;
    };
    let before = cf_fsnotify::stats();
    let b = cf_fsnotify::shared(path.clone(), FsConfig::default()).unwrap();
    ...
    let c = cf_fsnotify::shared(other.path().to_path_buf(), FsConfig::default()).unwrap();
    assert!(!Arc::ptr_eq(&a, &c), "different dirs must not share");
    assert_eq!(
        cf_fsnotify::stats().created_total - after.created_total,
        1,
    ...
    drop(a);
    drop(b);
    let d = cf_fsnotify::shared(path, FsConfig::default()).unwrap();
    assert_eq!(
        Arc::strong_count(&d),
        1,
...
async fn shared_watcher_scaling_demo() {
    ...
    for _ in 0..SESSIONS {
        let Ok(src) = cf_fsnotify::shared(path.clone(), FsConfig::default()) else {
            eprintln!("skipping: OS watcher unavailable (resource limit?)");
            return;
        };
        handles.push(src);
