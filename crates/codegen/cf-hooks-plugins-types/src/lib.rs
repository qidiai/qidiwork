...
//! `qidi-code` and `qidi-code` can depend on it without pulling
//! in domain logic.
//!
...
/// Plugin scope.
///
/// Maps from `PluginScope` in `qidi-code`. Variant renames:
...
///
/// Maps from `PluginOrigin` in `qidi-code`. Optional on [`PluginInfo`]
...
pub struct HookInfo {
    ...
    /// Whether this hook is disabled via ~/.qidi/disabled-hooks.
    #[serde(default)]
    pub disabled: bool,
}
...
mod tests {
    ...
    fn hooks_action_serde_roundtrip() {
        let action = HooksAction::Add {
            path: "/home/user/.qidi/hooks".into(),
        };
    ...
    fn hook_info_camel_case_fields() {
        let hook = HookInfo {
            ...
            url: None,
            timeout_ms: 5000,
            source_dir: "/home/user/.qidi/hooks".into(),
            disabled: false,
        };
    ...
    fn plugin_info_camel_case_fields() {
        let plugin = PluginInfo {
            ...
            root: "/home/user/.qidi/plugins/test-plugin".into(),
