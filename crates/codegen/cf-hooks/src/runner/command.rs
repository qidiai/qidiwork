...
pub async fn run_command_hook(
    ...
    let debug_payloads = std::env::var("QIDI_HOOK_DEBUG").is_ok_and(|v| v == "1");
    if debug_payloads {
        tracing::trace!(
            hook_name = %spec.name,
    ...
    // `cf_tools::util::detach_command`: Unix uses the same setsid /
    ...
    // plugin) can spoof `QIDI_HOOK_EVENT`, `QIDI_HOOK_NAME`, `QIDI_SESSION_ID`,
    // `QIDI_WORKSPACE_ROOT`, or `CLAUDE_PROJECT_DIR` -- which are the
    ...
    let mut child = match cmd
        ...
        .env("QIDI_HOOK_EVENT", envelope.hook_event_name.to_string())
        .env("QIDI_HOOK_NAME", &spec.name)
        .env("QIDI_SESSION_ID", ctx.session_id)
        .env("QIDI_WORKSPACE_ROOT", ctx.workspace_root)
...
mod tests {
    ...
    fn resolve_relative_path() {
        let spec = HookSpec {
            ...
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::path::PathBuf::from("/project/.qidi/hooks"),
            extra_env: std::collections::HashMap::new(),
        };
        assert_eq!(
            resolve_command_path(&spec),
            Some(std::path::PathBuf::from(
                "/project/.qidi/hooks/bin/check.sh"
            ))
        );
    }
    ...
    fn find_unresolved_skips_runner_vars() {
        ...
        let v = find_unresolved_env_vars(
            "${QIDI_HOOK_EVENT}/${CLAUDE_PROJECT_DIR}/${QIDI_SESSION_ID}/foo",
            &env,
        );
        assert!(
            v.is_empty(),
    ...
    async fn test_tilde_expansion_runs_via_shell() {
        ...
        // Inject HOME via extra_env so `sh -c "~/.qidi-test-hooks-gb856/..."`
        ...
        let spec = HookSpec {
            ...
            enabled: true,
            command: Some(std::path::PathBuf::from(
                "~/.qidi-test-hooks-gb856/tilde-test.sh",
            )),
            command_raw: Some("~/.qidi-test-hooks-gb856/tilde-test.sh".to_string()),
            url: None,
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::env::temp_dir(),
