...
// (`~/.qidi/trusted_folders.toml`) is the single authority for whether a repo's
...
pub fn legacy_trust_file_path() -> Option<PathBuf> {
    Some(cf_config::user_grok_home()?.join("trusted-hook-projects"))
}
...
pub fn disable_hook(hook_name: &str) -> Result<(), String> {
    let file = disabled_hooks_file_path()
        .ok_or_else(|| "no user grok home (set $QIDI_HOME or $HOME)".to_string())?;
...
/// Returns the path to `$QIDI_HOME/disabled-hooks`, or `None` when no user grok
/// home resolves.
fn disabled_hooks_file_path() -> Option<PathBuf> {
    Some(cf_config::user_grok_home()?.join("disabled-hooks"))
}
