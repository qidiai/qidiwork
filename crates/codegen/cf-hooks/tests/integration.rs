...
use cf_hooks::discovery::load_hooks;
use cf_hooks::dispatcher;
use cf_hooks::event::*;
use cf_hooks::result::HookDecision;
use cf_hooks::runner::RunContext;
...
async fn non_blocking_dispatch() {
    ...
    assert!(matches!(
        &results[0],
        cf_hooks::result::HookRunResult::Success { .. }
    ));
}
...
async fn hook_receives_env_vars() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$QIDI_HOOK_EVENT" > {f}; echo "NAME=$QIDI_HOOK_NAME" >> {f}; echo "SESSION=$QIDI_SESSION_ID" >> {f}; echo '{{"decision":"allow"}}'"#,
...
/// runner-reserved keys (`QIDI_HOOK_EVENT`, `QIDI_HOOK_NAME`,
/// `QIDI_SESSION_ID`, `QIDI_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$QIDI_HOOK_EVENT" > {f}; echo "NAME=$QIDI_HOOK_NAME" >> {f}; echo "SESSION=$QIDI_SESSION_ID" >> {f}; echo "ROOT=$QIDI_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
        f = output_file.display(),
    );
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "QIDI_HOOK_EVENT": "spoofed_event",
                                "QIDI_HOOK_NAME": "spoofed_name",
                                "QIDI_SESSION_ID": "spoofed_session",
                                "QIDI_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "QIDI_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed QIDI_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "QIDI_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed QIDI_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "QIDI_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed QIDI_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
...
async fn http_hook_url_env_expansion_end_to_end() {
    ...
    use cf_hooks::result::HookRunResult;
    let info = match run {
