...
//! `cf_sampler::shared_http` (one HTTP/2 pooled client plus
//! a pool-less HTTP/1.1 fallback shared across every
//! `SamplingClient`). The sampler reads `QIDI_POOL_*` /
//! `QIDI_CONNECT_TIMEOUT_SECS` once, when its shared client is
//! first built, and `QIDI_SAMPLER_SHARED_CLIENT=0` falls back to
...
// it without taking a circular dependency on `qidi-code`. Re-exported
...
// the same type via `cf_sampler::OriginClientInfo`. The shell-specific
...
/// `QIDI_CLIENT_VERSION` env vars. Returns `None` when
/// `QIDI_CLIENT_NAME` is unset.
...
/// Process-level client identifier (`QIDI_CLIENT_NAME` env var, default `"grok-shell"`).
pub fn process_client_identifier() -> String {
    std::env::var("QIDI_CLIENT_NAME").unwrap_or_else(|_| "grok-shell".to_string())
}
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn cf_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(cf_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
