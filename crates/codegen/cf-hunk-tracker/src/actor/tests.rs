...
impl TestHarness {
    ...
    fn populate(&self, files: usize, files_per_dir: usize) {
        cf_test_utils::git::write_fanout_tree(&self.working_dir, files, files_per_dir);
    ...
    fn feature_branch(&self, picks: usize) -> String {
        cf_test_utils::git::make_feature_branch(&self.working_dir, picks)
    }
}
...
/// never wall-clock.
///
/// Knobs: QIDI_PERF_GIT_FILES (default 5000), QIDI_PERF_GIT_PICKS (default 10).
#[tokio::test]
#[ignore = "perf repro; run with --ignored --nocapture"]
async fn refresh_storm_scan_count_during_rebase() {
    let files = env_usize("QIDI_PERF_GIT_FILES", 5000);
    let picks = env_usize("QIDI_PERF_GIT_PICKS", 10);
    ...
    let t = Instant::now();
    cf_test_utils::git::run_git_with_env(
        &wd,
        &["rebase", "-i", &base],
