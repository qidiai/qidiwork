//! Headless markdown analysis sharing QIDI Code's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `qidi-code` so analysis matches what Grok
...
/// The exact `pulldown-cmark` option set QIDI Code uses to render markdown.
///
...
/// Parse `text` with QIDI Code's options; count elements and flag structural issues.
