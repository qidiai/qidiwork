...
//
// Layer 1 (cross-process): filesystem lock at $QIDI_HOME/mcp_auth_{safe_name}.lock
...
fn auth_lock_path(server_name: &str) -> std::path::PathBuf {
    let safe: String = server_name
        ...
        .collect();
    cf_config::grok_home().join(format!("mcp_auth_{safe}.lock"))
}
...
/// # CSRF (B1)
///
/// This server does **not** validate the `state` value itself; it only relays
/// the received `(code, state)` pair back to the caller. CSRF protection is
/// enforced downstream by rmcp's `AuthorizationManager`: the `state` is minted
/// by [`get_authorization_url`](rmcp) via a random `CsrfToken` and stashed in
/// the manager's state store, and `exchange_code_for_token` looks the returned
/// `state` back up (single-use) and refuses the exchange if it is absent or
/// unknown. A forged callback therefore cannot complete a token exchange even
/// though it can reach this handler. Two local hardening properties this server
/// does own: the callback route is registered `GET`-only (a `POST` is answered
/// `405`, never routed into the handler), and a callback missing `code`/`state`
/// is surfaced as an error rather than forwarded as a partial success.
#[allow(clippy::type_complexity)]
fn start_oauth_callback_server(
    ...
    let app = Router::new().route("/callback", get(handler));
...
#[cfg(test)]
mod tests {
    //! B1 defensive tests for the loopback OAuth callback server. CSRF `state`
    //! validation proper is owned by rmcp's `AuthorizationManager` (see the
    //! doc comment on [`start_oauth_callback_server`]); these tests pin the two
    //! local guarantees this server does own — GET-only routing and rejecting a
    //! callback that is missing its `code`/`state` instead of forwarding a
    //! partial success.
    use super::start_oauth_callback_server;

    /// Bind a loopback listener on an ephemeral port and start the callback
    /// server on it. Returns `(base_url, abort_handle, callback_receiver)`.
    #[allow(clippy::type_complexity)]
    async fn spawn_server() -> (
        String,
        tokio::task::JoinHandle<()>,
        tokio::sync::oneshot::Receiver<Result<(String, String), String>>,
    ) {
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0")
            .await
            .expect("bind loopback listener");
        let addr = listener.local_addr().expect("local_addr");
        let (handle, rx) = start_oauth_callback_server(listener);
        (format!("http://{addr}"), handle, rx)
    }

    #[tokio::test]
    async fn callback_relays_code_and_state() {
        let (base, handle, rx) = spawn_server().await;
        let resp = reqwest::get(format!("{base}/callback?code=the-code&state=the-state"))
            .await
            .expect("GET callback");
        assert!(resp.status().is_success());
        let received = rx.await.expect("receiver").expect("callback ok");
        assert_eq!(received, ("the-code".to_string(), "the-state".to_string()));
        handle.abort();
    }

    #[tokio::test]
    async fn callback_missing_state_is_rejected() {
        let (base, handle, rx) = spawn_server().await;
        let _ = reqwest::get(format!("{base}/callback?code=only-code"))
            .await
            .expect("GET callback");
        let err = rx.await.expect("receiver").expect_err("missing state must error");
        assert!(err.contains("Missing state"), "unexpected error: {err}");
        handle.abort();
    }

    #[tokio::test]
    async fn callback_oauth_error_is_surfaced() {
        let (base, handle, rx) = spawn_server().await;
        let _ = reqwest::get(format!(
            "{base}/callback?error=access_denied&error_description=nope"
        ))
        .await
        .expect("GET callback");
        let err = rx.await.expect("receiver").expect_err("oauth error must error");
        assert!(err.contains("access_denied"), "unexpected error: {err}");
        handle.abort();
    }

    #[tokio::test]
    async fn callback_route_is_get_only() {
        let (base, handle, _rx) = spawn_server().await;
        let resp = reqwest::Client::new()
            .post(format!("{base}/callback?code=c&state=s"))
            .send()
            .await
        ...
        assert_eq!(
            ...
            "GET-only route must reject POST"
        );
        handle.abort();
    }
}
