...
use cf_tools::util::ProcessGroup;

/// MCP tool name delimiter: server names are qualified as `"server__tool"`.
/// Canonical definition lives in `cf_workspace_types`; re-exported here
/// for callers that historically imported it from this module.
pub use cf_workspace_types::MCP_TOOL_NAME_DELIMITER;
...
pub struct McpState {
    ...
    /// Persisted to `~/.qidi/config.toml` under `[mcp_servers.<name>].disabled_tools`.
    ...
    event_writer: cf_file_utils::events::EventWriter,
...
impl McpState {
    ...
    pub fn new_with_meta(configs: Vec<acp::McpServer>, meta_config_map: McpMetaConfigMap) -> Self {
        Self {
            ...
            event_writer: cf_file_utils::events::EventWriter::noop(),
    ...
    pub fn set_event_writer(&mut self, writer: cf_file_utils::events::EventWriter) {
        self.event_writer = writer;
    }

    pub fn event_writer(&self) -> &cf_file_utils::events::EventWriter {
        &self.event_writer
    }
...
pub use cf_telemetry::enums::McpInitStrategy;
...
impl McpError {
    ...
    pub fn error_category(&self) -> cf_file_utils::events::McpErrorCategory {
        use cf_file_utils::events::McpErrorCategory;
        match self {
...
/// `cf_tool_runtime::Tool` directly instead of going through typed wrappers.
...
impl cf_tool_runtime::Tool for McpErasedTool {
    ...
    fn id(&self) -> cf_tool_protocol::ToolId {
        ...
        cf_tool_protocol::ToolId::new(&qualified)
            .unwrap_or_else(|_| cf_tool_protocol::ToolId::new("mcp_tool").expect("valid"))
    }
    ...
    fn description(
        ...
        cf_tool_types::ToolDescription::new(&self.tool.name, &self.tool.description)
    }
    ...
    async fn run(
        ...
        event_writer.emit(cf_file_utils::events::Event::McpToolCallStarted {
            ...
            Err(first_err) if client.has_auth() && self.tool.read_only => {
                ...
                if reauth_ok {
                    self.try_call_tool(&client, &raw, &mut reconnect_attempted, &mut is_timeout, ew)
                        .await
                        .map_err(|e| {
                            cf_tool_runtime::ToolError::custom("process_manager", e.to_string())
                        })
                } else {
                    Err(first_err)
                }
        ...
        let call_result = match dispatch_result {
            Ok(result) => result,
            Err(e) => {
                ew.emit(cf_file_utils::events::Event::McpToolCallCompleted {
                    server_name: server.clone(),
                    tool_name: tool.clone(),
        ...
        event_writer.emit(cf_file_utils::events::Event::McpToolCallCompleted {
            server_name: server.clone(),
            tool_name: tool.clone(),
            call_id: qualified_name.clone(),
            duration_ms,
        ...
        cf_telemetry::session_ctx::log_event(cf_telemetry::events::McpToolCalled {
            server_name: server.clone(),
            tool_name: tool.clone(),
            qualified_name,
            success,
...
impl McpErasedTool {
    async fn try_call_tool(
        ...
        is_timeout: &mut bool,
        ew: &cf_file_utils::events::EventWriter,
    ) -> Result<rmcp::model::CallToolResult, cf_tool_runtime::ToolError> {
    ...
    async fn recover_and_retry(
        ...
        *reconnect_attempted = true;
        ...
        ew.emit(cf_file_utils::events::Event::McpTransportError {
        ...
        let mcp_service = match client.recover().await {
            Ok(service) => {
                ew.emit(cf_file_utils::events::Event::McpTransportReconnect {
                    server_name: self.tool.server_name.clone(),
                    success: true,
                    error: None,
                });
        ...
        match tokio::time::timeout(timeout_duration, mcp_service.call_tool(params)).await {
            Ok(Ok(call_result)) => Ok(call_result),
            Ok(Err(retry_err)) => Err(cf_tool_runtime::ToolError::custom(
                "process_manager",
                retry_err.to_string(),
            )),
            Err(_) => {
    ...
    W: AsyncWrite + Send + Unpin + 'static,
{
    ...
    fn record_decode_error(&self, line: &[u8], err: &serde_json::Error) {
        ...
        self.event_writer
            .emit(cf_file_utils::events::Event::McpTransportDecodeError {
...
impl McpClient {
    ...
    pub async fn ensure_initialized(&self) -> Result<McpService, McpError> {
        ...
        tracing::info!(target: cf_telemetry::instrumentation::TARGET, event = "timing", name = "mcp_try_handshake", elapsed_us = handshake_elapsed);
    ...
    pub async fn get_tool_registrations(
        ...
        let _ensure_init_timer =
            cf_telemetry::instrumentation::timer("mcp_ensure_initialized");
        let mcp_service = self.ensure_initialized().await?;

        let mut all_tools = Vec::new();
        let mut cursor: Option<String> = None;

        let _list_tools_timer = cf_telemetry::instrumentation::timer("mcp_list_tools");
        loop {
...
pub async fn start_mcp_server(
    ...
    let _per_server_timer = cf_telemetry::instrumentation::timer("mcp_start_one_server");
    match mcp_server {
        acp::McpServer::Stdio(acp::McpServerStdio {
            ...
            let _stdio_spawn_timer = cf_telemetry::instrumentation::timer("mcp_stdio_spawn");
                ...
                SafeTokioChildProcess::spawn(cmd, name.clone(), event_writer.clone()).map_err(
                    |e| {
                        ...
                        cf_telemetry::session_ctx::log_event(
                            cf_telemetry::events::McpServerFailed {
                                server_name: name.clone(),
                                error_type: cf_telemetry::events::McpErrorType::SpawnFailed,
        ...
        | acp::McpServer::Sse(acp::McpServerSse {
            ...
            let auth_prep = if has_existing_auth {
                ...
                let _auth_discovery_timer =
                    cf_telemetry::instrumentation::timer("mcp_http_auth_discovery");
                ...
                .await
                {
                    ...
                    Err(_) => {
                        ...
                        event_writer.emit(
                            cf_file_utils::events::Event::McpOAuthDiscoveryTimeout {
                                server_name: name.clone(),
                                url: url.clone(),
                            },
                        );
...
pub async fn start_mcp_servers(
    ...
    let _mcp_start_timer = cf_telemetry::instrumentation::timer("mcp_start_servers");
...
mod tests {
    ...
    async fn resilient_transport_skips_undecodable_line_and_keeps_stream_alive() {
        ...
        let mut transport = ResilientRwTransport::new(
            ...
            tokio::io::sink(),
            "fwbuild".to_string(),
            cf_file_utils::events::EventWriter::noop(),
        );
    ...
    fn is_figma_mcp_matches_name_and_host() {
        ...
        assert!(is_figma_mcp("QIDI_COM_FIGMA", "https://example.com/mcp"));
        assert!(is_figma_mcp("grok_com_FIGMA", "https://example.com/mcp"));
    ...
    fn test_same_raw_name_different_servers_no_local_registry_collision() {
        use cf_computer_hub_sdk::LocalRegistry;
        use cf_tool_runtime::Tool;
    ...
    async fn recover_and_retry_surfaces_original_error_when_recover_fails() {
        ...
        let mut reconnect_attempted = false;
        let mut is_timeout = false;
        let ew = cf_file_utils::events::EventWriter::noop();
    ...
    async fn try_call_tool_http_mcperror_recovers_then_retry_succeeds() {
        ...
        let ew = cf_file_utils::events::EventWriter::open(tmp.path());
    ...
    fn auth_required_records_as_auth_not_init_failed_and_maps_category() {
        ...
        assert!(matches!(
            err.error_category(),
            cf_file_utils::events::McpErrorCategory::AuthRequired
        ));
    }
