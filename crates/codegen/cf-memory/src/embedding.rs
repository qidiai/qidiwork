...
pub(super) fn build_middleware_client(
    ...
    cf_http::with_auth_retry(cf_http::shared_client(), credentials)
}

fn build_static_middleware_client(
    api_key: Option<String>,
) -> reqwest_middleware::ClientWithMiddleware {
    let provider: std::sync::Arc<dyn cf_auth::AuthCredentialProvider> = std::sync::Arc::new(
        cf_auth::StaticAuthCredentialProvider::new(Box::new(NoopHttpAuth), api_key),
    );
...
impl cf_auth::HttpAuth for NoopHttpAuth {
    ...
    async fn embed_batch(
        ...
        for batch in texts.chunks(self.max_batch_size) {
            ...
            for attempt in 0..MAX_RETRIES {
                ...
                let request = cf_http::shared_client()
