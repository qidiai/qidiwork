...
//! `~/.qidi/memory/{project-slug}-{hash8}/` to avoid polluting the user's repo.

use std::path::{Path, PathBuf};

use cf_tools::util::grok_home::grok_home;
...
pub enum MemoryScope {
    /// Global memory 鈥?shared across all workspaces.
    Global,
    /// Workspace-scoped memory 鈥?specific to one project.
    Workspace,
}
...
/// `~/.qidi/memory/`. Workspace-scoped files live under a directory
/// named `{project-slug}-{hash8}`, e.g. `~/.qidi/memory/xai-a3f7b2c9/`.
#[derive(Debug, Clone)]
pub struct MemoryStorage {
    /// `~/.qidi/memory/`
    global_dir: PathBuf,
    /// `~/.qidi/memory/{project-slug}-{hash8}/`
...
impl MemoryStorage {
    /// Create a new `MemoryStorage` rooted at `~/.qidi/memory/`.
    ///
    ...
    pub fn total_chunk_count(&self) -> usize {
        ...
        // mounts (SIGBUS); see cf_sqlite_journal::JournalMode::open_readonly.
        cf_sqlite_journal::JournalMode::for_db_path(&db_path)
    ...
    ///
    /// File path: `~/.qidi/memory/{project}-{hash8}/sessions/YYYY-MM-DD-{slug}-{sid8}.md`
    ///
    /// - `date`: e.g. `"2026-02-23"`
    ...
    /// canonicalizable 鈥?if either fails, the read is rejected.
    pub fn read_file(
        &self,
        path: &Path,
        from: Option<usize>,
        ...
        // Security: canonicalize both sides 鈥?fail hard if either doesn't exist.
        ...
        if !global_file.exists() {
            write_secure_file(
                ...
                "# Global Memory\n\
                 ...
                 > You can also edit it manually 鈥?changes will be indexed on next session.\n\
                 \n\
                 ## Preferences\n\
                 \n\
...
///
/// **Rules:**
/// 1. Content already starts with `#` 鈫?leave as-is (user-provided structure).
/// 2. Single-line content 鈫?wrap as `## {content}` (the note IS the heading).
/// 3. Multi-line, first line 鈮?80 chars 鈫?first line becomes `## {first_line}`,
///    remaining lines become the body paragraph.
/// 4. Multi-line, first line > 80 chars 鈫?use `## Note` as a generic heading,
...
pub fn normalize_memory_content(raw: &str) -> String {
    ...
    // Already has a Markdown heading 鈫?preserve user's structure.
    if trimmed.starts_with('#') {
        return trimmed.to_string();
    }

    match trimmed.find('\n') {
        // Single-line 鈫?the note IS the heading.
        None => format!("## {trimmed}"),

        // Multi-line 鈫?promote first line to heading if it's short enough.
        Some(pos) => {
...
/// like `/tmp/鈥 or `/var/folders/鈥?T/鈥. Creating persistent workspace
...
/// source 鈥?all clones, worktrees, and copies of the same repository
...
fn compute_workspace_hash(cwd: &Path) -> String {
    ...
    let (slug, hash_input) = match identity {
        ...
        None => {
            // Windows-only, non-git cwds: dunce changes the hash input, so the old-form dir is orphaned until session gc() reaps it after max_age_days 鈥?accepted over an unverifiable rename migration (Unix unchanged).
...
/// - `git@github.com:acme/widgets.git`       鈫?`"acme/widgets"`
/// - `https://github.com/acme/widgets.git`   鈫?`"acme/widgets"`
/// - `ssh://git@github.com/acme/widgets`     鈫?`"acme/widgets"`
...
mod tests {
    ...
    fn test_gc_skips_current_workspace() {
        ...
        // Create the current workspace: old, no sessions 鈥?would qualify for GC
    ...
    fn test_total_chunk_count_missing_and_empty_index() {
        ...
        // Missing index 鈫?0, and the journal-safe open must never create it.
        ...
        let _idx = crate::index::MemoryIndex::open_or_create(
            ...
            storage.clone(),
            cf_config::xai_grok_config_types::MemoryIndexConfig::default(),
            64,
        )
        .unwrap();
        assert_eq!(storage.total_chunk_count(), 0);
