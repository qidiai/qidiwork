...
use cf_pager::app::{WorkspaceMgmtArgs, WorkspaceMgmtCommand, WorkspaceStartArgs};
use cf_pager::client_identity::PAGER_CLIENT_VERSION;
use cf_shell::agent::app::{run_headless, run_leader, run_stdio_agent};
use cf_shell::agent::config::Config as AgentConfig;
use cf_shell::leader::{
...
use cf_update::{UpdateConfig, auto_update, enforce_minimum_version_or_exit};
...
fn apply_agent_endpoint_args(
    agent_args: &cf_pager::app::AgentArgs,
    config: &mut AgentConfig,
) {
    if let Some(v) = &agent_args.cli_chat_proxy_base_url {
        config.endpoints.cli_chat_proxy_base_url = Some(v.clone());
    }
    if let Some(v) = &agent_args.cf_api_base_url {
        config.endpoints.cf_api_base_url = v.clone();
    }
}
...
fn print_serve_startup_info(bind_addr: SocketAddr, secret: &str) {
    eprintln!();
    eprintln!("   QIDI Code agent server starting...");
    eprintln!();
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    ...
    use cf_telemetry::debug_log::RMCP_SSE_NOISE_TARGET;
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(cf_telemetry::sampling_log::layer())
        .with(cf_telemetry::instrumentation::layer())
        .with(cf_telemetry::hooks_log::layer())
        .with(cf_telemetry::otel_layer::build_otel_layer(
            cf_telemetry::otel_layer::OtelClientInfo {
                client_name: "qidi-code",
                client_version: cf_version::VERSION,
                service_version: env!("VERSION_WITH_COMMIT"),
                app_entrypoint,
            },
            cf_shell::auth::credential_provider::build_default_otel_layer_config(),
        ));
    cf_telemetry::debug_log::install_firehose(registry, app_entrypoint);
    cf_telemetry::external::init(
        cf_shell::agent::config::resolve_external_otel_config(
            cf_telemetry::external::config::ExternalClientInfo {
                service_version: env!("VERSION_WITH_COMMIT").to_owned(),
                client_version: cf_version::VERSION.to_owned(),
                app_entrypoint: app_entrypoint.to_owned(),
            },
        ),
    );
}
/// `grok setup`: rendering + exit codes only; fetch logic lives in `cf_shell::managed_config`.
...
) -> Result<(LeaderDescriptor, cf_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = cf_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = cf_shell::leader::LeaderClient::connect(
...
const WORKSPACE_COMMAND_ENV: &str = "QIDI_WORKSPACE_COMMAND";
...
/// The `QIDI_WORKSPACE_COMMAND` override, if set (`Some(true)`/`Some(false)`);
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&cf_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
fn fetch_remote_settings() -> Option<cf_shell::util::config::RemoteSettings> {
    join_early_prefetch(cf_shell::agent::models::start_early_prefetch(None))
}
...
async fn workspace_control(
    ...
    let raw_config = cf_shell::config::load_effective_config_disk_only()
...
async fn workspace_start(
    ...
    use cf_shell::auth::ensure_authenticated;
    cf_shell::util::config::set_remote_campaigns_from_settings(remote_settings.as_ref());
    let raw_config = cf_shell::config::load_effective_config()
        .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
    let agent_config = AgentConfig::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
...
fn shutdown_and_flush_telemetry(exit_code: i32) -> ! {
    cf_telemetry::sentry::flush_on_shutdown();
    cf_telemetry::otel_layer::shutdown_otel();
    cf_telemetry::debug_log::flush();
...
async fn run_agent_command(
    ...
    let _otel_guard = cf_telemetry::otel_layer::otel_guard();
    cf_telemetry::instrumentation::install_panic_hook();
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => cf_shell::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
                tracing::warn!(
    ...
    let early_prefetch = cf_shell::agent::models::start_early_prefetch(None);
    cf_shell::agent::mvp_agent::warm_async_http_client();
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(cf_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = cf_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
    ...
    agent_config.resolve_runtime_fields(&cf_shell::agent::config::RuntimeResolutionContext {
    ...
    if use_leader {
        ...
        let env_urls = cf_shell::leader::LeaderEnvUrls::from(&agent_config.grok_com_config);
        ...
        match mode {
            ClientMode::Stdio => {
                ...
                let stdin_task = tokio::spawn(async move {
                    let mut stdin_lines = cf_acp_lib::spawn_stdin_line_reader();
                    loop {
                        tokio::select! {
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = cf_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            cf_shell::agent::run_agent_server(server_config, agent_config).await
        }
                            ...
                            match auto_update::ensure_latest_on_disk(&uc).await {
                                Ok(outcome) => {
                                    if let Some(v) = &outcome.installed {
                                        if let Err(e) = cf_shell::managed_config::sync().await
                                        {
                                            tracing::warn!(
...
/// Sets `QIDI_OPEN_DASHBOARD_AT_STARTUP=1` if the user asked for
...
/// (`[dashboard].enabled = false` / `QIDI_AGENT_DASHBOARD=0`) is a CLI
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !cf_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             `[dashboard] enabled = false` from ~/.qidi/config.toml and \
             unsetting QIDI_AGENT_DASHBOARD=0."
        );
    }
    args.command = None;
    unsafe { std::env::set_var("QIDI_OPEN_DASHBOARD_AT_STARTUP", "1") };
    Ok(())
}
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<cf_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(cf_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<cf_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(cf_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    cf_shell::heap_profile::install(cf_shell::heap_profile::HeapProfileHooks {
...
fn real_main() -> i32 {
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    {
        cf_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        cf_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    if let Some(code) = cf_pager::app::mermaid_worker::maybe_run_render_subprocess() {
        std::process::exit(code);
    }
    cf_pager::memory_trace::start(
        cf_shell::util::grok_home::grok_home().join("memtrace"),
    );
    raise_fd_limit();
    ...
    let _sentry_guard = cf_telemetry::sentry::init(cf_telemetry::sentry::Config {
        client: "grok-pager",
        client_version: PAGER_CLIENT_VERSION,
        release: env!("VERSION_WITH_COMMIT"),
        disabled: cf_shell::agent::config::is_error_reporting_disabled_sync(),
    });
    cf_pager::docs::extract_user_guide_docs(&cf_shell::util::grok_home::grok_home());
    cf_crash_handler::install_terminal_restore_only();
    ...
    let crashed = cf_shell::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
...
async fn async_main() -> Result<()> {
    ...
    if let Some(ref mode) = args.compaction_mode {
        unsafe { std::env::set_var("QIDI_COMPACTION_MODE", mode) };
    }
    if let Some(ref detail) = args.compaction_detail {
        unsafe { std::env::set_var("QIDI_COMPACTION_DETAIL", detail) };
    }
    if args.chat() {
        unsafe {
            std::env::set_var(cf_shell::agent::chat_modes::QIDI_CHAT_MODE_ENV, "1");
        }
    }
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(cf_shell::leader::LEADER_SOCKET_ENV, socket) };
    }
    if let Some(ref path) = args.debug_file {
        unsafe {
            std::env::set_var("QIDI_DEBUG_LOG", path);
            std::env::remove_var("QIDI_LOG_FILE");
        }
    }
    if args.debug || args.debug_file.is_some() {
        ...
        set_if_unset("QIDI_DEBUG_LOG", "1");
        set_if_unset("QIDI_HOOKS_LOG", "1");
    }
    if let Some(Command::Completions { shell }) = &args.command {
        cf_pager::completions_cmd::run(*shell);
        return Ok(());
    }
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return cf_pager::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        cf_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        cf_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    cf_shell::http::set_client_name(if is_interactive {
        cf_workspace::permission::ClientType::GrokPager
    } else {
        cf_workspace::permission::ClientType::Generic
    });
    ...
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!(
                        ...
                        cf_update::channel_name().unwrap_or("unknown"), }
                    );
            ...
            Command::Inspect { json } => {
                ...
                cf_shell::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Setup { json } => {
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return cf_pager::mcp_cmd::run(mcp_args).await;
            }
            Command::Plugin(plugin_args) => {
                init_tracing_simple("cli");
                let _otel_guard = cf_telemetry::otel_layer::otel_guard();
                return cf_pager::plugin_cmd::run(plugin_args).await;
            }
            Command::Models => {
                init_tracing_simple("cli");
                let _otel_guard = cf_telemetry::otel_layer::otel_guard();
                let config = cf_shell::config::load_effective_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return cf_pager::models::list_available_models(&agent_config).await;
            }
            Command::Worktree(worktree_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return cf_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
            ...
            Command::Sessions(sessions_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return cf_pager::sessions_cmd::run(sessions_args, &agent_config).await;
            }
            Command::Share(ref share_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return cf_pager::share_cmd::run(share_args, &agent_config).await;
            }
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return cf_pager::export_cmd::run(export_args);
            }
            Command::Trace(trace_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return cf_pager::trace_cmd::run(trace_args, &agent_config).await;
            }
            Command::Memory(memory_args) => {
                return cf_pager::memory_cmd::run(memory_args);
            }
            Command::Update {
                check,
                json,
            ...
            Command::Login {
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                cf_shell::auth::run_cli_login(&config, oauth, device_auth, devbox).await?;
                println!();
                cf_shell::instrumentation::finalize_and_exit(0);
            }
            Command::Logout => {
                init_tracing_simple("cli");
                let config = cf_shell::config::load_effective_config_disk_only()
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                cf_shell::auth::run_cli_logout(&config)?;
                cf_shell::instrumentation::finalize_and_exit(0);
            }
    ...
    let headless_prompt = cf_pager::headless::HeadlessPrompt::from_args(
    ...
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(cf_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some() {
            if args.output_format == cf_pager::headless::OutputFormat::Plain {
                args.output_format = cf_pager::headless::OutputFormat::Json;
            }
            if args.self_verify {
                anyhow::bail!(
        ...
        return cf_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            cf_pager::headless::HeadlessOptions {
    ...
    let result = cf_pager::app::run(args, bg_update_rx).await;
    cf_sandbox::flush();
    match result {
        Ok(true) => {
    ...
    cryptify::flow_stmt!({
        {
            config.deployment_key =
                cf_shell::agent::config::EndpointsConfig::default().deployment_key;
        }
    });
    config.npm_registry = std::env::var(obfstr::obfstr!("QIDI_NPM_REGISTRY"))
        .ok()
        .or_else(cf_shell::util::config::load_npm_registry_sync);
    if let Ok(root) = cf_shell::config::load_effective_config_disk_only()
        && let Some(ch) = cf_shell::util::config::channel_from_toml_opt(&root)
    {
        config.channel = ch;
    }
    config
...
fn should_check_for_updates(no_auto_update_flag: bool) -> bool {
    ...
    if std::env::var_os("QIDI_DISABLE_AUTOUPDATER").is_some() {
        return false;
    }
    true
}
...
/// `QIDI_DISABLE_AUTOUPDATER`, debug builds) is layered on separately via
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in cf_shell::leader::discover_leaders().await {
        if d.classification != cf_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        let client = match cf_shell::leader::LeaderClient::connect(
            socket_path,
            "grok-pager-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        ...
        match client
            ...
            .await
        {
            Ok(Ok(cf_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(cf_shell::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            cf_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: cf_shell::heap_profile::JemallocStats) {
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(
            cf_shell::heap_profile::stats().expect("shell stats after install"),
        );
        if !require_opt_prof() {
            assert!(!cf_shell::heap_profile::prof_available());
            return;
        }
        assert!(cf_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(cf_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(cf_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        cf_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    ...
    #[serial_test::serial(QIDI_AGENT_DASHBOARD)]
    #[test]
    fn dashboard_subcommand_flags_startup_without_forcing_leader() {
        ...
        assert_eq!(
            std::env::var("QIDI_OPEN_DASHBOARD_AT_STARTUP").as_deref(),
            Ok("1"),
            "startup hook flag must be set",
        );
        unsafe { std::env::remove_var("QIDI_OPEN_DASHBOARD_AT_STARTUP") };
    }
    ...
    #[serial_test::serial(QIDI_WORKSPACE_COMMAND)]
    #[test]
    fn workspace_command_env_override_parsing() {
        unsafe { std::env::remove_var("QIDI_WORKSPACE_COMMAND") };
        assert_eq!(workspace_command_env_override(), None);
        unsafe { std::env::set_var("QIDI_WORKSPACE_COMMAND", "1") };
        assert_eq!(workspace_command_env_override(), Some(true));
        unsafe { std::env::set_var("QIDI_WORKSPACE_COMMAND", "off") };
