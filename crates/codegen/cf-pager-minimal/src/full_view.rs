...
use cf_pager::app::app_view::AppView;
...
pub fn pump_transcript(app: &mut AppView) {
    ...
    let appearance = super::commit::committed_appearance(&app.appearance);
    {
        ...
        let prev_thinking = cf_pager::appearance::cache::load_show_thinking_blocks();
        ...
        cf_pager::appearance::cache::set_show_thinking_blocks(prev_thinking);
    }
...
fn finish_transcript(app: &mut AppView, id: cf_pager::app::agent::AgentId, out: String) {
    if out.is_empty() {
        if let Some(agent) = app.agents.get_mut(&id) {
            agent
                .scrollback
                .push_block(cf_pager::scrollback::block::RenderBlock::system(
        ...
        Err(e) => {
            if let Some(agent) = app.agents.get_mut(&id) {
                agent.scrollback.push_block(
                    cf_pager::scrollback::block::RenderBlock::system(format!(
...
mod tests {
    use super::*;
    use cf_pager::scrollback::block::RenderBlock;
