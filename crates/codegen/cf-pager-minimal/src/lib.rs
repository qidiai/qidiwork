...
//! terminal's *native* scrollback (via `cf_ratatui_inline::Terminal::insert_before`,
...
//! # Wiring
//!
//! `qidi-code` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`cf_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`qidi-code-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `qidi-code`.
///
...
/// subsequent calls are ignored (see [`cf_pager::minimal_hook`]).
pub fn install() {
    cf_pager::minimal_hook::install(cf_pager::minimal_hook::MinimalHooks { draw });
}
