...
use cf_pager::render::Renderable;
use cf_pager::scrollback::state::ScrollbackState;
use cf_pager::scrollback::wrappers::EntryRenderer;
use cf_pager::theme::Theme;
use cf_pager::views::prompt_widget::PromptStyle;
use cf_pager::views::turn_status;
...
/// welcome box.
pub(super) fn live_left_inset(_appearance: &cf_pager::appearance::AppearanceConfig) -> u16 {
    0
}
...
pub(super) fn prompt_style(
    appearance: &cf_pager::appearance::AppearanceConfig,
) -> PromptStyle {
    PromptStyle {
        focused: true,
...
pub fn draw_live(app: &mut AppView, terminal: &mut PagerTerminal) {
    ...
    cf_pager::render::draw::draw_frame(terminal, cursor, |frame, _link_spans| {
        let area = frame.area();
        ...
        if minimal_api::extensions_modal(agent).is_some() {
            ...
            if let Some(state) = minimal_api::extensions_modal_mut(agent) {
                cf_pager::views::extensions_modal::render_extensions_modal(
                    frame.buffer_mut(),
                    area,
                    state,
                    None,
        ...
        (
            ...
            result
                .post_flush_escapes
                .map(cf_pager::terminal::overlay::PostFlush::from),
        )
    });
}
fn live_tail_renderer<'a>(
    entry: &'a cf_pager::scrollback::entry::ScrollbackEntry,
    theme: &'a Theme,
    appearance: &cf_pager::appearance::AppearanceConfig,
    cwd: &'a std::path::Path,
    tick: u64,
...
fn minimal_watchers(agent: &cf_pager::app::agent_view::AgentView) -> turn_status::Watchers {
    turn_status::Watchers {
        monitors: agent
            ...
            .values()
            .filter(|t| {
                t.is_monitor && t.status == cf_pager::app::agent::BgTaskStatus::Running
            })
            .count(),
...
fn minimal_advance_phase_timer(
    agent: &mut cf_pager::app::agent_view::AgentView,
) -> Option<cf_pager::acp::tracker::TurnActivity> {
...
fn render_minimal_status(
    buf: &mut Buffer,
    area: Rect,
    agent: &cf_pager::app::agent_view::AgentView,
    activity: &Option<cf_pager::acp::tracker::TurnActivity>,
...
fn render_idle_hint(buf: &mut Buffer, area: Rect, theme: &Theme) {
    ...
    buf.set_style(area, style);
    let hint = if cf_pager::app::minimal_auto_set_for_mouse_leak() {
...
fn render_prompt_info(
    ...
    use cf_pager::views::context_bar::fmt_tokens;
    ...
    if let (Some(used), Some(total)) = (used, total)
        && total > 0
    {
        let pct = cf_token_estimation::usage_percentage(used, total);
        segs.push((
...
fn minimal_pending_hint(
    pending: &Option<cf_pager::app::app_view::PendingAction>,
) -> Option<String> {
...
mod tests {
    use super::*;
    fn agent() -> cf_pager::app::agent_view::AgentView {
        minimal_api::test_agent_view(Some("s1"), std::path::PathBuf::from("/tmp"))
    }
    #[test]
    fn tail_height_uses_owning_session_cwd_for_tool_paths() {
        use cf_pager::app::agent::AgentState;
        use cf_pager::scrollback::RenderBlock;
        use cf_pager::scrollback::entry::ScrollbackEntry;
        use cf_pager::scrollback::types::DisplayMode;
        ...
        let appearance = super::super::commit::committed_appearance(
            &cf_pager::appearance::AppearanceConfig::default(),
        );
        let theme = Theme::current();
    ...
    fn minimal_status_shows_rich_activity_and_idle_hint() {
        use cf_pager::acp::tracker::TurnActivity;
        use cf_pager::app::agent::AgentState;
        let theme = Theme::current();
    ...
    fn minimal_status_shows_idle_watching_cue() {
        ...
        a.session.scheduled_tasks.insert(
            "loop-1".to_string(),
            cf_pager::app::agent::ScheduledTaskInfo {
    ...
    fn prompt_info_renders_model_context_and_queued() {
        let mut a = agent();
        a.context_state = Some(cf_shell::session::ContextInfo {
            used: 276_000,
            total: 2_000_000,
            ..Default::default()
        });
    ...
    fn prompt_info_shows_session_mode_flag() {
        ...
        let render = |a: &cf_pager::app::agent_view::AgentView| -> String {
            let mut buf = Buffer::empty(area);
    ...
    fn pending_hint_formats_press_again() {
        ...
        use cf_pager::app::app_view::PendingAction;
        use cf_pager::input::key::KeyShortcut;
