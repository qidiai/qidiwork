...
//! state is a no-op.
//!
//! [`Terminal::set_viewport_height`]: cf_ratatui_inline::Terminal::set_viewport_height

use ratatui::buffer::Buffer;
use ratatui::layout::Rect;
use ratatui::style::Style;

use cf_pager::app::PagerTerminal;
use cf_pager::app::agent_view::AgentView;
use cf_pager::app::app_view::{ActiveView, AppView};
use cf_pager::appearance::LayoutConfig;
use cf_pager::minimal_api;
use cf_pager::render::SafeBuf as _;
use cf_pager::theme::Theme;
use cf_pager::views::prompt_widget::{PromptStyle, PromptWidget};
...
fn active(prompt: &PromptWidget, items_width: u16) -> Option<(Kind, u16)> {
    use cf_pager::views::completion_dropdown::MAX_VISIBLE_ROWS;
    use cf_pager::views::file_search::dropdown::MAX_DROPDOWN_ROWS as FILE_MAX;
    use cf_pager::views::slash_dropdown::desired_item_rows;
...
pub fn render(
    ...
    match kind {
        Kind::FileSearch => {
            cf_pager::views::file_search::dropdown::render_dropdown(
                buf,
                items_rect,
                &prompt.file_search,
                theme,
        ...
        Kind::Slash => {
            ...
            cf_pager::views::slash_dropdown::render_dropdown(
                buf, items_rect, &snap, hovered, theme,
            );
        }
        Kind::Completion => {
            cf_pager::views::completion_dropdown::render_dropdown(
                buf,
                items_rect,
...
pub fn modal_height(modal: Modal, agent: &mut AgentView, screen_h: u16, content_w: usize) -> u16 {
    match modal {
        Modal::Permission => agent
            ...
            .front()
            .map(|p| {
                cf_pager::views::permission_view::permission_view_height(
        ...
        Modal::Question => {
            let input_mode = minimal_api::question_view(agent).is_some_and(|qv| {
                qv.focus == cf_pager::views::question_view::QuestionFocus::InputMode
            });
            ...
            minimal_api::question_view_mut(agent)
                .map(|qv| {
                    cf_pager::views::question_view::question_view_height(
        ...
        Modal::Rewind => minimal_api::rewind_state(agent)
            .map(|rw| cf_pager::views::rewind::rewind_overlay_height(&rw.phase, screen_h))
            .unwrap_or(0),
        Modal::Cancel => {
            if minimal_api::cancel_turn_view(agent).is_some() {
                cf_pager::views::modal::cancel_turn_panel_height(screen_h)
            } else {
                0
            }
        }
    ...
    match modal {
        ...
        Modal::Rewind => {
            if let Some(rw) = minimal_api::rewind_state(agent) {
                cf_pager::views::rewind::render_rewind_overlay(buf, area, &rw.phase, true);
            }
            None
        }
        Modal::Cancel => {
            ...
            let drawn = if let Some(ctv) = minimal_api::cancel_turn_view(agent) {
                cf_pager::views::modal::render_cancel_turn_panel(
                    buf,
                    area,
                    ctv,
                    true,
...
fn render_permission(
    ...
    let result = cf_pager::views::permission_view::render_permission_view(
        buf,
        area,
        perm,
        &followup,
...
fn render_question(
    ...
    use cf_pager::views::question_view::{QUESTION_VIEW_HPAD, QuestionFocus};
    ...
    if let Some(qv) = minimal_api::question_view_mut(agent) {
        let vis = qv.questions.get(qv.active_tab).map(|question| {
            cf_pager::views::question_view::visible_options_height(
                question,
                q_area.height,
                content_w,
                qv.focused_preview(),
    ...
    if let Some(qv) = minimal_api::question_view(agent) {
        cf_pager::views::question_view::render_question_view(
            buf,
            q_area,
            qv,
    ...
    if input_mode {
        ...
        let freeform_sel = minimal_api::question_view(agent)
            ...
            .copied()
            .unwrap_or(false);
        let embed = cf_pager::views::modal_window::embedded_row_style(theme, true);
        ...
        let marker = if is_multi {
            ...
            format!("({})", cf_pager::glyphs::filled_dot())
        } else {
            "(\u{25cb})".to_string()
        };
        ...
        buf.set_span_safe(
            area.x + 9,
            row_y,
            &ratatui::text::Span::styled(cf_pager::glyphs::prompt_arrow(), accent),
            2,
        );
        ...
        let editor = Rect {
            x: area.x + 11,
            y: row_y,
            width: cf_pager::views::question_view::inline_text_width(area.width),
            height: input_h,
        };
        return agent
            .prompt
...
fn question_editor_h(agent: &AgentView, area_w: u16, cap: u16, theme: &Theme) -> u16 {
    let text_w = cf_pager::views::question_view::inline_text_width(area_w);
    agent
        .prompt
...
mod tests {
    use super::*;
    use cf_pager::views::suggestion_controller::CompletionItemParsed;
    ...
    fn question_input_agent(text: &str) -> AgentView {
        use cf_pager::views::prompt_widget::StashedPrompt;
        use cf_pager::views::question_view::{Question, QuestionOption, QuestionViewState};
