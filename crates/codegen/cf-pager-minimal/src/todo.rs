    ...
    let mut lines: Vec<Line<'static>> = todos
        ...
        .map(|t| {
            let (glyph, style) = match t.status {
                ...
                TodoStatus::Completed => (cf_pager::glyphs::check_mark(), theme.muted()),
                TodoStatus::Cancelled => (
                    cf_pager::glyphs::ballot_x(),
...
mod tests {
    ...
    use cf_shell::tools::{TodoItem, TodoPriority};
