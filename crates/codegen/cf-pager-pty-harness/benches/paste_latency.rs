//! `paste-latency` — clipboard paste latency benchmark for `qidi-code`.
//!
...
//!
//! ```bash
//! cargo bench -p qidi-code-pty-harness --bench paste_latency -- --iterations 10
//!
//! # Compare an old release artifact, text mode only, JSON to a file:
//! cargo bench -p qidi-code-pty-harness --bench paste_latency -- \
...
#[command(
    name = "paste-latency",
    about = "Clipboard paste latency benchmark for qidi-code (macOS, real pasteboard)",
    long_about = None,
)]
struct Cli {
