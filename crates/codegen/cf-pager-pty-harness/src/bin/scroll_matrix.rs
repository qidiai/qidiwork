//! `scroll-matrix` — scroll validation matrix sweep for qidi-code.
//!
...
use cf_pager_pty_harness::pager_binary;
use cf_pager_pty_harness::scroll_matrix::{
...
#[command(
    name = "scroll-matrix",
    about = "Run the scroll validation matrix against qidi-code",
    long_about = None,
)]
struct Cli {
    /// Cell tier to run.
    ...
    /// Pager binary. Defaults to PAGER_BINARY, CARGO_BIN_EXE_qidi-code,
...
async fn run() -> Result<ExitCode> {
    ...
    // The pager child resolves QIDI_SCROLL_LOG against ITS cwd (the
