...
pub use cf_test_support::mock_server::LogEntry;
pub use cf_test_support::mock_server::MockModelEntry as MockModel;
pub use cf_test_support::mock_server::StorageUpload;
// SSE event builders for `enqueue_response` scripts (reasoning turns etc.).
pub use cf_test_support::sse;
pub use cf_test_support::{ScriptedResponse, SseEvent};
...
impl ContentController {
    ...
    /// Isolated `$HOME` directory that the pager should use (keeps its ~/.qidi
    ...
    ///
    /// Mirrors `cf_test_support::env::test_env_cmd_tokio`.
    pub fn env_for_pager(&self) -> Vec<(String, String)> {
        ...
        vec![
            ("HOME".into(), home),
            // Explicit QIDI_HOME prevents leaking the real user's
            // config.toml when $HOME alone isn't sufficient (e.g. if
            // QIDI_HOME is set in the test runner's env).
            ("QIDI_HOME".into(), grok_home),
            ("QIDI_CLI_CHAT_PROXY_BASE_URL".into(), self.url()),
            ("QIDI_XAI_API_BASE_URL".into(), self.url()),
            ...
            ("QIDI_PROMPT_SUGGESTIONS".into(), "false".into()),
...
mod tests {
    ...
    async fn env_for_pager_shape() {
        ...
        assert_eq!(get("QIDI_TELEMETRY_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("QIDI_FEEDBACK_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("QIDI_TRACE_UPLOAD").as_deref(), Some("false"));
        assert_eq!(get("QIDI_PROMPT_SUGGESTIONS").as_deref(), Some("false"));
        assert_eq!(get("QIDI_MAX_RETRIES").as_deref(), Some("0"));
