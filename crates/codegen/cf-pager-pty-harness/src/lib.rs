//! Unified PTY harness for qidi-code.
//!
...
//!    `qidi-code/tests/scenarios/`) — assert screen contents and
...
//! - **`scroll_matrix`**  — `QIDI_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
impl PtyHarness {
    ...
    /// # use cf_pager_pty_harness::{PtyHarness, ContentController, pager_binary};
