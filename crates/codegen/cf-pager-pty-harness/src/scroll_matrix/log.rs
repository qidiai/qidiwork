//! `QIDI_SCROLL_LOG` JSONL parsing, per-stream grouping, and finalize
//! synchronization.
//!
//! Wire schema source of truth: the pager's `ScrollLogRecord` in
//! `qidi-code/src/input/scroll_log.rs`. [`ScrollLogLine`] mirrors it
...
/// Parse a `QIDI_SCROLL_LOG` JSONL file into records.
///
...
/// session (`QIDI_SCROLL_LOG` set at spawn): `stream_start` → `flush`* →
...
mod tests {
    ...
    // in `qidi-code/src/input/mouse/tests.rs`.
