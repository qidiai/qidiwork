...
//! `QIDI_SCROLL_LOG` pointed at a tempfile, then validates the pager's
//! flight-recorder JSONL (producer:
//! `qidi-code/src/input/scroll_log.rs`) against gesture invariants.
//!
...
//! producer-side twin lives in `qidi-code/src/input/mouse/tests.rs`
