...
//!
//! ```bash
//! cargo test -p qidi-code-pty-harness --test prompt_history_durable_quit \
//!   -- --ignored --nocapture
//! ```
...
use cf_pager_pty_harness::{ContentController, PtyHarness, keys, pager_binary};
...
/// The per-CWD history file lives at `<home>/.qidi/sessions/<enc-cwd>/prompt_history.jsonl`.
