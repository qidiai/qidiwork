...
//!
//! Disk writes live in `cf_shell::util::config::set_<field>()`.
...
use cf_shared::ui_config::UiConfig;
...
/// Default ON when unset. The `QIDI_PROMPT_SUGGESTIONS` env var overrides
...
/// `QIDI_SCROLL_MODE` overrides `[ui].scroll_mode` (the `QIDI_SCROLL_SPEED`
...
pub fn load_scroll_mode() -> ScrollMode {
    SCROLL_MODE_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("QIDI_SCROLL_MODE")
                .ok()
...
/// `QIDI_INVERT_SCROLL` (`1`/`true`/`0`/`false`) overrides `[ui].invert_scroll`.
pub fn load_invert_scroll() -> bool {
    INVERT_SCROLL_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("QIDI_INVERT_SCROLL")
                .ok()
...
/// its own lines-per-tick. `QIDI_SCROLL_LINES` overrides `[ui].scroll_lines`.
pub fn load_scroll_lines() -> Option<u8> {
    SCROLL_LINES_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("QIDI_SCROLL_LINES")
                .ok()
...
fn load_bool_from_effective_config(key: &str, default: bool) -> bool {
    let root = match cf_config::load_effective_config_disk_only() {
        Ok(r) => r,
...
fn load_str_from_effective_config(key: &str) -> Option<String> {
    let root = cf_config::load_effective_config_disk_only().ok()?;
    root.get("ui")
        .and_then(|ui| ui.get(key))
        .and_then(|v| v.as_str())
        .map(str::to_string)
