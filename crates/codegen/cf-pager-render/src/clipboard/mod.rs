...
pub use cf_ratatui_textarea::{ClipboardProvider, InternalClipboard};
...
fn is_remote() -> bool {
    ...
    *REMOTE.get_or_init(cf_shared::clipboard::is_remote_session)
}
...
fn is_container_no_display() -> bool {
    ...
    *CONTAINER.get_or_init(cf_shared::clipboard::is_containerized_without_display)
}
...
/// clipboard (see `qidi-code`'s `pty_wrap` module). It advertises this to
...
///
/// Two names are accepted: the canonical `QIDI_OSC52_SINK` (inherited by local
/// children) and the `LC_`-prefixed `LC_QIDI_OSC52_SINK`, which the default
...
pub fn osc52_sink_active() -> bool {
    ...
    *SINK.get_or_init(|| {
        std::env::var_os("QIDI_OSC52_SINK").is_some()
            || std::env::var_os("LC_QIDI_OSC52_SINK").is_some()
    })
}
...
pub fn wayland_data_control_label() -> &'static str {
    match crate::host::DisplayServer::current() {
        crate::host::DisplayServer::Wayland => {
            if cf_shared::clipboard::wayland_data_control_supported() {
                "yes"
            } else {
                "no"
            }
...
fn write_tmux_buffer(text: &str) -> bool {
    ...
    let result = (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        let stdin = cf_shared::clipboard::spool_for_stdin(text.as_bytes())?;
        ...
        let status = cf_shared::clipboard::wait_with_deadline(
            &mut child,
...
///
/// Delegates to [`cf_shared::clipboard`] which uses
...
impl ClipboardProvider for SystemClipboard {
    fn get(&mut self) -> Option<String> {
        cf_shared::clipboard::get_text().ok().flatten()
    }
...
fn clipboard_write_with_route(text: &str, route: &ClipboardRoute) -> ClipboardWriteLegs {
    ...
    if route.native {
        let outcome = cf_shared::clipboard::set_text_with_outcome(text);
    ...
    if route.osc52 {
        match cf_shared::clipboard::set_text_osc52(text, route.osc52_tmux_passthrough) {
...
fn log_clipboard_copy_event(
    ...
    if !cf_telemetry::client::is_enabled() {
        return;
    }
    cf_telemetry::session_ctx::log_event(cf_telemetry::events::ClipboardCopy {
...
pub fn system_clipboard_read_text() -> Result<Option<String>, ClipboardTextReadError> {
    ...
    cf_shared::clipboard::get_text().map_err(|error| {
...
pub fn system_primary_selection_get() -> Option<String> {
    ...
    if !cf_shared::clipboard::x11_display_env_present() {
        return None;
    }
    cf_shared::clipboard::get_primary_text()
        .ok()
        .flatten()
        .filter(|text| !text.is_empty())
}
...
pub fn x11_primary_guidance_available() -> bool {
    #[cfg(target_os = "linux")]
    {
        is_native_x11(crate::host::DisplayServer::current())
            && cf_shared::clipboard::x11_display_env_present()
    }
...
pub fn log_paste_key_empty_host_clipboard(surface: &str) {
    ...
    cf_telemetry::session_ctx::log_event(
        cf_telemetry::events::PasteKeyEmptyHostClipboard {
            terminal,
...
fn log_clipboard_paste_event(
    ...
    cf_telemetry::session_ctx::log_event(cf_telemetry::events::ClipboardImagePaste {
        terminal: crate::terminal::terminal_context().telemetry_snapshot(),
...
fn system_clipboard_get_attachments() -> Result<AttachmentsProbeResult, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match cf_shared::clipboard::get_attachments() {
        Ok(att) => {
...
pub use cf_shared::clipboard::ImageData;
...
pub fn clipboard_image_snapshot() -> (Option<u64>, bool) {
    ...
    cf_shared::clipboard::clipboard_image_snapshot()
}
...
pub fn clipboard_change_count() -> Option<u64> {
    ...
    cf_shared::clipboard::clipboard_change_count()
}
...
pub fn clipboard_image_probe_supported() -> bool {
    ...
    cf_shared::clipboard::clipboard_image_probe_supported()
}
...
pub fn prewarm_image_probe() {
    ...
    WARMED.call_once(|| {
        std::thread::spawn(cf_shared::clipboard::clipboard_prewarm);
    });
}
...
fn system_clipboard_get_image_result() -> Result<Option<ImageData>, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match cf_shared::clipboard::get_image() {
        Ok(img) => {
...
#[cfg(all(any(test, feature = "test-helpers"), target_os = "linux"))]
