...
pub fn open_url(url: &str) {
    ...
    if let Ok(path) = std::env::var("QIDI_TEST_OPEN_URL_FILE") {
        use std::io::Write;
        ...
        {
            tracing::warn!(error = %e, path, "QIDI_TEST_OPEN_URL_FILE write failed");
        }
        return;
    }
    ...
    cf_tools::util::detach_std_command(&mut command);
...
///
/// The returned command is TTY-guarded via [`cf_tty_utils::detach_std_command`]
...
fn build_open_path_command(path: &std::path::Path) -> std::process::Command {
    ...
    command
        ...
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null());
    cf_tty_utils::detach_std_command(&mut command);
    command
}
...
///
/// Used by the SuperGrok upsell flow to attribute clicks to `referrer=cf-tools`,
...
mod tests {
    ...
    fn ensure_query_param_appends_when_missing() {
        let out = ensure_query_param("https://grok.com/supergrok", "referrer", "cf-tools");
        assert_eq!(out, "https://grok.com/supergrok?referrer=cf-tools");
    }
    ...
    fn ensure_query_param_keeps_other_query_pairs() {
        ...
        assert_eq!(
            out,
            "https://grok.com/supergrok?heavy=1&referrer=cf-tools"
        );
    }
    ...
    fn ensure_query_param_preserves_fragment() {
        ...
        let out = ensure_query_param("https://grok.com/#supergrok", "referrer", "cf-tools");
        assert_eq!(out, "https://grok.com/?referrer=cf-tools#supergrok");
    }

    #[test]
    fn ensure_query_param_returns_unchanged_on_parse_failure() {
        let out = ensure_query_param("not a url", "referrer", "cf-tools");
