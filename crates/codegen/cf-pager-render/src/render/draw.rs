...
//! [`cf_ratatui_inline::Terminal`] whose `flush()` returns `bool` indicating
...
use cf_ratatui_inline::LinkSpan;
...
pub type PagerTerminal = cf_ratatui_inline::Terminal<CrosstermBackend<TermWriter>>;
...
pub fn spawn_writer_thread() -> (mpsc::Sender<Vec<u8>>, WriterSync, WriterThread) {
    ...
    let test_delay = std::env::var("QIDI_TEST_FRAME_WRITE_DELAY_MS")
        .ok()
    ...
    let handle = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let mut writer: Box<dyn std::io::Write> = {
                let tui_out = cf_tty_utils::dup_tui_stderr().unwrap_or_else(|_| {
            ...
            while let Ok(data) = rx.recv() {
                ...
                {
                    let _guard = cf_shared::stderr::stderr_lock();
