...
impl TerminalContext {
    ...
    pub fn telemetry_snapshot(&self) -> cf_telemetry::events::TerminalTelemetry {
        ...
        cf_telemetry::events::TerminalTelemetry {
            ...
            clipboard_native_tool: cf_shared::clipboard::native_tool_name().to_owned(),
            clipboard_data_control: crate::clipboard::wayland_data_control_label().to_owned(),
        }
    }

    /// Extract terminal info for feedback submissions.
    pub fn feedback_info(&self) -> cf_shared::session::FeedbackTerminalInfo {
        use cf_shared::session::FeedbackTerminalInfo;
        ...
        FeedbackTerminalInfo {
            ...
            clipboard_native_tool: Some(cf_shared::clipboard::native_tool_name().to_owned()),
...
/// `qidi-code-pty-harness/src/pty.rs` (test-env hygiene — the PTY
...
///
/// Parsed from `[terminal] alt_screen` in `~/.qidi/pager.toml` and
