...
//! lookup must be cheaper than re-loading from `~/.qidi/config.toml`.
...
//! `cf_shell::util::config::set_theme()` (and friends), invoked
...
///
/// On the first call, reads from `~/.qidi/config.toml` (via the shell's
...
pub fn set_terminal_native_lock(locked: bool) {
    ...
    cf_markdown::set_color_level_cap(if locked {
        cf_markdown::ColorLevel::Basic
    } else {
        cf_markdown::ColorLevel::TrueColor
    });
}
...
///
/// Precedence:
/// 1. Environment variable (`QIDI_THEME`)
...
// -- Disk reads --------------------------------------------------------------
//
// All writes go through `cf_shell::util::config::set_theme()` (and
...
fn load_auto_theme_config() -> AutoThemeConfig {
    let Ok(root) = cf_config::load_effective_config_disk_only() else {
