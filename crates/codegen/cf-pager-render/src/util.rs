...
pub use cf_config::grok_home;

/// Path to `$QIDI_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    grok_home().join("pager.toml")
}

/// User-facing label for the user grok directory (``~/.qidi`` or ``$QIDI_HOME``).
///
/// Derived from resolved [`grok_home()`] vs `cf_config::default_grok_home()`,
/// not from whether `QIDI_HOME` is set in the environment.
pub fn display_grok_home_prefix() -> String {
    if grok_home() == cf_config::default_grok_home() {
        "~/.qidi".to_string()
    } else {
        "$QIDI_HOME".to_string()
    }
...
/// User-facing path under [`grok_home()`], e.g. ``~/.qidi/config.toml``.
...
mod tests {
    ...
    fn display_grok_home_prefix_default_install() {
        if std::env::var("QIDI_HOME").is_ok() {
            return;
        }
        assert_eq!(display_grok_home_prefix(), "~/.qidi");
    }
    ...
    fn abbreviate_path_uses_home_when_under_default_grok() {
        if let Ok(home) = std::env::var("HOME") {
            ...
            let full = format!("{home}/.qidi/memory/MEMORY.md");
