...
2. **settings.json** — `extraKnownMarketplaces` from `~/.qidi/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `qidi plugin marketplace add <source>`)
...
Sources land in `~/.qidi/config.toml`:
...
Or in `~/.qidi/settings.json` / `~/.claude/settings.json`:
