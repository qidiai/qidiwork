...
On first launch, Grok opens your browser to authenticate with grok.com. After you sign in, Grok stores your credentials in `~/.qidi/auth.json`, where they persist across sessions. Grok refreshes your credentials automatically and prompts you to sign in again when they can no longer be renewed.
...
Every conversation is a **session**. Sessions are automatically saved to `~/.qidi/sessions/` and can be resumed later. Each session tracks the full conversation history, tool calls, file edits, and task state.
...
```bash
# --yolo (alias of --always-approve) auto-approves tool calls. In CI, pair it
# with a sandbox so an unattended agent still has an OS-level guardrail:
qidi -p "Review changes for bugs" --output-format json --sandbox=readonly --yolo | jq -r '.text'
```

> Security note: a fully-open `--yolo` with the sandbox disabled leaves no
> OS-level protection. The permission layer also supports a YOLO *tool
> allowlist* (see [`YoloMode`](../../../cf-workspace/src/permission/yolo.rs)) so
> only chosen tools are auto-approved — prefer that over blanket `--yolo` when
...
```
~/.qidi/AGENTS.md           # Global rules (apply to all projects)
