...
MCP servers are configured in `~/.qidi/config.toml` under `[mcp_servers.<name>]` sections.
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. Grok reads most other config sections only from `~/.qidi/config.toml`.
...
You must authenticate OAuth-based MCP servers before you can use them. Grok stores the resulting tokens under `~/.qidi/mcp_credentials.json`. After you edit `config.toml`, press `r` in the `/mcps` modal to refresh the server list.
...
For stdio servers, Grok captures the process's standard error to `~/.qidi/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.qidi/logs/mcp/filesystem.stderr.log
```
