...
|----------|-------|-------------|
| `~/.qidi/memory/MEMORY.md` | Global | Facts that apply across all your projects |
| `~/.qidi/memory/<project-slug>-<hash8>/MEMORY.md` | Workspace | Project-specific conventions and context |
| `~/.qidi/memory/<project-slug>-<hash8>/sessions/` | Sessions | Per-session summaries and logs |
...
Ask Grok to remember something, and it appends the note to a `MEMORY.md` file -- the workspace file for project-specific items, or the global `~/.qidi/memory/MEMORY.md` for cross-project preferences:
...
Forget is best-effort: the model searches memory and removes entries that match. For guaranteed removal, edit the files under `~/.qidi/memory/` directly and delete the entry yourself. To locate a file, open the `/memory` browser and press `y` to copy its path.
...
You can edit memory files directly under `~/.qidi/memory/`. The file watcher reindexes your changes on the next memory search. Use `/flush` to save the current session now, and `/dream` to consolidate session logs into organized topics.
...
```
Memory saved to ~/.qidi/memory/MEMORY.md
```
...
To edit memory from the shell, open the files in your editor directly -- for example, `$EDITOR ~/.qidi/memory/MEMORY.md`.
...
| `watcher.enabled` | `true` | Watch `~/.qidi/memory/` for external edits and reindex |
...
By default, Grok watches `~/.qidi/memory/` for external file changes. If you edit memory files directly (e.g., in your editor), the changes are picked up automatically on the next memory search:
...
```bash
ls ~/.qidi/memory/
cat ~/.qidi/memory/MEMORY.md
$EDITOR ~/.qidi/memory/MEMORY.md
```
