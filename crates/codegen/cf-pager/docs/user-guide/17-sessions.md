...
Sessions are identified by a unique session ID (a UUIDv7 when QIDI Code generates it; a client may supply its own ID with `-s`) and stored on disk under `~/.qidi/sessions/`. Set `QIDI_HOME` to override the base directory; when it is unset, QIDI Code uses `~/.qidi`.
