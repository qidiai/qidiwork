...
fn main() {
    ...
    let updates_path = match std::env::var("QIDI_SESSION_PATH") {
        Ok(p) => std::path::PathBuf::from(p),
        Err(_) => {
            eprintln!("Set QIDI_SESSION_PATH to a session's updates.jsonl path");
            eprintln!(
                "Example: QIDI_SESSION_PATH=$HOME/.qidi/sessions/<cwd-encoded>/019e0000-0000-7000-8000-000000000001/updates.jsonl"
            );
            return;
        }
    };
    ...
    if !updates_path.exists() {
        ...
        eprintln!("Set QIDI_SESSION_PATH env var to point to a session's updates.jsonl");
        return;
    }
