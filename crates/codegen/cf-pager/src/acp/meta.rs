...
pub struct NotificationMeta {
    ...
    /// (`"{sessionId}-{counter}"`, see `qidi-code util::event_id`). The
...
/// replayed notifications (`_meta.isReplay`, stamped by qidi-code's
...
pub mod user_message_chunk_meta {
    ...
    /// ([`cf_shell::session::PromptOrigin::hide_user_echo_from_scrollback`]).
