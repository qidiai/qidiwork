...
use cf_acp_lib::{AcpAgentTx, AcpClientRx, acp_send};
...
pub struct AcpConnection {
    ...
    // When available, add field: startup_announcements: Option<Vec<cf_announcements::RemoteAnnouncement>>
    ...
    pub auth_manager: std::sync::Arc<cf_shell::auth::AuthManager>,
}
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<cf_shell::util::config::RemoteSettings>,
...
pub async fn connect_via_leader(
    ...
    cf_shell::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = cf_shell::util::grok_home::grok_home().join("config.toml");
...
mod tests {
    ...
    ///
    /// Enterprise/BYOK configs (e.g. an enterprise `~/.qidi/config.toml` with a
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use cf_shell::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};
    ...
    fn startup_auth_xai_api_key_not_first_still_requires_login() {
        use cf_shell::agent::auth_method::{QIDI_COM_METHOD_ID, XAI_API_KEY_METHOD_ID};

        let methods = vec![
            make_auth_method(QIDI_COM_METHOD_ID, "Grok", None),
