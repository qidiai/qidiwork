...
pub async fn spawn_grok_shell(
    ...
    memory_config: Option<cf_shell::config::MemoryConfig>,
) -> Result<SpawnedAgent> {
    ...
    // skills to ~/.qidi/skills/), and model catalog construction.
    let (agent_config, models_manager) =
        cf_shell::agent::init::bootstrap(&agent_config, &auth_manager, None)
