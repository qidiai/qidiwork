...
use cf_tools::types::output::{ReadFileOutput, SearchToolOutput, WebFetchOutput};
use cf_tools::util::strip_redundant_session_cd;
...
impl AcpUpdateTracker {
    ...
    fn handle_user_message(
        ...
        let mut block = if let Some(dt) = display_override {
            ...
            let skill_display =
                cf_tools::implementations::skills::skill::extract_skill_display_text(&text);
...
fn user_message_hidden_from_scrollback(
    ...
    if let Some(pid) = meta.prompt_id.as_deref()
        && cf_shell::session::PromptOrigin::from_prompt_id(pid)
...
fn extract_edit_error(tc: &acp::ToolCall) -> String {
    use cf_tools::types::output::SearchReplaceOutput;
    if let Some(ref raw) = tc.raw_output
...
fn extract_listdir_content(raw: &Option<serde_json::Value>) -> Option<String> {
    let val = raw.as_ref()?;
    match serde_json::from_value::<ToolOutput>(val.clone()) {
        Ok(ToolOutput::ListDir(cf_tools::types::output::ListDirOutput::Content(c))) => {
            Some(c.content)
        }
        _ => None,
    }
...
fn extract_use_tool_output(raw: &Option<serde_json::Value>) -> Option<String> {
    ...
    if let Ok(output) = serde_json::from_value::<ToolOutput>(val.clone()) {
        let text = match output {
            ToolOutput::MCP(mcp) => {
                use cf_tools::types::output::MCPOutputDetails;
                match mcp.output() {
...
mod tests {
    ...
    fn test_search_tool_call_flow() {
        use cf_tools::types::output::{GrepFileMatch, GrepLineMatch, GrepSearchOutput};
        let mut tracker = AcpUpdateTracker::new();
    ...
    #[test]
    fn is_task_tool_recognizes_qidi_build_variant() {
...
[CI] IN_PROGRESS=5 SUCCESS=38\n\
    ...
    fn video_tool_variants_use_typed_path_not_generic_scrape() {
        ...
        let cases: &[(&str, ToolOutput)] = &[
            (
                "ImageToVideo",
                ToolOutput::ImageToVideo(cf_tools::types::output::MediaGenOutput::new(
                    video_path.clone(),
                )),
            ),
            (
                "ReferenceToVideo",
                ToolOutput::ReferenceToVideo(cf_tools::types::output::MediaGenOutput::new(
                    video_path.clone(),
                )),
            ),
        ];
    ...
    fn media_gen_ref_skips_uploaded_only_video() {
        let output =
            ToolOutput::ImageToVideo(cf_tools::types::output::MediaGenOutput::uploaded(
    ...
    fn tier_restricted_media_shows_upsell_text_not_error() {
        let upsell = "Image generation is a SuperGrok feature. Upgrade at \
             https://grok.com/supergrok?referrer=cf-tools";
        let output = ToolOutput::Text(cf_tools::types::output::TextOutput::from(upsell));
        let tc = acp::ToolCall::new(
