    ...
    let changed = match session_notif.update {
            ...
            let hook_entries: Vec<HookRunEntry> = runs
                ...
                .map(|r| {
                    let status = match r.status {
                        cf_shell::extensions::notification::HookRunStatusDto::Success {
                            elapsed_ms,
                        } => HookRunStatus::Success {
                            elapsed: std::time::Duration::from_millis(elapsed_ms),
                        },
                        cf_shell::extensions::notification::HookRunStatusDto::Skipped => {
                            HookRunStatus::Skipped
                        }
                        cf_shell::extensions::notification::HookRunStatusDto::Failed {
                            error,
                            elapsed_ms,
        ...
        XaiSessionUpdate::HooksChanged {
            ...
            if let Some(ref mut modal) = agent.extensions_modal {
                ...
                modal.hooks_data =
                    TabDataState::Loaded(cf_hooks_plugins_types::HooksListResponse {
                        hooks,
                        project_trusted,
                        load_errors,
                    });
        ...
        XaiSessionUpdate::ModelChanged {
            ...
            if !agent.session.models.available.contains_key(&new_model_id) {
                if cf_shell::agent::chat_modes::process_chat_mode_enabled() {
                ...
                && let Some(info) = agent.subagent_sessions.get_mut(child_sid)
            {
                ...
                if let Some(cw) = info.context_window_tokens.filter(|&cw| cw > 0) {
                    info.context_usage_pct =
                        Some(cf_token_estimation::usage_percentage_u8(tokens_after, cw));
                }
            }
            changed
        }
...
pub(super) fn apply_image_compressed(
    agent: &mut AgentView,
    images: &[cf_shell::extensions::notification::ImageCompressedEntry],
    message: &str,
) -> bool {
    if images.is_empty() {
...
pub(super) fn apply_retry_state(
    retry: &cf_shell::extensions::notification::RetryState,
    session: &mut AgentSession,
    scrollback: &mut crate::scrollback::state::ScrollbackState,
    is_api_key_auth: bool,
) {
        ...
        RetryState::Exhausted {
            ...
            if *rate_limited {
                cf_telemetry::session_ctx::log_event(
                    cf_telemetry::events::RateLimitHit {
                        model_id: session
                            .models
                            .current
                            .as_ref()
    ...
    if is_credit_limit {
        cf_telemetry::session_ctx::log_event(cf_telemetry::events::CreditLimitHit {
            model_id: session
                .models
                .current
                .as_ref()
...
pub(super) fn detect_plan_mode_change(update: &acp::SessionUpdate, agent: &mut AgentView) -> bool {
    use cf_tools::types::SessionMode;
