#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.qidi` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.qidi` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.qidi` reads).
    #[test]
