...
use cf_shell::extensions::notification::RetryState;
use cf_shell::extensions::notification::SessionUpdate as XaiSessionUpdate;
...
pub(super) fn compressed_entry(
    index: usize,
) -> cf_shell::extensions::notification::ImageCompressedEntry {
    cf_shell::extensions::notification::ImageCompressedEntry {
        index,
...
pub(super) fn send_tool_call_update(
    ...
    handle(
        AcpClientMessage::SessionNotification(cf_acp_lib::AcpArgs {
...
pub(super) fn cf_model_switch_notif(
    session_id: &str,
    event_id: &str,
) -> acp::ExtNotification {
...
pub(super) fn cf_unhandled_notif(
    session_id: &str,
    event_id: &str,
) -> acp::ExtNotification {
    let payload = SessionNotification {
...
pub(super) fn cf_turn_completed_notif(
    session_id: &str,
    prompt_id: &str,
    stop_reason: &str,
    is_replay: bool,
...
pub(super) fn cf_wake_turn_completed_notif(
    session_id: &str,
    prompt_id: &str,
...
pub(super) fn cf_hook_execution_notif_for_prompt(
    session_id: &str,
    event_name: &str,
    prompt_id: Option<&str>,
    is_replay: bool,
) -> acp::ExtNotification {
    use cf_shell::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
...
pub(super) fn cf_hook_execution_notif(
    session_id: &str,
    event_name: &str,
    is_replay: bool,
) -> acp::ExtNotification {
    cf_hook_execution_notif_for_prompt(session_id, event_name, None, is_replay)
}
...
pub(super) fn make_ext_session_notification_with_method(
    ...
    AcpClientMessage::ExtNotification(cf_acp_lib::AcpArgs {
        request,
        response_tx: tx,
    })
}
...
/// Shared temp `QIDI_HOME` for disk-replay tests. `grok_home()` uses a
/// process-wide `OnceLock`, so parallel tests must not each set `QIDI_HOME`
...
pub(super) fn make_permission_message(
    ...
    let msg = AcpClientMessage::RequestPermission(cf_acp_lib::AcpArgs {
        request,
        response_tx: tx,
    });
    (msg, rx)
...
pub(super) fn make_git_head_changed_notif(
    ...
    let payload = cf_workspace::session::git::GitHeadChanged {
...
pub(super) fn make_server_status_notif(
    session_id: &str,
    name: &str,
    status: cf_shell::extensions::mcp::McpServerStatus,
    tools: Option<serde_json::Value>,
) -> acp::ExtNotification {
    use cf_shell::extensions::mcp::{
...
pub(super) fn make_tools_changed_notif_post_h2(
    ...
    let payload = cf_shell::extensions::mcp::McpToolsChanged {
        session_id: session_id.to_string(),
