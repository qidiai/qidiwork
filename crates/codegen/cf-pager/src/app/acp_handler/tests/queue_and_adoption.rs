#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn queue_changed_adopts_running_prompt_id_only_when_unset() {
        ...
        let shell_payload = cf_shell::session::prompt_queue::QueueChanged {
    ...
    fn queue_changed_still_adopts_running_prompt_not_terminal_in_replay() {
        ...
        app.agents.get_mut(&id).unwrap().session.loading_replay = true;
        let _ = handle_ext_notification(
            &cf_turn_completed_notif("sess-1", "p-old", "end_turn", true),
            &mut app,
        );
        app.agents.get_mut(&id).unwrap().session.loading_replay = false;
    ...
    fn adopted_running_prompt_lets_subsequent_chunk_through_gate() {
        ...
        let msg = AcpClientMessage::SessionNotification(cf_acp_lib::AcpArgs {
            request,
            response_tx: tx,
        });
        handle(msg, &mut app);
    ...
    fn bash_kind_round_trips_and_adoption_sets_bash_turn() {
        ...
        let shell = cf_shell::session::prompt_queue::QueueChanged {
            session_id: "sess-1".to_string(),
            entries: vec![cf_shell::session::prompt_queue::QueueEntryWire {
                id: "b1".to_string(),
                version: 0,
                owner: None,
                last_editor: None,
    ...
    fn viewer_stop_hooks_after_marker_attach_to_it() {
        ...
        let _ = handle_ext_notification(
            &cf_turn_completed_notif("sess-view-hooks", "pid-v", "end_turn", false),
            &mut app,
        );
        assert_eq!(
        ...
        let _ = handle_ext_notification(
            &cf_hook_execution_notif("sess-view-hooks", "stop", false),
            &mut app,
        );
        ...
        let _ = handle_ext_notification(
            &cf_hook_execution_notif("sess-view-hooks", "stop_failure", false),
            &mut app,
        );
        let agent = app.agents.get(&AgentId(0)).unwrap();
        assert_eq!(
