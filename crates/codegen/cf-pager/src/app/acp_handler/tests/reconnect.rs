#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn reconnect_reload_failure_restores_pre_outage_transcript() {
        ...
        let _ = handle_ext_notification(&cf_model_switch_notif("sess-rc", "sess-rc-30"), &mut app);
        assert_eq!(
    ...
    #[test]
    fn cf_session_update_dedup_drops_already_applied_event() {
        let mut app = make_app_with_agent("sess-xdup");
        let id = AgentId(0);

        assert!(handle_ext_notification(
            &cf_model_switch_notif("sess-xdup", "sess-xdup-10"),
            &mut app
        ));
        ...
        assert!(handle_ext_notification(
            &cf_model_switch_notif("sess-xdup", "sess-xdup-11"),
            &mut app
        ));
        ...
        // equality.
        assert!(!handle_ext_notification(
            &cf_model_switch_notif("sess-xdup", "sess-xdup-9"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].scrollback.len(),
    ...
    fn unhandled_xai_update_does_not_advance_cursor_or_highwater() {
        ...
        assert!(!handle_ext_notification(
            &cf_unhandled_notif("sess-ig", "sess-ig-7"),
            &mut app
        ));
        assert_eq!(
        ...
        assert!(handle_ext_notification(
            &cf_model_switch_notif("sess-ig", "sess-ig-8"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].last_seen_event_id.as_deref(),
    ...
    fn direct_xai_event_does_not_shoot_down_delayed_acp_chunk() {
        ...
        assert!(handle_ext_notification(
            &cf_model_switch_notif("sess-split", "sess-split-21"),
            &mut app
        ));
    ...
    #[test]
    fn cf_session_update_replay_gating_and_cursor() {
