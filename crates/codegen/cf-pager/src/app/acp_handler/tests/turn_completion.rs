#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn live_turn_completed_finalizes_viewer_turn_and_duplicate_is_noop() {
        ...
        let affected = handle_ext_notification(
            &cf_turn_completed_notif("sess-view", "pid-driver", "end_turn", false),
            &mut app,
        );
    ...
    fn live_turn_completed_driver_arms_reconcile() {
        ...
        let _ = handle_ext_notification(
            &cf_turn_completed_notif("sess-drive", "pid-local", "cancelled", false),
            &mut app,
        );
    ...
    fn wake_marker_without_tracked_start_omits_elapsed() {
        ...
        let _ = handle_ext_notification(
            &cf_wake_turn_completed_notif("sess-wake", "task-completed-bg1", None),
            &mut app,
        );
    ...
    fn wake_turn_completed_in_replay_only_records_pid() {
        ...
        let affected = handle_ext_notification(
            &cf_turn_completed_notif("sess-wake", "task-completed-bg1", "end_turn", true),
            &mut app,
        );
    ...
    fn scheduler_fired_turn_completed_keeps_adopted_path() {
        ...
        let affected = handle_ext_notification(
            &cf_wake_turn_completed_notif("sess-cron", "scheduler-fired-abc", Some(1_000)),
            &mut app,
        );
    ...
    fn failed_wake_turn_keeps_markerless_shape() {
        ...
        for stop_reason in ["error", "cancelled", "rate_limit"] {
            let _ = handle_ext_notification(
                &cf_turn_completed_notif("sess-wake", "task-completed-bg1", stop_reason, false),
                &mut app,
            );
        }
    ...
    fn dead_wake_reemits_skipped_work_status_line() {
        ...
        let affected = handle_ext_notification(
            &cf_turn_completed_notif("sess-wake", "task-completed-bg1", "cancelled", false),
            &mut app,
        );
    ...
    fn wake_terminal_during_local_turn_pushes_nothing() {
        ...
        let affected = handle_ext_notification(
            &cf_wake_turn_completed_notif("sess-wake", "task-completed-bg1", Some(6_000)),
            &mut app,
        );
    ...
    fn live_stop_hooks_during_turn_stash_instead_of_standalone_block() {
        ...
        let _ = handle_ext_notification(
            &cf_hook_execution_notif("sess-stop", "stop", false),
            &mut app,
        );
    ...
    fn replayed_stop_hooks_render_as_standalone_block() {
        ...
        let _ = handle_ext_notification(
            &cf_hook_execution_notif("sess-replay", "stop", true),
            &mut app,
        );
    ...
    fn foreign_turn_stop_hooks_never_stash_under_running_turn() {
        ...
        let _ = handle_ext_notification(
            &cf_hook_execution_notif_for_prompt("sess-foreign", "stop", Some("pid-old"), false),
            &mut app,
        );
        ...
        let _ = handle_ext_notification(
            &cf_hook_execution_notif_for_prompt("sess-foreign", "stop", Some("pid-new"), false),
            &mut app,
        );
        let agent = app.agents.get(&AgentId(0)).unwrap();
        let pending = agent
    ...
    fn foreign_stop_hooks_refused_at_idle_tail_marker() {
        ...
        let _ = handle_ext_notification(
            &cf_turn_completed_notif("sess-idle-foreign", "pid-new", "end_turn", false),
            &mut app,
        );

        // The marker's own batch (matching pid) merges…
        let _ = handle_ext_notification(
            &cf_hook_execution_notif_for_prompt(
                "sess-idle-foreign",
                "stop",
                Some("pid-new"),
                false,
    ...
    fn stamped_stop_hooks_merge_past_interleaved_tail_block() {
        ...
        let _ = handle_ext_notification(
            &cf_turn_completed_notif("sess-interleaved", "pid-new", "end_turn", false),
            &mut app,
        );
        app.agents
            .get_mut(&AgentId(0))
    ...
    fn same_name_stash_repeat_goes_standalone() {
        ...
        let _ = handle_ext_notification(
            &cf_hook_execution_notif("sess-stash-dup", "stop", false),
            &mut app,
        );
        let _ = handle_ext_notification(
            &cf_hook_execution_notif("sess-stash-dup", "stop", false),
    ...
    fn stash_key_prefers_wire_prompt_id() {
        ...
        let _ = handle_ext_notification(
            &cf_hook_execution_notif_for_prompt("sess-wire-key", "stop", Some("pid-a"), false),
            &mut app,
        );
    ...
    fn session_end_stop_hooks_without_live_turn_stay_standalone() {
        ...
        let _ = handle_ext_notification(
            &cf_hook_execution_notif("sess-end", "stop", false),
            &mut app,
        );
    ...
    fn non_stop_lifecycle_hooks_keep_standalone_block() {
        ...
        let _ = handle_ext_notification(
            &cf_hook_execution_notif("sess-ls", "session_start", false),
            &mut app,
        );
    ...
    fn replayed_turn_completed_blocks_session_loaded_adoption() {
        ...
        let affected = handle_ext_notification(
            &cf_turn_completed_notif("sess-1", "p-run", "end_turn", true),
            &mut app,
        );
        assert!(
            !affected,
    ...
    fn background_driver_live_turn_completed_arms_reconcile_and_reports_change() {
        ...
        let affected = handle_ext_notification(
            &cf_turn_completed_notif("sess-bg", "pid-bg", "cancelled", false),
            &mut app,
        );
        assert!(
            affected,
    ...
    fn second_load_does_not_inherit_first_loads_replay_window_state() {
        ...
        let _ = handle_ext_notification(
            &cf_turn_completed_notif("sess-1", "p-first", "end_turn", true),
            &mut app,
        );
        assert!(
            app.agents[&id]
    ...
    fn wake_stop_hooks_marker_first_attach() {
        ...
        let _ = handle_ext_notification(
            &cf_wake_turn_completed_notif("sess-wake-marker1st", "task-completed-bg1", None),
            &mut app,
        );
        assert_eq!(
    ...
    fn late_wake_stop_attaches_to_wake_marker_not_running_local_turn() {
        ...
        let _ = handle_ext_notification(
            &cf_wake_turn_completed_notif("sess-late-wake", "task-completed-bg1", None),
            &mut app,
        );
        assert_eq!(
            last_marker_stop_hook_groups(&app.agents[&AgentId(0)].scrollback),
    ...
    fn wake_repeat_stop_after_marker_renders_standalone() {
        ...
        let _ = handle_ext_notification(
            &cf_wake_turn_completed_notif("sess-wake-dup", "task-completed-bg1", None),
            &mut app,
        );
        for _ in 0..2 {
            let _ = handle_ext_notification(
