...
pub enum SwitchModelError {
    ...
    /// `acp::Error.data`.
    IncompatibleAgent {
        error: cf_shell::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Action {
    ...
    OpenExtensionsModal {
        ...
        trigger: cf_telemetry::events::ExtensionsModalTrigger,
    },
    ...
    ExecuteHooksAction(cf_hooks_plugins_types::HooksAction),
    /// Execute a plugins management action from the modal.
    ExecutePluginsAction(cf_hooks_plugins_types::PluginsAction),
    /// Execute a marketplace management action from the modal.
    ExecuteMarketplaceAction(cf_hooks_plugins_types::MarketplaceAction),
    ...
    AnnouncementsOpenCta(cf_telemetry::events::AnnouncementCtaSurface),
...
///
/// Both variants write to `~/.qidi/config.toml` and route ACP
...
pub enum Effect {
    ...
    ScanForeignSessions {
        cwd: std::path::PathBuf,
        compat: cf_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.qidi/config.toml`.
    ...
    /// Persist a typed setting to `~/.qidi/config.toml`. On failure,
    ...
    /// registry (`~/.qidi/active_sessions.json`).
    ...
    FetchPromptSuggestion {
        agent_id: AgentId,
        generation: u64,
        /// Suggestion model resolved by the pager (`cf-tools-0.1` when the
...
pub enum TaskResult {
    ...
    WorktreeForked {
        ...
        restore_degree: Option<cf_workspace::session::git::RestoreDegree>,
    },
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<cf_workspace::foreign_sessions::RecentForeignSession>,
    },
    ...
    BgTaskKilled {
        session_id: String,
        task_id: String,
        outcome: Option<cf_tools::types::KillOutcome>,
    },
    ...
    ChangelogFetched {
        ...
        entries: Vec<cf_shell::util::changelog::ChangelogEntry>,
    },
    ...
    HooksListLoaded {
        agent_id: AgentId,
        result: Result<cf_hooks_plugins_types::HooksListResponse, String>,
    },
    /// Plugins list fetched from shell.
    PluginsListLoaded {
        agent_id: AgentId,
        result: Result<cf_hooks_plugins_types::PluginsListResponse, String>,
    },
    /// Hooks action completed.
    HooksActionResult {
        agent_id: AgentId,
        result: Result<cf_hooks_plugins_types::ActionOutcome, String>,
    },
    ...
    MarketplaceListLoaded {
        agent_id: AgentId,
        result: Result<cf_hooks_plugins_types::MarketplaceListResponse, String>,
    },
    ...
    SkillsListLoaded {
        agent_id: AgentId,
        result: Result<Vec<cf_tools::implementations::skills::types::SkillInfo>, String>,
    },
    ...
    SessionInfoComplete {
        agent_id: AgentId,
        info: Box<cf_shell::session::SessionInfoResponse>,
        text: String,
    },
    ...
    AuthCopiedTimeout,
    DeepSearchResults {
        results: Vec<cf_shell::extensions::session_search::SearchSessionHit>,
        seq: u64,
    },
    ...
    GateRefreshed {
        settings: Option<cf_shell::util::config::RemoteSettings>,
    },
