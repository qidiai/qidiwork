...
pub struct PluginCtaState {
    ...
    pub candidates: Vec<cf_hooks_plugins_types::MarketplacePluginEntry>,
...
pub struct AgentView {
    ...
    pub context_state: Option<cf_shell::session::ContextInfo>,
    ...
    pub(crate) deferred_session_mode: Option<cf_tools::types::SessionMode>,
    ...
    match kind {
        ...
        LocalQuestionKind::CreditLimitUpsell { choices } => {
            ...
            let choice = choices
                .get(*idx)
                .copied()
                .unwrap_or(cf_telemetry::events::CreditLimitChoice::PayAsYouGo);
            cf_telemetry::session_ctx::log_event(
                cf_telemetry::events::CreditLimitUpsellClicked {
        ...
        LocalQuestionKind::FreeUsageUpsell { source } => {
            ...
            cf_telemetry::session_ctx::log_event(
                cf_telemetry::events::SuperGrokUpsellClicked {
                    source,
                    auth_method: None,
                },
            );
...
fn translate_project_select(
    ...
    use cf_telemetry::events::{ProjectPickerOutcome, ProjectPickerSelected};
    let project_dir_options = resolved_paths.len().saturating_sub(1);
    let emit = |outcome: ProjectPickerOutcome| {
        cf_telemetry::session_ctx::log_event(ProjectPickerSelected {
            outcome,
...
fn remember_mode_enabled() -> bool {
    let path = cf_tools::util::grok_home::grok_home().join("config.toml");
