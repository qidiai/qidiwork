...
impl AgentView {
    ...
    fn log_extensions_modal_action(
        &self,
        action: &str,
        input_method: cf_telemetry::events::ExtensionsInputMethod,
    ) {
    ...
    fn log_extensions_modal_action_with(
        ...
        if let Some(ref state) = self.extensions_modal {
            cf_telemetry::session_ctx::log_event(
                cf_telemetry::events::ExtensionsModalAction {
    ...
    pub(super) fn handle_extensions_modal_key(
            ...
            })
        {
            self.log_extensions_modal_action(
                "open_connectors",
                cf_telemetry::events::ExtensionsInputMethod::Keyboard,
            );
            return self.execute_modal_button_action(
        ...
        if let Some(ch) = chrome_shortcut_ch {
                ...
                .and_then(|s| crate::views::extensions_modal::resolve_key(s.active_tab, ch))
            {
                self.log_extensions_modal_resolved_action(
                    ch,
                    &action,
                    cf_telemetry::events::ExtensionsInputMethod::Mouse,
                );
    ...
    fn execute_modal_button_action(
        ...
        match action {
            ButtonAction::HooksAction(hooks_action) => {
                if let Some(ref mut state) = self.extensions_modal {
                    state.modal_message = None;
                    if matches!(hooks_action, cf_hooks_plugins_types::HooksAction::Reload) {
            ...
            ButtonAction::PluginsAction(plugins_action) => {
                if let Some(ref mut state) = self.extensions_modal {
                    ...
                    if matches!(
                        plugins_action,
                        cf_hooks_plugins_types::PluginsAction::Reload
                    ) {
                        // Reload rebuilds the entire plugin registry -- show
                        // tab-level "Loading..." instead of a single-entry badge.
                        state.pending_action = Some("Reloading...".into());
                        ...
                        let label = if matches!(
                            plugins_action,
                            cf_hooks_plugins_types::PluginsAction::Update { .. }
                        ) {
                            "Updating..."
                        } else {
                            "Processing..."
                        ...
                        && let Some(hook) = data.hooks.get(idx)
                    {
                        ...
                        return self.execute_modal_button_action(
                            crate::views::extensions_modal::ButtonAction::HooksAction(
                                cf_hooks_plugins_types::HooksAction::Remove { path },
                            ),
                        );
                    }
                }
            ...
            ButtonAction::ToggleSelectedHook => {
                    ...
                    && let Some(hook) = data.hooks.get(idx)
                {
                    ...
                    if is_collapsed {
                        ...
                        let group_hooks: Vec<&cf_hooks_plugins_types::HookInfo> = data
                            .hooks
                            .iter()
                        ...
                        let action = cf_hooks_plugins_types::HooksAction::ToggleSource {
                            hook_names,
                            disable: any_enabled,
                        };
                        ...
                        let action = if hook.disabled {
                            cf_hooks_plugins_types::HooksAction::Enable {
                                hook_name: hook.name.clone(),
                            }
                        } else {
                            cf_hooks_plugins_types::HooksAction::Disable {
                                hook_name: hook.name.clone(),
                            }
                        };
                        return self.execute_modal_button_action(ButtonAction::HooksAction(action));
            ...
            ButtonAction::ToggleSelectedPlugin => {
                    ...
                    && let Some(plugin) = data.plugins.get(idx)
                {
                    let action = if plugin.enabled {
                        cf_hooks_plugins_types::PluginsAction::Disable {
                            plugin_id: plugin.id.clone(),
                        }
                    } else {
                        cf_hooks_plugins_types::PluginsAction::Enable {
                            plugin_id: plugin.id.clone(),
                        }
                    };
            ...
            ButtonAction::UninstallSelectedPlugin => {
                if let Some(ref mut state) = self.extensions_modal
                    ...
                    && let Some(plugin) = data.plugins.get(idx)
                {
                    let action = cf_hooks_plugins_types::PluginsAction::Uninstall {
                        plugin_id: plugin.id.clone(),
                        confirmed: false,
                    };
                    return self.execute_modal_button_action(ButtonAction::PluginsAction(action));
            ...
            ButtonAction::UpdateSelectedPlugin => {
                ...
                if let Some(ref state) = self.extensions_modal
                    ...
                    && let Some(plugin) = data.plugins.get(idx)
                {
                    let action = cf_hooks_plugins_types::PluginsAction::Update {
            ...
            ButtonAction::InstallSelectedMarketplacePlugin => self
                .execute_selected_marketplace_plugin_action(
                    ...
                    |source_url_or_path, plugin_relative_path| {
                        cf_hooks_plugins_types::MarketplaceAction::Install {
                            source_url_or_path,
            ...
            ButtonAction::UpdateSelectedMarketplacePlugin => self
                .execute_selected_marketplace_plugin_action(
                    "Updating...",
                    |source_url_or_path, plugin_relative_path| {
                        cf_hooks_plugins_types::MarketplaceAction::Update {
                            source_url_or_path,
                            plugin_relative_path,
                        }
                    },
            ...
            ButtonAction::UninstallSelectedMarketplacePlugin => self
                .execute_selected_marketplace_plugin_action(
                    "Uninstalling...",
                    |source_url_or_path, plugin_relative_path| {
                        cf_hooks_plugins_types::MarketplaceAction::Uninstall {
                            source_url_or_path,
                            plugin_relative_path,
                        }
                    },
                        ...
                        if let Some(source) = source {
                            ...
                            state.modal_message = Some(
                                crate::views::extensions_modal::ModalMessage::MarketplaceConfirmation {
                                    message: msg,
                                    action:
                                        cf_hooks_plugins_types::MarketplaceAction::RemoveSource {
    ...
    fn execute_selected_marketplace_plugin_action(
        ...
        make_action: impl FnOnce(String, String) -> cf_hooks_plugins_types::MarketplaceAction,
    ) -> InputOutcome {
        if let Some(ref mut state) = self.extensions_modal {
            use crate::views::extensions_modal::TabDataState;
            if let TabDataState::Loaded(ref response) = state.marketplace_data
    ...
    fn update_selected_marketplace_plugin_dispatches_update_and_sets_pending_state() {
        ...
        modal.marketplace_data =
            TabDataState::Loaded(cf_hooks_plugins_types::MarketplaceListResponse {
                sources: vec![cf_hooks_plugins_types::MarketplaceScanResult {
...
mod extensions_action_target_tests {
    ...
    fn plugin_info(name: &str, enabled: bool) -> cf_hooks_plugins_types::PluginInfo {
    ...
    fn plugins_toggle_and_uninstall_resolve_name_and_state() {
        ...
        modal.plugins_data = TabDataState::Loaded(cf_hooks_plugins_types::PluginsListResponse {
        ...
        match outcome {
            crate::app::app_view::InputOutcome::Action(
                crate::app::actions::Action::ExecutePluginsAction(
                    cf_hooks_plugins_types::PluginsAction::Update { plugin_id },
                ),
    ...
    fn skills_toggle_resolves_name_and_resulting_state() {
        let skill = cf_tools::implementations::skills::types::SkillInfo {
            name: "my-skill".into(),
            enabled: false,
            ..Default::default()
        };
    ...
    fn hooks_modal(hooks: Vec<cf_hooks_plugins_types::HookInfo>) -> ExtensionsModalState {
        let mut modal = ExtensionsModalState::new(ExtensionsTab::Hooks);
        modal.hooks_data = TabDataState::Loaded(cf_hooks_plugins_types::HooksListResponse {
            hooks,
    ...
    fn loading_data_yields_no_target() {
        ...
        for action in [
            ...
            ButtonAction::ToggleSelectedMcpServer,
            ButtonAction::InstallSelectedMarketplacePlugin,
            ButtonAction::PluginsAction(cf_hooks_plugins_types::PluginsAction::Reload),
        ] {
            let (target, enabled) = AgentView::extensions_action_target(&modal, &action);
