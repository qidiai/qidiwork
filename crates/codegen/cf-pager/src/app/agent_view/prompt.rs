    ...
    pub(in crate::app) fn handle_prompt_key(
            ...
            && self.prompt.prompt_suggestion_visible()
        {
            if key!(Tab).matches(key) || key!(Right).matches(key) {
                if self.prompt.accept_prompt_suggestion() {
                    ...
                    cf_telemetry::session_ctx::log_event(
                        cf_telemetry::events::PromptSuggestion {
                            action: cf_telemetry::events::PromptSuggestionAction::Accepted,
                            chars,
                            words,
                        },
                    );
                ...
                && self.prompt.text().is_empty()
            {
                ...
                cf_telemetry::session_ctx::log_event(
                    cf_telemetry::events::PromptSuggestion {
                        action: cf_telemetry::events::PromptSuggestionAction::Dismissed,
                        chars,
                        words,
                    },
                );
            ...
            match self.prompt.handle_key(key) {
                PromptEvent::Edited => {
                    if undo_tip_accepted {
                        cf_telemetry::session_ctx::log_event(
                            cf_telemetry::events::ContextualTip {
