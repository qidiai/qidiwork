...
impl AgentView {
    ...
    pub fn apply_full_context_info(&mut self, next: cf_shell::session::ContextInfo) {
        if self.chat_kind {
    ...
    pub fn apply_context_used(&mut self, used: u64, total: u64) {
        ...
        match self.context_state.as_mut() {
            Some(snap) => {
                ...
                snap.usage_pct = cf_token_estimation::usage_percentage_u8(used, snap.total);
                snap.free_tokens = cf_token_estimation::free_tokens(snap.total, used);
            }
            None => {
                self.context_state = Some(cf_shell::session::ContextInfo::from_notification(
                    used, total,
                ));
            }
        }
