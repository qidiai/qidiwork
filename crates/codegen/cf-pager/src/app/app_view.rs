...
/// [`cf_shell::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<cf_shell::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: cf_shell::util::config::WorktreeHintMode) -> Self {
        use cf_shell::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        cf_shell::util::config::WorktreeHintMode::from_config_str(s).into()
    }
    ...
    pub fn resolve_from_hints_value(hints: Option<&toml::Value>) -> (Self, Self) {
        let (new_session, fork) =
            cf_shell::util::config::WorktreeHintMode::resolve_pair(hints);
...
/// [`cf_workspace::folder_trust::decide`] verdict; when the feature flag
...
/// Cap for the `QIDI_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets
/// exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `QIDI_ESC_DOUBLE_PRESS_MS`-overridable
...
pub(crate) fn esc_double_press_ttl() -> Duration {
    ...
    *TTL.get_or_init(|| parse_esc_ttl(std::env::var("QIDI_ESC_DOUBLE_PRESS_MS").ok()))
}
...
/// (toolset) gate via [`cf_shell::tier::is_restricted_tier_name`] so
...
fn is_restricted_tier(tier: Option<&str>) -> bool {
    match tier {
        None => true,
        Some(t) => cf_shell::tier::is_restricted_tier_name(t),
    }
}
...
pub struct AppView {
    ...
    /// stays sans-IO.
    pub current_ui: cf_shell::agent::config::UiConfig,
    /// Working directory.
    pub cwd: PathBuf,
    /// Whether the project picker question has already been shown this session.
    pub project_picker_shown: bool,
    /// "Don't ask me again" opt-out from [`cf_shell::util::config::resolve_hints`];
    ...
    /// Current appearance config (hot-reloadable from ~/.qidi/pager.toml).
    ...
    /// Scroll-diagnostics HUD (`QIDI_SCROLL_DEBUG` env / `/scroll-debug`).
    ...
    /// Release-safe FPS HUD (`/debug fps`; `QIDI_FPS` env on release
    /// builds, where the dev overlay is compiled out) — see the module doc.
    pub fps_hud: crate::views::fps_hud::FpsHud,
    pub active_announcements: Vec<cf_announcements::RemoteAnnouncement>,
    ...
    pub announcement: Option<cf_announcements::RemoteAnnouncement>,
    ...
    /// Whether the plugin marketplace CTA is enabled. Env `QIDI_PLUGIN_CTA`
    ...
    /// non-selectable headers. Gated by `QIDI_SESSION_PICKER_GROUPED` env var
    ...
    /// back into the input box. Gated by `QIDI_CANCEL_REWIND` env /
    ...
    /// empty and inert outside `--minimal`; the `qidi-code-minimal` crate
    ...
    pub session_picker_content_results:
        Option<Vec<cf_shell::extensions::session_search::SearchSessionHit>>,
    ...
    pub(crate) foreign_session_compat:
        cf_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    /// `cf_shell::util::config::resolve_auto_permission_mode_enabled`.
    ...
    /// at startup and on settings toggles from `QIDI_CONTEXTUAL_HINTS` (master)
    /// > `[ui.contextual_hints]` user config > remote tier > default.
    pub contextual_hints: cf_shell::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<cf_shell::util::config::ContextualHintsRemote>,
    ...
    /// Office-artifact manifest watch (QidiWork); lazily armed while the
    /// bound task workspace exists. See [`crate::app::office_watch`].
    pub(crate) office_watch: Option<crate::app::office_watch::OfficeWatch>,
    ...
    pub pending_gate_verification: Option<cf_shell::auth::GateInfo>,
    ...
    /// `~/.qidi/config.toml`. `None` when the file/section is absent
    ...
    /// kill switch or `QIDI_VOICE_MODE=0` — the STT pipeline is not started and
...
impl AppView {
    ...
    pub fn gate_from_settings(
        rs: &cf_shell::util::config::RemoteSettings,
    ) -> Option<cf_shell::auth::GateInfo> {
        let msg = rs.gate_message.as_ref()?;
        if msg.is_empty() {
            return None;
        }
        Some(cf_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    pub fn apply_auth_meta(&mut self, meta: &cf_shell::auth::AuthMeta) {
        ...
        if was_gated && self.gate.is_none() {
            ...
            cf_telemetry::session_ctx::log_event(
                cf_telemetry::events::SubscriptionActivated {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_ui: cf_shell::agent::config::UiConfig::default(),
            ...
            auto_mode_gate: cf_shell::util::config::auto_permission_mode_enabled_from_disk(),
            ...
            voice_config: cf_voice::VoiceConfig::default(),
            voice_auth: None,
            voice_cmd_tx: None,
    ...
    pub fn deferred_model_switch_from_cli(
        ...
        acp::ModelId,
        Option<cf_shell::sampling::types::ReasoningEffort>,
    )> {
    ...
    pub fn voice_can_start_pipeline(&self) -> bool {
        self.voice_mode_enabled && cf_voice::AUDIO_SUPPORTED
    }
    ...
    fn voice_send(&self, cmd: cf_voice::VoiceCommand) {
    ...
    pub(crate) fn voice_begin_recording(&mut self, target: VoiceTarget, hold: bool) {
        self.voice_send(cf_voice::VoiceCommand::PttPress);
    ...
    pub(crate) fn voice_stop_keeping_final(&mut self) {
        ...
        self.voice_send(cf_voice::VoiceCommand::PttRelease);
    ...
    /// Rows the dev `QIDI_FPS` overlay occupies (0 in non-dev builds), so
...
struct WelcomeInputCtx<'a> {
    ...
    sp_content_results:
        &'a Option<Vec<cf_shell::extensions::session_search::SearchSessionHit>>,
        ...
        if matches!(ctx.auth_state, AuthState::Done) {
            if ctx.upgrade_cta_keyboard && key!('o', CONTROL).matches(key) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    cf_telemetry::events::AnnouncementCtaSurface::Keyboard,
                ));
            }
    ...
    if let Event::Mouse(mouse) = ev {
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        cf_telemetry::events::AnnouncementCtaSurface::Welcome,
                    ));
                }
...
impl AppView {
    ...
    fn draw_inner(&mut self, terminal: &mut PagerTerminal) {
            ...
            )
        {
            ...
            if crate::terminal::terminal_context()
                ...
                .osc22_cursor
            {
                cf_shell::util::with_locked_stderr(|stderr| {
                ...
                match *active_view {
                    ActiveView::Welcome => {
                        ...
                        if !has_access && !self.access_gate_shown_logged {
                            ...
                            cf_telemetry::session_ctx::log_event(
                                cf_telemetry::events::SuperGrokUpsellShown {
                                    source:
                                        cf_telemetry::events::SuperGrokUpsell::WelcomeScreen,
                                    auth_method: self
                                        .login_method_id
                                        .as_ref()
    ...
    /// Log [`cf_telemetry::events::AnnouncementCtaShown`] for each
    ...
    pub(crate) fn log_announcement_cta_impressions(&mut self) {
        use cf_telemetry::events::AnnouncementCtaSurface;
        ...
        let key = cf_announcements::announcement_hide_key(owner);
        ...
        for (surface, _) in surfaces.into_iter().filter(|(_, painted)| *painted) {
            ...
            {
                cf_telemetry::session_ctx::log_event(
                    cf_telemetry::events::AnnouncementCtaShown {
                        id: id.clone(),
                        source: surface,
                    },
                );
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: cf_shell::util::config::ResolvedContextualHints,
    ) {
    ...
    fn apply_clipboard_probe(
        ...
        if agent.show_ephemeral_tip(
            ...
            cf_telemetry::session_ctx::log_event(cf_telemetry::events::ContextualTip {
                tip: cf_telemetry::events::ContextualTipKind::ImageInput,
                action: cf_telemetry::events::ContextualTipAction::Shown,
            });
            return true;
        }
        false
...
pub(crate) mod tests {
    ...
    fn app_draw_drains_deferred_release_after_flush() {
        ...
        let mut terminal = cf_ratatui_inline::Terminal::with_options(
            backend,
            TerminalOptions {
    ...
    fn slash_gate_resyncs_when_critical_expires_between_pushes() {
        ...
        app.active_announcements = vec![cf_announcements::RemoteAnnouncement {
    ...
    fn needs_animation_gates_todo_badge_flash() {
        ...
        app.agents
            ...
            .unwrap()
            .todo
            .update_todos(vec![cf_shell::tools::TodoItem {
                content: "do the thing".into(),
                priority: Default::default(),
                status: cf_shell::tools::TodoStatus::InProgress,
                meta: None,
            }]);
        assert!(
    ...
    fn apply_auth_meta_hides_usage_for_team_users() {
        ...
        let meta = cf_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_shows_usage_for_personal_users() {
        ...
        let meta = cf_shell::auth::AuthMeta::default();
        app.apply_auth_meta(&meta);
        assert!(app.usage_visible);
    }
    #[test]
    ...
    fn apply_auth_meta_api_key_enables_voice_and_skips_tier_gate() {
        ...
        app.apply_auth_meta(&cf_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_clears_gate_on_subscription() {
        let mut app = test_app();
        app.gate = Some(cf_shell::auth::GateInfo {
            message: "Subscribe to use QIDI Code".into(),
            url: Some("https://grok.com/supergrok?referrer=cf-tools".into()),
            label: None,
        });
    ...
    fn apply_auth_meta_gate_unchanged_when_still_gated() {
        let mut app = test_app();
        let gate = cf_shell::auth::GateInfo {
    ...
    fn welcome_ctrl_u_update_keeps_priority_over_foreign_resume() {
        ...
        app.apply_foreign_resume_detection(
            launch_token,
            &canonical_cwd,
            Some(cf_workspace::foreign_sessions::RecentForeignSession {
                tool: cf_workspace::foreign_sessions::ForeignSessionTool::Cursor,
    ...
    fn overlay_left_arrow_file_search_open_does_not_exit() {
        let (mut app, id) = neutral_overlay_app();
        {
            ...
            agent.prompt.file_search.set_test_state(
                ctx,
                vec![cf_workspace::file_system::FuzzyMatchResult {
