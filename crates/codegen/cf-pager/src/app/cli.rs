...
pub enum Command {
    /// Run QIDI Code without the interactive UI
    Agent(Box<AgentArgs>),
    /// Show the configuration QIDI Code discovers for this directory
    Inspect {
...
Examples:
  qidi wrap docker exec -it my-container bash
  qidi wrap kubectl exec -it my-pod -- bash

See ~/.qidi/README.md for more information.
")]
    Wrap(WrapArgs),
    ...
    /// `QIDI_WORKSPACE_COMMAND=1` to enable it locally for testing.
    #[command(hide = true)]
    ...
    /// `~/.qidi/config.toml` or when the `QIDI_AGENT_DASHBOARD=0` env
    /// var is set.
    Dashboard,
}
...
pub struct AgentArgs {
    ...
    pub cf_api_base_url: Option<String>,
    /// Agent runtime mode
    #[command(subcommand)]
...
pub struct ServeArgs {
    ...
    #[arg(long, env = "QIDI_AGENT_SECRET")]
...
fn version_with_channel() -> &'static str {
    ...
    V.get_or_init(|| {
        let label = cf_update::channel_label();
        cf_version::display_version_with_commit(env!("VERSION_WITH_COMMIT"), label)
    })
}
...
pub struct PagerArgs {
    ...
    #[arg(long)]
    pub cwd: Option<PathBuf>,
    /// Use a custom leader socket path instead of the default `~/.qidi/leader.sock`.
    #[arg(
        long = "leader-socket",
        value_name = "PATH",
        global = true,
    ...
    /// persists per-segment markdown to grep. Sets `QIDI_COMPACTION_MODE`.
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            cf_shell::agent::config::PermissionMode::VALID_VALUES
        )
    )]
    ...
    #[arg(long, env = "QIDI_SANDBOX", value_name = "PROFILE")]
    ...
    pub fullscreen: bool,
    /// Write sampling events to ~/.qidi/logs/sampling.jsonl.
    #[arg(long = "log-sampling", env = "QIDI_LOG_SAMPLING", hide = true)]
...
impl PagerArgs {
    ...
    /// explicit `--sandbox`/`QIDI_SANDBOX` that differs from the saved profile is
    ...
    pub fn saved_resume_profile(&self) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                cf_shell::session::persistence::resumed_session_sandbox_profile(
                    Some(&id),
                    cwd,
                )
            }
            ResumeTarget::MostRecentForCwd => {
                cf_shell::session::persistence::resumed_session_sandbox_profile(None, cwd)
            }
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<cf_sandbox::ProfileName>().ok()
                    != s.parse::<cf_sandbox::ProfileName>().ok() =>
            {
