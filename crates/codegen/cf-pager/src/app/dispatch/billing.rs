...
use cf_telemetry::events::{SuperGrokUpsell, SuperGrokUpsellClicked};
...
pub(crate) fn acp_error_is_free_usage_exhausted(err: &agent_client_protocol::Error) -> bool {
    err.data
        .as_ref()
        .and_then(cf_shell::sampling::error::error_detail_from_data)
        .as_deref()
...
pub(super) fn open_credit_limit_upsell(
    ...
    log_event(cf_telemetry::events::CreditLimitUpsellShown {
        surface: cf_telemetry::events::CreditLimitUpsellSurface::QuestionModal,
        max_tier: false,
        pay_as_you_go: payg_telemetry,
        unified_billing,
    });
    ...
    .with_local_kind(LocalQuestionKind::CreditLimitUpsell {
        choices: vec![
            cf_telemetry::events::CreditLimitChoice::UpgradeTier,
            second_choice,
        ],
    })
    .with_no_freeform();
...
fn open_supergrok_upsell(
    ...
    let options = vec![
        ...
        QuestionOption {
            ...
            description: "Get the most out of QIDI Code. Highest usage limits.".into(),
            preview: None,
...
pub(super) fn handle_check_subscription_complete(
    ...
    let applied = match meta {
        Some(meta_val) => {
            match serde_json::from_value::<cf_shell::auth::AuthMeta>(meta_val) {
                Ok(auth_meta) => {
...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if let Some(meta_val) = meta
        && let Ok(auth_meta) = serde_json::from_value::<cf_shell::auth::AuthMeta>(meta_val)
    {
        app.apply_auth_meta(&auth_meta);
    }
