...
pub(super) fn plugin_cta_candidates(
    response: cf_hooks_plugins_types::MarketplaceListResponse,
) -> (Vec<cf_hooks_plugins_types::MarketplacePluginEntry>, bool) {
    let mut candidates = Vec::new();
    let mut official_source_present = false;
    for source in response.sources {
        let is_official = source.source_name == cf_plugin_marketplace::OFFICIAL_SOURCE_NAME
            || cf_plugin_marketplace::is_official_source_url(&source.source_url_or_path);
        if !is_official {
            continue;
        }
...
pub(super) fn cta_install_relative_path(
    candidates: &[cf_hooks_plugins_types::MarketplacePluginEntry],
    name: &str,
) -> String {
    candidates
        .iter()
...
pub(super) fn cta_install_error_category(
    ...
    use cf_hooks_plugins_types::OutcomeStatus;
    match result {
...
pub(super) fn plugin_cta_phase_for(
    ...
    use cf_plugin_marketplace::matcher::{KeywordCandidate, match_plugin_keyword};
...
pub(super) fn handle_cta_plugin_install_done(
    ...
    log_event(cf_telemetry::events::PluginCtaInstalled {
    ...
    let install_result = match result {
        Ok(outcome) if outcome.status == cf_hooks_plugins_types::OutcomeStatus::Success => {
...
pub(super) fn handle_cta_plugin_reload_done(
    ...
    let reload_result = match result {
        Ok(outcome) if outcome.status == cf_hooks_plugins_types::OutcomeStatus::Success => Ok(()),
        Ok(outcome) => Err(crate::app::effects::sanitize_user_error(&outcome.message)),
        Err(e) => Err(e),
    };
    match reload_result {
        ...
        Ok(servers) => {
            ...
            if needs_auth {
                ...
                log_event(cf_telemetry::events::ExtensionsModalOpened {
                    trigger: cf_telemetry::events::ExtensionsModalTrigger::AuthHandoff,
    ...
    match result {
        Ok(mut response) => {
            ...
            if let Some(agent) = app.agents.get_mut(&agent_id) {
                ...
                if enabled {
                    agent.plugin_cta.dismissed = cf_shell::config::dismissed_plugin_ctas();
                }
                ...
                if matches!(
                    ...
                    {
                        log_event(cf_telemetry::events::PluginCtaImpression {
