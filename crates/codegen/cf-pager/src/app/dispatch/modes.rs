...
/// from `~/.qidi/sessions/<urlencoded_cwd>/<session_id>/plan.md`.
...
pub(super) fn set_plan_mode(
    ...
    let mode_id = acp::SessionModeId::new(if new {
        cf_tools::types::SessionMode::Plan.as_id()
    } else {
...
pub(super) fn set_yolo_mode_inner(app: &mut AppView, new: bool) {
    ...
    if previous_state != new {
        cf_telemetry::session_ctx::log_event(cf_telemetry::events::YoloToggled {
            enabled: new,
            previous_state,
            trigger: cf_telemetry::events::YoloTrigger::Pager,
        });
...
pub(super) fn dispatch_cycle_mode(app: &mut AppView) -> Vec<Effect> {
    ...
    {
        log_event(cf_telemetry::events::ContextualTip {
            tip: cf_telemetry::events::ContextualTipKind::PlanMode,
            action: cf_telemetry::events::ContextualTipAction::Accepted,
        });
...
fn dispatch_cycle_mode_inner(app: &mut AppView) -> Vec<Effect> {
    ...
    match (in_plan, in_auto, in_yolo) {
        ...
        (false, false, false) => {
            ...
            vec![Effect::SetSessionMode {
                session_id,
                mode_id: acp::SessionModeId::new(cf_tools::types::SessionMode::Plan.as_id()),
            }]
        }
