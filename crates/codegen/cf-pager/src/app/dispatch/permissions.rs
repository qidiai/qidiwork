...
///
/// Special case for [`cf_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID`]:
...
/// "always-approve"` to `~/.qidi/config.toml`, and fire the
...
pub(super) fn dispatch_permission_select(
    ...
    let enable_always_approve =
        option_id.0.as_ref() == cf_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID;
    ...
    let steers_next_cursor = !enable_always_approve
        && option_id.0.as_ref() != cf_workspace::permission::ALLOW_EDITS_SESSION_OPTION_ID;
        ...
        && perm.bash_selection_count > 0
    {
        ...
        serde_json::to_value(cf_workspace::permission::BashCommandSelectedTerms {
            command_parts: parts,
        })
        .ok()
        .and_then(|v| v.as_object().cloned())
