...
pub(super) fn dispatch_show_undo_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(cf_telemetry::events::ContextualTip {
            tip: cf_telemetry::events::ContextualTipKind::Undo,
...
pub(in crate::app) fn show_small_screen_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(cf_telemetry::events::ContextualTip {
            tip: cf_telemetry::events::ContextualTipKind::SmallScreen,
            action: cf_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
...
pub(super) fn dispatch_show_word_select_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(cf_telemetry::events::ContextualTip {
            tip: cf_telemetry::events::ContextualTipKind::WordSelect,
            action: cf_telemetry::events::ContextualTipAction::Shown,
        });
    }
...
fn maybe_show_send_now_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(cf_telemetry::events::ContextualTip {
            tip: cf_telemetry::events::ContextualTipKind::SendNow,
            action: cf_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
    ...
    if !literal && trimmed.starts_with('/') {
        ...
        let exec_result = {
            ...
            if let Some(invocation) = parse_invocation(trimmed) {
                ...
                {
                    use cf_telemetry::events::{PagerCommandSource, PagerSlashCommand};
                    use cf_telemetry::session_ctx::log_event;
