...
fn cta_outcome_reload(
    status: cf_hooks_plugins_types::OutcomeStatus,
    message: &str,
...
mod cta_e2e {
    ...
    fn figma_candidate() -> cf_hooks_plugins_types::MarketplacePluginEntry {
        let mut entry = cta_entry("figma", "not_installed");
    ...
    fn dismissed_plugin_suppressed_on_next_recompute() {
        ...
        let dismissed = cf_shell::config::dismissed_plugin_ctas_in_file(&config_path);
        let matched =
        ...
        cf_shell::config::add_dismissed_plugin_cta_to_file("figma", &config_path)
