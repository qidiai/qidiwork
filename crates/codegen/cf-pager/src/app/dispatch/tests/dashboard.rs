...
/// `QIDI_OPEN_DASHBOARD_AT_STARTUP` env var and stashes
...
fn dashboard_confirm_worktree_applies_pending_model_and_plan() {
    ...
    if let Some(d) = app.dashboard.as_mut() {
        d.pending_model = Some(crate::views::dashboard::PendingDispatchModel {
            id: model_id.clone(),
            effort: Some(cf_shell::sampling::types::ReasoningEffort::High),
    ...
    assert_eq!(
        ...
        Some((
            model_id,
            Some(cf_shell::sampling::types::ReasoningEffort::High)
        )),
...
fn dashboard_overlay_cycle_non_overlay_noop_when_dashboard_disabled() {
    let _guard =
        crate::test_util::EnvVarGuard::set("QIDI_AGENT_DASHBOARD", std::path::Path::new("0"));
    let mut app = test_app_with_agent();
    let id1 = AgentId(0);
    mark_agent_nonempty(&mut app, id1);
    let id2 = AgentId(1);
