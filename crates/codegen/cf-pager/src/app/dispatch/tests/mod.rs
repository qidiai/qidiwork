...
fn cta_entry(name: &str, status: &str) -> cf_hooks_plugins_types::MarketplacePluginEntry {
...
/// do not rely on setting `QIDI_HOME` mid-process). Caller must remove `sess_dir`.
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    let home = cf_shell::util::grok_home::grok_home();
    let encoded = cf_shell::util::grok_home::encode_cwd_dirname(&cwd.to_string_lossy());
        ...
        vec![
            acp::PermissionOption::new(
                acp::PermissionOptionId::new(Arc::from(
                    cf_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
                )),
...
const POLICY_WARNING: &str =
    cf_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
...
fn push_synthetic_permission(
    ...
    let request = cf_acp_lib::AcpArgs {
