...
fn turn_end_fetches_prompt_suggestion_when_enabled() {
    ...
    // No `cf-tools-0.1` in the test catalog and no env override →
    // `None` on the wire; the shell then uses its own `cf-tools-0.1`
...
/// The always-on pipeline end to end, with `QIDI_SUGGESTIONS` semantics
