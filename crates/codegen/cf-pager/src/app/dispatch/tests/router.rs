...
fn critical_announcement(id: &str) -> cf_announcements::RemoteAnnouncement {
...
fn promo_announcement(id: &str) -> cf_announcements::RemoteAnnouncement {
    cf_announcements::RemoteAnnouncement {
        id: Some(id.into()),
        message: Some(format!("{id} message")),
        severity: Some("promo".into()),
        cta: Some(cf_announcements::AnnouncementCta {
            label: Some("Go".into()),
...
/// the promo url (observed via the `QIDI_TEST_OPEN_URL_FILE` seam, from every
...
#[serial_test::serial(QIDI_TEST_OPEN_URL_FILE)]
#[test]
fn announcements_open_cta_opens_promo_and_noops_under_critical() {
    ...
    unsafe { std::env::set_var("QIDI_TEST_OPEN_URL_FILE", &url_file) };
    ...
    unsafe { std::env::remove_var("QIDI_TEST_OPEN_URL_FILE") };
    let _ = std::fs::remove_file(&url_file);
}
...
fn tick_propagates_available_commands_to_bootstrap() {
    ...
    let skill_meta = serde_json::json!(
        { "scope" : "user", "path" : "/home/user/.qidi/skills/pick-best/SKILL.md", }
    );
