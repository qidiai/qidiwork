...
fn voice_on_welcome_creates_session_and_records() {
    ...
    assert!(matches!(
        rx.try_recv(),
        Ok(cf_voice::VoiceCommand::PttPress)
    ));
}
#[test]
fn voice_final_routes_to_recording_session_not_active_view() {
    ...
    crate::voice::handle_voice_event(
        &mut app,
        cf_voice::VoiceEvent::UtteranceFinal {
            text: "hello".into(),
        },
    );
...
fn voice_auto_stops_when_leaving_recording_session() {
    ...
    assert!(matches!(
        rx.try_recv(),
        Ok(cf_voice::VoiceCommand::PttRelease)
    ));
}
#[test]
...
fn worktree_session_preserves_subdirectory_offset() {
    let mut app = test_app();
    app.cwd = PathBuf::from("/repo/crates/codegen/qidi-code");
    ...
    let id = AgentId(0);
    let worktree_root = PathBuf::from("/home/user/.qidi/worktrees/repo/pager-123");
    let session_cwd = worktree_root.join("crates/codegen/qidi-code");
    let effects = dispatch(
        Action::TaskComplete(TaskResult::WorktreeSessionCreated {
            agent_id: id,
...
/// trust. QIDI_HOME-isolated so the write hits a temp store, not the real one.
...
fn bg_task_killed_no_op_for_unknown_session() {
    ...
    let effects = dispatch(
        Action::TaskComplete(TaskResult::BgTaskKilled {
            ...
            outcome: Some(cf_tools::types::KillOutcome::AlreadyExited),
        }),
        &mut app,
    );
    assert!(effects.is_empty());
...
fn cycle_mode_pre_session_blocked_by_policy_pin() {
    ...
    app.yolo_policy_block = Some(POLICY_WARNING);
    {
        ...
        agent.deferred_session_mode = Some(cf_tools::types::SessionMode::Plan);
    }
...
fn dispatch_cycle_mode_pre_session_cycles_locally_and_creates_session() {
    ...
    assert_eq!(
        ...
        Some(cf_tools::types::SessionMode::Plan),
