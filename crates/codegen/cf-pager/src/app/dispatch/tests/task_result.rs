...
fn foreign_resume_hint(
    tool: cf_workspace::foreign_sessions::ForeignSessionTool,
) -> cf_workspace::foreign_sessions::RecentForeignSession {
    cf_workspace::foreign_sessions::RecentForeignSession {
        tool,
...
fn foreign_resume_results_require_launch_token_and_canonical_cwd() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                cf_workspace::foreign_sessions::ForeignSessionTool::Cursor,
            )),
        }),
        &mut launch,
    );
    assert_eq!(
        ...
        Some(cf_workspace::foreign_sessions::ForeignSessionTool::Cursor)
    );
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                cf_workspace::foreign_sessions::ForeignSessionTool::Codex,
            )),
        }),
        &mut stale,
    );
...
fn marketplace_list_loaded_sanitizes_components_at_ingestion() {
    ...
    entry.components = Some(cf_hooks_plugins_types::PluginComponents {
        skills: vec![cf_hooks_plugins_types::ComponentItem {
            name: "evil\u{1b}[31mskill".into(),
            description: Some(format!("\u{7}{}", "d".repeat(300))),
        }],
        ..Default::default()
    });
    let response = cf_hooks_plugins_types::MarketplaceListResponse {
...
fn plugins_action_success_sets_result_notice_and_autoreload_preserves_it() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::PluginsActionResult {
            agent_id: id,
            result: Ok(cf_hooks_plugins_types::ActionOutcome {
...
fn uninstall_result_notice_is_footer_only_not_row_anchored() {
    ...
    let id = AgentId(0);
    {
        ...
        modal.last_plugins_action = Some(cf_hooks_plugins_types::PluginsAction::Uninstall {
...
fn switch_to_non_reasoning_model_clears_persisted_effort() {
    ...
    app.agents
        ...
        .insert(
            model_id.clone(),
            acp::ModelInfo::new(model_id.clone(), "QIDI Code".to_string()),
        );
    app.agents
        .get_mut(&id)
        .unwrap()
...
fn switch_model_incompatible_agent_shows_question_modal() {
    ...
    let err = cf_shell::agent::config::ModelSwitchIncompatibleAgentError {
        code: "MODEL_SWITCH_INCOMPATIBLE_AGENT".into(),
        active_agent_type: "cf-tools".into(),
...
fn same_agent_type_switch_no_modal() {
    ...
    let mut app = test_app_with_agent();
    let id = AgentId(0);
    let model_a = acp::ModelId::new(std::sync::Arc::from("cf-tools-a"));
    let model_b = acp::ModelId::new(std::sync::Arc::from("cf-tools-b"));

    // Add both models to the catalog (no agentType → both use cf-tools).
    let agent = app.agents.get_mut(&id).unwrap();
    agent.session.models.available.insert(
        model_a.clone(),
        acp::ModelInfo::new(model_a.clone(), "QIDI Code A".to_string()),
    );
    agent.session.models.set_current(model_a, None);
    agent.session.models.available.insert(
        model_b.clone(),
        acp::ModelInfo::new(model_b.clone(), "QIDI Code B".to_string()),
    );
    agent.session.model_switch_pending = true;
...
fn delete_both_session_clears_modal_and_welcome_content_hits() {
    ...
    let hit = cf_shell::extensions::session_search::SearchSessionHit {
...
fn gate_refreshed_emits_check_subscription_on_gate_lift() {
    ...
    let settings = cf_shell::util::config::RemoteSettings::default();
...
fn gate_refreshed_no_effect_when_still_gated() {
    ...
    let settings = cf_shell::util::config::RemoteSettings {
...
fn test_gate() -> cf_shell::auth::GateInfo {
    cf_shell::auth::GateInfo {
        message: "Subscribe".into(),
        url: None,
        label: None,
    }
...
fn verify_check_with_meta_resolves_pending_gate() {
    ...
    let meta = serde_json::to_value(cf_shell::auth::AuthMeta::default()).unwrap();
    dispatch_task_result(
...
fn verify_check_with_gated_meta_shows_gate() {
    ...
    let meta = serde_json::to_value(cf_shell::auth::AuthMeta {
        gate: Some(test_gate()),
        ..Default::default()
    })
    .unwrap();
