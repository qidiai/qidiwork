...
/// Inclusive bounds for motion cadence env knobs (`QIDI_MIN_DRAW_MS`,
/// `QIDI_SCROLL_CADENCE_MS`).
...
pub fn start(
    ...
    let (min_draw_env_set, min_draw_env_ms) =
        cadence_ms_from_env("QIDI_MIN_DRAW_MS", default_cadence_ms);
    let (scroll_env_set, scroll_env_ms) =
        cadence_ms_from_env("QIDI_SCROLL_CADENCE_MS", default_cadence_ms);
...
fn spawn_terminal_and_display_refresh_telemetry(tel: StartupTel) {
    tokio::task::spawn_blocking(move || {
        ...
        cf_telemetry::session_ctx::log_event(t.clone());
        ...
        cf_telemetry::session_ctx::log_event(
            cf_telemetry::events::DisplayRefreshProbe {
                terminal: t,
