...
use cf_shell::sampling::error::http_status_from_error;
use cf_shell::session::{ExtMethodResult, SessionInfoResponse};
pub(crate) fn execute(
    ...
    match effect {
        Effect::RegisterActiveSession { session_id, cwd } => {
            ...
            if let Err(e) = cf_shell::active_sessions::register(cf_shell::active_sessions::ActiveSession {
                session_id,
                pid: std::process::id(),
                cwd,
        ...
        Effect::CreateSession {
            ...
            let tx = acp_tx.clone();
            let compat = cf_tools::types::compat::CompatConfig::default();
            ...
            tasks
                .spawn(async move {
                    if let Some(sid) = load_session_id {
                        ...
                        let wt_type = cf_shell::util::config::worktree_type();
            ...
            let abort_handle = tasks
                .spawn(async move {
                    ...
                    let summaries = tokio::task::spawn_blocking(move || {
                            let _permit = permit;
                            cf_workspace::foreign_sessions::scan_foreign_sessions(
                                &cwd,
                                enabled,
                            )
                        })
            ...
            tasks
                .spawn(async move {
                    ...
                    let recent = crate::app::foreign_sessions::with_gated_sources_async(
                            ...
                            &grok_home,
                            |enabled| async move {
                                tokio::task::spawn_blocking(move || cf_workspace::foreign_sessions::most_recent_foreign_session(
                                        &cwd_for_scan,
                                        enabled,
        ...
        Effect::RestoreAndLoadSession { agent_id, session_id, session_cwd: _ } => {
            use cf_shell::agent::session_registry_client::SessionRegistryClient;
            use cf_shell::session::restore::restore_session_with_storage;
            let setup_started = std::time::Instant::now();
            let raw_config = cf_shell::config::load_effective_config();
            let setup = raw_config
                .ok()
                .and_then(|raw| {
                    let cfg = cf_shell::agent::config::Config::new_from_toml_cfg(
                            &raw,
                        )
                        .ok()?;
                    ...
                    let storage = cf_shell::auth::credential_provider::build_storage_client_for_proxy(
                        &proxy_base,
                        deployment_key,
                        alpha_test_key,
            ...
            tasks
                .spawn(async move {
                    ...
                    let progress: Option<
                        cf_shell::session::restore::ProgressCallback,
                    > = {
                        use cf_shell::session::restore::{PhaseStep, RestorePhase};
                        Some(
                            Box::new(move |event| {
            ...
            tasks
                .spawn(async move {
                    ...
                    let detail = tokio::task::spawn_blocking(move || {
                            ...
                            let history_path = cf_shell::session::persistence::session_dir(
                                    &info,
                                )
        ...
        Effect::SendBashCommand { agent_id, session_id, command, prompt_id } => {
            ...
            tasks
                .spawn(async move {
                    use cf_shell::extensions::prompt_meta::PromptBlockMeta;
                    ulog::info(
                        "prompt.acp_send.start",
                        Some(&session_id.0),
                        Some(
        ...
        Effect::KillBgTask { session_id, task_id } => {
            ...
            tasks
                .spawn(async move {
                    let params = cf_shell::extensions::task::KillTaskRequest {
            ...
            tasks
                .spawn(async move {
                    ...
                    let result = acp_send(req, &tx)
                        ...
                        .map(|_| ())
                        .map_err(|e| {
                            use cf_shell::agent::config::ModelSwitchIncompatibleAgentError;
            ...
            tasks
                .spawn(async move {
                    let changelog = tokio::task::spawn_blocking(|| {
                            cf_shell::util::changelog::ChangelogManager::new()
                                .fetch()
                        })
                        .await
                        .unwrap_or_else(|e| {
                            tracing::warn!(error = % e, "changelog fetch task failed");
                            cf_shell::util::changelog::Changelog {
                                markdown: None,
                                entries: None,
                            }
                        });
        ...
        Effect::PersistAnnouncementsHidden { hidden_ids } => {
            tasks
                .spawn(async move {
                    cf_announcements::write_hidden_announcement_ids(&hidden_ids)
                        .await;
        ...
        Effect::PersistPreferredModel { model_id, reasoning_effort } => {
            ...
            tasks
                .spawn(async move {
                    let result = cf_shell::util::config::persist_models_default(
                            Some(model_id_str),
                            reasoning_effort,
                        )
                        .await
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                cf_hooks_plugins_types::HooksListResponse,
                            >(inner.clone())
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                cf_hooks_plugins_types::PluginsListResponse,
                            >(inner.clone())
        ...
        Effect::HooksAction { agent_id, session_id, action } => {
            ...
            tasks
                .spawn(async move {
                    let req_body = cf_hooks_plugins_types::HooksActionRequest {
        ...
        Effect::PluginsAction { agent_id, session_id, action } => {
            ...
            tasks
                .spawn(async move {
                    let req_body = cf_hooks_plugins_types::PluginsActionRequest {
                        session_id: session_id.0.to_string(),
                        action,
                    };
                    let req = acp::ExtRequest::new(
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                cf_hooks_plugins_types::MarketplaceListResponse,
                            >(inner.clone())
            ...
            tasks
                .spawn(async move {
                    ...
                    for (name, old, new, source_url, rel_path) in outdated {
                        let action = cf_hooks_plugins_types::MarketplaceAction::Update {
                            source_url_or_path: source_url.clone(),
                            plugin_relative_path: rel_path.clone(),
                        };
                        let req_body = cf_hooks_plugins_types::MarketplaceActionRequest {
                            session_id: session_id.0.to_string(),
                            action,
                        };
            ...
            tasks
                .spawn(async move {
                    let plugin_name = plugin_relative_path
                        ...
                        .to_string();
                    let action = cf_hooks_plugins_types::MarketplaceAction::Install {
                        source_url_or_path,
                        plugin_relative_path,
                    };
                    let req_body = cf_hooks_plugins_types::MarketplaceActionRequest {
        ...
        Effect::ReloadPluginsForCta { agent_id, session_id, plugin_name } => {
            ...
            tasks
                .spawn(async move {
                    let req_body = cf_hooks_plugins_types::PluginsActionRequest {
                        session_id: session_id.0.to_string(),
                        action: cf_hooks_plugins_types::PluginsAction::Reload,
                    };
                    let req = acp::ExtRequest::new(
                        "x.ai/plugins/action",
                        serde_json::value::to_raw_value(&req_body)
            ...
            tasks
                .spawn(async move {
                    ...
                    struct McpUpsertRequest {
                        ...
                        server_name: String,
                        #[serde(flatten)]
                        config: cf_shell::util::config::McpServerConfig,
                    }
        ...
        Effect::ShareSession { agent_id, session_id } => {
            use cf_shell::session::{ShareSessionRequest, ShareSessionResponse};
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    let request = acp::ExtRequest::new(
        ...
        Effect::SendFeedback { agent_id, session_id, feedback_text } => {
            use cf_shell::session::ClientType;
            use cf_shell::session::acp_types::ClientFeedbackInput;
        ...
        Effect::SaveMemoryNote { agent_id, text, cwd } => {
            tasks
                .spawn(async move {
                    let result = tokio::task::spawn_blocking(move || {
                            let storage = cf_shell::session::memory::MemoryStorage::new(
                                &cwd,
                                None,
                            );
                            storage
                                .append_to_memory(
                                    cf_shell::session::memory::MemoryScope::Global,
                                    &text,
                                )
                        })
                        .await
                            ...
                            Ok(Ok(resp)) => {
                                ...
                                if let Some(hits) = payload.get("results") {
                                    results = serde_json::from_value::<
                                        Vec<
                                            cf_shell::extensions::session_search::SearchSessionHit,
                                        >,
                                    >(hits.clone())
                                        .unwrap_or_default();
                                }
        ...
        Effect::HydrateSessionTitleFromDisk { agent_id, session_id, cwd } => {
            tasks
                .spawn(async move {
                    ...
                    let path = cf_shell::session::persistence::session_dir(&info)
                        .join("summary.json");
                    let title = tokio::task::spawn_blocking(move || -> Option<
                            (String, bool),
                        > {
                            let raw = std::fs::read_to_string(path).ok()?;
                            let summary: cf_shell::session::persistence::Summary = serde_json::from_str(
                                    &raw,
                                )
                                .ok()?;
        ...
        Effect::FetchBilling { agent_id, silent } => {
            ...
            tasks
                .spawn(async move {
                    use cf_shell::extensions::billing::BillingConfigResponse;
                    let req = acp::ExtRequest::new(
                        "x.ai/billing",
                        serde_json::value::to_raw_value(&serde_json::json!({}))
            ...
            tasks
                .spawn(async move {
                    let settings = tokio::task::spawn_blocking(|| {
                            if !cf_shell::util::config::resolve_remote_fetch_enabled() {
                                return None;
                            }
                            let grok_home = cf_shell::util::grok_home::grok_home();
                            let store = cf_shell::auth::read_auth_json(
                                    &grok_home.join("auth.json"),
                                )
                                .ok()?;
                            let scope = cf_shell::auth::GrokComConfig::default()
                                .auth_scope();
                            let auth = cf_shell::auth::lookup_auth(
                                &store,
                                &scope,
                            )?;
                            let proxy_base = std::env::var(
                                ...
                                .unwrap_or_else(|_| {
                                    cf_shell::agent::config::CLI_CHAT_PROXY_BASE_URL_DEFAULT
                                        .to_owned()
                                });
                            cf_shell::remote::fetch_settings_blocking(
                                &proxy_base,
                                &auth,
                                None,
                            )
...
async fn lookup_session_title(session_id: &acp::SessionId) -> Option<String> {
    let summaries = cf_shell::session::persistence::list_summaries(None)
        .await
        .ok()?;
    summaries
        .into_iter()
...
fn format_session_info(
    ...
    let model_display = cf_shell::session::model_display_name(
    ...
    let model_hash_line = if cf_shell::session::should_show_model_fingerprint(
    ...
    let backend_line = info
        ...
        .map(|b| format!("\n  API Backend: {b}"))
        .unwrap_or_default();
    let sandbox_line = match cf_sandbox::profile_name() {
        Some(profile) => {
            let net = if cf_sandbox::should_restrict_child_network() {
                " (network: restricted)"
            } else {
                ""
            };
    ...
    let conversation_line = info
        ...
        .map(|id| format!("\n  Conversation ID: {id}"))
        .unwrap_or_default();
    let version_display = cf_version::display_version(
