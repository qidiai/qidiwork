...
use super::*;
use cf_shell::extensions::billing::{BillingConfig, Cent, UsagePeriod};
...
fn parse_kill_outcome_reads_result_envelope() {
    use cf_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use cf_workspace::session::git::RestoreDegree;
...
fn spawn_fake_acp_agent(
    mut rx: tokio::sync::mpsc::UnboundedReceiver<cf_acp_lib::AcpAgentMessage>,
) -> Arc<AtomicUsize> {
    let counter = Arc::new(AtomicUsize::new(0));
    let counter_clone = counter.clone();
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let cf_acp_lib::AcpAgentMessage::ExtNotification(args) = msg {
...
/// Redirect `QIDI_HOME` to a tempdir for test isolation.
fn setup_grok_home_in_tempdir() -> tempfile::TempDir {
    let tmp = tempfile::tempdir().expect("tempdir creation");
    unsafe {
        std::env::set_var("QIDI_HOME", tmp.path());
    }
    tmp
}
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use cf_shell::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
        cf_shell::active_sessions::list_in(dir.path()).expect("list").is_empty(),
...
fn unregister_best_effort_is_nonblocking_under_lock_contention() {
    ...
    assert_eq!(
        cf_shell::active_sessions::list_in(dir.path()).expect("list").len(), 1,
...
async fn check_marketplace_updates_dispatches_update_and_skips_failed_notifications() {
    ...
    use cf_acp_lib::AcpAgentMessage;
    use cf_hooks_plugins_types::{ActionOutcome, MarketplaceAction, OutcomeStatus};
            ...
            if let AcpAgentMessage::ExtMethod(args) = msg {
                match args.request.method.as_ref() {
                    ...
                    "x.ai/marketplace/action" => {
                        ...
                        let req: cf_hooks_plugins_types::MarketplaceActionRequest = serde_json::from_str(
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: cf_workspace::foreign_sessions::EnabledForeignSessionSources::default(),
...
/// Default flags produce no agent profile (uses cf-tools default).
#[test]
fn default_flags_produce_no_profile() {
    let flags = SessionFlags::default();
    assert_eq!(flags.agent_profile(), None);
}
/// --subagents alone produces no profile (cf-tools already has TaskTool).
#[test]
...
/// Neutralize `QIDI_AGENT` for the profile-matrix tests below: agent-driven
...
/// Callers must be `#[serial_test::serial(QIDI_AGENT)]` (process-global env).
fn without_grok_agent() -> crate::test_util::EnvVarGuard {
    crate::test_util::EnvVarGuard::set("QIDI_AGENT", "")
}
...
/// --ask-user alone selects the cf-tools-ask-user profile.
#[serial_test::serial(QIDI_AGENT)]
#[test]
fn ask_user_alone_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "cf-tools-ask-user");
    assert!(meta.get("askUserQuestion").is_none());
    assert_eq!(meta["yoloMode"], false);
}
...
/// does not need an `agentProfile` (default `cf-tools` already has it).
#[test]
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> cf_shell::session::SessionInfoResponse {
    use cf_shell::session::acp_types::{ContextInfo, SessionInfoData};
    cf_shell::session::SessionInfoResponse {
...
fn format_session_info_shows_model_hash_for_coding_slug_without_flag() {
    let mut info = make_session_info("cf-tools", None, 1000, 10000);
    info.data.model_fingerprint = Some("abc123".into());
    info.data.show_model_fingerprint = false;
    let text = format_session_info(&info, None, false);
    assert!(text.contains("Model Hash: abc123"));
