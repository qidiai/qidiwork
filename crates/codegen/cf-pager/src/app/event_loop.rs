...
/// [`decide`](cf_workspace::folder_trust::decide) precedence.
///
...
pub(crate) async fn run(
    ...
    bg_update_rx: Option<
        tokio::sync::oneshot::Receiver<Option<cf_update::auto_update::UpdateAvailable>>,
    >,
    ...
    if args.log_sampling {
        ...
        unsafe { std::env::set_var("QIDI_LOG_SAMPLING", "1") };
    }
    ...
    let launch_auto = cf_shell::util::config::effective_auto_for_launch(
        args.yolo,
        args.permission_mode_flag.as_deref(),
        remote_permission_mode,
    );
    ...
    let launch_effective_ui = cf_shell::config::load_effective_config()
        .ok()
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(cf_shell::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    app.require_plan_approval = cf_shell::util::config::load_require_plan_approval();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match cf_shell::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.sharing_enabled = remote_settings
        ...
        .unwrap_or(false);
    app.plugin_cta_enabled = cf_config::env_bool("QIDI_PLUGIN_CTA")
    ...
    // switch; `QIDI_VOICE_MODE` overrides for local dev (env > remote > default on).
    ...
    app.session_picker_grouped = std::env::var("QIDI_SESSION_PICKER_GROUPED")
        ...
        .or_else(|| {
            cf_shell::config::load_effective_config()
                .ok()
    ...
    let mut post_render_effects = if needs_interactive_login {
        if connection.auth_methods.is_empty() {
            ...
            app.auth_state = super::app_view::AuthState::Pending {
                error: Some(
                    cf_shell::agent::auth_method::PREFERRED_API_KEY_UNAVAILABLE.to_string(),
                ),
            };
            vec![]
        } else {
    ...
    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<cf_shell::auth::AuthMeta>(meta.clone()) {
        ...
        app.is_api_key_auth = app.auth_methods.iter().any(|m| {
            m.id().0.as_ref() == cf_shell::agent::auth_method::XAI_API_KEY_METHOD_ID
        });
    ...
    app.hidden_announcement_ids = cf_announcements::read_hidden_announcement_ids().await;

    // Load config layers once, resolve announcements, tips, and feature flags.
    let requirements = cf_shell::config::load_merged_requirements();
    let user_config = cf_shell::config::load_from_disk().ok();
    let managed_config = cf_shell::config::load_managed_config().ok();

    // Full merge when every layer parses; partial merge below if any layer fails.
    let effective_config = match cf_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    let compat = cf_shell::agent::config::resolve_compat_sessions_from_raw(
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            app.voice_config = cf_voice::VoiceConfig::from_config_table(table);
        }
    }
    ...
    app.zdr_access_enabled = cf_shell::util::config::resolve_zdr_access_enabled(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        cf_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        cf_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        cf_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    {
        use cf_shell::util::config::{resolve_announcements, resolve_tips};
        ...
        app.active_announcements = cf_announcements::filter_expired(announcements);
        ...
        if !app.tips.is_empty() {
            let grok_home = cf_tools::util::grok_home::grok_home();
            app.tip = cf_shell::util::tips::pick_and_advance(&app.tips, &grok_home);
        }
    }

    let hints = cf_shell::util::config::resolve_hints(
    ...
    // Apply initial config (may come from existing ~/.qidi/pager.toml).
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        cf_shell::util::config::clamped_display_permission_mode(
            cf_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        cf_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = cf_shell::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = cf_shell::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    // QIDI_SCROLL_SPEED/_MODE/_LINES + QIDI_INVERT_SCROLL env overrides
    // apply on first load).
    ...
    let mut voice_rx = None::<tokio::sync::mpsc::Receiver<cf_voice::VoiceEvent>>;
    ...
    // Office-artifact manifest watch (see `app::office_watch`). Fires
    // immediately at startup so artifacts written before this session
    // render without waiting a tick.
    let mut office_watch_at: Option<Instant> = if app.office_watch_wanted() {
        Some(Instant::now())
    } else {
        None
    };
    ...
    // CLI subcommand wrote a `QIDI_OPEN_DASHBOARD_AT_STARTUP=1` env var
    // so we don't have to thread a flag through every arg struct.
    if std::env::var("QIDI_OPEN_DASHBOARD_AT_STARTUP").as_deref() == Ok("1") {
    ...
    loop {
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                tokio::spawn(cf_voice::run_voice_pipeline(
                    voice_config,
                    voice_auth.clone(),
                    cmd_rx,
                    event_tx,
                ...
                app.voice_state = VoiceState::Idle;
                app.voice_ui_active = false;
                app.show_toast("语音通道无法启动——请重启 QIDI");
            } else {
        ...
        // Office watch: constant cadence; `office_watch_wanted` also
        // covers the late-arm case where the workspace dir is created
        // mid-session.
        if office_watch_at.is_none() && app.office_watch_wanted() {
            office_watch_at = Some(
                Instant::now() + crate::app::office_watch::OFFICE_WATCH_INTERVAL,
            );
        }
        ...
        let office_watch = async {
            match office_watch_at {
                Some(at) => sleep_until(at).await,
                None => std::future::pending().await,
            }
        };
        ...
        tokio::select! {
            ...
            _ = office_watch => {
                office_watch_at = None;
                // Redraw only when the poll actually mutated the scrollback
                // (artifact block pushed/replaced); otherwise the block would
                // stay invisible until the next unrelated input event, since
                // idle turns stop the animation tick.
                if app.poll_office_watch() {
                    app.draw(terminal);
                    last_draw_at = Instant::now();
                    draw_scheduled_at = None;
                }
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.qidi/config.toml [ui]`
                // rather than `~/.qidi/pager.toml`. The watcher only knows
                        ...
                        let join_handle = tokio::spawn(async move {
                            ...
                            let ok = tokio::time::timeout(timeout, async {
                                ...
                                for (agent_id, plan) in load_plans {
                                    ...
                                    let mcp_servers = cf_shell::util::config::load_mcp_servers(
                                        &plan.cwd,
...
pub(crate) fn load_initial_ui_config() -> cf_shell::agent::config::UiConfig {
    use cf_shell::agent::config::UiConfig;
    let Ok(root) = cf_shell::config::load_effective_config() else {
