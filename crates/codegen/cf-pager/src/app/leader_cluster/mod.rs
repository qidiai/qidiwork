...
//!
//! Env sandboxing follows this crate's `serial(QIDI_HOME)` idiom; note
...
//! `QIDI_LEADER_SOCKET`, `QIDI_HOME`) for a real agent's whole lifetime, and
...
//!
//! ```bash
//! cargo test -p qidi-code --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
struct PagerLeaderCluster {
    ...
    /// agent on the same QIDI_HOME (two agents on one updates.jsonl is the
...
impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(QIDI_HOME)]`
    ...
    async fn start() -> Self {
        ...
        let env = vec![
            crate::test_util::EnvVarGuard::set("QIDI_HOME", grok_home.path()),
            crate::test_util::EnvVarGuard::set("QIDI_CLI_CHAT_PROXY_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("QIDI_XAI_API_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("XAI_API_KEY", "test-key-for-ci"),
            crate::test_util::EnvVarGuard::set("QIDI_TELEMETRY_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("QIDI_FEEDBACK_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("QIDI_TRACE_UPLOAD", "false"),
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        generation_tasks.push(tokio::task::spawn_local(async move {
            let _ = run_leader_server(
                ...
                cf_shell::agent::activity::AgentActivity::default(),
                tokio::sync::watch::channel(true).1,
                tokio::sync::watch::channel(false).0,
                tokio::sync::watch::channel(cf_shell::leader::ShutdownReason::Manual).0,
                None,
                control_state,
            )
            .await;
        ...
        // qidi-code/src/agent/app.rs (inside its LocalSet body) — a
        // deliberate copy so production stays untouched. Second copy of the
        // same wiring: qidi-code/tests/test_leader_soak.rs ("Real agent
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same QIDI_HOME when the next generation's
    ...
    /// `LeaderReconnector` (socket pinned via `QIDI_LEADER_SOCKET`, flock held
