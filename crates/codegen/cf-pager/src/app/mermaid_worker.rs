...
//! inside [`cf_mermaid::render_checked`] is a no-op there: a panic in the
...
//!
//! 1. The pager re-execs itself as `qidi-code __mermaid-render` (see
...
//!    via [`cf_mermaid::run_with_timeout`], which **kills and reaps** the
...
//!    ([`cf_mermaid::RenderLimits`]) and the raster megapixel/height caps
...
fn render_source_to_png(
    ...
    target_width_px: u32,
    quality: MermaidRenderQuality,
) -> Result<RenderedDiagram, cf_mermaid::MermaidError> {
...
thread_local! {
    ...
    /// `QIDI_HOME` (whose `grok_home()` value is cached first-write-wins, an
    /// isolation hazard under the full parallel suite — PNGs could land in the
    /// real `~/.qidi`). Thread-local, so each parallel test is independent; the
    /// `TempDir` guard lives here so the dir outlives the view. Mirrors the
    /// `subagent::REPLAY_QIDI_HOME` test seam. Production never sets this.
...
impl AgentView {
    ...
    fn mermaid_out_path(&self, key: &MermaidCacheKey) -> Option<PathBuf> {
        // Test seam: a hermetic per-test cache dir (no `QIDI_HOME` mutation).
        #[cfg(test)]
...
mod tests {
    ...
    /// hermetic, with no process-global `QIDI_HOME` mutation. The `TempDir` lives
