...
pub use cf_prompt_queue as prompt_queue;
mod acp_handler;
mod csi_filter;
mod dispatch;
...
/// (`[ui] mouse_reporting_toggle` / `QIDI_MOUSE_REPORTING_TOGGLE`). Seeded once
...
/// Whether voice mode is available (GA default on; remote/`QIDI_VOICE_MODE` can
...
///
/// Precedence: `env` override (`QIDI_VOICE_MODE`) > `remote` (`voice_mode_enabled`)
...
///
/// Uses [`cf_ratatui_inline::Terminal`] instead of stock `ratatui::Terminal`
...
pub(crate) enum ScreenMode {
    ...
    ///
    /// All minimal-mode rendering lives in the sibling `qidi-code-minimal`
...
pub fn resolve_use_leader(
    ...
    _remote_settings: Option<&cf_shell::util::config::RemoteSettings>,
    eligible: bool,
...
pub fn join_early_prefetch(
    handle: Option<cf_shell::agent::models::EarlyPrefetchHandle>,
) -> Option<cf_shell::util::config::RemoteSettings> {
    let handle = handle?;
...
pub async fn run(
    ...
    cf_tty_utils::redirect_native_stderr();
    ...
    let grok_com_config =
        match cf_shell::agent::config::Config::new_from_toml_cfg(&raw_config) {
            Ok(c) => c.grok_com_config,
            Err(e) => {
                tracing::warn!(
                    error = % e, "failed to parse config for auth refresh, using defaults"
                );
                cf_shell::auth::GrokComConfig::default()
            }
        };
    let refreshed_auth = cf_shell::auth::try_ensure_fresh_auth(&grok_com_config).await;
    let early_prefetch =
        cf_shell::agent::models::start_early_prefetch_with_auth(refreshed_auth);
    ...
    cf_shell::util::config::cache_remote_auto_mode(
    ...
    if let Some(reason) = policy_disable_reason {
        tokio::spawn(cf_shell::leader::kill_stale_reachable_leaders(reason));
    }
    if let Some(err) =
    ...
    {
        let summaries = cf_shell::session::persistence::list_summaries(None).await?;
    ...
    let env_hunk_tracker_mode = std::env::var("QIDI_HUNK_TRACKER").ok();
    ...
    // QIDI local patch: never enter scrollback-native minimal mode from the
    // startup resolution. A persisted `ui.screen_mode = "minimal"` config
    // write (side effect of a `--minimal` launch) or the mouse-leak auto
    // downgrade flipped sessions into the one-line welcome card, which looks
    // like a completely different product from the full-screen welcome.
    // Hard-pin the classic full TUI: minimal is reachable only via the
    // explicit `--minimal` CLI flag for that single launch and is never
    // persisted back into the config.
    let screen_mode = ScreenMode::Fullscreen;
    MINIMAL_AUTO_SET_FOR_MOUSE_LEAK.store(false, Ordering::Release);
    let _ = &config_screen_mode;
    let minimal = screen_mode.is_minimal();
    ...
    cancel.cancel();
    cf_tty_utils::global_process_scope().kill_all();
    match result {
        Ok(run_result) => {
...
fn disable_mouse_paste_raw() {
    cf_shell::util::with_locked_stderr(|stderr| {
        let _ = stderr.write_all(cf_crash_handler::terminal::MOUSE_PASTE_RESET);
        let _ = stderr.flush();
    });
}
...
fn init_terminal(
    ...
    cf_crash_handler::enable_terminal_escape_restore();
    ...
    (move || -> io::Result<(PagerTerminal, ScreenMode)> {
        ...
        cf_shell::util::with_locked_stderr(|stderr| {
            if !want_minimal {
                ...
                let _ = stderr.write_all(cf_crash_handler::terminal::MOUSE_TRACKING_RESET);
            }
            execute!(
                stderr,
        ...
        let skip_reason: Option<&str> =
            ctx.kitty_skip_reason()
                .or_else(|| {
                    // PTY e2e (ConPTY): conhost swallows the Kitty keyboard
                    // probe (`CSI ? u`) instead of answering it, so
                    // `supports_keyboard_enhancement()` blocks forever and the
                    // welcome screen never paints. Real terminals answer the
                    // probe (Windows Terminal is skipped via brand above).
                    // The PTY harness sets QIDI_SKIP_TERMINAL_PROBE=1.
                    if std::env::var_os("QIDI_SKIP_TERMINAL_PROBE").is_some() {
                        return Some("pty_probe_skipped");
                    }
        ...
        if mode.is_fullscreen() {
            ...
            Ok((
                cf_ratatui_inline::Terminal::new(backend)?,
            ...
            if let Ok(term) = cf_ratatui_inline::Terminal::with_options(
                probe_backend,
            ...
            let backend =
                CrosstermBackend::new(crate::render::draw::TermWriter::new(frame_tx, writer_sync));
            let term = cf_ratatui_inline::Terminal::with_options(
                backend,
                ratatui::TerminalOptions {
    ...
    .inspect_err(|_| {
        ...
        cf_crash_handler::disable_terminal_escape_restore();
    })
}
...
mod tests {
    ...
    fn remote_settings_leader_mode_true_enables_leader() {
        let rs = cf_shell::util::config::RemoteSettings {
