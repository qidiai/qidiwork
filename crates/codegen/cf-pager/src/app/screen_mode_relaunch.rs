...
pub(crate) const QIDI_SCREEN_MODE_ENV: &str = "QIDI_SCREEN_MODE";
...
///
/// Includes [`QIDI_SCREEN_MODE_ENV`] so the resume hits the same override path
...
pub(crate) fn screen_mode_relaunch_resume_hint(session_id: &str, want_minimal: bool) -> String {
    ...
    format!("{QIDI_SCREEN_MODE_ENV}={mode} grok {flag} --resume {session_id}")
}
...
pub(crate) fn exec_screen_mode_relaunch(session_id: &str, want_minimal: bool) -> io::Result<()> {
    ...
    cmd.env(QIDI_SCREEN_MODE_ENV, screen_mode_env_value(want_minimal));
...
/// Parse a [`QIDI_SCREEN_MODE_ENV`] value (pure; unit-tested directly).
///
/// | Value | Mode |
/// |---|---|
...
/// Consume the one-shot screen-mode override env (see [`QIDI_SCREEN_MODE_ENV`]).
///
...
pub(crate) fn take_screen_mode_env_override() -> Option<super::ScreenMode> {
    let raw = std::env::var_os(QIDI_SCREEN_MODE_ENV);
    if raw.is_some() {
        ...
        unsafe { std::env::remove_var(QIDI_SCREEN_MODE_ENV) };
    }
...
mod tests {
    ...
    fn failed_relaunch_hint_includes_screen_mode_env() {
        // Recovery command must carry QIDI_SCREEN_MODE so following the
        ...
        assert_eq!(
            ...
            "QIDI_SCREEN_MODE=fullscreen grok --fullscreen --resume abc-sid"
        );
        assert_eq!(
            screen_mode_relaunch_resume_hint("abc-sid", true),
            "QIDI_SCREEN_MODE=minimal grok --minimal --resume abc-sid"
        );
    }
    ...
    fn preference_config_screen_mode_beats_legacy_pager_toml() {
        ...
        // "full" alias accepted (same vocabulary as QIDI_SCREEN_MODE).
        assert_eq!(
