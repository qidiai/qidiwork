...
pub fn fork_session_params(
    ...
    let source_cwd = cf_shell::session::resolve_local_session_any_cwd(parent_session_id)
...
pub fn parent_session_is_worktree(session_id: &str, cwd: &Path) -> bool {
    ...
    let sessions_root = cf_shell::util::grok_home::grok_home().join("sessions");
    let encoded = cf_shell::util::grok_home::encode_cwd_dirname(&cwd_str);
...
pub fn local_build_session_on_disk(session_id: &str, cwd: &Path) -> bool {
    let cwd_str = cwd.to_string_lossy();
    cf_shell::session::resolve_local_session(session_id, &cwd_str).is_some()
}
...
async fn most_recent_session_id(cwd: &str) -> anyhow::Result<(String, Option<String>)> {
    let summaries = cf_shell::session::persistence::list_summaries(Some(cwd)).await?;
...
pub(crate) fn pre_acp_auth_manager(
    agent_config: &cf_shell::agent::config::Config,
) -> std::sync::Arc<cf_shell::auth::AuthManager> {
    let auth = std::sync::Arc::new(cf_shell::auth::AuthManager::new(
...
pub fn ensure_session_id_available(session_id: &str, cwd: &str) -> anyhow::Result<()> {
    ...
    if cf_shell::session::persistence::session_exists_for_cwd(session_id, cwd) {
...
async fn resolve_existing_session(
    ...
    if let Some(local_id) = cf_shell::session::resolve_local_session(session_id, cwd) {
        tracing::info!(
    ...
    if let Some(original_cwd) = cf_shell::session::resolve_local_session_any_cwd(session_id) {
        tracing::info!(
    ...
    let agent_config = cf_shell::agent::config::Config::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {}", e))?;
    use cf_shell::agent::session_registry_client::SessionRegistryClient;
    use cf_shell::auth::{AuthManager, ensure_authenticated_or_noninteractive};
    ...
    let storage_client = cf_shell::auth::credential_provider::build_storage_client_for_proxy(
    ...
    let progress: cf_shell::session::restore::ProgressCallback =
