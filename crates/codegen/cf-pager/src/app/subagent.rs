...
thread_local! {
    static REPLAY_QIDI_HOME : std::cell::RefCell < Option < std::path::PathBuf >> = const
...
pub(crate) fn set_replay_grok_home_for_tests(home: Option<std::path::PathBuf>) {
    REPLAY_QIDI_HOME.with(|h| *h.borrow_mut() = home);
}
fn effective_grok_home() -> std::path::PathBuf {
    if let Some(home) = REPLAY_QIDI_HOME.with(|h| h.borrow().clone()) {
        return home;
    }
...
pub(crate) fn replay_inherited_updates(
    ...
    let updates =
        match cf_shell::session::storage::load_updates_for_replay_at(child_session_id, &home)
        {
            Ok(Some(u)) => u,
            Ok(None) => return,
            Err(e) => {
