...
/// Floor for the `QIDI_SUBSCRIPTION_WATCH_INTERVAL_SECS` env override
...
impl AppView {
    ...
    pub fn subscription_watch_interval(&self) -> Option<std::time::Duration> {
        if let Ok(v) = std::env::var("QIDI_SUBSCRIPTION_WATCH_INTERVAL_SECS")
    ...
    #[must_use]
    pub fn impose_gate(&mut self, gate: cf_shell::auth::GateInfo) -> Vec<Effect> {
    ...
    #[must_use]
    fn defer_gate_for_verification(&mut self, gate: cf_shell::auth::GateInfo) -> Vec<Effect> {
...
mod tests {
    ...
    fn watch_gate() -> cf_shell::auth::GateInfo {
    ...
    fn impose_gate_direct_for_non_consumer_and_already_gated() {
        ...
        let new_copy = cf_shell::auth::GateInfo {
