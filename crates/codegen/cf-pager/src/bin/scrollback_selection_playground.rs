...
use cf_pager::scrollback::text_selection::{
...
use cf_pager::scrollback::types::slice_display_cols;
use cf_pager::scrollback::{RenderBlock, ScratchBuffer, ScrollbackPane, ScrollbackState};
...
fn selection_highlight_duration_ms() -> u64 {
    ...
    if cf_pager::appearance::cache::load_keep_text_selection().holds() {
        0
    } else {
        PLAYGROUND_FLASH_MS
    }
...
fn double_click_action_label() -> &'static str {
    if cf_pager::appearance::cache::load_keep_text_selection().selects_word() {
        "word_select"
    } else {
        "toggle_fold"
    }
...
impl App {
    fn new() -> Self {
        ...
        scrollback.push_block(RenderBlock::agent_message(
            "I'm QIDI Code, an interactive CLI agent built to help with software engineering tasks like coding, debugging, refactoring, and exploring codebases.",
        ));
        scrollback.push_block(RenderBlock::agent_message(
