...
use cf_acp_lib::{AcpAgentTx, AcpClientMessageBox, acp_send};
use cf_shell::agent::auth_method::AuthMethodKind;
use cf_shell::agent::config::Config as AgentConfig;
use cf_shell::extensions::task::{CancelSubagentRequest, KillTaskRequest};
use cf_shell::sampling::error::{RATE_LIMITED_ERROR_CODE, rate_limited_user_message};
use cf_shell::sampling::types::{
    REASONING_EFFORT_META_KEY, parse_canonical_effort_token, reasoning_effort_meta_value,
};
use cf_shell::util::config as cli_config;
...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<cf_workspace::permission::types::PermissionRule>> {
...
pub(crate) fn parse_permission_rules_inner(
    ...
    use cf_workspace::permission::types::RuleAction;
...
fn parse_cli_agents(
    json: &str,
) -> anyhow::Result<Vec<cf_shell::agent::config::AgentDefinition>> {
    ...
    for (name, mut value) in map {
        ...
        let mut def = cf_shell::agent::config::AgentDefinition::from_json(&value)
...
fn apply_agent_flag(agent: &Option<String>, config: &mut cf_shell::agent::config::Config) {
...
fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    cf_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
async fn authenticate(
    ...
    let method_id = crate::acp::select_eager_auth_method(auths, default_auth_method_id)
        .ok_or_else(|| {
            ...
            let interactive = std::io::stdin().is_terminal()
                && !cf_shell::util::clipboard::is_remote_session();
    ...
    if kind.needs_interactive_login() {
        use std::io::IsTerminal;
        let interactive =
            std::io::stdin().is_terminal() && !cf_shell::util::clipboard::is_remote_session();
...
async fn open_session(
    ...
    let mcp_servers =
        cli_config::load_mcp_servers(cwd, &cf_tools::types::compat::CompatConfig::default());
...
async fn apply_headless_model_and_effort(
    ...
    .map_err(|e| {
        if let Some(name) = model_name {
            anyhow::anyhow!(
                "Couldn't set model '{}': {}. Run 'available models' to see available models.",
                name,
                e
            )
        } else {
...
pub async fn run_single_turn(
    ...
    cf_shell::http::set_process_client_mode_headless();
    ...
    agent_config.mode = cf_shell::agent::config::AgentMode::Headless;
    ...
    agent_config.cli_agent_overrides = cf_shell::agent::config::CliAgentOverrides {
    ...
    if options.trust {
        cf_shell::agent::folder_trust::grant_folder_trust(&cwd);
    }
    ...
    let track_active = std::env::var("QIDI_TRACK_HEADLESS").is_ok();
    if track_active {
        let _ = cf_shell::active_sessions::register(
            cf_shell::active_sessions::ActiveSession {
    ...
    if options.self_verify {
        prompt_blocks.push(acp::ContentBlock::Text(acp::TextContent::new(
            skill_body(cf_shell::builtin::CHECK_SKILL_MD).to_string(),
        )));
    }
        ...
        let n = n.clamp(2, 10);
        {
            prompt_blocks.insert(
                ...
                acp::ContentBlock::Text(acp::TextContent::new(format!(
                    ...
                    skill_body(cf_shell::builtin::BEST_OF_N_SKILL_MD)
                ))),
            );
        }
    }
    ...
    // Handle result
    if track_active {
        // Non-blocking flock so a slow/network ~/.qidi can't hang exit.
        let _ = cf_shell::active_sessions::try_unregister(&session_id);
    }
    cancel.cancel();
    match prompt_result {
        Some(Ok(resp)) => {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = cf_shell::sampling::error::prompt_usage_from_error(&err)
...
fn handle_ext_notification(
    notif: &cf_acp_lib::AcpArgsBox<acp::ExtNotification>,
    format: OutputFormat,
) -> ExtEvent {
    ...
    let Ok(cf_notif) = serde_json::from_str::<XaiNotif>(notif.request.params.get()) else {
        return ExtEvent::None;
    };

    match cf_notif.update {
...
mod tests {
    ...
    use super::*;
    use cf_workspace::permission::types::{RuleAction, ToolFilter};
    ...
    fn domain_mode_web_fetch() {
        ...
        assert_eq!(
            ...
            cf_workspace::permission::types::PatternMode::Domain
        );
    ...
    ) -> cf_acp_lib::AcpArgsBox<acp::ExtNotification> {
