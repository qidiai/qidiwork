//! Scroll flight recorder — `QIDI_SCROLL_LOG` JSONL log of scroll-stream
...
//! (see [`super::mouse`]).
//!
//! Enablement: `QIDI_SCROLL_LOG=1` (or set-but-empty) logs to
//! `~/.qidi/logs/scroll-log-<timestamp>.jsonl`; any other non-`0` value is
//! used as the target path. Unset (or `0`, matching `QIDI_SCROLL_DEBUG`)
...
/// Appends [`ScrollLogRecord`]s to the `QIDI_SCROLL_LOG` file. Owned as
...
impl ScrollLogRecorder {
    /// Build from `QIDI_SCROLL_LOG` (see module docs for value semantics);
    /// `None` when unset or `0`.
    pub(crate) fn from_env_at(base: Instant) -> Option<Self> {
        let raw = std::env::var("QIDI_SCROLL_LOG").ok()?;
        let value = raw.trim();
        if value == "0" {
            return None;
        }
...
/// `~/.qidi/logs/scroll-log-<utc-ts>.jsonl` — the input-debug dump's dir
