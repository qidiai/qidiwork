...
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
pub enum McpScope {
    /// `~/.qidi/config.toml`, available in all your projects
    User,
    /// `./.qidi/config.toml`, shared with everyone working in this directory
    Project,
}
...
pub struct AddArgs {
    ...
    /// Config to write to: user (~/.qidi/config.toml) or project (./.qidi/config.toml)
...
fn run_list(json: bool) -> Result<()> {
    ...
    let servers = cf_shell::util::config::load_mcp_server_configs_with_project(&cwd);
...
async fn run_add(args: AddArgs) -> Result<()> {
    ...
    cf_shell::util::config::save_mcp_server_config_at(&path, name, &config).await?;
...
fn scope_target(scope: McpScope) -> PathBuf {
    match scope {
        McpScope::User => cf_shell::util::config::user_config_path(),
        McpScope::Project => {
            cf_shell::util::config::project_config_path(&current_dir_or_exit())
        }
    }
}
...
fn select_remove_site(
    ...
    use cf_shell::util::config::user_config_path;
    ...
    project_site
        ...
        .or_else(|| {
            user_defined.then(|| {
                (
                    McpScope::User,
                    cf_shell::util::config::user_config_path(),
                )
            })
        })
}
...
async fn run_remove(name: &str, requested_scope: Option<McpScope>) -> Result<()> {
    ...
    let find_project_site = || {
        cf_shell::config::find_project_configs(&cwd)
            .into_iter()
            .rev()
    ...
    // ancestor .qidi/config.toml, where it still resolves for sessions.
...
async fn run_doctor(json: bool, name: Option<String>) -> Result<()> {
    let cwd = current_dir_or_exit();
    let report = cf_shell::mcp_doctor::run_doctor(&cwd, name.as_deref()).await;
    ...
    if json {
        ...
        cf_shell::mcp_doctor::print_report(&report);
    }
...
mod tests {
    ...
    fn select_remove_site_covers_scope_presence_matrix() {
        let user = cf_shell::util::config::user_config_path();
        let project = PathBuf::from("/repo/.qidi/config.toml");
