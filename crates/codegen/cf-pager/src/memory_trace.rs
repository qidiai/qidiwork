...
//!   `QIDI_MEMTRACE_INTERVAL_SECS` (default 30s) from a detached thread.
...
//!   (`QIDI_MEMTRACE_THRESHOLD_MB`, default 1 GiB, then doubling), a full
...
//! ## Files
//!
//! `$QIDI_HOME/memtrace/<start-ts>-<pid>.jsonl` (+ `.1` after 4 MiB
...
//!
//! The lib cannot depend on jemalloc; `qidi-code-bin` installs:
...
/// Whether a trace sink is installed ([`start`] ran and `QIDI_MEMTRACE` is
...
/// Env: disable with `QIDI_MEMTRACE=0|false|off`.
fn enabled_by_env() -> bool {
    !matches!(
        std::env::var("QIDI_MEMTRACE").ok().as_deref(),
        Some("0") | Some("false") | Some("off")
    )
}

fn interval_from_env() -> Duration {
    let secs = std::env::var("QIDI_MEMTRACE_INTERVAL_SECS")
        .ok()
...
fn first_threshold_from_env() -> u64 {
    std::env::var("QIDI_MEMTRACE_THRESHOLD_MB")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .unwrap_or(1024)
        .saturating_mul(1 << 20)
...
/// `dir` (e.g. `$QIDI_HOME/memtrace/`) and spawn the detached sampler
...
/// don't trace. Inert when `QIDI_MEMTRACE=0`.
///
