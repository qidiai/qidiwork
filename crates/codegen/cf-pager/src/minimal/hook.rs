...
//! render mode.
//!
//! `qidi-code` cannot depend on `qidi-code-minimal`: that crate reads
...
//!
//! The composition-root binary (`qidi-code-bin`) wires it up once at
//! startup by calling `cf_pager_minimal::install()`. When nothing has
