...
fn execute_hook(
    ...
    cmd.arg("-c")
        .arg(command)
        .env("QIDI_EVENT", event_str)
        .env("QIDI_MESSAGE", message)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    if let Some(sid) = session_id {
        cmd.env("QIDI_SESSION_ID", sid);
    }
...
mod tests {
    ...
    fn sets_environment_variables() {
        ...
        assert!(
            content.contains("QIDI_EVENT=Turn complete"),
            "missing QIDI_EVENT: {content}"
        );
        assert!(
            content.contains("QIDI_MESSAGE=hello world"),
            "missing QIDI_MESSAGE: {content}"
        );
        assert!(
            content.contains("QIDI_SESSION_ID=sess-42"),
            "missing QIDI_SESSION_ID: {content}"
        );
    }
    ...
    fn omits_session_id_when_none() {
        ...
        let content = std::fs::read_to_string(&out).unwrap();
        assert!(
            !content.contains("QIDI_SESSION_ID"),
            "QIDI_SESSION_ID should not be set: {content}"
        );
    }
    ...
    fn run_hook_passes_correct_env_via_thread() {
        ...
        assert!(content.contains("QIDI_EVENT=Turn complete"));
        assert!(content.contains("QIDI_MESSAGE=test body payload"));
        assert!(content.contains("QIDI_SESSION_ID=test-session-123"));
    }
}
