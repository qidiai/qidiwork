//! `qidiwork plugin` CLI subcommand — manage plugins and marketplace sources.
//!
...
//!
//! Business logic lives in `cf_shell::plugin` (shared orchestration)
//! and lower crates (`qidi-code`, `xai-grok-plugin-marketplace`). This
...
use cf_agent::plugins::install_registry::{InstallKind, InstallRegistry};
use cf_agent::plugins::manifest::{ManifestLoadResult, PluginManifest, load_manifest};
use cf_plugin_marketplace::SourceKind;
use cf_plugin_marketplace::git::SourceCacheLease;
use cf_shell::plugin::{self, RepoUpdateOutcome, UninstallError};

// ── JSON output types ───────────────────────────────────────────────

/// Typed entry for `qidiwork plugin list --json`. The `status` field acts as a
...
/// Typed entry for `qidiwork plugin marketplace list --json`.
#[derive(Serialize)]
...
pub enum PluginCommand {
    ...
    Uninstall {
        /// Plugin name (as shown by `qidiwork plugin list`).
        name: String,
...
fn trust_prompt(subject: &str, source_arg: &str) -> String {
    format!(
        "Installing {subject} requires confirmation.\n\
         ...
         \n\
         To proceed, re-run with --trust:\n  qidiwork plugin install {source_arg} --trust"
    )
}
...
fn cmd_list(json: bool, available: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("No plugins installed. Run `qidiwork plugin install --help` to get started.");
    } else {
...
fn installed_plugins(
    repos: &[(
        &str,
        &cf_agent::plugins::install_registry::InstalledRepo,
    )],
) -> Vec<PluginEntry> {
    repos
        .iter()
...
fn available_plugins(registry: &InstallRegistry) -> Vec<PluginEntry> {
    let config = cf_shell::config::load_effective_config()
        .ok()
        .unwrap_or(toml::Value::Table(toml::map::Map::new()));
    let mut sources = cf_plugin_marketplace::load_sources(&config);
    sources.extend(cf_plugin_marketplace::load_extra_sources_from_settings(&sources));
    ...
    for source in &sources {
        ...
        for plugin in cf_plugin_marketplace::scan_marketplace(&root).entries {
            let already_installed =
                cf_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    registry,
                    &identity,
...
fn source_identity(source: &cf_plugin_marketplace::MarketplaceSource) -> String {
    match &source.kind {
...
fn install_kind(is_git: bool) -> cf_telemetry::events::InstallKind {
    if is_git {
        cf_telemetry::events::InstallKind::Git
    } else {
        cf_telemetry::events::InstallKind::Local
    }
}

fn log_plugin_installed(
    install_kind: cf_telemetry::events::InstallKind,
    success: bool,
    error_category: Option<String>,
) {
    cf_telemetry::session_ctx::log_event(cf_telemetry::events::PluginInstalled {
        install_kind,
        success,
        trust: true,
        error_category,
...
fn cmd_install(source: &str, trust: bool) -> Result<()> {
    if let Some(mref) = cf_plugin_marketplace::install_resolve::parse_marketplace_ref(source)
    {
    ...
    if !trust {
        use cf_agent::plugins::git_install::{self, InstallSource};
...
fn cmd_install_marketplace(
    source: &str,
    mref: &cf_plugin_marketplace::install_resolve::MarketplaceRef,
    trust: bool,
) -> Result<()> {
    if !trust {
            ...
            if outcome.already_installed {
                ...
                println!(
                    "Plugin \"{}\" is already installed from {}. \
                     Run `qidiwork plugin update {}` to update it.",
    ...
    match plugin::uninstall_plugin(name, confirm, keep_data) {
        Ok(outcome) => {
            cf_telemetry::session_ctx::log_event(
                cf_telemetry::events::PluginUninstalled {
                    confirmed: true,
                    success: true,
                },
            );
        ...
        Err(UninstallError::NeedsConfirm {
            ...
            "Plugin \"{name}\" belongs to repo \"{repo_key}\" which also contains:\n\
             ...
             Uninstalling will remove all {total} plugin(s). To proceed:\n\
               qidiwork plugin uninstall {name} --confirm",
            other_plugins
                .iter()
                .map(|p| format!("  - {p}"))
                .collect::<Vec<_>>()
    ...
    if registry.find_plugin(name).is_none() {
        bail!(
            "Plugin \"{name}\" not found.\n\
               Run `qidiwork plugin list` to see installed plugins."
        );
    }
    if let Err(e) = cf_shell::config::remove_disabled_plugin(name) {
        tracing::warn!("failed to remove from disabled list: {e}");
    }
    cf_shell::config::add_enabled_plugin(name)
...
fn cmd_disable(name: &str) -> Result<()> {
    ...
    if let Err(e) = cf_shell::config::remove_enabled_plugin(name) {
        tracing::warn!("failed to remove from enabled list: {e}");
    }
    cf_shell::config::add_disabled_plugin(name)
...
fn cmd_validate(path: &str) -> Result<()> {
    ...
    match load_manifest(&root) {
        ...
        Ok(ManifestLoadResult::NotFound) => {
            println!(
                "未找到 plugin.json。QIDI 会自动从标准目录发现技能（skills）、\
                 子代理（agents）与钩子（hooks）。\
                 只有在需要自定义路径或元信息时才需要该清单文件。"
            );
            Ok(())
        }
...
fn cmd_tag(path: &str, push: bool, force: bool, dry_run: bool) -> Result<()> {
    ...
    let version = match load_manifest(&root) {
        Ok(ManifestLoadResult::Found(m)) => m.version.ok_or_else(|| {
            anyhow::anyhow!(
                "No `version` field in plugin.json. Set a version to use `qidiwork plugin tag`."
            )
        })?,
    ...
    if json {
        ...
        println!(
            "No marketplace sources configured.\n\
             Run `qidiwork plugin marketplace add --help` to get started."
        );
    } else {
        for s in sources {
            let id = match &s.kind {
...
fn marketplace_add(
    ...
    use cf_shell::plugin::MarketplaceAddInput;
...
fn marketplace_remove(
    ...
    let local_input = match plugin::classify_marketplace_add_input(url, &cwd) {
        cf_shell::plugin::MarketplaceAddInput::LocalPath(p) => Some(p),
        _ => None,
    };
...
fn marketplace_update(
    ...
    marketplace_update_with_cache_root(
        sources,
        name,
        &cf_plugin_marketplace::git::default_cache_root(),
    )
}
...
mod tests {
    ...
    fn trust_prompt_marketplace_has_no_error_framing() {
        ...
        assert!(msg.contains(
            "To proceed, re-run with --trust:\n  qidiwork plugin install sentry@xai-org/plugin-marketplace --trust"
        ));
    ...
    fn trust_prompt_git_and_local_subjects() {
        ...
        assert!(git.ends_with("  qidiwork plugin install u/r --trust"), "{git}");
