...
pub(crate) fn run_wrapped_command(program: &str, args: &[String]) -> Result<i32> {
    ...
    // reads this (see `cf_pager_render::clipboard::osc52_sink_active`) to
    ...
    // these two without clearing it. The canonical `QIDI_OSC52_SINK` is
    ...
    cmd.env("QIDI_OSC52_SINK", "1");
    cmd.env("LC_QIDI_OSC52_SINK", "1");
...
///
/// Delegates to [`cf_shell::util::clipboard::set_text`] which uses
...
fn set_local_clipboard(data: &[u8]) {
    ...
    if let Err(e) = cf_shell::util::clipboard::set_text(text) {
...
fn host_clipboard_image_frame() -> Vec<u8> {
    let image = cf_pager_render::clipboard::system_clipboard_get_image();
