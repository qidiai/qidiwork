...
use super::blocks::{
    AgentMessageBlock, ArtifactBlock, BgTaskBlock, BtwBlock, ContextInfoBlock, CreditLimitBlock,
...
pub enum RenderBlock {
    ...
    /// Office artifact cards registered via office-artifact (manifest.json).
...
macro_rules! delegate_block {
    ($self:expr, $method:ident ( $($arg:expr),* )) => {
        match $self {
            ...
            RenderBlock::Artifact(b) => b.$method($($arg),*),
        }
    };
}
...
impl RenderBlock {
    ...
    pub fn context_info(
        snapshot: cf_shell::session::ContextInfo,
    ...
    pub fn accent_color(&self) -> Option<ratatui::style::Color> {
        ...
        match self {
            ...
            RenderBlock::Artifact(_) => Some(theme.accent_skill),
        }
    }
    ...
    pub fn searchable_text(&self) -> Option<String> {
        match self {
            ...
            RenderBlock::Artifact(b) => join_searchable(
                b.cards
                    .iter()
                    .map(|c| Some(format!("{} {} {}", c.name, c.path, c.note))),
            ),
        }
    }
    ...
    pub fn with_hyperlinks<R>(
        &self,
        f: impl FnOnce(&[cf_markdown::HyperlinkTarget]) -> R,
    ) -> R {
        match self {
...
mod searchable_text_tests {
    ...
    use cf_shell::session::ContextInfo;
