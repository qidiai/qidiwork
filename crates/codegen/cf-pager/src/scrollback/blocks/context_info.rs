...
use cf_shell::session::{ContextInfo, count_detail};
...
mod tests {
    use super::*;
    use cf_shell::session::TokenUsageCategory;
    ...
    fn bar_and_legend_reconcile_used_with_overhead_excluding_tools() {
        ...
        let block = ContextInfoBlock::new(snap, "cf-tools");
        let theme = test_theme();
        let lines = block.build_lines(&theme, BarLayout::WIDE);
