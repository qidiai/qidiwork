...
impl Default for PagerLocalSnapshot {
    fn default() -> Self {
        Self {
            ...
            voice_stt_language: cf_voice::STT_LANGUAGE_DEFAULT.to_string(),
        }
    }
}
...
///
/// Delegates to [`cf_voice::canonicalize_stt_language`] so the pager and
...
pub fn canonical_voice_stt_language(value: Option<&str>) -> &'static str {
    cf_voice::canonicalize_stt_language(value)
}
...
pub fn current_value_for(
    ...
    match key {
        ...
        // Live cache; `QIDI_PROMPT_SUGGESTIONS` env overrides at the gate.
        ...
        "fork_secondary_model" => Some(SettingValue::String({
            let baseline = cf_shell::models::default_model();
...
mod tests {
                    ...
                    assert_eq!(
                        *default,
                        ...
                        "toolset.ask_user_question.timeout_enabled default drifts from the \
                         shared resolver const in qidi-code"
                    );
                }
    ...
    /// Spot-check the delegation to `cf_voice::canonicalize_stt_language`
    ...
    fn voice_stt_language_settings_match_voice_crate_catalog() {
        ...
        for c in choices.iter() {
            ...
            let lang = cf_voice::stt_language_by_code(c.canonical)
        ...
        let crate_codes: HashSet<&str> = cf_voice::STT_LANGUAGES
            .iter()
            .map(|l| l.code)
            .collect();
        assert_eq!(
            setting_codes, crate_codes,
            "settings concrete languages must match cf_voice::STT_LANGUAGES exactly"
        );
    }
    ...
    fn contextual_hints_group_and_children_registered() {
        ...
        let ui = UiConfig {
            contextual_hints: cf_shell::agent::config::ContextualHints {
                undo: Some(false),
                ..Default::default()
            },
            ..UiConfig::default()
