...
use cf_shell::session::repo_changes::UploadMethod;
...
pub struct TraceArgs {
    ...
    #[arg(long)]
    pub local: bool,
    /// Output path (default: $QIDI_HOME/trace-exports/<session-id>.tar.gz)
    #[arg(short, long)]
...
pub fn build_session_tar(
    ...
    let mut file_count: u32 = 0;
    {
        ...
        let metadata = ExportMetadata {
            ...
            qidi_version: env!("VERSION_WITH_COMMIT").to_owned(),
...
pub(crate) fn find_session_dir(session_id: &str) -> Result<PathBuf> {
    cf_shell::session::persistence::find_session_dir_by_id(session_id).with_context(|| {
        format!(
...
async fn run_upload(
    ...
    if bucket_url.is_none()
        && matches!(
            upload_method,
            cf_shell::session::repo_changes::UploadMethod::Direct { .. }
        )
    {
        anyhow::bail!(
            "No trace upload bucket configured. Set `QIDI_TELEMETRY_GCS_BUCKET`, \
             `QIDI_TRACE_UPLOAD_BUCKET`, or `endpoints.trace_upload_bucket` in \
    ...
    let upload_config = cf_shell::session::repo_changes::TraceExportConfig {
...
async fn upload_with_retries(
    config: &cf_shell::session::repo_changes::TraceExportConfig,
    object_path: &str,
    archive: &[u8],
    ...
    (|| async {
        tokio::time::timeout(
            UPLOAD_TIMEOUT,
            cf_file_utils::gcs::upload_bytes(config, object_path, archive, "application/gzip"),
        )
        .await
...
pub async fn resolve_upload_method(agent_config: &AgentConfig) -> Option<UploadMethod> {
    ...
    let auth_token = cf_shell::auth::ensure_authenticated_or_noninteractive(
