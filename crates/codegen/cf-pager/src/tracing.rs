...
/// firehose (`QIDI_DEBUG_LOG` / `QIDI_LOG_FILE`).
pub use cf_telemetry::debug_log::ACP_UPDATE_PAYLOAD_TARGET;
...
pub use cf_telemetry::debug_log::ACP_UPDATE_TARGET;
...
pub fn init_tracing() -> TracingHandle {
    ...
    let directives = format!(
        "cf_shell=info,cf_pager=trace,cf_tools=info,cf_acp_lib=info,{RMCP_SSE_NOISE_TARGET}=error,sampling_log=off,{ACP_UPDATE_TARGET}=debug,{ACP_UPDATE_PAYLOAD_TARGET}={payload_level}"
    );
    ...
    let otel_layer = cf_telemetry::otel_layer::build_otel_layer(
    ...
    let instrumentation_layer = cf_telemetry::instrumentation::layer();
    let sampling_log_layer = cf_telemetry::sampling_log::layer();
    let hooks_log_layer = cf_telemetry::hooks_log::layer();
    let registry = tracing_subscriber::registry()
        ...
        .with(otel_layer);
    cf_telemetry::debug_log::install_firehose(registry, "tui");
