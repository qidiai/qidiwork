...
use cf_agent::config::{AgentDefinition, AgentScope, BuiltinAgentName};
use cf_shell::agent::config::AgentSelectionConfig;
use cf_tools::implementations::skills::discovery::extract_first_paragraph;
...
/// User-level vs project-level config files (`~/.qidi` vs `{cwd}/.grok`).
...
pub struct AgentsModalState {
    ...
    bundle: BundleState,
    /// Resolved startup agent name (same chain as shell: `[agent]`, `QIDI_AGENT`,
    /// model `agentType`, then `cf-tools`).
...
pub fn build_agent_list(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<AgentListEntry> {
    ...
    let subagent_names: Vec<String> = BuiltinAgentName::subagent_variants()
        ...
        .collect();
    let discovered = cf_agent::discovery::discover(cwd);
...
/// Union bundled personas with local `~/.qidi/personas` and `{cwd}/.qidi/personas`.
///
...
pub fn load_agent_toggle() -> HashMap<String, bool> {
    let root = match cf_shell::config::load_effective_config() {
        Ok(r) => r,
...
fn personas_dir_for_scope(scope: ConfigFileScope, cwd: &Path) -> PathBuf {
    match scope {
        ConfigFileScope::User => cf_config::grok_home().join("personas"),
        ConfigFileScope::Project => cwd.join(".grok").join("personas"),
    }
}
...
/// True when `path` is a deletable local persona file (user or project `.qidi/personas`).
pub fn persona_path_is_deletable(path: &Path) -> bool {
    config_path_is_user_or_project(path, "personas")
}
/// Shared guard: canonical path under `~/.qidi/{subdir}` or `{cwd}/.qidi/{subdir}`, not bundled.
...
fn load_agent_selection_config() -> AgentSelectionConfig {
    cf_shell::config::load_effective_config()
        .ok()
        .and_then(|root| cf_shell::agent::config::Config::new_from_toml_cfg(&root).ok())
        .map(|cfg| cfg.agent)
        .unwrap_or_default()
}
...
/// `MvpAgent::resolve_agent_definition` in qidi-code.
pub fn resolve_default_agent_name(cwd: &Path, model_agent_type: Option<&str>) -> String {
    ...
    cf_shell::agent::mvp_agent::MvpAgent::resolve_agent_definition(
        cwd,
        None,
        &agent_config,
        None,
...
pub fn format_agent_detail(entry: &AgentListEntry) -> Vec<String> {
    ...
    let mode_label = match def.prompt_mode {
        cf_agent::config::PromptMode::Extend => "extend",
        cf_agent::config::PromptMode::Full => "full",
    };
...
mod tests {
    use super::*;
    use cf_shell::agent::config::DEFAULT_AGENT_TYPE;
    #[test]
    ...
    fn persona_is_deletable_local_vs_bundled() {
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.qidi/bundled/personas/b.toml".into()),
            scope_label: None,
        };
