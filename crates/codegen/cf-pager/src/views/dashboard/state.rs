...
/// `~/.qidi/config.toml`. Lenient — corrupted fields fall back to
...
pub struct PendingDispatchModel {
    ...
    pub effort: Option<cf_shell::sampling::types::ReasoningEffort>,
    pub display: String,
}
    ...
    fn handle_mouse(&mut self, mouse: &crossterm::event::MouseEvent) -> InputOutcome {
        ...
        if matches!(mouse.kind, MouseEventKind::Down(MouseButton::Left)) {
            ...
            if self.upgrade_cta_hit.contains(mouse.column, mouse.row) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    cf_telemetry::events::AnnouncementCtaSurface::Dashboard,
                ));
            }
...
/// Load the full persisted dashboard from `~/.qidi/config.toml`.
///
...
/// `~/.qidi/config.toml` (`[ui]`, `[hints]`, `[mcpServers]`, …).
///
...
fn config_path() -> Option<PathBuf> {
    let home = cf_shell::util::grok_home::grok_home();
...
mod tests {
    ...
    fn peek_test_image() -> crate::prompt_images::PastedImage {
        crate::prompt_images::PastedImage {
            element_id: cf_ratatui_textarea::ElementId::from_raw(0),
            display_number: 0,
    ...
    /// per-key serial lock. The `QIDI_AGENT_DASHBOARD` key means this
    /// test runs serially with any other test that decorates itself
    /// with `#[serial_test::serial(QIDI_AGENT_DASHBOARD)]` — see the
    ...
    fn env_var_force_disables() {
        ...
        // same `QIDI_AGENT_DASHBOARD` key reads it concurrently.
