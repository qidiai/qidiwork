...
use cf_tools::implementations::skills::types::SkillInfo;
...
pub fn fuzzy_matches_hook(hook: &cf_hooks_plugins_types::HookInfo, query: &str) -> bool {
    if query.is_empty() {
        return true;
    }
...
pub(crate) fn source_has_matching_plugin(
    source: &cf_hooks_plugins_types::MarketplaceScanResult,
    query: &str,
) -> bool {
...
pub fn filtered_marketplace_count(
    sources: &[cf_hooks_plugins_types::MarketplaceScanResult],
    query: &str,
) -> usize {
...
pub(crate) fn test_plugin_info(
    name: &str,
    origin: Option<cf_hooks_plugins_types::PluginOrigin>,
) -> cf_hooks_plugins_types::PluginInfo {
    cf_hooks_plugins_types::PluginInfo {
        name: name.to_string(),
        id: format!("user/abcd1234/{name}"),
        root: format!("/tmp/{name}"),
        scope: cf_hooks_plugins_types::PluginScope::User,
        trusted: true,
        enabled: true,
        version: None,
        description: None,
        ...
        agent_count: 0,
        agent_names: vec![],
        hook_status: cf_hooks_plugins_types::HookStatus::None,
        hook_count: 0,
        mcp_server_count: 0,
        mcp_status: cf_hooks_plugins_types::McpStatus::None,
...
type GroupedPlugins<'a> = std::collections::BTreeMap<
    (u8, String, String),
    Vec<(usize, &'a cf_hooks_plugins_types::PluginInfo)>,
>;
...
/// sensible groups.
pub fn plugin_group(plugin: &cf_hooks_plugins_types::PluginInfo) -> PluginGroup {
    use cf_hooks_plugins_types::{PluginOrigin, PluginScope};
...
pub fn resolve_marketplace_index<'a>(
    sources: &'a [cf_hooks_plugins_types::MarketplaceScanResult],
    target: usize,
    query: &str,
...
pub enum MarketplaceHit<'a> {
    ...
    Plugin {
        source_index: usize,
        source: &'a cf_hooks_plugins_types::MarketplaceScanResult,
        plugin: &'a cf_hooks_plugins_types::MarketplacePluginEntry,
    },
}
...
fn build_hook_groups<'a>(
    hooks: &'a [cf_hooks_plugins_types::HookInfo],
    filter: StatusFilter,
    query: &str,
) -> Vec<(&'a str, Vec<usize>)> {
...
pub fn next_visible_hook(
    hooks: &[cf_hooks_plugins_types::HookInfo],
    current: usize,
...
impl ExtensionsTab {
    ...
    pub fn telemetry_tab(self) -> cf_telemetry::events::ExtensionsModalTab {
        use cf_telemetry::events::ExtensionsModalTab;
        match self {
...
pub enum ButtonAction {
    ...
    HooksAction(cf_hooks_plugins_types::HooksAction),
    /// Execute a plugins action via ACP (no args needed).
    PluginsAction(cf_hooks_plugins_types::PluginsAction),
    ...
    AddMcpServer {
        name: String,
        config: Box<cf_shell::util::config::McpServerConfig>,
    },
    ...
    MarketplaceAction(cf_hooks_plugins_types::MarketplaceAction),
...
pub enum ModalMessage {
    ...
    Confirmation {
        message: String,
        action: cf_hooks_plugins_types::PluginsAction,
    },
    /// A confirmation prompt for a marketplace action (install/uninstall/update).
    MarketplaceConfirmation {
        message: String,
        action: cf_hooks_plugins_types::MarketplaceAction,
    },
}
...
pub fn resolve_key(tab: ExtensionsTab, ch: char) -> Option<ButtonAction> {
    use cf_hooks_plugins_types::{HooksAction, MarketplaceAction, PluginsAction};
...
pub fn build_action_from_input(
    ...
    use cf_hooks_plugins_types::{HooksAction, PluginsAction};
    ...
    match command_prefix {
        ...
        "marketplace_add_source" => Some(ButtonAction::MarketplaceAction(
            cf_hooks_plugins_types::MarketplaceAction::AddSource {
                url: first.to_string(),
            },
        )),
        "mcp_add" => {
...
fn parse_mcp_add_fields(name: &str, url_or_cmd: &str) -> Option<ButtonAction> {
    use cf_shell::util::config::{McpServerConfig, McpServerTransportConfig};
...
pub struct ExtensionsModalState {
    ...
    pub hooks_data: TabDataState<cf_hooks_plugins_types::HooksListResponse>,
    /// Plugins list data (fetched from shell).
    pub plugins_data: TabDataState<cf_hooks_plugins_types::PluginsListResponse>,
    ...
    pub last_plugins_action: Option<cf_hooks_plugins_types::PluginsAction>,
    ...
    pub marketplace_data: TabDataState<cf_hooks_plugins_types::MarketplaceListResponse>,
...
impl ExtensionsModalState {
    ...
    pub fn seed_plugin_groups_once(&mut self, plugins: &[cf_hooks_plugins_types::PluginInfo]) {
...
pub fn derive_source_label(source_dir: &str) -> (String, bool) {
    let grok = cf_config::grok_home();
    let source_path = std::path::Path::new(source_dir);
    // Plugin / installed-plugin dirs, under the user grok home (QIDI_HOME-aware)
    // or a project-scoped `{cwd}/.qidi/<subdir>/`. Returns the first path
    ...
    // Global hooks under $QIDI_HOME/hooks
    ...
    // Project hooks
    if source_dir.ends_with("/.qidi/hooks") || source_dir.contains("/.qidi/hooks/") {
...
fn skill_source_str(skill: &SkillInfo) -> String {
    if let Some(ref cs) = skill.config_source {
        match cs {
            cf_tools::types::config_source::ConfigSource::User { path } => {
            ...
            cf_tools::types::config_source::ConfigSource::Project { path } => {
            ...
            cf_tools::types::config_source::ConfigSource::Plugin { plugin_name, .. } => {
...
fn build_plugin_fields(plugin: &cf_hooks_plugins_types::PluginInfo) -> Vec<String> {
    use cf_hooks_plugins_types::McpStatus;
...
fn component_categories(
    components: &cf_hooks_plugins_types::PluginComponents,
) -> [(&'static str, &[cf_hooks_plugins_types::ComponentItem]); 6] {
    use cf_hooks_plugins_types::ComponentCategory;
...
pub(crate) fn marketplace_components_summary(
    plugin: &cf_hooks_plugins_types::MarketplacePluginEntry,
) -> Option<String> {
    plugin
        .components
        .as_ref()
            ...
            ExtensionsTab::Hooks => {
                if let TabDataState::Loaded(ref data) = state.hooks_data {
                    ...
                    let mut groups: std::collections::BTreeMap<
                        String,
                        Vec<(usize, &cf_hooks_plugins_types::HookInfo)>,
                    > = std::collections::BTreeMap::new();
...
mod tests {
    ...
    fn derive_source_label_detects_project_scoped_plugins() {
        // Regression: project-scoped `{cwd}/.qidi/plugins/<name>/` must label as
        // a (non-removable) plugin, not a removable "Custom" source. The user
        // grok-home branch is QIDI_HOME-aware; this covers the project fallback.
        let (label, is_custom) = derive_source_label("/repo/work/.qidi/plugins/my-plugin/hooks");
        assert_eq!(label, "Plugin: my-plugin");
        assert!(!is_custom);

        let (label, is_custom) =
            derive_source_label("/repo/work/.qidi/installed-plugins/vendor-abc123/skills");
    ...
    fn make_skill(
        name: &str,
        desc: &str,
    ) -> cf_tools::implementations::skills::types::SkillInfo {
        cf_tools::implementations::skills::types::SkillInfo {
            name: name.to_string(),
            display_name: None,
            ...
            metadata: None,
            path: "test".to_string(),
            scope: cf_tools::implementations::skills::types::SkillScope::User,
            config_source: None,
            plugin_name: None,
            plugin_version: None,
            plugin_root: None,
    ...
    fn make_plugin_skill(
        ...
        skill.scope = cf_tools::implementations::skills::types::SkillScope::Plugin;
        skill.config_source = Some(cf_tools::types::config_source::ConfigSource::Plugin {
    ...
    fn make_plugin(name: &str) -> cf_hooks_plugins_types::PluginInfo {
        test_plugin_info(name, None)
    }

    fn make_plugin_with_origin(
        name: &str,
        origin: cf_hooks_plugins_types::PluginOrigin,
    ) -> cf_hooks_plugins_types::PluginInfo {
    ...
    fn make_plugin_with_enabled(name: &str, enabled: bool) -> cf_hooks_plugins_types::PluginInfo {
    ...
    fn space_footer_desc_is_contextual_enable_or_disable() {
        ...
        state.picker_state.selected = 0;
        state.plugins_data = TabDataState::Loaded(cf_hooks_plugins_types::PluginsListResponse {
            plugins: vec![
    ...
    fn mcp_add_url_derives_name() {
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                assert!(matches!(
                    config.transport,
                    cf_shell::util::config::McpServerTransportConfig::StreamableHttp { .. }
                ));
            }
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                match config.transport {
                    cf_shell::util::config::McpServerTransportConfig::Stdio {
                        command,
                        args,
                        ..
                    } => {
    ...
    fn build_action_single_field_plugins() {
        ...
        assert!(matches!(
            ...
            Some(ButtonAction::PluginsAction(
                cf_hooks_plugins_types::PluginsAction::Install { .. }
            ))
        ));
    }
    ...
    fn make_hook(
        ...
        source_dir: &str,
        disabled: bool,
    ) -> cf_hooks_plugins_types::HookInfo {
        cf_hooks_plugins_types::HookInfo {
            name: name.to_string(),
            event: cf_hooks_plugins_types::HookEvent::PreToolUse,
            handler_type: cf_hooks_plugins_types::HookHandlerType::Command,
            matcher: None,
    ...
    fn superpowers_source() -> cf_hooks_plugins_types::MarketplaceScanResult {
    ...
    fn local_plugins_source() -> cf_hooks_plugins_types::MarketplaceScanResult {
        cf_hooks_plugins_types::MarketplaceScanResult {
            source_name: "local-plugins".into(),
            source_kind: "local".into(),
            source_url_or_path: "/home/user/.qidi/marketplace/local".into(),
            plugins: vec![
                TestPlugin {
                    name: "my-linter",
                    version: Some("0.1.0"),
    ...
    struct TestPlugin {
        ...
        components: Option<cf_hooks_plugins_types::PluginComponents>,
    }
    ...
    impl TestPlugin {
        fn build(self) -> cf_hooks_plugins_types::MarketplacePluginEntry {
            ...
            cf_hooks_plugins_types::MarketplacePluginEntry {
    ...
    fn marketplace_add_source_builds_action() {
        ...
        match action {
            Some(ButtonAction::MarketplaceAction(
                cf_hooks_plugins_types::MarketplaceAction::AddSource { url },
            )) => {
    ...
    fn marketplace_key_r_dispatches_refresh() {
        ...
        assert!(matches!(
            action,
            Some(ButtonAction::MarketplaceAction(
                cf_hooks_plugins_types::MarketplaceAction::Refresh {
                    source_url_or_path: None
                }
            ))
        ));
    ...
    fn marketplace_modal_state_with_loaded_data() {
        ...
        state.marketplace_data =
            TabDataState::Loaded(cf_hooks_plugins_types::MarketplaceListResponse {
    ...
    fn marketplace_error_source_has_zero_plugins() {
        let error_source = cf_hooks_plugins_types::MarketplaceScanResult {
    ...
    fn component(name: &str, desc: Option<&str>) -> cf_hooks_plugins_types::ComponentItem {
        cf_hooks_plugins_types::ComponentItem::new(name, desc.map(str::to_string))
    }

    fn sample_components() -> cf_hooks_plugins_types::PluginComponents {
        cf_hooks_plugins_types::PluginComponents {
            skills: vec![
    ...
    fn marketplace_summary_empty_components_is_none() {
        let plugin = TestPlugin {
            name: "empty",
            components: Some(cf_hooks_plugins_types::PluginComponents::default()),
            ..Default::default()
        }
        .build();
    ...
    fn render_components_fields_caps_names_per_category() {
        let components = cf_hooks_plugins_types::PluginComponents {
            skills: (0..12)
    ...
    fn plugins_modal_state(
        plugins: Vec<cf_hooks_plugins_types::PluginInfo>,
    ) -> ExtensionsModalState {
        let mut state = ExtensionsModalState::new(ExtensionsTab::Plugins);
        state.plugins_data =
            TabDataState::Loaded(cf_hooks_plugins_types::PluginsListResponse { plugins });
        state
    }
    ...
    fn plugin_group_maps_each_origin_variant() {
        use cf_hooks_plugins_types::PluginOrigin;
    ...
    fn plugin_group_fallback_without_origin() {
        ...
        project.scope = cf_hooks_plugins_types::PluginScope::Project;
        ...
        cli.scope = cf_hooks_plugins_types::PluginScope::Cli;
        assert_eq!(plugin_group(&cli).key, "origin:cli");

        let mut config = make_plugin("cfg-tool");
        config.scope = cf_hooks_plugins_types::PluginScope::Config;
    ...
    fn plugin_group_unknown_origin_uses_scope_fallback() {
        let mut unknown = make_plugin_with_origin(
            "future-tool",
            cf_hooks_plugins_types::PluginOrigin::Unknown,
        );
    ...
    fn marketplace_placeholders_render_only_when_expanded() {
        ...
        source.plugins[0].components = Some(cf_hooks_plugins_types::PluginComponents::default());
