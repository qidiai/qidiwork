//! Release-safe FPS readout — `/debug fps`, `QIDI_FPS` on release builds.
//!
//! The full frame profiler (`render::frame_metrics`, `QIDI_FPS`) is compiled
...
//!
//! `QIDI_FPS` ownership: in debug/dev builds the env feeds `FrameMetrics` as
...
//! HUD from startup, so `QIDI_FPS=1` is never a silent no-op
//! ([`HONORS_QIDI_FPS_ENV`]).
//!
...
/// Whether this HUD owns the `QIDI_FPS` env gate: only where the dev
/// `FrameMetrics` overlay is compiled out. In debug/dev builds the env keeps
/// feeding that overlay alone.
const HONORS_QIDI_FPS_ENV: bool = true;
/// Runtime state for the FPS HUD. `QIDI_FPS` enables it at startup on
/// release binaries ([`HONORS_QIDI_FPS_ENV`]); `/debug fps` toggles it
...
impl FpsHud {
    pub fn new() -> Self {
        Self::with_env(std::env::var("QIDI_FPS").ok())
    }
    /// `env` is the raw `QIDI_FPS` value; the truthiness rule (nonempty and
    /// not `"0"`) matches `FrameMetrics` and `QIDI_SCROLL_DEBUG`.
    fn with_env(env: Option<String>) -> Self {
        let env_on = HONORS_QIDI_FPS_ENV && env.is_some_and(|v| !v.is_empty() && v != "0");
        Self {
            enabled: env_on,
...
pub struct FpsOverlay {
    body: String,
    /// Rows left free for overlays above (the dev `QIDI_FPS` line).
    pub top_offset: u16,
}
impl FpsOverlay {
...
mod tests {
    ...
    /// asserting against [`HONORS_QIDI_FPS_ENV`] keeps the test true under
    ...
    fn grok_fps_env_enables_hud_where_dev_overlay_absent() {
        for truthy in ["1", "full", " "] {
            assert_eq!(
                ...
                HONORS_QIDI_FPS_ENV,
                "QIDI_FPS={truthy:?} must track the env-gate owner"
            );
        }
