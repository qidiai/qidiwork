...
//! are written to `.qidi/config.toml`.
...
use cf_shell::claude_import::{ImportPlan, ImportableItem, PathKind, find_project_root};
...
fn build_rows(
    ...
    if !plan.global_items.is_empty() {
        ...
        let label = "Global  ~/.qidi/config.toml".to_string();
...
mod tests {
    use super::*;
    use cf_shell::claude_import::PathKind;
    use cf_workspace::permission::types::{PatternMode, PermissionRule, ToolFilter};
    ...
    fn filtered_plan_respects_per_item_selection_after_grouping() {
        use cf_shell::claude_import::ImportableItem;
