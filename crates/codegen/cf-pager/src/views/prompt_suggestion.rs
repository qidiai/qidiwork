...
/// Env override for the whole feature: `QIDI_PROMPT_SUGGESTIONS=0/1`.
/// When unset, the persisted `prompt_suggestions` setting applies.
pub const PROMPT_SUGGESTIONS_ENV: &str = "QIDI_PROMPT_SUGGESTIONS";

/// Env override for the model used by the suggestion call:
/// `QIDI_PROMPT_SUGGESTIONS_MODEL=<model-id>`.
pub const PROMPT_SUGGESTIONS_MODEL_ENV: &str = "QIDI_PROMPT_SUGGESTIONS_MODEL";
...
pub const PREFERRED_SUGGESTION_MODEL: &str = "cf-tools-0.1";
...
pub struct PromptSuggestionController {
    ...
    /// `QIDI_PROMPT_SUGGESTIONS` env var, falling back to the persisted
...
pub fn resolve_enabled() -> bool {
    ...
    ENV_OVERRIDE
        .get_or_init(|| cf_config::env_bool(PROMPT_SUGGESTIONS_ENV))
...
/// env override > `cf-tools-0.1` when the catalog offers it > `None`.
///
/// The hint is one tier of the shell-side resolution (env > config.toml >
/// remote settings > this hint > `cf-tools-0.1` default): the shell
