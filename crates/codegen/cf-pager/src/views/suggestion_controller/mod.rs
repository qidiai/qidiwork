...
// (e.g. qidi-code-minimal's) compiling when optional fields are added.
#[derive(Debug, Clone, Default)]
...
pub struct SuggestionController {
    ...
    /// `QIDI_SUGGESTIONS` env var. Tab-triggered completion in bash mode
    ...
    /// Resolved at construction from `QIDI_SUGGESTIONS_AI` env var.
    pub ai_enabled: bool,
    /// Model to use for AI suggestions. Sent in the `x.ai/suggest` request.
    /// Resolved at construction from `QIDI_SUGGESTIONS_AI_MODEL` env var.
...
impl SuggestionController {
    pub fn new() -> Self {
        Self {
            ...
            tab_pending: None,
            enabled: cf_config::env_bool("QIDI_SUGGESTIONS").unwrap_or(false),
            dropdown: CompletionDropdownState::default(),
            ai_enabled: cf_config::env_bool("QIDI_SUGGESTIONS_AI").unwrap_or(false),
            ai_model: std::env::var("QIDI_SUGGESTIONS_AI_MODEL")
                .ok()
                .filter(|s| !s.is_empty()),
        }
    }
