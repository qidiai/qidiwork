...
struct WelcomeLayoutInput<'a> {
    ...
    prompt_compact: bool,
    announcement: Option<&'a cf_announcements::RemoteAnnouncement>,
...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **QIDI Code** VERSION+channel **Beta** (right-aligned).
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: team | api_key | QIDI Code [channel] (right-aligned, gray).
    HeroFooter,
    /// Hero inline: **QIDI Code**  VERSION (left-aligned).
    HeroInline,
}
...
pub(super) fn render_version_badge(
    ...
    let channel = cf_update::channel_label();
    match &mode {
        VersionBadgeMode::Full { .. } => {
            ...
            spans.push(Span::styled(
                format!("{}{}", cf_version::VERSION, channel),
                Style::default().fg(theme.gray),
            ));
            spans.push(Span::styled(
                " Beta | 测试版",
...
pub struct WelcomeRenderParams<'a> {
    ...
    pub announcement: Option<&'a cf_announcements::RemoteAnnouncement>,
    ...
    pub foreign_resume_hint: Option<&'a cf_workspace::foreign_sessions::RecentForeignSession>,
    pub is_api_key_auth: bool,
    pub session_picker_content_results:
        Option<&'a [cf_shell::extensions::session_search::SearchSessionHit]>,
    ...
    pub gate: Option<&'a cf_shell::auth::GateInfo>,
...
pub fn render_welcome(
    ...
    let mut result = match params.auth_state {
        AuthState::Pending { error } => {
            let label = params.login_label.unwrap_or("QIDI");
        ...
        AuthState::Done if params.is_zdr_blocked => {
            ...
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                ...
                buf,
                Some((
                    "QIDI Code is not yet available for this account.",
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
...
/// multi-line block showing the workspace path and the warning that QIDI Code
...
fn render_welcome_trust(
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            "QIDI Code may run or modify contents in this directory,",
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        Line::from(Span::styled(
...
fn render_announcement_section(
    ...
    buf: &mut Buffer,
    theme: &Theme,
    announcement: &cf_announcements::RemoteAnnouncement,
    min_width_hint: u16,
    content_height: u16,
    expanded: bool,
    mouse_pos: Option<(u16, u16)>,
...
fn render_welcome_done(
    ...
    let (cursor_pos, post_flush_escapes) = if show_picker {
        ...
        let gate_text = p
            ...
            .unwrap_or("需要升级到 pro 订阅 / Upgrade to a pro plan");
        let msg = Line::from(Span::styled(
            gate_text,
            Style::default().fg(theme.gray_bright),
        ))
...
mod tests {
    ...
    fn foreign_resume_tip_names_each_tool_and_age() {
        use cf_workspace::foreign_sessions::ForeignSessionTool;
        ...
        for (tool, label) in [
            ...
            let hint = cf_workspace::foreign_sessions::RecentForeignSession {
                tool,
    ...
    fn long_ann() -> cf_announcements::RemoteAnnouncement {
...
the usual channels. "
    ...
    fn announcement_equal_for_short_message() {
        let area = Rect::new(0, 0, 120, 60);
        let a = cf_announcements::RemoteAnnouncement {
            title: Some("FYI".into()),
