...
use cf_voice::{SharedVoiceAuth, VoiceAuthProvider};
...
pub fn build_voice_auth(auth_manager: Arc<cf_shell::auth::AuthManager>) -> SharedVoiceAuth {
    Arc::new(AuthManagerVoiceAuth(
        cf_shell::auth::shared_api_key_provider(auth_manager),
    ))
}
