...
use cf_fast_worktree::WorktreeRecord;
...
async fn dispatch(command: WorktreeCommand, tx: &cf_acp_lib::AcpAgentTx) -> Result<()> {
    match command {
...
async fn cmd_show(tx: &cf_acp_lib::AcpAgentTx, id_or_path: &str) -> Result<()> {
...
async fn cmd_db(tx: &cf_acp_lib::AcpAgentTx, command: WorktreeDbCommand) -> Result<()> {
    match command {
...
mod tests {
    ...
    fn ext_envelope_unwraps_success_result() {
        let json = r#"{"result": {"path": "/home/user/.qidi/worktrees.db"}, "error": null}"#;
        #[derive(serde::Deserialize)]
        struct PathResp {
            path: String,
        }
        ...
        assert_eq!(inner.path, "/home/user/.qidi/worktrees.db");
    }
