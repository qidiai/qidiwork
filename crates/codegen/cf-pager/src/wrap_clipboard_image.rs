...
//! `QIDI_OSC52_SINK` / `LC_QIDI_OSC52_SINK`). Do not generalize this pattern
//! to untrusted multiplexers without an explicit allowlist.

use base64::Engine as _;
use cf_pager_render::clipboard::{ImageData, osc52_sink_active};
...
/// Successful host image frame: `QIDI_WRAP_IMG\n<mime>\n<base64>`.
pub const MAGIC_IMG: &str = "QIDI_WRAP_IMG";

/// Host has no image (`QIDI_WRAP_NONE`; not a prefix of [`MAGIC_IMG`]).
pub const MAGIC_NONE: &str = "QIDI_WRAP_NONE";
...
mod tests {
    ...
    fn garbage_is_not_wrap_paste() {
        ...
        assert_eq!(try_decode_wrap_host_image_paste("QIDI_WRAP_IM"), None);
    }

    #[test]
    fn malformed_img_frame_consumed_not_text() {
        assert_eq!(
            try_decode_wrap_host_image_paste("QIDI_WRAP_IMG"),
            Some(WrapImagePaste::NoImage)
        );
        assert_eq!(
            try_decode_wrap_host_image_paste("QIDI_WRAP_IMG\nbad"),
    ...
    fn oversized_b64_rejected_before_decode() {
        ...
        let payload = format!("QIDI_WRAP_IMG\nimage/png\n{huge}");
        assert_eq!(
