...
//!
//! Uses `qidi-code-pty-harness` (`PtyHarness`) + Shift+Tab (CSI Z,
...
//! Always-Approve.
//!
//! Auth: seeds `HOME/.qidi/auth.json` from `QIDI_AUTH_JSON` (path) or the
//! developer's `~/.qidi/auth.json` so the pager skips device-login when
...
//!
//! Run with:
//! `cargo test -p qidi-code --test pty_auto_mode -- --ignored --nocapture`

use std::path::{Path, PathBuf};
use std::time::Duration;

use cf_pager_pty_harness::{PtyHarness, pager_binary};
...
/// Prefer explicit path, else the user's real `~/.qidi/auth.json`.
fn auth_json_source() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("QIDI_AUTH_JSON") {
...
fn prepare_sandbox(home: &Path, gate_on: bool) -> Vec<(String, String)> {
    ...
    if let Some(src) = auth_json_source() {
        ...
        eprintln!("pty_auto_mode: no ~/.qidi/auth.json — may hit device login");
    }

    let home_s = home.display().to_string();
    let mut env = vec![
        ("HOME".into(), home_s.clone()),
        ("QIDI_HOME".into(), grok.display().to_string()),
    ...
    // of the developer's shell. `QIDI_AUTO_PERMISSION_MODE` is the highest gate
    // layer below requirements; "1"/"0" parse to on/off (cf_config::
    ...
    env.push((
        "QIDI_AUTO_PERMISSION_MODE".into(),
...
fn require_auth() -> bool {
    std::env::var("QIDI_PTY_REQUIRE_AUTH").is_ok_and(|v| v == "1" || v == "true")
}
...
fn pty_shift_tab_cycles_to_auto_mode_banner() {
    ...
    let mut harness = PtyHarness::new(&binary, ROWS, COLS, &[], &env_refs)
        .expect("spawn pager in PTY (qidi-code-pty-harness)");
    ...
    if is_login_screen(&early) {
        assert!(
            !require_auth(),
            "QIDI_PTY_REQUIRE_AUTH set but pager hit the login screen — auth seeding broke"
        );
