...
pub(crate) use cf_pager_pty_harness::{
...
pub(crate) fn contextual_hints_env(content: &ContentController) -> Vec<(String, String)> {
    ...
    env.push(("QIDI_CONTEXTUAL_HINTS".into(), "1".into()));
    env
}
...
// `cf_pager_pty_harness::flows` (re-exported above).
...
/// would never fetch `/v1/settings`. Spawns WITHOUT `QIDI_ANNOUNCEMENTS_OVERRIDE`
...
pub(crate) fn spawn_polling_session_with_env(
    ...
    env_refs.push(("QIDI_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS", "1"));
...
/// (`QIDI_TEST_VERSION`) and an explicit `QIDI_FOLDER_TRUST` — `1` when `feature_on`,
/// else `0` (an explicit opt-out that overrides the now-on default). HOME/QIDI_HOME
...
pub(crate) fn trust_env(content: &ContentController, feature_on: bool) -> Vec<(String, String)> {
    ...
    // auto-trust.
    env.push(("QIDI_TEST_VERSION".into(), "0.0.0-sim".into()));
    // Set QIDI_FOLDER_TRUST explicitly: the default is on, so `0` is the opt-out
    // that exercises the feature-off path rather than relying on an absent var.
    let folder_trust = if feature_on { "1" } else { "0" };
    env.push(("QIDI_FOLDER_TRUST".into(), folder_trust.into()));
    env
}
...
// `wait_for_labels_absent` lives in `cf_pager_pty_harness::flows`
// (re-exported above).
...
/// Seed `~/.qidi/config.toml` with a `[ui]` section body (e.g.
...
pub(crate) fn mouse_toggle_env(content: &ContentController) -> Vec<(String, String)> {
    let mut env = content.env_for_pager();
    env.push(("QIDI_MOUSE_REPORTING_TOGGLE".into(), "true".into()));
    env
}
...
pub(crate) const ESC_DOUBLE_PRESS_ENV: &str = "QIDI_ESC_DOUBLE_PRESS_MS";
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    ...
    env.push((
        ...
        cf_pager::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string(),
    ));
    let env_refs: Vec<(&str, &str)> = env.iter().map(|(k, v)| (k.as_str(), v.as_str())).collect();
    PtyHarness::new(&binary, DEFAULT_ROWS, DEFAULT_COLS, &[], &env_refs).expect("spawn pager")
}
...
/// `QIDI_HOME`, returning the exit code (`None` if it never exited within
...
pub(crate) fn run_wrap(wrap_args: &[&str], extra_env: &[(&str, &str)]) -> (Option<u32>, String) {
    ...
    let mut env: Vec<(&str, &str)> = vec![("QIDI_HOME", &home_str), ("NO_COLOR", "1")];
...
/// Dump an asciinema cast of `harness` into `$QIDI_PTY_CAST_DIR/<file_name>`
...
pub(crate) fn write_cast_if_requested(harness: &PtyHarness, file_name: &str) {
    let Ok(dir) = std::env::var("QIDI_PTY_CAST_DIR") else {
        return;
    };
    if dir.is_empty() {
        return;
...
pub(crate) use cf_pager_pty_harness::host_clipboard::{
...
pub(crate) use cf_pager_pty_harness::host_clipboard::clipboard_roundtrip_works;
