...
fn spawn_with_announcements(content: &ContentController, override_json: &str) -> PtyHarness {
    ...
    env.push((
        "QIDI_ANNOUNCEMENTS_OVERRIDE".into(),
...
/// safe-open path — observed via the `QIDI_TEST_OPEN_URL_FILE` seam, never a
...
async fn promo_cta_click_opens_link_and_hide_roundtrip() {
    ...
    let extra_env = [
        ("QIDI_TEST_OPEN_URL_FILE", url_file_str.as_str()),
...
/// Pinned (`dismissible:false`) promo seeded via `QIDI_ANNOUNCEMENTS_OVERRIDE`.
...
/// `QIDI_TEST_OPEN_URL_FILE` seam + a `TERM_PROGRAM` pin for OSC 8).
