...
//
// `QIDI_SCROLL_MODE=wheel` + `QIDI_SCROLL_LINES=1` on a `TERM_PROGRAM=zed`
...
// - Without QIDI_SCROLL_LINES=1, Zed's wheel profile prices the burst at
//   3 lines/event → 9 rows.
// - Without QIDI_SCROLL_MODE=wheel, Auto finalizes a 3-event ZERO-interval
...
// no acceleration, and the harness's hermetic QIDI_HOME pins scroll_speed at
// its default (50 → 1.0x).
...
/// **Env-forced wheel pricing e2e.** The `QIDI_SCROLL_MODE` /
/// `QIDI_SCROLL_LINES` overrides must reach the live config: a 3-event burst
