...
async fn prompt_suggestion_ghost_tab_accepts() {
    // The suggestion model (`cf-tools-0.1`, the shell's built-in default)
    ...
    env.retain(|(k, _)| k != "QIDI_PROMPT_SUGGESTIONS");
    env.push(("QIDI_PROMPT_SUGGESTIONS".into(), "true".into()));
