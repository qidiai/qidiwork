...
use cf_pager_pty_harness::PtyHarness;
pub(crate) use cf_pager_pty_harness::{SGR_SCROLL_DOWN, SGR_SCROLL_UP};
...
/// (`qidi-code-pty-harness/src/scroll_matrix/runner.rs`) ports this
...
/// `qidi-code-pty-harness/src/scroll_matrix/session.rs` is a port of
...
/// (e.g. `QIDI_SCROLL_SPEED`) appended to the controller's env.
