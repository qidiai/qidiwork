...
/// Like [`seed_fake_oauth`], but under the `QIDI_LOCAL_AUTH` dev issuer
...
/// `QIDI_LOCAL_AUTH=1` in the spawn env so the shell's scope-key lookup
/// resolves this entry.
...
fn spawn_subscription_pager(
    ...
    env_refs.push(("QIDI_LOCAL_AUTH", "1"));
...
#[ignore = "PTY e2e; run with cargo test -p qidi-code --test pty_e2e -- --ignored"]
async fn subscription_watch_polls_free_tier_then_goes_dormant_after_upgrade() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subwatch",
        &[("QIDI_SUBSCRIPTION_WATCH_INTERVAL_SECS", "1")],
    );
...
async fn startup_gate_shows_paywall_for_free_user_after_live_check() {
    ...
    content.server().set_settings(json!({
        ...
        "gate_url": "https://grok.com/supergrok?referrer=cf-tools",
...
async fn stale_gate_push_never_flashes_paywall_for_subscribed_user() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subgate-paid",
        &[("QIDI_SUBSCRIPTION_WATCH_INTERVAL_SECS", "0")],
    );
