...
/// `QIDI_OSC52_SINK=1` (the signal an inner `grok` uses to trust OSC 52 over
...
fn wrap_osc52_sink_env_advertised_through_shell() {
    let (code, raw) = run_wrap(
        &["echo sink=$QIDI_OSC52_SINK"],
        &[("SHELL", "/bin/sh"), ("QIDI_OSC52_SINK", "0")],
    );

    assert!(
        raw.contains("sink=1"),
        "wrapped child must see QIDI_OSC52_SINK=1 through the shell route\nraw:\n{raw}"
    );
    assert!(
        !raw.contains("sink=0"),
