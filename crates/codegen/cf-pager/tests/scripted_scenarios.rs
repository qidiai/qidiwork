...
//!
//! These tests exercise the real `qidi-code` binary through a PTY using
...
/// enterprise deploy report: with `QIDI_GOAL=1`, `/goal` must
...
/// explicitly off (`QIDI_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// Full folder-trust session: with `QIDI_FOLDER_TRUST=1` and a git repo that
...
/// also pins `QIDI_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
