...
fn render_with_filter_active_and_small_viewport_clamps_scroll() {
    ...
    cf_pager::views::settings_modal::render_settings_modal(
...
fn defaults_round_trip_through_registry() {
    use cf_pager::settings::{SettingValue, current_value_for};
    let reg = SettingsRegistry::defaults();
    ...
    cf_pager::appearance::cache::set_keep_text_selection(
        cf_pager::appearance::TextSelection::Flash,
    );
    cf_pager::appearance::cache::set_show_thinking_blocks(true);
    cf_pager::appearance::cache::set_prompt_suggestions(true);
    cf_pager::appearance::cache::set_group_tool_verbs(true);
    cf_pager::appearance::cache::set_scroll_mode(
        cf_pager::appearance::ScrollMode::Auto,
    );
    cf_pager::appearance::cache::set_invert_scroll(false);
    // 3 = the registry default shown while the profile is in charge.
    cf_pager::appearance::cache::set_scroll_lines(3);
    ...
    for meta in reg.all() {
        ...
        let default_value = cf_pager::settings::default_value_for(meta);
...
// `crates/codegen/qidi-code/src/app/dispatch.rs::tests` (next to
...
fn pr6_current_value_for_reads_pager_snapshot() {
    use cf_pager::settings::current_value_for;
...
fn pr6_permission_mode_picker_enter_dispatches_set_permission_mode_commit() {
    use cf_pager::app::actions::PermissionModeKind;
    let reg = SettingsRegistry::defaults();
...
fn reset_overlay_dims_all_rows_except_target() {
    ...
    use cf_pager::views::settings_modal::ResetConfirmOverlay;
...
fn render_modal_to_string(s: &mut SettingsModalState, width: u16, height: u16) -> String {
    ...
    let mut buf = Buffer::empty(area);
    cf_pager::views::settings_modal::render_settings_modal(&mut buf, area, s, false, None);
...
fn restart_pill_hidden_when_edited_but_collapsed() {
    use cf_pager::settings::{PagerLocalSnapshot, SettingsRegistry};
    use cf_shell::agent::config::UiConfig;
...
fn reset_confirm_prompt_helper_builds_well_formed_string_for_every_setting() {
    use cf_pager::settings::{PagerLocalSnapshot, SettingsRegistry};
    use cf_pager::views::modal::{ActiveModal, ModalConfirmation, reset_confirm_prompt};
    use cf_shell::agent::config::UiConfig;
    let reg = SettingsRegistry::defaults();
    for meta in reg.all() {
...
fn pr8_default_model_and_max_thoughts_width_defaults_roundtrip() {
    ...
    assert_eq!(
        cf_pager::settings::default_value_for(dm_meta),
    ...
    assert_eq!(
        cf_pager::settings::default_value_for(mt_meta),
...
fn pr9_privacy_slash_command_parses_aliases() {
    use cf_pager::slash::commands::privacy::parse_privacy_arg;
...
fn pr10_plan_mode_picker_enter_dispatches_set_commit() {
    use cf_pager::app::actions::PlanModeKind;
...
fn render_mermaid_picker_enter_dispatches_set_commit() {
    use cf_pager::appearance::RenderMermaid;
...
fn pr14_string_settings_use_known_model_validator() {
    use cf_pager::settings::DynamicEnumSource;
    let reg = SettingsRegistry::defaults();
...
fn keep_text_selection_picker_enter_dispatches_set_commit() {
    use cf_pager::appearance::TextSelection;
    ...
    cf_pager::appearance::cache::set_keep_text_selection(TextSelection::Flash);
    let mut s = make_state();
    navigate_to(&mut s, "keep_text_selection");
    let _ = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn keep_text_selection_hold_snapshot_seeds_picker_at_hold() {
    ...
    cf_pager::appearance::cache::set_keep_text_selection(TextSelection::Hold);
    let ui = UiConfig {
...
fn invert_scroll_enter_dispatches_typed_setter() {
    cf_pager::appearance::cache::set_invert_scroll(true);
    let mut s = make_state();
    navigate_to(&mut s, "invert_scroll");
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn show_thinking_blocks_space_dispatches_typed_setter() {
    ...
    cf_pager::appearance::cache::set_show_thinking_blocks(false);
    let mut s = make_state();
...
fn prompt_suggestions_space_dispatches_typed_setter() {
    // Pin off so space toggles to true.
    cf_pager::appearance::cache::set_prompt_suggestions(false);
    let mut s = make_state();
...
fn group_tool_verbs_cache_off_dispatches_on() {
    ...
    cf_pager::appearance::cache::set_group_tool_verbs(false);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
...
fn collapsed_edit_blocks_space_dispatches_typed_setter() {
    ...
    cf_pager::appearance::cache::set_collapsed_edit_blocks(false);
    let mut s = make_state();
...
fn collapsed_edit_blocks_cache_on_dispatches_off() {
    ...
    cf_pager::appearance::cache::set_collapsed_edit_blocks(true);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
