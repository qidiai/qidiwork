...
use cf_agent::plugins::manifest::{ManifestLoadResult, load_manifest, name_from_dirname};
...
fn ensure_manifest_for_root_skills(
    ...
    use cf_agent::plugins::manifest::load_manifest;

    // Skip if a manifest already exists.
    if let Ok(cf_agent::plugins::manifest::ManifestLoadResult::Found(_)) =
...
mod tests {
    ...
    fn with_test_registry<T>(f: impl FnOnce(&mut InstallRegistry) -> T) -> T {
        ...
        // test in this binary can cache the real `~/.qidi` before this runs,
    ...
    fn transactional_update_save_failure_rolls_back_install_and_disk_registry() {
        with_test_registry(|registry| {
            ...
            unsafe { std::env::set_var("XAI_QIDI_TEST_FAIL_REGISTRY_SAVE_AFTER_SERIALIZE", "1") };
            let result = update_from_marketplace_entry_transactional(
                marketplace.path(),
                &entry,
                provenance(marketplace.path(), "plugins/demo"),
            ...
            unsafe { std::env::remove_var("XAI_QIDI_TEST_FAIL_REGISTRY_SAVE_AFTER_SERIALIZE") };
