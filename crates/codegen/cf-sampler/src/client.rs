...
use cf_sampling_types::error::{parse_error_bytes, try_parse_stream_error};
...
pub use cf_sampling_types::ApiBackend;
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = cf_sampling_types::reported_cost_ticks(
        value
...
pub struct SamplingClient {
    ...
    /// Proactive rate limiter to avoid hitting API 429s.
...
struct ClientDefaults {
    ...
    doom_loop_recovery: Option<cf_sampling_types::DoomLoopRecoveryPolicy>,
}
...
impl SamplingClient {
    ...
    pub fn new(config: SamplerConfig) -> Result<Self> {
        ...
        Ok(Self {
            ...
            rate_limiter: Arc::new(crate::rate_limiter::ApiRateLimiter::default()),
        })
    }
    ...
    /// Access the proactive rate limiter.
    pub fn rate_limiter(&self) -> &crate::rate_limiter::ApiRateLimiter {
        &self.rate_limiter
    }
    ...
    pub async fn conversation_stream_responses(
        ...
        let extra_tools = cf_sampling_types::extra_raw_tools(&request.hosted_tools);
...
mod tests {
    ...
    fn new_applies_extra_headers() {
        ...
        cfg.extra_headers
            .insert("x-XAI-token-auth".to_string(), "cli".to_string());
    ...
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = cf_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
        ...
        let make = |ticks: i64| {
            format!(
                ...
                "response": {{
                    ...
                    "model": "cf-tools", "status": "completed", "output": [],
                    "usage": {{
                        "input_tokens": 10,
