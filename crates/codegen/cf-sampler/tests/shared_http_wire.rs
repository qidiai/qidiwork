...
use cf_sampler::SamplingClient;
use cf_test_support::spawn_counting_server;

/// Pin the env these assertions depend on before any client is built, so
/// ambient shell exports (`QIDI_SAMPLER_SHARED_CLIENT=0`,
/// `QIDI_POOL_MAX_IDLE=0`) cannot flip the expected pooling behavior.
fn pin_env() {
    ...
    PIN.call_once(|| {
        ...
        unsafe {
            std::env::remove_var("QIDI_SAMPLER_SHARED_CLIENT");
            std::env::set_var("QIDI_POOL_MAX_IDLE", "2");
            std::env::set_var("QIDI_POOL_IDLE_TIMEOUT_SECS", "90");
        }
    });
}
