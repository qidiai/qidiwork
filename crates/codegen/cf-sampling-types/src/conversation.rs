...
// engine in `crates/common/cf-compaction` operate over cf-tools's
...
impl cf_compaction::CompactionItem for ConversationItem {
    fn role(&self) -> cf_compaction::CompactionRole {
        use cf_compaction::CompactionRole;
        // cf-tools has no distinct `Developer` role; everything maps onto
    ...
    fn is_compaction_summary(&self) -> bool {
        // cf-tools has no structural marker that uniquely identifies a prior
        ...
        // (add a dedicated marker) before routing cf-tools history through
        // the shared `history`/`inter` filter.
        false
    }

    fn attachment_refs(&self) -> Vec<cf_compaction::CompactionFileRef> {
        // cf-tools `UserItem`s carry only `Text`/`Image { url }` content
...
impl cf_compaction::CompactionItemFactory for ConversationItem {
...
/// ([stream/messages.rs:340](crates/codegen/cf-sampler/src/stream/messages.rs))
...
///
/// This is `pub` so downstream crates (`qidi-code`) can re-use it
...
impl From<ChatResponseMessage> for ConversationItem {
    fn from(msg: ChatResponseMessage) -> Self {
        ...
        // ([crates/codegen/cf-sampler/src/stream/chat_completions.rs])
...
/// ([crates/codegen/cf-sampler/src/stream/messages.rs]) instead
...
mod compaction_item_bridge_tests {
    use super::*;
    use cf_compaction::{CompactionItem, CompactionItemFactory, CompactionRole};
    ...
    fn metadata_accessors_are_conservative() {
        // cf-tools has no structural compaction-summary marker, and no
...
mod tests {
    ...
    fn test_transform_cwd_transforms_tool_call_arguments() {
        ...
        let worktree = "/home/user/.qidi/worktrees/project/ab-uuid-a";
    ...
    fn test_transform_cwd_worktree_to_root_syncback() {
        ...
        let worktree = "/home/user/.qidi/worktrees/myproject/fork-a";
    ...
    fn test_transform_cwd_assistant_only_tool_calls_no_content() {
        ...
        let worktree = "/home/user/.qidi/worktrees/proj/fork-a";
    ...
    //      (earlier cf-tools / chat-completions written as v1)
