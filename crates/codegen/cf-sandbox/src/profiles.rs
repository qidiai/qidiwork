...
//! `off`. Custom profiles via `~/.qidi/sandbox.toml` or `.qidi/sandbox.toml`.
...
/// Load sandbox config from `~/.qidi/sandbox.toml` and `.qidi/sandbox.toml`.
///
...
impl ProfileName {
    ...
    pub fn to_capability_set_with_config(
        ...
        // directories like ~/.qidi/ that may not exist on first run.
                ...
                let profile_config = config.profiles.get(name).ok_or_else(|| {
                    anyhow::anyhow!(
                        "Custom sandbox profile '{name}' not found. \
                         Define it in ~/.qidi/sandbox.toml or .qidi/sandbox.toml:\n\n\
                         [profiles.{name}]\n\
                         extends = \"workspace\"\n\
                         read_only = [\"/data\"]\n"
                    )
