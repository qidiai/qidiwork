...
fn profile_from_env() -> cf_sandbox::ProfileName {
    cf_sandbox::ProfileName::Custom(std::env::var(PROFILE_ENV).expect(PROFILE_ENV))
}
...
fn subprocess_entry() {
    ...
    #[cfg(target_os = "linux")]
    {
        if !cf_sandbox::is_inside_bwrap() {
            ...
            match cf_sandbox::bwrap_reexec_for_profile(&profile_from_env(), workspace) {
                Some(mut cmd) => {
    ...
    match scenario.as_str() {
        "block_deny" => {
            let mut sandbox = cf_sandbox::SandboxManager::new(profile_from_env(), workspace);
...
fn run_deny_case(
    ...
    let support = cf_sandbox::SandboxManager::support_info();
