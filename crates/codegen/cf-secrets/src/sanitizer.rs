...
/// B6: 已知敏感字段名（大小写不敏感匹配）。命中时直接 mask 整个值，不看内容——
/// 配合基于值模式的正则脱敏（[`redact_secrets`]）兜底，堵住"值本身看不出是密钥
/// （如短随机串、JWT payload）但字段名已说明它是敏感数据"的漏网场景。
...
/// Scrub every string value in a JSON document.
///
/// B6: 遍历时优先按字段名脱敏——object 的 key 命中 [`SENSITIVE_FIELD_NAMES`] 时，
/// 该 key 下的整个值（含嵌套 object/array 里的字符串）直接 mask，不看内容；否则
/// 递归下去，叶子字符串走原有基于值模式的正则脱敏（[`redact_secrets`]）兜底。
...
/// B6: 把一个值（及其嵌套子值里的所有字符串）整体替换为 [`REDACTED`]。
/// 用于字段名已判定为敏感的场景——不看值内容。number/bool/null 保持不动。
fn redact_value_in_place(v: &mut serde_json::Value) {
    match v {
...
mod tests {
    ...
    // --- B6: structured field-name redaction ---

    #[test]
    fn sensitive_field_name_redacted_regardless_of_value() {
        // 值本身看不出是密钥，但字段名 api_key 已说明敏感 → 整值 mask。
        let mut v = serde_json::json!({ "api_key": "hunter2", "note": "plain text" });
        redact_json_string_values(&mut v);
        assert_eq!(v["api_key"], serde_json::json!(REDACTED));
        assert_eq!(v["note"], serde_json::json!("plain text"));
    }

    #[test]
    fn jwt_payload_field_redacted() {
        let mut v = serde_json::json!({
            "id_token": "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxIn0.abc"
        });
        redact_json_string_values(&mut v);
        assert_eq!(v["id_token"], serde_json::json!(REDACTED));
    }

    #[test]
    fn non_sensitive_field_falls_back_to_regex() {
        let mut v = serde_json::json!({
            "comment": "Authorization: Bearer eyJhbGciOiJIUzI1NiJ9.foo.bar",
            "plain": "just normal text",
        });
        redact_json_string_values(&mut v);
        let comment = v["comment"].as_str().unwrap();
        assert!(comment.contains(REDACTED), "bearer not scrubbed: {comment}");
        assert_eq!(v["plain"], serde_json::json!("just normal text"));
    }

    #[test]
    fn case_insensitive_field_name_match() {
        for key in ["API_KEY", "ApiKey", "api_key", "Authorization"] {
            assert!(is_sensitive_field_name(key), "should match: {key}");
        }
        assert!(!is_sensitive_field_name("comment"));

        let mut v = serde_json::json!({ "API_KEY": "abc", "ApiKey": "def" });
        redact_json_string_values(&mut v);
        assert_eq!(v["API_KEY"], serde_json::json!(REDACTED));
    ...
    #[test]
    fn nested_object_sensitive_field_redacted() {
        let mut v = serde_json::json!({
            "outer": { "token": "x", "keep": "y" },
            "list": [ { "secret": "z" } ],
        });
        redact_json_string_values(&mut v);
        assert_eq!(v["outer"]["token"], serde_json::json!(REDACTED));
        assert_eq!(v["outer"]["keep"], serde_json::json!("y"));
        assert_eq!(v["list"][0]["secret"], serde_json::json!(REDACTED));
    }

    #[test]
    fn sensitive_field_masks_whole_nested_value() {
        // 字段名敏感时，其下嵌套 object 里的所有字符串都被 mask（不看内容）。
        let mut v = serde_json::json!({
            "credentials": { "user": "alice", "pass": "p" }
        });
        redact_json_string_values(&mut v);
        assert_eq!(v["credentials"]["user"], serde_json::json!(REDACTED));
        assert_eq!(v["credentials"]["pass"], serde_json::json!(REDACTED));
    }
}
