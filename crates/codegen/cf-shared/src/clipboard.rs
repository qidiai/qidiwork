...
//! `QIDI_CLIPBOARD_NO_NATIVE_READ=1` disables the in-process read entirely
...
/// the `QIDI_CLIPBOARD_NO_DATA_CONTROL` kill-switch env var is set. The kill
...
mod platform {
    ...
    /// path, and `None` when `QIDI_CLIPBOARD_NO_NATIVE_READ` is set (kill
    ...
    pub(super) fn native_image_read() -> Option<super::ImageData> {
        if std::env::var_os("QIDI_CLIPBOARD_NO_NATIVE_READ").is_some() {
            return None;
        }
        let _guard = PASTEBOARD_LOCK.lock();
        objc2::rc::autoreleasepool(|_| {
...
mod platform {
    ...
    /// Memoized read of the `QIDI_CLIPBOARD_NO_DATA_CONTROL` kill switch —
    ...
    fn data_control_kill_switch_set() -> bool {
        ...
        *SET.get_or_init(|| std::env::var_os("QIDI_CLIPBOARD_NO_DATA_CONTROL").is_some())
    }

    /// True when the arboard leg must be skipped entirely: on Wayland the
    /// `QIDI_CLIPBOARD_NO_DATA_CONTROL` kill switch has to stop arboard from
        ...
        LEASE
            .get_or_init(|| {
                if arboard_wayland_bypassed() {
                    tracing::debug!("arboard leg disabled (QIDI_CLIPBOARD_NO_DATA_CONTROL)");
                    return None;
                }
    ...
    fn arboard_read_with_deadline<T: Send + 'static>(
        ...
        if arboard_wayland_bypassed() {
            anyhow::bail!("arboard leg disabled (QIDI_CLIPBOARD_NO_DATA_CONTROL)");
        }
    ...
    pub(super) fn wayland_data_control_supported() -> bool {
        ...
        // switch (precedent: QIDI_CLIPBOARD_NO_NATIVE_READ) and non-Wayland
