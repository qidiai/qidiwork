...
//! image bytes.
//!
//! Both the TUI ([`cf_pager::prompt_images`]) and the server-side
...
//! [`cf_pager::prompt_images::display_text`]. The shape of this
...
/// position (the two diverge — see `AttachedImages` in `qidi-code`).
...
/// [`AttachedImages`](cf_tools::types::resources::AttachedImages))
...
///   separator — see [`cf_pager::prompt_images::display_text`].
...
/// [`cf_pager::prompt_images::read_image_at_path`], which has no
...
fn decode_image_mime(data: &[u8]) -> Option<&'static str> {
    cf_tools::util::image_validate::validate_image_bytes_with(data, false)
        .ok()
        .map(|(_, _, mime)| mime)
}
...
mod tests {
    ...
    /// `cf_pager::prompt_images::display_text`. A
    ...
    fn attached_image_references_prefers_file_path_over_data() {
        ...
        let img = agent_client_protocol::ImageContent::new("AAAA", "image/png")
            .uri(Some(
                "file:///Users/me/.qidi/sessions/s/images/image-1.png".into(),
            ))
        ...
        assert_eq!(
            ...
            vec![(
                1,
                "/Users/me/.qidi/sessions/s/images/image-1.png".to_string()
            )]
        );
    }
