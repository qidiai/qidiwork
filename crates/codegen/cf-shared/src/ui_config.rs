...
pub struct UiConfig {
    ...
    /// connect time (CLI `--hunk-tracker-mode` / `QIDI_HUNK_TRACKER` override
    ...
    /// `None` = on (client default). Written by the pager's settings modal;
    /// the `QIDI_PROMPT_SUGGESTIONS` env var overrides at runtime.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl Default for UiConfig {
    fn default() -> Self {
        Self {
            ...
            theme: None,
            fork_secondary_model: cf_models::default_model().to_string(),
            yolo: false,
            ui_theme: None,
            compact_mode: false,
            simple_mode: None,
