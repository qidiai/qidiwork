...
pub const QIDI_GATEWAY_URL_ENV: &str = "QIDI_GATEWAY_URL";
...
pub const QIDI_DISABLE_CUSTOM_BRIDGE_ENV: &str = "QIDI_DISABLE_CUSTOM_BRIDGE";
/// `true` when the custom-method bridge passthrough is force-disabled via
/// [`QIDI_DISABLE_CUSTOM_BRIDGE_ENV`]. Accepts `1`/`true` (case-insensitive).
pub fn custom_bridge_disabled() -> bool {
    std::env::var(QIDI_DISABLE_CUSTOM_BRIDGE_ENV)
        .map(|v| {
            let v = v.trim();
...
/// Parse `QIDI_GATEWAY_URL` into a [`url::Url`]. Unset, empty, or
...
pub fn parse_gateway_url() -> Option<url::Url> {
    let raw = std::env::var(QIDI_GATEWAY_URL_ENV).ok()?;
    if raw.is_empty() {
        return None;
    }
    if true {
        tracing::warn!(
            env = QIDI_GATEWAY_URL_ENV,
            "QIDI_GATEWAY_URL is set but this build lacks the `chat` feature; staying in local mode"
        );
        return None;
    }
    match url::Url::parse(&raw) {
        ...
        Err(err) => {
            tracing::warn!(
                env = QIDI_GATEWAY_URL_ENV, error = % err,
                "QIDI_GATEWAY_URL is not a valid URL; falling back to local mode (raw value omitted to avoid leaking userinfo)"
            );
            None
        }
    }
...
mod tests {
    ...
    fn parse_gateway_url_returns_none_when_unset() {
        let _env = EnvVarGuard::remove(QIDI_GATEWAY_URL_ENV);
        assert!(parse_gateway_url().is_none());
    }
    #[test]
    fn parse_gateway_url_returns_none_when_empty() {
        let _env = EnvVarGuard::set(QIDI_GATEWAY_URL_ENV, "");
        assert!(parse_gateway_url().is_none());
    }
    #[test]
    fn parse_gateway_url_returns_none_for_malformed_url() {
        let _env = EnvVarGuard::set(QIDI_GATEWAY_URL_ENV, "not a url");
        assert!(
    ...
    fn custom_bridge_disabled_defaults_false_when_unset() {
        let _env = EnvVarGuard::remove(QIDI_DISABLE_CUSTOM_BRIDGE_ENV);
        assert!(!custom_bridge_disabled());
    }
    #[test]
    fn custom_bridge_disabled_true_for_one_and_true() {
        for v in ["1", "true", "TRUE", " true "] {
            let _env = EnvVarGuard::set(QIDI_DISABLE_CUSTOM_BRIDGE_ENV, v);
            assert!(
