...
impl ChangelogManager {
    pub fn new() -> Self {
        // Prefer live `$QIDI_HOME` so harness-injected homes (PTY e2e) always
    ...
    /// `grok_home()` OnceLock). A seeded `$QIDI_HOME` set on the pager
    /// process is always honoured even if some earlier init path cached a
    /// different home.
    fn from_env_home() -> Self {
        let home = std::env::var_os("QIDI_HOME")
    ...
    ///
    /// When `QIDI_CHANGELOG_OFFLINE` is set (PTY / integration tests), skip
    ...
    /// `$QIDI_HOME` so harness-injected env always applies.
    ///
    ...
    /// without mutating process-global env (`QIDI_HOME` /
    /// `QIDI_CHANGELOG_OFFLINE`), which races across the parallel test
    ...
    fn fetch_with(&self, offline: bool, base: &str) -> Changelog {
        ...
        // any on-disk seed under `$QIDI_HOME` even when offline mode was not
...
fn changelog_offline() -> bool {
    std::env::var_os("QIDI_CHANGELOG_OFFLINE").is_some_and(|v| !v.is_empty() && v != "0")
}
...
mod tests {
    ...
    /// `$QIDI_HOME` env so tests never race the parallel harness.
