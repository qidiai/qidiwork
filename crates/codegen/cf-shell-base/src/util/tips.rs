...
//! persisted to `~/.qidi/tip_cursor.json`.
...
/// Load the cursor from `~/.qidi/tip_cursor.json`. Returns 0 on any error.
...
/// Save the cursor to `~/.qidi/tip_cursor.json`. Silently ignores write errors.
...
/// cursor in `~/.qidi/tip_cursor.json`, so every session sees the next tip
