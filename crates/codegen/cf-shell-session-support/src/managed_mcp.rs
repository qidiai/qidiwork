...
async fn get_authenticated_json<T: serde::de::DeserializeOwned>(
    ...
    let resp = match cf_http::shared_client()
        .get(url)
...
///
/// Single source of truth lives in `qidi-code` (which matches policy
/// `serverName`s against it); re-exported here so the two never drift.
pub use cf_workspace::permission::resolution::MANAGED_MCP_PREFIX;
...
pub fn to_managed_name(display_name: &str) -> String {
    ...
    cf_shell_base::util::truncate(&raw, MANAGED_MCP_NAME_MAX_CHARS).to_string()
}
