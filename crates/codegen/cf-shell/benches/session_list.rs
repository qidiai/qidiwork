...
//!
//! Run: `cargo bench -p qidi-code --bench session_list`
...
use cf_fast_worktree::{ListFilter, WorktreeDb, WorktreeKind, WorktreeRecord, WorktreeStatus};
use cf_shell::session::info::Info;
use cf_shell::session::persistence::Summary;
use cf_shell::session::storage::{JsonlStorageAdapter, StorageAdapter};
use cf_shell::session::unified_list::{ListReq, build_unified_list};
...
impl Fixture {
    fn new(home: TempDir) -> Self {
        ...
        for workspace_index in 0..WORKSPACE_COUNT {
            ...
            let encoded = cf_shell::util::grok_home::encode_cwd_dirname(&cwd);
    ...
    fn assert_correct(&self, runtime: &tokio::runtime::Runtime) {
        let discovered = cf_shell::session::worktree::candidate_worktree_cwds_for_same_repo(
...
fn create_same_repo_cwds(home: &Path) -> SameRepoTopology {
    ...
    let worktree_base = cf_shell::session::worktree::worktree_base_dir(&main);
    ...
    for (index, (path, label)) in linked.iter().enumerate().take(DB_OVERLAP_WORKTREE_COUNT) {
        db.register(&WorktreeRecord {
            ...
            metadata: Some(cf_shell::session::worktree::build_label_metadata(
                label, false,
            )),
        })
...
fn bench_session_list(c: &mut Criterion) {
    ...
    unsafe {
        std::env::set_var("QIDI_HOME", home.path());
    }
    assert_eq!(cf_shell::util::grok_home::grok_home(), home.path());
