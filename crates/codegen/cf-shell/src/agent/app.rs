    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = cf_agent::prompt::workspace_user::optional_workspace_user_dir();
...
/// ([`cf_fsnotify::shared`]) run their event loops on a runtime that outlives
...
fn register_fs_watch_runtime() {
    cf_fsnotify::set_runtime_handle(tokio::runtime::Handle::current());
}
...
pub async fn run_stdio_agent(
    ...
    cf_telemetry::unified_log::set_version(cf_version::VERSION);
    ...
    cf_file_utils::queue::cleanup_orphaned_uploads(
        &grok_home::grok_home(),
        cf_file_utils::queue::DEFAULT_MAX_AGE,
    );
    ...
    if let Ok(version) = std::env::var("QIDI_CLIENT_VERSION") {
    ...
    // blocking stdin reads (see `cf_acp_lib::spawn_stdin_line_reader`): on
...
async fn run_headless_inner(
    ...
    let on_first_connect: Box<dyn FnOnce() + Send + 'static> = Box::new(move || {
        if !did_browser_flow && !no_browser {
            ...
            eprintln!();
            eprintln!(
                "Open QIDI Code: {} (press Enter to open in browser)",
                grok_code_url
            );
            eprintln!();
...
pub async fn run_leader(
    ...
    local_set
        .run_until(async move {
            ...
            // Gated on user_grok_home() so a cwd-relative .qidi/auth.json is never
            // read as the user auth store when no home resolves.
            let initial_auth_key_hash = cf_config::user_grok_home()
                ...
                while let Some(update) = config_update_rx.recv().await {
                    match update {
                        ...
                        ConfigUpdate::ModelsCacheChanged => {
                            // External write to ~/.qidi/models_cache.json
