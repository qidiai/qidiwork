...
pub const LEGACY_XAI_API_KEY_ENV_VAR: &str = "QIDI_API_KEY";

/// Read the API key from the environment.
///
/// Checks `XAI_API_KEY` first, then falls back to `QIDI_API_KEY` for
/// backward compatibility with older cf-tools deployments.
...
/// Returns `true` if either `XAI_API_KEY` or `QIDI_API_KEY` is set.
...
/// `QIDI_DISABLE_API_KEY_AUTH`) is the admin kill switch: when true the
...
fn build_pinned_api_key(has_external_api_key: bool) -> BuiltAuthMethods {
    ...
    BuiltAuthMethods {
        ...
        methods: vec![cf_api_key_auth_method()],
...
fn build_unpinned(
    ...
    if has_external_api_key {
        ...
        methods.push(cf_api_key_auth_method());
...
impl AuthMethodKind {
    ...
    pub fn from_id(id: &acp::AuthMethodId) -> Self {
        ...
        match id.0.as_ref() {
            ...
            QIDI_COM_METHOD_ID => Self::GrokCom,
...
pub fn cf_api_key_auth_method() -> acp::AuthMethod {
...
pub fn cached_token_auth_method() -> acp::AuthMethod {
    ...
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.qidi/auth.json".to_string())),
...
pub const QIDI_COM_METHOD_ID: &str = "grok.com";
...
mod tests {
    ...
    /// `~/.qidi/config.toml` skeleton from the bug report, walks it through
    ...
    /// Legacy `QIDI_API_KEY` env var is accepted as a fallback
    ...
    /// When both `XAI_API_KEY` and `QIDI_API_KEY` are set,
    ...
    // When this token is present via the `QIDI_AUTH` env var (or via legacy
    ...
    // to-end. A regression in QIDI_AUTH JSON parsing or in auth method
    ...
    /// expires_at) present in the `QIDI_AUTH` env var, with no other auth
    ...
    fn grok_login_legacy_token_does_not_require_login() {
        ...
        let _g1 = EnvGuard::unset("QIDI_AUTH_PATH");
        ...
        // Provide it via QIDI_AUTH env var (highest priority code path in
        ...
        let _g = EnvGuard::set("QIDI_AUTH", &legacy_json);
        ...
        assert!(
            ...
            "legacy token in QIDI_AUTH env MUST be loaded directly -- if this fails, 
    ...
    fn no_legacy_token_means_no_cached_token_advertised() {
        ...
        let _g1 = EnvGuard::unset("QIDI_AUTH");

        let _g2 = EnvGuard::unset("QIDI_AUTH_PATH");
    ...
    fn pin_oidc_with_session_hides_api_key() {
        ...
        assert_eq!(
            ...
            vec![CACHED_TOKEN_AUTH_METHOD_ID, QIDI_COM_METHOD_ID]
    ...
    fn pin_oidc_without_session_is_interactive_only() {
        ...
        assert_eq!(method_ids(&built), vec![QIDI_COM_METHOD_ID]);
