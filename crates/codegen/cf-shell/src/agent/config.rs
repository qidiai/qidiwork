...
use cf_sampler::{AuthScheme, SamplerConfig};
...
pub struct EndpointsConfig {
    ...
    /// Env: `QIDI_MODELS_BASE_URL`. Enables custom endpoint mode.
    /// List URL defaults to `{models_base_url}/models`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub models_base_url: Option<String>,
    /// Env: `QIDI_MODELS_LIST_URL`. Overrides the default `{base}/models` list URL.
    #[serde(alias = "models_endpoint", skip_serializing_if = "Option::is_none")]
    pub models_list_url: Option<String>,
    /// Env: `QIDI_FEEDBACK_BASE_URL`. Where feedback submissions go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub feedback_base_url: Option<String>,
    /// Env: `QIDI_TRACE_UPLOAD_URL`. Where trace uploads go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_url: Option<String>,
    /// Env: `QIDI_TRACE_UPLOAD_BUCKET`. Direct bucket (`gs://` or `s3://`), bypasses proxy.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_bucket: Option<String>,
    /// Env: `QIDI_TRACE_UPLOAD_REGION`. AWS region (S3 only).
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_region: Option<String>,
    /// Env: `QIDI_TRACE_UPLOAD_CREDENTIALS_FILE`. Path to GCS SA key or AWS credentials file.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Env: `QIDI_TRACE_UPLOAD_ENDPOINT_URL`. Custom S3-compatible endpoint.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_endpoint_url: Option<String>,
    /// Env: `QIDI_DEPLOYMENT_KEY`. Management API key for enterprise deployments.
    /// Sent on telemetry and service requests for deployment-level attribution.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub deployment_key: Option<String>,
    /// Env: `QIDI_MANAGED_CONFIG_URL`. Override the managed config endpoint.
    ...
    /// favor of `QIDI_INTERNAL_OTLP_TRACES_ENDPOINT`, and ignored by the internal
    /// pipeline when `QIDI_EXTERNAL_OTEL` is set (the standard `OTEL_*` vars then
    ...
    /// Env: `QIDI_INTERNAL_OTLP_TRACES_ENDPOINT`. Full INTERNAL traces endpoint,
    ...
    /// Env: `QIDI_INTERNAL_OTLP_HEADERS`. `k=v,k2=v2` extra headers for the
    ...
    /// (requirement pin > `QIDI_EXTERNAL_OTEL` env > `[telemetry].otel_enabled`
...
pub(crate) fn default_asset_server_url() -> String {
    std::env::var("QIDI_ASSET_SERVER_URL").unwrap_or_else(|_| ASSET_SERVER_URL_DEFAULT.to_owned())
}
...
/// format, shared with `QIDI_INTERNAL_OTLP_HEADERS`): split on `,`,
...
impl EndpointsConfig {
    ...
    /// the public default. NEVER falls back to `cf_api_base_url` — that is the
    ...
    /// cli-chat-proxy, never `cf_api_base_url`.
    ...
    /// else `proxy_url` + `/deployment/config`. Never `cf_api_base_url`, so the
    ...
    /// 3. `proxy_url` + `/traces`.
    /// Uses the proxy default (not the `cf_api_base_url` fallback) so
            ...
            && let Some(legacy) = self.legacy_internal_otlp_traces_endpoint()
        {
            tracing::warn!(
                "Repointing the internal trace pipeline via OTEL_EXPORTER_OTLP_ENDPOINT / \
                 ...
                 QIDI_INTERNAL_OTLP_TRACES_ENDPOINT instead — the standard OTEL_* vars will \
    ...
    /// still honored by the internal pipeline even with `QIDI_EXTERNAL_OTEL`
...
impl Default for EndpointsConfig {
    fn default() -> Self {
        Self {
            cli_chat_proxy_base_url: std::env::var("QIDI_CLI_CHAT_PROXY_BASE_URL").ok(),
            cf_api_base_url: std::env::var("QIDI_XAI_API_BASE_URL")
                .unwrap_or_else(|_| XAI_API_BASE_URL_DEFAULT.to_owned()),
            alpha_test_key: None,
            models_base_url: env_string("QIDI_MODELS_BASE_URL"),
            models_list_url: env_string("QIDI_MODELS_LIST_URL"),
            feedback_base_url: env_string("QIDI_FEEDBACK_BASE_URL"),
            trace_upload_url: env_string("QIDI_TRACE_UPLOAD_URL"),
            trace_upload_bucket: env_string("QIDI_TRACE_UPLOAD_BUCKET"),
            trace_upload_region: env_string("QIDI_TRACE_UPLOAD_REGION"),
            trace_upload_credentials_file: env_string("QIDI_TRACE_UPLOAD_CREDENTIALS_FILE"),
            trace_upload_credentials: None,
            trace_upload_endpoint_url: env_string("QIDI_TRACE_UPLOAD_ENDPOINT_URL"),
            deployment_key: env_string("QIDI_DEPLOYMENT_KEY"),
            managed_config_url: env_string("QIDI_MANAGED_CONFIG_URL"),
            ...
            grok_internal_otlp_traces_endpoint: env_string("QIDI_INTERNAL_OTLP_TRACES_ENDPOINT"),
            grok_internal_otlp_headers: env_string("QIDI_INTERNAL_OTLP_HEADERS"),
...
pub use cf_config::xai_grok_config_types::{BoolFlag, ConfigSource, LazinessDetectorPerModelConfig, Resolved};
...
fn resolve_compaction_mode_from(
    ...
    config: Option<&str>,
    remote: Option<&str>,
) -> cf_chat_state::CompactionMode {
    use cf_chat_state::CompactionMode;
...
fn resolve_compaction_detail_from(
    ...
    config: Option<&str>,
    remote: Option<&str>,
) -> cf_chat_state::CompactionDetail {
    use cf_chat_state::CompactionDetail;
...
fn resolve_compat_cell(
    ...
    resolve_compat_cell_with_env(cf_config::env_bool(env), cfg, remote, default)
}
...
pub(crate) fn compat_config_cell(
    ...
    cell: cf_tools::types::compat::CompatCell,
...
pub(crate) use cf_telemetry::config::env_telemetry_mode;
pub use cf_telemetry::config::{TelemetryConfig, TelemetryMode};
...
impl PluginsConfig {
    ...
    /// Native `.qidi/config.toml` entries already present take precedence:
    ...
    pub fn merge_claude_enabled_plugins(&mut self, _cwd: Option<&std::path::Path>) {
        ...
        for path in &paths {
            let (claude_enabled, claude_disabled) =
                cf_agent::plugins::marketplace::load_enabled_disabled_plugins(path);
    ...
    pub fn to_discovery_config(&self) -> cf_agent::plugins::discovery::DiscoveryConfig {
        cf_agent::plugins::discovery::DiscoveryConfig {
...
pub struct ModelsConfig {
    ...
    /// Unset = remote pin, then the client hint / built-in `cf-tools-0.1`
...
impl SandboxSettingsConfig {
    ...
    pub fn resolve_profile(
        ...
        resolve_string_flag(cli_arg, "QIDI_SANDBOX", self.profile.as_deref(), None)
    ...
    pub fn resolve_auto_allow_bash(&self, requirement: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("QIDI_SANDBOX_AUTO_ALLOW_BASH")
...
impl SuggestionsConfig {
    ...
    pub fn resolve_ai_enabled(
        ...
        BoolFlag::env("QIDI_SUGGESTIONS_AI")
    ...
    pub fn resolve_ai_model(&self) -> String {
        resolve_string_flag(
            None,
            "QIDI_SUGGESTIONS_AI_MODEL",
            self.ai_model.as_deref(),
            None,
        )
        .map(|r| r.value)
        .unwrap_or_else(|| "cf-tools".to_owned())
    }
}
...
///
/// These supplement the built-in scan locations (`.qidi/skills/`,
/// `.agents/skills/`, `~/.qidi/skills/`). They're written by `/import-claude`
...
pub struct Config {
    ...
    #[serde(default, skip_serializing)]
    pub announcements: Vec<cf_announcements::RemoteAnnouncement>,
    ...
    /// `[marketplace]` — also read by `cf_plugin_marketplace::load_sources()`.
    #[serde(default, skip_serializing)]
    ...
    /// Defaults to reading from QIDI_STORAGE_MODE env var.
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub cli_agents: Vec<cf_agent::config::AgentDefinition>,
    #[serde(skip)]
    pub cli_agent_overrides: CliAgentOverrides,
    /// Whether subagent (task tool) support is enabled. Enabled by default;
    /// disabled only via `QIDI_SUBAGENTS=0` or `[subagents] enabled = false`.
    /// Not remotely gated.
    #[serde(skip)]
    ...
    /// and `.qidi/roles/*.toml` file discovery.
    #[serde(skip)]
    pub subagent_roles:
        std::collections::HashMap<String, cf_subagent_resolution::config::SubagentRole>,
    #[serde(skip)]
    pub subagent_personas:
        std::collections::HashMap<String, cf_subagent_resolution::config::SubagentPersona>,
    ...
    /// Image describe model (`cf-tools` default via `ModelOverrideConfig::resolve`).
    #[serde(skip)]
...
pub struct CliAgentOverrides {
    ...
    pub permission_rules: Vec<cf_workspace::permission::types::PermissionRule>,
    pub max_turns: Option<u32>,
    pub permission_mode: Option<cf_agent::config::PermissionMode>,
}
impl CliAgentOverrides {
    ...
    pub fn apply_to_definition(&self, def: &mut cf_agent::config::AgentDefinition) {
    ...
    pub fn apply_to_subagent_definition(&self, def: &mut cf_agent::config::AgentDefinition) {
...
pub use cf_agent::config::AgentDefinition;
pub use cf_agent::config::Effort;
pub use cf_agent::config::PermissionMode;
pub use cf_shared::ui_config::{ContextualHints, UiConfig};
...
/// ```toml
/// [agent]
/// # Use a named agent (looked up via discovery: .qidi/agents/, ~/.qidi/agents/, built-ins)
...
/// 4. `QIDI_AGENT` env var
/// 5. Default `cf-tools` agent
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(default)]
pub struct AgentSelectionConfig {
    /// Name of a built-in or discovered agent definition.
    /// Looked up via `cf_agent::discovery::by_name_in_cwd()`.
    /// Examples: "cf-tools", "browser-use", or a custom agent name.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub name: Option<String>,
    /// Path to an agent definition file (.md with YAML frontmatter).
    /// When set, the agent is loaded from this file.
    /// Supports environment variable expansion (e.g., `$HOME/.qidi/agents/my-agent.md`).
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl Default for Config {
    fn default() -> Self {
        ...
        let mut cfg = Self {
            ...
            client_version: Some(cf_version::VERSION.to_string()),
...
impl Config {
    ...
    pub fn resolve_runtime_fields(&mut self, ctx: &RuntimeResolutionContext<'_>) {
        ...
        self.auto_wake_enabled = BoolFlag::env("QIDI_AUTO_WAKE")
    ...
    fn apply_env_overrides(&mut self) {
        ...
        if let Some(mode) = env_telemetry_mode("QIDI_TELEMETRY_ENABLED") {
    ...
    /// config.toml key, or `QIDI_TWO_PASS_COMPACTION` env.
    ...
    pub(crate) fn resolve_trace_upload(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("QIDI_TELEMETRY_TRACE_UPLOAD")
    ...
    pub(crate) fn trace_upload_decision_debug(&self) -> serde_json::Value {
        ...
        serde_json::json!(
            ...
            "in_env_trace_upload" : std::env::var("QIDI_TELEMETRY_TRACE_UPLOAD").ok(),
            "in_env_telemetry_enabled" : std::env::var("QIDI_TELEMETRY_ENABLED").ok(),
    ...
    pub(crate) fn resolve_feedback(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("QIDI_FEEDBACK_ENABLED")
    ...
    pub(crate) fn resolve_two_pass_compaction(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("QIDI_TWO_PASS_COMPACTION")
    ...
    /// `QIDI_DOOM_LOOP_RECOVERY` > TOML `enabled` > remote `enabled` >
    ...
    pub(crate) fn resolve_doom_loop_recovery(
        &self,
    ) -> Option<cf_sampling_types::DoomLoopRecoveryPolicy> {
        use cf_sampling_types::DoomLoopRecoveryPolicy as Policy;
        let remote = self
            .remote_settings
            .as_ref()
            .and_then(|s| s.doom_loop_recovery.as_ref());
        let enabled = BoolFlag::env("QIDI_DOOM_LOOP_RECOVERY")
    ...
    /// Precedence: env `QIDI_OFFICIAL_MARKETPLACE_AUTO_REGISTER` > remote settings >
    ...
    pub(crate) fn resolve_official_marketplace_auto_register(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("QIDI_OFFICIAL_MARKETPLACE_AUTO_REGISTER")
            .feature_flag(ff)
            .default(false)
            .resolve()
    }
    ...
    pub(crate) fn resolve_ask_user_question(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("QIDI_ASK_USER_QUESTION")
    ...
    pub(crate) fn resolve_image_edit(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("QIDI_IMAGE_EDIT").default(true).resolve()
    }
    ...
    /// Precedence: env `QIDI_IMAGE_GEN_MODEL_OVERRIDE` > `[features]
    ...
    pub(crate) fn resolve_goal_classifier_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("QIDI_GOAL_CLASSIFIER")
    ...
    pub(crate) fn resolve_goal_planner_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("QIDI_GOAL_PLANNER")
    ...
    pub(crate) fn resolve_goal_summary_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("QIDI_GOAL_SUMMARY")
    ...
    pub(crate) fn resolve_goal_use_current_model_only(&self) -> Resolved<bool> {
        BoolFlag::env("QIDI_GOAL_USE_CURRENT_MODEL_ONLY")
    ...
    pub(crate) fn resolve_backend_tools(&self) -> Resolved<bool> {
        BoolFlag::env("QIDI_BACKEND_SEARCH")
            .config(self.features.backend_tools)
            .default(true)
            .resolve()
    }
    /// Resolve the mode (env `QIDI_COMPACTION_MODE` > config > remote settings >
    /// default, unrecognized falling through) and, for `Segments`, attach the
    /// separately-resolved detail level.
    pub(crate) fn resolve_compaction_mode(&self) -> cf_chat_state::CompactionMode {
        resolve_compaction_mode_from(
            env_string("QIDI_COMPACTION_MODE").as_deref(),
    ...
    /// Resolve verbatim-input flag: env `QIDI_COMPACTION_VERBATIM_INPUT` > config > remote settings > default `true`.
    pub(crate) fn resolve_compaction_verbatim_input(&self) -> bool {
        BoolFlag::env("QIDI_COMPACTION_VERBATIM_INPUT")
    ...
    /// Precedence: env `QIDI_COMPACTION_DETAIL`, then config
    ...
    fn resolve_compaction_detail(&self) -> cf_chat_state::CompactionDetail {
        resolve_compaction_detail_from(
            env_string("QIDI_COMPACTION_DETAIL").as_deref(),
    ...
    pub fn resolve_cancel_rewind(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("QIDI_CANCEL_REWIND")
    ...
    ///
    /// Priority: `--oauth` > QIDI_OAUTH_ENABLED env > default (true = OAuth).
    pub fn resolve_grok_oauth(&self, cli_oidc: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("QIDI_OAUTH_ENABLED")
            .cli(cli_oidc)
            .default(true)
            .resolve()
    }
    ...
    /// narrow non-recursive watches for `<cwd>/` and `<cwd>/.qidi/`.
    ///
    /// Thin delegate to the canonical
...
///
/// `requirement > cli > env (QIDI_MCP_LIVENESS_WATCHERS) > config >
...
pub fn resolve_mcp_liveness_watchers(
    ...
    BoolFlag::env("QIDI_MCP_LIVENESS_WATCHERS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// `BoolFlag` precedence:
///
/// `requirement > cli > env (QIDI_MCP_AUTO_RESTART) > config >
/// managed > feature_flag > default (true)`.
///
...
/// precedence is single-sourced.
///
/// Recovery is on by default; opt out via `QIDI_MCP_AUTO_RESTART=false`,
...
pub fn resolve_mcp_auto_restart(
    ...
    BoolFlag::env("QIDI_MCP_AUTO_RESTART")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
///
/// `requirement > cli > env (QIDI_MCP_PUSH_SERVER_STATUS) > config >
/// managed > feature_flag > default (true)`.
///
...
pub fn resolve_mcp_push_server_status(
    ...
    BoolFlag::env("QIDI_MCP_PUSH_SERVER_STATUS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// [`resolve_mcp_liveness_watchers`]:
///
/// `requirement > cli > env (QIDI_MCP_RECURSIVE_CONFIG_WATCH) >
...
/// operators flip this flag (e.g. via `QIDI_MCP_RECURSIVE_CONFIG_
...
pub fn resolve_mcp_recursive_config_watch(
    ...
    BoolFlag::env("QIDI_MCP_RECURSIVE_CONFIG_WATCH")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
impl SyncBoolFlag {
    ...
    /// Either-direction env resolver (typically `QIDI_*`). Returns
...
pub fn is_error_reporting_disabled_sync() -> bool {
    !SyncBoolFlag::new(error_reporting_enabled_from_toml)
        ...
        .enable_env(|| env_bool("QIDI_ERROR_REPORTING"))
...
/// `QIDI_TELEMETRY_ENABLED` resolved through `TelemetryMode::parse` so the
/// extended string forms (e.g. `"session_metrics"`) are accepted.
fn grok_telemetry_env_enabled() -> Option<bool> {
    env_telemetry_mode("QIDI_TELEMETRY_ENABLED").map(|m| !m.is_disabled())
}
/// Load `~/.qidi/requirements.toml` standalone so the admin pin can beat
...
/// stream's activation does: **requirement pin > `QIDI_EXTERNAL_OTEL` env >
...
pub(crate) fn external_otel_master_switch_resolved() -> bool {
    external_otel_master_switch_from(
        cf_config::load_merged_requirements().as_ref(),
        env_bool("QIDI_EXTERNAL_OTEL"),
...
pub fn resolve_external_otel_config(
    client: cf_telemetry::external::config::ExternalClientInfo,
) -> Option<cf_telemetry::external::ExternalOtelConfig> {
...
pub(crate) fn resolve_external_otel_config_with(
    ...
    let file_cfg: Option<cf_telemetry::external::ExternalOtelFileConfig> = effective_config
        .and_then(|cfg| cfg.get("telemetry"))
        .map(|t| cf_telemetry::external::ExternalOtelFileConfig {
    ...
    let getenv_pinned = |name: &str| -> Option<String> {
        let pin = match name {
            cf_telemetry::external::config::ENV_MASTER_SWITCH => req_enabled,
    ...
    let mut resolved = cf_telemetry::external::ExternalOtelConfig::resolve_with(
        getenv_pinned,
        file_cfg.as_ref(),
    )?;
...
pub fn apply_external_otel_remote_policy(settings: Option<&crate::util::config::RemoteSettings>) {
    ...
    let policy = cf_telemetry::external::ExternalOtelRemotePolicy {
    ...
    if policy.force_disable || policy.lock_content_gates {
        cf_telemetry::external::apply_remote_policy(policy);
    }
}
...
fn managed_settings_env_flag(key: &str) -> Option<bool> {
    let path = cf_config::claude_managed_settings_path()?;
    let content = std::fs::read_to_string(&path).ok()?;
    let json: serde_json::Value = serde_json::from_str(&content).ok()?;
    cf_workspace::permission::resolution::json_env_flag(json.get("env"), key)
}
...
struct DefaultModelJson {
    ...
    /// Optional per-model base URL override (e.g. OpenAI/Anthropic endpoints).
    /// When present, overrides the default xAI endpoint routing.
    #[serde(default)]
    base_url: Option<String>,
    /// Optional per-model API key (BYOK).
    #[serde(default)]
    api_key: Option<String>,
    /// Optional per-model env var name(s) for the API key.
    #[serde(default)]
    env_key: Option<EnvKeys>,
    /// Optional per-model auth scheme override.
    #[serde(default)]
    auth_scheme: Option<AuthScheme>,
    /// Optional per-model extra headers (e.g. anthropic-version).
    #[serde(default)]
...
fn default_models(endpoints: &EndpointsConfig) -> IndexMap<String, ModelEntryConfig> {
    ...
    entries
        ...
        .map(|m| {
            ...
            let default_base_url = endpoints.resolve_inference_base_url();
            let base_url = m.base_url.clone().unwrap_or_else(|| default_base_url.clone());
            // Only set api_base_url to the xAI endpoint when the model actually
            // routes to xAI. For third-party providers (OpenAI/Anthropic), leave
            // it as None so base_url is used for all auth methods.
            let api_base_url = if m.base_url.is_some() {
                None
            } else {
                Some(endpoints.cf_api_base_url.clone())
            };
...
pub struct ModelEntryConfig {
    ...
    /// Can also be set via the `QIDI_MAX_RETRIES` environment variable.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub max_retries: Option<u32>,
...
pub struct ModelInfo {
    ...
    /// Always has a value; defaults to `"cf-tools-plan"` when the server
...
pub struct AutoModeConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prompt_type: Option<cf_workspace::permission::ClassifierPromptType>,
...
pub struct Features {
    ...
    /// env (`QIDI_COMPACTION_MODE`). Parsed via `CompactionMode::parse`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub compaction_mode: Option<String>,
    /// `none` | `minimal` | `balanced` | `verbose` (default). `None` = defer to
    /// env (`QIDI_COMPACTION_DETAIL`). The `segments` verbatim detail level.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// `QIDI_MCP_AUTO_RESTART` to opt out).
    ///
...
pub struct ResolvedCredentials {
    pub api_key: Option<String>,
    pub base_url: String,
    pub auth_type: cf_chat_state::AuthType,
    pub auth_scheme: AuthScheme,
}
...
pub fn resolve_credentials(model: &ModelEntry, session_key: Option<&str>) -> ResolvedCredentials {
    ...
    let (api_key, base_url, auth_type) = if let Some(key) = model.own_credential() {
        (
            Some(key),
            info.base_url.clone(),
            cf_chat_state::AuthType::ApiKey,
        )
    } else if let Some(key) = session_key {
        (
            Some(key.to_owned()),
            info.base_url.clone(),
            cf_chat_state::AuthType::SessionToken,
        )
        ...
        (Some(key), url, cf_chat_state::AuthType::ApiKey)
    } else {
...
pub fn enforce_disable_api_key_auth(
        ...
        && crate::util::is_first_party_qidi_url(&creds.base_url)
    {
        ...
        cf_telemetry::unified_log::debug(
...
pub use cf_telemetry::config::deployment_id_from_key;
...
fn byok_from_lookup(lookup: &ModelLookup<'_>) -> ModelByok {
    match lookup {
...
fn with_resolved_model<T>(model_id: &str, f: impl FnOnce(ModelLookup<'_>) -> T) -> T {
...
/// on BYOK / non-proxy routes for internal slugs like `cf-tools`).
...
pub fn resolve_chat_state_auth_type(
    model_id: &str,
    session_key: Option<&str>,
    fallback: cf_chat_state::AuthType,
) -> cf_chat_state::AuthType {
...
mod tests {
    ...
    fn auto_mode_config_parses_from_toml_and_json_equivalently() {
        use cf_workspace::permission::ClassifierPromptType;
        let toml_src = r#"
enabled = true
...
"#;
    ...
    fn finalize_image_describe_sampler_some_stamps_session_fields() {
        ...
        assert_eq!(model, "cf-tools");
        assert_eq!(cfg.model, "cf-tools");
    ...
    fn sampling_config_uses_fallback_when_no_model_api_key() {
        ...
        let sampling_config = sampling_config_for_model(
            ...
            ResolvedCredentials {
                ...
                auth_type: cf_chat_state::AuthType::ApiKey,
    ...
    fn env_keys_resolve_first_set_wins() {
        let keys = EnvKeys::new(["QIDI_TEST_ENV_KEY_PRIMARY", "QIDI_TEST_ENV_KEY_FALLBACK"]);
        assert_eq!(keys.resolve_value_with(|_| None), None, "none set");
        assert_eq!(
            keys.resolve_value_with(
                |n| (n == "QIDI_TEST_ENV_KEY_FALLBACK").then(|| "from-fallback".into())
            ),
            Some("from-fallback".into())
        );
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "QIDI_TEST_ENV_KEY_PRIMARY" => Some("from-primary".into()),
                "QIDI_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
        ...
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "QIDI_TEST_ENV_KEY_PRIMARY" => Some(String::new()),
                "QIDI_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
            Some("from-fallback".into())
    ...
    fn env_keys_resolve_skips_whitespace_only_value() {
        let keys = EnvKeys::new(["QIDI_TEST_WS_PRIMARY", "QIDI_TEST_WS_FALLBACK"]);
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "QIDI_TEST_WS_PRIMARY" => Some("   ".into()),
                "QIDI_TEST_WS_FALLBACK" => Some("real".into()),
                _ => None,
            }),
            Some("real".into())
        );
        assert_eq!(
            EnvKeys::single("QIDI_TEST_WS_ONLY").resolve_value_with(|_| Some("   ".into())),
            None
        );
        assert_eq!(
            EnvKeys::single("QIDI_TEST_WS_PAD").resolve_value_with(|_| Some("  tok  ".into())),
            Some("  tok  ".into())
        );
    }
    #[test]
    ...
    fn first_own_credential_empty_api_key_falls_through_to_env_key() {
        use cf_test_support::EnvGuard;
        let var = "QIDI_TEST_FIRST_OWN_CRED_ENV";
    ...
    fn resolve_credentials_multi_env_key_uses_lc_alias() {
        ...
        let primary = "QIDI_TEST_MULTI_ENV_PRIMARY";
        let alias = "QIDI_TEST_MULTI_ENV_LC_ALIAS";
        unsafe {
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_session() {
        use cf_chat_state::AuthType;
        use cf_test_support::EnvGuard;
        let primary = "QIDI_TEST_EMPTY_ENV_PRIMARY";
        let alias = "QIDI_TEST_EMPTY_ENV_LC_ALIAS";
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_global_key() {
        ...
        let primary = "QIDI_TEST_EMPTY_ENV_GLOBAL_PRIMARY";
        let alias = "QIDI_TEST_EMPTY_ENV_GLOBAL_ALIAS";
        let _primary = EnvGuard::set(primary, "");
        let _alias = EnvGuard::set(alias, "");
    ...
    fn resolve_credentials_no_session_key_returns_api_key() {
        let model = test_model_entry("m", "https://example.com/v1", None, None, None);
        let creds = resolve_credentials(&model, None);
        assert_eq!(creds.auth_type, cf_chat_state::AuthType::ApiKey);
    }
    ...
    fn x_api_key_auth_scheme_flows_from_config_to_sampler() {
        ...
        assert_eq!(config.api_backend, ApiBackend::Messages);
        let client = cf_sampler::SamplingClient::new(config).expect("client should build");
    ...
    fn user_override_parses_compaction_at_tokens_from_toml() {
        use cf_sampling_types::CompactionAtTokens;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn user_override_parses_compactions_remaining_from_toml() {
        use cf_sampling_types::CompactionsRemaining;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn compaction_mode_precedence_env_over_config_over_remote_over_default() {
        ...
        assert_eq!(
            ...
            CompactionMode::Segments(cf_chat_state::CompactionDetail::default())
        );
        assert_eq!(
    ...
    /// qidi-code/src/agent/models.rs:857 for any BYOK Claude config.
    #[test]
    ...
    fn config_models_default_is_not_overwritten_by_default_models_json() {
        ...
        let resolved = resolve_string_flag(
            None,
            "QIDI_DEFAULT_MODEL_TEST_NONEXISTENT",
            config_default,
    ...
    fn e2e_enterprise_endpoints_plus_partial_model_override() {
        ...
        let (_, models) = resolve_models_from_toml(
            ...
            cf_api_base_url = "https://enterprise-api.acme.com/v1"
    ...
    fn e2e_enterprise_endpoints_only_no_model_override() {
        ...
        assert_eq!(
            model.api_base_url.as_deref(),
            Some("https://enterprise-api.acme.com/v1"),
            "default model should use enterprise cf_api_base_url"
        );
    }
    ...
    fn unset_endpoint_env_vars() {
        for k in [
            "QIDI_CLI_CHAT_PROXY_BASE_URL",
            "QIDI_XAI_API_BASE_URL",
            "QIDI_FEEDBACK_BASE_URL",
            "QIDI_TRACE_UPLOAD_URL",
            "QIDI_MANAGED_CONFIG_URL",
            "QIDI_MODELS_BASE_URL",
            "QIDI_MODELS_LIST_URL",
            "OTEL_EXPORTER_OTLP_ENDPOINT",
            "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT",
            "OTEL_EXPORTER_OTLP_HEADERS",
            "QIDI_INTERNAL_OTLP_TRACES_ENDPOINT",
            "QIDI_INTERNAL_OTLP_HEADERS",
            "QIDI_EXTERNAL_OTEL",
        ] {
    ...
    /// `cf_api_base_url` — overriding ONLY inference keeps every aux endpoint on
    ...
    fn aux_endpoints_resolve_to_proxy_never_inference() {
        ...
        let cfg = EndpointsConfig {
            cf_api_base_url: inference.to_string(),
        ...
        assert_eq!(cfg.cf_api_base_url, inference);
    ...
    /// REGRESSION: the managed-config URL never follows `cf_api_base_url`
    ...
    fn resolve_feedback_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("QIDI_FEEDBACK_ENABLED") };
        unsafe { std::env::remove_var("QIDI_TELEMETRY_ENABLED") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("QIDI_SESSION_RECAP") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_env_off_overrides_default() {
        unsafe { std::env::set_var("QIDI_SESSION_RECAP", "0") };
        let cfg = Config::default();
        let r = cfg.resolve_session_recap();
        assert!(!r.value);
    ...
    fn resolve_two_pass_compaction_precedence() {
        unsafe { std::env::remove_var("QIDI_TWO_PASS_COMPACTION") };
        ...
        assert!(r.value);
        assert_eq!(r.source, ConfigSource::Config);
        unsafe { std::env::set_var("QIDI_TWO_PASS_COMPACTION", "0") };
        let r = config_over_remote.resolve_two_pass_compaction();
    ...
    fn resolve_doom_loop_recovery_precedence() {
        ...
        unsafe { std::env::remove_var("QIDI_DOOM_LOOP_RECOVERY") };
        let default_cfg = Config::default();
        assert!(
        ...
        unsafe { std::env::set_var("QIDI_DOOM_LOOP_RECOVERY", "0") };
        assert!(
    ...
    fn resolve_feedback_env_overrides_all() {
        unsafe { std::env::set_var("QIDI_FEEDBACK_ENABLED", "true") };
        let mut cfg = Config::default();
    ...
    fn resolve_trace_upload_disabled_when_telemetry_off_despite_remote_flag() {
        unsafe { std::env::remove_var("QIDI_TELEMETRY_ENABLED") };
        unsafe { std::env::remove_var("QIDI_TELEMETRY_TRACE_UPLOAD") };
        let mut cfg = Config::default();
    ...
    fn resolve_goal_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("QIDI_GOAL") };
        let cfg = Config::default();
    ...
    fn resolve_goal_env_overrides_config() {
        unsafe { std::env::set_var("QIDI_GOAL", "1") };
        let mut cfg = Config::default();
    ...
    fn resolve_ask_user_question_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("QIDI_ASK_USER_QUESTION") };
        let cfg = Config::default();
    ...
    fn resolve_ask_user_question_env_overrides_remote_settings() {
        unsafe { std::env::set_var("QIDI_ASK_USER_QUESTION", "1") };
        let cfg = Config {
            remote_settings: Some(crate::util::config::RemoteSettings {
    ...
    fn resolve_image_gen_model_override_remote_settings_or_config() {
        unsafe { std::env::remove_var("QIDI_IMAGE_GEN_MODEL_OVERRIDE") };
    ...
    fn imagine_tools_disabled_gates_image_edit() {
        unsafe { std::env::remove_var("QIDI_IMAGE_EDIT") };
        ...
        unsafe { std::env::set_var("QIDI_IMAGE_EDIT", "1") };
    ...
    fn clear_goal_envs() {
        unsafe {
            std::env::remove_var("QIDI_GOAL");
            std::env::remove_var("QIDI_GOAL_CLASSIFIER");
            std::env::remove_var("QIDI_GOAL_PLANNER");
            std::env::remove_var("QIDI_GOAL_SUMMARY");
            std::env::remove_var("QIDI_GOAL_VERIFIER_N");
            std::env::remove_var("QIDI_GOAL_CLASSIFIER_MAX");
            std::env::remove_var("QIDI_GOAL_STRATEGIST_EVERY");
            std::env::remove_var("QIDI_GOAL_REVERIFY_AFTER");
        }
    }
    ...
    fn resolve_goal_classifier_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("QIDI_GOAL_CLASSIFIER", "0") };
        let r = cfg_with_goal_and_remote(true, remote_classifier(true))
            .resolve_goal_classifier_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("QIDI_GOAL_CLASSIFIER", "1") };
    ...
    fn resolve_goal_planner_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("QIDI_GOAL_PLANNER", "0") };
        let r =
            cfg_with_goal_and_remote(true, remote_planner(true)).resolve_goal_planner_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("QIDI_GOAL_PLANNER", "1") };
    ...
    fn resolve_goal_summary_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("QIDI_GOAL_SUMMARY", "0") };
        let r =
...
"#,
    ...
    const GOAL_USE_CURRENT_ENV: &str = "QIDI_GOAL_USE_CURRENT_MODEL_ONLY";
...
[goal]
enabled = true
planner_model = { model = "cf-tools", agent_type = "cf-tools-plan" }
...
"#;
        ...
        let cfg = Config::new_from_toml_cfg(&raw).unwrap();
        assert_eq!(cfg.goal.planner_model.as_ref().unwrap().model, "cf-tools");
        assert_eq!(
            cfg.goal.strategist_model.as_ref().unwrap().agent_type,
            "cursor"
        );
        assert_eq!(cfg.goal.skeptic_models.len(), 2);
        assert_eq!(cfg.goal.skeptic_models[0].model, "cf-tools");
        assert_eq!(
...
planner_model = { agent_type = "cf-tools-plan" }
"#;
        let raw: toml::Value = toml::from_str(toml_str).unwrap();
        let cfg = Config::new_from_toml_cfg(&raw)
...
"#,
    ...
    fn internal_otlp_endpoint_grok_internal_wins_regardless_of_switch() {
        for switch in [false, true] {
            ...
            assert_eq!(
                ...
                "switch={switch}: QIDI_INTERNAL_OTLP_TRACES_ENDPOINT must win verbatim (trailing / trimmed)"
            );
        }
    }
    ...
    fn ext_client() -> cf_telemetry::external::config::ExternalClientInfo {
        cf_telemetry::external::config::ExternalClientInfo::default()
    }
    #[test]
    ...
    fn external_otel_requirements_pin_wins_over_env() {
        ...
        assert!(
            resolve_external_otel_config_with(
                None,
                Some(&req),
                ext_env(&[("QIDI_EXTERNAL_OTEL", "1"), ("OTEL_LOGS_EXPORTER", "otlp"),]),
                ext_client(),
                false,
            )
            .is_none()
    ...
    /// (managed config / requirements — no `QIDI_EXTERNAL_OTEL` env var) must
    ...
    fn clear_runtime_env_vars() {
        unsafe {
            std::env::remove_var("QIDI_SUBAGENTS");
            std::env::remove_var("QIDI_RESPECT_GITIGNORE");
            std::env::remove_var("QIDI_WEB_SEARCH_MODEL");
            std::env::remove_var("QIDI_SESSION_SUMMARY_MODEL");
            std::env::remove_var("QIDI_CURSOR_SKILLS_ENABLED");
            std::env::remove_var("QIDI_CURSOR_RULES_ENABLED");
            std::env::remove_var("QIDI_CURSOR_AGENTS_ENABLED");
            std::env::remove_var("QIDI_CLAUDE_SKILLS_ENABLED");
            std::env::remove_var("QIDI_CLAUDE_RULES_ENABLED");
            std::env::remove_var("QIDI_CLAUDE_AGENTS_ENABLED");
        }
    }
    fn clear_managed_mcp_env_vars() {
        unsafe {
            std::env::remove_var("QIDI_MANAGED_MCPS_ENABLED");
            std::env::remove_var("QIDI_MANAGED_MCP_GATEWAY_TOOLS_ENABLED");
        }
    }
...
"#,
    ...
    fn resolve_raw_compat_sessions_load_failure_allows_env_override() {
        let _env = isolate_compat_env();
        let _codex = EnvGuard::set("QIDI_CODEX_SESSIONS_ENABLED", "true");
        let resolved = resolve_compat_sessions_from_raw(Err(()), None);
        assert!(!resolved.cursor.sessions);
        assert!(!resolved.claude.sessions);
        assert!(resolved.codex.sessions);
    ...
    fn resolve_compat_env_sessions_disable_independently() {
        ...
        for (vendor, env_var) in [
            (CompatVendor::Cursor, "QIDI_CURSOR_SESSIONS_ENABLED"),
            (CompatVendor::Claude, "QIDI_CLAUDE_SESSIONS_ENABLED"),
            (CompatVendor::Codex, "QIDI_CODEX_SESSIONS_ENABLED"),
        ] {
    ...
    fn resolve_compat_precedence_and_reserved_codex_hook() {
        ...
        let _session = EnvGuard::set("QIDI_CURSOR_SESSIONS_ENABLED", "true");
        let _hook = EnvGuard::set("QIDI_CODEX_HOOKS_ENABLED", "true");
        let resolved = resolve_compat_config(&config, Some(&remote));
        assert!(resolved.cursor.sessions);
        assert!(resolved.codex.hooks);
    }
    ...
    fn resolve_runtime_fields_compat_asymmetric_sources() {
        let _env = isolate_compat_env();
        let _cursor = EnvGuard::set("QIDI_CURSOR_SESSIONS_ENABLED", "false");
        let raw: toml::Value =
    ...
    fn resolve_runtime_fields_gitignore_from_env() {
        clear_runtime_env_vars();
        unsafe { std::env::set_var("QIDI_RESPECT_GITIGNORE", "0") };
        let raw = empty_config();
        let mut cfg = Config::new_from_toml_cfg(&raw).unwrap();
        cfg.resolve_runtime_fields(&RuntimeResolutionContext {
            raw_config: &raw,
...
"#,
    ...
    fn is_telemetry_explicitly_disabled_sync_env_signals() {
        unsafe { std::env::set_var("QIDI_TELEMETRY_ENABLED", "0") };
        unsafe { std::env::remove_var("DISABLE_TELEMETRY") };
        assert!(is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::set_var("QIDI_TELEMETRY_ENABLED", "1") };
        assert!(!is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::remove_var("QIDI_TELEMETRY_ENABLED") };
...
"#,
        ...
        cf_config::apply_version_overrides(&mut value, &v).unwrap();
        let cfg = Config::new_from_toml_cfg(&value).unwrap();
        assert_eq!(cfg.models.default.as_deref(), Some("grok-4.5"));
    }
    /// Reproduce the enterprise managed config bug: [model.cf-tools] sets
    ...
    fn slug_propagation_enterprise_managed_config_key_mismatch() {
        ...
        let by_key = resolved
            .get("cf-tools")
            .expect("cf-tools key must exist");
        ...
        assert_eq!(
            ...
            "BUG: prefetched 'grok-4.5' should inherit 500k from \
             sibling 'cf-tools' (same model slug), not stay at {default_cw}"
        );
    }
    ...
    /// Regression: enterprise managed config aliases cf-tools to their own
    /// endpoint with env_key. The bundled cf-tools has supported_in_api=false.
    ...
    fn mcp_liveness_watchers_default_is_true() {
        unsafe { std::env::remove_var("QIDI_MCP_LIVENESS_WATCHERS") };
    ...
    fn mcp_liveness_watchers_requirement_wins_over_everything() {
        unsafe { std::env::set_var("QIDI_MCP_LIVENESS_WATCHERS", "true") };
    ...
    fn mcp_liveness_watchers_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("QIDI_MCP_LIVENESS_WATCHERS", "false") };
    ...
    fn mcp_auto_restart_default_is_true() {
        unsafe { std::env::remove_var("QIDI_MCP_AUTO_RESTART") };
    ...
    fn mcp_auto_restart_requirement_wins_over_everything() {
        unsafe { std::env::set_var("QIDI_MCP_AUTO_RESTART", "false") };
    ...
    fn mcp_auto_restart_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("QIDI_MCP_AUTO_RESTART", "true") };
    ...
    fn mcp_push_server_status_default_is_true() {
        unsafe { std::env::remove_var("QIDI_MCP_PUSH_SERVER_STATUS") };
    ...
    fn mcp_push_server_status_requirement_wins_over_everything() {
        unsafe { std::env::set_var("QIDI_MCP_PUSH_SERVER_STATUS", "true") };
    ...
    fn mcp_push_server_status_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("QIDI_MCP_PUSH_SERVER_STATUS", "false") };
    ...
    fn mcp_recursive_config_watch_default_is_true() {
        unsafe { std::env::remove_var("QIDI_MCP_RECURSIVE_CONFIG_WATCH") };
    ...
    fn mcp_recursive_config_watch_requirement_wins_over_everything() {
        unsafe { std::env::set_var("QIDI_MCP_RECURSIVE_CONFIG_WATCH", "true") };
    ...
    fn mcp_recursive_config_watch_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("QIDI_MCP_RECURSIVE_CONFIG_WATCH", "false") };
