...
// ============================================================================
// Turn delta wire types (local to qidi-code until cli-chat-proxy catches up)
// ============================================================================
...
impl FeedbackClient {
    ...
    fn build_middleware_client(
        ...
        reqwest_middleware::ClientBuilder::new(http.clone())
            .with(cf_auth::AuthRetryMiddleware::new(provider, 0))
            .build()
    }

    fn make_auth_provider(
        credentials: &crate::util::grok_auth_credentials::GrokAuthCredentials,
    ) -> Arc<dyn cf_auth::AuthCredentialProvider> {
        if let Some(am) = credentials.auth_manager() {
            ...
            Arc::new(cf_auth::StaticAuthCredentialProvider::new(
    ...
    fn add_common_headers(&self, builder: RequestBuilder) -> RequestBuilder {
        let builder = builder
            .header(CLIENT_VERSION_HEADER, cf_version::VERSION)
            .header(
        ...
        if self.credentials.deployment_key.is_none() {
            builder.header("X-XAI-Token-Auth", "cli")
        } else {
            builder
        }
    }
