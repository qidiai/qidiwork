...
//! inside a cloned repository (`.mcp.json`, project `.qidi/config.toml`,
//! `~/.claude.json` `projects.<cwd>`, project `.qidi/lsp.json`). Those configs
...
//! interactive prompt, and the durable [`cf_workspace::trust::TrustStore`]
//! reads/writes — lives in `qidi-code` (client-side); this module keeps
...
//! on [`cf_workspace::folder_trust::decide`]; the consume-side nuance is
...
use cf_workspace::trust::{TrustStore, is_unsafe_trust_root, workspace_key};

// Decision-side (scan/decide/prompt/store) relocated to `qidi-code`
// (client crate). `grant_folder_trust` is the ONLY moved item referenced from
// OUTSIDE this module (shell call sites + the pager's
// `cf_shell::agent::folder_trust::grant_folder_trust`), so only it is
...
pub use cf_workspace::folder_trust::grant_folder_trust;
use cf_workspace::folder_trust::{
...
// NOTE: this folder-trust store (`~/.qidi/trusted_folders.toml`) is SEPARATE
// from the pre-existing per-plugin trust store
// (`cf_agent::plugins::TrustStore` at `~/.qidi/trusted-plugins`, plus the
...
/// [`cf_workspace::folder_trust::revoke_folder_trust_store`].
///
...
pub fn revoke_folder_trust(cwd: &Path) -> bool {
    ...
    let was_trusted = cf_workspace::folder_trust::revoke_folder_trust_store(cwd);
...
/// ([`cf_workspace::folder_trust::repo_config_kinds`] /
...
/// folder is gated (so an `.qidi/lsp.json`-only repo still has a non-empty reason
...
pub(crate) fn detected_config_kinds(cwd: &Path) -> Vec<String> {
    cf_workspace::folder_trust::repo_config_kinds(cwd)
        .into_iter()
        .map(str::to_string)
        .collect()
}
...
pub(crate) fn agent_inline_hooks_allowed(
    scope: cf_agent::config::AgentScope,
    trusted: impl FnOnce() -> bool,
) -> bool {
    scope != cf_agent::config::AgentScope::Project || trusted()
}
...
///
/// Sources: project `.qidi/config.toml [mcp_servers]` (NOT the user-tier global
...
/// `~/.qidi/config.toml` is dropped when untrusted. This is intended — untrusted
...
pub fn project_scoped_mcp_names(cwd: &Path) -> HashSet<String> {
    ...
    // `.qidi/config.toml [mcp_servers]` entries tagged project (the loader's key
...
pub fn filter_untrusted_project_lsp(
    ...
    sourced: std::collections::BTreeMap<
        String,
        (
            cf_tools::implementations::lsp::config::LspServerConfig,
            cf_tools::types::config_source::ConfigSource,
        ),
    >,
) -> std::collections::BTreeMap<String, cf_tools::implementations::lsp::config::LspServerConfig>
{
    cf_tools::implementations::lsp::config::filter_project_lsp_when_untrusted(
        sourced,
...
mod tests {
    ...
    use cf_workspace::folder_trust::repo_configs_present;
    ...
    fn simulate_release_build() -> EnvGuard {
        EnvGuard::set(cf_version::TEST_VERSION_ENV, "0.0.0-sim")
    }
    ...
    fn revoke_folder_trust_downgrades_cache() {
        ...
        // true; QIDI_HOME-isolated so the seed can't touch the real user file and
        // `#[serial]` because QIDI_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("QIDI_HOME", home.path());
        let tmp = repo_tmp();
    ...
    fn revoke_never_trusted_folder_writes_no_deny_and_preserves_cascade() {
        ...
        // mid-session untrust. QIDI_HOME-isolated so the grant writes to a temp
        // store; `#[serial]` because QIDI_HOME is global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("QIDI_HOME", home.path());
    ...
    fn revoke_ancestor_cascade_trusted_child_writes_explicit_untrust() {
        ...
        // the cache. QIDI_HOME-isolated so the grant writes to a temp store;
        // `#[serial]` because QIDI_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("QIDI_HOME", home.path());
    ...
    fn revoke_on_unrecordable_home_root_records_no_deny() {
        ...
        // home; QIDI_HOME-isolated store; QIDI_FOLDER_TRUST unset so the
        // default-on flag applies.
        let home = tempfile::tempdir().unwrap();
        let _home = EnvGuard::set("HOME", home.path());
        let grok_home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("QIDI_HOME", grok_home.path());
        let _flag = EnvGuard::unset("QIDI_FOLDER_TRUST");
    ...
    fn envrc_gate_drops_untrusted_then_loads_when_store_trusted() {
        ...
        // actually reads `.envrc`. QIDI_HOME-isolated so the trust store is empty;
        // QIDI_FOLDER_TRUST unset so the default-on feature flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("QIDI_HOME", home.path());
        let _flag = EnvGuard::unset("QIDI_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        let env = cf_workspace::envrc::load_envrc_or_empty(tmp.path());
        assert_eq!(
            env.get("GATED_ENVRC"),
            Some(&"1".to_string()),
    ...
    fn claude_env_gate_drops_project_env_when_untrusted() {
        ...
        // folder merges it. QIDI_HOME-isolated so the trust store is empty;
        // QIDI_FOLDER_TRUST unset so the default-on feature flag applies.
        use cf_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("QIDI_HOME", home.path());
        let _flag = EnvGuard::unset("QIDI_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn claude_env_gate_drops_subdir_project_env_when_untrusted() {
        ...
        // this). QIDI_HOME-isolated so the trust store is empty.
        use cf_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("QIDI_HOME", home.path());
        let _flag = EnvGuard::unset("QIDI_FOLDER_TRUST");
    ...
    fn project_agent_inline_hooks_gated_when_untrusted_but_user_kept() {
        ...
        // predicate used at mvp_agent/subagent. QIDI_HOME-isolated (empty store).
        use cf_agent::config::AgentScope;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("QIDI_HOME", home.path());
        let _flag = EnvGuard::unset("QIDI_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        .unwrap();
        let def = cf_agent::discovery::by_name_in_cwd("explore", tmp.path())
    ...
    fn project_scope_allowed_denies_untrusted_repo_with_configs() {
        ...
        // code-exec config (here `.qidi/hooks`), and no store grant must be
        // DENIED — even though no verdict was recorded first (the gate re-resolves
        // fail-closed rather than defaulting open). QIDI_HOME-isolated (empty
        // store); QIDI_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("QIDI_HOME", home.path());
        let _flag = EnvGuard::unset("QIDI_FOLDER_TRUST");
    ...
    fn project_scope_allowed_allows_repo_without_configs() {
        ...
        // would wrongly deny it. Release-stamped + QIDI_HOME-isolated so the
    ...
    fn project_scope_allowed_allows_store_trusted_repo() {
        ...
        // configs present. QIDI_HOME-isolated so the seeded store is the temp one;
        // QIDI_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("QIDI_HOME", home.path());
        let _flag = EnvGuard::unset("QIDI_FOLDER_TRUST");
    ...
    fn project_scope_allowed_allows_inert_local_build() {
        ...
        // QIDI_TEST_VERSION unset so `is_local_build()` is genuinely true.
        let _unset_ver = EnvGuard::unset(cf_version::TEST_VERSION_ENV);
        if option_env!("QIDI_VERSION").is_some() {
    ...
    fn project_scope_allowed_denies_untrusted_plugin_only_repo() {
        // A plugin-only untrusted repo (just `.qidi/plugins/<x>/`, no
        ...
        // discover_plugins/build_for_cwd/reload. QIDI_HOME-isolated (empty store);
        // QIDI_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("QIDI_HOME", home.path());
    ...
    fn kill_switch_allows_untrusted_repo_after_authoritative_resolve() {
        ...
        // QIDI_HOME-isolated (empty store); QIDI_FOLDER_TRUST unset so the kill-switch
        // is the only signal.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("QIDI_HOME", home.path());
    ...
    fn build_for_cwd_with_trust_verdict_gates_active_project_plugin() {
        ...
        // un-gating here.
        //
        // QIDI_HOME-isolated so both the folder-trust store and the plugin trust
        // store start empty (deterministic untrusted); QIDI_FOLDER_TRUST unset so
        // the default-on flag applies; `#[serial]` because both are process-global.
        use cf_agent::plugins::discovery::DiscoveryConfig;
        use cf_agent::plugins::{PluginRegistry, SharedPluginRegistryHandle};
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("QIDI_HOME", home.path());
        let _flag = EnvGuard::unset("QIDI_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn discover_hooks_gates_then_loads_project_hook_via_trust_verdict() {
        ...
        // QIDI_HOME-isolated so the grant writes to a temp store; QIDI_FOLDER_TRUST unset
        ...
        let has_project_hook = |reg: &cf_hooks::discovery::HookRegistry| {
            reg.all_hooks()
                .iter()
        ...
        let git_root = cf_workspace::session::git::find_git_root_from_path(tmp.path()).ok();
        let (reg, _errs) = crate::util::hooks::discover_hooks(
            git_root.as_deref(),
            &cf_tools::types::compat::CompatConfig::default(),
            untrusted,
        );
        assert!(
    ...
    fn filter_untrusted_project_lsp_drops_only_project() {
        ...
        use cf_tools::implementations::lsp::config::LspServerConfig;
            ...
            m.insert(
                ...
                (
                    ...
                    ConfigSource::Project {
                        path: PathBuf::from("/repo/.qidi/lsp.json"),
                    },
                ),
            );
            m.insert(
                ...
                (
                    LspServerConfig::default(),
                    ConfigSource::User {
                        path: PathBuf::from("/home/.qidi/lsp.json"),
                    },
                ),
            );
            m
    ...
    fn load_servers_sourced_tags_project_lsp_json() {
        use cf_tools::implementations::lsp::config::load_servers_with_plugins_sourced;
        use cf_tools::types::config_source::ConfigSource;

        // A `<cwd>/.qidi/lsp.json` server must be tagged `Project` so the gate
        // can distinguish it from user/plugin servers. Asserts on the specific
        // key, so any real `~/.qidi/lsp.json` on the test host is irrelevant.
        let tmp = repo_tmp();
    ...
    fn untrusted_workspace_drops_loaded_project_lsp() {
        ...
        // from `<cwd>/.qidi/lsp.json` is dropped once the workspace is untrusted.
        let tmp = repo_tmp();
        let grok = tmp.path().join(".grok");
        std::fs::create_dir_all(&grok).unwrap();
        std::fs::write(grok.join("lsp.json"), r#"{"projlsp": {"command": "true"}}"#).unwrap();
    ...
    /// (`.mcp.json`) and `projtoml` (`.qidi/config.toml [mcp_servers]`).
    ...
    /// `.qidi/config.toml`, `.mcp.json`, and `.cursor/mcp.json` must appear in
    ...
    fn explicit_child_untrust_survives_reload_despite_ancestor_trust() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let store_path = tmp.path().join(cf_workspace::trust::TRUST_FILE_NAME);
    ...
    fn no_configs_trust_is_provisional_and_regated_when_configs_appear() {
        ...
        unsafe { std::env::set_var("QIDI_FOLDER_TRUST", "1") };
        // Simulate a release-stamped build: an unstamped local/dev build (as in CI,
        // no QIDI_VERSION) auto-trusts, so the gate would never engage without this.
        unsafe { std::env::set_var(cf_version::TEST_VERSION_ENV, "0.0.0-sim") };
        let tmp = repo_tmp();
        ...
        unsafe { std::env::remove_var(cf_version::TEST_VERSION_ENV) };
        unsafe { std::env::remove_var("QIDI_FOLDER_TRUST") };
    }
    ...
    fn resolve_launch_dir_trust_matches_resolve_and_record() {
        ...
        // QIDI_HOME so the store is empty/seeded in temp. `#[serial]` because both
        // vars are process-global.
        let _feature = EnvGuard::set("QIDI_FOLDER_TRUST", "1");
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("QIDI_HOME", home.path());
    ...
    fn local_build_is_inert_launch_trust_auto_trusts() {
        ...
        // `is_local_build_honors_test_version_override`), with QIDI_TEST_VERSION
        // unset so `is_local_build()` is genuinely true. QIDI_HOME-isolated so the
        // real store is never touched.
        let _sim = EnvGuard::unset(cf_version::TEST_VERSION_ENV);
        if option_env!("QIDI_VERSION").is_some() {
            return; // a release-stamped test binary is not a local build
        }
        let home = tempfile::tempdir().unwrap();
    ...
    fn prompt_warranted_true_for_untrusted_repo_with_configs() {
        ...
        // => the GUI prompt is warranted. QIDI_HOME-isolated so the store starts
        // empty; `#[serial]` because QIDI_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("QIDI_HOME", home.path());
        let tmp = repo_tmp();
        ...
        // QIDI_FOLDER_TRUST unset: env outranks the remote flag, so an ambient
        // opt-out would otherwise false-fail the Prompt assertion.
        let _sim = EnvGuard::set(cf_version::TEST_VERSION_ENV, "0.0.0-sim");
        let _flag = EnvGuard::unset("QIDI_FOLDER_TRUST");
        let remote = RemoteSettings {
    ...
    fn prompt_warranted_false_when_feature_disabled() {
        ...
        // local-build path is not what's under test; QIDI_HOME-isolated and
        // QIDI_FOLDER_TRUST unset so the kill-switch is the only signal.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("QIDI_HOME", home.path());
        let _flag = EnvGuard::unset("QIDI_FOLDER_TRUST");
        let _sim = simulate_release_build();
    ...
    fn detected_config_kinds_reports_lsp_only_repo() {
        ...
        // `.qidi/lsp.json` must still produce a non-empty reason list.
        let tmp = repo_tmp();
