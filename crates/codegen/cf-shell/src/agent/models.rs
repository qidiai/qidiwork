...
use cf_sampling_types::{ReasoningEffort, ReasoningEffortOption};
...
struct Inner {
    ...
    gateway: RwLock<Option<cf_acp_lib::AcpAgentGatewaySender>>,
...
impl ModelsManager {
    ...
    pub(crate) fn set_gateway(&self, gateway: cf_acp_lib::AcpAgentGatewaySender) {
    ...
    pub fn model_compactions_remaining(
        &self,
        model_id: &str,
    ) -> Option<cf_sampling_types::CompactionsRemaining> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    pub fn model_compaction_at_tokens(
        &self,
        model_id: &str,
    ) -> Option<cf_sampling_types::CompactionAtTokens> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    /// differ from the slug for custom/enterprise ids (e.g. key `enterprise-cf-tools`
    ...
    /// Hot-reload the catalog from `~/.qidi/models_cache.json` after an
    ...
    /// When another grok process sharing `~/.qidi` (a `--no-leader` run, a
...
impl ModelsCacheManager {
    ...
    fn load_fresh(
        ...
        if cache.qidi_version.as_deref() != Some(cf_version::VERSION) {
    ...
    fn persist(
        ...
        let cache = ModelsCache {
            fetched_at: Utc::now(),
            qidi_version: Some(cf_version::VERSION.to_string()),
...
/// routing slug (e.g. "Auto" and "QIDI Code" both route to `cf-tools`)
    ...
    match fetch_models_blocking(endpoints, auth, fetch_auth) {
        Ok(FetchModelsResult { models, etag }) if !models.is_empty() => {
            let api_base_url_override = match fetch_auth {
                ModelFetchAuth::ApiKey => Some(endpoints.cf_api_base_url.clone()),
                _ => None,
            };
...
/// the catalog and `/model` picker use config keys (e.g. `enterprise-cf-tools`).
...
mod tests {
    ...
    fn reasoning_effort_override_skips_models_that_do_not_offer_level() {
        use indexmap::IndexMap;
        use cf_sampling_types::ReasoningEffortOption;
    ...
    fn resolve_falls_back_to_session_when_nothing_set() {
        ...
        let _unset_legacy = EnvGuard::unset("QIDI_CODE_XAI_API_KEY");
        let endpoints = config::EndpointsConfig::default();
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, false),
            ModelFetchAuth::Session,
    ...
    fn build_prefetched_map_distinct_ids_same_slug() {
        let entries = vec![
            make_entry_config_with_id(Some("auto"), "cf-tools", Some("Auto")),
            make_entry_config_with_id(Some("cf-tools"), "cf-tools", Some("QIDI Code")),
        ...
        assert!(map.contains_key("cf-tools"));
        assert!(map.contains_key("grok-composer-2.5-fast"));
        assert_eq!(
            map["auto"].info.model, "cf-tools",
            "auto entry should still route to cf-tools"
        );
        assert_eq!(map["cf-tools"].info.model, "cf-tools");
    }
    ...
    fn build_prefetched_map_duplicate_id_overwrites() {
        let entries = vec![
            make_entry_config_with_id(Some("cf-tools"), "cf-tools", Some("First")),
            make_entry_config_with_id(Some("cf-tools"), "cf-tools", Some("Second")),
        ];
        let map = build_prefetched_map(entries, None);

        assert_eq!(map.len(), 1, "duplicate id: second overwrites first");
        assert_eq!(map["cf-tools"].info.name.as_deref(), Some("Second"));
    }
    ...
    fn resolve_default_model_prefers_id_over_model_slug() {
        ...
        catalog.insert(
            "auto-cf-tools".to_string(),
            make_model_entry("cf-tools"),
        );
        catalog.insert("cf-tools".to_string(), make_model_entry("cf-tools"));

        let mut cfg = config::Config::default();
        cfg.models.default = Some("cf-tools".to_string());

        let (key, _, _) = resolve_default_model(&cfg, &catalog, true);
        assert_eq!(key, "cf-tools", "must match id, not first slug hit");
    }
    ...
    fn resolve_catalog_key_maps_routing_slug_to_config_key() {
        ...
        assert_eq!(key.0.as_ref(), "enterprise-cf-tools");
    }
    ...
    fn resolve_catalog_key_last_slug_match_wins() {
        let mut models = IndexMap::new();
        models.insert(
            "default-cf-tools".to_string(),
            make_model_entry("grok-4.5"),
        );
        models.insert("user-cf-tools".to_string(), make_model_entry("grok-4.5"));

        let persisted = acp::ModelId::new("grok-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("slug must resolve");
        assert_eq!(key.0.as_ref(), "user-cf-tools");
    }
    ...
    fn selectable_prefers_available_identity_over_non_selectable_exact_key() {
        let mut models = IndexMap::new();
        models.insert("cf-tools".to_string(), make_model_entry("cf-tools"));
        models.insert(
            "enterprise-cf-tools".to_string(),
            make_model_entry("cf-tools"),
        );
        ...
        let available = test_available_keys(&["enterprise-cf-tools", "grok-4.3"]);

        let persisted = acp::ModelId::new("cf-tools");
        assert_eq!(
    ...
    fn selectable_prefers_exact_key_over_later_slug_match() {
        let mut models = IndexMap::new();
        models.insert("cf-tools".to_string(), make_model_entry("grok-4.5"));
        models.insert("other".to_string(), make_model_entry("cf-tools"));

        let available = test_available_keys(&["cf-tools", "other"]);

        let persisted = acp::ModelId::new("cf-tools");
        let key = selectable_catalog_key_for_persisted(&models, &available, &persisted)
            .expect("exact selectable key must win");
        assert_eq!(key.0.as_ref(), "cf-tools");
    }
