...
impl MvpAgent {
    ...
    pub(super) fn ensure_telemetry_client(&self) {
        ...
        if !mode.is_disabled() {
            ...
            cf_telemetry::client::init_if_needed(
                cfg.telemetry.clone(),
                mode,
                user_id,
                team_id,
    ...
    pub(crate) fn plugin_registry_handle(
        &self,
    ) -> &cf_agent::plugins::SharedPluginRegistryHandle {
    ...
    fn ensure_local_workspace_ops(
        &self,
    ) -> Result<cf_workspace::WorkspaceOps, acp::Error> {
        ...
        let workspace_identity = self
            ...
            .map(|a| match a.team_id.filter(|t| !t.is_empty()) {
                Some(team) => {
                    cf_workspace::WorkspaceIdentity::team(a.user_id, team)
                }
                None => {
                    cf_workspace::WorkspaceIdentity::new(
                        a.user_id,
                        a.principal_type,
                        a.principal_id,
                    )
            ...
            .unwrap_or_default();
        let ops = match cf_workspace::handle::WorkspaceHandle::new_minimal(
            cwd.to_path_buf(),
            workspace_identity,
            project_lsp_trusted,
        ) {
            Ok(handle) => cf_workspace::WorkspaceOps::local(handle),
            Err(e) => {
    ...
    pub(crate) fn auth_type(&self) -> cf_chat_state::AuthType {
    ...
    pub(super) async fn authenticate_after_cached_token_unavailable(
        ...
        let meta = if method_id.0.as_ref() == auth_method::QIDI_COM_METHOD_ID {
    ...
    pub(super) fn apply_polled_announcements(
        ...
        pre_fetch: Option<Vec<cf_announcements::RemoteAnnouncement>>,
    ) {
        let mut cfg = self.cfg.borrow_mut();
    ...
    pub(crate) fn prepare_sampling_config_for_model(
        ...
        if matches!(preferred, Some(crate ::auth::PreferredAuthMethod::Oidc))
            ...
            && credentials.auth_type == cf_chat_state::AuthType::ApiKey
        {
            credentials.api_key = None;
            credentials.auth_type = cf_chat_state::AuthType::SessionToken;
        }
        ...
        if !has_session_key && credentials.auth_type == cf_chat_state::AuthType::ApiKey
    ...
    ///
    /// Both BYOK and session (OAuth) users go direct to `cf_api_base_url`.
    ...
    pub(super) fn prepare_image_gen_config(
        ...
        let cfg = self.cfg.borrow();
        let base_url = cfg.endpoints.cf_api_base_url.clone();
        let version = cfg
            .client_version
            .clone()
            .unwrap_or_else(|| cf_version::VERSION.to_string());
        let alpha_test_key = cfg.endpoints.alpha_test_key.clone();
        let mut headers = indexmap::IndexMap::new();
        headers.insert("user-agent".to_string(), format!("qidi-code/{version}"));
    ...
    ///
    /// Enabled gate: `disable_web_search` kill-switch > `QIDI_WEB_FETCH` env >
    /// remote settings `web_fetch_enabled` > default (false).
    ///
    /// Params resolution (TOML > env > remote settings > default):
    /// - `proxy_endpoint`: `[toolset.web_fetch] proxy_endpoint` > `QIDI_WEB_FETCH_PROXY` > remote settings > None
    ...
    pub fn with_models(
        ...
        let instance = Self {
            ...
            plugin_registry_handle: cf_agent::plugins::SharedPluginRegistryHandle::new(
                None,
    ...
    pub async fn kill_background_task(
        ...
        session_id: &str,
        task_id: &str,
    ) -> Result<cf_tools::types::KillOutcome, String> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    pub async fn list_tasks(
        &self,
        session_id: &str,
    ) -> Option<Vec<cf_tools::types::TaskSnapshot>> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    /// 2. Config file/env enables it ([relay] enabled or QIDI_RELAY_SYNC_ENABLED)
    ...
    pub async fn list_hooks(
        &self,
        session_id: &acp::SessionId,
    ) -> Option<cf_hooks_plugins_types::HooksListResponse> {
    ...
    pub async fn execute_hooks_action(
        &self,
        session_id: &acp::SessionId,
        action: cf_hooks_plugins_types::HooksAction,
    ) -> Option<cf_hooks_plugins_types::ActionOutcome> {
        if matches!(action, cf_hooks_plugins_types::HooksAction::Untrust)
            && let Some(cwd) = self.get_session_cwd(session_id)
        {
            self.interactive_trust_prompted
                .borrow_mut()
                .remove(&cf_workspace::trust::workspace_key(&cwd));
        }
        let handle = self.get_session_handle(session_id)?;
    ...
    pub async fn execute_plugins_action(
        ...
        let is_reload = matches!(action, cf_hooks_plugins_types::PluginsAction::Reload);
        let handle = self.get_session_handle(session_id)?;
        let outcome = handle.execute_plugins_action(action).await;
        let succeeded = matches!(
            outcome.as_ref().map(| o | & o.status),
            Some(cf_hooks_plugins_types::OutcomeStatus::Success)
        );
    ...
    pub fn plugin_registry_snapshot(
        &self,
    ) -> Option<std::sync::Arc<cf_agent::plugins::PluginRegistry>> {
    ...
    pub(super) async fn upload_harness_trace_turns(
        ...
        model: &str,
        turns: Vec<Vec<cf_sampling_types::conversation::ConversationItem>>,
    ) {
    ...
    pub(super) async fn build_harness_trace_uploads(
        ...
        base: u64,
        turns: Vec<Vec<cf_sampling_types::conversation::ConversationItem>>,
    ) -> Vec<(PromptTraceContext, PromptMetadata, cf_chat_state::TurnCapture)> {
        ...
        for (offset, items) in turns.into_iter().enumerate() {
            ...
            let metadata = PromptMetadata {
                ...
                agent_type: None,
                shell_version: Some(cf_version::VERSION.to_string()),
                workspace_type: None,
                sandbox: local_sandbox_telemetry(),
            };
            let capture = cf_chat_state::TurnCapture {
                messages: items,
    ...
    pub(crate) async fn get_trace_context(
        ...
        let upload_method = match upload_method {
            ...
            None => {
                ...
                cf_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::TraceUploadSkipped {
    ...
    /// 5. `QIDI_AGENT` env var.
    /// 6. Built-in default agent.
    ///
    /// `QIDI_AGENT` and an explicit `[agent] name` bypass step 1.
    /// Strict-harness classification is structural — see
    /// [`cf_agent::config::is_strict_harness_agent_type`].
    ///
    ...
    pub fn resolve_agent_definition(
        ...
        if let Some(ref name) = agent_config.name {
            ...
            if let Some(def) = cf_agent::discovery::by_name_in_cwd(name, cwd) {
                return def;
            }
            tracing::warn!(
                agent_name = % name,
            ...
            && resolved.name != required
        {
            ...
            if let Some(def) = cf_agent::discovery::by_name_in_cwd(required, cwd) {
                return def;
            }
            tracing::warn!(
    ...
    pub(super) async fn spawn_and_register_session(
        ...
        let fs: Arc<dyn cf_workspace::file_system::AsyncFileSystem> = if use_acp_fs {
        ...
        let (hunk_tracker_handle, hunk_event_rx) = match hunk_plan.actor_mode {
            ...
            None => (cf_hunk_tracker::HunkTrackerHandle::noop(), None),
        };
        ...
        let loc_aggregate_rx = match hunk_event_rx {
            Some((hunk_event_rx, loc_cancel)) if loc_tracking_enabled => {
                ...
                let loc_writer = cf_hunk_tracker::JsonlHunkRecordWriter::new(loc_path);
                let loc_ctx = cf_hunk_tracker::LocSinkContext {
                    session_id: session_info.id.0.to_string(),
                    agent_id: agent_id(),
        ...
        let mut session_env = cf_workspace::permission::claude_settings::load_claude_env_with_project(
            cwd.as_path(),
            project_env_trusted,
        );
        let envrc = match preloaded_envrc {
            Some(env) => env,
            None => {
                cf_workspace::envrc::load_envrc_or_empty_when_trusted(
                    cwd.as_path(),
        ...
        let pinned_model: Option<(acp::ModelId, ModelEntry)> = match &agent_definition
            .model
        {
            cf_agent::config::ModelOverride::Override(id) => {
            ...
            cf_agent::config::ModelOverride::Inherit => None,
        };
        ...
        if lsp_tools_enabled && tool_ctx.lsp.is_none() {
            ...
            let (
                ...
                .unzip();
            let sourced = cf_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
            ...
            if servers.is_empty() {
                let user_path = cf_tools::util::grok_home::grok_home()
                    .join("lsp.json");
        ...
        let (mut handle, permission_events_rx, agent_system_prompt, session_thread) = {
            ...
            let credentials = cf_chat_state::Credentials {
            ...
            let attribution_callback: Option<
                cf_sampler::SharedAttributionCallback,
            > = Some(
            ...
            let agent_hook_registry_override = agent_definition
                ...
                .and_then(|hooks_config| {
                    ...
                    let (specs, errors) = cf_hooks::config::parse_hooks_from_value_with_dir(
                        &hooks_val,
                    ...
                    let git_root = cf_workspace::session::git::find_git_root_from_path(
                            cwd,
                        )
                        .ok();
            ...
            spawn_session_on_thread(
                    ...
                    cf_agent::prompt::context::PromptAudience::Primary,
                    None,
                    None,
                    disable_web_search,
        ...
        if let Some(mut loc_rx) = loc_aggregate_rx {
            ...
            tokio::spawn(async move {
                while let Some(agg) = loc_rx.recv().await {
                    match agg {
                        cf_hunk_tracker::LocAggregate::LinesChanged {
                            author_type,
                            lines_added,
                            lines_removed,
                            file_path,
                            ...
                            let is_agent = author_type
                                == cf_hunk_tracker::AuthorType::Agent;
                            signals
                                .record_loc_change(
                                    is_agent,
                                    lines_added,
                        ...
                        cf_hunk_tracker::LocAggregate::LinesReverted {
