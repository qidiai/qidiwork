...
impl MvpAgent {
    ...
    pub(crate) fn start_codebase_index_for_code_nav(
        ...
        cwd: &std::path::Path,
    ) -> Option<(std::sync::Arc<cf_codebase_graph::IndexManagerHandle>, bool)> {
    ...
    pub(super) fn code_nav_eligibility_inner(
        ...
        let git_root = cf_workspace::session::git::find_git_root_from_path(cwd).ok();
        match &setting {
    ...
    pub(super) fn indexed_roots_for(&self, cwd: &std::path::Path) -> Vec<String> {
        ...
        if let Ok(git_root) = cf_workspace::session::git::find_git_root_from_path(cwd)
    ...
    pub(super) fn get_or_create_codebase_index(
        &self,
        cwd: PathBuf,
    ) -> (std::sync::Arc<cf_codebase_graph::IndexManagerHandle>, bool) {
    ...
    pub fn get_codebase_index(
        &self,
        cwd: &std::path::Path,
    ) -> Option<std::sync::Arc<cf_codebase_graph::IndexManagerHandle>> {
