...
use cf_telemetry::id::{agent_id, agent_instance_id};
use cf_telemetry::session_ctx::log_event;
use cf_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use cf_workspace::permission::{ClientType, PermissionEvent};
...
use cf_workspace::session::git::GitDiscoveryResult;
use cf_hunk_tracker::HunkTrackerActor;
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a cf_sampling_types::TokenUsage>,
...
fn announcements_push_payload(
    stored: Option<&[cf_announcements::RemoteAnnouncement]>,
    last_emitted: &[cf_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<cf_announcements::RemoteAnnouncement>> {
    let current = cf_announcements::filter_expired_at(
...
/// Override with `QIDI_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn announcements_refresh_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("QIDI_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS")
...
pub struct MvpAgent {
    ...
    /// **non-recursive** watch on `<cwd>/` and `<cwd>/.qidi/`).
    ///
    ...
    session_index_claims: RefCell<
        HashMap<acp::SessionId, std::sync::Arc<cf_codebase_graph::IndexManagerHandle>>,
    >,
    ...
    subagent_roles: std::collections::HashMap<
        String,
        cf_subagent_resolution::config::SubagentRole,
    >,
    subagent_personas: std::collections::HashMap<
        String,
        cf_subagent_resolution::config::SubagentPersona,
    >,
    ...
    pub(crate) plugin_registry_handle: cf_agent::plugins::SharedPluginRegistryHandle,
    ...
    workspace_ops: RefCell<Option<cf_workspace::WorkspaceOps>>,
    ...
    last_emitted_announcements: RefCell<Vec<cf_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &cf_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<cf_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !cf_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = cf_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
/// unchanged — compatible stock switches (e.g. `cf-tools` →
/// `cf-tools-plan`) intentionally preserve the session's original ACP
/// `agentProfile`.
...
/// default wire format and toolset, so switching e.g. `cf-tools` →
/// `cf-tools-plan` doesn't require rebuilding the harness and would
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use cf_agent::config::is_strict_harness_agent_type;
    match (
...
fn resolve_hunk_tracking_mode(
    mode_str: Option<&str>,
) -> Option<cf_hunk_tracker::TrackingMode> {
    ...
    Some(
        serde_json::from_value(serde_json::Value::String(mode.to_ascii_lowercase()))
            .unwrap_or(cf_hunk_tracker::TrackingMode::AllDirty),
    )
}
...
struct HunkTrackingPlan {
    ...
    actor_mode: Option<cf_hunk_tracker::TrackingMode>,
}
...
impl MvpAgent {
    ...
    fn forward_raw_replay_line(
        ...
        completions: &mut Vec<
            tokio::sync::oneshot::Receiver<cf_acp_lib::AcpResult<()>>,
        >,
        mark_replay: bool,
    ...
    pub(super) fn replay_session_updates_from_offset_enqueue(
        ...
        target_client_id: Option<&serde_json::Value>,
        mark_replay: bool,
    ) -> Vec<tokio::sync::oneshot::Receiver<cf_acp_lib::AcpResult<()>>> {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        for task in &orphaned {
            let snapshot = cf_tools::types::TaskSnapshot {
    ...
    ///    against `~/.qidi/bundled/` and the manifest.
