...
mod hunk_tracking_mode {
    ...
    use cf_hunk_tracker::TrackingMode;
    #[test]
...
fn harness_pair(id: &str) -> Vec<cf_sampling_types::conversation::ConversationItem> {
    use cf_sampling_types::ToolCall;
...
fn resolve_agent_definition_defaults_to_qidi_build() {
    ...
    if let Some(v) = prev {
        unsafe { std::env::set_var("QIDI_AGENT", v) }
    }
}
/// When model_agent_type = Some("codex"), the codex agent is selected even
/// though the default chain would return cf-tools.
#[test]
#[serial_test::serial]
...
fn resolve_agent_definition_acp_profile_wins_when_model_agent_type_is_default() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = cf_agent::AgentDefinition::from_json(&serde_json::json!(
...
/// `cf-tools-plan`, models in the catalog that still declare
/// `agent_type = "cf-tools"` explicitly must NOT preempt an ACP
/// profile. Any value in the `cf-tools*` family is the stock harness
...
fn resolve_agent_definition_acp_profile_wins_for_explicit_qidi_build_family() {
    ...
    .expect("agent definition must parse");
    for family_variant in ["cf-tools", "cf-tools-plan", "cf-tools-concise"] {
        let def = MvpAgent::resolve_agent_definition(
            tmp.path(),
            None,
            &config::AgentSelectionConfig::default(),
        ...
        assert_eq!(
            def.name, "custom-devbox-profile",
            "ACP profile must win for cf-tools family variant `{family_variant}`"
        );
    }
    if let Some(v) = prev {
        unsafe { std::env::set_var("QIDI_AGENT", v) }
...
fn inherited_harness_template_skips_nonstrict_model() {
    use cf_agent::prompt::user_message::UserMessageTemplate;
    let tmp = tempfile::tempdir().unwrap();
    assert!(
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    assert_eq!(
        def.model,
        cf_agent::config::ModelOverride::Override("test-model-123".to_string()),
        "agent profile model override must be preserved through resolution"
    );
    match prev {
        Some(v) => unsafe { std::env::set_var("QIDI_AGENT", v) },
        None => unsafe { std::env::remove_var("QIDI_AGENT") },
    }
}
#[test]
...
/// flow: a zero-turn switch from `cf-tools` (a client profile name) to
/// `cf-tools-plan` (the default model agent_type) must be
...
fn harnesses_are_compatible_for_stock_family_pairs() {
    assert!(harnesses_are_compatible("cf-tools", "cf-tools-plan"));
    assert!(harnesses_are_compatible("cf-tools-plan", "cf-tools"));
    assert!(harnesses_are_compatible("cf-tools", "cf-tools"));
...
fn harnesses_are_compatible_rejects_strict_mismatches() {
    ...
    assert!(!harnesses_are_compatible("cf-tools-plan", "codex"));
}
#[test]
fn explicit_agent_type_wins_over_session_default() {
    assert_eq!(
        resolve_required_agent_type(Some("cursor"), "cf-tools-plan"),
        "cursor"
    );
}
#[test]
...
fn null_agent_type_returns_to_session_default_after_cursor_switch() {
    let session_default = "cf-tools-plan";
    let required_after_null = resolve_required_agent_type(None, session_default);
    assert_eq!(required_after_null, "cf-tools-plan");
...
fn agent_name_unchanged_without_harness_rebuild() {
    let unchanged = agent_name_after_model_switch(false, "cf-tools-plan", "remote-sidebar");
    assert_eq!(
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    use cf_tools::computer::local::{LocalFs, LocalTerminalBackend};
    ...
    let ctx = SessionContext {
        ...
        system_reminder_tag: cf_tools::reminders::DEFAULT_REMINDER_TAG,
    };
    let toolset = builder
...
fn make_test_handle(
    ...
    let hunk_tracker_handle = cf_hunk_tracker::HunkTrackerActor::spawn(
        ...
        hunk_event_tx,
        cf_hunk_tracker::TrackingMode::AllDirty,
        hunk_cancel,
    );
    crate::session::SessionHandle {
        cmd_tx,
        ...
        max_turns: None,
        hunk_tracker_handle,
        chat_state_handle: cf_chat_state::ChatStateHandle::noop(),
        ...
        permission_handle: cf_workspace::permission::PermissionHandle::allow_all(),
        attribution_callback: None,
        agent_name: "cf-tools".to_string(),
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use cf_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
async fn session_config_options_resolves_routing_slug_to_catalog_model() {
    use crate::agent::config::{EndpointsConfig, ModelEntry};
    use cf_sampling_types::ReasoningEffort;
    let agent = build_minimal_agent_for_tests();
...
fn drain_roster_changed(
    rx: &mut tokio::sync::mpsc::UnboundedReceiver<cf_acp_lib::AcpClientMessage>,
) -> Option<crate::agent::roster::RosterChanged> {
    let mut found = None;
    while let Ok(msg) = rx.try_recv() {
        if let cf_acp_lib::AcpClientMessage::ExtNotification(args) = msg {
            if found.is_none()
...
async fn auth_type_session_based_no_current_returns_session_token() {
    for method_id in [
        ...
        crate::agent::auth_method::QIDI_COM_METHOD_ID,
...
async fn auth_type_session_based_with_current_returns_session_token() {
    ...
    assert_eq!(agent.auth_type(), cf_chat_state::AuthType::SessionToken,);
}
...
async fn auth_type_no_method_id_no_current_returns_api_key() {
    ...
    assert!(agent.auth_manager.current().is_none());
    assert_eq!(agent.auth_type(), cf_chat_state::AuthType::ApiKey,);
}
...
async fn cached_token_fallthrough_respects_kill_switch() {
    use crate::agent::auth_method::{QIDI_COM_METHOD_ID, XAI_API_KEY_ENV_VAR};
    use cf_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("QIDI_DISABLE_API_KEY_AUTH");
    let _key = EnvGuard::set(XAI_API_KEY_ENV_VAR, "test-deployment-key");
...
async fn cached_token_fallthrough_falls_to_grok_com_without_credentials() {
    use crate::agent::auth_method::{
        QIDI_COM_METHOD_ID, LEGACY_XAI_API_KEY_ENV_VAR, XAI_API_KEY_ENV_VAR,
    };
    use cf_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("QIDI_DISABLE_API_KEY_AUTH");
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = cf_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
fn ext_method_rewind_uses_local_dispatch_without_bridge() {
    use acp::Agent as _;
    let _env = crate::env::EnvVarGuard::remove(crate::env::QIDI_DISABLE_CUSTOM_BRIDGE_ENV);
...
fn build_agent_with_gateway_rx() -> (
    MvpAgent,
    tokio::sync::mpsc::UnboundedReceiver<cf_acp_lib::AcpClientMessage>,
) {
    use crate::agent::config::Config as AgentConfig;
    use crate::auth::{AuthManager, GrokComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
...
async fn answer_folder_trust_request(
    gw_rx: &mut tokio::sync::mpsc::UnboundedReceiver<cf_acp_lib::AcpClientMessage>,
    outcome: &str,
) -> serde_json::Value {
    ...
    let cf_acp_lib::AcpClientMessage::ExtMethod(args) = msg else {
...
fn interactive_trust_prompt_grant_reloads_project_mcp() {
    use cf_test_support::EnvGuard;
    use cf_workspace::trust::{TrustStore, workspace_key};
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("QIDI_HOME", home.path());
    let _sim = EnvGuard::set(cf_version::TEST_VERSION_ENV, "0.0-sim");
...
fn interactive_trust_prompt_client_error_fails_closed() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        let msg = tokio::time::timeout(std::time::Duration::from_secs(2), gw_rx.recv())
            ...
            .expect("trust request must be sent")
            .expect("gateway channel open");
        assert!(matches!(msg, cf_acp_lib::AcpClientMessage::ExtMethod(_)));
        drop(msg);
        assert!(
            tokio::time::timeout(std::time::Duration::from_millis(300), cmd_rx.recv())
                .await
...
fn interactive_trust_prompt_dedups_same_workspace() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(first, Ok(Some(cf_acp_lib::AcpClientMessage::ExtMethod(_)))),
...
fn interactive_trust_prompt_reprompts_after_untrust() {
    use cf_test_support::EnvGuard;
    use cf_hooks_plugins_types::HooksAction;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("QIDI_HOME", home.path());
    let _sim = EnvGuard::set(cf_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("QIDI_FOLDER_TRUST");
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(
                ...
                Ok(Some(cf_acp_lib::AcpClientMessage::ExtMethod(_)))
            ),
...
fn ann(id: &str) -> cf_announcements::RemoteAnnouncement {
    cf_announcements::RemoteAnnouncement {
...
/// fields as needed).
fn settings_with(
    announcements: Option<Vec<cf_announcements::RemoteAnnouncement>>,
) -> crate::util::config::RemoteSettings {
    crate::util::config::RemoteSettings {
        announcements,
        ..Default::default()
...
fn announcements_push_gate_ignores_expired_only_addition() {
    let now = test_now();
    let expired = cf_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_emits_on_expiry_crossing() {
    let expiring = cf_announcements::RemoteAnnouncement {
...
async fn emit_announcements_gate_emits_updates_baseline_and_bumps_gen() {
    ...
    let recv_gen =
        |rx: &mut tokio::sync::mpsc::UnboundedReceiver<cf_acp_lib::AcpClientMessage>| {
            ...
            let cf_acp_lib::AcpClientMessage::ExtNotification(args) = msg else {
