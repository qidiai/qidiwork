...
pub(crate) async fn handle_subagent_request(
    ...
    {
        use cf_tool_types::SubagentIsolationMode;
        if effective_runtime.isolation == SubagentIsolationMode::None
            && definition.isolation
                == Some(cf_agent::config::IsolationMode::Worktree)
        {
    ...
    if let Some(ref source) = resume_source {
        ...
        if let Err(e) = cf_subagent_resolution::validate_resume_identity(
            &request.subagent_type,
    ...
    let worktree_path = if let Some(ref source) = resume_source {
        if effective_runtime.isolation != cf_tool_types::SubagentIsolationMode::None
    ...
    } else if effective_runtime.isolation != cf_tool_types::SubagentIsolationMode::None
    {
        ...
        let creation_mode: cf_fast_worktree::CreationMode = ctx.worktree_type.into();
        let btrfs_delegate = crate::session::worktree::btrfs_delegate_from_env();
        match tokio::task::spawn_blocking(move || {
                let mut builder = cf_fast_worktree::WorktreeBuilder::new(
                        &source_clone,
                        &dest,
                    )
                    .working_tree_mode(
                        cf_fast_worktree::WorkingTreeMode::PreserveWorkingTree,
                    )
                    .creation_mode(creation_mode)
                    .worktree_kind(cf_fast_worktree::WorktreeKind::Subagent)
    ...
    {} else if context_source != InitialContextSource::Resumed && !verbatim_mirror_fork {
        if let Some(ref pi) = effective_runtime.persona_instructions {
            let reminder = cf_sampling_types::conversation::ConversationItem::system_reminder(
    ...
    let child_cwd_abs = cf_paths::AbsPathBuf::new(child_cwd)
        .unwrap_or_else(|_| {
            cf_paths::AbsPathBuf::new(std::env::current_dir().unwrap_or_default())
    ...
    let parent_traceparent = cf_file_utils::trace_context::current_traceparent();
    let tracker_child_cwd = child_session_info.cwd.clone();
    let tracker_model_id = effective_model_id.0.to_string();
    let initial_child_tokens = cf_chat_state::estimate_conversation_tokens(
        &forked_conversation,
    );
    ...
    let attribution_callback: Option<cf_sampler::SharedAttributionCallback> = effective_sampling_config
    ...
    let yolo_policy_block = cf_workspace::permission::resolution::yolo_disabled_by_policy();
    ...
    if let Some(scope) = agent_memory_scope {
        ...
        if memory_md.is_file() && let Ok(content) = std::fs::read_to_string(&memory_md) {
            ...
            let truncated: String = content
                ...
                .collect::<Vec<_>>()
                .join("\n");
            let truncated = cf_tools::util::truncate::truncate_str(
                    &truncated,
                    MAX_BYTES,
                )
                .to_string();
        ...
        if is_plugin_agent {
            ...
            if !specs.is_empty() {
                let specs: Vec<_> = specs
                    .into_iter()
                    .map(|mut s| {
                        if s.event == cf_hooks::event::HookEventName::Stop {
                            s.event = cf_hooks::event::HookEventName::SubagentStop;
                        }
                        s
                    })
                    .collect();
    ...
    let agent_mcp_servers: Vec<_> = if is_plugin_agent {
        ...
        definition
                ...
                .filter_map(|entry| match entry {
                    cf_agent::config::McpServerRef::Named(name) => {
                    ...
                    cf_agent::config::McpServerRef::Inline { name, config } => {
    ...
    if inherit_skills && ctx.parent_skills.is_none() {
        ...
        ctx.parent_skills = Some(
            cf_agent::prompt::skills::list_skills_with_plugins(
                    Some(&parent_cwd_str),
    ...
    cf_telemetry::session_ctx::log_event(cf_telemetry::events::SubagentLaunched {
        subagent_id: request.id.clone(),
    ...
    let spawn_result = session::spawn_session_on_thread(
            ...
            cf_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
            cf_chat_state::CompactionMode::Summary,
            ...
            std::sync::Arc::new(
                parking_lot::Mutex::new(
                    cf_workspace::file_system::CodebaseIndexManager::new(),
                ),
            ),
            false,
            subagent_fs_watch,
            ...
            if inherit_skills {
                ...
                cf_agent::prompt::skills::SkillsConfig::default()
            },
            ...
            ctx.yolo_mode
                || matches!(
                    ...
                    cf_agent::config::PermissionMode::BypassPermissions
                ),
            false,
            None,
            ...
            cf_agent::prompt::context::PromptAudience::Subagent,
    ...
    let outcome = if result.success {
        cf_telemetry::events::Outcome::Completed
    } else if result.cancelled {
        cf_telemetry::events::Outcome::Cancelled
    } else {
        cf_telemetry::events::Outcome::Error
    };
    cf_telemetry::session_ctx::log_event(cf_telemetry::events::SubagentCompleted {
        subagent_id: request.id.clone(),
        parent_session_id: request.parent_session_id.clone(),
        outcome,
