...
pub(crate) struct SubagentTracker {
    ...
    pub color: Option<cf_agent::config::AgentColor>,
}
...
pub(crate) struct SubagentSpawnContext {
    ...
    pub lsp: Option<std::sync::Arc<dyn cf_tools::implementations::lsp::LspBackend>>,
    #[expect(
        dead_code,
        reason = "unused in production; remove expect when wired or delete the item"
    )]
    ...
    pub sampling_config: cf_sampler::SamplerConfig,
    ...
    pub parent_terminal_backend: Option<Arc<dyn cf_tools::computer::types::TerminalBackend>>,
    ...
    pub parent_notification_handle:
        Option<cf_tools::notification::types::ToolNotificationHandle>,
    ...
    pub parent_chat_state: Option<cf_chat_state::ChatStateHandle>,
    ...
    pub plugin_registry: Option<std::sync::Arc<cf_agent::plugins::PluginRegistry>>,
    ...
    pub file_tool_overrides: Option<Vec<cf_tools::registry::types::ToolConfig>>,
    ...
    pub hook_registry: Option<std::sync::Arc<cf_hooks::discovery::HookRegistry>>,
    #[expect(
        dead_code,
        reason = "unused in production; remove expect when wired or delete the item"
    )]
    pub hook_workspace_root: String,
    pub permission_handle: Option<cf_workspace::permission::PermissionHandle>,
    pub worktree_type: crate::util::config::WorktreeType,
    pub api_key_provider: Option<cf_tools::types::SharedApiKeyProvider>,
    pub image_description_model: String,
    /// Dual-mode workspace operations handle.
    pub workspace_ops: cf_workspace::WorkspaceOps,
    ...
    pub attribution_callback: Option<cf_sampler::SharedAttributionCallback>,
    /// Parent session's agent name (e.g. "cf-tools").
    ...
    pub parent_tool_snapshot: Option<Vec<cf_sampling_types::ToolSpec>>,
    /// Pre-discovered skills from the parent session, captured at spawn time.
    pub parent_skills: Option<Vec<cf_tools::implementations::skills::types::SkillInfo>>,
    /// Parent's skills config for the child's SkillManager.
    pub parent_skills_config: cf_agent::prompt::skills::SkillsConfig,
    ...
    pub parent_compat: cf_tools::types::compat::CompatConfig,
    /// Shared set of IDs delivered via auto-wake synthetic prompts.
    pub auto_wake_delivered:
        Option<cf_tools::reminders::task_completion::AutoWakeDeliveredIds>,
...
impl SubagentSpawnContext {
    ...
    fn apply_session_cli_overrides(&self, def: &mut cf_agent::config::AgentDefinition) {
    ...
    pub fn resolve_compaction_verbatim_input(&self) -> bool {
        crate::agent::config::BoolFlag::env("QIDI_COMPACTION_VERBATIM_INPUT")
            .config(
                self.agent_config
                    .as_ref()
    ...
    pub fn resolve_subagent_worktree_snapshot_enabled(&self) -> bool {
        crate::agent::config::BoolFlag::env("QIDI_SUBAGENT_WORKTREE_SNAPSHOT")
            .config(
                self.agent_config
                    .as_ref()
...
use cf_subagent_resolution::ResumeSourceData;
...
use cf_subagent_resolution::resolve_effective_overrides;
...
async fn resolve_subagent_sampling_config(
    agent_name: &str,
    agent_model: &cf_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (cf_sampler::SamplerConfig, acp::ModelId) {
    use cf_agent::config::ModelOverride;
...
async fn resolve_effective_model_config(
    ...
    subagent_type: &str,
    definition_model: &cf_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (cf_sampler::SamplerConfig, acp::ModelId) {
...
fn log_subagent_model_resolution(
    agent_name: &str,
    priority: &str,
    resolved: &cf_sampler::SamplerConfig,
    resolved_id: &acp::ModelId,
    parent: &cf_sampler::SamplerConfig,
) {
    ...
    if let Some(ref chat_state) = ctx.parent_chat_state {
        if let Some(cfg) = chat_state.get_sampling_config().await {
            ...
            let auth_scheme = crate::agent::config::try_resolve_model_credentials(&cfg.model, None)
                .map(|r| r.auth_scheme)
                .unwrap_or_default();
            let inherited = cf_sampler::SamplerConfig {
                ...
                // Subagents never fail over on their own; only the main
                // conversation config carries fallback entries.
...
fn resolve_model_override_to_config(
    model_id: &str,
    ctx: &SubagentSpawnContext,
) -> Option<(cf_sampler::SamplerConfig, acp::ModelId)> {
...
pub(crate) fn resume_inherited_prefix_len(
    conversation: &[cf_sampling_types::conversation::ConversationItem],
) -> usize {
    use cf_sampling_types::conversation::ConversationItem;
    conversation
        .iter()
...
struct InitialContext {
    ...
    conversation: Vec<cf_sampling_types::conversation::ConversationItem>,
...
fn forked_initial_context(
    mut items: Vec<cf_sampling_types::conversation::ConversationItem>,
) -> InitialContext {
    ...
    let (conversation, prefix_len) =
        cf_subagent_resolution::context::normalize_forked_context(items);
    if prefix_len < 2 {
        return InitialContext {
            source: InitialContextSource::New,
...
fn conversation_tail_is_complete(
    items: &[cf_sampling_types::conversation::ConversationItem],
) -> bool {
    use cf_sampling_types::conversation::ConversationItem;
    matches!(
...
fn verbatim_or_normalize_fork(
    items: Vec<cf_sampling_types::conversation::ConversationItem>,
    ...
    let estimated_tokens = cf_chat_state::estimate_conversation_tokens(&items);
    ...
    let (conversation, prefix_len) =
        cf_subagent_resolution::context::normalize_forked_context(filtered);
    InitialContext {
        source: InitialContextSource::Forked,
        copy_error: None,
        prefix_len: Some(prefix_len),
    ...
    if let Some(source) = resume_source {
        ...
        return match storage.copy_session_data_sync(
            ...
            Ok(result) => {
                ...
                let estimated_tokens = cf_chat_state::estimate_conversation_tokens(&conversation);
...
fn filter_pool_by_inheritance(
    ...
    inheritance: &cf_agent::config::McpInheritance,
) -> Option<crate::session::mcp_servers::SharedMcpPool> {
    use cf_agent::config::McpInheritance;
    match inheritance {
...
fn resolve_agent_definition(
    subagent_type: &str,
    ctx: &SubagentSpawnContext,
) -> Option<cf_agent::config::AgentDefinition> {
    let mut def = cf_agent::discovery::by_name_in_cwd_with_plugins(
        subagent_type,
        &ctx.parent_cwd,
...
pub(crate) struct SubagentValidationContext {
    pub parent_cwd: PathBuf,
    pub plugin_registry: Option<Arc<cf_agent::plugins::PluginRegistry>>,
...
pub(crate) fn validate_subagent_type(
    ...
    let resolves = ctx.cli_agent_names.iter().any(|n| n == subagent_type)
        || cf_agent::discovery::by_name_in_cwd_with_plugins(
            subagent_type,
            &ctx.parent_cwd,
            ctx.plugin_registry.as_deref(),
        )
        .is_some();
    if !resolves {
        let mut available: Vec<String> = cf_agent::discovery::all_subagents_with_plugins(
            &ctx.parent_cwd,
            &ctx.subagent_toggle,
            ctx.plugin_registry.as_deref(),
        )
...
/// (so a cf-tools session can run an alternate-harness verifier and vice-versa);
...
fn resolve_subagent_toolset(
    ...
    ctx: &SubagentSpawnContext,
    definition: &mut cf_agent::config::AgentDefinition,
) {
...
fn resolve_subagent_source_repo(ctx: &SubagentSpawnContext) -> std::path::PathBuf {
    ...
    cf_workspace::session::git::find_main_repo_root_from_path(&source_cwd)
...
/// auto-backgrounded (non-destructively). Env override: `QIDI_SUBAGENT_AWAIT_BUDGET_MS`.
const SUBAGENT_AWAIT_BUDGET: std::time::Duration = std::time::Duration::from_secs(600);
fn subagent_await_budget() -> std::time::Duration {
    std::env::var("QIDI_SUBAGENT_AWAIT_BUDGET_MS")
        .ok()
...
fn cancellation_error_message(
    category: Option<cf_file_utils::events::types::CancellationCategory>,
...
fn inject_subagent_completed_prompt(
    ...
    request: &SubagentRequest,
    auto_wake_delivered: &Option<cf_tools::reminders::task_completion::AutoWakeDeliveredIds>,
    ...
    let message = cf_tools::reminders::task_completion::format_subagent_completion(
        &summary,
        Some(task_output_tool_name),
    );
    let wrapped = cf_tools::reminders::wrap_reminder(&message);
