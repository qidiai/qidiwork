...
async fn subagent_inherits_session_cli_overrides() {
    use cf_agent::config::{AgentDefinition, PermissionMode};
...
async fn emit_subagent_notification_stamps_one_event_id_on_both_paths() {
    ...
    let broadcast_id = match gateway_rx.try_recv().expect("broadcast must fire") {
        cf_acp_lib::AcpClientMessage::ExtNotification(args) => {
...
fn subagent_inherits_parent_lsp_via_context() {
    let parent: std::sync::Arc<dyn cf_tools::implementations::lsp::LspBackend> = Arc::new(
        DummyLspDispatch,
    );
...
fn inject_subagent_completed_prompt_sends_prompt_and_marks_delivered() {
    ...
    let auto_wake = cf_tools::reminders::task_completion::AutoWakeDeliveredIds::default();
...
fn dummy_tracker(
    ...
    let handle = SessionHandle {
        ...
        max_turns: None,
        hunk_tracker_handle: cf_hunk_tracker::HunkTrackerHandle::noop(),
...
fn explicit_override_takes_precedence_over_role() {
    let overrides = SubagentRuntimeOverrides {
        ...
        capability_mode: Some(cf_tool_types::SubagentCapabilityMode::All),
        ..Default::default()
    };
    let role = cf_subagent_resolution::config::SubagentRole {
    ...
    assert_eq!(
        resolved.capability_mode, Some(cf_tool_types::SubagentCapabilityMode::All)
    );
}
#[test]
fn role_default_used_when_no_explicit_override() {
    ...
    assert_eq!(
        resolved.capability_mode, Some(cf_tool_types::SubagentCapabilityMode::ReadOnly)
    );
}
#[test]
...
fn partial_override_fills_from_role() {
    ...
    assert_eq!(resolved.model.as_deref(), Some("explicit-model"));
    assert_eq!(
        resolved.capability_mode, Some(cf_tool_types::SubagentCapabilityMode::Execute)
    );
}
#[test]
...
fn model_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_model = cf_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
...
fn reasoning_effort_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_re = cf_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
    let r = resolve_effective_overrides(
    ...
    if let ConversationItem::User(ref u) = ctx.conversation[1] {
        let text: String = u
            ...
            .filter_map(|p| match p {
                cf_sampling_types::conversation::ContentPart::Text { text } => {
                    Some(text.as_ref())
                }
                _ => None,
            })
...
fn forked_initial_context_inherits_parent_across_reasoning() {
    ...
    let items = vec![
        ...
        ConversationItem::Reasoning(cf_sampling_types::synthesized_reasoning_item("deliberating",)),
...
fn resume_vs_fork_helper_shapes_differ() {
    ...
    assert!(
        ...
        cf_sampling_types::conversation::ContentPart::Text { text } if text
...
fn verbatim_fork_keeps_items_byte_for_byte_when_small() {
    use cf_sampling_types::conversation::{
    ...
    let items = vec![
        ...
        ConversationItem::Reasoning(cf_sampling_types::synthesized_reasoning_item("thinking",)),
        ConversationItem::assistant("ack"),
    ];
...
fn verbatim_fork_falls_back_to_summary_when_oversize() {
    use cf_sampling_types::conversation::{ContentPart, ConversationItem};
    let items = vec![
        ConversationItem::system("parent system"),
                    ...
                    ConversationItem::User(u) => {
                        Some(
                            u
                                ...
                                .iter()
                                .filter_map(|p| match p {
                                    cf_sampling_types::conversation::ContentPart::Text {
                                        text,
                                    } => Some(text.as_ref()),
                                    _ => None,
                                })
...
fn subagent_await_budget_default_and_override() {
    unsafe { std::env::remove_var("QIDI_SUBAGENT_AWAIT_BUDGET_MS") };
    assert_eq!(SUBAGENT_AWAIT_BUDGET, std::time::Duration::from_secs(600));
    assert_eq!(subagent_await_budget(), SUBAGENT_AWAIT_BUDGET);
    unsafe { std::env::set_var("QIDI_SUBAGENT_AWAIT_BUDGET_MS", "1500") };
    assert_eq!(subagent_await_budget(), std::time::Duration::from_millis(1500));
    unsafe { std::env::set_var("QIDI_SUBAGENT_AWAIT_BUDGET_MS", "0") };
    assert_eq!(subagent_await_budget(), SUBAGENT_AWAIT_BUDGET);
    unsafe { std::env::set_var("QIDI_SUBAGENT_AWAIT_BUDGET_MS", "not-a-number") };
    assert_eq!(subagent_await_budget(), SUBAGENT_AWAIT_BUDGET);
    unsafe { std::env::remove_var("QIDI_SUBAGENT_AWAIT_BUDGET_MS") };
}
#[test]
...
/// Regression: on the DEFAULT cf-tools host —
...
async fn cancel_pending_at_promote_removes_fresh_worktree_preserves_resumed() {
    ...
    cf_fast_worktree::WorktreeBuilder::new(&repo, &fresh)
        .standalone(true)
        .create()
        .unwrap();
    assert!(fresh.exists());
    ...
    cf_fast_worktree::WorktreeBuilder::new(&repo, &resumed)
        .standalone(true)
        .create()
        .unwrap();
...
fn normalize_forked_context_empty_parent() {
    ...
    let (conv, prefix_len) = cf_subagent_resolution::context::normalize_forked_context(
        items,
    );
...
fn test_sampling_config(model_slug: &str) -> cf_sampling_types::SamplingConfig {
...
fn spawn_test_parent_chat_state(model_slug: &str) -> cf_chat_state::ChatStateHandle {
    let (mock, _persistence_rx) = cf_chat_state::MockChatPersistence::new();
    let (event_tx, _event_rx) = mpsc::unbounded_channel();
    let token = tokio_util::sync::CancellationToken::new();
    cf_chat_state::ChatStateActor::spawn(
        vec![],
