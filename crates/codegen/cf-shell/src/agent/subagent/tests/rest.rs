...
fn normalize_forked_context_strips_project_layout() {
    ...
    let (conv, _) = cf_subagent_resolution::context::normalize_forked_context(
        items,
    );
...
fn end_to_end_normalized_conversation_shape() {
    ...
    let (mut conv, prefix_len) = cf_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
    assert_eq!(prefix_len, 2);
...
fn last_user_message_is_task_after_normalization() {
    ...
    let (mut conv, _) = cf_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
        ...
        .filter(|i| {
            if let ConversationItem::User(u) = i {
                u.content
                    ...
                    .any(|p| {
                        matches!(
                            p, cf_sampling_types::conversation::ContentPart::Text {
...
async fn completion_snapshot_sequence_persists_ref_then_removes_worktree() {
    ...
    cf_fast_worktree::WorktreeBuilder::new(&repo, &wt)
        .standalone(true)
        .create()
        .unwrap();
...
fn resume_source_worktree_reuse() {
    let source_with_worktree = ResumeSourceData {
        ...
        worktree_path: Some(
            PathBuf::from("/home/user/.qidi/worktrees/myrepo/subagent-sub-wt"),
        ),
        snapshot_ref: None,
        subagent_type: "general-purpose".into(),
        persona: None,
    ...
    assert_eq!(
        worktree.as_deref(),
        Some(Path::new("/home/user/.qidi/worktrees/myrepo/subagent-sub-wt",)),
...
fn select_override_cwd_resume_never_falls_through_to_request_cwd() {
    let source = ResumeSourceData {
        ...
        worktree_path: Some(
            PathBuf::from("/home/user/.qidi/worktrees/repo/subagent-sub-wt"),
        ),
        snapshot_ref: None,
        subagent_type: "general-purpose".into(),
        persona: None,
...
fn token_estimation_for_window_safety() {
    ...
    let estimated = cf_chat_state::estimate_conversation_tokens(&conversation);
...
fn token_estimation_accounts_for_images() {
    ...
    let multi_tokens = cf_chat_state::estimate_conversation_tokens(&multi_image);
...
fn resume_identity_does_not_gate_on_model() {
    ...
    assert!(
        cf_subagent_resolution::validate_resume_identity("general-purpose", None, &
        source,).is_ok()
    );
    assert_eq!(
...
fn non_cursor_persona_injected_as_system_reminder() {
    use cf_sampling_types::conversation::{ConversationItem, SyntheticReason};
...
fn filter_inheritance_all_passes_everything_through() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &cf_agent::config::McpInheritance::All,
    );
...
fn filter_inheritance_none_returns_none() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &cf_agent::config::McpInheritance::None,
    );
...
fn filter_inheritance_named_selects_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &cf_agent::config::McpInheritance::Named(
...
fn filter_inheritance_except_excludes_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &cf_agent::config::McpInheritance::Except(
...
fn filter_inheritance_named_empty_list_gives_empty_pool() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &cf_agent::config::McpInheritance::Named(vec![]),
    );
...
fn filter_inheritance_except_empty_list_keeps_all() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &cf_agent::config::McpInheritance::Except(vec![]),
    );
    let result = result.expect("Except([]) should return Some");
...
fn filter_inheritance_except_nonexistent_servers_ignored() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &cf_agent::config::McpInheritance::Except(vec!["nonexistent".into()]),
    );
    let result = result.expect("Except should return Some");
    assert_eq!(pool_names(& result), vec!["github", "linear"]);
}
...
fn filter_inheritance_named_all_nonexistent_gives_empty() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &cf_agent::config::McpInheritance::Named(vec!["foo".into(), "bar".into()]),
    );
    let result = result.expect("Named should return Some");
    assert_eq!(result.server_names().count(), 0);
}
...
fn make_test_skill(
    ...
    cf_tools::implementations::skills::types::SkillInfo {
        ...
        scope: cf_tools::implementations::skills::types::SkillScope::Local,
        enabled: true,
        user_invocable: true,
