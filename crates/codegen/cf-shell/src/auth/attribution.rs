...
//!
//! [`cf_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`cf_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`cf_sampler::SamplerConfig::attribution_callback`] at every
...
impl ShellAttribution {
    ...
    /// [`cf_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `cf_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn cf_tools::Auth401AttributionCallback>` for the
...
pub(crate) fn record_auth_401(
    ...
    // Sink 1 -- local file (~/.qidi/logs/unified.jsonl) + scrubbed
...
mod tests {
    ...
    /// nothing from a developer's actual `~/.qidi/auth.json` leaks in.
    ...
    /// `ShellAttribution` implements `cf_tools::Auth401AttributionCallback`
