...
pub struct GrokComConfig {
    ...
    /// Login button label (env: `QIDI_AUTH_PROVIDER_LABEL`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// refresh works. Env: `QIDI_AUTH_TOKEN_TTL`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// can't bypass the deployment's IdP login. Env: `QIDI_DISABLE_API_KEY_AUTH`.
...
/// OAuth2 provider configuration (`QIDI_OAUTH2_ISSUER` / `QIDI_OAUTH2_CLIENT_ID`).
///
...
pub fn use_local_auth() -> bool {
    std::env::var("QIDI_LOCAL_AUTH")
        .map(|v| !v.is_empty() && v != "0")
        .unwrap_or(false)
}
/// Returns the active xAI OAuth2 issuer — the local-dev issuer when
/// `QIDI_LOCAL_AUTH=1` is set, otherwise the production issuer.
pub fn cf_oauth2_issuer() -> &'static str {
    if use_local_auth() {
...
impl GrokComConfig {
    ...
    /// `QIDI_DISABLE_API_KEY_AUTH` env lockdown is sticky: because the env value
...
impl OAuth2ProviderConfig {
    ...
    pub fn from_env() -> Option<Self> {
        let issuer = std::env::var("QIDI_OAUTH2_ISSUER").ok()?;
        let client_id = std::env::var("QIDI_OAUTH2_CLIENT_ID").ok()?;
        let principal_type = std::env::var("QIDI_OAUTH2_PRINCIPAL_TYPE").ok();
        let principal_id = std::env::var("QIDI_OAUTH2_PRINCIPAL_ID").ok();
        ...
        Some(Self {
            issuer,
            client_id,
            scopes: std::env::var("QIDI_OAUTH2_SCOPES")
                .map(|s| s.split(',').map(|s| s.trim().to_owned()).collect())
                .unwrap_or(default_scopes),
            principal_type,
            principal_id,
            referrer: Some(
                std::env::var("QIDI_OAUTH2_REFERRER")
...
impl Default for GrokComConfig {
    fn default() -> Self {
        ...
        Self {
            grok_ws_origin: std::env::var("QIDI_WS_ORIGIN")
                .unwrap_or_else(|_| PROD_WS_ORIGIN.to_owned()),
            grok_ws_url: std::env::var("QIDI_WS_URL")
                .unwrap_or_else(|_| PROD_RELAY_WS_URL.to_owned()),
            token_header: "cli".to_owned(),
            oidc,
            oauth2,
            auth_provider_command: std::env::var("QIDI_AUTH_PROVIDER_COMMAND").ok(),
            auth_provider_label: std::env::var("QIDI_AUTH_PROVIDER_LABEL").ok(),
            auth_token_ttl: std::env::var("QIDI_AUTH_TOKEN_TTL")
                .ok()
                .and_then(|v| v.parse().ok()),
            disable_api_key_auth: std::env::var("QIDI_DISABLE_API_KEY_AUTH")
                .ok()
...
/// `no`, empty) count as disabled — so e.g. `QIDI_DISABLE_API_KEY_AUTH=false`
...
/// True when the admin has set `QIDI_DISABLE_API_KEY_AUTH` to a truthy value in
...
fn env_lockdown_forced() -> bool {
    std::env::var("QIDI_DISABLE_API_KEY_AUTH")
        .ok()
        .is_some_and(|v| env_flag_enabled(&v))
}
impl OidcAuthConfig {
    pub fn from_env() -> Option<Self> {
        let issuer = std::env::var("QIDI_OIDC_ISSUER").ok()?;
        let client_id = std::env::var("QIDI_OIDC_CLIENT_ID").ok()?;
        Some(Self {
            issuer,
            client_id,
            scopes: std::env::var("QIDI_OIDC_SCOPES")
                .map(|s| s.split(',').map(|s| s.trim().to_owned()).collect())
                .unwrap_or_else(|_| default_oidc_scopes()),
            audience: std::env::var("QIDI_OIDC_AUDIENCE").ok(),
        })
    }
}
#[cfg(test)]
