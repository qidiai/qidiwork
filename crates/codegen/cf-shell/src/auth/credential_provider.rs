...
fn api_key_id_for(auth: Option<&crate::auth::GrokAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| cf_telemetry::config::deployment_id_from_key(&a.key))
}
...
///
/// - `"grok-shell"`   — classic Grok CLI / TUI (qidi-code)
/// - `"grok-pager"`   — new Grok Pager / TUI (qidi-code)
...
pub fn build_storage_client_for_proxy(
...
) -> cf_file_utils::storage_client::StorageClient {
    ...
    if let Some(am) = auth_manager {
        ...
        let bridge: Arc<dyn cf_file_utils::storage_client::Auth401AttributionCallback> =
            Arc::new(StorageClientAttributionBridge::new(am, session_id));
        cf_file_utils::storage_client::StorageClient::with_provider(
            proxy_base_url,
            http_client,
            provider,
        )
        .with_client_identity(cf_version::VERSION, client_identifier)
...
pub fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        cf_telemetry::external::set_identity(
            cf_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
/// `qidi-code` and `xai-grok-tui` need at tracing init time.
///
...
pub fn build_default_otel_layer_config() -> cf_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let grok_com_config = crate::auth::GrokComConfig::default();
    let exporter = cf_telemetry::otel_layer::OtelExporterConfig {
    ...
    cf_telemetry::otel_layer::OtelLayerConfig {
...
mod tests {
    ...
    use std::sync::Mutex;
    use cf_auth::AuthCredentialProvider;
    /// Serializes tests that pin `QIDI_AUTH_EARLY_INVALIDATION_SECS`, since
    /// env vars are process-global and parallel tests would race.
    static EARLY_INVALIDATION_LOCK: Mutex<()> = Mutex::new(());
    /// RAII guard: pins `QIDI_AUTH_EARLY_INVALIDATION_SECS` to the production
    ...
    impl EarlyInvalidationGuard {
        fn pin_to_default() -> Self {
            ...
            let previous = std::env::var("QIDI_AUTH_EARLY_INVALIDATION_SECS").ok();
            unsafe { std::env::set_var("QIDI_AUTH_EARLY_INVALIDATION_SECS", "300") };
            Self {
                _lock: lock,
                previous,
            }
        ...
        fn drop(&mut self) {
            unsafe {
                match self.previous.take() {
                    Some(prev) => std::env::set_var("QIDI_AUTH_EARLY_INVALIDATION_SECS", prev),
                    None => std::env::remove_var("QIDI_AUTH_EARLY_INVALIDATION_SECS"),
                }
            }
        }
    }
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use cf_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
