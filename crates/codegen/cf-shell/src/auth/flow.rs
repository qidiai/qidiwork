...
fn resolve_device_flow(
    ...
    crate::agent::config::BoolFlag::env("QIDI_LOGIN_DEVICE_FLOW")
...
async fn should_use_device_flow(login_override: LoginTransportOverride) -> bool {
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = crate::agent::config::env_bool("QIDI_LOGIN_DEVICE_FLOW");
...
async fn run_external_auth_provider(
    ...
    if is_refresh {
        cmd.env("QIDI_AUTH_EXPIRED", "1");
    }

    cf_tools::util::detach_command(&mut cmd);
    cmd.envs(cf_tools::util::pager_env());
    // The external auth script is user-configured arbitrary code; don't let it
...
async fn run_auth_flow_inner(
    ...
    // --device-auth / QIDI_LOGIN_DEVICE_FLOW / [auth] login_device_flow.
...
/// otherwise `QIDI_LOGIN_DEVICE_FLOW` env / `[auth] login_device_flow` config /
...
pub struct LogoutResult {
    ...
    /// `true` if `XAI_API_KEY` / `QIDI_CODE_XAI_API_KEY` env var is set.
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ...
        cf_telemetry::external::set_identity(
            cf_telemetry::external::IdentityAttrs::default(),
        );
        cf_telemetry::external::flush();
...
mod tests {
    ...
    /// Run `f` with `QIDI_LOGIN_DEVICE_FLOW` set to `value` (unset for `None`).
    ...
    fn with_device_flow_env<T>(value: Option<bool>, f: impl FnOnce() -> T) -> T {
        let _guard = match value {
            Some(true) => EnvVarGuard::set("QIDI_LOGIN_DEVICE_FLOW", "true"),
            Some(false) => EnvVarGuard::set("QIDI_LOGIN_DEVICE_FLOW", "false"),
            None => EnvVarGuard::remove("QIDI_LOGIN_DEVICE_FLOW"),
        };
        f()
    }
    ...
    async fn preresolved_bypasses_resolver_and_is_never_cli() {
        ...
        {
            let _guard = EnvVarGuard::set("QIDI_LOGIN_DEVICE_FLOW", "false");
            assert!(
        ...
        {
            let _guard = EnvVarGuard::set("QIDI_LOGIN_DEVICE_FLOW", "true");
            assert!(
    ...
    async fn run_auth_flow_falls_through_when_no_refresh_token() {
        ...
        // QIDI_LOGIN_DEVICE_FLOW / the real config file (the CLI override
