...
use cf_telemetry::events::ManualAuthSurface;
...
pub struct AuthManager {
    ...
    power_listener: parking_lot::Mutex<Option<cf_system_power::SystemPowerListener>>,
...
impl AuthManager {
    pub fn new(grok_home: &Path, grok_com_config: GrokComConfig) -> Self {
        ...
        cf_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                ...
                "QIDI_HOME": std::env::var("QIDI_HOME").unwrap_or_else(|_| "(unset)".into()),
                "QIDI_AUTH_PATH": std::env::var("QIDI_AUTH_PATH").unwrap_or_else(|_| "(unset)".into()),
                "QIDI_AUTH": std::env::var("QIDI_AUTH").map(|_| "(set)".to_string()).unwrap_or_else(|_| "(unset)".into()),
            })),
        );

        // QIDI_AUTH: inline JSON credentials (highest priority, read-only).
        if let Ok(inline_json) = std::env::var("QIDI_AUTH") {
            ...
            tracing::warn!("QIDI_AUTH set but failed to parse as JSON, falling back to file");
        }

        // QIDI_AUTH_PATH: custom file path (overrides default $QIDI_HOME/auth.json).
        let path = std::env::var("QIDI_AUTH_PATH")
            .map(PathBuf::from)
    ...
    /// (inline `QIDI_AUTH` vs. on-disk `auth.json`), which differ only in the
    ...
    pub(crate) async fn update(self: &Arc<Self>, auth: GrokAuth) -> std::io::Result<GrokAuth> {
        ...
        match &write_result {
            Ok(()) => cf_telemetry::unified_log::info(
            ...
            Err(e) => cf_telemetry::unified_log::error(
    ...
    fn observe_disk_state(
        ...
        match new_state {
            // Recovery (or first observation in QIDI_AUTH mode).
            DiskAuthState::Ok => {
                cf_telemetry::unified_log::info(
    ...
    pub(crate) async fn try_devbox_recovery(self: &Arc<Self>) -> Result<GrokAuth, AuthError> {
        ...
        cf_telemetry::unified_log::info("auth: devbox recovery starting", None, None);
    ...
    fn check_refresh_deferral(&self, reason: RefreshReason) -> Result<(), AuthError> {
        ...
        // Dark wake (see `cf_system_power::PowerState` for the canonical
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<cf_telemetry::events::ManualAuth> {
...
impl cf_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
/// Build a refreshing [`ApiKeyProvider`](cf_tools::types::ApiKeyProvider)
...
/// [`current_api_key_async`](cf_tools::types::ApiKeyProvider::current_api_key_async),
/// so it works for both OAuth/session (refreshes) and API-key auth.
pub fn shared_api_key_provider(
    auth_manager: Arc<AuthManager>,
) -> cf_tools::types::SharedApiKeyProvider {
