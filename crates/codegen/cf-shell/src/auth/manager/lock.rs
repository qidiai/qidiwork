...
mod tests {
    ...
    // `QIDI_TEST_LOCK_HOLDER` env var on an `#[ignore]`d entry-point test —
    // no external `python3` dependency.

    /// Line printed to stdout once the subprocess holds the flock.
    #[cfg(unix)]
    const LOCK_HOLDER_READY: &str = "__QIDI_LOCK_HOLDER_READY__";

    /// Subprocess entry point for the cross-process lock tests. Only does
    /// anything when re-executed with `QIDI_TEST_LOCK_HOLDER` set; a normal
    ...
    fn subprocess_lock_holder() {
        let Ok(spec) = std::env::var("QIDI_TEST_LOCK_HOLDER") else {
    ...
    fn spawn_lock_holder_subprocess(
        ...
        let mut child = std::process::Command::new(exe)
            .env("QIDI_TEST_LOCK_HOLDER", spec)
            .args([
                "--ignored",
                "--exact",
                "--nocapture",
