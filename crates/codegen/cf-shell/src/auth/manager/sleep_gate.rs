...
impl SleepGate {
    pub(super) fn raise(&self) {
        *self.raised_at.write() = Some(GateRaise::now());
        cf_telemetry::unified_log::warn("auth.sleep.gate_set", None, None);
    }
...
impl AuthManager {
    ...
    /// [`cf_system_power::PowerState`] for the canonical explanation of what a
    ...
    pub(crate) fn is_dark_wake(&self) -> bool {
        ...
        matches!(
            cf_system_power::current_power_state(),
            cf_system_power::PowerState::DarkWake
        )
    }
    ...
    pub fn start_system_power_listener(self: &Arc<Self>) {
        ...
        let listener = cf_system_power::SystemPowerListener::start(move |event| {
            if let Some(this) = weak.upgrade() {
                let imminent = matches!(event, cf_system_power::PowerEvent::WillSleep);
