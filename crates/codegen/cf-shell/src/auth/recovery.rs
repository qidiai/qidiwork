...
use cf_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};
...
impl ManualAuthTracker {
    ...
    pub(crate) fn record(
        ...
        cf_telemetry::session_ctx::log_event(event);
    }
...
impl UnauthorizedRecovery {
    ...
    async fn try_reload_from_disk(&self) -> Option<GrokAuth> {
        ...
        let Some(disk_auth) = self.auth_manager.read_disk_auth() else {
            ...
            cf_telemetry::unified_log::debug("auth recovery: no disk entry", None, None);
            return None;
        };
