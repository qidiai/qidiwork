//! Built-in files extracted to `~/.qidi/` on startup.
...
///
/// These directories under `~/.qidi/skills/` will be deleted on startup
...
fn resolve_skill_content(name: &str, raw: &str, grok_home: &std::path::Path) -> String {
    match name {
        ...
        "help" => {
            ...
            raw.replace("~/.qidi/", &grok_home_str)
        }
        _ => raw.to_string(),
    }
}

/// Extract bundled files to `~/.qidi/` on startup.
///
