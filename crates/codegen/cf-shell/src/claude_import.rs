// claude_import.rs
// Scans Claude settings and generates TOML patches for .qidi/config.toml.
//
...
use cf_workspace::permission::claude_settings::{
    find_claude_settings_paths, load_claude_settings,
};
use cf_workspace::permission::rules::parse_permission_rule;
use cf_workspace::permission::types::{PatternMode, PermissionRule, RuleAction, ToolFilter};
...
pub enum ImportScope {
    /// User-level: writes to `~/.qidi/config.toml`.
    Global,
    /// Project-level: writes to `<repo>/.qidi/config.toml`.
    Project,
}
...
pub struct ImportPlan {
    /// Items to write to `~/.qidi/config.toml`.
    pub global_items: Vec<ImportableItem>,
    /// Items to write to `<repo>/.qidi/config.toml`.
...
//
// The marker `[claude_compat] imported = true` in `~/.qidi/config.toml` is
...
///
/// Reads `[claude_compat] imported = true` from `~/.qidi/config.toml` once
...
/// Write `[claude_compat] imported = true` to `~/.qidi/config.toml`.
///
...
/// overwritten).
///
/// Project items are written to `<repo_root>/.qidi/config.toml` (discovered
/// via `git2::Repository::discover`), not `cwd/.qidi/config.toml`, to avoid
...
pub fn apply_import(plan: &ImportPlan, cwd: &Path) -> anyhow::Result<ImportResult> {
    ...
    if !plan.global_items.is_empty() {
        ...
        // Hooks are written separately to ~/.qidi/hooks/imported-from-claude.json.
...
fn apply_items_to_config(config_path: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    for item in items {
        match item {
            ...
            // Hooks are written to .qidi/hooks/ JSON files in apply_hooks_to_dir,
...
/// (Claude-compatible). The native hooks loader scans `.qidi/hooks/*.json`
...
fn apply_hooks_to_dir(hooks_dir: &Path, items: &[ImportableItem]) -> anyhow::Result<usize> {
    ...
    if count > 0 || dirty {
        ...
        info!(
            path = %target.display(),
            count,
            "Wrote imported hooks to .qidi/hooks/imported-from-claude.json"
        );
    }
...
mod tests {
    ...
    impl Drop for MarkerGuard {
        fn drop(&mut self) {
            ...
            unsafe { std::env::remove_var("_QIDI_CLAUDE_MARKER_OVERRIDE") };
        }
    }
    ...
    fn discover_hook_source_paths_skips_cursor_when_disabled() {
        ...
        refresh_marker_cache(false);
        let dir = tempfile::tempdir().unwrap();
        let mut compat = cf_tools::types::compat::CompatConfig::default();
    ...
    fn discover_hooks_honors_claude_compat_gate() {
        ...
        // test host (from the non-injectable ~/.claude, ~/.qidi) don't interfere.
        let has_probe = |reg: &cf_hooks::discovery::HookRegistry| {
...
"#;
    ...
    fn scan_claude_path_dirs_dedupes_global_and_project_when_same() {
        ...
        // requires `QIDI_HOME` plumbing which is intentionally out of scope.
    ...
    async fn gate_resolve_permissions_with_provenance_skips_claude_when_marker_set() {
        ...
        // managed settings, and the developer's real `~/.qidi/config.toml`.
        // We can't isolate `grok_home()` because it's `OnceLock`-cached.
        // Instead, assert on rule *provenance*: no rule should originate from
        // our tempdir's `.claude/settings.json`. The dev's real ~/.qidi
        // config rules (if any) are out of scope for this test.
        let resolved =
            cf_workspace::permission::resolution::resolve_permissions_with_provenance(
                dir.path(),
            )
            .await;
        if let Some(r) = resolved {
            let tempdir_claude = claude_dir.join("settings.json");
            use cf_workspace::permission::types::RequirementSource;
    ...
    fn gate_marker_cache_unset_means_uses_disk() {
        ...
        // dev's real `~/.qidi/config.toml` may legitimately have the marker
