//! Data APIs for `available models`. Clients own display.
...
/// Status for the `available models` banner (display order ≠ sampling priority; see [`AuthStatus::resolve`]).
...
mod tests {
    ...
    fn isolate_auth_sources() -> (tempfile::TempDir, [EnvGuard; 7]) {
        ...
        let guards = [
            ...
            EnvGuard::unset("QIDI_AUTH"),
            EnvGuard::set("QIDI_AUTH_PATH", auth_path.to_str().unwrap()),
    ...
    fn resolve_oauth_session() {
        ...
        let _auth = EnvGuard::set("QIDI_AUTH", &json);
