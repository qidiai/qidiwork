...
pub enum ConfigUpdate {
    ...
    ///
    /// 1. The global `[mcp_servers]` table in `~/.qidi/config.toml`
    ///    changed.
    ...
    ///
    /// Project-scoped changes (`<cwd>/.qidi/config.toml`,
    ...
    /// (`<cwd>/.qidi/config.toml`, `<cwd>/.mcp.json`, or
    ...
    ProjectMcpServersChanged {
        /// The project root whose `.qidi/`, `.mcp.json`, or
    ...
    ModelsChanged,
    /// `~/.qidi/models_cache.json` was rewritten on disk (possibly by another
...
impl ConfigReloader {
    ...
    pub async fn run(
        ...
        loop {
            ...
            // same tick (e.g. `~/.qidi/config.toml` AND
    ...
    fn reload_config(&mut self) -> anyhow::Result<()> {
        ...
        // config (`~/.qidi/config.toml`) via toml::Value. Project-
        // scoped changes (`<cwd>/.qidi/config.toml`,
...
/// |----------------------------|-------------------------|-------------------|
/// | `ProjectConfigChanged`     | `<cwd>/.qidi/config.toml` | `<cwd>`           |
...
/// for `.qidi/config.toml`, `find_mcp_json_files` for `.mcp.json`) so
...
/// (`~/.qidi/config.toml`, `~/.claude.json`, `~/.cursor/mcp.json`)
...
pub(crate) fn parse_skills_config(
    config: &toml::Value,
) -> cf_agent::prompt::skills::SkillsConfig {
    config
        .get("skills")
...
fn parse_compat_config(config: &toml::Value) -> cf_tools::types::compat::CompatConfigToml {
    config
        .get("compat")
        .and_then(|v| v.clone().try_into().ok())
        .unwrap_or_default()
...
mod tests {
    ...
    /// The hash must reflect ancestor `.qidi/config.toml` and `.mcp.json`
    ...
    fn hash_project_mcp_config_covers_ancestors() {
        ...
        assert_ne!(
            h2, h3,
            "ancestor .qidi/config.toml create must change the hash"
        );
    }
    ...
    fn parse_skills_config_with_paths() {
        let config: toml::Value = toml::from_str(
            r#"
[skills]
paths = ["/home/user/.qidi/skills"]
ignore = ["/tmp"]
"#,
        )
        .unwrap();
        let skills = parse_skills_config(&config);
        assert_eq!(skills.paths, vec!["/home/user/.qidi/skills".to_string()]);
...
"#,
    ...
    /// `ProjectConfigChanged` (`<cwd>/.qidi/config.toml`) and
    ...
    fn collect_project_cwds_excludes_home_claude_json() {
        let batch = vec![
            ...
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/x/.qidi/config.toml"),
            },
        ];
    ...
    fn collect_project_cwds_dedupes_and_extracts() {
        let batch = vec![
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/a/.qidi/config.toml"),
            },
            ConfigChangeEvent::McpConfigChanged {
                path: PathBuf::from("/repo/a/.mcp.json"),
            },
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/b/.qidi/config.toml"),
            },
        ];
        let cwds = collect_project_cwds(&batch);
        assert_eq!(
