...
fn expands_env_vars_in_toml_strings() {
    with_env_var(
        "QIDI_TEST_CONFIG_EXPAND",
        "expanded",
        || {
            let toml_str = r#"
[mcp_servers.test]
command = "$QIDI_TEST_CONFIG_EXPAND/bin/server"
args = ["--path", "${QIDI_TEST_CONFIG_EXPAND}/data"]
"#;
...
fn leaves_missing_env_vars_unchanged() {
    let toml_str = r#"
[mcp_servers.test]
command = "$QIDI_TEST_CONFIG_MISSING/bin/server"
"#;
    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
    expand_env_vars_in_toml(&mut value);
    let toml::Value::Table(table) = value else {
    ...
    let command = test.get("command").and_then(|v| v.as_str()).unwrap();
    assert_eq!(command, "$QIDI_TEST_CONFIG_MISSING/bin/server");
}
#[test]
...
/// Mutex to serialize tests that touch the QIDI_MEMORY env var.
...
/// Run `f` with QIDI_MEMORY explicitly unset.
fn without_grok_memory<T>(f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("QIDI_MEMORY", None, f)
}
/// Run `f` with QIDI_MEMORY set to a specific value.
fn with_grok_memory<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("QIDI_MEMORY", Some(value), f)
}
#[test]
...
fn memory_config_env_var_zero_does_not_enable() {
    with_grok_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(! mem.enabled, "QIDI_MEMORY=0 should not enable memory");
        },
    );
}
#[test]
fn memory_config_env_var_false_does_not_enable() {
    with_grok_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(! mem.enabled, "QIDI_MEMORY=false should not enable memory");
        },
    );
}
#[test]
...
fn memory_config_env_zero_force_disables_toml_enabled() {
    with_grok_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "QIDI_MEMORY=0 should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_env_false_force_disables_toml_enabled() {
    with_grok_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "QIDI_MEMORY=false should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_cli_flag_overrides_env_disable() {
    with_grok_memory(
        ...
        || {
            ...
            let mem = MemoryConfig::resolve(true, false, &config, None);
            assert!(
                mem.enabled, "CLI --experimental-memory should override QIDI_MEMORY=0"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_all() {
    with_grok_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "--no-memory should override --experimental-memory, QIDI_MEMORY=1, and TOML enabled=true"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_env_enable() {
    with_grok_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, true, &config, None);
            assert!(! mem.enabled, "--no-memory should override QIDI_MEMORY=1");
        },
    );
}
#[test]
        ...
        || {
            with_env_var(
                ...
                || {
                    let toml_str = r#"
[mcp_servers.test]
command = "$QIDI_TEST_USER $QIDI_TEST_ROOT"
"#;
                    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
                    expand_env_vars_in_toml(&mut value);
                    let toml::Value::Table(table) = value else {
...
/// Mutex to serialize tests that touch the QIDI_SUBAGENTS env var.
static SUBAGENTS_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());
/// Run `f` with QIDI_SUBAGENTS explicitly unset.
fn without_grok_subagents<T>(f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("QIDI_SUBAGENTS", None, f)
}
/// Run `f` with QIDI_SUBAGENTS set to a specific value.
fn with_grok_subagents<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("QIDI_SUBAGENTS", Some(value), f)
}
#[test]
    ...
    with_grok_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str("[subagents]\nenabled = true")
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config, None);
            assert!(! sa.enabled, "QIDI_SUBAGENTS=0 should override config file");
        },
    );
}
#[test]
...
fn subagents_config_env_var_disables_default() {
    with_grok_subagents(
        ...
        || {
            ...
            let sa = SubagentsConfig::resolve(false, &config, None);
            assert!(
                ! sa.enabled, "QIDI_SUBAGENTS=0 should override the enabled default"
            );
        },
    );
}
...
fn subagents_config_cli_flag_overrides_env_var() {
    with_grok_subagents(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let sa = SubagentsConfig::resolve(true, &config, None);
            assert!(sa.enabled, "--subagents CLI flag should override QIDI_SUBAGENTS=0");
        },
    );
}
#[test]
    ...
    with_grok_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str(
                ...
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config, None);
            assert!(sa.enabled, "QIDI_SUBAGENTS=1 should enable");
            assert_eq!(sa.models.get("explore").unwrap(), "grok-3-fast");
        },
    );
}
...
fn with_managed_mcp_env<T>(
    ...
    with_env_var_opt(
        ...
        managed_mcps,
        || with_env_var_opt("QIDI_MANAGED_MCP_GATEWAY_TOOLS_ENABLED", gateway_tools, f),
    )
}
#[test]
#[serial_test::serial]
...
fn with_model_overrides_env_full<T>(
    ...
    let _guard = LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt(
        "QIDI_WEB_SEARCH_MODEL",
        ws,
        || with_env_var_opt(
            "QIDI_SESSION_SUMMARY_MODEL",
            ss,
            || with_env_var_opt(
                "QIDI_IMAGE_DESCRIPTION_MODEL",
                id,
                || with_env_var_opt("QIDI_PROMPT_SUGGESTIONS_MODEL", ps, f),
            ),
        ),
    )
}
...
fn with_tools_env<T>(
    ...
    with_env_var_opt(
        "QIDI_RESPECT_GITIGNORE",
        respect_gitignore,
        || with_env_var_opt("QIDI_DISABLE_ZDR_INCOMPATIBLE_TOOLS", disable_zdr, f),
    )
}
    ...
    with_grok_respect_gitignore(
        ...
        || {
            ...
            assert!(
                ...
                "QIDI_RESPECT_GITIGNORE=false should override config file"
            );
        },
    );
}
...
fn roles_parse_from_toml() {
    let toml_str = r#"
            ...
            prompt_file = ".qidi/prompts/impl.md"
        "#;
    ...
    assert_eq!(implementer.prompt_file.as_deref(), Some(".qidi/prompts/impl.md"));
}
#[test]
...
fn validate_roles_accepts_valid_prompt_file() {
    let toml_str = r#"
            ...
            prompt_file = ".qidi/prompts/ok.md"
        "#;
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
    assert!(cfg.validate_roles().is_empty());
}
...
fn personas_parse_from_toml() {
    ...
    assert_eq!(concise.instructions_file.as_deref(), Some(".qidi/personas/concise.md"));
}
#[test]
...
fn bundled_personas_and_roles_have_lowest_priority_in_resolve_order() {
    ...
    std::fs::write(
        ...
        .unwrap();
    std::fs::write(
            home.join(".qidi/roles/reviewer.toml"),
            r#"description = "User reviewer""#,
        )
        .unwrap();
    std::fs::write(
            home.join(".qidi/personas/reviewer.toml"),
            r#"instructions = "User persona""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".qidi/roles/reviewer.toml"),
            r#"description = "Project reviewer""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".qidi/personas/reviewer.toml"),
    ...
    std::fs::remove_file(workspace.join(".qidi/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(workspace.join(".qidi/personas/reviewer.toml")).unwrap();
    with_env_var(
        "HOME",
        home.to_str().unwrap(),
        || {
    ...
    std::fs::remove_file(home.join(".qidi/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(home.join(".qidi/personas/reviewer.toml")).unwrap();
    with_env_var(
        "HOME",
        home.to_str().unwrap(),
        || {
...
fn render_io_summary_shows_bundled_for_bundled_personas() {
    let persona = SubagentPersona {
        ...
        source_path: Some("/tmp/home/.qidi/bundled/personas/reviewer.toml".to_string()),
        ..Default::default()
    };
...
fn enterprise_two_file_merge_routes_deployment_key_to_proxy() {
    for k in [
        ...
        "QIDI_TRACE_UPLOAD_ENDPOINT_URL",
    ] {
...
fn apply_requirements_value_overrides_user_settings() {
    ...
    assert_eq!("https://managed-api.example/v1", cfg.endpoints.cf_api_base_url);
    assert_eq!(
...
fn validate_hooks_path_rejects_outside_grok_home() {
    ...
    assert!(
        msg.contains("must be under ~/.qidi/"),
        "should mention ~/.qidi/ restriction, got: {msg}"
    );
}
#[test]
...
fn validate_hooks_path_accepts_grok_hooks_subdir() {
    ...
    assert!(result.is_ok(), "path under ~/.qidi/ should be accepted");
}
#[test]
fn managed_settings_disables_features_and_requirements_overrides() {
    use cf_workspace::permission::resolution::ManagedSettingsFeatures;
...
fn simulate_release_build() -> cf_test_support::EnvGuard {
    cf_test_support::EnvGuard::set(cf_version::TEST_VERSION_ENV, "0.0.0-sim")
}
...
/// verdicts untouched). QIDI_HOME-isolated + `#[serial]` for folder-trust
/// store hygiene (empty store ⇒ deterministic untrusted;
/// `EnvGuard` restores QIDI_HOME even on panic). No user-global
/// `$QIDI_HOME/config.toml` is seeded: `grok_home()` is `OnceLock`-cached,
...
/// in `qidi-code`. An ABSOLUTE plugin path is used so the merged
...
/// QIDI_HOME-isolated + `#[serial]` (`EnvGuard` restores it even on panic).
#[test]
#[serial_test::serial]
fn discover_plugins_excludes_untrusted_configpath_plugin_end_to_end() {
    use cf_agent::plugins::{TrustStore, discover_plugins};
    use cf_test_support::EnvGuard;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("QIDI_HOME", home.path());
    let _flag = EnvGuard::unset("QIDI_FOLDER_TRUST");
...
/// arm (store-only reconcile) could never lift. QIDI_HOME-isolated (empty
/// store); QIDI_FOLDER_TRUST unset so the kill-switch is the only signal.
#[test]
#[serial_test::serial]
