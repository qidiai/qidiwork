...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.qidi/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** grok process
    /// sharing the same `~/.qidi` (the writer may also be this process;
...
/// Watches `~/.qidi/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.qidi/config.toml`, `.mcp.json`,
...
/// `<cwd>/.qidi/` (catches `<cwd>/.qidi/config.toml`). Recursing on `<cwd>`
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.qidi/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        // includes `<cwd>/.qidi/config.toml` so the loop above
        // watches `<cwd>/.qidi/`), the call below installs a
    ...
    /// Register `<cwd>/` and `<cwd>/.qidi/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.qidi/config.toml` trigger a
    ...
    /// `<cwd>/.qidi/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.qidi/` does not yet
/// exist at session-open time, the `.qidi/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.qidi/` followed by a
/// write to `<cwd>/.qidi/config.toml` will NOT be observed — the
...
/// expected for a freshly-opened session whose `<cwd>/.qidi/` hasn't
...
/// Watches skill directories (`~/.qidi/skills/`, `<repo>/.qidi/skills/`, etc.)
...
/// recursively: `grok_home` (`~/.qidi`, or `$QIDI_HOME`) or a known vendor dir
/// directly under `$HOME` ([`HOME_VENDOR_DIRS`]).
///
/// These hold large non-skill trees — `~/.qidi` alone has `worktrees/`,
...
/// `~/.qidi` these exist at startup; later creation is caught on restart).
...
impl SkillsFileWatcher {
    ...
    ///
    /// Uses [`collect_skill_config_dirs`](cf_agent::prompt::skills::collect_skill_config_dirs)
    ...
    pub fn start(
        ...
        let dirs_to_watch = cf_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &grok_home,
            config_paths,
...
mod tests {
    ...
    /// `SKILL.md` under a sibling subtree (e.g. `~/.qidi/worktrees/`) must not
    ...
    /// A write to `<cwd>/.qidi/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.qidi/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.qidi/config.toml` writes
