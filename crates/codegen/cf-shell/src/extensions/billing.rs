...
//!
//! Fetches the authenticated user's QIDI Code billing configuration
...
///
/// Keeps history to a count + the most recent period so `~/.qidi/logs/unified.jsonl`
    ...
    fn billing_config_deserializes_credits_config_shape() {
        ...
        let json = serde_json::json!({
            "config": {
                ...
                "productUsage": [
                    {"product": "PRODUCT_QIDI_BUILD", "usagePercent": 61.2}
                ],
                "history": [
                    {
                        "period": {
