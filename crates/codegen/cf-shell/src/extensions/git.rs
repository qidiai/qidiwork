...
async fn resolve_git_root(
    ...
    if let Some(sid) = session_id {
        if let Some(cwd) = agent.get_session_cwd(sid) {
            let result = ops
                .dispatch(
                    &cf_workspace::workspace_ops::GitResolveRootReq { cwd },
                    None,
                )
                .await
                .map_err(|e| {
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        ...
        "x.ai/git/status" => {
            ...
            if let Some(ref git_root) = git_root {
                let current_commit = ops
                    .dispatch(
                        &cf_workspace::workspace_ops::GitCurrentCommitReq {
        ...
        "x.ai/git/checkout_session_head" => {
            ...
            let result = ops
                .dispatch(
                    &cf_workspace::workspace_ops::GitCheckoutCommitReq {
                        git_root: git_root.clone(),
                        head_commit,
