...
use cf_hooks::event::{HookEventEnvelope, HookEventName};
use cf_hooks::matcher::HookMatcher;
use cf_hooks_plugins_types::{HookEvent, HookHandlerType, HookInfo};
...
pub fn hook_spec_to_info(spec: &cf_hooks::config::HookSpec) -> HookInfo {
    ...
    HookInfo {
        ...
        disabled: cf_hooks::trust::is_hook_disabled(&spec.name),
    }
}
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "x.ai/hooks/action" => {
            let req: cf_hooks_plugins_types::HooksActionRequest = super::parse_params(args)?;
            let sid = acp::SessionId::new(req.session_id);
...
mod tests {
    ...
    fn client_hook_dispatch_serializes_envelope() {
        use cf_hooks::event::{HookEventEnvelope, HookPayload};
