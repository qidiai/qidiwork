...
fn load_filtered_marketplace_sources() -> Vec<cf_plugin_marketplace::MarketplaceSource> {
    ...
    let source_names: Vec<String> = sources
        ...
        .map(|s| {
            let url = match &s.kind {
                cf_plugin_marketplace::SourceKind::Git { url, .. } => url.as_str(),
                cf_plugin_marketplace::SourceKind::Local { path } => {
...
async fn handle_action(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let outcome = match req.action {
        MarketplaceAction::Refresh { source_url_or_path } => {
            ...
            for source in &sources {
                if let Some(ref filter) = source_url_or_path {
                    let identity = match &source.kind {
                        ...
                        cf_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
                    };
                    if &identity != filter {
                        continue;
                    }
                }
                if let cf_plugin_marketplace::SourceKind::Git { url, branch } = &source.kind {
                    let cache_root = cf_plugin_marketplace::git::default_cache_root();
                    if let Err(e) = cf_plugin_marketplace::git::force_sync_source_cache(
                        url,
                        branch.as_deref(),
                        &cache_root,
                    ) {
            ...
            cf_hooks_plugins_types::ActionOutcome {
                status: cf_hooks_plugins_types::OutcomeStatus::Success,
                message: msg,
...
async fn handle_update(
    ...
    use cf_plugin_marketplace::installer;
    use cf_hooks_plugins_types::{ActionOutcome, OutcomeStatus};

    let sources = load_filtered_marketplace_sources();

    let source_identity = |s: &cf_plugin_marketplace::MarketplaceSource| -> String {
        match &s.kind {
            cf_plugin_marketplace::SourceKind::Local { path } => path.display().to_string(),
            cf_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
        }
    };
    ...
    let plugin_path =
        match cf_plugin_marketplace::MarketplaceRelativePath::parse(plugin_relative_path) {
            Ok(path) => path,
            Err(e) => {
                return ActionOutcome {
    ...
    let marketplace_root = match &source.kind {
        ...
        cf_plugin_marketplace::SourceKind::Git { url, branch } => {
            let cache_root = cf_plugin_marketplace::git::default_cache_root();
            match cf_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache_root,
                cf_plugin_marketplace::git::SyncMode::Force,
            ) {
                Ok(lease) => {
    ...
    let scan = cf_plugin_marketplace::scan_marketplace(&marketplace_root);
    ...
    let provenance = cf_agent::plugins::install_registry::MarketplaceProvenance {
    ...
    let mut registry = cf_agent::plugins::install_registry::InstallRegistry::load();
    ...
    match update_result {
        Ok(result) => {
            ...
            let reload_outcome = agent
                .execute_plugins_action(sid, cf_hooks_plugins_types::PluginsAction::Reload)
                .await;
            let mut msg = format!(
                "Updated {} ({} -> {})",
                result.repo_key,
        ...
        Err(cf_agent::plugins::install_registry::InstallError::PluginNotFound { .. }) => {
            ActionOutcome {
                status: OutcomeStatus::NotFound,
                message: format!(
...
async fn auto_install_defaults(
    agent: &MvpAgent,
    sources: &[cf_plugin_marketplace::MarketplaceSource],
    ...
    for (source, scan) in sources.iter().zip(results.iter()) {
        ...
        let mut reg = cf_agent::plugins::install_registry::InstallRegistry::load();
        ...
        if let Some((existing_key, _)) = existing {
            ...
            let _ = reg.save();
            reg = cf_agent::plugins::install_registry::InstallRegistry::load();
        }
...
fn scan_source(
    source: &cf_plugin_marketplace::MarketplaceSource,
    ...
    let scan = cf_plugin_marketplace::scan_marketplace(&root);
    let catalog_loaded = scan.catalog_loaded;
    let discovered = scan.entries;
    drop(lease);

    // Cross-reference with install registry.
    let registry = cf_agent::plugins::install_registry::InstallRegistry::load();
    let plugins = discovered
        .into_iter()
        .map(|p| {
            let (install_status, installed_version) =
                match cf_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    &registry,
                    &source_url_or_path,
                    &p.relative_path,
                ) {
...
fn to_plugin_entry(
    p: cf_plugin_marketplace::MarketplaceEntry,
...
/// Add a new git or local-path marketplace source to `~/.qidi/config.toml`.
async fn handle_add_source(url: &str) -> cf_hooks_plugins_types::ActionOutcome {
    ...
    let allowlist =
        &cf_workspace::permission::resolution::managed_settings().marketplace_allowlist;
    ...
    let is_official = matches!(&input, MarketplaceAddInput::GitUrl(u)
        if cf_plugin_marketplace::is_official_source_url(u));
    let name = if is_official {
        cf_plugin_marketplace::OFFICIAL_SOURCE_NAME.to_string()
    } else {
        match &input {
    ...
    let config_path = cf_config::grok_home().join("config.toml");
    let grok_home = cf_config::grok_home();
...
/// Remove a marketplace source from `~/.qidi/config.toml` and uninstall all
/// plugins that were installed from it.
async fn handle_remove_source(source_url_or_path: &str) -> cf_hooks_plugins_types::ActionOutcome {
    ...
    match tokio::task::spawn_blocking(move || remove_source_locked(&src)).await {
        Ok(outcome) => outcome,
        Err(e) => cf_hooks_plugins_types::ActionOutcome {
            status: cf_hooks_plugins_types::OutcomeStatus::InternalError,
            message: format!("Config write task failed: {e}"),
            requires_reload: false,
            requires_restart: false,
        },
...
fn remove_source_locked(source_url_or_path: &str) -> cf_hooks_plugins_types::ActionOutcome {
    use crate::plugin;
    use cf_hooks_plugins_types::{ActionOutcome, OutcomeStatus};
    ...
    let is_official = cf_plugin_marketplace::is_official_source_url(source_url_or_path);
...
pub fn ensure_official_marketplace_source(grok_home: &std::path::Path) {
    ...
    let toml_sources = cf_plugin_marketplace::load_sources(&parsed);
    let json_sources = cf_plugin_marketplace::load_extra_sources_from_settings_in(
        &toml_sources,
        std::slice::from_ref(&grok_home.to_path_buf()),
    );
    let already_present = toml_sources.iter().chain(json_sources.iter()).any(|s| {
        matches!(&s.kind, cf_plugin_marketplace::SourceKind::Git { url, .. }
            if cf_plugin_marketplace::is_official_source_url(url))
    });
    ...
    let write_result = if already_present {
        ...
        add_marketplace_source(
            &config_path,
            cf_plugin_marketplace::OFFICIAL_SOURCE_NAME,
            &crate::plugin::MarketplaceAddInput::GitUrl(
                cf_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.to_string(),
            ),
            true,
        )
    };
    ...
    match write_result {
        Ok(()) if !already_present => {
            tracing::info!(
                url = cf_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
...
mod official_source_tests {
    ...
    fn read_sources(
        config_path: &std::path::Path,
    ) -> Vec<cf_plugin_marketplace::MarketplaceSource> {
        let raw = std::fs::read_to_string(config_path).unwrap_or_default();
        let parsed: toml::Value =
            toml::from_str(&raw).unwrap_or_else(|_| toml::Value::Table(Default::default()));
        cf_plugin_marketplace::load_sources(&parsed)
    }
    ...
    fn add_marketplace_source_local_path_writes_path_entry() {
        ...
        assert!(matches!(
            &sources[0].kind,
            cf_plugin_marketplace::SourceKind::Local { path } if path == &dir
        ));
    ...
    fn removing_last_nonofficial_source_preserves_flag_and_blocks_readd() {
        ...
        assert!(
            !sources.iter().any(|s| matches!(&s.kind,
                cf_plugin_marketplace::SourceKind::Git { url, .. }
                    if cf_plugin_marketplace::is_official_source_url(url))),
    ...
    fn first_run_creates_source_and_sets_flag() {
        ...
        assert!(matches!(
            &sources[0].kind,
            cf_plugin_marketplace::SourceKind::Git { url, .. }
                if url == cf_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL
        ));
    ...
    fn pre_existing_official_source_just_sets_flag() {
        ...
        std::fs::write(
            ...
            format!(
                "[[marketplace.sources]]\nname = \"{}\"\ngit = \"{}\"\n",
                cf_plugin_marketplace::OFFICIAL_SOURCE_NAME,
                cf_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
...
mod conversion_tests {
    ...
    fn to_plugin_entry_carries_homepage_and_keywords() {
        let entry = cf_plugin_marketplace::MarketplaceEntry {
            name: "demo".into(),
            ...
            components: Some(cf_hooks_plugins_types::PluginComponents {
                skills: vec![cf_hooks_plugins_types::ComponentItem::new(
                    "code-review",
