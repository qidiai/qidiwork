...
/// `cf_mcp::wire`), these methods are private to the agent↔client channel and
...
use crate::util::config::mcp::to_acp_mcp_server;
...
pub async fn notify_servers_updated(
    gateway: &cf_acp_lib::AcpAgentGatewaySender,
...
pub async fn build_mcp_status(
    ...
    tool_bridge: &Arc<cf_tools::bridge::ToolBridge>,
    event_writer: Option<&cf_file_utils::events::EventWriter>,
) -> McpStatusSnapshot {
    ...
    for client in &clients {
        ...
        if let Some(ew) = event_writer {
            ew.emit(cf_file_utils::events::Event::McpHealthCheck {
...
pub async fn init_agent_mcp_pool(mcp_state: &Arc<TokioMutex<McpState>>, cwd: &std::path::Path) {
    ...
    let noop = cf_file_utils::events::EventWriter::noop();
...
//
// Implements the `McpResourceProvider` trait from qidi-code so that
...
impl cf_tools::types::resources::McpResourceProvider for McpStateResourceProvider {
    async fn list_resources(
        &self,
        server: Option<String>,
    ) -> Result<Vec<cf_tools::types::resources::McpResourceInfo>, String> {
            ...
            match mcp_service.list_all_resources().await {
                Ok(all_resources) => {
                    for r in all_resources {
                        resources.push(cf_tools::types::resources::McpResourceInfo {
                            uri: r.uri.clone(),
    ...
    async fn read_resource(
        ...
        server: String,
        uri: String,
    ) -> Result<cf_tools::types::resources::McpResourceReadResult, String> {
        let client = {
            let state = self.0.lock().await;
            Arc::clone(
                state
        ...
        match first {
            rmcp::model::ResourceContents::TextResourceContents {
                ...
                text,
                ..
            } => Ok(cf_tools::types::resources::McpResourceReadResult {
                uri: content_uri,
                name: None,
                description: None,
                mime_type,
                content: Some(cf_tools::types::resources::McpResourceContent::Text(
                    text,
                )),
            }),
            rmcp::model::ResourceContents::BlobResourceContents {
                ...
                description: None,
                mime_type,
                content: Some(cf_tools::types::resources::McpResourceContent::Blob(
                    blob.into_bytes(),
                )),
            }),
...
async fn handle_upsert(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let server_config = to_acp_mcp_server(&req.config, &req.server_name)
