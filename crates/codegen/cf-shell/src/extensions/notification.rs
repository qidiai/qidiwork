...
impl PromptUsage {
    ...
    pub fn project_from_ledger(
        ledger: Option<&cf_chat_state::UsageLedger>,
        incomplete: bool,
    ) -> Option<Self> {
...
impl From<&cf_chat_state::UsageTotals> for PromptUsageModel {
    fn from(t: &cf_chat_state::UsageTotals) -> Self {
...
impl From<&cf_chat_state::UsageLedger> for PromptUsage {
    fn from(ledger: &cf_chat_state::UsageLedger) -> Self {
        let mut usage = Self {
...
pub enum SessionUpdate {
    ...
    HooksChanged {
        hooks: Vec<cf_hooks_plugins_types::HookInfo>,
...
pub struct CompactionSegmentFile {
    pub items: Vec<cf_sampling_types::ConversationItem>,
    /// Curated summary, analysis tags already stripped.
    pub summary: String,
    pub detail: cf_chat_state::CompactionDetail,
...
pub struct CompactionRequestFile {
    ...
    /// or `"detailed"` (10-section structured prompt for cf-tools and similar agents).
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub attempt_details: Vec<cf_chat_state::compaction_utils::CompactionAttempt>,
}
...
mod tests {
    ...
    fn compaction_request_file_v2_roundtrips_attempt_details() {
        use cf_chat_state::compaction_utils::CompactionAttempt;
    ...
    fn memory_flush_completed_with_path_roundtrips() {
        let update = SessionUpdate::MemoryFlushCompleted {
            result: "written".into(),
            path: Some("/home/user/.qidi/memory/ws/sessions/log.md".into()),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_dream_completed_roundtrips() {
        let update = SessionUpdate::MemoryDreamCompleted {
            ...
            path: Some("/home/user/.qidi/memory/ws/MEMORY.md".into()),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_session_saved_roundtrips() {
        let update = SessionUpdate::MemorySessionSaved {
            path: "/home/user/.qidi/memory/ws/sessions/2026-01-15-fix-auth-abc12345.md".into(),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_files_variant_roundtrips_through_json() {
        let update = SessionUpdate::MemoryFiles {
            files: vec![
                MemoryFileInfo {
                    path: "/home/user/.qidi/memory/MEMORY.md".into(),
                    source: "global".into(),
                    size_bytes: 1024,
                    modified_epoch_secs: Some(1_700_000_000),
                },
                MemoryFileInfo {
                    path: "/project/.qidi/memory/MEMORY.md".into(),
