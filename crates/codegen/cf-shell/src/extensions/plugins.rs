...
pub fn loaded_plugin_to_info(plugin: &cf_agent::plugins::LoadedPlugin) -> PluginInfo {
    use cf_agent::plugins::discovery::PluginScope as AgentScope;
...
fn origin_to_dto(origin: &cf_agent::plugins::PluginOrigin) -> PluginOrigin {
    use cf_agent::plugins::PluginOrigin as AgentOrigin;
    match origin {
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "x.ai/plugins/action" => {
            let req: cf_hooks_plugins_types::PluginsActionRequest = super::parse_params(args)?;
            let sid = acp::SessionId::new(req.session_id);
...
mod tests {
    use super::*;
    use cf_agent::plugins::PluginOrigin as AgentOrigin;
    use cf_agent::plugins::discovery::{PluginId, PluginScope as AgentScope};

    fn make_loaded_plugin(origin: AgentOrigin) -> cf_agent::plugins::LoadedPlugin {
        let root = std::path::PathBuf::from("/tmp/test-plugin");
        cf_agent::plugins::LoadedPlugin {
