...
mod tests {
    ...
    async fn suggest_var_prefix_lists_expansion_inserts_raw() {
        ...
        let _env = cf_test_support::EnvGuard::set("QIDI_SUGGEST_TEST_DIR", tmp.path());
        let ctx = SuggestContext::new(
            "cat $QIDI_SUGGEST_TEST_DIR/no".into(),
            "cat $QIDI_SUGGEST_TEST_DIR/no".len(),
            "/ignored".into(),
        );
        let results = FilePathProvider.suggest(&ctx).await;
        assert_eq!(results.len(), 1);
        assert_eq!(
            results[0].token_text.as_deref(),
            Some("$QIDI_SUGGEST_TEST_DIR/notes.md")
        );
    }
