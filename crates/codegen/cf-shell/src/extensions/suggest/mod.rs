...
struct SuggestPromptRequest {
    ...
    /// override, or `cf-tools-0.1` when its catalog offers it). One tier
    /// of the shell-side resolution in
    /// `prompt_suggest::effective_suggest_model`: env > config.toml > remote
    /// > this hint > `cf-tools-0.1` default, catalog-guarded (a
...
/// forever. Reasoning models (e.g. `cf-tools`) can take ~30s on a cold
