...
impl PendingDump {
    ...
    pub async fn execute(self) -> DumpAttemptOutcome {
        ...
        let ts_unix = std::time::SystemTime::now()
            ...
            .map(|d| d.as_secs())
            .unwrap_or(0);
        let version = cf_version::installed();
...
fn log_upload_result(heap_object: &str, file_size: u64, ok: bool, err: Option<&str>) -> bool {
    if ok {
        tracing::info!(
            object_path = %heap_object,
            bytes = file_size,
            multipart = file_size > cf_file_utils::gcs::MULTIPART_UPLOAD_THRESHOLD,
...
async fn upload_pair(
    ...
    if let Err(e) = cf_file_utils::gcs::upload_file(&config, heap_object, heap_path, heap_ct).await
    {
        return log_upload_result(heap_object, file_size, false, Some(&e.to_string()));
    }
    match cf_file_utils::gcs::upload_file(&config, meta_object, meta_path, meta_ct).await {
