...
pub struct LoginPolicyReport {
    /// Raw `disable_api_key_auth` knob (env `QIDI_DISABLE_API_KEY_AUTH`).
...
pub struct ConfigSources {
    ...
    /// .qidi/config.toml files). Driven from the same resolvers used at runtime
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    let trust_store = cf_agent::plugins::TrustStore::load();
    ...
    let discovered_plugins = cf_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    let plugin_registry = cf_agent::plugins::PluginRegistry::from_discovered(
    ...
    InspectReport {
        qidi_version: cf_version::VERSION.to_string(),
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let configs = cf_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd.display().to_string(),
        cf_agent::prompt::skills::CompatConfig::default(),
    )
    .await;
    ...
    // `cf_agent::prompt::agents_md::read_agents_config_with_paths`
...
async fn list_permissions(cwd: &Path) -> PermissionsReport {
    use cf_workspace::permission::resolution;
...
fn list_hooks(
    ...
    discovered_plugins: &[cf_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = cf_tools::types::compat::CompatConfig::default();
    ...
    let (registry, _errors) =
        cf_hooks::discovery::load_hooks_from_sources(&global_sources, &project_sources);
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &cf_agent::plugins::PluginRegistry,
    skills_config: &cf_agent::prompt::skills::SkillsConfig,
) -> Vec<SkillEntry> {
    // Discover with all vendors ON so inspect shows the full set.
    let skills = cf_agent::prompt::skills::list_skills_with_plugins(
...
fn skill_entry_source(
    s: &cf_agent::prompt::skills::SkillInfo,
    grok_home: &Path,
) -> ConfigSource {
    use cf_tools::implementations::skills::types::SkillScope;
...
fn list_agents(
    ...
    let agents = cf_agent::discovery::all_subagents_with_plugins(
        cwd,
        &HashMap::new(),
        Some(plugin_registry),
    );
...
fn list_plugins(discovered: &[cf_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                cf_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                cf_agent::plugins::PluginScope::Project => Scope::Project,
                cf_agent::plugins::PluginScope::User => Scope::User,
                cf_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: cf_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    cf_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = cf_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
...
/// `.qidi/config.toml` files (via `find_project_configs`). The macOS MDM
...
fn print_human(r: &InspectReport) {
    println!();
    println!("  Environment");
    println!("  {TREE} Version: {} [{}]", r.qidi_version, r.channel);
    ...
    if r.mcp_servers.is_empty() {
        ...
        println!("  {TREE} (none) \u{2014} 参见 `qidiwork mcp add --help`");
    } else {
        print_columns(
            "MCP Servers",
            &r.mcp_servers,
...
mod tests {
    use super::*;
    use cf_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use cf_tools::implementations::skills::types::SkillScope;
    ...
    fn vendor_rule_paths_select_rules_compatibility_cells() {
        ...
        for path in ["/repo/.qidi/rules/team.md", r"C:\repo\.qidi\rules\team.md"] {
    ...
    fn requirements_layer_contributes_requires_non_empty_post_strip_table() {
        ...
        let path = "/home/u/.qidi/requirements.toml";
    ...
    fn skill_entry_source_maps_scopes() {
        ...
        let s = skill_fixture("b", "/repo/.qidi/skills/b/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("c", "/home/u/.qidi/skills/c/SKILL.md", SkillScope::User);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::User { .. }
        ));
        ...
        let s = skill_fixture(
            "d",
            "/home/u/.qidi/server-skills/d/SKILL.md",
            SkillScope::Server,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        let s = skill_fixture("e", "/home/u/.qidi/bundled/e/SKILL.md", SkillScope::Bundled);
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    fn skill_entry_source_relabels_extracted_bundled_skills() {
        ...
        let s = skill_fixture(
            "help",
            "/home/u/.qidi/skills/help/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        let s = skill_fixture("help", "/repo/.qidi/skills/help/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));
        ...
        let s = skill_fixture(
            "help",
            "/home/u/.qidi/skills/my-tools/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        let s = skill_fixture(
            "my-skill",
            "/home/u/.qidi/skills/my-skill/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    async fn list_skills_honors_skills_config() {
        ...
        // Test-unique names: discovery also reads this machine's real ~/.qidi dirs.
        ...
        let registry = cf_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);
