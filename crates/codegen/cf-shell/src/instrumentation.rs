//! Shim — see `cf_telemetry::instrumentation` for the implementation.
//!
...
//!   (i.e. `cf_shell::instrumentation_timer!`). Keeping the macro here
...
pub use cf_telemetry::instrumentation::{
...
///
/// Stays in shell so callers can keep calling `cf_shell::instrumentation::finalize_and_exit`.
...
///
/// Macro stays in shell so `$crate` continues to resolve to `cf_shell`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `cf_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in
/// `cf_telemetry::instrumentation`.
#[macro_export]
