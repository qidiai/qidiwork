...
use fs2::FileExt;
use cf_workspace::util::is_lock_contended;
...
/// `~/.qidi/leader.sock` — e.g. run a local branch build's leader without
...
/// bypassed entirely.
pub const LEADER_SOCKET_ENV: &str = "QIDI_LEADER_SOCKET";
...
mod tests {
    ...
    fn override_socket_path_wins_over_ws_url_derivation() {
        ...
        assert_eq!(
            ...
            PathBuf::from("/home/u/.qidi/leader-branch.lock")
        );
    }
