...
//! │  └────────────────────────┬────────────────────────────────┘│
//! └───────────────────────────┼──────────────────────────────────┘
//!                             │ IPC (Unix socket at ~/.qidi/leader.sock)
//!         ┌───────────────────┼───────────────────┐
//!         ▼                   ▼                   ▼
//! ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
...
//!
//! ```ignore
//! use cf_shell::leader::{connect_or_spawn, ClientCapabilities, ClientMode};
//!
...
const CLIENT_LEADER_VERSION: &str = cf_version::VERSION;
                ...
                Ok(()) => {
                    ...
                    crate::unified_log::warn(
                        ...
                        Some(serde_json::json!(
                            ...
                            "reason" : reason, "killer_ver" : cf_version::VERSION,
                            }
                        )),
                    );
                }
...
/// (e.g. `~/.qidi/...`) — prefer the managed `~/.qidi/bin/grok` symlink. After an
...
/// `cf_update::auto_update::resolve_restart_exe`.
///
...
/// calling binary.
///
/// Falls back to `~/.qidi/bin/grok` only when `current_exe()` is unavailable.
...
fn spawn_leader_subprocess(env_urls: &LeaderEnvUrls) -> Result<u32, ConnectionError> {
    ...
    let leader_log = std::env::var("QIDI_LEADER_LOG")
        .or_else(|_| std::env::var("RUST_LOG"))
        .unwrap_or_else(|_| "cf_shell=info,cf_acp_lib=warn,cf_mcp=warn".into());
