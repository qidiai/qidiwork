...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    for name in [
        "managed_config.toml",
        "requirements.toml",
        cf_config::signed_policy::SIGNATURE_SIDECAR_FILE,
...
/// Base is 1s; `QIDI_DEPLOYMENT_CONFIG_BACKOFF_MS` overrides it for tests.
fn retry_backoff(attempt: u32) -> std::time::Duration {
    let base = std::env::var("QIDI_DEPLOYMENT_CONFIG_BACKOFF_MS")
        .ok()
...
/// Override with `QIDI_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn managed_config_sync_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("QIDI_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS")
...
/// Resolve deployment key from `QIDI_DEPLOYMENT_KEY` env var, then config files.
...
pub fn is_fetch_enabled() -> bool {
    if let Some(v) = crate::agent::config::env_bool("QIDI_MANAGED_CONFIG") {
        return v;
    }
...
/// Fetch managed config + requirements and write to `~/.qidi/`, trying the
...
fn apply_fetched(
    ...
    let verified = if cf_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(cf_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        cf_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
    }
...
pub fn managed_policy_gate() -> Result<(), String> {
    ...
    // `QIDI_HOME` and would flake on the dev machine's real marker/auth. The pure decision
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    if source.is_some()
        && cf_config::signed_policy::verification_active()
