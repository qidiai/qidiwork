...
use cf_config::signed_policy::now_unix;
...
pub enum ManagedConfigError {
    ...
    #[error(
        "The deployment key was rejected. Confirm that QIDI_DEPLOYMENT_KEY is set correctly and hasn't expired."
    )]
    ...
    SignatureRejected,
    #[error(
        "Can't save the configuration to ~/.qidi. Make sure the directory exists and is writable.\n  ({0})"
    )]
...
pub(super) struct ManagedConfigResponse {
    ...
    #[serde(default)]
    pub(super) signatures: Option<Vec<cf_config::signed_policy::SignatureEnvelope>>,
}
...
impl ManagedConfigResponse {
    ...
    pub(super) fn signature_sidecar(
        &self,
    ) -> Option<cf_config::signed_policy::SignatureEnvelope> {
        self.signature_sidecar_with(cf_config::signed_policy::embedded_key_id_trusted)
    }
...
pub(super) struct VerifiedEnvelope {
    pub(super) sidecar: cf_config::signed_policy::SignatureEnvelope,
    pub(super) payload: cf_config::signed_policy::SignedPayload,
}
...
mod tests {
    ...
    fn signature_sidecar_picks_trusted_envelope_then_falls_back() {
        use cf_config::signed_policy::SignatureEnvelope;
