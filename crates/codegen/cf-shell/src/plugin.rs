...
use cf_agent::plugins::git_install::{self, UpdateStatus};
use cf_agent::plugins::install_registry::{
    InstallError, InstallKind, InstallRegistry, InstalledRepo, MarketplaceProvenance,
};
use cf_plugin_marketplace::git::{self, SourceCacheLease};
use cf_plugin_marketplace::{
...
/// When `keep_data` is true, `~/.qidi/plugin-data/<id>/` is preserved.
...
fn marketplace_root_for_provenance(
    provenance: &cf_agent::plugins::install_registry::MarketplaceProvenance,
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &cf_workspace::permission::resolution::MarketplaceAllowlist,
...
fn plan_install(
    ...
    match qualifier {
        ...
        None => {
            ...
            let scanned: Vec<install_resolve::ScannedEntry<'_>> = owned
                .iter()
...
/// `~/.qidi/` and `~/.claude/`. Returns `true` if removed from at least one file.
pub fn try_remove_source_from_json_files(source_url_or_path: &str) -> bool {
    ...
    // home separately, so removal still runs from $QIDI_HOME when no home dir
...
mod tests {
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> cf_workspace::permission::resolution::MarketplaceAllowlist {
        cf_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = cf_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
