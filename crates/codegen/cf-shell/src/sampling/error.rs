...
//!
//! The canonical error types now live in `cf_sampling_types::error`.
...
pub use cf_sampling_types::error::*;
...
/// This stays in qidi-code because it depends on `agent_client_protocol::Error`.
    ...
    match err {
        ...
        SamplingError::MaxTokensTruncation => {
            acp::Error::internal_error().data(terminal_error_data(
                err.to_string(),
                None,
                cf_sampler::SamplingErrorKind::MaxTokensTruncation,
            ))
        }
...
pub fn terminal_error_data(
    ...
    kind: cf_sampler::SamplingErrorKind,
) -> serde_json::Value {
    if kind != cf_sampler::SamplingErrorKind::MaxTokensTruncation {
...
pub fn stop_reason_for_turn_error(err: &acp::Error) -> &'static str {
    let is_max_tokens = err
        ...
        .and_then(|v| v.as_str())
        .is_some_and(|k| k == cf_sampler::SamplingErrorKind::MaxTokensTruncation.as_str());
...
mod tests {
    ...
    fn attach_prompt_usage_preserves_error_kind_and_round_trips() {
        let mut ledger = cf_chat_state::UsageLedger::default();
        ledger.record_main_loop_call(
            "m",
            &cf_sampling_types::TokenUsage {
                prompt_tokens: 3,
                completion_tokens: 1,
                total_tokens: 4,
                reasoning_tokens: 0,
    ...
    fn with_api_key_env<F: FnOnce()>(key: Option<&str>, f: F) {
        ...
        let prev_legacy = std::env::var("QIDI_CODE_XAI_API_KEY").ok();
        // SAFETY: serial_test ensures no concurrent env mutation.
        unsafe {
            std::env::remove_var("XAI_API_KEY");
            std::env::remove_var("QIDI_CODE_XAI_API_KEY");
            if let Some(k) = key {
        ...
        unsafe {
            ...
            if let Some(v) = prev_legacy {
                std::env::set_var("QIDI_CODE_XAI_API_KEY", v);
            }
        }
        if let Err(e) = result {
    ...
    fn forbidden_subscription_error_includes_api_key_hint_when_env_set() {
        with_api_key_env(Some("xai-test"), || {
            let err = SamplingError::Api {
                status: StatusCode::FORBIDDEN,
                message: "The model 'cf-tools' requires a Grok subscription.".into(),
                model_metadata: None,
                retry_after_secs: None,
                should_retry: None,
            };
