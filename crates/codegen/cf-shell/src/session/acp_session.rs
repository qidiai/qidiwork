...
use cf_agent::prompt::agents_md::LEGACY_AGENTS_MD_REMINDER_PREFIX;
use cf_agent::prompt::skills::SkillsConfig;
use cf_sampler::SamplerConfig as SamplingConfig;
...
use cf_workspace::file_system::CodebaseIndexManager;
use cf_workspace::permission::{
    AccessKind, ClientType, Decision, PermissionEvent, PermissionHandle,
};
use cf_workspace::session::file_state::{FileStateHandle, FileStateTracker};
const SESSION_LOG: &str = "cf_session";
#[path = "compaction.rs"]
mod compaction;
...
impl cf_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
        ...
        cf_tools::types::resources::ManagedGatewayToolCallResponse,
        cf_tool_runtime::ToolError,
    > {
        let auth_key = self
            .auth_manager
            .get_valid_token()
            ...
            .ok_or_else(|| cf_tool_runtime::ToolError::unauthorized("no auth token available"))?;
    ...
    match error {
        crate::session::managed_mcp::ManagedMcpFetchError::Status { status, message } => {
            ...
            let mut err = if status == reqwest::StatusCode::UNAUTHORIZED {
                cf_tool_runtime::ToolError::unauthorized(detail)
            } else if status == reqwest::StatusCode::FORBIDDEN {
                cf_tool_runtime::ToolError::permission_denied(detail)
            } else {
                let tool_id = cf_tool_protocol::ToolId::new(caller)
                    .unwrap_or_else(|_| cf_tool_protocol::ToolId::new("use_tool").expect("valid"));
                cf_tool_runtime::ToolError::execution(tool_id, detail)
            };
        ...
        crate::session::managed_mcp::ManagedMcpFetchError::Transport(e) => {
            cf_tool_runtime::ToolError::network_error(format!(
                "Managed MCP gateway tool call failed: {}",
                e.without_url()
            ))
        }
        crate::session::managed_mcp::ManagedMcpFetchError::NoAuth => {
            cf_tool_runtime::ToolError::unauthorized("no auth token available")
        }
    }
}
#[cfg(test)]
mod managed_gateway_error_tests {
    ...
    fn unauthorized_status_maps_to_unauthorized_and_carries_status() {
        ...
        assert_eq!(err.kind, cf_tool_runtime::ToolErrorKind::Unauthorized);
    ...
    fn forbidden_status_maps_to_permission_denied_and_carries_status() {
        ...
        assert_eq!(err.kind, cf_tool_runtime::ToolErrorKind::PermissionDenied);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    fn general_status_maps_to_execution_with_caller_tool_id() {
        ...
        assert_eq!(err.kind, cf_tool_runtime::ToolErrorKind::Execution);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    async fn transport_error_maps_to_network_error_without_url() {
        ...
        assert_eq!(err.kind, cf_tool_runtime::ToolErrorKind::NetworkError);
...
pub(crate) struct SessionActor {
    ...
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<cf_sampler::SharedAttributionCallback>,
    ...
    pub(crate) chat_state_handle: cf_chat_state::ChatStateHandle,
    ...
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<cf_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<cf_sampling_types::CompactionAtTokens>>,
    ...
    pub(crate) upload_queue:
        std::sync::Arc<std::sync::OnceLock<cf_file_utils::queue::UploadQueue>>,
    ...
    pub(crate) agent: std::cell::RefCell<cf_agent::Agent>,
    ...
    /// Kill-switch (`QIDI_GOAL_USE_CURRENT_MODEL_ONLY` / `[features]
    ...
    pub(crate) mcp_announced_servers:
        Mutex<HashMap<String, cf_tools::implementations::search_tool::ServerFingerprint>>,
    ...
    pub(crate) extension_registry: cf_agent_lifecycle::LocalExtensionRegistry,
    ...
    pub(crate) hook_registry:
        std::cell::RefCell<Option<Arc<cf_hooks::discovery::HookRegistry>>>,
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<cf_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<cf_agent::plugins::SharedPluginRegistryHandle>,
    ...
    pub(crate) observability_bridge: cf_computer_hub_sdk::ObservabilityBridge,
    ...
    pub(crate) sampler_handle: cf_sampler::SamplerHandle,
    /// Cached recipe for constructing this session's [`cf_agent::Agent`].
    ///
    /// Populated once at session spawn and then reused by
    /// `handle_rebuild_agent_for_definition` to build a fresh `Agent`
    /// (system prompt, [`cf_tools::bridge::ToolBridge`], tool
    ...
    pub(crate) workspace_ops: cf_workspace::WorkspaceOps,
...
impl SessionActor {
    ...
    /// Fire-and-forget — failures are logged but do not interrupt the turn.
    async fn send_after_turn_event(&self, payload: cf_tool_protocol::turn_hook::AfterTurnPayload) {
        self.workspace_ops
...
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<cf_tools::bridge::ToolBridge> {
...
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &cf_agent::PromptContext) {
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use cf_tools::types::output::{MCPOutput, ToolOutput};
    use cf_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl cf_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
    ...
    impl cf_tool_runtime::Tool for FixtureMcpTool {
        type Args = serde_json::Value;
        type Output = ToolOutput;
        fn id(&self) -> cf_tool_protocol::ToolId {
            cf_tool_protocol::ToolId::new("server__tool").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::cf_tool_runtime::ListToolsContext,
        ) -> cf_tool_types::ToolDescription {
            cf_tool_types::ToolDescription::new("server__tool", "fixture")
        }
        async fn run(
            &self,
...
mod tool_meta_stamp_tests {
    ...
    use cf_tools::tool_taxonomy::TOOL_META_KEY;
    use cf_workspace::permission::PermissionCommand;
...
mod managed_gateway_tool_tests {
    ...
    use cf_tools::types::output::{MCPOutput, ToolOutput};
    use cf_tools::types::tool::{ToolKind, ToolNamespace};
    use cf_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<cf_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
