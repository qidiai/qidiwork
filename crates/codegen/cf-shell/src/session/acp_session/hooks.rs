...
//! both matched by the agent ([`cf_hooks::matcher::HookMatcher`], shared with
//! file hooks):
...
use cf_hooks::event::{HookEventEnvelope, HookEventName, HookPayload};
use cf_telemetry::events::ClientHookGateOutcome;
...
impl SessionActor {
    ...
    pub(super) async fn deny_tool(
        ...
        cf_telemetry::session_ctx::log_event(cf_telemetry::events::HookBlocked {
    ...
    pub(super) async fn run_pre_tool_use_client_hook(
        ...
        let tool_name = cf_hooks::dispatcher::extract_tool_name(envelope)
            ...
            .map(|(callback_id, timeout)| {
                ...
                async move {
                    ...
                    cf_telemetry::session_ctx::log_event(
                        cf_telemetry::events::ClientHookGate {
...
mod tests {
    ...
    async fn hook_event_active_inert_vs_active() {
        use cf_hooks::event::HookEventName;
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    cf_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.hook_event_active(HookEventName::Stop));
    ...
    fn matching_callback_ids_filters_by_matcher() {
        use cf_hooks::matcher::HookMatcher;
