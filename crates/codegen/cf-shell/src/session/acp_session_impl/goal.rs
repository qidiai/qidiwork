...
impl SessionActor {
    ...
    pub(crate) async fn resolve_goal_role_override(
        ...
        //    silently run cf-tools flavor. Non-strict names (cf-tools
        //    family) run the default flavor and pass; unresolvable names fall
        //    through to the describe `Unknown` arm below.
        if cf_agent::config::is_strict_harness_agent_type(&pair.agent_type)
    ...
    /// agrees with `from_summary` — `search_replace` on the default cf-tools
    /// host, not the literal `write`). The `{TOOLSET_TOOLS}` block is empty on
    /// this path (no per-role toolset enumeration); on the default cf-tools
    ...
    pub(super) async fn prune_prior_goal_continuation_directives(&self) {
        use cf_sampling_types::conversation::SyntheticReason;
