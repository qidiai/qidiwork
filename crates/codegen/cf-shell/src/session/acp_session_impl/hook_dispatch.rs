...
) -> cf_tool_protocol::turn_hook::TurnHookOutcome {
...
) -> cf_tool_protocol::session_event::ToolCallOutcome {
    use crate::session::events::ToolOutcome;
    use cf_tool_protocol::session_event::ToolCallOutcome;
    match outcome {
...
impl SessionActor {
    pub(super) fn hook_run_ctx(&self) -> cf_hooks::runner::RunContext<'_> {
        cf_hooks::runner::RunContext {
    ...
    pub(super) async fn send_hook_execution(
        ...
        results: &[cf_hooks::result::HookRunResult],
    ) {
        if results.is_empty()
            || results
                .iter()
                .all(|r| matches!(r, cf_hooks::result::HookRunResult::Skipped { .. }))
        {
            return;
        }
    ...
    pub(super) async fn dispatch_hook(
        &self,
        event: cf_hooks::event::HookEventName,
        payload: cf_hooks::event::HookPayload,
        prompt_id: Option<&str>,
        tool_name: Option<&str>,
    ) {
        ...
        let results =
            cf_hooks::dispatcher::dispatch_non_blocking(&registry, event, &envelope, &ctx)
                .await;
        ...
        for r in results {
            let (hook_name, elapsed, outcome) = match r {
                cf_hooks::result::HookRunResult::Success {
                    ...
                    hook_name,
                    elapsed,
                    cf_telemetry::events::HookOutcome::Success,
                ),
                cf_hooks::result::HookRunResult::Failed {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    cf_telemetry::events::HookOutcome::Error,
                ),
                cf_hooks::result::HookRunResult::Skipped { .. } => continue,
            };
            cf_telemetry::session_ctx::log_event(cf_telemetry::events::HookExecuted {
...
mod notification_hook_filter_tests {
    ...
    fn task_completed_does_not_fire_via_filter() {
        let update = XaiSessionUpdate::TaskCompleted {
            task_snapshot: cf_tools::types::TaskSnapshot {
                task_id: "task-1".into(),
