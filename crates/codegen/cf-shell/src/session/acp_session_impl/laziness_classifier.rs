        ...
        match item {
            ...
            ConversationItem::User(user) => {
                ...
                for part in &user.content {
                    if let cf_sampling_types::ContentPart::Text { text: t } = part {
                        if !text.is_empty() {
                            text.push(' ');
                        }
                        text.push_str(t);
...
pub(crate) fn build_classifier_turns(
    items: &[ConversationItem],
    max_items: usize,
) -> Vec<cf_workspace::permission::ClassifierTurn> {
        ...
        match item {
            ConversationItem::User(user) => {
                ...
                if !text.is_empty() {
                    ...
                    let text = cf_tools::util::truncate_str_with_marker(
                        &text,
            ...
            ConversationItem::Assistant(assistant) => {
                for tc in &assistant.tool_calls {
                    ...
                    let args = cf_tools::util::truncate_str_with_marker(
                        &args,
                        CLASSIFIER_TURN_MAX_LEN,
                    )
                    .into_owned();
