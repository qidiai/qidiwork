...
impl SessionActor {
            ...
            Err(e) => {
                ...
                if terminal {
                    ...
                    crate::session::telemetry::emit_mcp_connection_span(
                        ...
                        Some(elapsed_ms),
                        None,
                        Some(cf_telemetry::events::McpErrorType::Auth.as_str()),
                    );
    ...
    /// or removed it from `~/.qidi/config.toml` while we were
    /// sleeping.
    ///
    ...
    /// which parses `~/.qidi/config.toml` + the project
    /// `.qidi/config.toml`) on every call. The auto-restart task
    /// calls this at most:
    ...
    pub(crate) async fn respawn_stdio(&self, server: &str) -> Result<(), String> {
        ...
        if let Some(tx) = event_tx {
            ...
            let _ = tx.send(cf_mcp::servers::McpClientEvent::ToolsChanged {
                server: server.to_string(),
            });
        }
        let arc_client = std::sync::Arc::new(new_client);
        let _ = arc_client
            .arm_liveness_watcher(cf_mcp::liveness::DEFAULT_POLL_INTERVAL)
            .await;
        {
            let mut mcp_state = self.mcp_state.lock().await;
            mcp_state
    ...
    pub(super) async fn ensure_mcp_tools_initialized(&self) {
        ...
        if mcp_server_configs.is_empty() && !has_acp {
            ...
            if mcp_state.generation() == generation {
                ...
                self.events
                    .emit(cf_file_utils::events::Event::McpInitCancelled {
        ...
        }
        {
            ...
            let managed_count = mcp_server_configs
                ...
                .count() as u32;
            self.events
                .emit(cf_file_utils::events::Event::McpManagedConfigResult {
        ...
        let mcp_clients: Vec<_> = mcp_results
            ...
            .filter_map(|result| match result {
                ...
                Err(e) => {
                    ...
                    self.events
                        .emit(cf_file_utils::events::Event::McpServerFailed {
                            server_name: sname,
        ...
        tokio::task::spawn_local(async move {
            ...
            for client in mcp_clients.iter() {
                ...
                futs.push(async move {
                    ...
                    ew.emit(cf_file_utils::events::Event::McpServerStarting {
            ...
            > = std::collections::HashMap::new();
            {
                ...
                if mcp_state.generation() != generation {
                    ...
                    event_writer.emit(cf_file_utils::events::Event::McpInitCancelled {
                        reason: MCP_INIT_CANCELLED_CONFIG_CHANGED.to_string(),
                    });
                    return;
                }
                    ...
                    match result {
                        Ok((server_name, registrations, elapsed, timeout_sec)) => {
                            ...
                            for reg in registrations {
                                ...
                                if reg.model_visible {
                                        ...
                                        .await
                                    {
                                        ...
                                        event_writer
                                            .emit(cf_file_utils::events::Event::McpToolRegistrationFailed {
                                                server_name: server_name.clone(),
                            ...
                            let transport_enum = match server_transport_map
                                ...
                                .unwrap_or("unknown")
                            {
                                "stdio" => cf_telemetry::events::McpTransport::Stdio,
                                "sse" => cf_telemetry::events::McpTransport::Sse,
                                _ => cf_telemetry::events::McpTransport::Http,
                            };
                            cf_telemetry::session_ctx::log_event(
                                cf_telemetry::events::McpServerConnected {
                                    server_name: server_name.clone(),
                                    tool_count,
                            ...
                            let transport_str = server_transport_map
                                ...
                                .copied()
                                .unwrap_or("unknown");
                            event_writer.emit(cf_file_utils::events::Event::McpServerConnected {
                                server_name: server_name.clone(),
                        ...
                        Err((server_name, ref e, needs_auth, elapsed, timeout_sec)) => {
                            ...
                            let error_type_label = match error_cat {
                                cf_file_utils::events::McpErrorCategory::AuthRequired => {
                                    cf_telemetry::events::McpErrorType::Auth
                                }
                                cf_file_utils::events::McpErrorCategory::Timeout => {
                                    cf_telemetry::events::McpErrorType::Timeout
                                }
                                _ => cf_telemetry::events::McpErrorType::HandshakeFailed,
                            };
                            cf_telemetry::session_ctx::log_event(
                            ...
                            event_writer.emit(cf_file_utils::events::Event::McpServerFailed {
                                server_name: server_name.clone(),
                            ...
                            let detail = (!needs_auth).then(|| {
                                cf_tools::util::truncate_str_with_marker(&e.to_string(), 200)
                                    .into_owned()
                            });
                ...
                cf_telemetry::session_ctx::log_event(
                    cf_telemetry::events::McpInitCompleted {
                ...
                event_writer.emit(cf_file_utils::events::Event::McpInitCompleted {
    ...
    pub(crate) fn connected_server_summaries(
        &self,
    ) -> Vec<cf_tools::types::tool_index::ServerSummary> {
        use cf_tools::types::tool_index::ToolSearchIndex;
    ...
    pub(super) async fn mcp_announcement_snapshot(&self) -> Option<McpAnnouncementSnapshot> {
        let server_summaries = self.connected_server_summaries();
        let mut text =
            cf_tools::implementations::search_tool::build_server_reminder(&server_summaries)?;
        if let Some(hint) = self.rendered_mcp_hint().await {
            text.push_str(&hint);
        }
