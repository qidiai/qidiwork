...
pub(super) struct MemoryFlushSnapshot {
    counts: cf_chat_state::ConversationCounts,
...
impl SessionActor {
    ...
    pub(super) async fn register_memory_tools(
        &self,
        bridge: &cf_tools::bridge::ToolBridge,
    ) -> Result<(), String> {
        ...
        bridge
            .register_mcp_tools(
                ...
                cf_tools::implementations::memory::search_tool::MemorySearchImpl,
                None,
            )
            .await
        ...
        bridge
            .register_mcp_tools(
                ...
                cf_tools::implementations::memory::get_tool::MemoryGetImpl,
                None,
            )
            .await
    ...
    pub(super) fn emit_memory_session_summary(
        ...
        cf_telemetry::session_ctx::log_event(
            cf_telemetry::memory_telemetry::MemorySessionSummary {
    ...
    pub(super) async fn run_memory_flush(
        ...
        tracing::info!(target: cf_telemetry::memory_log::TARGET, "MEMORY_FLUSH: starting");
        ...
        let result = async {
            ...
            cf_telemetry::session_ctx::log_event(
                cf_telemetry::memory_telemetry::MemoryFlushStart {
                    session_id: self.session_info.id.to_string(),
        ...
        tracing::info!(target: cf_telemetry::memory_log::TARGET, outcome = %outcome, "MEMORY_FLUSH: completed");
        ...
        cf_telemetry::session_ctx::log_event(
            cf_telemetry::memory_telemetry::MemoryFlushComplete {
                session_id: self.session_info.id.to_string(),
                trigger: trigger.to_owned(),
        ...
        let flush_trigger = match trigger {
            "slash_command" => cf_telemetry::events::MemoryFlushTrigger::SlashCommand,
            "interval" => cf_telemetry::events::MemoryFlushTrigger::Interval,
            "pre_compaction" => cf_telemetry::events::MemoryFlushTrigger::PreCompaction,
            _ => cf_telemetry::events::MemoryFlushTrigger::UserRequested,
        };
        cf_telemetry::session_ctx::log_event(cf_telemetry::events::MemoryFlushed {
    ...
    pub(super) async fn snapshot_memory_flush_state(&self) -> MemoryFlushSnapshot {
        ...
        let chat_history = crate::sampling::conversation_to_chat_messages(
            cf_chat_state::compaction_utils::prepare_conversation_for_summarization(conversation),
        );
        MemoryFlushSnapshot {
            counts,
            chat_history,
    ...
    /// LLM call using the `cf-tools` model.
    ///
    ...
    pub(super) async fn handle_rewrite_memory_note(
        ...
        let result = match sampling_client.api_backend() {
            crate::sampling::ApiBackend::ChatCompletions => {
                ...
                let events =
                    cf_sampler::stream_chat_completions(raw, meta, request_id, idle_timeout);
                cf_sampler::collect_response(events).await
            }
            crate::sampling::ApiBackend::Responses => {
                let (raw, meta, doom_loop) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = cf_sampler::stream_responses(
                    raw,
                    meta,
                    request_id,
                    idle_timeout,
            ...
            crate::sampling::ApiBackend::Messages => {
                let (raw, meta) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = cf_sampler::stream_messages(raw, meta, request_id, idle_timeout);
                cf_sampler::collect_response(events).await
            }
        };
