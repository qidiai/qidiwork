...
impl SessionActor {
    pub(super) async fn handle_set_session_model(
        ...
        self.chat_state_handle
            .update_sampling_config(cf_sampling_types::SamplingConfig {
                ...
                if let ConversationItem::System(sys) = item {
                    if use_concise {
                        sys.content = std::sync::Arc::<str>::from(
                            cf_agent::prompt::template::COMPACT_SYSTEM_PROMPT,
                        );
                    } else {
                        sys.content =
    ...
    ///
    /// Builds a fresh [`cf_agent::Agent`] from the cached
    /// [`crate::session::agent_rebuild::AgentRebuildSpec`] + the supplied
    /// [`cf_agent::AgentDefinition`], replaces `self.agent`,
