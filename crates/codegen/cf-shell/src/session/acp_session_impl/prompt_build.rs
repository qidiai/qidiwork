...
use super::*;
/// Partition the AGENTS.md / Claude.md / `.qidi/rules/*.md` files returned
/// by `read_agents_config_with_paths` into "workspace" (cwd / repo root /
/// extra workspace user dir) and "user" (`~/.qidi/`, `~/.claude/`,
/// `~/.qidi/bundled/`) buckets, mirroring the split between
...
fn partition_rules_by_scope(
    files: Vec<cf_agent::prompt::agents_md::AgentConfigFile>,
) -> (
    Vec<cf_agent::prompt::user_message::RuleEntry>,
    Vec<cf_agent::prompt::user_message::RuleEntry>,
) {
    let home = dirs::home_dir().map(|p| p.to_string_lossy().to_string());
    let user_prefixes: Vec<String> = match home {
        Some(h) => vec![format!("{h}/.qidi/"), format!("{h}/.claude/")],
        None => vec![],
    };
    let mut workspace = Vec::new();
    let mut user = Vec::new();
    for f in files {
        let entry = cf_agent::prompt::user_message::RuleEntry::from(f);
...
impl SessionActor {
        ...
        if drop_startup_skill_reminder {
            conversation.retain(|item| {
                !matches!(
                    ...
                    Some(cf_sampling_types::SyntheticReason::SystemReminder)
                )
            });
        }
    }
    ...
    async fn build_templated_user_message(
        &self,
        cwd: &std::path::Path,
        template: cf_agent::prompt::user_message::UserMessageTemplate,
    ) -> Option<String> {
        use cf_agent::prompt::agents_md::read_agents_config_with_paths;
        use cf_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use cf_workspace::file_system::{git_status_short, jj_status};
        use cf_workspace::session::git::VcsKind;
    ...
    ) -> Vec<cf_agent::prompt::user_message::McpServerEntry> {
        use cf_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let current_query = crate::session::image_describe::strip_template_context_tags(
            &cf_chat_state::compaction_utils::extract_user_query(&original_user_message),
        );
        ...
        let client = cf_sampler::SamplingClient::new(sampler_config).map_err(|e| {
