...
impl SessionActor {
    ...
    pub(super) async fn handle_side_question(&self, question: &str) -> Result<String, String> {
        ...
        let mut items: Vec<ConversationItem> =
            cf_chat_state::compaction_utils::strip_reasoning_blocks(
    ...
    /// (`QIDI_PROMPT_SUGGESTIONS_MODEL`) > `[models] prompt_suggestion`
    /// (config.toml) > remote `prompt_suggestion_model` (remote settings) >
    /// (config.toml) > remote `prompt_suggestion_model` (remote settings) >
    /// [`prompt_suggest::DEFAULT_SUGGEST_MODEL`] (`cf-tools-0.1`). Every
    ...
    /// `cf-tools-0.1` for OAuth users) the request is **skipped
    ...
    /// a reasoning model's response, and some models (e.g. `cf-tools`)
    ...
    pub(super) async fn handle_suggest_prompt(
        ...
        tracing::debug!(
            raw_preview = %cf_tools::util::truncate_str(raw.trim(), 60),
