...
pub(crate) fn resolve_reminder_policy(
    ...
    todo_gate: bool,
) -> cf_agent::ReminderPolicy {
    let mut policy = cf_agent::ReminderPolicy::default();
...
pub(super) fn todo_gate_active(
    policy: &cf_agent::system_reminder::ReminderPolicy,
    audience: cf_agent::prompt::context::PromptAudience,
...
impl SessionActor {
    ...
    /// in `cf_tools::reminders`.
    pub(super) fn reminder_wrapper_tag(&self) -> &'static str {
        cf_tools::reminders::DEFAULT_REMINDER_TAG
    }
    ...
    pub(super) async fn drain_between_turn_completions(&self) {
        ...
        if !bash_completions.is_empty() {
            ...
            if goal_loop_active {
                ...
                let task_output_name =
                    ...
                    .await;
                let read_tool_name =
                    cf_tools::reminders::task_completion::resolve_read_tool_name(&bridge)
                        .await;
                let reminder = cf_tools::reminders::task_completion::format_between_turn_bash_completions(
                    &bash_completions,
        ...
        let reminder =
            cf_tools::reminders::task_completion::format_between_turn_completion_reminder(
                &completions,
                &bridge,
            )
            .await;
    ...
    pub(super) fn todo_gate_policy(
        &self,
    ) -> Option<cf_agent::system_reminder::TodoGateConfig> {
    ...
    pub(super) async fn collect_todo_gate_input(&self, prompt_id: &str) -> CollectedTodoGateInput {
        ...
        let incomplete_terminal_tasks = bridge
            ...
            .await
            .into_iter()
            .filter(cf_tools::computer::types::TaskSnapshot::is_outstanding)
            .count();
