...
impl SessionActor {
    ...
    pub(super) async fn get_rewind_points(&self) -> RewindPointsResponse {
        ...
        let file_meta_map: std::collections::HashMap<
            usize,
            &cf_workspace::session::file_state::RewindPointMeta,
    ...
    pub(super) async fn handle_rewind(
        ...
        let mut files_to_revert: std::collections::HashMap<
            cf_workspace::session::file_state::FlexiblePath,
            Option<String>,
    ...
    /// session: run `cf_chat_state::compaction_utils::repair_history` inside
    ...
    pub(super) async fn handle_repair_history(
        &self,
        dry_run: bool,
    ) -> anyhow::Result<cf_chat_state::compaction_utils::HistoryRepairReport> {
        ...
        if turn_flag.load(std::sync::atomic::Ordering::SeqCst) {
            anyhow::bail!(cf_chat_state::commands::RepairHistoryBlocked);
        }
