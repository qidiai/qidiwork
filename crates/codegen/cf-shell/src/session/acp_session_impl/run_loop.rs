...
pub(super) async fn run_session(
    ...
    mut chat_state_event_rx: mpsc::UnboundedReceiver<cf_chat_state::ChatStateEvent>,
    ...
    if !session.startup_hints.is_subagent && liveness_watchers_enabled {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<cf_mcp::servers::McpClientEvent>();
        {
    ...
    loop {
        tokio::select! {
            ...
            cf_telemetry::memory_log::TARGET,
            ...
            } else { tracing::debug!(target : cf_telemetry::memory_log::TARGET,
            ...
            cf_telemetry::memory_log::TARGET, "MEMORY_DREAM_CHECK: timer fired");
            ...
            Some(cf_chat_state::ChatStateEvent::ConversationReset { new_len }) => {
            ...
            Some(cf_chat_state::ChatStateEvent::ImageBudget { body_bytes, trigger_bytes,
            reclaim_target_bytes, inline_images, needs_image_compaction, evicted,
            body_bytes_after, }) => {
            cf_telemetry::unified_log::info("shell.image_budget", Some(session
            ...
            })),); } Some(cf_chat_state::ChatStateEvent::PromptIndexChanged { .. }) |
            Some(cf_chat_state::ChatStateEvent::TokensUpdated { .. }) => {} None => {} }
            ...
            .fire_hook(cf_hooks::event::HookEventName::SessionEnd, None,
            cf_hooks::event::HookPayload::SessionEnd { reason : "channel_closed"
            ...
            cf_hooks::dispatcher::dispatch_non_blocking(& registry,
            cf_hooks::event::HookEventName::SessionEnd, & envelope, & ctx,). await;
            session.send_hook_execution("session_end", None, None, & results). await; }
            let envelope = session.fire_hook(cf_hooks::event::HookEventName::Stop,
            None, cf_hooks::event::HookPayload::Stop { reason : "channel_closed"
            .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
            .clone() { let ctx = session.hook_run_ctx(); let results =
            cf_hooks::dispatcher::dispatch_non_blocking(& registry,
            cf_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
            ...
            cf_telemetry::memory_log::TARGET, result = ? result, tool_searches =
            ...
            cf_telemetry::session_ctx::log_event(cf_telemetry::events::SessionEnded
            ...
            cf_file_utils::trace_context::link_current_span_to_meta(& meta); } let
            ...
            .update_credentials(cf_chat_state::Credentials { api_key : r.api_key,
            ...
            .send(cf_hooks_plugins_types::HooksListResponse { hooks, project_trusted,
            ...
            cf_agent::prompt::skills::list_skills_with_plugins(Some(& cwd), &
            ...
            .send(cf_mcp::servers::McpClientEvent::ConfigDiff { added : diff.added
            ...
            .emit(cf_file_utils::events::Event::McpServerToggled { server_name :
            ...
            session.fire_hook(cf_hooks::event::HookEventName::SessionStart, None,
            cf_hooks::event::HookPayload::SessionStart { source, model_id : None,
            agent_type : None, },); if let Some(registry) = session.hook_registry
            .borrow().clone() { let ctx = session.hook_run_ctx(); let results =
            cf_hooks::dispatcher::dispatch_non_blocking(& registry,
            cf_hooks::event::HookEventName::SessionStart, & envelope, & ctx,).
            ...
            .fire_hook(cf_hooks::event::HookEventName::SessionEnd, None,
            cf_hooks::event::HookPayload::SessionEnd { reason : "shutdown"
            .to_string(), turn_count : None, tool_call_count : None, },); if let
            Some(registry) = session.hook_registry.borrow().clone() { let ctx = session
            .hook_run_ctx(); let results =
            cf_hooks::dispatcher::dispatch_non_blocking(& registry,
            ...
            session.send_hook_execution("session_end", None, None, & results). await; }
            let envelope = session.fire_hook(cf_hooks::event::HookEventName::Stop,
            None, cf_hooks::event::HookPayload::Stop { reason : "shutdown"
            .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
            .clone() { let ctx = session.hook_run_ctx(); let results =
            cf_hooks::dispatcher::dispatch_non_blocking(& registry,
            cf_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[cf_sampling_types::ConversationItem],
    turn_number: usize,
    ...
    let extracted = cf_chat_state::compaction_utils::extract_user_query(&raw);
