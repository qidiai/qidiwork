...
pub(super) fn is_auth_tool_error(err: &cf_tool_runtime::ToolError) -> bool {
...
pub(super) async fn call_with_auth_retry<F, Fut>(
    ...
    tool_name: &str,
    mut call: F,
) -> Result<cf_tools::types::output::ToolRunResult, cf_tool_runtime::ToolError>
where
    F: FnMut() -> Fut,
    Fut: std::future::Future<
            Output = Result<
                cf_tools::types::output::ToolRunResult,
...
impl SessionActor {
    ...
    pub(super) async fn reconstruct_full_config(&self) -> SamplingConfig {
        ...
        impl cf_sampler::HeaderInjector for TraceContextInjector {
            fn inject(&self, headers: &mut reqwest::header::HeaderMap) {
                if let Some(tp) = cf_file_utils::trace_context::current_traceparent()
        ...
        impl cf_sampler::BearerResolver for AuthManagerBearerResolver {
        ...
        let cfg = self
            ...
            .get_sampling_config()
            .await
            .unwrap_or_else(|| cf_sampling_types::SamplingConfig {
                base_url: String::new(),
                model: String::new(),
        ...
        SamplingConfig {
            ...
            bearer_resolver: if use_bearer_resolver {
                self.auth_manager
                    .as_ref()
                    .map(|am| -> cf_sampler::SharedBearerResolver {
    ...
    pub(crate) async fn wire_permission_auto_llm_classifier(self: &Arc<Self>) {
        ...
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel::<(
            Vec<cf_workspace::permission::ClassifierMessage>,
            ...
            while let Some((messages, respond_to)) = rx.recv().await {
                let result = async {
                    ...
                    let items = messages
                        .into_iter()
                        .map(|m| match m.role {
                            cf_workspace::permission::ClassifierMessageRole::System => {
                                ConversationItem::system(m.text)
                            }
                            cf_workspace::permission::ClassifierMessageRole::User => {
                    ...
                    let request = ConversationRequest {
                        ...
                        json_schema: Some(
                            cf_workspace::permission::classifier_output_json_schema(),
                        ),
        ...
        let clf =
            cf_workspace::permission::LlmPermissionClassifier::with_channel(tx, prompt_type);
        debug_assert!(
            clf.has_side_query(),
    ...
    pub(super) async fn resolve_aux_sampler_config(
        &self,
        slug: &str,
    ) -> Option<cf_sampler::SamplerConfig> {
        let creds = self.chat_state_handle.get_credentials().await;
        let session_key = self
            .auth_manager
            .as_ref()
    ...
    async fn resolve_auto_classifier_sampler(
        &self,
        slug: &str,
    ) -> Option<(cf_sampler::SamplingClient, String)> {
        ...
        let client = cf_sampler::SamplingClient::new(cfg)
            .map_err(|e| {
                tracing::warn!(
                    error = % e,
    ...
    pub(super) async fn prepare_chat_completion(
        &self,
        force_http1: bool,
    ) -> Result<cf_sampler::SamplingClient, acp::Error> {
        self.refresh_token_if_expired().await;
        let mut full_config = self.reconstruct_full_config().await;
        full_config.force_http1 = force_http1;
        let sampling_client =
            cf_sampler::SamplingClient::new(full_config).map_err(|e| self.to_acp_error(e))?;
        Ok(sampling_client)
    }
    ...
    pub(crate) async fn handle_sampling_failure(
        self: &Arc<Self>,
        error: cf_sampler::SamplingErrorInfo,
    ) -> Result<SamplerFailureRecovery, acp::Error> {
        use cf_sampler::SamplingErrorKind;
        ...
        let client_version = cf_version::VERSION;
        ...
        let error_type = if cf_sampling_types::is_context_length_error(&error.message) {
            "context_length"
        } else {
            error.kind.as_str()
        };
    ...
    pub(crate) async fn run_turn_via_sampler(
        ...
        let request_id = cf_sampler::RequestId::random();
            ...
            .await
        {
            ...
            Err(rich_err) => {
                self.turn_stream_drained.lock().take();
                let info = cf_sampler::SamplingErrorInfo::from(&rich_err);
