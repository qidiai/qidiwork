...
impl SessionActor {
    ...
    ) -> Option<cf_tools::types::skill_discovery_tracker::SkillUpdateEffects> {
        let bridge = self.agent.borrow().tool_bridge().clone();
    ...
    pub(super) async fn reload_skills_from_disk(&self) -> usize {
        ...
        let new_skills = cf_agent::prompt::skills::list_skills_with_plugins(
            Some(cwd),
            &skills_config,
    ...
    pub(super) fn wrap_skill_reminder(
        &self,
        effects: &cf_tools::types::skill_discovery_tracker::SkillUpdateEffects,
    ) -> Option<ConversationItem> {
        use cf_tools::types::skill_discovery_tracker::SkillUpdateKind;
        let is_cursor = self.is_cursor_harness();
    ...
    pub(super) async fn maybe_refresh_model_metadata_on_resume(&self) {
        ...
        let _ = am.auth().await;
        let provider: Arc<dyn cf_auth::AuthCredentialProvider> = Arc::new(
    ...
    pub(super) async fn handle_model_metadata_update(
        ...
        let updated_config = cf_sampling_types::SamplingConfig {
    ...
    pub(super) async fn inject_deny_read_globs(&self) {
        ...
        self.agent
            .borrow()
            .tool_bridge()
            .update_resource(cf_tools::types::resources::DenyReadGlobs(
    ...
    pub(super) async fn build_session_info(&self) -> SessionInfoData {
        ...
        let system_prompt_tokens = system_message
            .as_ref()
            .map(cf_chat_state::estimate_system_message_tokens)
            .unwrap_or(0);
        ...
        let free_tokens = cf_token_estimation::free_tokens(context_window, total_tokens);
        let usage_pct = cf_token_estimation::usage_percentage_u8(total_tokens, context_window);
