...
impl SessionActor {
    ...
    pub(super) async fn execute_builtin_slash_command(
        ...
        cf_telemetry::session_ctx::log_event(cf_telemetry::events::SlashCommandUsed {
        ...
        match action {
            ...
            BuiltinAction::SetYolo { enabled } => {
                ...
                if let Some(actual) = yolo_toggle_report(was, actual) {
                    ...
                    cf_telemetry::session_ctx::log_event(
                        cf_telemetry::events::YoloToggled {
                            enabled: actual,
                            previous_state: was,
                            trigger: cf_telemetry::events::YoloTrigger::SlashCommand,
                        },
                    );
                    tracing::info_span!(
            ...
            BuiltinAction::HooksTrust => {
                let msg = match Self::do_hooks_trust_project(&self.session_info.cwd) {
                    Ok(root) => {
                        cf_telemetry::session_ctx::log_event(
                            cf_telemetry::events::HookTrusted { success: true },
                        );
                        format!("Trusted: {}.", root.display())
                    }
                    Err(e) => {
                        cf_telemetry::session_ctx::log_event(
                            cf_telemetry::events::HookTrusted { success: false },
                        );
                        e
                    }
                };
            ...
            BuiltinAction::HooksAdd { path } => {
                if path.is_empty() {
                    self.send_slash_command_output(
                        "Usage: /hooks add <path>\nProvide a path to a hook JSON file or directory under ~/.qidi/.",
                    )
                    .await;
                } else {
                    // CWE-427: Use shared add_hooks_path() which validates
                    // paths are under ~/.qidi/ to prevent hook path injection.
                    match crate::config::add_hooks_path(&path) {
                        Ok(()) => {
                            cf_telemetry::session_ctx::log_event(
                                cf_telemetry::events::HookAdded { success: true },
                            );
                        ...
                        Err(e) => {
                            cf_telemetry::session_ctx::log_event(
                                cf_telemetry::events::HookAdded { success: false },
                            );
                            self.send_slash_command_output(&format!(
            ...
            BuiltinAction::HooksRemove { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::remove_hooks_path(&path) {
                        Ok(()) => {
                            cf_telemetry::session_ctx::log_event(
                                cf_telemetry::events::HookRemoved { success: true },
                            );
                            self.send_slash_command_output(&format!(
                        ...
                        Err(e) => {
                            cf_telemetry::session_ctx::log_event(
                                cf_telemetry::events::HookRemoved { success: false },
                            );
                            self.send_slash_command_output(&format!(
            ...
            BuiltinAction::PluginsReload => {
                match &self.plugin_registry_handle {
                    Some(handle) => {
                        ...
                        cf_telemetry::session_ctx::log_event(
                            cf_telemetry::events::PluginReloaded { success: true },
                        );
                        self.send_slash_command_output(&msg).await;
                    }
                    None => {
                        cf_telemetry::session_ctx::log_event(
                            cf_telemetry::events::PluginReloaded { success: false },
                        );
                        self.send_slash_command_output(
            ...
            BuiltinAction::SessionInfo => {
                ...
                let context_pct = cf_token_estimation::usage_percentage(ctx.used, ctx.total);
            ...
            BuiltinAction::PluginsAdd { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::add_plugin_path(&path_str) {
                        Ok(()) => {
                            cf_telemetry::session_ctx::log_event(
                                cf_telemetry::events::PluginAdded {
                                    source: cf_telemetry::events::PluginSource::LocalPath,
                                    success: true,
                                },
                            );
            ...
            BuiltinAction::PluginsRemove { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::remove_plugin_path(&path_str) {
                        Ok(()) => {
                            cf_telemetry::session_ctx::log_event(
                                cf_telemetry::events::PluginRemoved { success: true },
                            );
                        ...
                        Err(e) => {
                            cf_telemetry::session_ctx::log_event(
                                cf_telemetry::events::PluginRemoved { success: false },
                            );
                            self.send_slash_command_output(&format!(
            ...
            BuiltinAction::PluginsInstall { source, trust } => {
                if source.is_empty() {
                    ...
                    if !trust {
                        let install_source =
                            cf_agent::plugins::git_install::parse_install_source(
                                &source, cwd,
                            );
                        let source_desc = match &install_source {
                            cf_agent::plugins::git_install::InstallSource::Git {
                                url,
                                ..
                            } => {
                                format!("remote git repo: {url}")
                            }
                            cf_agent::plugins::git_install::InstallSource::Local {
                                path,
                                ..
                            } => {
                        ...
                        match crate::plugin::install_plugin(&source, cwd) {
                            Ok(outcome) => {
                                ...
                                cf_telemetry::session_ctx::log_event(
                                    cf_telemetry::events::PluginInstalled {
                                        install_kind: kind,
                                        success: true,
                                        trust: true,
                                        error_category: None,
                        ...
                        if let Err(e) = storage.ensure_initialized() {
                            ...
                            *self.memory.search_counter.borrow_mut() =
                            ...
                            let backend: std::sync::Arc<
                                dyn cf_tools::types::memory_backend::MemoryBackend,
