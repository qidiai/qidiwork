...
fn drop_cli_catchall_allows(
    rules: Vec<cf_workspace::permission::types::PermissionRule>,
    policy_block: Option<&'static str>,
) -> (
    Vec<cf_workspace::permission::types::PermissionRule>,
    Vec<cf_workspace::permission::types::PermissionRule>,
) {
    ...
    for rule in rules {
        if cf_workspace::permission::resolution::is_catchall_allow(&rule) {
            dropped.push(rule);
        } else {
            kept.push(rule);
        }
...
mod cli_catchall_drop_tests {
    ...
    use cf_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    use cf_workspace::permission::rules::parse_permission_rule;
    use cf_workspace::permission::types::{PermissionRule, RuleAction, ToolFilter};
...
pub(crate) async fn spawn_session_actor(
    ...
    credentials: cf_chat_state::Credentials,
    auth_method_id: crate::agent::auth_method::SharedAuthMethodId,
    auth_manager: Option<Arc<AuthManager>>,
    attribution_callback: Option<cf_sampler::SharedAttributionCallback>,
    ...
    compaction_mode: cf_chat_state::CompactionMode,
    ...
    preloaded_skills: Option<Vec<cf_tools::implementations::skills::types::SkillInfo>>,
    compat: CompatConfig,
    ...
    prompt_audience: cf_agent::prompt::context::PromptAudience,
    ...
    plugin_registry: Option<std::sync::Arc<cf_agent::plugins::PluginRegistry>>,
    plugin_registry_handle: Option<cf_agent::plugins::SharedPluginRegistryHandle>,
    models_manager: crate::agent::models::ModelsManager,
    inherited_permission_handle: Option<cf_workspace::permission::PermissionHandle>,
    api_key_provider: Option<cf_tools::types::SharedApiKeyProvider>,
    image_description_model: String,
    hook_registry_override: Option<std::sync::Arc<cf_hooks::discovery::HookRegistry>>,
    workspace_ops: cf_workspace::WorkspaceOps,
    cli_permission_rules: Vec<cf_workspace::permission::types::PermissionRule>,
    todo_gate: bool,
    ...
    cf_agent::AgentBuildError,
> {
    if max_turns == Some(0) {
        return Err(cf_agent::AgentBuildError::InvalidConfig(
        ...
        inherited_permission_handle
    {
        ...
        let mut permission_config =
            cf_workspace::permission::resolution::resolve_permission_config_with_fallback(
                tool_context.cwd.as_path(),
            )
            .await;
        let yolo_pin = cf_workspace::permission::resolution::yolo_disabled_by_policy();
            ...
            match &mut permission_config {
                ...
                None => {
                    permission_config = Some(
                        cf_workspace::permission::types::PermissionConfig::new(
        ...
        let deny_read_globs = permission_config
            .as_ref()
            .map(cf_workspace::permission::resolution::deny_read_globs_from_config)
            .unwrap_or_default();
        let hub_permission = if cf_workspace::permission::hitl_permission_live_enabled() {
            ...
            let transport = server
                .and_then(|server| {
                    cf_workspace::permission::ToolServerPermissionTransport::from_session_id(
                        server,
                ...
                .map(|t| {
                    std::sync::Arc::new(t)
                        as std::sync::Arc<
                            dyn cf_workspace::permission::PermissionHookTransport,
                        >
                });
        ...
        let (permissions, permission_events_rx) =
            cf_workspace::permission::spawn_permission_manager_with_hub(
    ...
    let context_window_override = std::env::var("QIDI_DEBUG_CONTEXT_WINDOW")
        .ok()
    ...
    if let Some(cw) = context_window_override {
        tracing::warn!(
            ...
            "QIDI_DEBUG_CONTEXT_WINDOW override active"
        );
    }
    let chat_state_sampling_config = cf_sampling_types::SamplingConfig {
    ...
    let actor_pruning_config = cf_chat_state::PruningConfig {
    ...
    let chat_state_handle = cf_chat_state::ChatStateActor::spawn_with_pruning(
        conversation.clone(),
    ...
    let auto_wake_delivered =
        cf_tools::reminders::task_completion::AutoWakeDeliveredIds::default();
    ...
    *synthetic_trace_tx_shared.lock().unwrap() = tool_context.synthetic_trace_tx.clone();
    ...
    let index_root_for_session =
        cf_workspace::session::git::find_git_root_from_path(tool_context.cwd.as_path())
    ...
    let resolve_search_shadows = || {
        ...
        cf_tools::computer::local::SearchShadowConfig {
            find_bfs,
            grep_ugrep,
        }
    };
    ...
    let terminal_backend: std::sync::Arc<dyn cf_tools::computer::types::TerminalBackend> =
        match terminal_backend_kind {
            ...
            TerminalBackendKind::AcpClient => {
                std::sync::Arc::new(crate::terminal::AcpTerminalAdapter::new(
                    ...
                    as std::sync::Arc<dyn cf_tools::computer::types::TerminalBackend>
            }
    ...
    let fs_backend: std::sync::Arc<dyn cf_tools::computer::types::AsyncFileSystem> =
        if client_fs_capable && tool_context.gateway.is_some() {
            std::sync::Arc::new(cf_workspace::file_system::AcpFsAdapter::new(
                tool_context.gateway.clone().unwrap(),
                tool_context.session_id.clone().unwrap(),
            ))
        } else {
    ...
    let harness_metrics = if telemetry_enabled || cf_telemetry::external::is_active() {
        ...
        Some(super::telemetry::SessionHarnessMetrics {
            ...
            permission_mode: if session_yolo_mode {
                cf_telemetry::enums::PermissionMode::AlwaysApprove
            } else if session_auto_mode
                && crate::util::config::auto_permission_mode_enabled_from_disk()
            {
                cf_telemetry::enums::PermissionMode::Auto
            } else {
                cf_telemetry::enums::PermissionMode::Ask
            },
    ...
    let compaction_policy = cf_agent::CompactionPolicy {
    ...
    let memory_backend_for_spec: Option<
        std::sync::Arc<dyn cf_tools::types::memory_backend::MemoryBackend>,
    > = if let Some(ref storage) = memory_storage_for_session {
        if let Err(e) = storage.ensure_initialized() {
            tracing::warn!(
                target : cf_telemetry::memory_log::TARGET, error = % e,
        ...
        }
        {
            ...
            tokio::task::spawn_blocking(move || match gc_storage.gc(gc_max_age) {
                Ok(removed) if removed > 0 => {
                    tracing::info!(
                        target : cf_telemetry::memory_log::TARGET, removed,
        ...
        let params = crate::session::memory::MemoryBackendParams {
            ...
            auth_credentials: auth_manager.as_ref().map(|am| {
                ...
                ) as std::sync::Arc<dyn cf_auth::AuthCredentialProvider>
            }),
        };
        ...
        let backend: std::sync::Arc<dyn cf_tools::types::memory_backend::MemoryBackend> =
        ...
        tracing::info!(
            target : cf_telemetry::memory_log::TARGET, workspace = % storage
        ...
        cf_telemetry::session_ctx::log_event(
            cf_telemetry::memory_telemetry::MemorySessionInit {
                session_id: session_info.id.to_string(),
                memory_enabled: true,
    ...
    let managed_gateway_tool_client = auth_manager.as_ref().map(|am| {
        cf_tools::types::resources::ManagedGatewayToolClient(Arc::new(
    ...
    let resolved_task_output =
        cf_tools::reminders::task_completion::resolve_task_output_tool_name(
            agent.tool_bridge(),
        )
        .await;
    let resolved_read =
        cf_tools::reminders::task_completion::resolve_read_tool_name(agent.tool_bridge())
            .await;
    ...
    tool_context.task_output_tool_name = resolved_task_output.unwrap_or_else(|| {
        cf_tools::reminders::task_completion::DEFAULT_TASK_OUTPUT_TOOL.to_string()
    });
    ...
    let resolved_workspace_root = cf_workspace::session::git::find_git_root_from_path(
    ...
    let sampler_retry_policy = cf_sampler::RetryPolicy {
        max_retries: max_retries.unwrap_or(5),
        rate_limit_retry_threshold: 2,
    };
    let (sampler_event_tx, sampler_event_rx) =
        tokio::sync::mpsc::unbounded_channel::<cf_sampler::SamplingEvent>();
    let sampler_handle = cf_sampler::SamplerActor::spawn(
    ...
    let mut hook_discovery_errors: Vec<cf_hooks::error::HookError> = Vec::new();
    let built_hook_registry: Option<Arc<cf_hooks::discovery::HookRegistry>> =
        if let Some(override_reg) = hook_registry_override {
            ...
            let git_root = cf_workspace::session::git::find_git_root_from_path(cwd_path).ok();
    ...
    let obs_bridge = {
        let sid = cf_tool_protocol::SessionId::new(&*session_info.id.0)
            .unwrap_or_else(|_| cf_tool_protocol::SessionId::new("unknown").expect("valid"));
        cf_computer_hub_sdk::ObservabilityBridge::new(None, sid)
    };
    ...
    let session = Arc::new_cyclic(|weak: &std::sync::Weak<SessionActor>| SessionActor {
        ...
        max_turns,
        max_retries: cf_sampler::resolve_max_retries(max_retries),
        ...
        vcs_kind: {
            ...
            match cf_workspace::session::git::discover_git_root(root) {
                cf_workspace::session::git::GitDiscoveryResult::Found(git_root) => {
                    cf_workspace::session::git::detect_vcs_kind(&git_root)
                }
                _ => cf_workspace::session::git::VcsKind::None,
            }
        },
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(cf_tools::types::tool_index::ToolIndex(
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(cf_tools::types::resources::PlanFilePath(plan_path))
            .await;
    }
    ...
    if let Some(storage) = session.memory.storage() {
        ...
        tokio::task::spawn_local(async move {
            ...
            ) && let Ok(files) = storage.list_memory_files()
            {
                ...
                tracing::info!(
                    target : cf_telemetry::memory_log::TARGET, files = files.len(),
                ...
                cf_telemetry::session_ctx::log_event(
                    cf_telemetry::memory_telemetry::MemoryReindex {
    ...
    let telemetry_ctx = cf_telemetry::session_ctx::TelemetryCtx::new(
    ...
    if let Some(metrics) = harness_metrics {
        ...
        tokio::spawn(async move {
            ...
            cf_telemetry::session_ctx::log_event_dual(telemetry_enabled, ev);
        });
    }
    tokio::task::spawn_local(async move {
        cf_telemetry::session_ctx::with_session_ctx(
            telemetry_ctx,
            run_session(
                session,
                cmd_rx,
...
> {
    let (init_tx, init_rx) = tokio::sync::oneshot::channel::<
        Result<SessionInitResult, cf_agent::AgentBuildError>,
    >();
            ...
            local.block_on(&rt, async move {
                let _trace_span = parent_traceparent.as_ref().map(|tp| {
                    let meta = serde_json::json!({ "traceparent" : tp })
                        ...
                        .cloned()
                        .unwrap_or_default();
                    let span = cf_file_utils::trace_context::span_from_meta_traceparent(&meta);
                    span.entered()
                });
