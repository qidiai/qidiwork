...
fn is_interruptible_wait_tool(tool_name: &str, args: &serde_json::Value) -> bool {
    match tool_name {
        ...
        | "get_terminal_command_output" => cf_tool_types::task_output_waits_from_json(args),
...
fn interrupted_wait_tool_result_with_msg(args: &serde_json::Value, msg: &str) -> ToolRunResult {
    use cf_tool_types::{TaskOutputOutput, TaskOutputResult};
    let task_id = args
        .get("task_ids")
...
///
/// Uses `cf_acp_lib`'s TYPED [`AcpChannelFailure`](cf_acp_lib::AcpChannelFailure)
...
fn ext_method_no_client(err: &acp::Error) -> bool {
    matches!(
        cf_acp_lib::acp_channel_failure(err),
        Some(cf_acp_lib::AcpChannelFailure::SendFailed)
    )
}
...
impl SessionActor {
    ...
    pub(super) fn stamp_tool_meta(
        ...
        cf_tools::normalization::merge_tool_meta(
            &toolset,
    ...
    pub(super) async fn execute_tool_calls(
        ...
        for call in tool_calls.into_iter() {
            ...
            self.observability_bridge
                .emit(
                    cf_tool_protocol::session_event::SessionEvent::ToolCallStarted {
                ...
                .await?
            {
                ...
                Err(tool_loop) => {
                        ...
                        crate::session::mcp_servers::parse_mcp_tool_name(&call_name)
                    {
                        ...
                        self.emit_event(cf_file_utils::events::Event::McpToolCallCompleted {
        ...
        while let Some((idx, mut result)) = dispatch_rx.recv().await {
                ...
                && server.starts_with(crate::session::managed_mcp::MANAGED_MCP_PREFIX)
            {
                let auth_rejected = match &result {
                    Err(err) => cf_mcp::servers::is_auth_rejection_message(&err.to_string()),
                    Ok(tool_result) => {
                        tool_result.output.is_error()
                            && cf_mcp::servers::is_auth_rejection_message(
            ...
            let tool_loop = match result {
                Ok(tool_result) => {
                    ...
                    post_tool_use_result = self
                        .hook_event_active(cf_hooks::event::HookEventName::PostToolUse)
                        .then(|| {
                ...
                Err(err) => {
                    ...
                    if self
                        .hook_event_active(cf_hooks::event::HookEventName::PostToolUseFailure)
                    {
                        let raw_input: serde_json::Value =
                            serde_json::from_str(&prepared.raw_arguments)
                                .unwrap_or(serde_json::Value::Null);
                        let (tool_input_value, tool_input_truncated) =
                            cf_hooks::event::truncate_payload(raw_input);
                        let hook_tool_name = prepared.hook_tool_name();
                        self.dispatch_hook(
                            cf_hooks::event::HookEventName::PostToolUseFailure,
                            cf_hooks::event::HookPayload::PostToolUseFailure {
            ...
            if let Some(tool_result_value) = post_tool_use_result {
                ...
                let (tool_result_val, tool_result_truncated) =
                    cf_hooks::event::truncate_payload(tool_result_value);
                let hook_tool_name = prepared.hook_tool_name();
                self.dispatch_hook(
                    cf_hooks::event::HookEventName::PostToolUse,
                    cf_hooks::event::HookPayload::PostToolUse {
                        tool_name: hook_tool_name.to_owned(),
                        tool_use_id: prepared.call_id.clone(),
                        tool_input: tool_input_value,
            ...
            self.observability_bridge
                .emit(
                    cf_tool_protocol::session_event::SessionEvent::ToolCallCompleted {
                ...
                .await;
            let (ext_file_path, ext_parameters) = if cf_telemetry::external::is_active() {
            ...
            cf_telemetry::session_ctx::log_event(
                cf_telemetry::events::ToolCallCompleted {
                    tool_name: prepared.tool_name.clone(),
                    outcome: tool_outcome,
                    duration_ms,
    ...
    pub(crate) async fn prepare_tool_call(
        ...
        if self.hook_event_active(cf_hooks::event::HookEventName::PreToolUse) {
            let (hook_tool_input, hook_tool_input_truncated) =
                cf_hooks::event::truncate_payload(raw_input.clone());
            let envelope = self.make_hook_envelope(
                cf_hooks::event::HookEventName::PreToolUse,
                None,
                cf_hooks::event::HookPayload::PreToolUse {
            ...
            if let Some(registry) = hook_registry_snapshot {
                ...
                let pre_result =
                    cf_hooks::dispatcher::dispatch_pre_tool_use(&registry, &envelope, &ctx)
                        .await;
                ...
                .await;
                if let cf_hooks::result::HookDecision::Deny { reason, hook_name } =
                    pre_result.decision
                {
                    return Ok(Err(self
                        .deny_tool(
        ...
        if !plan_file_auto_approve {
            ...
            let (telemetry_access_kind, _access_detail) = match &access_kind {
                cf_workspace::permission::AccessKind::Read(p) => (
                    cf_telemetry::events::AccessKind::Read,
                    p.clone().unwrap_or_default(),
                ),
                cf_workspace::permission::AccessKind::Edit(p) => {
                    (cf_telemetry::events::AccessKind::Edit, p.clone())
                }
                cf_workspace::permission::AccessKind::Bash(cmd) => {
                    (cf_telemetry::events::AccessKind::Bash, cmd.clone())
                }
                cf_workspace::permission::AccessKind::Grep { path, glob } => (
                    cf_telemetry::events::AccessKind::Grep,
                    path.clone().or_else(|| glob.clone()).unwrap_or_default(),
                ),
                cf_workspace::permission::AccessKind::MCPTool { name, .. } => {
                    (cf_telemetry::events::AccessKind::Mcp, name.clone())
                }
                cf_workspace::permission::AccessKind::WebFetch(u) => {
                    (cf_telemetry::events::AccessKind::Web, u.clone())
                }
            ...
            cf_telemetry::session_ctx::log_event(
                cf_telemetry::events::PermissionPrompted {
                    tool_name: call.function.name.clone(),
            ...
            self.events.permission_resolved(
                ...
                match &decision {
                    Decision::Allow | Decision::Ask => {
                        cf_file_utils::events::types::PermissionDecision::Allow
                    }
                    Decision::Reject(_) | Decision::PolicyDeny(_) => {
                        cf_file_utils::events::types::PermissionDecision::Deny
                    }
                    Decision::Cancelled => {
                        cf_file_utils::events::types::PermissionDecision::Cancelled
                    }
                    Decision::FollowupMessage(_) => {
                        cf_file_utils::events::types::PermissionDecision::Followup
                    }
                },
                perm_start,
            );
            ...
            let (decision_outcome, _reject_reason) = match &decision {
                Decision::Allow | Decision::Ask => {
                    (cf_telemetry::events::PermissionOutcome::Allow, None)
                }
                Decision::Reject(reason) | Decision::PolicyDeny(reason) => (
                    cf_telemetry::events::PermissionOutcome::Deny,
                    Some(reason.to_string()),
                ),
                Decision::Cancelled => (
                    cf_telemetry::events::PermissionOutcome::Cancelled,
                    None,
                ),
                Decision::FollowupMessage(_) => (
                    cf_telemetry::events::PermissionOutcome::Followup,
                    None,
                ),
            };
            tracing::info_span!(
            ...
            .in_scope(|| {});
            cf_telemetry::session_ctx::log_event(
                cf_telemetry::events::PermissionDecisionPayload {
                    tool_name: call.function.name.clone(),
                    access_kind: telemetry_access_kind,
            ...
            match decision {
                Decision::PolicyDeny(ref reason) | Decision::Reject(ref reason) => {
                    ...
                    let (tool_input_value, tool_input_truncated) =
                        cf_hooks::event::truncate_payload(raw_input.clone());
                    self.dispatch_hook(
                        cf_hooks::event::HookEventName::PermissionDenied,
                        cf_hooks::event::HookPayload::PermissionDenied {
                            tool_name: resolved_tool_name.clone(),
        ...
        if deactivated {
            ...
            *self.turn_prompt_mode.lock() = PromptMode::Agent;
            ...
            self.enqueue_current_mode_update(acp::SessionModeId::new(
                cf_tools::types::SessionMode::Default.as_id(),
            ));
        }
    }
        ...
        let (title, kind, locations, content) = match tool_call_input {
            ...
            ToolInput::ReadFile(read_file) => (
                ...
                vec![
                    acp::ToolCallLocation::new(read_file.path).line(
                        cf_tools::normalization::norm_offset_i64(read_file.offset)
                            .map(|l| l as u32),
                    ),
                ],
                Vec::new(),
            ...
            ToolInput::WaitTasks(wait) => (
                format!(
                    ...
                    wait.task_ids.len(),
                    match wait.mode {
                        cf_tool_types::WaitMode::WaitAny => "wait_any",
                        cf_tool_types::WaitMode::WaitAll => "wait_all",
                    }
                ),
                acp::ToolKind::Other,
                vec![],
            ...
            ToolInput::Skill(skill) => {
                cf_telemetry::session_ctx::log_event(
                    cf_telemetry::events::SkillDispatched {
    ...
    /// `cf_tools::reminders::task_completion::consumed_completion_ids`,
    ...
    fn record_git_pr_signals(&self, effective_tool_name: &str, result: &ToolRunResult) {
        use cf_telemetry::enums::PrCreationSource;
        use cf_tools::util::git_detect;
        match &result.output {
            cf_tools::types::output::ToolOutput::Bash(b) if b.exit_code == 0 => {
                ...
                if ops.pr_merged {
                    ...
                    cf_telemetry::session_ctx::log_event(
                        cf_telemetry::events::PrMerged {},
                    );
                }
            }
            cf_tools::types::output::ToolOutput::MCP(m)
    ...
    fn record_pr_created(
        &self,
        pr: cf_tools::util::git_detect::PrRef,
        source: cf_telemetry::enums::PrCreationSource,
    ) {
        self.signals_handle()
    ...
    pub(super) async fn handle_bridge_tool_success(
        ...
        let consumed_ids =
            cf_tools::reminders::task_completion::consumed_completion_ids(&result.output);
        if !consumed_ids.is_empty() {
        ...
        if matches!(
            ...
            ToolsToolOutput::SearchReplace(
                cf_tools::types::output::SearchReplaceOutput::EditsApplied(_)
            ) | ToolsToolOutput::Bash(_)
        ) {
            self.maybe_notify_git_branch().await;
        }
        if let cf_tools::types::output::ToolOutput::Bash(ref b) = result.output
            && b.was_bare_echo
        {
            ...
            acp_tool_update(&result.output, call_id, path_rewriter.as_ref(), tool_meta)
        {
            ...
            if matches!(
                &result.output,
                cf_tools::types::output::ToolOutput::EnterPlanMode(_)
                    | cf_tools::types::output::ToolOutput::ExitPlanMode(_)
            ) {
        ...
        let extraction = if !self.is_cursor_harness()
            ...
            cf_tools::util::base64_images::extract_base64_images(prompt_text)
        } else {
            cf_tools::util::base64_images::ExtractionResult {
                text: prompt_text,
                images: Vec::new(),
            }
        };
    ...
    /// Translate one [`cf_sampler::SamplingEvent`] from the
    ...
    pub(crate) async fn handle_sampling_event(
        self: &Arc<Self>,
        event: cf_sampler::SamplingEvent,
    ) {
        use cf_sampler::{SamplingChannel, SamplingEvent};
        match event {
            ...
            SamplingEvent::Retrying {
                ...
                if kind == cf_sampler::SamplingErrorKind::DoomLoopDetected {
            ...
            SamplingEvent::Failed { request_id, error } => {
                cf_telemetry::unified_log::error(
...
fn execute_tool_call_parts(
    ...
    let display = cf_tools::util::strip_redundant_session_cd(command, cwd);
    (
...
mod plan_mode_edit_gate_tests {
    ...
    use cf_workspace::permission::AccessKind;
    ...
    fn write(path: &str) -> ToolInput {
        use cf_tools::implementations::opencode::write::WriteInput;
    ...
    fn grok_edits_outside_plan_file_rejected() {
        ...
        assert_eq!(
            ...
            PlanEditGate::RejectNonPlanFile,
            "tools get no markdown exception — plan file only"
        );
    }
    ...
    fn apply_patch_rejected_in_plan_mode() {
        use cf_tools::implementations::codex::apply_patch::ApplyPatchInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
...
mod plan_approval_helper_tests {
    ...
    fn ext_method_no_client_defaults_false_for_untagged_error() {
        assert!(!ext_method_no_client(&cf_acp_lib::acp_internal_error(
...
mod wait_interrupt_tests {
    ...
    use cf_tools::types::output::ToolOutput;
    use cf_tool_types::TaskOutputOutput;
    ...
    async fn pending_interjection_aborts_in_flight_wait() {
        ...
        use cf_interjection_core::PendingInterjection;
