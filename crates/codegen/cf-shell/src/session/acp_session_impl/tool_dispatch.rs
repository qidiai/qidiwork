...
/// `list_dir`=`target_directory`. Grammar lives in `cf_chat_state`.
pub(super) fn compaction_artifact_read(
    args: &serde_json::Value,
) -> Option<cf_chat_state::compaction_transcript::CompactionArtifact> {
    let path = str_arg(
        args,
        &["target_file", "file_path", "path", "target_directory"],
    )?;
    cf_chat_state::compaction_transcript::classify_compaction_path(path)
}
...
impl SessionActor {
    ...
    pub(super) async fn handle_direct_bash_command(
        ...
        let title_command = cf_tools::util::strip_redundant_session_cd(
            &command,
        ...
        let exec_wire = {
            ...
            agent
                .tool_bridge()
                .toolset()
                .tool_name_for_kind(cf_tools::types::tool::ToolKind::Execute)
        };
