...
impl SessionActor {
    ...
    pub(super) async fn handle_prompt(
        ...
        *self.turn_prompt_mode.lock() = prompt_mode;
        ...
        let turn_start_input = cf_agent_lifecycle::TurnStartInput::new(
        ...
        let prompt_blocks = match slash_commands::resolve(
            ...
            Err(SlashCommandOutcome::Builtin(action)) => {
                ...
                let slash_used = cf_telemetry::events::SlashCommandUsed {
                ...
                match action {
                    BuiltinAction::GoalSet {
                        ...
                        cf_telemetry::session_ctx::log_event(slash_used);
            ...
            Err(SlashCommandOutcome::InvokeSkill {
                ...
                for sk in &parsed_skills {
                    cf_telemetry::session_ctx::log_event(
                        cf_telemetry::events::SlashCommandUsed {
                    ...
                    if let Some(ref pname) = sk.plugin_name {
                        cf_telemetry::session_ctx::log_event(
                            cf_telemetry::events::PluginUsed {
        ...
        self.observability_bridge
            .emit(
                cf_tool_protocol::session_event::SessionEvent::TurnStarted {
                    turn_number,
                    model_id: model_id.clone(),
                    yolo_mode,
                },
            ...
            .await;
        self.send_before_turn_event(cf_tool_protocol::turn_hook::BeforeTurnPayload {
            turn_number: self.chat_state_handle.get_prompt_index().await as u64,
            model_id: model_id.clone(),
            yolo_mode: self.permissions.is_yolo_mode(),
            conversation_message_count: msg_count,
            session_relationship: cf_tool_protocol::turn_hook::DEFAULT_SESSION_RELATIONSHIP
                .to_string(),
        ...
        *self.tool_context.prompt_index.lock().await = current_prompt_index;
        ...
        let (query, extra_images) = if !self.is_cursor_harness() {
            let extraction = cf_tools::util::base64_images::extract_base64_images(query);
        ...
        let model_id = self
            ...
            .map(|c| c.model)
            .unwrap_or_default();
        if self.telemetry_enabled || cf_telemetry::external::is_active() {
            let effective_client_identifier =
                prompt_client_identifier.or_else(|| self.client_identifier.clone());
            let ev = cf_telemetry::events::PromptSubmitted {
            ...
            cf_telemetry::session_ctx::log_event_dual(self.telemetry_enabled, ev);
        }
        ...
        self.tool_bridge_handle()
            .update_resource(cf_tools::types::resources::AttachedImages(
                attached_image_refs,
            ))
            .await;
        ...
        self.dispatch_hook(
            cf_hooks::event::HookEventName::UserPromptSubmit,
            cf_hooks::event::HookPayload::UserPromptSubmit {
        ...
        self.observability_bridge
            .emit(cf_tool_protocol::session_event::SessionEvent::TurnEnded {
        ...
        match &result {
            Ok(TurnOutcome::Completed { .. }) => {
                ...
                self.send_after_turn_event(cf_tool_protocol::turn_hook::AfterTurnPayload {
                    turn_number: current_prompt_index as u64,
                    outcome: cf_tool_protocol::turn_hook::TurnHookOutcome::Completed,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id.clone(),
                ...
                .await;
                cf_telemetry::session_ctx::log_event(
                    cf_telemetry::events::TurnCompleted {
                        outcome: cf_telemetry::events::Outcome::Completed,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
            ...
            Ok(TurnOutcome::Cancelled { category, context }) => {
                ...
                self.send_after_turn_event(cf_tool_protocol::turn_hook::AfterTurnPayload {
                    turn_number: current_prompt_index as u64,
                    outcome: cf_tool_protocol::turn_hook::TurnHookOutcome::Cancelled,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id.clone(),
                    written_repo_paths: Vec::new(),
                ...
                cf_telemetry::session_ctx::log_event(
                    cf_telemetry::events::TurnCompleted {
                        outcome: cf_telemetry::events::Outcome::Cancelled,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
                        model_id: turn_model_id,
            ...
            Err(err) => {
                ...
                self.send_after_turn_event(cf_tool_protocol::turn_hook::AfterTurnPayload {
                    turn_number: current_prompt_index as u64,
                    outcome: cf_tool_protocol::turn_hook::TurnHookOutcome::Error,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id.clone(),
                    written_repo_paths: Vec::new(),
                ...
                cf_telemetry::session_ctx::log_session_event(
                    cf_telemetry::events::ApiError {
                ...
                cf_telemetry::session_ctx::log_event(
                    cf_telemetry::events::TurnCompleted {
                        outcome: cf_telemetry::events::Outcome::Error,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
                        model_id: turn_model_id,
                        cancellation_category: None,
                ...
                self.dispatch_hook(
                    cf_hooks::event::HookEventName::StopFailure,
                    cf_hooks::event::HookPayload::StopFailure {
                        error: format!("{err}"),
                    },
                    Some(prompt_id),
                    None,
        ...
        self.dispatch_hook(
            cf_hooks::event::HookEventName::Stop,
            cf_hooks::event::HookPayload::Stop {
        ...
        match &result {
            Ok(TurnOutcome::Completed { .. }) => {
                for contributor in self.extension_registry.turn_lifecycle_contributors() {
                    contributor
                        .on_turn_done(&cf_agent_lifecycle::TurnDoneInput)
                        .await;
                }
            }
            Ok(TurnOutcome::Cancelled { .. }) | Ok(TurnOutcome::MaxTurnsReached { .. }) => {
                let input = cf_agent_lifecycle::TurnAbortInput::new(
                    cf_agent_lifecycle::TurnAbortReason::Interrupted,
                );
                for contributor in self.extension_registry.turn_lifecycle_contributors() {
            ...
            Err(err) => {
                ...
                let input = cf_agent_lifecycle::TurnErrorInput { message: &message };
                for contributor in self.extension_registry.turn_lifecycle_contributors() {
    ...
    pub(crate) async fn inject_pending_monitor_events(&self) {
        ...
        let Some(body) = cf_tools::reminders::task_completion::format_monitor_events(
            &mine,
            Some(&self.tool_context.task_output_tool_name),
        ) else {
            return;
        };
        let wrapped = cf_tools::reminders::wrap_reminder(&body);
    ...
    pub(crate) async fn first_turn_memory_reminder(&self) -> Option<String> {
        ...
        use cf_tools::types::memory_backend::MemoryBackend as _;
        ...
        tracing::info!(
            target : cf_telemetry::memory_log::TARGET, configured_min_score,
            "MEMORY_INJECT_SEARCH: results={result_count}"
        );
        cf_telemetry::session_ctx::log_event(
            cf_telemetry::memory_telemetry::MemoryInjection {
    ...
    async fn handle_structured_output_tool_call(
        &self,
        tool_calls: &mut Vec<cf_sampling_types::conversation::ToolCall>,
        validator: &Result<jsonschema::Validator, String>,
        retries: &mut u32,
    ...
    async fn finalize_turn_bookkeeping(
        ...
        if let Some(snap) = snapshot.as_mut() {
            ...
            for pr in &snap.delta.prs_created_this_turn {
                cf_telemetry::session_ctx::log_event(cf_telemetry::events::PrCreated {
                    source: pr.source,
    ...
    async fn process_conversation_turn(
        ...
        loop {
            ...
            request.x_grok_agent_id = Some(cf_telemetry::id::agent_id());
            ...
            self.observability_bridge
                .emit(
                    cf_tool_protocol::session_event::SessionEvent::PhaseChanged {
                        phase: cf_tool_protocol::session_event::SessionPhase::Sampling,
                    },
                )
                .await;
            cf_telemetry::unified_log::info(
            ...
            let model_duration_ms = model_timer.elapsed().as_millis() as u64;
            {
                let model_id = self.current_model_id().await;
                cf_telemetry::session_ctx::log_event(
                    cf_telemetry::events::ModelResponseReceived {
                        model_id,
            ...
            let turn_refused =
                stop_reason == Some(cf_sampling_types::StopReason::ContentFilter);
            ...
            for item in response.items {
                match item {
                    cf_sampling_types::ConversationItem::Assistant(_) => {
            ...
            self.observability_bridge
                .emit(
                    cf_tool_protocol::session_event::SessionEvent::PhaseChanged {
                        phase: cf_tool_protocol::session_event::SessionPhase::ToolExecution,
                    },
                )
                .await;
