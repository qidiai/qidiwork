...
impl SessionActor {
    ...
    pub(super) async fn record_subagent_usage(
        &self,
        by_model: &[(String, cf_chat_state::UsageTotals)],
    ...
    fn log_outbound_xai_buffered(&self, notification: &XaiSessionNotification) {
        ...
        tracing::info!(
            target : "acp_event", event = "cf_buffered_notification_sent", session_id =
        ...
        match &notification.update {
            XaiSessionUpdate::SubagentSpawned {
                ...
                let envelope = self.fire_hook(
                    cf_hooks::event::HookEventName::SubagentStart,
                    None,
                    cf_hooks::event::HookPayload::SubagentStart {
                ...
                if let Some(registry) = hook_registry_snapshot {
                    ...
                    let _ = cf_hooks::dispatcher::dispatch_non_blocking(
                        &registry,
                        cf_hooks::event::HookEventName::SubagentStart,
                        &envelope,
                        &ctx,
            ...
            XaiSessionUpdate::SubagentFinished {
                ...
                let envelope = self.fire_hook(
                    cf_hooks::event::HookEventName::SubagentEnd,
                    None,
                    cf_hooks::event::HookPayload::SubagentStop {
                        subagent_id: subagent_id.clone(),
    ...
    pub(super) async fn dispatch_notification_hook(
        ...
        let envelope = self.fire_hook(
            cf_hooks::event::HookEventName::Notification,
            None,
            cf_hooks::event::HookPayload::Notification {
...
#[cfg(test)]
mod cf_event_id_stamping_tests {
