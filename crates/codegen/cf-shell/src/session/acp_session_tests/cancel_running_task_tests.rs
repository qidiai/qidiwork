...
async fn persist_ack_waits_for_disk_flush_before_success() {
    ...
    local
        .run_until(async {
            ...
            let sampling_client = crate::sampling::Client::new(cf_sampler::SamplerConfig {
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                ConversationItem::assistant_tool_calls(vec![cf_sampling_types::ToolCall {
                    id: "call-1".into(),
...
async fn cancel_propagates_to_sampler_handle_so_no_further_emission() {
    ...
    local
        .run_until(async {
            ...
            let cfg = cf_sampler::SamplerConfig {
                api_key: Some("test-key".to_string()),
                ...
                temperature: None,
                top_p: None,
                api_backend: cf_sampler::ApiBackend::Responses,
                auth_scheme: Default::default(),
                extra_headers: Default::default(),
                context_window: 100_000,
                client_version: None,
            ...
            let (sampler_event_tx, _sampler_event_rx) = tokio::sync::mpsc::unbounded_channel::<
                cf_sampler::SamplingEvent,
            >();
            let sampler_handle = cf_sampler::SamplerActor::spawn(
                cfg,
                cf_sampler::RetryPolicy::default(),
                sampler_event_tx,
            );
            let (gateway_tx, _gateway_rx) = tokio::sync::mpsc::unbounded_channel::<
                cf_acp_lib::AcpClientMessage,
            ...
            let request = ConversationRequest {
                items: vec![
                    ConversationItem::User(cf_sampling_types::UserItem { content :
                    vec![cf_sampling_types::ContentPart::Text { text : "hi".into(),
...
async fn skill_reminder_deferred_while_turn_running_flushed_when_idle() {
    use cf_tools::types::skill_discovery_tracker::{SkillUpdateEffects, SkillUpdateKind};
        ...
        actor
            ...
            .filter(|item| {
                matches!(
                    ...
                    matches!(p, cf_sampling_types::ContentPart::Text { text } if
