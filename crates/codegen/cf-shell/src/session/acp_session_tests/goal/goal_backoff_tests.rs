    ...
    local
        .run_until(async {
            ...
            *actor.goal_update_rx.borrow_mut() = Some(rx);
            tx.send(
                cf_tools::implementations::qidi_build::update_goal::envelope_for_test(
...
fn spawn_gateway_notification_capture(
    mut gateway_rx: tokio::sync::mpsc::UnboundedReceiver<cf_acp_lib::AcpClientMessage>,
    ...
    local
        .run_until(async {
            ...
            let cancelled: PromptTurnResult = Ok(crate::session::commands::PromptTurnOk {
                ...
                completion_kind: crate::session::commands::PromptCompletionKind::Cancelled {
                    category: Some(
                        cf_file_utils::events::types::CancellationCategory::MidTurnAbort,
                    ),
                    context: None,
                },
                structured_output: None,
...
async fn drain_goal_updates_completes_after_blocked_does_not_leak_pause_message() {
    ...
    local
            .run_until(async {
                ...
                let (tx, rx) = tokio::sync::mpsc::unbounded_channel();
                *actor.goal_update_rx.borrow_mut() = Some(rx);
                tx.send(cf_tools::implementations::qidi_build::update_goal::envelope_for_test(cf_tools::implementations::qidi_build::update_goal::UpdateGoalInput {
                        completed: None,
                        message: None,
...
-> cf_tools::implementations::qidi_build::update_goal::UpdateGoalEnvelope {
    let input = cf_tools::implementations::qidi_build::update_goal::UpdateGoalInput {
        completed: Some(true),
        message: None,
        blocked_reason: None,
    };
