...
//!
//! Tests that mutate `QIDI_GOAL_CLASSIFIER` carry the
...
async fn goal_classifier_env_override_disables_when_remote_enabled() {
    ...
    assert!(
        !resolved.value,
        "QIDI_GOAL_CLASSIFIER=0 must disable even with remote=true",
    );
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
...
fn tool_error_code(err: &cf_tool_runtime::ToolError) -> &str {
    err.details
        .as_ref()
...
/// `cf-tools-plan` (non-strict), and `opencode` (non-strict + unrepresentable
...
async fn resolve_role_override_harness_flavor_representability_gate() {
    ...
    local
        .run_until(async {
            ...
            let honored = ["cf-tools-plan", "opencode"];
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                cf_sampling_types::conversation::ConversationItem::assistant(SUMMARY),
            );
            let _ = actor.run_verification_stage_for_drain(1, 3).await;
            assert_eq!(
                actor
...
async fn reverification_replays_anchor_and_does_not_overwrite_it() {
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                cf_sampling_types::conversation::ConversationItem::assistant(NOTE),
            );
