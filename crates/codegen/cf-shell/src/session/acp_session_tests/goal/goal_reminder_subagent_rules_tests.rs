...
fn resolve_goal_reverify_after_env() {
    unsafe { std::env::remove_var("QIDI_GOAL_REVERIFY_AFTER") };
    assert_eq!(reverify(None), GOAL_REVERIFY_AFTER_DEFAULT);
    unsafe { std::env::set_var("QIDI_GOAL_REVERIFY_AFTER", "3") };
    assert_eq!(reverify(Some(9)), 3, "env beats config");
    unsafe { std::env::set_var("QIDI_GOAL_REVERIFY_AFTER", "0") };
    assert_eq!(reverify(None), 1, "0 floors to 1");
    unsafe { std::env::set_var("QIDI_GOAL_REVERIFY_AFTER", "garbage") };
    assert_eq!(
        reverify(None),
