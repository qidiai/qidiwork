...
fn assistant_with_reasoning_items(
    ...
    if !reasoning_text.is_empty() {
        out.push(ConversationItem::Reasoning(
            cf_sampling_types::rs::ReasoningItem {
                id: String::new(),
                summary: vec![cf_sampling_types::rs::SummaryPart::SummaryText(
                    cf_sampling_types::rs::SummaryTextContent {
...
fn flatten_skips_reasoning_when_encrypted_only() {
    ...
    let items = vec![
        ConversationItem::Reasoning(cf_sampling_types::rs::ReasoningItem {
            id: String::new(),
            summary: vec![],
            content: None,
...
fn synthetic_user_text(
    text: &str,
    reason: cf_sampling_types::SyntheticReason,
) -> ConversationItem {
    ConversationItem::User(UserItem {
        content: vec![ContentPart::Text { text: text.into() }],
...
fn window_assistant_text_pin_skips_empty_assistant_turns() {
    ...
    let empty_asst = ConversationItem::Assistant(cf_sampling_types::AssistantItem {
