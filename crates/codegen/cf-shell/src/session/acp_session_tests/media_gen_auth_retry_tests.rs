...
fn ok_result(text: &str) -> Result<ToolRunResult, cf_tool_runtime::ToolError> {
    Ok(ToolRunResult {
...
fn err(msg: &str) -> Result<ToolRunResult, cf_tool_runtime::ToolError> {
...
fn http_err(status: u16, msg: &str) -> Result<ToolRunResult, cf_tool_runtime::ToolError> {
    Err(
        cf_tool_runtime::ToolError::new(cf_tool_runtime::ToolErrorKind::Custom, msg.to_owned())
            .with_details(
...
fn is_auth_tool_error_classification() {
    ...
    let cases: Vec<(bool, cf_tool_runtime::ToolError)> = vec![
        ...
        (
            true,
            cf_tool_runtime::ToolError::invalid_arguments("response: invalid api key for project"),
        ),
        // Fallback path: OAuth 2.0 `invalid_token` payload (RFC 6749)
        // surfaced as raw JSON without a structured status code.
        (
            true,
            cf_tool_runtime::ToolError::invalid_arguments(r#"{"error":"invalid_token"}"#),
        ),
        // Fallback path: case-insensitive "unauthorized" anywhere in
        // the message body.
        (
            true,
            cf_tool_runtime::ToolError::invalid_arguments("UNAUTHORIZED"),
        ),
        // Negative: transport failure must not trigger a token refresh.
        (
            false,
            cf_tool_runtime::ToolError::invalid_arguments("Image generation timed out after 60s"),
        ),
        // Negative: structural not-found error; not a network response.
        (
            false,
            cf_tool_runtime::ToolError::not_found(
                cf_tool_protocol::ToolId::new("image_gen").expect("valid"),
        ...
        (
            false,
            cf_tool_runtime::ToolError::invalid_arguments("request id req_401abc failed"),
        ),
    ];
