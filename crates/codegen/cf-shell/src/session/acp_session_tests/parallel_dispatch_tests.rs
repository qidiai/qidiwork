...
//!
//! These tests verify the parallel dispatch path (QIDI_PARALLEL_TOOL_DISPATCH):
...
/// JSON key. The compat toolset input types use `path`, and qidi_build's
...
#[test]
fn lock_path_for_args_matches_qidi_build_file_path() {
    // qidi_build search_replace / opencode EditTool / WriteTool / etc.
...
#[test]
fn lock_path_for_args_matches_qidi_build_target_file() {
    // qidi_build read_file uses #[serde(rename = "target_file")].
    let args = serde_json::json!({
...
#[test]
fn lock_path_for_args_buckets_qidi_build_and_compat_to_same_lock_for_same_file() {
    // A mixed batch (e.g. qidi_build search_replace + StrReplace
