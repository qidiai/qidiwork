...
fn test_json_round_trip() {
    let ctx = cf_agent::PromptContext {
        ..Default::default()
    };

    let json = serde_json::to_string_pretty(&ctx).unwrap();
    let loaded: cf_agent::PromptContext = serde_json::from_str(&json).unwrap();
...
fn test_json_round_trip_via_filesystem() {
    ...
    let ctx = cf_agent::PromptContext::default();
    ...
    let loaded: cf_agent::PromptContext = serde_json::from_str(&read_json).unwrap();
...
fn test_system_prompt_is_plain_text_not_json() {
    let prompt = "You are a QIDI Code subagent.";
...
fn test_canonical_artifacts_coexist() {
    ...
    let read_ctx: cf_agent::PromptContext = serde_json::from_str(
...
fn test_system_prompt_matches_chat_history_system_message() {
    ...
    let system_prompt = "You are a QIDI Code subagent.\n\n<tool_calling>\n...";
...
fn test_corrupt_json_returns_error() {
    ...
    let result: Result<cf_agent::PromptContext, _> = serde_json::from_str(&json);
