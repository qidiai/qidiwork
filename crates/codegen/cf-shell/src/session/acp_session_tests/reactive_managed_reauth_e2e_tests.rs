...
use cf_mcp::servers::{ClientStateKind, HttpConfig, McpClient};
...
async fn actor_with_proxy(
    proxy_base: &str,
    gw_tx: tokio::sync::mpsc::UnboundedSender<cf_acp_lib::AcpClientMessage>,
...
async fn entry_b_routes_auth_rejection_but_not_policy_denial() {
    use cf_mcp::servers::{is_auth_rejection_message, parse_mcp_tool_name};
