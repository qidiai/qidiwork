...
async fn manual_recap_over_budget_trims_persisted_request_and_is_display_only() {
    ...
    local
        .run_until(async {
            ...
            while let Ok(msg) = persistence_rx.try_recv() {
                if let PersistenceMsg::RecapRequest(artifact) = msg {
                    let est = cf_chat_state::estimate_conversation_tokens(&artifact.chat_history);
                    assert!(
                        est <= PROMPT_BUDGET,
...
fn over_budget_recap_serializes_to_well_formed_messages_request() {
    ...
    use cf_sampling_types::messages::{ContentBlock, MessageContent, MessageRole};
    use cf_sampling_types::{ConversationRequest, ToolCall, rs};
    ...
    let msg = cf_sampling_types::build_messages_request(&req);
