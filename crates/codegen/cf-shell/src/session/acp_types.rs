...
impl TokenUsageCategory {
    ...
    pub fn skills_listing(text: &str, skill_count: usize) -> Self {
        Self {
            ...
            tokens: cf_token_estimation::estimate_tokens(text),
...
pub struct ContextInfo {
    ...
    /// always matches the actual trigger (e.g. 65 for cf-tools in remote settings).
...
impl ContextInfo {
    ...
    pub fn from_notification(used: u64, total: u64) -> Self {
        Self {
            used,
            total,
            usage_pct: cf_token_estimation::usage_percentage_u8(used, total),
            free_tokens: cf_token_estimation::free_tokens(total, used),
...
pub struct SessionInfoData {
    /// Agent definition name for this session (e.g. `cf-tools`).
...
pub fn is_coding_model_slug(model: &str) -> bool {
    matches!(model, "cf-tools" | "grok-4.5")
}
...
mod tests {
    ...
    fn should_show_model_fingerprint_truth_table() {
        ...
        assert!(should_show_model_fingerprint(false, "cf-tools"));
