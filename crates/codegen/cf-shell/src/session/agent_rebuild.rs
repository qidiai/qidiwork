...
//! [`cf_agent::AgentBuilder::new`]. Both initial session spawn
...
//! ## Why this exists
//!
//! [`cf_agent::Agent`] owns an [`cf_tools::bridge::ToolBridge`]
...
//!
//! ## WHEN ADDING A NEW [`cf_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use cf_agent::config::AgentDefinition;
use cf_agent::error::AgentBuildError;
use cf_agent::prompt::context::PromptAudience;
...
use cf_tools::types::SharedApiKeyProvider;
use cf_tools::types::compat::CompatConfig;
use cf_tools::types::memory_backend::MemoryBackend;
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub attribution_callback: Option<cf_tools::SharedAttributionCallback>,
    ...
    pub managed_gateway_tool_client:
        Option<cf_tools::types::resources::ManagedGatewayToolClient>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        agent
            .tool_bridge()
            .update_resource(cf_tools::types::resources::RespectGitignore(
                *respect_gitignore,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(cf_tools::types::resources::PathNotFoundHints(
                *path_not_found_hints,
            ))
            .await;
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            cf_tools::computer::local::LocalTerminalBackend::new_local(
