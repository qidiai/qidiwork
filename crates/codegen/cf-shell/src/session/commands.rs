...
pub enum SessionCommand {
    ...
    SetSessionModel {
        sampling_config: cf_sampler::SamplerConfig,
        use_concise: bool,
    ...
    GetModelMetadata {
        responds_to: oneshot::Sender<cf_chat_state::ModelMetadata>,
    },
    ...
    ReloadPlugins {
        registry: Option<std::sync::Arc<cf_agent::plugins::PluginRegistry>>,
    },
    /// Re-discover the session's own project hooks (`.qidi/hooks`,
    ...
    RepairHistory {
        dry_run: bool,
        respond_to:
            oneshot::Sender<anyhow::Result<cf_chat_state::compaction_utils::HistoryRepairReport>>,
    },
    GetRewindPoints {
    ...
    RecordSubagentUsage {
        by_model: Vec<(String, cf_chat_state::UsageTotals)>,
    ...
    SnapshotToolDefinitions {
        respond_to: oneshot::Sender<Vec<cf_sampling_types::ToolSpec>>,
    },
    ...
    KillBackgroundTask {
        task_id: String,
        respond_to: oneshot::Sender<Result<cf_tools::types::KillOutcome, String>>,
    },
    DeleteScheduledTask {
        task_id: String,
    ...
    ListTasks {
        respond_to: oneshot::Sender<Option<Vec<cf_tools::types::TaskSnapshot>>>,
    },
    ...
    GetHooksList {
        respond_to: oneshot::Sender<cf_hooks_plugins_types::HooksListResponse>,
    },
    /// Execute a hooks management action from the pager modal.
    HooksAction {
        action: cf_hooks_plugins_types::HooksAction,
        respond_to: oneshot::Sender<cf_hooks_plugins_types::ActionOutcome>,
    },
    ...
    PluginsList {
        respond_to:
            oneshot::Sender<Option<std::sync::Arc<cf_agent::plugins::PluginRegistry>>>,
    },
    ...
    /// tool-free model call (default `cf-tools-0.1` when available via
    ...
    /// `cf-tools` model, low temperature, and capped output tokens.
    RewriteMemoryNote {
        raw_text: String,
    ...
    TakeTurnMessages {
        respond_to: oneshot::Sender<Option<cf_chat_state::TurnCapture>>,
    },
    ...
    TakeHarnessTraceTurns {
        respond_to:
            oneshot::Sender<Vec<Vec<cf_sampling_types::conversation::ConversationItem>>>,
    },
