...
use std::sync::Arc;
use cf_chat_state::compaction_utils::{
...
use cf_sampling_types::{ApiBackend, ConversationItem};
...
/// Override with `QIDI_PREFIRE_LEAD_PERCENT`.
const DEFAULT_PREFIRE_LEAD_PERCENT: u64 = 10;
fn prefire_lead_percent() -> u64 {
    std::env::var("QIDI_PREFIRE_LEAD_PERCENT")
        .ok()
...
mod two_pass_prefire_helper_tests {
    ...
    fn prefire_lead_percent_defaults_to_10() {
        unsafe { std::env::remove_var("QIDI_PREFIRE_LEAD_PERCENT") };
...
impl SessionActor {
    ...
    pub(crate) async fn should_prefire_two_pass(&self) -> bool {
        ...
        cf_token_estimation::exceeds_threshold(estimated_total, cw, start_pct as u8)
    }
    ...
    async fn run_prefire_pass1_inner(self: &Arc<Self>) -> PrefirePass1Run {
        ...
        if std::env::var("QIDI_DEBUG_TWO_PASS_FAIL_PASS1")
            .is_ok_and(|v| matches!(v.trim(), "1" | "true" | "yes" | "on"))
        {
            tracing::info!(
                target : "two_pass",
                "two_pass: DEBUG QIDI_DEBUG_TWO_PASS_FAIL_PASS1 — prefire pass1 produces no cache"
            );
        ...
        let prefix_est_tokens = prefix_prepared
            .iter()
            .map(cf_chat_state::estimate_item_tokens)
            .sum::<u64>();
...
impl SessionActor {
    ...
    pub(crate) async fn run_compact(
        ...
        if let Err(e) = self
            .run_compact_inner(
                user_context,
                None,
                cf_telemetry::events::CompactionTrigger::Manual,
            )
            .await
        {
            let span = tracing::Span::current();
    ...
    async fn suppress_auto_compaction(
            ...
            .is_ok()
        {
            ...
            cf_telemetry::session_ctx::log_event(
                cf_telemetry::events::AutoCompactSuppressed {
        ...
        match preserve_inherited_prefix(&full_conv, compacted_history, prefix_len) {
            Ok(preserved) => {
                let projected_preserved = project_preserved_reseed_tokens(
                    cf_chat_state::estimate_conversation_tokens(&preserved),
                    tokens_before,
                    cf_chat_state::estimate_conversation_tokens(&full_conv),
                );
                if cf_token_estimation::exceeds_threshold(
                    projected_preserved,
                    context_window,
                    self.compaction.threshold_percent.get(),
                ) {
    ...
    async fn run_compact_inner(
        ...
        trigger: cf_telemetry::events::CompactionTrigger,
    ) -> Result<(), acp::Error> {
        ...
        let trigger_str = match trigger {
            cf_telemetry::events::CompactionTrigger::Manual => "manual",
            cf_telemetry::events::CompactionTrigger::Auto => "auto",
        };
        let sampling_config = self.chat_state_handle.get_sampling_config().await;
        let context_window = sampling_config
            .as_ref()
        ...
        let compaction = cf_telemetry::events::CompactionScope::begin(
            trigger,
            tokens_before,
            context_window,
            model_id.clone(),
        ...
        self.dispatch_hook(
            cf_hooks::event::HookEventName::PreCompact,
            cf_hooks::event::HookPayload::PreCompact {
        ...
        let segment_messages = if self.compaction.compaction_mode.writes_segments() {
            cf_chat_state::compaction_utils::prepare_conversation_for_segment(
        ...
        let simplified_messages = if verbatim_input_enabled {
            cf_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                full_conversation,
                summary_strips_reasoning,
            )
        } else {
            cf_chat_state::compaction_utils::prepare_conversation_for_summarization(
                full_conversation,
            )
        };
        if conv_len == 0 {
        ...
        let compaction_tools: Vec<cf_sampling_types::ToolSpec> = effective_tool_defs
            .into_iter()
            .map(cf_sampling_types::ToolSpec::from)
            .collect();
        let compaction_hosted_tools: Vec<cf_sampling_types::HostedTool> =
            if use_backend_search {
        ...
        let estimated_input_tokens =
            cf_chat_state::estimate_conversation_tokens(&simplified_messages);
        let auto_trigger = matches!(trigger, cf_telemetry::events::CompactionTrigger::Auto);
        ...
        let fr_config = cf_compaction::FullReplaceConfig {
        ...
        while compact_summary.is_none() {
            match cf_compaction::sample_full_replace_summary(
                &sampler,
                &request_turns,
            ...
            .await
            {
                ...
                Err(cf_compaction::FullReplaceError::NothingToCompact) => {
                    last_error = Some(
                        acp::Error::internal_error().data("compact failed: nothing to compact"),
                    );
                    break;
                }
                Err(cf_compaction::FullReplaceError::EmptyResponse) => {
                ...
                Err(cf_compaction::FullReplaceError::Sampler {
                    message,
                    deterministic,
                    context_overflow,
                }) => {
                    if context_overflow {
                        ...
                        if let Some(stage) = next_stage {
                            ...
                            cf_telemetry::session_ctx::log_event(
                                cf_telemetry::events::CompactionRetryDegraded {
                                    trigger,
                            ...
                            request_turns = match stage {
                                InputStage::VerbatimFitted => {
                                    ...
                                    let verbatim = cf_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                                        conv,
                                        summary_strips_reasoning,
                                    );
                                    cf_chat_state::compaction_utils::fit_conversation_to_budget(
                                        verbatim, budget,
                                    )
                                }
                                InputStage::Lossy => {
                    ...
                    let running_tasks: Vec<_> = pending_tasks
                        ...
                        .map(|t| {
                            let tool_name = match t.kind {
                                cf_tools::computer::types::TaskKind::Monitor => {
                                    monitor_tool_name.clone()
                                }
                                cf_tools::computer::types::TaskKind::Bash => {
        ...
        let memory_ref: Option<&dyn cf_tools::types::memory_backend::MemoryBackend> =
            if memory_opt_out {
                None
            } else {
                memory_backend_impl
                    .as_ref()
                    .map(|b| b as &dyn cf_tools::types::memory_backend::MemoryBackend)
            };
        ...
        if let Some(ref recovery_backend) = memory_backend_impl {
            ...
            if n > 0 {
                ...
                tracing::debug!(
                    target : cf_telemetry::memory_log::TARGET, count = n,
            ...
            .and_then(|item| match item {
                ConversationItem::User(parts) => {
                    parts.content.into_iter().next().and_then(|p| match p {
                        cf_sampling_types::ContentPart::Text { text } => {
        ...
        self.dispatch_hook(
            cf_hooks::event::HookEventName::PostCompact,
            cf_hooks::event::HookPayload::PostCompact {
                source: compact_source.into(),
            },
            None,
            None,
    ...
    pub(crate) fn should_auto_compact(
        ...
        if cf_token_estimation::exceeds_threshold(
            ...
            let percentage = cf_token_estimation::usage_percentage_u8(total_tokens, cw);
    ...
    pub(crate) async fn should_compact_on_error(
        &self,
        err: &cf_sampler::SamplingErrorInfo,
    ) -> bool {
        if self
            .compaction
            .auto_compact_suppressed
    ...
    pub(crate) async fn check_auto_compact_needed(&self) -> Option<AutoCompactTriggerInfo> {
        ...
        if self
            ...
            .is_ok()
        {
            let percentage = cf_token_estimation::usage_percentage_u8(estimated_total, cw);
            tracing::info!(
    ...
    pub(crate) async fn run_compact_only(
        ...
        cf_telemetry::session_ctx::log_event(cf_telemetry::events::AutoCompactFired {
        ...
        let result = self
            .run_compact_inner(
                None,
                None,
                cf_telemetry::events::CompactionTrigger::Auto,
            )
            .await;
    ...
    fn persist_compaction_request_artifact(
        ...
        tools: Vec<cf_sampling_types::ToolSpec>,
        user_context: Option<&str>,
...
mod inline_auto_compact_flow_tests {
    ...
    use cf_workspace::file_system::MockFs;
    use cf_workspace::permission::PermissionHandle;
    #[derive(Debug)]
    ...
    async fn create_test_actor(
        ...
        let hunk_tracker_handle = cf_hunk_tracker::HunkTrackerActor::spawn(
            "test-auto-compact".to_string(),
            cwd.to_path_buf(),
            hunk_tx,
            cf_hunk_tracker::TrackingMode::AgentOnly,
        ...
        SessionActor {
            ...
            compaction: crate::session::compaction_config::CompactionConfig {
                ...
                compaction_mode: cf_chat_state::CompactionMode::Transcript,
                verbatim_input: true,
            ...
            extension_registry: cf_agent_lifecycle::LocalExtensionRegistry::default(),
            ...
            vcs_kind: cf_workspace::session::git::VcsKind::Git,
            ...
            sampler_handle: cf_sampler::SamplerHandle::noop(),
            ...
            workspace_ops: cf_workspace::WorkspaceOps::for_test(),
        ...
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) =
                    mpsc::unbounded_channel::<cf_acp_lib::AcpClientMessage>();
    ...
    async fn forked_prefix_released_under_pressure_and_stays_released() {
        use crate::session::compaction_config::SUPPRESS_NONE;
        use std::sync::atomic::Ordering::Relaxed;
        use cf_test_support::MockInferenceServer;
        let local = tokio::task::LocalSet::new();
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) = mpsc::unbounded_channel();
    ...
    fn classify_suppress_reason_maps_error_text() {
        ...
        assert_eq!(
            classify("API error (status 402 Payment Required): QIDI Code usage balance exhausted"),
            SuppressReason::CreditBlock
        );
        assert_eq!(
            classify("QIDI Code usage limit reached"),
            SuppressReason::CreditBlock
        );
        assert_eq!(
    ...
    fn api_error_with_context_window(context_window: u64) -> cf_sampler::SamplingErrorInfo {
        cf_sampler::SamplingErrorInfo {
            kind: cf_sampler::SamplingErrorKind::Api,
    ...
    async fn test_compact_on_error_noop_without_model_metadata() {
        ...
        local
            .run_until(async {
                ...
                let err = cf_sampler::SamplingErrorInfo {
                    kind: cf_sampler::SamplingErrorKind::Api,
                    status_code: Some(400),
                    message: "prompt is too long".to_string(),
                    is_retryable: false,
    ...
    async fn get_transcript_path_returns_some_when_file_exists() {
        ...
        local
            .run_until(async {
                ...
                let mut actor =
                    create_test_actor(50_000, 200_000, 85, gateway_tx, persistence_tx).await;
                actor.compaction.compaction_mode = cf_chat_state::CompactionMode::Transcript;
                ...
                actor.compaction.compaction_mode = cf_chat_state::CompactionMode::Summary;
