...
pub(crate) fn new_submission(
    ...
    s.shell_version = Some(cf_version::VERSION.to_string());
    s
}

/// Pipeline: persist → strip → submit → telemetry (telemetry fields captured
/// before strip). Callers merge `QIDI_USER_METADATA` and set
...
pub(crate) async fn submit_feedback_workflow(
    ...
    if telemetry_enabled {
        ...
        cf_telemetry::session_ctx::log_event(cf_telemetry::events::UserFeedback {
...
pub struct FeedbackManagerConfig {
    ...
    /// ratings). Gated by `QIDI_FEEDBACK_ENABLED`.
    pub feedback_enabled: bool,
    /// Whether session analytics (signal sync, turn deltas) are enabled.
    /// Gated by `QIDI_TELEMETRY_ENABLED`. These are analytics data that
...
pub struct FeedbackManager {
    ...
    upload_queue_stats: std::sync::OnceLock<Arc<cf_file_utils::queue::UploadQueueStats>>,
}
...
impl FeedbackManager {
    ...
    pub fn set_upload_queue_stats(&self, stats: Arc<cf_file_utils::queue::UploadQueueStats>) {
    ...
    pub(crate) async fn submit_text_feedback(
        ...
        if let Some(user_meta) =
            crate::agent::mvp_agent::parse_json_object_env("QIDI_USER_METADATA")
        {
    ...
    pub async fn shutdown(&self, queue: Option<&cf_file_utils::queue::UploadQueue>) {
...
mod tests {
    ...
    async fn test_shutdown_with_upload_queue_drains() {
        ...
        use std::sync::Arc;
        use cf_file_utils::queue::{TraceExportSource, UploadQueue, UploadRetryPolicy};
