...
use cf_workspace::file_system::{CodebaseIndexManager, FileIndex, WalkOptions};
...
fn fs_event_to_delta(
    ...
    kind: FsEventKind,
    root: &Path,
) -> cf_workspace::file_system::FileIndexDelta {
    use cf_workspace::file_system::FileIndexDelta;
...
impl GitHead {
    ...
    async fn emit(&self) {
        let branch = cf_workspace::session::git::get_branch(&self.cwd).await;
        let commit = cf_workspace::session::git::get_current_commit(&self.cwd).await;
        let worktree = cf_workspace::session::git::get_worktree_info(&self.cwd).await;
        ...
        if changed {
            let params = cf_workspace::session::git::GitHeadChanged {
...
pub(crate) struct FsWatchPlan {
    ...
    fs_config: cf_fsnotify::FsConfig,
    cwd: PathBuf,
}
...
impl FsWatchPlan {
    pub(crate) fn build(caps: FsWatchCapabilities, deps: FsWatchDeps) -> Self {
        ...
        let hunk = (caps.hunk_tracking && deps.hunk_tracking_enabled).then(|| {
            let git_root =
                cf_workspace::session::git::find_git_root_from_path(&deps.cwd).ok();
            HunkTracking {
...
pub(crate) fn spawn(plan: FsWatchPlan) -> FsWatchHandle {
    ...
    tokio::task::spawn_local(async move {
        ...
        let source = {
            ...
            let result =
                tokio::task::spawn_blocking(move || cf_fsnotify::shared(init_cwd, fs_config))
                    .await;
            let ws = cf_fsnotify::stats();
...
mod tests {
    ...
    async fn run_rebase_cadence(
        ...
        let settle = Duration::from_millis(cf_fsnotify::SETTLE_MS);
    ...
    async fn debounce_armed_pre_op_defers_mid_op_and_fires_once_after_completed() {
        ...
        match on_event(
            FsEvent::GitMetaChanged {
                kind: cf_fsnotify::GitMetaKind::RefsChanged,
            },
            &mut in_op,
            &mut op_buffer,
        ) {
