...
/// `QIDI_GOAL_CLASSIFIER_MAX` or remote `goal_classifier_max_runs` to
/// raise it arbitrarily (only the `GOAL_CLASSIFIER_MAX_RUNS_MIN` floor
/// is enforced).
pub(crate) const GOAL_CLASSIFIER_MAX_RUNS_DEFAULT: u32 = 10;

/// Floor for `QIDI_GOAL_CLASSIFIER_MAX` / remote `goal_classifier_max_runs`.
...
/// attempt. Override via `QIDI_GOAL_VERIFIER_N` (clamped 1..=5) or the
...
/// Lower/upper bounds for `QIDI_GOAL_VERIFIER_N` / remote
...
/// (via [`cf_tool_types::format_resume_footer`]): trace tooling
...
pub(crate) fn build_subagent_trace_items(
    ...
    prompt: &str,
    output: &str,
) -> Vec<cf_sampling_types::conversation::ConversationItem> {
    use cf_sampling_types::conversation::{ConversationItem, ToolCall};
    ...
    let footer = cf_tool_types::format_resume_footer(subagent_id, subagent_type, None);
...
/// the phase via [`cf_chat_state::ChatStateHandle::flush_harness_trace_turn`]
...
pub(crate) fn record_subagent_trace(
    sink: Option<&(cf_chat_state::ChatStateHandle, String)>,
    subagent_id: &str,
    subagent_type: &str,
    description: &str,
    prompt: Option<&str>,
...
pub(crate) struct ChannelSpawner {
    ...
    pub(crate) trace_sink: Option<(cf_chat_state::ChatStateHandle, String)>,
...
mod tests {
    ...
    fn verifier_template_renders_per_agent_type_and_falls_back() {
        ...
        tool_names.insert(
            cf_tools::types::tool::ToolKind::ListDir,
            "cursor_ls".to_string(),
        );
        tool_names.insert(
            cf_tools::types::tool::ToolKind::Search,
        ...
        // cf-tools explicit render: no leftover placeholder either.
        let grok = RoleToolNames::from_summary(&summary_with(&[
            (cf_tools::types::tool::ToolKind::Read, "read_file"),
            (cf_tools::types::tool::ToolKind::ListDir, "list_dir"),
            (cf_tools::types::tool::ToolKind::Search, "grep"),
        ]))
    ...
    fn cold_and_resume_renders_are_symmetric_for_an_index() {
        let summary = summary_with(&[
            (cf_tools::types::tool::ToolKind::Read, "cursor_read"),
            (cf_tools::types::tool::ToolKind::ListDir, "cursor_ls"),
            (cf_tools::types::tool::ToolKind::Search, "cursor_grep"),
        ]);
    ...
    fn resolve_goal_verifier_count_env_clamps() {
        unsafe { std::env::set_var("QIDI_GOAL_VERIFIER_N", "0") };
        assert_eq!(
            cfg_verifier(None, None).resolve_goal_verifier_count().value,
            GOAL_VERIFIER_SKEPTIC_MIN
        );
        unsafe { std::env::set_var("QIDI_GOAL_VERIFIER_N", "99") };
        assert_eq!(
            cfg_verifier(None, None).resolve_goal_verifier_count().value,
            GOAL_VERIFIER_SKEPTIC_MAX
        );
        unsafe { std::env::set_var("QIDI_GOAL_VERIFIER_N", "garbage") };
        assert_eq!(
            cfg_verifier(None, None).resolve_goal_verifier_count().value,
            GOAL_VERIFIER_SKEPTIC_COUNT,
            "invalid env falls through to the default"
        );
        unsafe { std::env::remove_var("QIDI_GOAL_VERIFIER_N") };
    }
    ...
    fn resolve_goal_verifier_count_precedence_and_clamp() {
        ...
        // env > config.
        unsafe { std::env::set_var("QIDI_GOAL_VERIFIER_N", "2") };
    ...
    fn resolve_goal_classifier_max_runs_env_clamps_and_no_ceiling() {
        unsafe { std::env::set_var("QIDI_GOAL_CLASSIFIER_MAX", "0") };
        assert_eq!(
        ...
        unsafe { std::env::set_var("QIDI_GOAL_CLASSIFIER_MAX", "999") };
        assert_eq!(
            cfg_max_runs(None, None)
                .resolve_goal_classifier_max_runs()
                .value,
        ...
        unsafe { std::env::set_var("QIDI_GOAL_CLASSIFIER_MAX", "garbage") };
        assert_eq!(
            cfg_max_runs(None, None)
                .resolve_goal_classifier_max_runs()
                .value,
        ...
        unsafe { std::env::remove_var("QIDI_GOAL_CLASSIFIER_MAX") };
    }
    ...
    fn resolve_goal_classifier_max_runs_precedence_and_floor() {
        ...
        // env > config.
        unsafe { std::env::set_var("QIDI_GOAL_CLASSIFIER_MAX", "4") };
    ...
    fn resolve_strategist_every_default_tracks_cap_floored_at_one() {
        unsafe { std::env::remove_var("QIDI_GOAL_STRATEGIST_EVERY") };
    ...
    fn resolve_strategist_every_precedence_and_floor() {
        ...
        // env > config + remote.
        unsafe { std::env::set_var("QIDI_GOAL_STRATEGIST_EVERY", "7") };
        let r = cfg_strategist(Some(3), Some(4)).resolve_goal_strategist_every(10);
        assert_eq!(r.value, 7);
        assert_eq!(r.source, ConfigSource::Env);
        // invalid env falls through to the default (cap/2).
        unsafe { std::env::set_var("QIDI_GOAL_STRATEGIST_EVERY", "not-a-number") };
        assert_eq!(
            cfg_strategist(None, None)
                .resolve_goal_strategist_every(10)
                .value,
