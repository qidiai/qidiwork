...
pub(crate) struct RoleToolNames {
    ...
    /// (the default cf-tools host's `search_replace` mutator) when `Write`
...
impl RoleToolNames {
    ...
    /// `search_replace` on the default cf-tools host) instead of the literal
    ...
    pub(crate) fn from_summary(
        ...
        Self::from_parts(
            ...
            get(ToolKind::Search),
            // The default cf-tools host's pre-spawn describe probe exposes only
...
pub(crate) mod tests {
    ...
    fn web_tools_fall_back_when_absent_from_the_toolset() {
        ...
        // on the default cf-tools host (the stock `web_search`/`web_fetch`).
    ...
    fn from_parent_write_falls_back_to_edit_when_bridge_has_no_write() {
        // Default cf-tools parent bridge: no `Write`, only `Edit`
