//! cf-tools's L5 wiring onto the shared full-replace engine
//! (`cf_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](cf_compaction::sample_full_replace_summary);
//! this module adapts cf-tools's transport and telemetry to its two seams:
//!
...
//! [`FullReplaceError`](cf_compaction::FullReplaceError).
...
use cf_sampling_types::{ConversationItem, HostedTool, ToolSpec};
use cf_telemetry::events::{CompactionRetryDegraded, CompactionTrigger};
...
/// [`CompactionSampler`] for cf-tools's full-replace pass.
///
...
/// else the structured cf-tools prompt), so the shared `CompactionPrompt`
/// the engine passes is ignored — the engine builds the cf-tools prompt,
...
impl CompactionSampler for ShellCompactionSampler {
    ...
    async fn sample_compaction(
        ...
        // message (compat short vs structured cf-tools), ignoring the shared
...
/// Map cf-tools's [`CompactFailure`] onto the shared engine's
...
/// [`FullReplaceObserver`] that reproduces cf-tools's per-attempt telemetry:
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    cf_telemetry::session_ctx::log_event(CompactionRetryDegraded {
