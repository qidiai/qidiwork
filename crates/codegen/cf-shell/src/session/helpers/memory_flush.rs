...
/// Memory log target — matches `cf_telemetry::memory_log::TARGET`.
const LOG: &str = "cf_memory";
...
pub fn should_flush(
    ...
    let should = cf_token_estimation::exceeds_threshold_with_headroom(
        total_tokens,
        context_window,
...
pub fn is_duplicate(content: &str, db_path: &std::path::Path) -> bool {
    ...
    let conn = match cf_sqlite_journal::JournalMode::for_db_path(db_path).open_readonly(db_path) {
        Ok(c) => c,
        Err(_) => {
