...
/// prediction.
pub(crate) const DEFAULT_SUGGEST_MODEL: &str = "cf-tools-0.1";
...
/// (`cf-tools-0.1`) is API-key-only and excluded from OAuth catalogs, so
...
/// `QIDI_PROMPT_SUGGESTIONS_MODEL` keeps working for models the catalog does
...
mod tests {
    ...
    fn effective_model_default_requires_catalog() {
        ...
        // OAuth catalogs exclude cf-tools-0.1 → skip the request entirely,
    ...
    fn effective_model_env_pin_bypasses_catalog_guard() {
        // QIDI_PROMPT_SUGGESTIONS_MODEL is the explicit escape hatch: used
