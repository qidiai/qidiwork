...
pub use cf_chat_state::compaction_utils::{
...
///
/// All other agents (cf-tools, etc.) continue to use the detailed
...
pub(crate) use cf_sampling_types::is_context_length_error;
...
pub(crate) async fn generate_session_compact(
    ...
    let output = match sampling_config.api_backend {
        ApiBackend::ChatCompletions => {
            ...
            message.x_grok_agent_id = Some(cf_telemetry::id::agent_id());
            tracing::info!(
                    ...
                    Ok(chunk) => {
                        if let Some(choice) = chunk.choices.first() {
                            ...
                            if let Some(fr) = choice.finish_reason {
                                let sr = cf_sampling_types::StopReason::from(fr);
                                truncated =
                                    matches!(sr, cf_sampling_types::StopReason::Length);
        ...
        ApiBackend::Responses => {
            let request = ConversationRequest {
                ...
                x_grok_agent_id: Some(cf_telemetry::id::agent_id()),
                ..Default::default()
            };
            ...
            loop {
                ...
                match chunk_result {
                    Ok(event) => {
                        if !matches!(
                            &event,
                            cf_sampling_types::messages::MessageStreamEvent::Ping
                        ) {
                            last_progress_at = std::time::Instant::now();
                        }
                        match event {
                            cf_sampling_types::messages::MessageStreamEvent::ContentBlockDelta {
                                delta: cf_sampling_types::messages::StreamDelta::TextDelta {
                                    text,
                                },
                                ..
                            } => {
                            ...
                            cf_sampling_types::messages::MessageStreamEvent::MessageDelta {
                                delta,
                                ..
                            } => {
                                if let Some(sr) = delta.stop_reason {
                                    truncated = matches!(
                                        sr, cf_sampling_types::messages::StopReason::MaxTokens
                                        |
                                        cf_sampling_types::messages::StopReason::ModelContextWindowExceeded
                                    );
                                    stop_reason = Some(
                                        match sr {
                                            cf_sampling_types::messages::StopReason::EndTurn => {
                                                "end_turn".to_string()
                                            }
                                            cf_sampling_types::messages::StopReason::MaxTokens => {
                                                "max_tokens".to_string()
                                            }
                                            cf_sampling_types::messages::StopReason::ToolUse => {
                                                "tool_use".to_string()
                                            }
                                            cf_sampling_types::messages::StopReason::StopSequence => {
                                                "stop_sequence".to_string()
                                            }
                                            cf_sampling_types::messages::StopReason::Refusal => {
                                                "refusal".to_string()
                                            }
                                            cf_sampling_types::messages::StopReason::PauseTurn => {
                                                "pause_turn".to_string()
                                            }
                                            cf_sampling_types::messages::StopReason::ModelContextWindowExceeded => {
                                                "model_context_window_exceeded".to_string()
                                            }
                                            cf_sampling_types::messages::StopReason::Unknown(
                                                s,
                                            ) => s,
                                        },
                                    );
...
mod compacted_history_shape_tests {
    ...
    async fn test_compacted_history_raw_strings() {
        ...
        let formatted_summary =
            cf_chat_state::compaction_utils::format_compact_summary_content(compaction_summary);
        assert_eq!(
    ...
    /// Regression guard: cf-tools must DROP the working
    /// tail post-compaction. A prior change routed cf-tools to keep `recent_messages`,
    ...
    async fn qidi_build_compaction_drops_working_tail_regression_206460() {
        ...
        assert!(
            ...
            "cf-tools must drop recent_messages post-compaction",
        );
        let compacted = build_compacted_history(
            "You are a helpful assistant.",
            "<user_info>\nOS: macos\n</user_info>",
        ...
        assert!(
            !compacted
                ...
                .any(|i| matches!(i, ConversationItem::ToolResult(_))
                    || i.text_content() == "Tool call omitted..."),
            "no tail (ToolResult or stub) may leak into the cf-tools compacted history",
        );
    }
    ...
    fn fallback_minimal_history_has_no_tool_results() {
        use cf_chat_state::compaction_utils::validate_compacted_history;
