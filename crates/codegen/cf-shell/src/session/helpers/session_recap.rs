...
use cf_chat_state::{compaction_utils, estimate_conversation_tokens, estimate_item_tokens};
...
pub(crate) fn build_recap_items(
    ...
    let mut items = if strip_reasoning {
        cf_chat_state::compaction_utils::strip_reasoning_blocks(conversation)
    } else {
        conversation
    };
...
/// `max_prompt_length` for current `cf-tools` / `grok-4.5` product backends
...
/// that unlikely for normal cf-tools sessions).
///
...
mod tests {
    ...
    fn main_turn_count_counts_real_users_only() {
        use std::sync::Arc;
        use cf_sampling_types::{ContentPart, SyntheticReason, ToolCall, UserItem};
    ...
    fn mk_tool_call(id: &str, args: &str) -> cf_sampling_types::ToolCall {
        use std::sync::Arc;
