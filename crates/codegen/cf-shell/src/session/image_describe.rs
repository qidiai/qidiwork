...
use cf_chat_state::compaction_utils::{extract_real_user_queries, extract_user_query};
use cf_sampling_types::conversation::{ContentPart, ConversationItem, UserItem};
use cf_tools::util::truncate::truncate_middle;
...
impl ImageDescribeCache {
    ...
    pub async fn get_or_describe(
        &self,
        client: cf_sampler::SamplingClient,
        model: &str,
        raw_bytes: &[u8],
        mime_type: &str,
        outline: Option<&str>,
...
mod tests {
    use super::*;
    use cf_sampling_types::conversation::{ConversationItem, UserItem};
    #[test]
    ...
    fn user(text: &str) -> ConversationItem {
        ConversationItem::User(UserItem {
            content: vec![cf_sampling_types::conversation::ContentPart::Text {
                text: text.into(),
            }],
            synthetic_reason: None,
            ..Default::default()
