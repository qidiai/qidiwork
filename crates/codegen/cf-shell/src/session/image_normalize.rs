...
use bytes::Bytes;
use std::borrow::Cow;
use cf_tools::util::image_compress::{FilterType, ReEncodeParams, re_encode_under_limit};
...
/// `cf_tools::reminders` (free-fn shape because this module has no
...
pub(crate) fn persisted_image_reject_reason(bytes: &[u8]) -> Option<String> {
    ...
    use cf_tools::util::image_validate as iv;
...
pub(crate) fn inline_attach_verdict(data_b64: &str) -> InlineAttachVerdict {
    ...
    let Ok((w, h, _)) =
        cf_tools::util::image_validate::validate_image_bytes_with(&raw, false)
    else {
        return InlineAttachVerdict::Unreadable;
    };
...
fn compute_normalized_blocking(
    ...
    let (orig_w, orig_h, orig_mime) =
        cf_tools::util::image_validate::validate_image_bytes_with(&raw_bytes, false)
            .map_err(|e| NormalizeError(format!("validate: {e}")))?;
    if !cf_tools::util::image_validate::image_structurally_complete(&raw_bytes) {
    ...
    if !exceeded_size && !exceeded_dimensions {
        if let Err(e) = cf_tools::util::image_validate::validate_image_bytes(&raw_bytes) {
...
mod tests {
    ...
    fn make_ico_content(width: u32, height: u32) -> ImageContent {
        let png = make_test_png(width, height);
        let buf = cf_test_utils::image::ico_with_png_frame(&png, width as u8, height as u8);
        ImageContent::new(
    ...
    fn persisted_image_reject_reason_verdicts() {
        ...
        let ico = cf_test_utils::image::ico_with_png_frame(&make_test_png(16, 16), 16, 16);
