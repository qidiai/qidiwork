...
//! `cf_shell_session_support::managed_mcp` and is re-exported here so
//! `crate::session::managed_mcp::…` paths keep resolving unchanged.

pub use cf_shell_session_support::managed_mcp::*;
...
pub fn merge_managed_mcp_servers(
    ...
    plugin_registry: Option<&cf_agent::plugins::PluginRegistry>,
...
pub fn merge_managed_mcp_servers_with_policy(
    ...
    let allowlist = &cf_workspace::permission::resolution::managed_settings().mcp_allowlist;
...
impl McpDisabledReason {
    ...
    pub fn for_blocked_server(
        policy: &cf_workspace::permission::resolution::McpServerAllowlist,
...
fn apply_mcp_server_policy(
    ...
    allowlist: &cf_workspace::permission::resolution::McpServerAllowlist,
) -> Vec<McpServerWithPolicy> {
    merged
        .into_iter()
        .filter_map(|server| {
...
pub fn merge_managed_mcp_servers_sourced(
    ...
    let config_source = ConfigSource::ConfigToml {
        path: cf_tools::util::grok_home::grok_home().join("config.toml"),
    };
...
fn load_plugin_mcp_servers_from_value(
    ...
    let normalized = cf_agent::plugins::manifest::normalize_inline_mcp_servers(root);
...
fn load_plugin_mcp_servers_from_config(
    ...
    let sub = |s: &str| -> String {
        let s = cf_agent::plugins::manifest::substitute_env_vars(s, plugin_root, plugin_data);
...
mod tests {
    ...
    fn merge_drops_denied_server_and_classifies_as_denylist() {
        use cf_workspace::permission::resolution::{AllowedMcpServer, McpServerAllowlist};
...
"#,
    ...
    fn load_plugin_mcp_creates_stdio_server_with_env_substitution() {
        let config: crate::util::config::McpConfig = serde_json::from_value(serde_json::json!({
            "mcpServers": {
                "echo-mcp": {
                    "command": "python3",
                    "args": ["${QIDI_PLUGIN_ROOT}/mcp-echo-server.py"]
                }
            }
        }))
        ...
        let (servers, _) = load_plugin_mcp_servers_from_config(
            &config,
            "team-tool",
            "/home/user/.qidi/plugins/team-tool",
            "/home/user/.qidi/plugin-data/team-tool",
        );
        ...
        match &servers[0] {
            acp::McpServer::Stdio(acp::McpServerStdio {
                ...
                assert_eq!(
                    args.as_slice(),
                    &["/home/user/.qidi/plugins/team-tool/mcp-echo-server.py"]
                );
            }
    ...
    fn plugin_server_deduped_across_file_and_inline() {
        use cf_agent::plugins::PluginRegistry;
        use cf_agent::plugins::PluginScope;
        use cf_agent::plugins::discovery::{DiscoveredPlugin, PluginId};
        use cf_agent::plugins::manifest::{PathOrInline, PluginManifest};
        ...
        let dp = DiscoveredPlugin {
            ...
            origin: cf_agent::plugins::PluginOrigin::UserGrok,
            trusted: true,
            skill_dirs: vec![],
            command_dirs: vec![],
            agent_dirs: vec![],
    ...
    fn collect_plugin_oauth_configs_reads_byo_client_id_from_mcp_json() {
        ...
        use cf_agent::plugins::PluginScope;
        use cf_agent::plugins::discovery::{DiscoveredPlugin, PluginId};
        use cf_agent::plugins::manifest::PluginManifest;
