...
//!
//! Receives [`cf_mcp::servers::McpClientEvent`]s emitted by:
//! - per-client transport-liveness watchers
//!   ([`cf_mcp::liveness`]),
//! - the [`cf_mcp::servers::GrokClientHandler`] (server-pushed
...
pub struct McpServerStatusPayload {
    ...
    /// or `local` (user `.qidi/config.toml`).
...
pub enum McpServerStatus {
    /// Client is in [`cf_mcp::servers::ClientStateKind::Ready`]
...
/// [`cf_mcp::servers::McpClientEventKind`]). Used as the
    ...
    let (status, reason, detail) = match (kind, event) {
        ...
        (McpClientEventKind::HandshakeFailed, McpClientEvent::HandshakeFailed { reason, .. })
            if source == McpServerSource::Managed
                && cf_mcp::servers::is_auth_rejection_message(reason) =>
        {
            (
...
///
/// `gateway` is a [`cf_acp_lib::AcpAgentGatewaySender`] (forwarded
...
mod tests {
    ...
    async fn dispatcher_drops_dead_clients_on_transport_closed() {
        ...
        use cf_mcp::servers::{McpClient, McpState};
    ...
    fn discard_gateway() -> cf_acp_lib::AcpAgentGatewaySender {
        let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
        cf_acp_lib::AcpAgentGatewaySender::new(tx)
    }
    ...
    async fn run_dispatcher_stale_transport_closed_is_fully_inert() {
        ...
        let gateway = cf_acp_lib::AcpAgentGatewaySender::new(gw_tx);
