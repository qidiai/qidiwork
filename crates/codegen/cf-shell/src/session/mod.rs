...
pub use cf_fsnotify::{FsConfig, FsEvent, FsEventKind, FsEventSource, FsNotifyError, GitMetaKind};
...
pub(crate) fn is_cursor_user_template(
    _template: &cf_agent::prompt::user_message::UserMessageTemplate,
) -> bool {
    false
}
/// `false` twin of [`is_cursor_system_template`]; see [`is_cursor_user_template`].
pub(crate) fn is_cursor_system_template(
    _template: &cf_agent::prompt::context::TemplateOverride,
) -> bool {
    false
}
...
/// Client-requested fs notification mode (was cf_fsnotify::FsNotifyMode).
...
pub use cf_shared::session::info;
pub mod managed_mcp;
...
pub mod persistence;
pub use cf_shared::placeholder_images;
pub mod plan_mode;
