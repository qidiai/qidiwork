...
/// A feedback entry persisted to `~/.qidi/sessions/.../feedback.jsonl`.
///
...
pub enum PersistenceMsg {
    ...
    CurrentModel {
        ...
        /// The active agent definition name (e.g. `"cf-tools"`).
...
pub use cf_shared::session::session_dir;
...
/// Searches across ALL cwd directories under `~/.qidi/sessions/`.
///
/// Use `session_exists_for_cwd` instead when the target cwd is known
/// (e.g., the `-r` resume path) to avoid false-positive matches.
/// Find a session by ID across **all** CWD directories under `~/.qidi/sessions/`.
///
...
impl Summary {
    pub fn new(info: &Info, model_id: acp::ModelId) -> std::io::Result<Self> {
        let git_metadata =
            cf_workspace::session::git::resolve_persisted_session_git_metadata_sync(
...
/// Collect MCP server stderr logs from `~/.qidi/logs/mcp/` for inclusion in the session archive.
fn collect_mcp_stderr_logs(files: &mut Vec<CopiedSessionFile>) {
    let mcp_log_dir = cf_config::grok_home().join("logs").join("mcp");
...
/// Walk `~/.qidi/sessions/` and delete files with mtime older than `ttl_days`.
...
pub fn cleanup_stale_sessions(skip_session_dir: Option<&Path>) {
    CLEANUP_SESSIONS_ONCE.call_once(|| {
        ...
        tracing::info!(
            target: "cf_shell::session::persistence",
