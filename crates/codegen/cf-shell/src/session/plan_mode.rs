...
impl PlanModeTracker {
    ...
    /// directory (e.g., `~/.qidi/sessions/<encoded-cwd>/<session-id>/`).
...
mod tests {
    ...
    fn plan_file_in_session_dir() {
        let t = PlanModeTracker::new(PathBuf::from("/home/user/.qidi/sessions/proj/abc-123"));
        assert_eq!(
            t.plan_file_path(),
            Path::new("/home/user/.qidi/sessions/proj/abc-123/plan.md")
        );
    }
    #[test]
    ...
    /// Build a test TemplateRenderer with standard QIDI Code tool mappings.
    ...
    fn is_plan_file_write_exact_match() {
        let plan = Path::new("/home/user/.qidi/sessions/proj/abc/plan.md");
        let target = Path::new("/home/user/.qidi/sessions/proj/abc/plan.md");
