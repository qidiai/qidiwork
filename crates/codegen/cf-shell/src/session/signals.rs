...
pub use cf_telemetry::enums::PrCreationSource;
...
impl SessionSignalsHandle {
    ...
    pub fn snapshot_gcs_queue(&self, stats: &cf_file_utils::queue::UploadQueueStats) {
        ...
        let _ = self.tx.send(SignalEvent::RecordGcsQueueSnapshot {
            ...
            orphans_cleaned: cf_file_utils::queue::last_orphans_cleaned(),
        });
    }
...
pub(crate) fn merge_tightest_trigger(current: Option<String>, new: &[String]) -> Option<String> {
    cf_sampling_types::doom_loop::DoomLoopSignal::tightest(
        current
            .iter()
            .map(String::as_str)
