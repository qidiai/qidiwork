...
use cf_tools::implementations::skills::skill::format_skill_name;
...
///   matching today's QIDI Code flow that calls our dedicated `skill` tool.
...
pub(super) async fn build_skill_information_for_refs(
    ...
    use cf_tools::implementations::skills::skill::{
...
/// `qidi-code` so it stays identical to the pager's `LoopCommand` and the
