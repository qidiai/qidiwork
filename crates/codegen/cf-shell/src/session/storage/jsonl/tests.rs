...
async fn write_compaction_segment_numbers_and_indexes_resume_safely() {
    ...
    let seg = |summary: &str| CompactionSegmentFile {
        ...
        detail: cf_chat_state::CompactionDetail::Verbose,
    ...
    let base = adapter
        .session_dir(&info)
        .join(cf_chat_state::compaction_transcript::COMPACTION_DIR);
...
async fn merge_rewind_points_from_round_trips_file_snapshots() {
    use cf_paths::RelPathBuf;
    use cf_workspace::session::file_state::{FileSnapshot, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
async fn reads_never_modify_rewind_or_updates_files() {
    use cf_workspace::session::file_state::{FileStateTracker, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
async fn test_xai_session_update_round_trip() {
    ...
    let info = create_test_info();
    adapter.init_session(&info, default_model_id()).await.unwrap();
    let cf_notification = XaiSessionNotification {
    ...
    adapter
        .append_update(&info, &SessionUpdate::Xai(Box::new(cf_notification.clone())))
        .await
        .unwrap();
...
async fn copy_session_data_copies_compaction_segments_when_enabled() {
    ...
    assert!(
        ...
        .join(cf_chat_state::compaction_transcript::COMPACTION_DIR).exists()
    );
}
#[tokio::test]
...
async fn test_copy_session_data_transforms_xai_updates() {
    ...
    adapter
        .append_update(&source_info, &SessionUpdate::Xai(Box::new(cf_notification)))
        .await
        .unwrap();
    let target_info = Info {
...
async fn init_session_stamps_configured_profile_on_new_session() {
    let tmp = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(tmp.path().to_path_buf());
    cf_sandbox::set_configured_profile("workspace");
    let expected = cf_sandbox::configured_profile_name().map(String::from);
    let info = Info {
...
fn fork_filter_keeps_multi_tool_cycle_turn_with_reasoning() {
    ...
    let mut items = vec![
        ConversationItem::system("sys"), ConversationItem::user("q"),
        ConversationItem::Reasoning(cf_sampling_types::synthesized_reasoning_item("plan",)),
        ...
        ConversationItem::Reasoning(cf_sampling_types::synthesized_reasoning_item("reflect",)),
...
fn fork_filter_keeps_multi_tool_turn_with_reasoning_between_results() {
    ...
    let mut items = vec![
        ...
        ConversationItem::Reasoning(cf_sampling_types::synthesized_reasoning_item("mid")),
...
/// path end-to-end (loader + serde + cf_sampling_types::
...
/// the legacy cf-tools / Opus / chat-completions shape.
...
fn read_chat_history_upgrades_legacy_singular_reasoning_to_sibling() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"results...","reasoning":{"text":"the results are about cats","encrypted":"enc-blob","id":"rs_legacy"},"model_id":"cf-tools"}"#,
        ],
    );
    assert_eq!(
        items.len(), 5,
    ...
    match &items[3] {
        ConversationItem::Reasoning(r) => {
            ...
            let cf_sampling_types::rs::SummaryPart::SummaryText(s) = &r.summary[0];
...
fn read_chat_history_handles_hybrid_legacy_and_post_pr_lines() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"a1","reasoning":{"text":"legacy thinking","encrypted":"enc","id":"rs_legacy"},"model_id":"cf-tools"}"#,
            ...
            r#"{"type":"assistant","content":"a2","model_id":"cf-tools"}"#,
        ],
    );
    let kinds: Vec<&'static str> = items
        .iter()
    ...
    assert_eq!(
        legacy_assistant.model_id.as_deref(), Some("cf-tools"),
    ...
    let cf_sampling_types::rs::SummaryPart::SummaryText(s) = &reconstructed
        .summary[0];
...
fn read_chat_history_is_idempotent_on_post_pr_sessions() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"a","model_id":"cf-tools"}"#,
        ],
    );
    let kinds: Vec<&'static str> = items
        .iter()
...
fn read_chat_history_skips_merged_line_from_interrupted_append() {
    ...
    let good_2 = r#"{"type":"assistant","content":"after","model_id":"cf-tools"}"#;
    let raw = format!("{good_1}\n{partial}{merged_onto}\n{good_2}\n");
    let temp_dir = TempDir::new().unwrap();
