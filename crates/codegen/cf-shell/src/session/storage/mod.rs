...
mod tests {
    ...
    fn cf_envelope(session_update_json: &str) -> String {
        format!(
    ...
    #[test]
    fn cf_rewind_marker_yields_rewind_to() {
        let line = cf_envelope(
    ...
    #[test]
    fn cf_rewind_to_zero_yields_rewind_to_zero() {
        let line = cf_envelope(
    ...
    #[test]
    fn cf_diff_review_yields_not_user() {
        let line = cf_envelope(r#"{"sessionUpdate":"diff_review","content":[]}"#);
        assert_eq!(
            parse_prompt_extract_event(&line),
            PromptExtractEvent::NotUserMessage
        );
    ...
    fn from_str_unknown_xai_variant_deserializes_via_envelope() {
        ...
        let line = cf_envelope(r#"{"sessionUpdate":"git_branch_update","branch":"main"}"#);
    ...
    fn from_str_known_xai_variant_still_works() {
        let line = cf_envelope(r#"{"sessionUpdate":"memory_flush_started"}"#);
        let update = SessionUpdateEnvelope::from_str(&line).unwrap();
        match update {
            SessionUpdate::Xai(notif) => {
                assert_eq!(
