...
/// User-configurable overrides via `~/.qidi/settings.json` are deferred
/// to a follow-up.
...
fn search_db_path(root_dir: &Path) -> PathBuf {
    ...
    cf_sqlite_journal::JournalMode::for_db_path(&path).effective_db_path(&path)
}
...
mod tests {
    ...
    fn cf_update(session_update_json: &str) -> String {
        format!(
