...
///
/// Parsed from `[session_search.remote_sync]` in `~/.qidi/config.toml`.
/// Default: disabled.
...
pub async fn maybe_upload_index(
    ...
    gcs_config: cf_file_utils::TraceExportConfig,
...
impl ProxyHttpClient for crate::upload::gcs::TraceExportConfigWithAuth {
    fn proxy_http_client(&self) -> Option<reqwest::Client> {
        <Self as cf_file_utils::gcs::StorageConfig>::proxy_http_client(self)
    }
}
