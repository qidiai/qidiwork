use cf_telemetry::events::SessionHarness;
use cf_workspace::permission::Decision;
...
pub(crate) fn format_hook_name(spec: &cf_hooks::config::HookSpec) -> String {
...
/// project dirs contain `/.qidi/`.)
fn format_hook_source(spec: &cf_hooks::config::HookSpec) -> &'static str {
...
impl HookRegInfo {
    pub(crate) fn from_spec(spec: &cf_hooks::config::HookSpec) -> Self {
        Self {
...
pub(crate) struct SessionHarnessMetrics {
    ...
    pub permission_mode: cf_telemetry::enums::PermissionMode,
    pub mcp_server_names: Vec<String>,
    pub lsp_server_names: Vec<String>,
    pub memory_enabled: bool,
    pub auto_update: Option<bool>,
    pub cwd: String,
    pub skills_config: cf_agent::prompt::skills::SkillsConfig,
    /// Resolved vendor-compat config, so recorded skill / AGENTS.md names match
    /// what the session actually discovers.
    pub compat: cf_tools::types::compat::CompatConfig,
...
impl SessionHarnessMetrics {
    pub async fn into_event(self, hooks: Vec<HookRegInfo>) -> SessionHarness {
        ...
        let agents_md_dir_names = cf_agent::prompt::agents_md::read_agents_config_with_paths(
            &self.cwd,
            self.compat,
        )
        .await
        ...
        .collect();
        let skill_names = cf_agent::prompt::skills::list_skills_with_plugins(
            Some(&self.cwd),
            &self.skills_config,
        ...
        SessionHarness {
            ...
            is_git_repo: cf_telemetry::context::collect_git_context(&self.cwd).is_git_repo,
