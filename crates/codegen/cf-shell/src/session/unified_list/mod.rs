...
pub fn conversations_lane_enabled() -> bool {
    ...
    std::env::var("QIDI_SESSION_LIST_CONVERSATIONS")
        .ok()
        .is_some_and(|v| {
            !matches!(
...
/// Env lane (desktop `QIDI_SESSION_LIST_CONVERSATIONS`) OR process-wide
/// `--chat` (`QIDI_CHAT_MODE`); hard-off in release builds.
...
mod tests {
    ...
    fn cf_auth_manager(dir: &std::path::Path) -> std::sync::Arc<crate::auth::AuthManager> {
        ...
        am.hot_swap(crate::auth::GrokAuth {
            ...
            oidc_issuer: Some(crate::auth::cf_oauth2_issuer().to_owned()),
    ...
    async fn forced_kind_serves_conversations_only() {
        ...
        .await;
        let _env = cf_test_support::EnvGuard::set(
            "QIDI_CONVERSATIONS_BASE_URL",
            format!("http://{addr}"),
        );
        let home = tempfile::tempdir().expect("tempdir");
        let client = ConversationsClient::new(cf_auth_manager(home.path()));
        let mut req = ListReq {
            meta: Some(serde_json::json!({ "x.ai/facetFilters" : { "kind" : ["build"] }, })),
            ..ListReq::default()
        };
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = cf_test_support::EnvGuard::unset("QIDI_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on = cf_test_support::EnvGuard::set("QIDI_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off = cf_test_support::EnvGuard::set("QIDI_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        use crate::agent::chat_modes::QIDI_CHAT_MODE_ENV;
        let _chat_off = cf_test_support::EnvGuard::unset(QIDI_CHAT_MODE_ENV);
        let _desktop_off =
            cf_test_support::EnvGuard::unset("QIDI_SESSION_LIST_CONVERSATIONS");
        assert!(
        ...
        {
            let _desktop =
                cf_test_support::EnvGuard::set("QIDI_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_active(), false);
        }
        {
            let _chat = cf_test_support::EnvGuard::set(QIDI_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        .to_string();
        {
            let _off = cf_test_support::EnvGuard::unset(QIDI_CHAT_MODE_ENV);
        ...
        {
            let _on = cf_test_support::EnvGuard::set(QIDI_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
