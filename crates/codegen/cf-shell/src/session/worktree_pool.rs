...
//!   under `~/.qidi/worktree_pool/<instance_id>/` with a `.pid` liveness
...
use cf_fast_worktree::{WorktreeBuilder, WorktreeSync};

const WORKTREE_POOL_LOG: &str = "cf_worktree_pool";
use crate::util::config::PoolConfig;
use crate::util::grok_home::grok_home;

use cf_tty_utils::git_command;
...
/// `~/.qidi/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.qidi/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.qidi/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.qidi/worktree_pool/`, reads each `.pid` file, and checks
...
async fn warm_source_repo(repo_path: &Path) {
    ...
    let _ = tokio::task::spawn_blocking(move || {
        ...
        for &(key, val) in cf_tty_utils::GIT_AUTH_SUPPRESSION_ENVS.iter() {
            cmd.env(key, val);
        }
        let result = cmd
...
/// The base pool directory under `~/.qidi/`.
...
/// `CleanAll` mode. Placed under `~/.qidi/worktree_pool/<instance_id>/<uuid>/`.
async fn create_pooled_worktree(
    ...
    let result = tokio::task::spawn_blocking(move || {
        WorktreeBuilder::new(&source, &dest_clone)
            .creation_mode(cf_fast_worktree::CreationMode::GitCheckout)
            .parallelism(parallelism)
            .worktree_kind(cf_fast_worktree::WorktreeKind::Pool)
            .create()
    })
    .await?;
...
fn schedule_cleanup(path: PathBuf) {
    ...
    tokio::task::spawn(async move {
        ...
        if path.exists() {
            ...
            let result = tokio::task::spawn_blocking(move || {
                cf_fast_worktree::remove_worktree(&path_clone)
            })
            .await;
...
async fn remove_worktree_registration(path: &Path) {
    ...
    let result =
        tokio::task::spawn_blocking(move || cf_fast_worktree::remove_worktree(&path_owned)).await;
