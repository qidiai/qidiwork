...
/// Path format: `~/.qidi/sessions/{session_id}/tasks/{task_id}.log`
...
pub struct BackgroundTaskManifestEntry {
    ...
    pub cwd: String,
    #[serde(default)]
    pub kind: cf_tools::computer::types::TaskKind,
}
...
pub fn format_resumed_tasks_reminder(entries: &[BackgroundTaskManifestEntry]) -> String {
    ...
    for entry in entries {
        ...
        let kind_label = match entry.kind {
            cf_tools::computer::types::TaskKind::Monitor => " [monitor]",
...
mod tests {
    ...
    fn format_reminder_labels_monitor_tasks() {
        ...
        entry.kind = cf_tools::computer::types::TaskKind::Monitor;
        let reminder = format_resumed_tasks_reminder(&[entry]);
