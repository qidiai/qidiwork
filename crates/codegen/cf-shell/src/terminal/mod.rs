...
/// Resolved absolute path to bash. On Unix uses the `cf_config` shell
/// resolution cascade (`$QIDI_SHELL` > `$SHELL` > `which` > common dirs >
...
pub fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        cf_config::shell::unix_shell_path(cf_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `qidi-code`.
pub use cf_tools::util::pager_env;
...
/// qidi-code which renders its own UI and doesn't need raw ANSI codes).
///
