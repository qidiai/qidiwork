...
/// after we stop waiting. Mirrors the bound the `qidi-code` local terminal
...
fn notification_interval() -> Duration {
    std::env::var("QIDI_TERMINAL_NOTIFICATION_INTERVAL_MS")
        .ok()
...
#[async_trait::async_trait]
impl SessionNotificationSender for cf_acp_lib::AcpAgentGatewaySender {
    async fn session_notification(
        &self,
        notification: acp::SessionNotification,
    ) -> Result<(), acp::Error> {
