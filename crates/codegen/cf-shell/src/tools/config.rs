...
impl BashToolConfig {
    ...
    pub fn to_bash_params_json(
        ...
        // production cf-tools opts up to 10h by sending it explicitly
...
pub struct WebFetchToolConfig {
    ...
    /// this URL. Resolution: TOML > `QIDI_WEB_FETCH_PROXY` env > remote settings > None.
...
impl WebFetchToolConfig {
    ...
    pub fn resolve_params(
        ...
        let proxy_endpoint = self
            ...
            .as_ref()
            .cloned()
            .or_else(|| env_string("QIDI_WEB_FETCH_PROXY"))
...
/// from `cf_tools::registry::types::ToolsetConfig` which holds
...
impl ShellToolsetConfig {
    ...
    pub fn new(base: Option<Self>, sampling_config: Option<SamplerConfig>) -> Self {
        let default_base = SamplerConfig {
            api_key: None,
            base_url: "https://api.qidiai.ltd/v1".to_string(),
            model: String::new(),
...
mod tests {
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            cf_agent::config::AgentDefinition::plan(),
            cf_agent::config::AgentDefinition::explore(),
        ] {
    ...
    fn max_timeout_secs_defaults_to_production_10h() {
        ...
        assert_eq!(
            ...
            "production cf-tools must set the 10h foreground ceiling"
        );
    }
