//! Notification bridge: translates `qidi-code` `ToolNotification` events
//! into `qidi-code`'s native systems (ACP gateway, hunk tracker, file state tracker).
...
use cf_tools::types::output::{BashOutput, ToolOutput};
...
use cf_workspace::session::file_state::FileStateTracker;
...
pub struct NotificationBridgeConfig {
    ...
    pub auto_wake_delivered: cf_tools::reminders::task_completion::AutoWakeDeliveredIds,
...
async fn emit_current_mode_update(
    ...
    mode: cf_tools::types::SessionMode,
) {
...
async fn handle_notification(
    ...
    match notification {
        ...
        ToolNotification::TaskCompleted(task_snapshot) => {
            let is_monitor =
                task_snapshot.kind == cf_tools::computer::types::TaskKind::Monitor;
            ...
            if task_snapshot.block_waited || task_snapshot.explicitly_killed {
                ...
                let body = if is_monitor {
                    cf_tools::reminders::task_completion::format_monitor_completion(
                        &task_snapshot,
                        tool_name,
                    )
                } else {
                    cf_tools::reminders::task_completion::format_bash_completion(
                        &task_snapshot,
                        tool_name,
                        read_name,
                    )
                };
                let message = cf_tools::reminders::wrap_reminder(&body);
        ...
        ToolNotification::PlanModeEntered(entered) => {
            ...
            if activated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
                ...
                emit_current_mode_update(config, cf_tools::types::SessionMode::Plan).await;
            }
            tracing::info!(
        ...
        ToolNotification::PlanModeExited(exited) => {
            ...
            if deactivated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Agent;
                ...
                emit_current_mode_update(config, cf_tools::types::SessionMode::Default).await;
            }
            tracing::info!(
...
mod tests {
    use super::*;
    use cf_tools::computer::types::TaskKind;
    use cf_tools::types::TaskSnapshot;
    ...
    fn make_test_config_full() -> (
        NotificationBridgeConfig,
        mpsc::UnboundedReceiver<cf_acp_lib::AcpClientMessage>,
        mpsc::UnboundedReceiver<PersistenceMsg>,
        mpsc::UnboundedReceiver<SessionCommand>,
    ) {
        let (gateway_tx, gateway_rx) = mpsc::unbounded_channel();
        let gateway = cf_acp_lib::AcpAgentGatewaySender::new(gateway_tx);
    ...
    fn task_completed_will_wake(
        gateway_rx: &mut mpsc::UnboundedReceiver<cf_acp_lib::AcpClientMessage>,
    ) -> Option<bool> {
        while let Ok(msg) = gateway_rx.try_recv() {
            if let cf_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                && args.request.method.as_ref() == "x.ai/task_completed"
    ...
    async fn monitor_event_skipped_after_task_completed_auto_wake() {
        ...
        handle_notification(
            &config,
            ToolNotification::MonitorEvent(cf_tools::notification::types::MonitorEvent {
    ...
    async fn scheduled_task_created_is_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskCreated(
            cf_tools::notification::types::ScheduledTaskCreated {
                task_id: "loop-1".into(),
    ...
    async fn bash_output_chunk_persists_and_broadcasts_one_event_id() {
        ...
        let notification = ToolNotification::BashOutputChunk(
            cf_tools::notification::types::BashOutputChunk {
                base: cf_tools::notification::types::BashNotificationBase {
        ...
        let broadcast_id = match gateway_rx.try_recv().expect("chunk must be broadcast") {
            cf_acp_lib::AcpClientMessage::SessionNotification(args) => args
                .request
                .meta
                .as_ref()
                .and_then(|m| m.get("eventId"))
    ...
    async fn scheduled_task_removed_is_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskRemoved(
            cf_tools::notification::types::ScheduledTaskRemoved {
                task_id: "loop-1".into(),
            },
        );
        let mut offsets = HashMap::new();
        ...
        match msg {
            PersistenceMsg::Update(crate::session::storage::SessionUpdate::Xai(notif)) => {
                ...
                assert!(
                    cf_persisted_event_id(&notif).is_some(),
    ...
    async fn task_backgrounded_persisted_line_is_stamped() {
        ...
        let notification = ToolNotification::BashExecutionBackgrounded(
            cf_tools::notification::types::BashExecutionBackgrounded {
                base: cf_tools::notification::types::BashNotificationBase {
        ...
        match persistence_rx.try_recv().expect("must persist") {
            PersistenceMsg::Update(crate::session::storage::SessionUpdate::Xai(notif)) => {
                assert!(cf_persisted_event_id(&notif).is_some());
            }
    ...
    async fn scheduled_task_fired_is_not_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskFired(
            cf_tools::notification::types::ScheduledTaskFired {
                task_id: "loop-1".into(),
                prompt: "check deploy".into(),
                human_schedule: "every 5 minutes".into(),
                next_fire_at: Some("2026-01-01T00:00:00Z".into()),
    ...
    async fn legacy_monitor_event_without_owner_is_injected() {
        // Legacy / non-cf-tools backends record no owner; such events must
    ...
    async fn plan_mode_exited_emits_current_mode_update_default() {
        ...
        *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
        ...
        let notification =
            ToolNotification::PlanModeExited(cf_tools::notification::types::PlanModeExited {
        ...
        while let Ok(msg) = gateway_rx.try_recv() {
            if let cf_acp_lib::AcpClientMessage::SessionNotification(args) = msg
    ...
    async fn plan_mode_entered_emits_current_mode_update_plan() {
        ...
        let notification = ToolNotification::PlanModeEntered(
            cf_tools::notification::types::PlanModeEntered {
