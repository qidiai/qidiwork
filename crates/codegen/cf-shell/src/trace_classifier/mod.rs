...
/// two-function shape in `qidi-code` (F29).
...
/// `qidi-code` — existing items keep their prior content when
...
/// binary. Wraps a `cf_sampler::SamplingClient` and pulls the
/// text content out of the response.
pub struct SamplerClassifierClient {
    inner: cf_sampler::SamplingClient,
}

impl SamplerClassifierClient {
    pub fn new(inner: cf_sampler::SamplingClient) -> Self {
        Self { inner }
    }
}
...
pub struct RunArgs {
    ...
    /// `cf_shell::util::grok_home::grok_home()` when `None`.
...
/// provider command (if `QIDI_AUTH_PROVIDER_COMMAND` is set) is
...
async fn build_sampler_client(
...
) -> Result<cf_sampler::SamplingClient> {
    let default_home;
    ...
    let config = cf_sampler::SamplerConfig {
        api_key: Some(resolved),
        base_url,
        model,
        max_completion_tokens: Some(LAZINESS_MAX_OUTPUT_TOKENS),
        ..cf_sampler::SamplerConfig::default()
    };
    cf_sampler::SamplingClient::new(config).map_err(|e| anyhow!("build SamplingClient: {e}"))
}
...
mod tests {
    use super::*;
    use cf_sampling_types::{AssistantItem, ToolCall, ToolResultItem};
    ...
    async fn classifier_request_matches_production_shape() {
        ...
        assert_eq!(
            ...
            Some(cf_telemetry::id::agent_id().as_str())
        );
    }
    ...
    /// * `QIDI_AUTH` — inline-JSON credentials override that bypasses
    ///   the on-disk read entirely (`AuthManager::new`).
    /// * `QIDI_AUTH_PATH` — overrides the auth.json path; if set to
    ///   the operator's real `~/.qidi/auth.json`, the test would read
    ///   live OIDC credentials instead of the scratch fixture.
    /// * `QIDI_AUTH_PROVIDER_COMMAND` — selects an external
