...
//! to an `cf_file_utils::gcs::*` helper.
...
/// shell-side adapter that lets `cf_file_utils::gcs::*` helpers wire
...
///
/// ```ignore
/// cf_file_utils::gcs::upload_bytes(
...
/// `QIDI_TELEMETRY_GCS_BUCKET`; `None` disables trace uploads until a bucket
/// is configured.
pub(crate) const SESSION_TRACES_BUCKET: Option<&str> =
    option_env!("QIDI_SESSION_TRACES_BUCKET_DEFAULT");
...
pub(crate) async fn upload_to_auth_diagnostics(
    ...
    match cf_file_utils::gcs::upload_bytes(
