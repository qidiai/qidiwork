...
use url::Url;
use cf_file_utils::queue::{EnqueueOutcome, TraceExportSource, UploadQueue, UploadRetryPolicy};
use cf_workspace::permission::PermissionEvent;
...
pub(crate) fn spawn_trace_upload<T: Serialize + Send + 'static>(
    ...
    tokio::spawn(async move {
        ...
        let ok = cf_file_utils::gcs::upload_bytes(
            &gcs_config,
            &object_path,
            &bytes,
            "application/json",
...
fn record_upload_failure(ctx: &PromptTraceContext, f: UploadFailure<'_>) {
    ...
    if level == UploadFailureLogLevel::Warn {
        cf_telemetry::unified_log::warn(&msg, sid, log_ctx);
    } else {
        cf_telemetry::unified_log::error(&msg, sid, log_ctx);
    }
}
...
pub(crate) fn local_sandbox_telemetry() -> Option<LocalSandboxTelemetry> {
    let profile = cf_sandbox::configured_profile_name()?;
    Some(LocalSandboxTelemetry {
        profile: profile.to_owned(),
        applied: cf_sandbox::is_active(),
    })
}
...
fn classify_workspace(cwd: &str) -> String {
    ...
    if path.ancestors().any(|p| p.join(".git").exists()) {
        "git".to_owned()
    } else if cf_file_utils::workspace_classifier::is_project_dir(path) {
        "project".to_owned()
    } else {
...
pub(crate) async fn upload_subagent_metadata(
    ...
    if let Err(e) =
        cf_file_utils::gcs::upload_bytes(&config, &gcs_path, &json, "application/json").await
    {
        tracing::warn!(
...
pub(crate) async fn upload_plugin_state(
    ctx: &PromptTraceContext,
    registry: Option<&cf_agent::plugins::PluginRegistry>,
) {
    use cf_agent::plugins::discovery::PluginScope;
...
use cf_file_utils::gcs::upload_bytes;
...
pub(crate) async fn upload_artifact_to_gcs(
    ...
    match upload_bytes(&config, gcs_path, content, content_type).await {
        ...
        Err(e) => {
            let status_code = e
                .downcast_ref::<cf_file_utils::storage_client::HttpUploadError>()
                .map(|e| e.status_code);
...
pub(crate) async fn upload_unified_log(ctx: &PromptTraceContext, wait: UploadWait) {
    ...
    let log_bytes = match tokio::task::spawn_blocking(move || {
        cf_telemetry::unified_log::snapshot_session_log(&session_id)
    })
    .await
    {
        Ok(Some(bytes)) => bytes,
    ...
    .await;
    let full_log_bytes =
        tokio::task::spawn_blocking(cf_telemetry::unified_log::snapshot_log).await;
    let user_id = ctx
        .auth_manager
        .current_or_expired()
        .map(|a| a.user_id)
...
pub(crate) async fn upload_turn_messages(
    ctx: &PromptTraceContext,
    _capture: cf_chat_state::TurnCapture,
    _wait: UploadWait,
) -> bool {
    super::manifest::skip_artifact(
        &ctx.artifact_tracker,
...
pub(crate) fn build_chat_history_session_state(
    _messages: &[cf_sampling_types::conversation::ConversationItem],
...
impl TraceExportSource for DynamicResolver {
    ...
    fn proxy_attribution(
        &self,
    ) -> Option<Arc<dyn cf_file_utils::storage_client::Auth401AttributionCallback>> {
        cf_file_utils::gcs::StorageConfig::proxy_attribution(&self.with_auth())
    }
    fn proxy_credentials(&self) -> Option<Arc<dyn cf_auth::AuthCredentialProvider>> {
        cf_file_utils::gcs::StorageConfig::proxy_credentials(&self.with_auth())
    }
    fn proxy_http_client(&self) -> Option<reqwest::Client> {
        cf_file_utils::gcs::StorageConfig::proxy_http_client(&self.with_auth())
    }
...
pub(crate) fn spawn_startup_spill_reconcile(
    ...
    tokio::spawn(async move {
        match queue {
            Some(queue) => {
                let report =
                    cf_workspace::recovery::run_startup_recovery(&grok_home, &queue).await;
                tracing::info!(?report, "startup spill recovery complete");
                queue.cleanup_orphans(cf_file_utils::queue::DEFAULT_MAX_AGE);
            }
            None => {
                let purged = tokio::task::spawn_blocking(move || {
                    cf_workspace::recovery::purge_spilled_items(&grok_home)
                })
                .await;
                match purged {
                    Ok(removed) => {
...
pub(crate) async fn upload_trace_artifact_blocking(
    ...
    let queue_result = if let Some(queue) = &ctx.upload_queue {
            ...
            .await
        {
            ...
            Err(e)
                if e.downcast_ref::<cf_file_utils::queue::QueueClosed>()
                    .is_some() =>
            {
                tracing::debug!(
...
mod tests {
    ...
    fn dynamic_resolver_supplies_proxy_credentials_and_attribution() {
        use crate::session::repo_changes::UploadMethod;
        use cf_file_utils::queue::TraceExportSource;
        let dir = tempfile::tempdir().unwrap();
        let auth_manager = Arc::new(crate::auth::AuthManager::new(
            dir.path(),
    ...
    fn home_project_dir() -> Option<tempfile::TempDir> {
        ...
        if !cf_file_utils::workspace_classifier::is_project_dir(&home.join("probe")) {
            return None;
        }
