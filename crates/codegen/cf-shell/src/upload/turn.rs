...
pub(crate) use cf_telemetry::session_metrics::TraceUploadReason;
...
pub(crate) struct PromptTraceContext {
    ...
    pub(crate) upload_queue: Option<cf_file_utils::queue::UploadQueue>,
...
pub(crate) async fn complete_prompt_trace(
    ...
    >,
    turn_messages: Option<cf_chat_state::TurnCapture>,
...
pub(crate) fn parse_agent_profile_from_meta(
...
) -> Option<cf_agent::AgentDefinition> {
    let value = meta?.get("agentProfile")?;
    if value.is_object() {
        return match cf_agent::AgentDefinition::from_json(value) {
            Ok(def) => {
                tracing::info!(
                    agent_name = % def.name,
    ...
    if let Some(name) = value.as_str() {
        ...
        return cf_agent::discovery::by_name(name);
    }
    tracing::warn!(
...
mod tests {
    ...
    fn parse_ask_user_question_returns_none_when_absent() {
        let meta = serde_json::json!({ "agentProfile" : "cf-tools-plan" });
