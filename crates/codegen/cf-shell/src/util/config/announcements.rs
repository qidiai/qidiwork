...
pub use cf_announcements::RemoteAnnouncement;
...
/// Dev/test override for announcements via `QIDI_ANNOUNCEMENTS_OVERRIDE` (a JSON
...
pub fn announcements_override() -> Option<Vec<RemoteAnnouncement>> {
    let raw = std::env::var("QIDI_ANNOUNCEMENTS_OVERRIDE").ok()?;
    match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
        Ok(list) => Some(list),
        Err(_) => {
            tracing::warn!("invalid QIDI_ANNOUNCEMENTS_OVERRIDE JSON; ignoring");
            None
        }
    }
}
...
/// `QIDI_ANNOUNCEMENTS_OVERRIDE` env var overrides everything (dev-only escape hatch).
