...
pub fn load_relay_sync_enabled_sync() -> bool {
    toml_bool_sync(Some("QIDI_RELAY_SYNC_ENABLED"), "relay", "enabled")
}
...
/// 1. Nearest project `.qidi/config.toml` with `[permission]` section (from cwd upward)
/// 2. Global `~/.qidi/config.toml` `[permission]` section
///
