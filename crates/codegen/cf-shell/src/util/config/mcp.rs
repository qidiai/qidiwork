...
use cf_tools::types::compat::{CompatConfig, CompatConfigToml};

pub use cf_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
...
pub use cf_config::xai_grok_config_types::{McpJsonOAuthBlock, McpServerConfig, McpServerTransportConfig};
...
pub use cf_config::xai_grok_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use cf_config::xai_grok_config_types::PoolConfig;


/// Convert a cf-config McpServerConfig into an acp::McpServer.
/// Returns None if the server is disabled.
pub fn to_acp_mcp_server(config: &McpServerConfig, name: &str) -> Option<acp::McpServer> {
    if !config.enabled {
        return None;
    }

    let acp_server = match &config.transport {
        cf_config::xai_grok_config_types::McpServerTransportConfig::Stdio {
            command,
            args,
            env,
            ..
        } => {
            let env_vars: Vec<acp::EnvVariable> = env
                .iter()
                .flatten()
                .map(|(k, v)| acp::EnvVariable::new(k.clone(), v.clone()))
                .collect();

            let mut server = acp::McpServerStdio::new(name.to_string(), command);
            server.args = args.clone();
            server.env = env_vars;
            acp::McpServer::Stdio(server)
        }
        cf_config::xai_grok_config_types::McpServerTransportConfig::StreamableHttp {
            url,
            headers,
            ..
        } => {
            let http_headers: Vec<acp::HttpHeader> = headers
                .iter()
                .flatten()
                .map(|(k, v)| acp::HttpHeader::new(k.clone(), v.clone()))
                .collect();

            let mut server = acp::McpServerHttp::new(name.to_string(), url);
            server.headers = http_headers;
...
/// Walks from cwd up to the git repo root checking `.qidi/config.toml` at each level.
/// Project-scoped `.qidi/config.toml` entries override global `~/.qidi/config.toml`
...
/// `.qidi/config.toml`, otherwise user (global config, `~/.claude.json`,
...
pub fn load_mcp_servers_with_oauth(
    ...
    for (name, mut config) in servers_map {
        ...
        if let Some(acp_server) = to_acp_mcp_server(&config, &name) {
...
/// Load MCP servers with project-scoped overrides from `.qidi/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.qidi/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.qidi/config.toml` at each level
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .qidi/config.toml"
                );
...
///
/// Uses the top-level `disabled_mcp_servers` array in `~/.qidi/config.toml`.
...
/// Upsert an MCP server entry in `~/.qidi/config.toml`.
///
...
/// config file, e.g. a project-scoped `.qidi/config.toml`.
...
/// Delete an MCP server entry from `~/.qidi/config.toml`.
///
...
/// config file, e.g. a project-scoped `.qidi/config.toml`. OAuth credential
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = cf_mcp::credentials::McpCredentialStore::load_default() {
...
/// project-scoped `.qidi/config.toml` files.
...
// `.mcp.json` discovery moved to `qidi-code` (client-side, shared with
// the folder-trust gate); re-exported so `crate::util::config::*` paths keep working.
pub use cf_workspace::project_config::{
...
pub(crate) fn parse_mcp_config_with_oauth(
    ...
    for (name, server_config) in &config.mcp_servers {
        ...
        if let Some(server) = to_acp_mcp_server(&server_config, &name.clone()) {
            servers.push(server);
        } else {
            tracing::warn!(
                source = source_label,
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.qidi/config.toml` files from `cwd` up to the
...
/// Path to the user-level config file (`~/.qidi/config.toml`).
pub fn user_config_path() -> PathBuf {
    config_path()
}

/// Path to a project-level config file (`<dir>/.qidi/config.toml`).
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.qidi/skills", "~/.qidi/skills/special/SKILL.md"]
ignore = ["~/.qidi/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.qidi/skills", "~/.qidi/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.qidi/skills/noisy/SKILL.md"]);
    }
    ...
    fn mcp_json_env_var_default_value() {
        ...
        std::fs::write(
            ...
            r#"{
                "mcpServers": {
                    "api": {
                        "url": "${QIDI_TEST_MCP_UNSET_VAR_12345:-https://fallback.example.com}/mcp"
                    }
                }
            }"#,
        )
