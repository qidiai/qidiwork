...
/// to work.
pub use cf_telemetry::enums::PermissionMode;
...
/// `qidi-code`.
use cf_workspace::permission::resolution::yolo_disabled_by_policy;
...
mod tests {
    ...
    fn effective_auto_for_launch_cli_auto_not_yolo() {
        ...
        unsafe { std::env::set_var("QIDI_AUTO_PERMISSION_MODE", "1") };
        ...
        unsafe { std::env::remove_var("QIDI_AUTO_PERMISSION_MODE") };
    }
    ...
    /// With the gate forced OFF (`QIDI_AUTO_PERMISSION_MODE=0`), explicit
    ...
    fn effective_auto_for_launch_inert_when_gate_off() {
        let _g = crate::util::config::resolve::AUTO_PERMISSION_MODE_ENV_LOCK
            .lock()
            .unwrap_or_else(|p| p.into_inner());
        unsafe { std::env::set_var("QIDI_AUTO_PERMISSION_MODE", "0") };
        assert!(
    ...
    // definition in `cf_workspace::permission::claude_compat`.

    #[test]
    fn resolve_launch_yolo_policy_pin_neutralizes_requested_bypass() {
        let warning = cf_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
