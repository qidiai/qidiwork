...
pub(crate) const ENV_AUTO_COMPACT_THRESHOLD_PERCENT: &str = "QIDI_AUTO_COMPACT_THRESHOLD_PERCENT";
...
///   1. env `QIDI_AUTO_COMPACT_THRESHOLD_PERCENT`
...
const ENV_COMPACTION_WALL_CLOCK_BUDGET_SECS: &str = "QIDI_COMPACTION_WALL_CLOCK_SECS";

/// Resolve the per-compaction wall-clock budget (seconds). Precedence: env
/// `QIDI_COMPACTION_WALL_CLOCK_SECS` > remote settings global
...
mod compaction_wall_clock_budget_tests {
    ...
    // Assumes QIDI_COMPACTION_WALL_CLOCK_SECS is unset in the test env.
    #[test]
