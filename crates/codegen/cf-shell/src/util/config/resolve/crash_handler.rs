...
pub(crate) const ENV_CRASH_HANDLER: &str = "QIDI_CRASH_HANDLER";
...
/// Precedence: requirements > env (`QIDI_CRASH_HANDLER`) >
...
fn load_managed_toml_layers() -> Option<TomlValue> {
    ...
    match (system, managed) {
        ...
        (None, Some(m)) => Some(m),
        (Some(mut s), Some(m)) => {
            cf_config::deep_merge_toml(&mut s, &m);
            Some(s)
        }
    }
}
...
mod crash_handler_gate_tests {
    ...
    // `QIDI_CRASH_HANDLER` is process-global; serialize and force it unset at
