...
/// [`cf_config::env_bool`] parser (`QIDI_TOOLS_FIND_BFS` /
/// `QIDI_TOOLS_GREP_UGREP`, plus the `QIDI_FIND_BFS` / `QIDI_GREP_UGREP` aliases).
///
/// Pass the **merged** requirements ([`crate::config::load_merged_requirements`])
/// so an org policy in any requirements layer — not only
/// `~/.qidi/requirements.toml` — is honored. Returns `(find_bfs, grep_ugrep)`,
/// which the caller bakes into a
/// [`cf_tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
pub fn resolve_search_tools_enabled(
    ...
    let disable = cf_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    ...
    let resolve = |primary: &str, alias: &str, key: &str| -> bool {
        let env = cf_config::env_bool(primary).or_else(|| cf_config::env_bool(alias));
    ...
    (
        resolve("QIDI_TOOLS_FIND_BFS", "QIDI_FIND_BFS", "find_bfs"),
        resolve("QIDI_TOOLS_GREP_UGREP", "QIDI_GREP_UGREP", "grep_ugrep"),
    )
}
...
const ENV_PERSISTENT_SHELL: &str = "QIDI_PERSISTENT_SHELL";
...
mod persistent_local_shell_tests {
    ...
    // QIDI_PERSISTENT_SHELL is process-global (the documented kill-switch a dev
...
/// the shared [`cf_config::env_bool`] via `BoolFlag`). The secs env var
/// lives in the tools crate (`RESPONSE_TIMEOUT_ENV`), parsed once there.
const ENV_ASK_USER_QUESTION_TIMEOUT_ENABLED: &str = "QIDI_ASK_USER_QUESTION_TIMEOUT_ENABLED";
...
///
/// Precedence: requirements > env (`QIDI_ASK_USER_QUESTION_TIMEOUT_ENABLED`)
...
///
/// Precedence: requirements > env (`QIDI_ASK_USER_QUESTION_TIMEOUT_SECS`,
...
mod tests {
    ...
    // Assumes QIDI_TOOLS_* / DISABLE_EMBEDDED_SEARCH_TOOLS are unset in the test env.
    #[test]
