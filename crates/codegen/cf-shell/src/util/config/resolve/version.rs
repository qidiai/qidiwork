...
///
/// Reads `stable_version` from `~/.qidi/version.json` (written by the
...
///
/// This is a lightweight duplicate of `cf_update::channel_name()` for
/// use in `qidi-code` which cannot depend on `xai-grok-update`.
pub fn channel_name_from_cache() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        let current = semver::Version::parse(cf_version::VERSION).ok()?;
