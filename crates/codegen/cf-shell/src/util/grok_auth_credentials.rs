...
/// User token (xAI users) sends `Bearer` + `X-XAI-Token-Auth: qidi-code`.
...
impl GrokAuthCredentials {
    ...
    pub fn auth_error_hint(&self) -> &'static str {
        if self.deployment_key.is_some() {
            "Your QIDI_DEPLOYMENT_KEY is invalid or expired. Please contact a team admin."
        } else if self.user_token.is_some() {
            "Your auth token is invalid or expired. Run `qidiwork login` to re-authenticate."
        } else {
            "Not authenticated."
        }
    }
...
impl cf_auth::HttpAuth for GrokAuthCredentials {
