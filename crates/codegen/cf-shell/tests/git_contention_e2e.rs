...
//!
//! Knobs: QIDI_PERF_GIT_FILES (default 300), QIDI_PERF_GIT_PICKS (default 6).
...
use cf_test_support::{MockInferenceServer, ScriptedResponse, SseEvent};

const DUPLEX_BUFFER_BYTES: usize = 8 * 1024 * 1024;

use cf_test_utils::env::env_usize;

/// Run a git command with deterministic author/committer; assert success.
fn git(dir: &Path, args: &[&str]) -> String {
    cf_test_utils::git::run_git(dir, args)
}

// ── scan counting ─────────────────────────────────────────────────────────

use cf_hunk_tracker::{REFRESH_SCAN_LOG_PREFIX, REFRESH_SKIP_LOG_PREFIX};
use cf_test_utils::tracing_capture::MessagePrefixCounter;
...
fn install_global_scan_counter() -> ScanCounter {
    // QIDI_E2E_LOG=<filter> tees shell logs to stderr for local debugging.
    let filter = std::env::var("QIDI_E2E_LOG").ok();
    ScanCounter(
        cf_test_utils::tracing_capture::install_prefix_counter_global(
...
fn build_repo(files: usize, picks: usize) -> (TempDir, String) {
    ...
    cf_test_utils::git::write_fanout_tree(wd, files, 100);
    git(wd, &["add", "."]);
    git(wd, &["commit", "-m", "populate tree"]);

    let base = cf_test_utils::git::make_feature_branch(wd, picks);
    (dir, base)
}
...
async fn run_storm(
    ...
    let files = env_usize("QIDI_PERF_GIT_FILES", 300);
    let picks = env_usize("QIDI_PERF_GIT_PICKS", 6);
