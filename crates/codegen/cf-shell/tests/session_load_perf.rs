...
//! `QIDI_PERF_SESSION_SRC=/path/to/<session-dir>`.
//!
//! Run:
//!   cargo test -p qidi-code --test session_load_perf -- --nocapture
//!   cargo test -p qidi-code --test session_load_perf full_session_load_e2e -- --ignored --nocapture
...
use cf_shell::session::info::Info;
use cf_shell::session::storage::{
    JsonlStorageAdapter, StorageAdapter, load_updates_for_replay_at,
};
use cf_workspace::session::file_state::{FileSnapshot, FlexiblePath, RewindPoint};
...
impl GenOpts {
    fn from_env() -> Self {
        ...
        let scale = g("QIDI_PERF_SCALE", 1).max(1);
        Self {
            turns: g("QIDI_PERF_TURNS", 80) * scale,
            acu_per_turn: g("QIDI_PERF_ACU_PER_TURN", 15),
            catalog_commands: g("QIDI_PERF_CATALOG_COMMANDS", 64),
            catalog_desc_len: g("QIDI_PERF_CATALOG_DESC_LEN", 320),
            agent_chunks_per_turn: g("QIDI_PERF_AGENT_CHUNKS_PER_TURN", 8),
            agent_chunk_len: g("QIDI_PERF_AGENT_CHUNK_LEN", 2000),
            rewind_points: g("QIDI_PERF_REWIND_POINTS", 60) * scale,
            files_per_rewind: g("QIDI_PERF_FILES_PER_REWIND", 40),
            file_content_len: g("QIDI_PERF_FILE_CONTENT_LEN", 8000),
        }
    }
}
...
/// `Info` and the session directory path. Uses `QIDI_PERF_SESSION_SRC` if set
...
async fn prepare_session(root: &Path, cwd: &Path, opts: &GenOpts) -> (Info, PathBuf) {
    ...
    if let Ok(src) = std::env::var("QIDI_PERF_SESSION_SRC") {
...
/// `--ignored` (optionally `QIDI_PERF_SESSION_SRC=...`) to get the numbers.
...
fn generate_or_restore_rewind(path: &Path, opts: &GenOpts) {
    if std::env::var("QIDI_PERF_SESSION_SRC").is_ok() {
        return;
    }
...
use cf_shell::agent::mvp_agent::MvpAgent;
...
async fn full_session_load_e2e() {
    ...
    let server = cf_test_support::MockInferenceServer::start()
        .await
        .unwrap();
    ...
    unsafe {
        std::env::set_var("QIDI_HOME", grok_home.path());
        std::env::set_var("QIDI_INSTRUMENTATION", "log");
        std::env::set_var("QIDI_INSTRUMENTATION_LOG", &instr_log);
        std::env::set_var("QIDI_CLI_CHAT_PROXY_BASE_URL", server.url());
        std::env::set_var("QIDI_XAI_API_BASE_URL", server.url());
        std::env::set_var("XAI_API_KEY", "test-key-for-ci");
        std::env::set_var("QIDI_TELEMETRY_ENABLED", "false");
        std::env::set_var("QIDI_FEEDBACK_ENABLED", "false");
        std::env::set_var("QIDI_TRACE_UPLOAD", "false");
    }
    ...
    let _ = tracing_subscriber::registry()
        .with(cf_shell::instrumentation::layer::<Registry>())
        .try_init();
        ...
        .run_until(async move {
            ...
            tokio::task::spawn_local(
                GatewayReceiver::new(gw_rx, agent_conn)
                    .with_on_meta(cf_file_utils::trace_context::span_from_meta_traceparent)
                    .run(),
            );
            ...
            let _ = cf_shell::instrumentation::finalize();
