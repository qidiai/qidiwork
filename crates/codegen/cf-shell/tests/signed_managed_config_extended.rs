...
use cf_config::signed_policy::{self, SignedPayload};
...
async fn sync_fail_closed_policy(home: &std::path::Path, kp: &ring::signature::Ed25519KeyPair) {
    ...
    assert!(cf_shell::managed_config::managed_policy_gate().is_ok());
}
...
async fn rejected_signature_surfaces_as_setup_and_login_failure() {
    ...
    assert!(
        matches!(
            outcome,
            cf_shell::managed_config::SetupOutcome::Failed(
                cf_shell::managed_config::ManagedConfigError::SignatureRejected
            )
        ),
        "setup must surface the signature rejection, got {outcome:?}"
    );

    let login = cf_shell::managed_config::post_login_sync(None).await;
    assert_eq!(
        login,
...
async fn withdrawn_requirements_is_deleted_and_covered_by_the_new_sidecar() {
    ...
    assert!(
        !cf_shell::config::is_managed_config_hard_stale_for(&team_identity("team-007")),
