...
//! `QIDI_HOME` (the `grok_home` `OnceLock` allows a single value per process)
...
use cf_shell::config::ServingIdentity;
use cf_test_support::spawn_counting_server;
...
/// Shared temp dir used as QIDI_HOME for the whole test binary (the grok_home
...
fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe {
            std::env::set_var("QIDI_HOME", &path);
            ...
            for var in [
                "QIDI_DEPLOYMENT_KEY",
                "QIDI_MANAGED_CONFIG",
                "QIDI_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS",
                "QIDI_DEPLOYMENT_CONFIG_CACHE_TTL_SECS",
                "HTTP_PROXY",
                "HTTPS_PROXY",
                "ALL_PROXY",
                "http_proxy",
            ...
            std::env::set_var("QIDI_DEPLOYMENT_CONFIG_BACKOFF_MS", "10");
        }
        path
    })
}
...
fn write_team_auth_expiry(home: &std::path::Path, team_id: &str, expires_at: &str) {
    let scope = cf_shell::auth::GrokComConfig::default().auth_scope();
...
async fn team_sync_writes_files() {
    ...
    let wrote = cf_shell::managed_config::sync()
        .await
...
async fn team_switch_evicts_prior_teams_policy() {
    ...
    assert!(!cf_shell::config::is_managed_config_stale_for(
...
async fn withdrawn_artifact_is_removed_on_next_sync() {
    ...
    assert!(
        !cf_shell::config::is_managed_config_stale_for(&team_identity("team-007")),
...
async fn empty_dk_response_with_failing_team_leaves_team_policy_intact() {
    ...
    let err = cf_shell::managed_config::sync()
        .await
...
async fn served_then_deleted_refetches_best_effort() {
    ...
    let auth_manager = std::sync::Arc::new(cf_shell::auth::AuthManager::new(
        &home,
        cf_shell::auth::GrokComConfig::default(),
    ));
    cf_shell::managed_config::ensure_managed_policy_present(&auth_manager).await;
    assert!(
        home.join("requirements.toml").exists(),
...
async fn expired_refreshable_team_token_heals_after_auth_refresh() {
    ...
    std::fs::remove_file(home.join("requirements.toml")).unwrap();
    assert!(
        !cf_shell::managed_config::has_principal(),
...
async fn managed_policy_gate_fails_closed_on_deleted_policy_offline() {
    ...
    assert!(
        cf_shell::managed_config::managed_policy_gate().is_ok(),
    ...
    std::fs::remove_file(home.join("requirements.toml")).unwrap();
    assert!(
        cf_shell::managed_config::managed_policy_gate().is_err(),
    ...
    write_config(&home, &url);
    write_team_auth(&home, "team-007");
    cf_shell::config::mark_managed_config_synced(cf_shell::config::SyncMarker {
...
async fn bootstrap_fails_closed_when_managed_policy_compromised() {
    ...
    let cfg = cf_shell::agent::config::Config::default();
    let auth_manager = std::sync::Arc::new(cf_shell::auth::AuthManager::new(
        &home,
        cf_shell::auth::GrokComConfig::default(),
    ));
    // `bootstrap`'s Ok type isn't `Debug`, so match rather than `expect_err`.
    let err = match cf_shell::agent::init::bootstrap(&cfg, &auth_manager, None) {
        Err(e) => e,
        Ok(_) => {
...
/// Live wiring guard: an offline `QIDI_DEPLOYMENT_KEY` switch on a fail_closed install must FAIL CLOSED, else a
...
async fn managed_policy_gate_fails_closed_on_deployment_key_switch_offline() {
    ...
    unsafe { std::env::set_var("QIDI_DEPLOYMENT_KEY", "KEY-AAA") };
    cf_shell::managed_config::sync()
        .await
    ...
    // QIDI_MANAGED_CONFIG=0 disables any incidental background fetch (the gate is sync anyway).
    // SAFETY: #[serial] test; restored before any assertion below.
    unsafe { std::env::set_var("QIDI_MANAGED_CONFIG", "0") };

    // Same key A → matching fingerprint → ALLOW (exact-equality vs recorded blake3).
    let gate_same_key = cf_shell::managed_config::managed_policy_gate();

    // Switch to a different key B → REFUSE: exercises the full offline wiring.
    // SAFETY: #[serial] test; restored immediately below.
    unsafe { std::env::set_var("QIDI_DEPLOYMENT_KEY", "KEY-BBB") };
    let gate_switched_key = cf_shell::managed_config::managed_policy_gate();

    // SAFETY: #[serial] test; restore env BEFORE asserting so a failed assert can't leak it to later tests.
    unsafe {
        std::env::remove_var("QIDI_DEPLOYMENT_KEY");
        std::env::remove_var("QIDI_MANAGED_CONFIG");
    }
...
/// The session-start gate reads no env: `QIDI_MANAGED_CONFIG_FAIL_CLOSED=0` must NOT disarm a fail_closed
...
async fn fail_closed_env_cannot_disarm_the_gate() {
    ...
    unsafe {
        std::env::set_var("QIDI_MANAGED_CONFIG", "0"); // offline (the gate is sync anyway)
        std::env::set_var("QIDI_MANAGED_CONFIG_FAIL_CLOSED", "0"); // attempt a local disarm
    }
    let gate = cf_shell::managed_config::managed_policy_gate();
    unsafe {
        std::env::remove_var("QIDI_MANAGED_CONFIG");
        std::env::remove_var("QIDI_MANAGED_CONFIG_FAIL_CLOSED");
    }

    assert!(
        gate.is_err(),
        "QIDI_MANAGED_CONFIG_FAIL_CLOSED=0 must not disarm the session-start gate (no local bypass)"
    );
}
...
async fn setup_lock_skip_is_not_reported_as_no_config() {
    ...
    let outcome = cf_shell::managed_config::run_setup().await;
    lock.unlock().unwrap();

    assert!(
        matches!(
            outcome,
            cf_shell::managed_config::SetupOutcome::Installed
        ),
...
async fn shared_client_reuses_pooled_connection() {
    ...
    let client = cf_shell::http::shared_client();
...
/// `QIDI_MANAGED_CONFIG=0` is an explicit opt-out: the post-login sync must
/// make zero requests.
#[tokio::test]
#[serial]
async fn managed_config_opt_out_makes_no_requests() {
    ...
    unsafe { std::env::set_var("QIDI_MANAGED_CONFIG", "0") };
    let outcome = cf_shell::managed_config::post_login_sync(None).await;
    unsafe { std::env::remove_var("QIDI_MANAGED_CONFIG") };

    assert_eq!(
        outcome,
        cf_shell::managed_config::ManagedConfigSync::Skipped
    );
...
async fn post_login_pins_authenticated_team_over_disk() {
    ...
    let pinned: cf_shell::auth::GrokAuth = serde_json::from_value(serde_json::json!({
        "key": "pinned-token",
        "auth_mode": "oidc",
        "create_time": "2026-01-01T00:00:00Z",
        "expires_at": FAR_FUTURE,
    ...
    let outcome = cf_shell::managed_config::post_login_sync(Some(pinned)).await;
    assert_eq!(
        outcome,
        cf_shell::managed_config::ManagedConfigSync::Updated { is_team: true }
    );
    assert_eq!(
        auths.lock().unwrap().last().map(String::as_str),
...
async fn post_login_sync_is_latency_bounded() {
    ...
    assert_eq!(
        outcome,
        cf_shell::managed_config::ManagedConfigSync::Failed
    );
    assert_eq!(
        *count.lock().unwrap(),
        2,
