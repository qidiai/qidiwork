...
//! Run locally:
//! ```bash
//! cargo test -p qidi-code --test test_agent_type_invariant -- --ignored
//! ```
...
/// - `default-model`: no agent_type (→ defaults to "cf-tools")
...
/// - `model-a`: no agent_type (→ "cf-tools")
/// - `model-b`: no agent_type (→ "cf-tools")
...
/// `cf-tools` harness. The system prompt sent to the LLM should contain
...
async fn test_default_model_uses_qidi_build_harness() {
    with_local_set(|| async {
        ...
        assert!(
            ...
            "default model should use cf-tools harness\nsystem prompt preview: {}",
...
async fn test_model_without_agent_type_defaults_to_qidi_build() {
    with_local_set(|| async {
            ...
            assert!(
                sys_prompt.contains("Grok") || sys_prompt.contains("grok"),
                "model without agent_type should default to cf-tools harness\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;
}
/// The `QIDI_AGENT` escape hatch should override the model's agent_type.
/// Setting `QIDI_AGENT=cf-tools` with an alternate-agent model should use
/// cf-tools harness.
#[tokio::test]
#[ignore]
async fn test_grok_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
            ...
            cmd.args(["agent", "stdio"])
                ...
                .kill_on_drop(true);
            cf_test_support::env::test_env_cmd_tokio(
                &mut cmd,
                &server.url(),
                home.path(),
            );
            cmd.env("QIDI_AGENT", "cf-tools");
            ...
            let incoming = cf_acp_lib::LineBufferedRead::spawn_local(incoming);
            ...
            assert!(
                sys_prompt.contains("Grok") || sys_prompt.contains("grok"),
                "QIDI_AGENT=cf-tools should override cursor model's agent_type\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;
