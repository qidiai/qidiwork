//! Built-binary end-to-end tests for the grok (qidi-code) binary.
//!
...
//! ```bash
//! cargo test -p qidi-code --test test_built_binary_e2e -- --ignored
//! ```
//!
//! In CI, set `QIDI_BINARY` to point at the release artifact:
//! ```bash
//! QIDI_BINARY=./artifacts/grok-0.1.159-linux-x86_64 \
//!   cargo test -p qidi-code --test test_built_binary_e2e -- --ignored
//! ```
...
use cf_test_support::env::test_env_cmd_tokio;
use cf_test_support::*;
...
async fn test_version_with_crash_handler_exits_zero() {
    ...
    let output = Command::new(&binary)
        .arg("--version")
        .env("QIDI_CRASH_HANDLER", "1")
        .output()
        .unwrap_or_else(|e| panic!("failed to run {}: {e}", binary.display()));

    assert!(
        output.status.success(),
        "grok --version with QIDI_CRASH_HANDLER=1 failed (exit {:?}):\n{}",
        output.status.code(),
        String::from_utf8_lossy(&output.stderr)
    );
}
...
async fn test_headless_free_usage_exhausted_prints_paywall_message() {
    ...
    assert!(
        combined.contains("reached your free QIDI Code usage limit"),
...
async fn test_headless_json_reports_server_cost() {
    use cf_test_support::scripted::SseEvent;
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        cf_test_support::scripted::ScriptedResponse::sse(vec![
...
async fn test_headless_json_reports_usage_on_max_turns() {
    ...
    server.enqueue_response(
        "/v1/chat/completions",
        cf_test_support::scripted::ScriptedResponse::sse(
            cf_test_support::sse::chat_completions_reasoning_then_tool_call_events(
                "let me look",
                "call-1",
                "read_file",
                r#"{"path":"README.md"}"#,
...
/// Isolated headless run with a custom `~/.qidi/`. Clean env (no leaked
...
impl ConfigTestHarness {
    fn new(server: &MockInferenceServer) -> Self {
        ...
        Self {
            ...
            workdir: git_workdir(),
            env: vec![
                ("QIDI_CLI_CHAT_PROXY_BASE_URL".into(), server.url()),
                ("QIDI_TELEMETRY_ENABLED".into(), "false".into()),
                ("QIDI_FEEDBACK_ENABLED".into(), "false".into()),
                ("QIDI_TRACE_UPLOAD".into(), "false".into()),
                ("QIDI_INSTRUMENTATION".into(), "disabled".into()),
                ("QIDI_DISABLE_AUTOUPDATER".into(), "1".into()),
            ],
        }
    }
...
/// Enterprise BYOK: managed_config.toml overrides cf-tools with a custom
...
"#,
    ...
    .unwrap();
    h.env("QIDI_TEST_BYOK_TOKEN", "test-byok-secret-token");
