...
/// All tests share this as QIDI_HOME (the `OnceLock` in xai-grok-config
...
fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe { std::env::set_var("QIDI_HOME", &path) };
        path
    })
}
...
async fn update_config_does_not_leak_requirements_into_user_config() {
    ...
    let effective = cf_shell::config::load_effective_config().unwrap();
    let effective_cfg = cf_shell::util::config::load_config_from_toml(&effective);
    assert_eq!(
    ...
    cf_shell::util::config::update_config(|cfg| {
    ...
    let user_cfg = cf_shell::util::config::load_config_from_toml(&user_toml);
