...
//! caller-owned `$QIDI_HOME`, then inspects `~/.qidi/debug/`:
...
//!   `~/.qidi/debug/<sessionId>.txt` with first-party content, and does NOT enable
//!   sampling/instrumentation. Regression for the master switch having bundled
//!   `QIDI_LOG_SAMPLING`/`QIDI_INSTRUMENTATION`, whose global `TargetFilterLayer`
...
//! - a live `agent` session (explicit `QIDI_DEBUG_LOG=1`) writes a per-session
//!   `~/.qidi/debug/<sessionId>.txt` with real first-party content + `latest.txt`.
//! - `--debug-file <path>` writes one explicit file and bypasses per-session
//!   routing entirely (no `~/.qidi/debug/` files).
//! - `QIDI_LOG_FILE=<path>` writes that explicit file (back-compat single file).
//!
...
//! ```bash
//! cargo test -p qidi-code --test test_debug_logging -- --ignored
//! ```
...
/// Build a headless `grok -p` command with a pinned `$QIDI_HOME` so the firehose
/// lands under `<home>/.qidi/debug`. Firehose env knobs are cleared so the test
...
/// assert it carries first-party (`cf_grok`) content. Panics with the captured
...
/// A live `agent` session writes `~/.qidi/debug/<sessionId>.txt` with real
...
async fn agent_session_writes_named_session_file() {
    with_local_set(|| async {
        ...
        let client = GrokStdioClient::spawn_with_home_and_env(
            ...
            workdir.path(),
            home,
            &[("QIDI_DEBUG_LOG", "1"), ("QIDI_HOME", &grok_home_str)],
        )
        .await;
        ...
        // file, so `tail -f ~/.qidi/debug/latest.txt` follows the live session.
        #[cfg(unix)]
        {
...
/// The `--debug` FLAG (not `QIDI_DEBUG_LOG` directly) drives the firehose end to
/// end through the master switch. Regression: the master switch used to also set
/// `QIDI_LOG_SAMPLING`/`QIDI_INSTRUMENTATION`, whose `TargetFilterLayer` globally
...
async fn debug_flag_master_switch_enables_firehose() {
    with_local_set(|| async {
        ...
        // clears QIDI_LOG_SAMPLING (hermetic), so the sampling layer stays off and
        // `~/.qidi/logs/sampling.jsonl` is never written — the `--debug`
        ...
        // QIDI_INSTRUMENTATION=disabled, so that assertion would be vacuous.
...
/// `--debug-file <path>` writes one explicit file and bypasses per-session
/// routing entirely (no `~/.qidi/debug/` files created).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
...
/// `QIDI_LOG_FILE=<path>` (no `--debug`) writes that exact file (back-compat).
#[tokio::test]
#[ignore] // requires pre-built binary; run with --ignored
async fn grok_log_file_explicit_path_is_written() {
    ...
    cmd.env("QIDI_LOG_FILE", &custom);
    let result = run_headless_with_cmd(cmd).await;

    assert_headless_success(&result, "grok QIDI_LOG_FILE=path", Some(&server));
    assert_no_crashes(&result.stderr);
    assert!(
        custom.exists(),
        "explicit QIDI_LOG_FILE path not written: {custom:?}\nstderr tail:\n{}",
        stderr_tail(&result.stderr, 800)
    );
    // Single-file mode bypasses per-session routing.
    assert!(
        firehose_txt_files(home.path()).is_empty(),
        "QIDI_LOG_FILE must bypass per-session routing, found: {:?}",
        firehose_txt_files(home.path())
    );
}
