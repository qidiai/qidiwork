...
use cf_sampler::RetryPolicy;
use cf_sampling_types::doom_loop::{DoomLoopSignalKind, SAMPLE_CHECK_EVENT_DATA_CUMULATIVE};
...
use cf_test_support::{MockInferenceServer, MockModelEntry, ScriptedResponse};
...
/// ```bash
/// cargo test -p qidi-code --test test_doom_loop_recovery -- --ignored
/// ```
#[tokio::test]
...
async fn headless_config_enables_doom_loop_check_header() {
    ...
    let mut cmd = tokio::process::Command::new(cf_test_support::grok_binary());
    cmd.args(["-p", "say hi", "--yolo", "--output-format", "json"])
        ...
        .kill_on_drop(true);
    cf_test_support::env::test_env_cmd_tokio(&mut cmd, &server.url(), home.path());
    cmd.env("QIDI_HOME", grok_home);
    // Don't attach to a developer's ambient leader; spawn fresh against the mock.
    cmd.env_remove("QIDI_LEADER_SOCKET");

    let result = cf_test_support::run_headless_with_cmd(cmd).await;
    cf_test_support::assert_headless_success(&result, "doom-loop header e2e", Some(&server));
