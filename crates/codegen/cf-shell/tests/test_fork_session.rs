...
use cf_shell::sampling::ConversationItem;
...
async fn create_test_session(storage: &JsonlStorageAdapter, session_id: &str, cwd: &str) -> Info {
    ...
    storage
        .append_update(
            &info,
            &cf_shell::session::storage::SessionUpdate::Acp(Box::new(notification)),
        )
        .await
        .unwrap();
...
async fn test_fork_session_creates_new_session_with_parent_tracking() {
    ...
    let options = cf_shell::session::storage::CopySessionOptions {
    ...
    match &loaded.updates[0] {
        cf_shell::session::storage::SessionUpdate::Acp(notification) => {
