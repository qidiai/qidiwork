...
//!
//! ```bash
//! cargo test -p qidi-code --test test_heap_profile_monitor
//! ```
...
use cf_shell::auth::{AuthManager, AuthMode, GrokAuth, GrokComConfig};
use cf_shell::heap_profile::{
...
use cf_test_support::{EnvGuard, MockInferenceServer};
use cf_version::TEST_VERSION_ENV;
...
fn assert_storage_auth(uploads: &[cf_test_support::mock_server::StorageUpload]) {
    for u in uploads {
        assert_eq!(
