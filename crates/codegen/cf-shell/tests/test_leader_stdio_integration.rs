...
use cf_shell::cpu_profile::ControlErrorCode;
use cf_shell::leader::{
...
async fn setup_control_test_server(
    temp: &TempDir,
) -> (std::path::PathBuf, cf_shell::leader::ServerHandle) {
...
async fn test_session_ownership_cleanup_on_disconnect() {
    use cf_shell::leader::run_leader_server;
    ...
    tokio::spawn(async move {
        ...
        let (shutdown_tx, _shutdown_rx) =
            tokio::sync::watch::channel(cf_shell::leader::ShutdownReason::Manual);
...
async fn test_raw_registration_handshake_not_ready_then_ready() {
    ...
    tokio::spawn(async move {
        ...
        let _ = run_leader_server(
            ...
            watch::channel(cf_shell::leader::ShutdownReason::Manual).0,
            None,
            control_state,
        )
        .await;
...
async fn test_version_mismatch_notification_sent_to_client() {
    ...
    use tokio::sync::{mpsc, watch};
    use tokio_util::sync::CancellationToken;
    use cf_shell::leader::{ClientCapabilities, ClientMode, LeaderClient, run_leader_server};
...
async fn test_auto_update_shutdown_reason_reaches_client() {
    use cf_shell::leader::{ClientCapabilities, ClientMode, LeaderClient, ShutdownReason};
    ...
    tokio::time::timeout(
        ...
        disconnect_rx.wait_for(|r| *r != cf_shell::leader::DisconnectReason::Connected),
    )
    .await
    ...
    assert_eq!(
        *disconnect_rx.borrow(),
        cf_shell::leader::DisconnectReason::LeaderShutdown
    );
}
...
///
/// This binary does not sandbox QIDI_HOME, so on a dev machine these entries
/// land in the real `~/.qidi` log — accepted: the server already writes
...
fn orphan_log_count(request_id: &str) -> usize {
    let Some(bytes) = cf_telemetry::unified_log::snapshot_log() else {
        return 0;
    };
...
async fn wait_for_client_disconnected_log(client_id: u64) -> bool {
    let deadline = tokio::time::Instant::now() + Duration::from_secs(5);
    loop {
        let seen = cf_telemetry::unified_log::snapshot_log().is_some_and(|bytes| {
...
async fn test_hung_agent_leaves_transport_healthy_and_forwards_cancel() {
    ...
    assert_eq!(
        *client.disconnect_reason().borrow(),
        cf_shell::leader::DisconnectReason::Connected,
