...
//! - `QIDI_BINARY_LEADER` — the binary that elects the initial leader
//!   (typically the latest released stable, e.g. fetched from
//!   `https://storage.googleapis.com/cf-tools-public-artifacts/cli/grok-<ver>-linux-x86_64`).
//! - `QIDI_BINARY_CLIENT` — the second client (typically a freshly built main).
//!
...
//!
//! ```bash
//! QIDI_BINARY_LEADER=/path/to/grok-old QIDI_BINARY_CLIENT=/path/to/grok-new \
//!   cargo test -p qidi-code --test test_leader_version_skew -- --ignored --nocapture
//! ```
...
fn skew_binaries() -> Option<(std::path::PathBuf, std::path::PathBuf)> {
    ...
    if old == new {
        eprintln!(
            "SKIP: QIDI_BINARY_LEADER/QIDI_BINARY_CLIENT resolve to the same binary ({})",
            old.display()
        );
        return None;
    }
...
#[tokio::test]
#[ignore = "two-binary version-skew test; set QIDI_BINARY_LEADER/QIDI_BINARY_CLIENT and run with --ignored"]
...
/// `signal_leaders_to_relaunch` in `qidi-code-bin/src/main.rs` (which is
