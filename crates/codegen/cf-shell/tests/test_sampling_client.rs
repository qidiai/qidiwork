...
use cf_shell::sampling::{
    ApiBackend, Client, ConversationItem, ConversationRequest, ConversationToolChoice,
    SamplingError, ToolCall, ToolSpec, rs,
};
use cf_shell::session::storage::JsonlStorageAdapter;
use cf_test_support::sse::responses_api_reasoning_and_text_events;
...
/// Upgrade path, cf-tools / Responses API: a legacy session whose
...
async fn responses_upgrade_roundtrips_reconstructed_reasoning_as_typed_input() {
    // 1. Seed a legacy cf-tools chat_history.jsonl (inline reasoning
    ...
    std::fs::write(
        ...
        concat!(
            ...
            r#"{"type":"user","content":[{"type":"text","text":"q1"}]}"#,
            "\n",
            r#"{"type":"assistant","content":"a1","reasoning":{"text":"legacy cf-tools reasoning","encrypted":"ENC_BLOB_xyz","id":"rs_grokbuild_legacy"},"model_id":"cf-tools"}"#,
            "\n",
        ),
    )
    .unwrap();
    ...
    assert_eq!(
        ...
        Some("legacy cf-tools reasoning")
    );
}
...
async fn test_responses_api_streaming_text() {
    ...
    while let Some(event_result) = stream.next().await {
        ...
        use cf_shell::sampling::rs::ResponseStreamEvent;
        match event {
    ...
    while let Some(event_result) = stream.next().await {
        ...
        if let ResponseStreamEvent::ResponseCompleted(completed) = event {
            for output in completed.response.output {
                use cf_shell::sampling::rs::OutputItem;
                ...
                match output {
                    ...
                    OutputItem::Message(msg) => {
                        for content in &msg.content {
                            use cf_shell::sampling::rs::OutputMessageContent;
...
async fn test_doom_loop_check_enabled_sends_header_and_absorbs_check_event() {
    use cf_sampling_types::doom_loop::{DOOM_LOOP_CHECK_EVENT_TYPE, SAMPLE_CHECK_EVENT_DATA};
