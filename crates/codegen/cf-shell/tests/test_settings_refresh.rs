...
//! Run locally:
//! ```bash
//! cargo test -p qidi-code --test test_settings_refresh
//! ```
...
async fn test_fetch_settings_blocking_round_trip() {
    with_local_set(|| async {
        ...
        let auth = cf_shell::auth::GrokAuth {
            key: "test-key".into(),
            ..Default::default()
        };
        let result = tokio::task::spawn_blocking({
            ...
            let auth = auth.clone();
            move || cf_shell::remote::fetch_settings_blocking(&url, &auth, None)
        })
        .await
        .unwrap();
        assert!(
...
async fn test_doom_loop_recovery_settings_round_trip() {
    use cf_shell::util::config::DoomLoopRecoverySettings;
