...
//! 4. Smoke-validate session JSON under `$QIDI_HOME/sessions/` after exit.
//!
//! Requires a built `grok` binary (`QIDI_BINARY` or cargo-built pager) for the
//! ignored headless test.
//!
//! ```bash
//! cargo test -p qidi-code --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p qidi-code --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```
...
use cf_agent::plugins::SharedPluginRegistryHandle;
use cf_agent::plugins::discovery::DiscoveryConfig;
use cf_agent::plugins::git_install::{InstallSource, install_from_source};
use cf_agent::plugins::install_registry::{
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _grok_guard = EnvVarGuard::set("QIDI_HOME", &grok_home);
    ...
    // grok_home() caches the first QIDI_HOME per process; a separate test
    ...
    assert_eq!(
        plugin.scope,
        cf_agent::plugins::PluginScope::CliOverride
    );
...
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    // The spawned binary gets HOME/QIDI_HOME via `cmd.env` below; this global env
    ...
    // Smoke: session storage under QIDI_HOME/sessions has JSON artifacts.
