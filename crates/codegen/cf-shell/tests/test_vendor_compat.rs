...
//! `QIDI_<VENDOR>_<SURFACE>_ENABLED` env vars via `cmd.env`), sends one prompt,
...
//! ```bash
//! cargo test -p qidi-code --test test_vendor_compat -- --ignored
//! ```
...
const MARKER_QIDI_SKILL: &str = "ZZ_QIDI_SKILL_MARKER";
...
fn seed_fixtures(home: &Path, cwd: &Path) {
    ...
    write_skill(home, ".grok", "grok-skill", MARKER_QIDI_SKILL);
...
async fn vendor_compat_defaults_include_vendor_skills_but_drop_denylisted() {
    with_local_set(|| async {
        ...
        assert!(
            body.contains(MARKER_QIDI_SKILL),
...
/// `QIDI_CURSOR_SKILLS_ENABLED=false` drops the cursor-vendor skill; grok stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("QIDI_CURSOR_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_QIDI_SKILL),
...
/// `QIDI_CLAUDE_SKILLS_ENABLED=false` drops the claude-vendor skill; grok stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("QIDI_CLAUDE_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_QIDI_SKILL),
            "grok-skill always present"
        );
...
/// `QIDI_CURSOR_RULES_ENABLED=false` drops cursor-vendor rules; claude-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("QIDI_CURSOR_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_RULE),
...
/// `QIDI_CLAUDE_RULES_ENABLED=false` drops claude-vendor rules; cursor-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("QIDI_CLAUDE_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_RULE),
...
/// `QIDI_CURSOR_AGENTS_ENABLED=false` drops cursor-vendor AGENTS.md; claude-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("QIDI_CURSOR_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_AGENTS),
...
/// `QIDI_CLAUDE_AGENTS_ENABLED=false` drops claude-vendor AGENTS.md; cursor-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("QIDI_CLAUDE_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_AGENTS),
...
async fn vendor_compat_all_cursor_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("QIDI_CURSOR_SKILLS_ENABLED", "false"),
            ("QIDI_CURSOR_RULES_ENABLED", "false"),
            ("QIDI_CURSOR_AGENTS_ENABLED", "false"),
        ])
        .await;
        ...
        assert!(body.contains(MARKER_QIDI_SKILL), "grok always present");
...
async fn vendor_compat_all_claude_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("QIDI_CLAUDE_SKILLS_ENABLED", "false"),
            ("QIDI_CLAUDE_RULES_ENABLED", "false"),
            ("QIDI_CLAUDE_AGENTS_ENABLED", "false"),
        ])
        .await;
