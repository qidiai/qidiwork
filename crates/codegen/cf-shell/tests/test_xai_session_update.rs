...
use cf_shell::extensions::notification::{
    DiffContent, SessionNotification, SessionUpdate as XaiSessionUpdate,
};
use cf_shell::session::info::Info as SessionInfo;
use cf_shell::session::persistence::default_model_id;
use cf_shell::session::storage::{JsonlStorageAdapter, SessionUpdate, StorageAdapter};
...
async fn test_xai_session_notification_storage_roundtrip() {
    ...
    let cf_notification = SessionNotification {
    ...
    adapter
        .append_update(
            &info,
            &SessionUpdate::Xai(Box::new(cf_notification.clone())),
        )
        .await
        .unwrap();
...
async fn test_turn_completed_round_trips_through_storage() {
    ...
    adapter
        .append_update(&info, &SessionUpdate::Xai(Box::new(cf_notification)))
        .await
        .unwrap();
