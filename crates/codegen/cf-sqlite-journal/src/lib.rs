...
//! do not provide. When `$HOME` (and thus `~/.qidi`) is NFS-mounted on
...
impl JournalMode {
    ...
    /// yet), so callers must create it first. `QIDI_SQLITE_JOURNAL_MODE`
    /// (`wal`|`truncate`) overrides detection as a field kill-switch.
    pub fn for_db_path(db_path: &Path) -> Self {
        let env = std::env::var("QIDI_SQLITE_JOURNAL_MODE").ok();
        match mode_from_env(env.as_deref()) {
            EnvOverride::Mode(mode) => {
                ...
                tracing::info!(
                    ...
                    mode = mode.as_str(),
                    source = "env",
                    "sqlite journal mode forced by QIDI_SQLITE_JOURNAL_MODE"
                );
                return mode;
            }
            EnvOverride::Invalid => {
                ...
                tracing::warn!(
                    ...
                    "invalid QIDI_SQLITE_JOURNAL_MODE (accepted: wal, truncate); using detection"
                );
            }
            EnvOverride::Unset => {}
        }
...
/// Parse result of the `QIDI_SQLITE_JOURNAL_MODE` kill-switch.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum EnvOverride {
    Unset,
...
/// `QIDI_SQLITE_JOURNAL_MODE` override parsing (pure for testability).
...
mod tests {
    ...
    fn for_db_path_defaults_to_wal_on_local_fs() {
        ...
        if std::env::var("QIDI_SQLITE_JOURNAL_MODE").is_ok() {
            return;
        }
        let tmp = TempDir::new().unwrap();
