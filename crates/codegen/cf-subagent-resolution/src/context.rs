...
//!
//! Extracted from `qidi-code/src/agent/subagent/` `normalize_forked_context()`.
...
/// `qidi-code/src/session/storage/jsonl.rs` (it truncates to the last
...
mod tests {
    use super::*;
    use cf_sampling_types::conversation::{ConversationItem, ToolCall, ToolResultItem};
    ...
    fn reasoning_item(text: &str) -> ConversationItem {
        ConversationItem::Reasoning(cf_sampling_types::synthesized_reasoning_item(text))
    }
