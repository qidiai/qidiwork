...
pub struct TelemetryConfig {
    ...
    /// External OTEL master switch (`= QIDI_EXTERNAL_OTEL`, env wins).
...
impl Default for TelemetryConfig {
    fn default() -> Self {
        ...
        let build_url = build_env_default(option_env!("QIDI_TELEMETRY_BUILD_EVENTS_URL"));
        let build_key = build_env_default(option_env!("QIDI_TELEMETRY_BUILD_EVENTS_API_KEY"));
        let build_token = build_env_default(option_env!("QIDI_TELEMETRY_BUILD_MIXPANEL_TOKEN"));
...
impl TelemetryConfig {
    pub fn apply_env_overrides(&mut self) {
        self.normalize();
        if let Some(value) = Self::env_override("QIDI_TELEMETRY_EVENTS_URL") {
            self.events_url = value;
        }
        if let Some(value) = Self::env_override("QIDI_TELEMETRY_EVENTS_API_KEY") {
            self.events_api_key = value;
        }
        if let Some(value) = Self::env_override("QIDI_TELEMETRY_MIXPANEL_TOKEN") {
            self.mixpanel_token = value;
        }
        if let Some(value) = env_bool("QIDI_TELEMETRY_MIXPANEL_ENABLED") {
...
///
/// Local copy of `cf_shell::agent::config::env_bool` so this crate
...
mod tests {
    ...
    fn default_is_build_env_layer_when_feature_off() {
        ...
        let url = build_env_default(option_env!("QIDI_TELEMETRY_BUILD_EVENTS_URL"));
        let key = build_env_default(option_env!("QIDI_TELEMETRY_BUILD_EVENTS_API_KEY"));
        let token = build_env_default(option_env!("QIDI_TELEMETRY_BUILD_MIXPANEL_TOKEN"));
