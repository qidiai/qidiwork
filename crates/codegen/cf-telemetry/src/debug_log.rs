...
//! - PerSession (`QIDI_DEBUG_LOG=1`): a routing layer fans each session's
//!   firehose to `~/.qidi/debug/<session_id>.txt` (one file per session), with a
...
//! - SingleFile (explicit path via `QIDI_LOG_FILE` or `QIDI_DEBUG_LOG=<path>`):
...
impl DebugSource {
    fn label(self) -> &'static str {
        match self {
            Self::GrokLogFile => "QIDI_LOG_FILE",
            Self::GrokDebugLog => "QIDI_DEBUG_LOG",
        }
    }
}
...
///
/// Lives here (not in `qidi-code`) so the firehose directives below and
...
/// safe. See `qidi-code/src/tracing.rs` for the consumer side.
...
// Broad firehose filter for the routing/QIDI_DEBUG_LOG sources: capture our
...
// until added here.
const FIREHOSE_BASE_DIRECTIVES: &str = "info,cf_pager=debug,cf_shell=debug,cf_tools=debug,cf_telemetry=debug,cf_agent=debug,cf_mcp=debug,cf_acp_lib=debug,sampling_log=off";
...
// QIDI_DEBUG_LOG single-file source (mirrors `default_file_filter`).
fn firehose_filter() -> EnvFilter {
    EnvFilter::new(firehose_directives())
}

// RUST_LOG-respecting filter for the QIDI_LOG_FILE source: DEBUG default, honor
// RUST_LOG, silence sampling_log (preserves QIDI_LOG_FILE back-compat).
...
/// filter by source (QIDI_LOG_FILE respects RUST_LOG). Open failures warn AFTER
...
pub(crate) enum DebugTarget {
    /// `QIDI_DEBUG_LOG=1` → route per session into `<dir>` (`~/.qidi/debug`).
    PerSession { dir: PathBuf },
    /// An explicit path → one flat `fmt` file, routing bypassed.
    SingleFile { path: PathBuf, src: DebugSource },
}

/// Resolve the debug target, honoring precedence: explicit QIDI_LOG_FILE wins
/// (single file, RUST_LOG filter); else QIDI_DEBUG_LOG — a truthy bool routes
/// per session into `~/.qidi/debug`, an explicit path writes a single file.
///
/// Read via `var_os` (not `var`) so a non-UTF-8 path isn't silently dropped.
pub(crate) fn resolve_debug_target() -> Option<DebugTarget> {
    let grok_log_file = std::env::var_os("QIDI_LOG_FILE");
    let grok_debug_log = std::env::var_os("QIDI_DEBUG_LOG");
...
/// `~/.qidi/debug` older than [`LOG_RETENTION`] so the dir doesn't grow
...
mod tests {
        ...
        for v in ["0", "false", "off", "no", "", "  "] {
            assert!(
                resolve_debug_target_inner(None, Some(OsStr::new(v)), Path::new("/debug"))
                    .is_none(),
                "expected None for QIDI_DEBUG_LOG={v:?}"
            );
        }
    }
    ...
    fn resolve_target_debug_log_enabled_is_per_session_dir() {
        for v in ["1", "true", "on", "yes"] {
            ...
            assert_eq!(
                ...
                "expected PerSession for QIDI_DEBUG_LOG={v:?}"
            );
        }
    }
    ...
    fn resolve_target_empty_log_file_falls_through_to_debug_log() {
        // Empty / whitespace QIDI_LOG_FILE is treated as unset (mirrors QIDI_DEBUG_LOG).
        for blank in ["", "   "] {
            let target = resolve_debug_target_inner(
    ...
    fn resolve_target_non_utf8_debug_log_path_is_single_file() {
        ...
        // A non-UTF-8 QIDI_DEBUG_LOG value is a path, not a bool keyword, and its
    ...
    fn layer_routes_event_by_session_span() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            ...
            tracing::info_span!("session", session_id = %"sess-xyz").in_scope(|| {
                tracing::info!(target: "cf_shell", "inside session");
            });
            tracing::info!(target: "cf_shell", "outside session");
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_under_real_firehose_filter() {
        ...
        // `cf_telemetry::session_ctx`) survives the real filter so
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!(
                target: "cf_telemetry::session_ctx",
                "session",
                session_id = %"sid-real"
            )
            .in_scope(|| {
                tracing::debug!(target: "cf_shell", "filtered routing works");
            });
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_two_sessions_to_distinct_files() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!("session", session_id = %"sid-one").in_scope(|| {
                ...
                tracing::info!(target: "cf_shell", "one first");
                tracing::info!(target: "cf_shell", "one second");
            });
            tracing::info_span!("session", session_id = %"sid-two").in_scope(|| {
                tracing::info!(target: "cf_shell", "two only");
            });
        });
        crate::appender::flush_file_log_guards();
