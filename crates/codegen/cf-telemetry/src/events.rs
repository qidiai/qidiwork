...
//!
//! These structs were extracted from `qidi-code` so they can be
...
impl CompactionScope {
    pub fn begin(
        ...
        let percentage = cf_token_estimation::usage_percentage_u8(tokens_used, context_window);
...
pub struct ToolCallCompleted {
    pub tool_name: String,
    pub outcome: cf_file_utils::events::types::ToolOutcome,
    pub duration_ms: u64,
...
// ---------------------------------------------------------------------------
// Pager events (called from qidi-code via log_event)
// ---------------------------------------------------------------------------
...
/// first prompt when QIDI Code is launched from a non-project directory).
...
/// attributed to QIDI Code: the user saw the gate in QIDI Code, went and
...
pub struct SubscriptionActivated {
    ...
    /// is strongly attributable to QIDI Code's upsell surface.
