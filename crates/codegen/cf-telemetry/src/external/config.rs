...
//! `QIDI_EXTERNAL_OTEL=1` *and* at least one of `OTEL_METRICS_EXPORTER` /
...
/// Master switch env var. Deliberately *not* `QIDI_ENABLE_TELEMETRY`: that
/// would be a word-order typo away from the long-standing
/// `QIDI_TELEMETRY_ENABLED` (product events/Mixpanel mode), and the two control
/// opposite-pointing data flows (to xAI vs. to the customer's collector).
pub const ENV_MASTER_SWITCH: &str = "QIDI_EXTERNAL_OTEL";
...
mod tests {
    ...
    fn master_switch_alone_enables_nothing() {
        // RQ7: QIDI_EXTERNAL_OTEL=1 without an explicit exporter is inert.
        assert!(
            ExternalOtelConfig::resolve_with(env(&[("QIDI_EXTERNAL_OTEL", "1")]), None).is_none()
        );
    }
    ...
    fn file_config_layered_under_env() {
        ...
        let cfg =
            ExternalOtelConfig::resolve_with(env(&[("QIDI_EXTERNAL_OTEL", "0")]), Some(&file));
        assert!(cfg.is_none());
    }
