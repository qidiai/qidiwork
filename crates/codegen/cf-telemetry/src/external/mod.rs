...
//! collector (standard `OTEL_*` env vars + the `QIDI_EXTERNAL_OTEL` master
...
//!   headers.
//! - Default **off**: with `QIDI_EXTERNAL_OTEL` unset (or no exporter
...
impl IdentityAttrs {
    pub fn from_snapshot(snapshot: &cf_auth::CredentialSnapshot) -> Self {
        Self {
    ...
    if cfg.internal_pipeline_consumed_otel_vars {
        tracing::warn!(
            "external otel: refusing to activate — the internal trace pipeline consumed \
             ...
             QIDI_INTERNAL_OTLP_* to use the external stream."
        );
        return None;
    }
