...
//! file logger that writes to `~/.qidi/logs/hooks.log`.
//!
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with
//! targets `cf_hooks` or `cf_agent::plugins` at key lifecycle
//! points — discovery, dispatch, execution, errors.
//!
//! ## Enabling
//!
//! ```bash
//! QIDI_HOOKS_LOG=1 grok              # enable, write to ~/.qidi/logs/hooks.log
//! QIDI_HOOKS_LOG=/tmp/h.log grok     # write to custom path
//! QIDI_HOOKS_LOG=0 grok              # explicitly disable
//! tail -f ~/.qidi/logs/hooks.log     # watch in another terminal
//! ```
...
const ENV_HOOKS_LOG: &str = "QIDI_HOOKS_LOG";
...
///
/// Writes to `~/.qidi/logs/hooks.log` (or custom path via `QIDI_HOOKS_LOG`).
/// Filters to hooks (`cf_hooks`) and plugins (`cf_agent::plugins`) targets.
/// Set `QIDI_HOOKS_LOG=0` to disable, `QIDI_HOOKS_LOG=/path` to redirect.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "cf_hooks=debug,cf_agent::plugins=debug",
    );
