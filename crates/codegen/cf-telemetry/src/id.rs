...
//!
//! Extracted from `qidi-code::agent::unique_identifier` so the
//! telemetry engine can stamp events without depending on shell internals.
//! `$QIDI_HOME` is resolved through `xai-grok-config::grok_home`.
...
/// This function caches the result in `$QIDI_HOME/agent_id` so subsequent calls
...
fn load_or_compute_agent_id() -> String {
    let cache_path = cf_config::grok_home().join("agent_id");
...
///
/// Enabled only when `QIDI_TELEMETRY_SPECIAL_USER=1` (or `true`). There is no
/// hardcoded username allowlist.
pub fn is_special_user() -> bool {
    matches!(
        std::env::var("QIDI_TELEMETRY_SPECIAL_USER").as_deref(),
