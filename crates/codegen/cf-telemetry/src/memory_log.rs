...
//!
//! Provides a dedicated tracing target (`cf_memory`) with an optional
//! file logger that writes to `~/.qidi/logs/memory.log`.
//!
//! ## When to use
//!
...
//! QIDI_MEMORY_LOG=0 grok                # disable even when enabled
//! tail -f ~/.qidi/logs/memory.log      # watch in another terminal
//! ```

/// Tracing target for all memory system operations.
pub const TARGET: &str = "cf_memory";
...
mod inner {
    ...
    const ENV_MEMORY_LOG: &str = "QIDI_MEMORY_LOG";
    ...
    ///
    /// Writes to `~/.qidi/logs/memory.log`. Filters to `cf_memory=trace`.
    /// Set `QIDI_MEMORY_LOG=0` to disable, `QIDI_MEMORY_LOG=/path` to redirect.
