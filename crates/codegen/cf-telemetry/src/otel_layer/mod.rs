...
//!
//! `qidi-code` uses this module to set up OTLP
...
const ENV_OTEL_FILTER: &str = "QIDI_OTEL_FILTER";
...
///
/// Built by the binaries (`qidi-code`) from their own
/// configuration. The credentials provider is constructed by shell's
/// `cf_shell::auth::credential_provider::build_otel_credential_provider`.
pub struct OtelLayerConfig {
    ...
    /// Value for the `X-XAI-Token-Auth` header (typically `"cli"`).
...
pub struct OtelExporterConfig {
    /// Full OTLP traces endpoint URL (e.g. `https://api.qidiai.ltd/v1/traces`).
    ...
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let otel_filter = tracing_subscriber::filter::EnvFilter::try_new(&otel_filter)
        .unwrap_or_else(|e| {
            eprintln!(
                "[otel] Invalid QIDI_OTEL_FILTER '{}': {}. Using default '{}'.",
...
fn build_export_headers(
    ...
    snapshot: &cf_auth::CredentialSnapshot,
...
mod tests {
    ...
    use std::sync::Mutex;
    use cf_auth::{AuthCredentialProvider, CredentialSnapshot, HttpAuth};
