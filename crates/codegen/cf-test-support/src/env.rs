...
pub fn grok_binary() -> PathBuf {
    if let Ok(path) = std::env::var("QIDI_BINARY") {
        ...
        assert!(p.exists(), "QIDI_BINARY does not exist: {}", p.display());
        return p;
    }
...
pub fn test_env_cmd_tokio(
    ...
    cmd.env("HOME", home)
        ...
        // without an explicit QIDI_HOME every spawned child shares the real
        ...
        .env("QIDI_CLI_CHAT_PROXY_BASE_URL", mock_url)
        .env("QIDI_XAI_API_BASE_URL", mock_url)
        ...
        .env("QIDI_DISABLE_AUTOUPDATER", "1");
}
