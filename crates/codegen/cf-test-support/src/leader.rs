...
use cf_acp_lib::LineBufferedRead;
...
/// (`QIDI_BINARY_LEADER`, else the shared [`grok_binary`] resolution).
pub fn leader_binary() -> PathBuf {
    role_binary(LEADER_BINARY_ENV)
}

/// Binary for the client side of a version-skew test (`QIDI_BINARY_CLIENT`,
...
impl LeaderStdioClient {
    ...
    pub async fn spawn_with_binary(
        ...
        cmd.args(["agent", "--leader", "stdio"])
            .current_dir(cwd)
            // Hermetic env: the developer's shell may export QIDI_* vars
            // (e.g. QIDI_LEADER_SOCKET pointing at a REAL leader on this
            ...
            .env("QIDI_TELEMETRY_ENABLED", "false")
            .env("QIDI_FEEDBACK_ENABLED", "false")
            .env("QIDI_TRACE_UPLOAD", "false")
            .env("QIDI_INSTRUMENTATION", "disabled")
            // Inherited by the spawned leader, whose stderr goes to
            // ~/.qidi/leader.log — keep it chatty for diagnosis.
            .env("RUST_LOG", "cf_shell=debug");
