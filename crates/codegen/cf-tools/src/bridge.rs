//! ToolBridge: adapter that wraps `qidi-code`'s `ToolRegistry` and
...
impl ToolBridge {
    ...
    /// `Some("get_task_output")` for the qidi_build agent and `None` for
    ...
    pub async fn kill_background_task(
        &self,
        task_id: &str,
    ) -> Result<KillOutcome, cf_tool_runtime::ToolError> {
        if let Some(terminal) = &self.terminal {
    ...
    pub async fn delete_scheduled_task(
        &self,
        task_id: &str,
    ) -> Result<bool, cf_tool_runtime::ToolError> {
        use crate::implementations::qidi_build::scheduler::types::{
        ...
        let sender = {
            ...
            res.get::<SchedulerHandle>()
                .ok_or_else(|| {
                    cf_tool_runtime::ToolError::custom("missing_resource", "SchedulerHandle")
                })?
                .0
                .clone()
        };
        ...
        sender
            ...
            .map_err(|_| {
                cf_tool_runtime::ToolError::custom("process_manager", "Scheduler actor stopped")
            })?;
        reply_rx.await.map_err(|_| {
            cf_tool_runtime::ToolError::custom("process_manager", "Scheduler actor dropped reply")
        })
    }
...
mod tests {
    ...
    impl cf_tool_runtime::Tool for KindFixture {
        type Args = serde_json::Value;
        type Output = String;

        fn id(&self) -> cf_tool_protocol::ToolId {
            cf_tool_protocol::ToolId::new(self.id).expect("valid id")
        }
        fn description(
            &self,
            _ctx: &::cf_tool_runtime::ListToolsContext,
        ) -> cf_tool_types::ToolDescription {
            cf_tool_types::ToolDescription::new(self.id, "kind fixture")
        }
        async fn run(
            &self,
    ...
    fn tool_kind_returns_registered_kind_per_namespace_and_none_for_unknown() {
        ...
        // PascalCase + qidi_build's snake_case in one registry
