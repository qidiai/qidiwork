...
//! config helper `qidi-code::util::config::resolve_search_tools_enabled`
//! (requirements > env `QIDI_TOOLS_FIND_BFS` / `QIDI_TOOLS_GREP_UGREP` (+
//! `QIDI_FIND_BFS` / `QIDI_GREP_UGREP` aliases, `DISABLE_EMBEDDED_SEARCH_TOOLS`
...
//! (release builds, self-extracted to `~/.qidi/vendor/<name>-<ver>-<target>`) →
//! `~/.qidi/vendor/{name}` if a regular file → `which` on the agent `$PATH`.
...
const BFS_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-bfs/bfs-",
    env!("QIDI_TOOLS_BFS_VER"),
    "-",
    env!("QIDI_TOOLS_BFS_TARGET"),
    ".bin"
));
...
const UGREP_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-ugrep/ugrep-",
    env!("QIDI_TOOLS_UGREP_VER"),
    "-",
    env!("QIDI_TOOLS_UGREP_TARGET"),
    ".bin"
));
...
fn resolved_tools() -> &'static ResolvedTools {
    ...
    TOOLS.get_or_init(|| ResolvedTools {
        bfs: resolve_tool("bfs", "QIDI_TOOLS_BFS_PATH", bundled_bfs()),
        ugrep: resolve_tool("ugrep", "QIDI_TOOLS_UGREP_PATH", bundled_ugrep()),
    })
}

/// Write embedded `bytes` to `~/.qidi/vendor/<versioned_name>` (chmod 755) on
...
/// `~/.qidi/vendor/<bin>` → `which`. Env and vendor only require `is_file()` here
...
mod tests {
    ...
    /// `QIDI_TOOLS_BUNDLE_{BFS,UGREP}_PATH` at build time). Verifies the embedded
    /// bytes self-extract under `~/.qidi/vendor` and the extracted `bfs` runs.
    ...
    /// mode-0644 `QIDI_TOOLS_*_PATH` / vendor copy) must fall through to the OS
