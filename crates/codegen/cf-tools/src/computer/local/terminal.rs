...
impl LocalTerminalActor {
    ...
    async fn spawn_persistent_command(
        ...
        // Apply SHELL_ENV_OVERRIDES (TERM=dumb, NO_COLOR, QIDI_AGENT=1, etc.)
        ...
        #[cfg(target_os = "linux")]
        if cf_sandbox::should_restrict_child_network() {
            unsafe {
                cmd.pre_exec(|| cf_sandbox::child_net::install_child_network_filter());
            }
        }
                ...
                && process.exit_status.is_none()
            {
                ...
                if process.kind == crate::computer::types::TaskKind::Monitor {
                    ...
                    let start_offset = std::fs::metadata(&pipeline_output_file)
                        ...
                        .unwrap_or(0);
                    tokio::spawn(async move {
                        crate::implementations::qidi_build::monitor::tool::run_monitor_pipeline(
                            &pipeline_task_id,
...
fn spawn_shell_command(
    ...
    let build_cmd = |with_breakaway: bool| {
        ...
        let inv = cf_config::shell::shell_command_argv(command);
...
mod tests {
    ...
    async fn test_persistent_shell_env_var_persists() {
        ...
        let result = backend
            .run(make_request("export QIDI_PERSIST_TEST=hello123"))
            .await
            .unwrap();
        assert_eq!(result.exit_code, Some(0));

        let result = backend
            .run(make_request("echo $QIDI_PERSIST_TEST"))
            .await
            .unwrap();
        assert_eq!(result.exit_code, Some(0));
        assert_eq!(
