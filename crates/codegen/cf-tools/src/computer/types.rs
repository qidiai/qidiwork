...
/// (`qidi-code::extensions::task::KillTaskResponse`) and deserialized
/// by clients (qidi-code), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in qidi-code, spawns processes)
/// - `AcpTerminalBackend` (in qidi-code, calls ACP protocol)
#[async_trait::async_trait]
