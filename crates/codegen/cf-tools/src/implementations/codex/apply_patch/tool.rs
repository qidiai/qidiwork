        ...
        && !parent.as_os_str().is_empty()
    {
        tokio::fs::create_dir_all(parent).await.map_err(|e| {
            cf_tool_runtime::ToolError::execution(
                cf_tool_protocol::ToolId::new("apply_patch").expect("valid"),
                e.to_string(),
            )
        })?;
    }
...
impl cf_tool_runtime::Tool for ApplyPatchTool {
    type Args = ApplyPatchInput;
    type Output = ApplyPatchOutput;

    fn id(&self) -> cf_tool_protocol::ToolId {
        cf_tool_protocol::ToolId::new("apply_patch").expect("valid tool id")
    }
    ...
    ) -> Result<ApplyPatchOutput, cf_tool_runtime::ToolError> {
...
mod tests {
    ...
    async fn add_file_creates_with_correct_content() {
        ...
        let result =
            cf_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), make_input(&patch))
                .await
                .unwrap();
