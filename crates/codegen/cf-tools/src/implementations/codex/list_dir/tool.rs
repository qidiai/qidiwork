...
//! absolute paths. See the plan document for the full diff vs the cf-tools
//! `ListDirTool`.
...
impl cf_tool_runtime::Tool for CodexListDirTool {
...
mod tests {
    ...
    async fn empty_directory_returns_success() {
        ...
        let output = cf_tool_runtime::Tool::run(&tool, ctx, input)
            .await
            .unwrap();
        match output {
            ListDirOutput::Content(content) => {
