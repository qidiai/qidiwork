...
impl cf_tool_runtime::Tool for BashTool {
    ...
    fn id(&self) -> cf_tool_protocol::ToolId {
        cf_tool_protocol::ToolId::new("bash").expect("valid tool id")
    }
        ...
        let result = match backend.run(request).await {
            ...
            Err(e) => {
                ...
                return Err(cf_tool_runtime::ToolError::execution(
                    cf_tool_protocol::ToolId::new("run_terminal_command").expect("valid"),
                    e.to_string(),
                ));
            }
        };
...
mod tests {
    ...
    fn rt_ctx_with_call_id(
        ...
        let id = cf_tool_protocol::ToolCallId::new(call_id).unwrap();
    ...
    async fn tool_metadata() {
        ...
        let tool = BashTool;
        assert_eq!(cf_tool_runtime::Tool::id(&tool).as_str(), "bash");
