...
impl cf_tool_runtime::ToolOutput for GlobOutput {}
...
impl cf_tool_runtime::Tool for GlobTool {
    type Args = GlobInput;
    type Output = GlobOutput;

    fn id(&self) -> cf_tool_protocol::ToolId {
        cf_tool_protocol::ToolId::new("glob").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: GlobInput,
    ) -> Result<GlobOutput, cf_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        let tool = GlobTool;
        assert_eq!(cf_tool_runtime::Tool::id(&tool).as_str(), "glob");
    ...
    async fn gitignore_respected() {
        ...
        cf_test_utils::git::ensure_hermetic_git_on_path();
