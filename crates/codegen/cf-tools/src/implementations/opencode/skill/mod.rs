...
${%- endfor %}
${%- else %}
(No skills available. Skills can be added in ~/.qidi/skills/ or .qidi/skills/)
${%- endif %}
</available_skills>"#;
...
impl cf_tool_runtime::Tool for SkillTool {
    type Args = SkillInput;
    type Output = SkillOutput;

    fn id(&self) -> cf_tool_protocol::ToolId {
        cf_tool_protocol::ToolId::new("skill").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: SkillInput,
    ) -> Result<SkillOutput, cf_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        let tool = SkillTool;
        assert_eq!(cf_tool_runtime::Tool::id(&tool).as_str(), "skill");
