...
//!
//! - **`qidi-code`** — tool blocks on a oneshot, formats the result.
//! - **`qidi-code`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`qidi-code`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `qidi-code`.
...
register_resource!("qidi_build", "UserQuestionSender", UserQuestionSender);
