...
use regex::Regex;
use cf_config::shell::AmpersandSemantics;
...
static BASH_CAPABILITIES: LazyLock<cf_tool_protocol::ToolCapabilities> =
    LazyLock::new(|| cf_tool_protocol::ToolCapabilities {
        is_read_only: false,
        tool_scope: Some(cf_tool_protocol::ToolScope::Write),
        streaming: Some(cf_tool_protocol::StreamingSpec {
...
pub struct BashParams {
    ...
    /// cf-tools opts up to 10h via qidi-code's `BashToolConfig`.
    ///
    /// When set:
...
impl crate::types::resources::ResourceType for BashParams {
    const ID: &'static str = "qidi_build.Bash";
...
impl cf_tool_runtime::ToolOutput for BashToolOutput {
    fn chat_completion_output(&self) -> Option<cf_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Foreground(bash) => cf_tool_runtime::ToolOutput::chat_completion_output(bash),
...
// particular production cf-tools sets it to 10h in qidi-code's
...
/// [`cf_config::shell::ampersand_semantics`] reports POSIX `&` semantics
/// (Unix + Git Bash).
...
/// should use `background: true`. Env override: `QIDI_MAX_FOREGROUND_BLOCK_MS`.
const MAX_FOREGROUND_BLOCK: Duration = Duration::from_secs(300); // 5 minutes

fn max_foreground_block() -> Duration {
    std::env::var("QIDI_MAX_FOREGROUND_BLOCK_MS")
        .ok()
...
// ───────────────────────────────────────────────────────────────────────────
// Bare `echo "<msg>"` detection (for statistics + hints in qidi_build bash)
// ───────────────────────────────────────────────────────────────────────────
...
/// communication. We want statistics on this pattern for the qidi_build
...
impl BashTool {
    ...
    ///   the terminal backend default applies (`QIDI_FOREGROUND_BLOCK_BUDGET_MS`
    ...
    /// still honors `QIDI_FOREGROUND_BLOCK_BUDGET_MS` via `None` on the request.
    ///
...
${%- endif %}"#
    ...
    /// B7: 把 `cmd_prefix` 解析为 shell-safe 的 argv tokens，避免直接字符串拼接
    /// 导致的 shell 注入。
    ///
    /// 返回 `(prefix_args, command)`：`prefix_args` 由 `shlex::split` 解析；
    /// 若 prefix 含无效 shell 语法（如未闭合引号）则丢弃并 warn。调用方对每个
    /// token 再做 `shlex::try_quote` 转义后才拼进命令行，因此 prefix 里的 shell
    /// 元字符（`;`、`&&`、`|` 等）只会作为字面量参数，不会被解释执行。
    fn get_prefixed_command(cmd_prefix: &Option<String>, command: &str) -> (Vec<String>, String) {
        match cmd_prefix {
            Some(prefix) => match shlex::split(prefix) {
                Some(args) => (args, command.to_string()),
                None => {
                    tracing::warn!(
                        prefix = %prefix,
                        "cmd_prefix contains invalid shell syntax; dropping prefix"
                    );
                    (Vec::new(), command.to_string())
                }
            },
            None => (Vec::new(), command.to_string()),
        }
    }

    /// B7: 用解析后的 prefix tokens 组装最终 shell 命令行。每个 token 经
    /// `shlex::try_quote` 转义，元字符只作字面量；prefix 作为命令的前缀 wrapper
    /// （如 `env FOO=bar`、`nice -n 19`、`timeout 10`）出现在真实命令之前。
    fn compose_prefixed_command(prefix_args: &[String], command: String) -> String {
        ...
        for arg in prefix_args {
            ...
            let safe = shlex::try_quote(arg)
                .map(|c| c.into_owned())
                .unwrap_or_else(|_| arg.clone());
            quoted.push_str(&safe);
        }
...
impl cf_tool_runtime::Tool for BashTool {
    type Args = BashToolInput;
    type Output = BashToolOutput;

    fn id(&self) -> cf_tool_protocol::ToolId {
        cf_tool_protocol::ToolId::new("run_terminal_cmd").expect("valid tool id")
    }
    ...
    async fn execute(
        &self,
        mut ctx: cf_tool_runtime::ToolCallContext,
        input: BashToolInput,
    ) -> cf_tool_runtime::ToolStream<BashToolOutput> {
        ...
        if input.is_background {
            ...
            return Box::pin(async_stream::stream! {
                yield cf_tool_runtime::ToolStreamItem::Terminal(this.run(ctx, input).await);
            });
        }
        ...
        let Some(spec) = BASH_CAPABILITIES.streaming.as_ref() else {
            return Box::pin(async_stream::stream! {
                yield cf_tool_runtime::ToolStreamItem::Terminal(Err(
            ...
            loop {
                tokio::select! {
                    ...
                    maybe = rx.recv() => {
                        match maybe {
                            Some(ToolNotification::BashOutputChunk(chunk)) => {
                                ...
                                {
                                    yield cf_tool_runtime::ToolStreamItem::Progress(p);
                                }
                            }
                            Some(notif) => {
                    ...
                    result = &mut run_fut => {
                        ...
                        yield cf_tool_runtime::ToolStreamItem::Terminal(result);
                        break;
                    }
                }
            }
    ...
    async fn run(
        ...
        input: BashToolInput,
    ) -> Result<BashToolOutput, cf_tool_runtime::ToolError> {
        ...
        let ampersand = cf_config::shell::ampersand_semantics();
        ...
        // --- Prefix ---
        // B7: prefix 先 shlex 解析成 argv tokens，再逐个 shell-quote 拼接，
        // 元字符只作字面量，避免注入。
        let (prefix_args, base_command) =
            Self::get_prefixed_command(&params.cmd_prefix, &input.command);
        let command = Self::compose_prefixed_command(&prefix_args, base_command);
        ...
        if input.is_background {
            ...
            let request = TerminalRunRequest {
                ...
                // existing qidi_build callers that never opted in are
                // unaffected.
            ...
            // We tag it for statistics (qidi_build backend) and can surface an educational
...
mod tests {
    ...
    fn read_chunk_progress(p: &cf_tool_runtime::ToolProgress) -> (String, usize, bool, bool) {
        match p {
            cf_tool_runtime::ToolProgress::Custom { subkind, payload } => {
    ...
    fn bash_output_chunk_progress_caps_oversized_delta() {
        ...
        while last < total {
            let p = bash_output_chunk_progress(spec, &chunk, &mut last).unwrap();
            let cf_tool_runtime::ToolProgress::Custom { payload, .. } = p else {
    ...
    async fn bash_streaming_progress_suppressed_when_gate_off() {
        ...
        let mut terminal: Option<Result<BashToolOutput, cf_tool_runtime::ToolError>> = None;
        while let Some(item) = stream.next().await {
            match item {
                cf_tool_runtime::ToolStreamItem::Progress(_) => {
                    progress += 1;
                }
    ...
    async fn cmd_prefix_prepended() {
        // B7: get_prefixed_command 现在返回 (prefix_args, command)。
        let (args, cmd) =
            BashTool::get_prefixed_command(&Some("source ~/.bashrc".to_string()), "ls");
        assert_eq!(args, vec!["source".to_string(), "~/.bashrc".to_string()]);
        assert_eq!(cmd, "ls");

        let (args, cmd) = BashTool::get_prefixed_command(&None, "ls");
        assert!(args.is_empty());
        assert_eq!(cmd, "ls");
    }

    #[tokio::test]
    async fn cmd_prefix_with_shell_metacharacters_not_injected() {
        // B7: prefix 含 "; rm -rf /" 被 shlex 拆成字面量 token，不会作为命令分隔符执行。
        let (args, cmd) = BashTool::get_prefixed_command(&Some("; rm -rf /".to_string()), "ls");
        assert_eq!(
            args,
            vec![
                ";".to_string(),
        ...
        // 组装后每个 token 被 shell-quote，`;` 只是字面量参数而非分隔符。
        let composed = BashTool::compose_prefixed_command(&args, cmd);
        assert!(
            composed.contains("';'") || composed.contains("\";\""),
            "metacharacter must be quoted, got: {composed}"
        );
    }

    #[tokio::test]
    async fn cmd_prefix_invalid_shell_dropped() {
        // B7: 未闭合引号是无效 shell 语法，shlex 拒绝解析，prefix 被丢弃。
        let (args, cmd) = BashTool::get_prefixed_command(&Some("'unclosed".to_string()), "ls");
        assert!(args.is_empty());
        assert_eq!(cmd, "ls");
    }

    #[tokio::test]
    async fn cmd_prefix_wrapper_composed_before_command() {
        // B7: wrapper 风格 prefix（env FOO=bar）出现在真实命令之前。
        let (args, cmd) =
    ...
    async fn errors_when_terminal_not_in_resources() {
        ...
        let result =
            cf_tool_runtime::Tool::run(&tool, test_ctx(resources.into_shared()), make_input("ls"))
                .await;
        assert!(result.is_err());
        assert!(
            result
    ...
    fn foreground_chat_completion_emits_code_execution_result() {
        let resp = cf_tool_runtime::ToolOutput::chat_completion_output(
    ...
    fn background_chat_completion_is_none() {
        ...
        assert!(cf_tool_runtime::ToolOutput::chat_completion_output(&out).is_none());
    }
    ...
    mod versioned_background_check {
        ...
        ) -> cf_tool_runtime::ToolCallContext {
            let mut ctx =
                cf_tool_runtime::ToolCallContext::new(cf_tool_protocol::ToolCallId::new_v7());
            ctx.extensions.insert(resources);
            ctx.extensions.insert(cf_tool_runtime::BehaviorVersion(
