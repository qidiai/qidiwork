...
crate::register_resource!("qidi_build", "Grep", GrepParams);
...
fn grep_timeout() -> Duration {
    Duration::from_secs(grep_timeout_secs(cf_tty_utils::is_wsl()))
}
...
static GREP_CAPABILITIES: LazyLock<cf_tool_protocol::ToolCapabilities> =
...
impl cf_tool_runtime::Tool for GrepTool {
    type Args = GrepSearchInput;
    type Output = GrepSearchOutput;

    fn id(&self) -> cf_tool_protocol::ToolId {
        cf_tool_protocol::ToolId::new("grep").expect("valid tool id")
    }
    ...
    async fn execute(
    ...
    ) -> cf_tool_runtime::ToolStream<GrepSearchOutput> {
        ...
        let Some(spec) = admitted_spec else {
            return cf_tool_runtime::terminal_only(self.run(ctx, input).await);
        };
    ...
    async fn run(
        ...
        ctx: cf_tool_runtime::ToolCallContext,
        input: GrepSearchInput,
    ) -> Result<GrepSearchOutput, cf_tool_runtime::ToolError> {
...
fn grep_progress_stream(
    ctx: cf_tool_runtime::ToolCallContext,
    input: GrepSearchInput,
    spec: &'static cf_tool_protocol::StreamingSpec,
    span: tracing::Span,
) -> cf_tool_runtime::ToolStream<GrepSearchOutput> {
    Box::pin(async_stream::stream! {
        ...
        let GrepReady {
            ...
            Ok(GrepStep::Early(out)) => {
                ...
                yield cf_tool_runtime::ToolStreamItem::Terminal(Ok(out));
                return;
            }
            Err(e) => {
                yield cf_tool_runtime::ToolStreamItem::Terminal(Err(e));
                return;
            }
        };
        ...
        if timed_out {
            ...
            if stdout_buf.is_empty() {
                yield cf_tool_runtime::ToolStreamItem::Terminal(Ok(grep_timeout_output(secs)));
            } else {
                ...
                output.exit_code = -1;
                yield cf_tool_runtime::ToolStreamItem::Terminal(Ok(output));
            }
            return;
        }
...
async fn prepare_grep(
    ctx: &cf_tool_runtime::ToolCallContext,
    input: &GrepSearchInput,
) -> Result<GrepStep, cf_tool_runtime::ToolError> {
...
struct BodyStreamer<'a> {
    spec: &'a cf_tool_protocol::StreamingSpec,
...
impl<'a> BodyStreamer<'a> {
    fn new(spec: &'a cf_tool_protocol::StreamingSpec, config: &'a GrepFormatConfig) -> Self {
        Self {
            spec,
            config,
            body: String::new(),
    ...
    fn feed(&mut self, bytes: &[u8]) -> Vec<cf_tool_runtime::ToolProgress> {
    ...
    fn finish(&mut self) -> Option<cf_tool_runtime::ToolProgress> {
    ...
    fn push_line(&mut self, line: &[u8]) -> Option<cf_tool_runtime::ToolProgress> {
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        let tool = GrepTool;
        assert_eq!(cf_tool_runtime::Tool::id(&tool).as_str(), "grep");
    ...
    async fn tool_grep_no_matches() {
        ...
        let tool = GrepTool;
        let output = cf_tool_runtime::Tool::run(
            &tool,
    ...
    async fn deny_read_globs_exclude_denied_files() {
        ...
        let run = |resources: Resources, input| async {
            let out =
                cf_tool_runtime::Tool::run(&GrepTool, test_ctx(resources.into_shared()), input)
                    .await
                    .unwrap();
    ...
    fn read_grep_delta(p: &cf_tool_runtime::ToolProgress) -> String {
        match p {
    ...
    async fn grep_streaming_body_matches_card_body() {
        ...
        let mut terminal: Option<Result<GrepSearchOutput, cf_tool_runtime::ToolError>> = None;
    ...
    async fn grep_streaming_suppressed_when_gate_off() {
        ...
        let tool = GrepTool;
        let mut stream =
            cf_tool_runtime::Tool::execute(&tool, ctx, make_grep_input("findme")).await;

        let mut progress = 0usize;
        let mut terminal: Option<Result<GrepSearchOutput, cf_tool_runtime::ToolError>> = None;
        while let Some(item) = stream.next().await {
            match item {
                cf_tool_runtime::ToolStreamItem::Progress(_) => progress += 1,
                cf_tool_runtime::ToolStreamItem::Terminal(r) => {
                    assert!(terminal.is_none(), "more than one Terminal yielded");
                    terminal = Some(r);
                }
    ...
    async fn grep_streaming_body_matches_card_body_when_truncated() {
        ...
        let tool = GrepTool;
        let mut stream =
            cf_tool_runtime::Tool::execute(&tool, test_ctx(resources.into_shared()), input).await;
