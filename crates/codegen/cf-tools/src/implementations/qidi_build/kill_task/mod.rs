...
use crate::implementations::qidi_build::task::types::SubagentCancelOutcome;
use crate::types::requirements::{Expr, ToolRequirement};
use crate::types::resources::Terminal;
use crate::types::template_renderer::TemplateRenderer;
use crate::types::tool::ToolKind;
use crate::types::tool::ToolNamespace;
use cf_tool_types::{KillTaskOutput, KillTaskResult, KillTaskToolInput};
...
impl crate::types::tool_metadata::ToolMetadata for KillTaskTool {
    ...
    fn description_template(&self) -> &str {
        ...
        // fallback mirrors the default cf-tools toolset on the current OS.
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            cf_tool_types::build_kill_task_description(&cf_tool_types::KillTaskToolNaming {
    ...
    fn requires_expr(&self) -> Expr<ToolRequirement> {
        ...
        let task_tool = Expr::Value(ToolRequirement::Tool {
            ...
            id: cf_tool_runtime::Tool::id(&TaskTool).to_string(),
            if_params: None,
        });
        let mut arms =
            crate::implementations::qidi_build::task_output::background_bash_requires_exprs();
        arms.push(task_tool);
        Expr::Or(arms)
    }
}
...
/// [`cf_tool_types::build_kill_task_description`] builder so the CLI and
...
impl cf_tool_runtime::Tool for KillTaskTool {
    type Args = KillTaskToolInput;
    type Output = KillTaskOutput;

    fn id(&self) -> cf_tool_protocol::ToolId {
        cf_tool_protocol::ToolId::new("kill_task").expect("valid tool id")
    }
    ...
    ) -> Result<KillTaskOutput, cf_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn make_ctx_with_version(
        ...
        ctx.extensions
            .insert(cf_tool_runtime::BehaviorVersion(version.to_owned()));
        ctx
    }
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(cf_tool_runtime::Tool::id(&tool).as_str(), "kill_task");
