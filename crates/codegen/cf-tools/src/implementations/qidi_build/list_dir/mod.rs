...
crate::register_resource!("qidi_build", "ListDir", ListDirParams);
...
impl cf_tool_runtime::Tool for ListDirTool {
    type Args = ListDirInput;
    type Output = ListDirOutput;
    fn id(&self) -> cf_tool_protocol::ToolId {
        cf_tool_protocol::ToolId::new("list_dir").expect("valid tool id")
    }
    fn description(
        &self,
    ...
    async fn run(
        ...
        input: ListDirInput,
    ) -> Result<ListDirOutput, cf_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        let tool = ListDirTool;
        assert_eq!(cf_tool_runtime::Tool::id(&tool).as_str(), "list_dir");
