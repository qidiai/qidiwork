...
//! `LspTool` (Tool trait impl) under the `QidiBuild` namespace.
...
impl cf_tool_runtime::ToolOutput for LspToolOutput {}
...
impl cf_tool_runtime::Tool for LspTool {
    type Args = LspToolInput;
    type Output = LspToolOutput;

    fn id(&self) -> cf_tool_protocol::ToolId {
        cf_tool_protocol::ToolId::new("lsp").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: LspToolInput,
    ) -> Result<LspToolOutput, cf_tool_runtime::ToolError> {
            ...
            handle = res
                ...
                .ok_or_else(|| {
                    cf_tool_runtime::ToolError::custom(
                        "process_manager",
                        "LSP tool is unavailable. Configure ~/.qidi/lsp.json or <cwd>/.qidi/lsp.json and ensure the language server can start.",
                    )
                })?
                .clone();
        }
