...
register_resource!("qidi_build", "SearchReplace", SearchReplaceParams);
...
) -> Result<SearchReplaceOutput, cf_tool_runtime::ToolError> {
            ...
            let output = match e.io_error_kind() {
                ...
                _ => {
                    return Err(cf_tool_runtime::ToolError::execution(
                        cf_tool_protocol::ToolId::new("search_replace").expect("valid"),
                        e.to_string(),
                    ));
                }
            };
...
impl cf_tool_runtime::Tool for SearchReplaceTool {
    type Args = SearchReplaceInput;
    type Output = SearchReplaceOutput;
    fn id(&self) -> cf_tool_protocol::ToolId {
        cf_tool_protocol::ToolId::new("search_replace").expect("valid tool id")
    }
    fn description(
        &self,
...
mod tests {
    ...
    async fn consecutive_edits_succeed_without_prior_read() {
        ...
        let result1 = cf_tool_runtime::Tool::run(
            &tool,
        ...
        let result2 = cf_tool_runtime::Tool::run(
            &tool,
            test_ctx(shared),
