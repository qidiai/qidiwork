...
async fn build_render_context(
    ...
    file_path: &str,
) -> Result<RenderContext, cf_tool_runtime::ToolError> {
...
pub(crate) async fn downgrade_structured_errors(
    ...
    use SearchReplaceOutput::{FileAlreadyExists, FileNotFound, FilenameTooLong, MultipleMatchesFound, NoMatchesFound};
    let ctx = build_render_context(resources, file_path).await?;
    Ok(match output {
        FileAlreadyExists(_) => SearchReplaceOutput::InvalidInput(render_error(ErrorKind::FileAlreadyExists, &ctx)),
        FileNotFound(_) => SearchReplaceOutput::InvalidInput(render_error(ErrorKind::FileNotFound, &ctx)),
        FilenameTooLong(msg) => SearchReplaceOutput::InvalidInput(msg),
        MultipleMatchesFound(_) => {
            SearchReplaceOutput::InvalidInput(render_error(ErrorKind::MultipleMatchesFound, &ctx))
        }
        NoMatchesFound(_) => SearchReplaceOutput::InvalidInput(render_error(ErrorKind::NoMatchesFound, &ctx)),
        other => other,
    })
}
