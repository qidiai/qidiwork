...
use cf_tool_types::{SubagentCompletedOutput, SubagentIsolationMode, TaskToolInput};
...
impl crate::types::tool_metadata::ToolMetadata for TaskTool {
    ...
    fn description_template(&self) -> &str {
        // The Task tool description for QIDI Code is *never* taken from here.
        // It is always supplied via `ToolConfig::with_description(...)` using
        // the dynamically built string from `build_task_description()` in
        // qidi-code/src/builder.rs (HEADER + per-subagent blocks + FOOTER).
        //
        ...
        "<see build_task_description() in qidi-code>"
    }
...
impl cf_tool_runtime::Tool for TaskTool {
    type Args = TaskToolInput;
    type Output = ToolOutput;

    fn id(&self) -> cf_tool_protocol::ToolId {
        cf_tool_protocol::ToolId::new("task").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let model = cf_tool_types::sanitize_optional_arg(input.model);
            ...
            && resume_from.is_none()
        {
            ...
            if !p.is_dir() {
                ...
                return Err(cf_tool_runtime::ToolError::invalid_arguments(detail));
            }
        }
        ...
        if let Some(ref requested) = model {
            ...
            if let Some(error) = validator.error_for(requested) {
                return Err(cf_tool_runtime::ToolError::invalid_arguments(error));
            }
        }
        ...
        if input.run_in_background {
            ...
            return Ok(ToolOutput::Text(
                cf_tool_types::format_subagent_started_background(
                    &id,
                    &input.subagent_type,
                    &input.description,
                    &task_output_name,
...
mod tests {
    ...
    async fn invalid_model_returns_error_before_background_spawn() {
        ...
        let result =
            cf_tool_runtime::Tool::run(&TaskTool, test_ctx(resources.into_shared()), input).await;
    ...
    async fn model_threads_to_runtime_overrides() {
        ...
        let result = cf_tool_runtime::Tool::run(&TaskTool, test_ctx(shared), input)
            .await
            .unwrap();
        handle.await.unwrap();
        match result {
    ...
    async fn model_whitespace_is_trimmed() {
        ...
        let mut input = task_input("general-purpose", false);
        input.model = Some("  test-model  ".into());
        let _ = cf_tool_runtime::Tool::run(&TaskTool, test_ctx(shared), input)
            .await
            .unwrap();
        handle.await.unwrap();
    }
