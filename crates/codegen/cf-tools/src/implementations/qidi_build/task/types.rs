...
//! (in `qidi-code`) and the subagent coordinator (in `qidi-code`).
//!
//! ## Resource types
//!
...
use cf_tool_types::{SubagentCapabilityMode, SubagentIsolationMode, WaitMode};
...
pub struct SubagentRuntimeOverrides {
    ...
    /// `"cf-tools-plan"`) whose `AgentDefinition` decides the child's harness
...
/// Re-export of [`cf_tool_types::is_not_sentinel`] for existing call sites.
pub use cf_tool_types::is_not_sentinel;
...
/// defense-in-depth check in `qidi-code`'s subagent coordinator.
...
/// the cross-crate drain loop in `qidi-code` relies on
...
register_resource!("qidi_build", "SubagentEventSender", SubagentEventSender);
...
pub struct MonitorEventNotification {
    ...
    /// events inside another session's turn. `None` for legacy / non-cf-tools
...
pub type MonitorEventBuffer = cf_interjection_core::EventQueue<MonitorEventNotification>;

register_resource!("qidi_build", "MonitorEventBuffer", MonitorEventBuffer);
...
/// qidi-code produces it, the channel protocol carries it, and the
...
register_resource!("qidi_build", "SubagentDepthCounter", SubagentDepthCounter);
...
register_resource!("qidi_build", "TaskModelValidator", TaskModelValidator);
...
register_resource!("qidi_build", "SessionIdResource", SessionIdResource);

/// Carries the current parent prompt/turn ID for TaskTool subagent scoping.
///
/// Set by qidi-code immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by qidi-code at turn start.
...
register_resource!("qidi_build", "GoalLoopActive", GoalLoopActive);
