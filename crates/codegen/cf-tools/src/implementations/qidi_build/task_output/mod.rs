...
use crate::implementations::qidi_build::task::TaskTool;
use crate::implementations::qidi_build::task::backend::SubagentBackendResource;
use crate::implementations::qidi_build::task::types::{SubagentSnapshot, SubagentSnapshotStatus};
use crate::implementations::qidi_build_concise::BashConciseTool;
...
/// `TaskOutput` cap. Env override: `QIDI_MAX_WAIT_BLOCK_MS`.
const MAX_WAIT_BLOCK: Duration = Duration::from_secs(600);

fn max_wait_block() -> Duration {
    std::env::var("QIDI_MAX_WAIT_BLOCK_MS")
        .ok()
...
pub(crate) fn background_bash_requires_exprs() -> Vec<Expr<ToolRequirement>> {
    ...
    let qidi_build_bash = Expr::Value(ToolRequirement::Tool {
        namespace: ToolMetadata::tool_namespace(&BashTool).to_string(),
        id: cf_tool_runtime::Tool::id(&BashTool).as_str().to_string(),
    ...
    let qidi_build_concise_bash = Expr::Value(ToolRequirement::Tool {
        namespace: ToolMetadata::tool_namespace(&BashConciseTool).to_string(),
        id: cf_tool_runtime::Tool::id(&BashConciseTool)
            .as_str()
            .to_string(),
        if_params: Some(Expr::Value(ToolParamsRequirement {
            key: "enabled_background".to_string(),
    ...
    let opencode_bash = Expr::Value(ToolRequirement::Tool {
        ...
        id: cf_tool_runtime::Tool::id(&OpenCodeBashTool)
            .as_str()
            .to_string(),
        if_params: None,
    });
    vec![qidi_build_bash, qidi_build_concise_bash, opencode_bash]
}
...
pub(crate) fn task_output_requires_expr() -> Expr<ToolRequirement> {
    ...
    let task_tool = Expr::Value(ToolRequirement::Tool {
        ...
        id: cf_tool_runtime::Tool::id(&TaskTool).as_str().to_string(),
        if_params: None,
    });
...
impl TaskOutputTool {
    async fn run_single_task(
    ...
    ) -> Result<TaskOutputOutput, cf_tool_runtime::ToolError> {
        let contract_version = ctx
            .extensions
            .get::<cf_tool_runtime::BehaviorVersion>()
            .map(|v| v.0.clone());
        ...
        let waits = cf_tool_types::task_output_waits(timeout_ms);
        ...
        if let Some(snapshot) = snapshot {
            let read_file_name;
            {
                ...
                read_file_name = renderer
                    ...
                    .map_err(|e| cf_tool_runtime::ToolError::invalid_arguments(e.to_string()))?;
            }
...
pub(crate) use cf_tool_types::MAX_MULTI_WAIT_IDS;
...
fn format_subagent_snapshot(snap: &SubagentSnapshot) -> TaskOutputOutput {
    ...
    match &snap.status {
        ...
        SubagentSnapshotStatus::Completed {
            ...
            output.push_str("\n\n");
            output.push_str(&cf_tool_types::format_resume_footer(
                &snap.subagent_id,
                &snap.subagent_type,
...
impl crate::types::tool_metadata::ToolMetadata for TaskOutputTool {
    ...
    fn description_template(&self) -> &str {
        ...
        // fallback mirrors the default cf-tools toolset.
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            cf_tool_types::build_task_output_description(&cf_tool_types::TaskOutputToolNaming {
...
/// [`cf_tool_types::build_task_output_description`] builder so the CLI and
...
impl cf_tool_runtime::Tool for TaskOutputTool {
    type Args = TaskOutputToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> cf_tool_protocol::ToolId {
        cf_tool_protocol::ToolId::new("get_task_output").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        // The static fallback is the shared builder's default cf-tools
    ...
    fn resources_with_backend_query() -> (
        ...
        tokio::sync::mpsc::UnboundedReceiver<
            crate::implementations::qidi_build::task::types::SubagentEvent,
        >,
    ) {
        use crate::implementations::qidi_build::task::backend::{
            ChannelBackend, SubagentBackendResource,
        };
        use crate::implementations::qidi_build::task::types::SubagentEvent;
    ...
    fn unwrap_query(
        event: crate::implementations::qidi_build::task::types::SubagentEvent,
    ) -> crate::implementations::qidi_build::task::types::SubagentQueryRequest {
        match event {
            crate::implementations::qidi_build::task::types::SubagentEvent::Query(q) => q,
