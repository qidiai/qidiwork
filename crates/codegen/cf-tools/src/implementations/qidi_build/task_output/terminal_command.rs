...
use cf_tool_types::{TaskOutputOutput, TaskOutputToolInput};
...
impl cf_tool_runtime::Tool for GetTerminalCommandOutputTool {
    type Args = TaskOutputToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> cf_tool_protocol::ToolId {
        cf_tool_protocol::ToolId::new("get_terminal_command_output").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        cf_tool_runtime::Tool::run(&TaskOutputTool, ctx, input).await
    }
}
...
mod tests {
    use super::*;
    use crate::implementations::qidi_build::task_output::test_helpers::{
