...
use crate::implementations::qidi_build::task_output::{
...
use cf_tool_types::{MultiTaskOutputResult, TaskOutputOutput, WaitMode, WaitTasksToolInput};
...
impl crate::types::tool_metadata::ToolMetadata for WaitTasksTool {
    ...
    fn description_template(&self) -> &str {
        ...
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            cf_tool_types::build_wait_tasks_description(&cf_tool_types::WaitTasksToolNaming {
...
/// [`cf_tool_types::build_wait_tasks_description`] builder so the CLI and
...
impl cf_tool_runtime::Tool for WaitTasksTool {
    type Args = WaitTasksToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> cf_tool_protocol::ToolId {
        cf_tool_protocol::ToolId::new("wait_tasks").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let timeout =
            crate::implementations::qidi_build::task_output::capped_wait_timeout(input.timeout_ms);
