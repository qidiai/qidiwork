...
crate::register_resource!("qidi_build", "Todo", TodoState);
...
impl cf_tool_runtime::Tool for TodoWriteTool {
    type Args = TodoWriteInput;
    type Output = TodoWriteOutput;

    fn id(&self) -> cf_tool_protocol::ToolId {
        cf_tool_protocol::ToolId::new("todo_write").expect("valid tool id")
    }
    ...
    ) -> Result<TodoWriteOutput, cf_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn name_and_description() {
        ...
        assert_eq!(cf_tool_runtime::Tool::id(&tool).as_str(), "todo_write");
    ...
    async fn replace_mode_creates_items() {
        ...
        let output = expect_success(
            cf_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input)
                .await
                .unwrap(),
        );
    ...
    async fn replace_clears_previous_state() {
        ...
        cf_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input1)
            .await
            .unwrap();
        ...
        let output = expect_success(
            cf_tool_runtime::Tool::run(&tool, test_ctx(shared.clone()), input2)
                .await
                .unwrap(),
        );
    ...
    async fn state_serialization_roundtrip() {
        ...
        assert!(
            state_map.get("qidi_build.Todo").is_some(),
            "TodoState should serialize under 'qidi_build.Todo'"
        );
