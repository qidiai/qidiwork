...
pub use crate::types::slash_commands::UPDATE_GOAL_TOOL_NAME;
...
impl cf_tool_runtime::ToolOutput for UpdateGoalOutput {}
...
impl cf_tool_runtime::Tool for UpdateGoalTool {
    type Args = UpdateGoalInput;
    type Output = UpdateGoalOutput;

    fn id(&self) -> cf_tool_protocol::ToolId {
        cf_tool_protocol::ToolId::new(UPDATE_GOAL_TOOL_NAME).expect("valid tool id")
    }
    ...
    ) -> Result<UpdateGoalOutput, cf_tool_runtime::ToolError> {
...
pub fn render_ack_into_output(
    ...
    match ack {
        ...
        UpdateGoalAck::Rejected { reason, detail } => Err(cf_tool_runtime::ToolError::custom(
            reason.error_code(),
            detail,
        )),
    }
