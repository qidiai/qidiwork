...
pub use crate::types::slash_commands::{
...
impl S3AccessCredentials {
    ...
    fn to_static(&self) -> cf_file_utils::s3::S3StaticCredentials {
        cf_file_utils::s3::S3StaticCredentials {
...
impl VideoGenClient {
    ...
    pub async fn generate_with_images(
    ...
    ) -> Result<VideoOutcome, cf_tool_runtime::ToolError> {
        ...
        if !status.is_success() {
            ...
            return Err(cf_tool_runtime::ToolError::new(
                cf_tool_runtime::ToolErrorKind::Custom,
                ...
                "done" => {
                    ...
                    return match presigned {
                        ...
                        None if video_url.is_empty() => {
                            Err(cf_tool_runtime::ToolError::invalid_arguments(
    ...
    async fn download_video(&self, url: &str) -> Result<Vec<u8>, cf_tool_runtime::ToolError> {
        let response = self.download_http.get(url).send().await.map_err(|e| {
            cf_tool_runtime::ToolError::invalid_arguments(format!("Failed to download video: {e}"))
        })?;
    ...
    async fn finish_zdr_video(
        ...
        let config = self.zdr_video_output_s3.as_ref().ok_or_else(|| {
            cf_tool_runtime::ToolError::invalid_arguments(
    ...
    async fn presign_zdr_output_urls(
    ...
    ) -> Result<ZdrPresignedUrls, cf_tool_runtime::ToolError> {
        ...
        let upload_url = cf_file_utils::s3::presign_put_url(
            &config.region,
            endpoint,
    ...
    async fn presign_and_download(
        ...
        urls: &ZdrPresignedUrls,
        request_id: &str,
    ) -> Result<Vec<u8>, cf_tool_runtime::ToolError> {
        let get_url = self
    ...
    async fn presign_zdr_get_url(
        ...
        let url = cf_file_utils::s3::presign_get_url(
            &config.region,
            endpoint,
            &creds.to_static(),
            &config.bucket,
...
async fn resolve_image_reference(value: &str) -> Result<String, cf_tool_runtime::ToolError> {
    ...
    if value.starts_with("data:image/") {
        let comma = value.find(',').ok_or_else(|| {
            cf_tool_runtime::ToolError::invalid_arguments("malformed data URL in image reference")
        })?;
    ...
    let (_w, _h, mime) =
        crate::util::image_validate::validate_image_bytes(&raw_bytes).map_err(|e| {
            cf_tool_runtime::ToolError::invalid_arguments(format!("invalid image reference: {e}"))
        })?;
...
fn validate_one_of(
    ...
    Err(cf_tool_runtime::ToolError::invalid_arguments(format!(
        "`{field}` must be one of: {}. Got {value}.",
        allowed.join(", ")
    )))
}

fn validate_imagine_duration(duration: Option<u32>) -> Result<(), cf_tool_runtime::ToolError> {
...
) -> Result<(VideoGenClient, std::path::PathBuf), cf_tool_runtime::ToolError> {
...
async fn save_video_bytes(
    ...
    video_bytes: &[u8],
) -> Result<std::path::PathBuf, cf_tool_runtime::ToolError> {
...
) -> Result<MediaGenOutput, cf_tool_runtime::ToolError> {
    match outcome {
...
impl cf_tool_runtime::Tool for ImageToVideoTool {
    type Args = ImageToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> cf_tool_protocol::ToolId {
        cf_tool_protocol::ToolId::new(IMAGE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
...
impl cf_tool_runtime::Tool for ReferenceToVideoTool {
    type Args = ReferenceToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> cf_tool_protocol::ToolId {
        cf_tool_protocol::ToolId::new(REFERENCE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
