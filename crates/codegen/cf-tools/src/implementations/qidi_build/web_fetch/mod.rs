...
impl cf_tool_runtime::Tool for WebFetchTool {
    type Args = WebFetchInput;
    type Output = WebFetchOutput;

    fn id(&self) -> cf_tool_protocol::ToolId {
        cf_tool_protocol::ToolId::new("web_fetch").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: WebFetchInput,
    ) -> Result<WebFetchOutput, cf_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(cf_tool_runtime::Tool::id(&tool).as_str(), "web_fetch");
        assert_eq!(
