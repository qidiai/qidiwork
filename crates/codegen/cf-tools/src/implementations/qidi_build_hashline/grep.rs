...
use crate::implementations::qidi_build::grep::{GrepSearchInput, GrepTool, OutputMode};
...
impl cf_tool_runtime::Tool for HashlineGrepTool {
    type Args = GrepSearchInput;
    type Output = GrepSearchOutput;

    fn id(&self) -> cf_tool_protocol::ToolId {
        cf_tool_protocol::ToolId::new("hashline_grep").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let mut rt_ctx = cf_tool_runtime::ToolCallContext::new(call_id);
        rt_ctx.extensions.insert(resources.clone());
        rt_ctx.extensions.insert(cf_tool_runtime::Cwd(cwd));
        let mut result = cf_tool_runtime::Tool::run(&grep, rt_ctx, input)
            .await
            .map_err(|e| {
                cf_tool_runtime::ToolError::execution(
                    cf_tool_protocol::ToolId::new("grep").expect("valid"),
                    e.to_string(),
                )
            })?;
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        assert_eq!(cf_tool_runtime::Tool::id(&tool).as_str(), "hashline_grep");
        assert_eq!(ToolMetadata::kind(&tool), ToolKind::Search);
        assert!(cf_tool_runtime::Tool::capabilities(&tool).is_read_only);
        assert!(matches!(
    ...
    async fn inject_anchors_content_mode() {
        ...
        let fs = Arc::new(LocalFs::unconfined());
    ...
    async fn inject_anchors_timeout_returns_unanchored() {
        ...
        let fast_fs = Arc::new(LocalFs::unconfined());
