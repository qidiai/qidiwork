...
pub fn bytes_to_metadata(file_bytes: &[u8]) -> Result<FileMetadata, cf_tool_runtime::ToolError> {
    let size = file_bytes.len();
    let data = infer::get(file_bytes).ok_or_else(|| {
        cf_tool_runtime::ToolError::invalid_arguments("failed to infer file type from magic bytes")
    })?;
