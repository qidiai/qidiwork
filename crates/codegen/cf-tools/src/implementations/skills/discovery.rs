...
///
/// `skills` is the standard layout (`.qidi/skills/`, `.claude/skills/`,
/// `.cursor/skills/`). The product-specific `skills-cursor/` layout is no
/// longer scanned — it pulled vendor default skills into QIDI Code sessions.
...
/// name (e.g. `~/.qidi/skills/shell`) is NOT dropped — only skills physically
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into QIDI Code.
...
/// `cwd` (exclusive). At each directory, checks for `.qidi/skills/`,
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.qidi is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.qidi/skills/shell/SKILL.md",
            "shell"
        ));
    }
