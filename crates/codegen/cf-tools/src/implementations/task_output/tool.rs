...
mod tests {
    ...
    fn test_snapshot_to_result_wraps_long_lines() {
        ...
        let output = ToolOutput::TaskOutput(cf_tool_types::TaskOutputOutput::Result(result));
    ...
    fn test_snapshot_to_result_prefers_display_command() {
        ...
        snapshot.display_command = Some("cargo test -p qidi-code".to_string());

        let result = snapshot_to_result(snapshot, "read_file", DEFAULT_TOOL_OUTPUT_BYTES);
        assert_eq!(
            result.command, "cargo test -p qidi-code",
