//! Conversion from the gRPC wire config types (`qidi-code-api`) to the
...
pub fn tool_config_from_entry(
    index: usize,
    entry: crate::types::ToolConfigEntry,
) -> Result<ToolConfig, ToolConfigEntryError> {
    let crate::types::ToolConfigEntry {
        id,
        params_json,
        name_override,
    ...
    let params = crate::types::config_validation::parse_params_json(
        index,
        &id,
        params_json.as_deref(),
    )?;
    crate::types::config_validation::validate_name_override(index, &id, name_override.as_deref(), params_json.as_deref())?;
    Ok(ToolConfig {
        id,
        params,
        name_override,
...
pub fn tool_server_config_from_entries(
    entries: Vec<crate::types::ToolConfigEntry>,
