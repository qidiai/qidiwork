...
/// this crate: instead of `qidi-code` referencing an out-of-tree tool
/// pack, the pack calls
...
impl ToolConfig {
    ...
    /// `cf_tool_runtime::Tool::id()`. Use this for built-in tools known
    ...
    where
        T: crate::types::tool_metadata::ToolMetadata + cf_tool_runtime::Tool + Default,
    {
        Self::from(&T::default())
    }
...
impl<T: crate::types::tool_metadata::ToolMetadata + cf_tool_runtime::Tool> From<&T>
    for ToolConfig
{
    fn from(tool: &T) -> Self {
        Self {
            id: format!(
                "{}:{}",
                tool.tool_namespace(),
                cf_tool_runtime::Tool::id(tool).as_str()
            ),
            params: None,
            name_override: None,
            params_name_overrides: None,
...
pub struct SessionContext {
    ...
    /// Auth provider which returns a cf_computer_hub_sdk::AuthCredential. Can be used by
    ...
    pub auth_provider: Option<cf_computer_hub_sdk::SharedAuthProvider>,
...
pub async fn drain_value_stream(
    mut stream: cf_tool_runtime::ToolStream<cf_tool_runtime::TypedToolOutput>,
) -> Result<serde_json::Value, cf_tool_runtime::ToolError> {
    use futures::StreamExt;
    while let Some(item) = stream.next().await {
        match item {
            cf_tool_runtime::ToolStreamItem::Progress(_) => continue,
            cf_tool_runtime::ToolStreamItem::Terminal(result) => {
...
fn stream_no_terminal_error() -> cf_tool_runtime::ToolError {
    cf_tool_runtime::ToolError::custom(
        "stream_no_terminal",
...
struct DispatchParts {
    ...
    lr_handle: Arc<dyn cf_computer_hub_core::ToolHandle>,
...
struct ToolEntry {
    ...
    parse_input: Box<
        dyn Fn(serde_json::Value) -> Result<ToolInput, cf_tool_runtime::ToolError> + Send + Sync,
    >,
    /// Registers this tool into a `LocalRegistry` using the concrete type.
    /// Captured at `register::<T>()` time when T is known.
    register_in_local: Box<dyn Fn(&cf_computer_hub_sdk::LocalRegistry) + Send + Sync>,
}
...
pub struct FinalizedToolset {
    ...
    local_registry: cf_computer_hub_sdk::LocalRegistry,
    ...
    workspace_viewer_ctx: Option<cf_tool_runtime::WorkspaceViewerContext>,
}
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
...
pub struct ToolRegistryBuilder {
    ...
    shared_local_registry: Option<cf_computer_hub_sdk::LocalRegistry>,
}
...
impl ToolRegistryBuilder {
            ...
            + 'static,
    {
        ...
        let id = cf_tool_runtime::Tool::id(&tool).as_str().to_string();
        let kind = tool.kind();
        ...
        self.tools.insert(
            ...
            ToolEntry {
                ...
                register_in_local: Box::new(|lr: &cf_computer_hub_sdk::LocalRegistry| {
    ...
    pub fn with_local_registry(mut self, registry: cf_computer_hub_sdk::LocalRegistry) -> Self {
    ...
    pub fn validate_config(&self, config: &ToolServerConfig) -> Vec<RequirementError> {
        ...
        let proposed: Vec<ProposedTool<'_>> = resolved
            .iter()
    ...
    pub fn finalize_with_trunc_config(
        ...
        let proposed: Vec<ProposedTool<'_>> = tools
            .iter()
            .map(|t| ProposedTool {
...
///
/// Implements the canonical `cf_tool_runtime::ToolDispatch` trait so the
...
#[async_trait::async_trait]
impl cf_tool_runtime::ToolDispatch for InnerDispatchForToolset {
    async fn call(
        &self,
        ...
        let result = self
            ...
            .and_then(|output| {
                let value = serde_json::to_value(&output).map_err(|e| {
                    cf_tool_runtime::ToolError::custom("output_encoding", e.to_string())
                })?;
                Ok(cf_tool_runtime::TypedToolOutput::from_value(
                    tool_id.clone(),
                    value,
                ))
            });
        cf_tool_runtime::terminal_only(result)
    }
}
impl FinalizedToolset {
    ...
    pub fn empty_for_test() -> Self {
        Self {
            ...
            local_registry: cf_computer_hub_sdk::LocalRegistry::new(),
    ...
    pub fn local_registry(&self) -> &cf_computer_hub_sdk::LocalRegistry {
        &self.local_registry
    }
    ...
    fn tool_not_found_error(tool_name: &str) -> cf_tool_runtime::ToolError {
        let tid = cf_tool_protocol::ToolId::new(tool_name)
            .unwrap_or_else(|_| cf_tool_protocol::ToolId::new("unknown").expect("valid"));
        cf_tool_runtime::ToolError::not_found(tid, format!("Tool not found: {tool_name}"))
    }
    pub async fn try_parse(
        &self,
        tool_name: &str,
        tool_params: &serde_json::Value,
    ) -> Result<ToolInput, cf_tool_runtime::ToolError> {
    ...
    async fn call_raw(
        ...
        parent_ctx: cf_tool_runtime::ToolCallContext,
    ) -> Result<crate::types::output::ToolOutput, cf_tool_runtime::ToolError> {
        ...
        let mut ctx = cf_tool_runtime::ToolCallContext::new(parent_ctx.call_id.clone());
        ctx.extensions.insert(self.resources.clone());
        ctx.extensions.insert_arc(Arc::clone(&self.renderer));
        if let Some(cwd) = parent_ctx.extensions.get::<cf_tool_runtime::Cwd>() {
            ctx.extensions.insert((*cwd).clone());
        }
        let tool_id = cf_tool_protocol::ToolId::new(&registry_id)
            .unwrap_or_else(|_| cf_tool_protocol::ToolId::new("unknown").expect("valid"));
        let lr_handle = self.local_registry.find(&tool_id).ok_or_else(|| {
            cf_tool_runtime::ToolError::not_found(
                tool_id,
        ...
        (output_converter)(value)
            .map_err(|e| cf_tool_runtime::ToolError::custom("output_decoding", e.to_string()))
    }
    ...
    ) -> Result<ToolRunResult, cf_tool_runtime::ToolError> {
        use futures::StreamExt;
        let mut stream = self.call_streaming(tool_name, tool_args, tool_call_id, cwd_override);
        while let Some(item) = stream.next().await {
            match item {
                cf_tool_runtime::ToolStreamItem::Progress(_) => continue,
                cf_tool_runtime::ToolStreamItem::Terminal(result) => return result,
            }
        }
        Err(stream_no_terminal_error())
    }
    ...
    ///
    /// [`ToolStream`]: cf_tool_runtime::ToolStream
    /// [`ToolStreamItem::Progress`]: cf_tool_runtime::ToolStreamItem::Progress
    pub fn call_streaming(
        ...
        tool_call_id: &str,
        cwd_override: Option<std::path::PathBuf>,
    ) -> cf_tool_runtime::ToolStream<ToolRunResult> {
        use futures::StreamExt;
        ...
        Box::pin(async_stream::stream! {
            ...
            cf_tool_runtime::ToolStreamItem::Terminal(Err(e)); return; } }; let
            ...
            match item { cf_tool_runtime::ToolStreamItem::Progress(p) => { yield
            cf_tool_runtime::ToolStreamItem::Progress(p); }
            cf_tool_runtime::ToolStreamItem::Terminal(Err(e)) => { yield
            cf_tool_runtime::ToolStreamItem::Terminal(Err(e)); return; }
            cf_tool_runtime::ToolStreamItem::Terminal(Ok(typed)) => { let run_result
            = this.finalize_output(typed.value, & output_converter,
            effective_tool_name). await; yield
            cf_tool_runtime::ToolStreamItem::Terminal(run_result); return; } } }
            yield
            cf_tool_runtime::ToolStreamItem::Terminal(Err(stream_no_terminal_error()));
        })
    }
    ...
    fn prepare_dispatch(
        ...
        tool_call_id: &str,
        cwd_override: Option<std::path::PathBuf>,
    ) -> Result<DispatchParts, cf_tool_runtime::ToolError> {
        let (registry_id, output_converter, reverse_params) = {
            let tools = self.tools.read();
            let entry = tools
                .iter()
        ...
        let rt_call_id = cf_tool_protocol::ToolCallId::new(tool_call_id)
            .unwrap_or_else(|_| cf_tool_protocol::ToolCallId::new_v7());
        let mut ctx = cf_tool_runtime::ToolCallContext::new(rt_call_id);
        ctx.extensions.insert(self.resources.clone());
        ctx.extensions.insert_arc(Arc::clone(&self.renderer));
        if let Some(cwd) = cwd_override {
            ctx.extensions.insert(cf_tool_runtime::Cwd(cwd));
        }
        if let Some(ref version) = contract_version {
            ctx.extensions
                .insert(cf_tool_runtime::BehaviorVersion(version.clone()));
        }
    ...
    ///
    /// The tool must implement `cf_tool_runtime::Tool + ToolMetadata`.
    ...
    ) -> Result<(), cf_tool_runtime::ToolError>
    where
        T: cf_tool_runtime::Tool + ToolMetadata + std::fmt::Debug + Send + Sync + 'static,
        T::Output: serde::Serialize,
    {
        ...
        let kind = tool.kind();
        let registry_id = cf_tool_runtime::Tool::id(&tool).as_str().to_owned();
    ...
    pub fn unregister_tools_by_prefix(&self, prefix: &str) -> usize {
        ...
        let to_remove: Vec<_> = tools
            ...
            .filter_map(|t| cf_tool_protocol::ToolId::new(&t.registry_id).ok())
            .collect();
    ...
    pub fn unregister_tool_by_name(&self, name: &str) -> bool {
        ...
        let tool_id = tools
            ...
            .and_then(|t| cf_tool_protocol::ToolId::new(&t.registry_id).ok());
        let before = tools.len();
...
mod tests {
    ...
    impl cf_tool_runtime::Tool for FakeMcpTool {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> cf_tool_protocol::ToolId {
            cf_tool_protocol::ToolId::new("fake_mcp").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::cf_tool_runtime::ListToolsContext,
        ) -> cf_tool_types::ToolDescription {
            cf_tool_types::ToolDescription::new("fake_mcp", &self.description)
        }
        async fn run(
            &self,
            _ctx: cf_tool_runtime::ToolCallContext,
            _input: serde_json::Value,
        ) -> Result<String, cf_tool_runtime::ToolError> {
            Ok("ok".into())
        }
    }
    #[tokio::test]
    ...
    impl cf_tool_runtime::Tool for NonStreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> cf_tool_protocol::ToolId {
            cf_tool_protocol::ToolId::new("non_streaming_stub").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::cf_tool_runtime::ListToolsContext,
        ) -> cf_tool_types::ToolDescription {
            cf_tool_types::ToolDescription::new("non_streaming_stub", "non-streaming stub")
        }
        async fn run(
            &self,
            _ctx: cf_tool_runtime::ToolCallContext,
    ...
    impl cf_tool_runtime::Tool for StreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> cf_tool_protocol::ToolId {
            cf_tool_protocol::ToolId::new("streaming_stub").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::cf_tool_runtime::ListToolsContext,
        ) -> cf_tool_types::ToolDescription {
            cf_tool_types::ToolDescription::new("streaming_stub", "streaming stub")
        }
        async fn run(
            &self,
            _ctx: cf_tool_runtime::ToolCallContext,
        ...
        async fn execute(
            ...
            _ctx: cf_tool_runtime::ToolCallContext,
            _input: serde_json::Value,
        ) -> cf_tool_runtime::ToolStream<String> {
            Box::pin(futures::stream::iter(vec![
                cf_tool_runtime::ToolStreamItem::Progress(cf_tool_runtime::ToolProgress::Text {
                    text: "progress-1".into(),
                }),
                cf_tool_runtime::ToolStreamItem::Terminal(Ok("terminal-value".to_string())),
            ]))
        }
    }
        ...
        while let Some(item) = stream.next().await {
            match item {
                cf_tool_runtime::ToolStreamItem::Progress(p) => {
                    ...
                    assert!(matches!(p, cf_tool_runtime::ToolProgress::Text { .. }));
                    progress_count += 1;
                }
                cf_tool_runtime::ToolStreamItem::Terminal(result) => {
    ...
    impl cf_tool_runtime::Tool for NoTerminalStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> cf_tool_protocol::ToolId {
            cf_tool_protocol::ToolId::new("no_terminal_stub").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::cf_tool_runtime::ListToolsContext,
        ) -> cf_tool_types::ToolDescription {
            cf_tool_types::ToolDescription::new("no_terminal_stub", "no-terminal stub")
        }
        async fn run(
            &self,
            _ctx: cf_tool_runtime::ToolCallContext,
    ...
    /// cf-tools agent config (all tools present) and that the new examples
    ...
    async fn task_description_renders_with_default_agent_config() {
        ...
        let toolset = builder
            .finalize(config, ctx)
            .expect("finalize should succeed with default cf-tools tools");
        let defs = toolset.tool_definitions();
        let task_def = defs
            .iter()
    ...
    async fn prepare_dispatch_stamps_workspace_viewer_ctx_when_present() {
        let (toolset, _tmp) =
            toolset_with_viewer_ctx(Some(cf_tool_runtime::WorkspaceViewerContext {
        ...
        let wvc = parts
            .ctx
            .extensions
            .get::<cf_tool_runtime::WorkspaceViewerContext>()
    ...
    async fn bash_streaming_progress_emitted_via_finalized_toolset_when_gate_on() {
        ...
        while let Some(item) = stream.next().await {
            match item {
                cf_tool_runtime::ToolStreamItem::Progress(_) => progress_count += 1,
                cf_tool_runtime::ToolStreamItem::Terminal(r) => {
