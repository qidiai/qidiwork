...
//! after every tool call to check for SKILL.md files in `.qidi/skills/`,
...
/// Directories that contain skill definitions (`.qidi/skills/`, `.agents/skills/`,
...
impl SkillDiscoveryReminder {
    ...
    /// (`.qidi/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.qidi/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.qidi/) are outside the git root so the walk breaks early.
