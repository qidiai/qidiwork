...
impl AutoWakeDeliveredIds {
    ...
    ///
    /// Used by the between-turn completion drain in `qidi-code` to
...
/// scope only when it has no recorded owner (legacy / non-cf-tools backends)
...
/// - `qidi-code`'s `SessionActor` to sweep matching synthetic
...
mod tests {
    ...
    use cf_tool_types::{MultiTaskOutputResult, TaskOutputResult};
    struct MockTerminal {
