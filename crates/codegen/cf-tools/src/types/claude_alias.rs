...
//! matchers); the agent builder (`qidi-code`) needs the [`ToolKind`] a `tools:`
...
const CLAUDE_TOOLS: &[ClaudeTool] = &[
    ...
    k("Skill",           Read,                 &["skill"]),                           // matcher: opencode's `skill` tool; allowlist Read (cf-tools reads SKILL.md)
...
/// Every distinct Grok name the table references, for the `qidi-code` drift-check
