...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.qidi/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.qidi/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.qidi/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.qidi/ or ~/.claude/.
    User { path: PathBuf },
