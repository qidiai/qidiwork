use crate::implementations::qidi_build::bash::BashError;
use crate::implementations::qidi_build::todo::TodoError;
use crate::implementations::qidi_build::web_fetch::WebFetchError;
...
/// Generate `impl From<$err> for cf_tool_runtime::ToolError` that wraps the
/// error as an execution failure tagged with the given tool ID.
macro_rules! impl_runtime_error_from {
    ($($err:ty => $tool_id:literal),+ $(,)?) => {
        $(
            impl From<$err> for cf_tool_runtime::ToolError {
                fn from(err: $err) -> Self {
                    cf_tool_runtime::ToolError::execution(
                        cf_tool_protocol::ToolId::new($tool_id).expect("valid static tool id"),
                        err.to_string(),
                    )
                }
            }
