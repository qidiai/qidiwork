...
//!
//! `MemoryBackend` is defined in `qidi-code` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `qidi-code`.
/// Mirrors `cf_shell::session::memory_log::TARGET`.
pub const MEMORY_LOG_TARGET: &str = "cf_memory";
