...
pub(crate) fn typed_tool_output_preserving_cco(
    ...
    source: &impl cf_tool_runtime::ToolOutput,
) -> cf_tool_runtime::TypedToolOutput {
    let cco = source.chat_completion_output();
    let value = serde_json::to_value(payload).unwrap_or(serde_json::Value::Null);
    cf_tool_runtime::TypedToolOutput::from_value(tool_id, value).with_chat_completion_output(cco)
}
...
/// protocol boundary in `qidi-code`.
#[derive(Debug, Clone, Serialize, Deserialize, schemars::JsonSchema)]
...
use cf_tool_types::KillTaskOutput;
...
impl cf_tool_runtime::ToolOutput for BashToolOutput {
    fn chat_completion_output(&self) -> Option<cf_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Bash(bash) => cf_tool_runtime::ToolOutput::chat_completion_output(bash),
...
impl cf_tool_runtime::ToolOutput for ToolOutput {
    fn chat_completion_output(&self) -> Option<cf_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Bash(bash) => cf_tool_runtime::ToolOutput::chat_completion_output(bash),
            _ => None,
...
impl cf_tool_runtime::ToolOutput for BashOutput {
    fn chat_completion_output(&self) -> Option<cf_tool_runtime::ToolChatCompletionResponse> {
        ...
        Some(cf_tool_runtime::ToolChatCompletionResponse {
            result: Some(cf_tool_runtime::ToolChatCompletion {
                sender: "assistant".into(),
                message_tag: Some("raw_function_result".into()),
                code_execution_result: Some(cf_tool_runtime::ToolCodeExecutionResult {
                    stdout,
                    stderr: String::new(),
...
impl cf_tool_runtime::ToolOutput for GrepSearchOutput {}
impl cf_tool_runtime::ToolOutput for ReadFileOutput {}
impl cf_tool_runtime::ToolOutput for ListDirOutput {}
impl cf_tool_runtime::ToolOutput for SearchReplaceOutput {}
impl cf_tool_runtime::ToolOutput for TodoWriteOutput {}
impl cf_tool_runtime::ToolOutput for WebSearchOutput {}
impl cf_tool_runtime::ToolOutput for WebFetchOutput {}
impl cf_tool_runtime::ToolOutput for SkillOutput {}
impl cf_tool_runtime::ToolOutput for ApplyPatchOutput {}
impl cf_tool_runtime::ToolOutput for CodexGrepFilesOutput {}
impl cf_tool_runtime::ToolOutput for SearchToolOutput {}
impl cf_tool_runtime::ToolOutput for EnterPlanModeOutput {}
impl cf_tool_runtime::ToolOutput for ExitPlanModeOutput {}
impl cf_tool_runtime::ToolOutput for AskUserQuestionOutput {}
impl cf_tool_runtime::ToolOutput for MCPOutput {}
#[cfg(test)]
mod tests {
    use super::*;
    ...
    fn assert_cer(
        resp: &cf_tool_runtime::ToolChatCompletionResponse,
        stdout: &str,
        exit_code: i32,
        timed_out: bool,
    ) {
    ...
    fn bash_output_chat_completion_carries_exit_and_stdout() {
        let resp = cf_tool_runtime::ToolOutput::chat_completion_output(&sample_bash(
            0, b"hello\n", false,
        ))
        .unwrap();
    ...
    fn bash_output_chat_completion_empty_stdout_still_emits() {
        let resp =
            cf_tool_runtime::ToolOutput::chat_completion_output(&sample_bash(0, b"", false))
                .unwrap();
    ...
    fn bash_output_chat_completion_truncation_marker_and_extra() {
        ...
        let resp = cf_tool_runtime::ToolOutput::chat_completion_output(&bash).unwrap();
        let result = resp.result.as_ref().unwrap();
    ...
    fn bash_tool_output_foreground_delegates_background_skips() {
        let resp = cf_tool_runtime::ToolOutput::chat_completion_output(&BashToolOutput::Bash(
            sample_bash(0, b"ok", false),
        ))
        .unwrap();
        assert_cer(&resp, "ok", 0, false);
        assert!(
            cf_tool_runtime::ToolOutput::chat_completion_output(
    ...
    fn aggregate_tool_output_bash_delegates_background_skips() {
        let resp = cf_tool_runtime::ToolOutput::chat_completion_output(&ToolOutput::Bash(
        ...
        assert!(
            cf_tool_runtime::ToolOutput::chat_completion_output(&ToolOutput::Text(
    ...
    fn bash_tool_id() -> cf_tool_protocol::ToolId {
        cf_tool_protocol::ToolId::new("bash").unwrap()
    }
    #[test]
    fn into_typed_tool_output_preserves_bash_foreground_cco() {
        ...
        let dropped = cf_tool_runtime::TypedToolOutput::from_value(typed.tool_id, expected_value);
    ...
    fn into_typed_tool_output_non_bash_cco_is_none() {
        ...
        let typed = run.into_typed_tool_output(cf_tool_protocol::ToolId::new("text").unwrap());
