...
/// ```
#[macro_export]
/// Macro to register a resource tracker.
...
impl Resources {
    ...
    pub fn require<T: Send + Sync + 'static>(&self) -> Result<&T, cf_tool_runtime::ToolError> {
...
/// tool falls back to `Cwd/.qidi/plan.md`.
#[derive(Debug, Clone)]
...
pub const PLAN_FILE_RELATIVE_PATH: &str = ".qidi/plan.md";
...
/// [`Cwd`]`/.qidi/plan.md`, else the bare relative `.qidi/plan.md`.
...
pub(crate) fn require_plan_file_path(
    res: &Resources,
) -> Result<(PathBuf, String), cf_tool_runtime::ToolError> {
...
/// Newtype wrapper for `Arc<dyn cf_tool_runtime::ToolDispatch>` so it can
...
#[derive(Clone)]
pub struct InnerDispatch(pub std::sync::Arc<dyn cf_tool_runtime::ToolDispatch>);
#[derive(Debug, Clone)]
...
pub trait ManagedGatewayToolCaller: Send + Sync {
    async fn call_tool(
        ...
        caller: &str,
    ) -> Result<ManagedGatewayToolCallResponse, cf_tool_runtime::ToolError>;
}
#[derive(Clone)]
