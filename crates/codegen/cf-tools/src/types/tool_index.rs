...
//!
//! `ToolSearchIndex` is defined in `qidi-code` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `qidi-code`
...
/// in `qidi-code` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `qidi-code` after MCP initialization.
#[derive(Clone)]
