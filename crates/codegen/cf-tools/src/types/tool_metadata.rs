...
//! 1. `cf_tool_runtime::Tool` — typed Args/Output, `run()` with actual logic
...
//! ## Context helpers
//!
//! Tools access session state through `cf_tool_runtime::ToolCallContext`
...
///
/// Each tool struct implements this alongside `cf_tool_runtime::Tool`.
...
) -> Result<SharedResources, cf_tool_runtime::ToolError> {
    ctx.extensions
        .get::<SharedResources>()
        .map(|arc| (*arc).clone())
        .ok_or_else(|| {
...
pub async fn resolve_cwd(
...
) -> Result<PathBuf, cf_tool_runtime::ToolError> {
    if let Some(cwd) = ctx.extensions.get::<cf_tool_runtime::Cwd>() {
    ...
    res.get::<crate::types::resources::Cwd>()
        .map(|c| c.0.clone())
        .ok_or_else(|| {
            cf_tool_runtime::ToolError::custom("missing_cwd", "Cwd not available in Resources")
        })
}
...
pub fn test_ctx(resources: SharedResources) -> cf_tool_runtime::ToolCallContext {
    ...
    ctx.extensions
        .insert(cf_tool_runtime::WorkspaceViewerContext {
...
pub fn test_ctx_with_call_id(
    ...
    let id = cf_tool_protocol::ToolCallId::new(call_id)
        .unwrap_or_else(|_| cf_tool_protocol::ToolCallId::new_v7());
    let mut ctx = cf_tool_runtime::ToolCallContext::new(id);
    ctx.extensions.insert(resources);
    ctx.extensions
        .insert(cf_tool_runtime::WorkspaceViewerContext {
            stream_tool_progress: true,
...
pub fn behavior_version(ctx: &cf_tool_runtime::ToolCallContext) -> Option<String> {
    ctx.extensions
