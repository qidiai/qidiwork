...
//!    [`ENV_QIDI_MAX_MCP_OUTPUT_BYTES`] / [`ENV_MAX_MCP_OUTPUT_BYTES`]
//! 4. Built-in default
...
pub const ENV_QIDI_MAX_MCP_OUTPUT_BYTES: &str = "QIDI_MAX_MCP_OUTPUT_BYTES";
...
/// Env tier: `QIDI_MAX_MCP_OUTPUT_BYTES` then `MAX_MCP_OUTPUT_BYTES`.
///
...
pub fn mcp_max_output_bytes_from_env() -> Option<usize> {
    parse_positive_bytes_env(ENV_QIDI_MAX_MCP_OUTPUT_BYTES)
...
mod tests {
    ...
    fn host_set_overrides_env_fallback() {
        with_mcp_limit_lock(|| {
            ...
            let prev_grok = std::env::var(ENV_QIDI_MAX_MCP_OUTPUT_BYTES).ok();
            unsafe {
                std::env::remove_var(ENV_MAX_MCP_OUTPUT_BYTES);
                std::env::remove_var(ENV_QIDI_MAX_MCP_OUTPUT_BYTES);
            }
            assert_eq!(
            ...
            unsafe {
                ...
                match prev_grok {
                    Some(v) => std::env::set_var(ENV_QIDI_MAX_MCP_OUTPUT_BYTES, v),
                    None => std::env::remove_var(ENV_QIDI_MAX_MCP_OUTPUT_BYTES),
                }
            }
    ...
    fn env_parser_rejects_zero_and_junk() {
        with_mcp_limit_lock(|| {
            ...
            // QIDI_* wins over MAX_* when both set.
            unsafe { std::env::set_var(ENV_QIDI_MAX_MCP_OUTPUT_BYTES, "99999") };
            assert_eq!(mcp_max_output_bytes_from_env(), Some(99_999));

            unsafe { std::env::remove_var(ENV_QIDI_MAX_MCP_OUTPUT_BYTES) };
            assert_eq!(mcp_max_output_bytes_from_env(), Some(12_345));
