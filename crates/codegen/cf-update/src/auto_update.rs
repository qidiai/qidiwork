...
use crate::version::{
    UpdateConfig, fetch_latest_version, get_installed_qidi_version, get_latest_version,
    is_version_cache_fresh, try_fetch_stable_pointer, write_version_cache,
};
use cf_shell::util::config;
use cf_shell::util::grok_home::{grok_application, grok_home};
...
pub fn print_update_status(status: &UpdateStatus, json: bool) -> anyhow::Result<()> {
    ...
    if status.update_available {
        if let Some(latest_version) = status.latest_version.as_deref() {
            println!(
                "A new version of QIDI Code is available: {} -> {}{}",
                status.current_version, latest_version, channel_label
            );
        } else {
            println!("A new version of QIDI Code is available.");
        }
        return Ok(());
    }
    ...
    println!("QIDI Code - v{}{}", status.current_version, channel_label);
    Ok(())
}
...
pub async fn check_update_status(update_config: &UpdateConfig) -> UpdateStatus {
    ...
    let current_version = get_installed_qidi_version();
...
pub async fn auto_update_target(update_config: &UpdateConfig) -> Option<(&'static str, String)> {
    ...
    let current = get_installed_qidi_version();
...
pub async fn ensure_latest_on_disk(update_config: &UpdateConfig) -> Result<EnsureLatestOutcome> {
    ...
    let effective_current =
        disk_version_for_installer(installer).unwrap_or_else(get_installed_qidi_version);
    if needs_update(
        &effective_current,
        &latest,
        &update_config.channel,
    ...
    let running = get_installed_qidi_version();
...
fn env_installer() -> Option<&'static str> {
    if let Ok(v) = std::env::var("QIDI_INSTALLER") {
    ...
    if std::env::var_os("QIDI_MANAGED_BY_NPM").is_some() {
        return Some("npm");
    }
    if std::env::var_os("QIDI_MANAGED_BY_INTERNAL").is_some() {
...
/// Prefer `~/.qidi/bin/grok` which always points to the latest version.
...
/// Delete `~/.qidi/models_cache.json` after a successful update.
///
...
/// Remove the stale `grok-pager` symlink/binary from `~/.qidi/bin/` left by
...
/// config and writes to `~/.qidi/bin/`, so callers must isolate
/// `QIDI_HOME`.
#[doc(hidden)]
...
/// A downloaded and smoke-tested binary in `~/.qidi/downloads/`, not yet
...
/// B2: 下载 `<binary>.sig`（原始 Ed25519 detached signature）并对已下载的二进制
/// 做完整性校验。校验失败时删除下载的二进制并返回 `Err`，调用方保留旧版本、
/// 拒绝替换。仅在 [`crate::signing::verification_active`] 为真时被调用。
async fn verify_downloaded_signature(
    ...
    let base = gcs_base_url.trim_end_matches('/');
    let sig_name = format!("{binary_name}.sig");
    let sig_url = format!("{base}/{sig_name}");
    let sig_path = binary_path.with_file_name(&sig_name);

    download_silent(&sig_url, &sig_path)
        .await
        .with_context(|| format!("downloading signature sidecar {sig_name}"))?;

    let binary_bytes = tokio::fs::read(binary_path)
        .await
        .context("reading downloaded binary for signature verification")?;
    let sig_bytes = tokio::fs::read(&sig_path)
        .await
        .context("reading downloaded signature sidecar")?;
    // sidecar 校验后即可删除，最佳努力清理。
    let _ = tokio::fs::remove_file(&sig_path).await;

    match crate::signing::verify_download(&binary_bytes, &sig_bytes) {
        Ok(()) => {
            tracing::info!("Ed25519 signature verified; proceeding with activation");
            Ok(())
        }
        Err(e) => {
            let _ = tokio::fs::remove_file(binary_path).await;
            anyhow::bail!(
                "signature verification failed for downloaded binary: {e}. \
...
async fn activate_verified_download(download: &VerifiedDownload) -> Result<()> {
    ...
    // Atomic swap of ~/.qidi/bin/{grok,agent} -> downloaded binary.
...
async fn regenerate_completions(binary: &std::path::Path, grok_home: &std::path::Path) {
    // Derive $HOME independently — grok_home may be overridden via QIDI_HOME
...
///
/// When both paths share a grandparent (e.g. `~/.qidi/bin/grok` and
/// `~/.qidi/downloads/grok-0.1.203-linux-x86_64`), returns a relative path
...
///
/// Relative symlinks survive Docker bind-mounts where `~/.qidi/` is mapped
...
/// Swap `~/.qidi/bin/{grok,agent}` to point at `binary_path`. Returns the
...
/// bind-mounts of `~/.qidi/`). Windows: [`windows_replace_exe`].
///
...
async fn install_gh_release(target: Option<&str>) -> Result<()> {
    ...
    // through it (e.g. /usr/local/bin/grok -> ~/.qidi/downloads/grok-latest)
    ...
    // ~/.qidi/downloads/ (legacy layout — skips the grok-latest indirection).
    ...
    for name in ["grok", "agent"] {
        ...
        if let Ok(existing_target) = tokio::fs::read_link(&system_link).await {
            ...
            if target_str.contains(".qidi/downloads/") && !target_str.ends_with("grok-latest") {
...
/// inode is unlinked.
///
/// While our postinstall.js now uses versioned binaries under ~/.qidi/bin/
...
async fn refresh_deployment_config() {
    if !cf_shell::managed_config::has_principal() {
        return;
    }
    if !cf_shell::managed_config::is_fetch_enabled() {
        return;
    }
    // Clear a logged-out team's files before deciding to fetch (mirrors the loop).
    cf_shell::managed_config::clear_orphan();
    if !cf_shell::config::is_managed_config_stale_for(
        &cf_shell::managed_config::current_serving_identity(),
    ) {
        return;
    }
...
mod tests {
    ...
    fn test_tmp_download_path_is_unique_per_version_and_per_attempt() {
        ...
        let dest_181 = std::path::Path::new("/home/u/.qidi/downloads/grok-0.1.181-linux-x86_64");
        let dest_182 = std::path::Path::new("/home/u/.qidi/downloads/grok-0.1.182-linux-x86_64");
        ...
        assert_eq!(
            a.parent(),
            std::path::Path::new("/home/u/.qidi/downloads").into(),
    ...
    fn test_relative_symlink_target_sibling_dirs() {
        ...
        let target = std::path::Path::new("/home/alice/.qidi/downloads/grok-0.1.203");
        let link = std::path::Path::new("/home/alice/.qidi/bin/grok");
    ...
    fn test_relative_symlink_target_same_dir() {
        ...
        let target = std::path::Path::new("/home/alice/.qidi/downloads/grok-0.1.203");
        let link = std::path::Path::new("/home/alice/.qidi/downloads/grok-latest");
        let result = relative_symlink_target(target, link);
    ...
    fn test_relative_symlink_target_cross_tree_stays_absolute() {
        // /usr/local/bin/grok -> /home/alice/.qidi/downloads/grok-0.1.203
        ...
        assert_eq!(
            result,
            std::path::PathBuf::from("/home/alice/.qidi/downloads/grok-0.1.203")
        );
    }
    ...
    async fn test_relative_symlink_survives_directory_move() {
        // Simulates Docker bind-mount: create ~/.qidi/ layout at path A,
    ...
    //   1. QIDI_INSTALLER (npm | internal | gh-release | gh)
    //   2. QIDI_MANAGED_BY_NPM       → npm
    //   3. QIDI_MANAGED_BY_INTERNAL  → internal
    ...
    fn test_env_installer_explicit_npm() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("QIDI_INSTALLER", "npm") };
    ...
    fn test_env_installer_explicit_internal() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("QIDI_INSTALLER", "internal") };
    ...
    fn test_env_installer_explicit_gh_release() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("QIDI_INSTALLER", "gh-release") };
    ...
    fn test_env_installer_explicit_gh_alias() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("QIDI_INSTALLER", "gh") };
        assert_eq!(env_installer(), Some("gh-release"));
    }
    ...
    fn test_env_installer_explicit_unknown_value_returns_none() {
        ...
        // to the other env vars or to config. So `QIDI_INSTALLER=brew`
        // disables the env-installer detection entirely.
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("QIDI_INSTALLER", "brew") };
        // Even if MANAGED_BY_NPM is also set, the explicit var wins (and rejects).
        unsafe { std::env::set_var("QIDI_MANAGED_BY_NPM", "1") };
        assert_eq!(
            env_installer(),
            None,
            "explicit QIDI_INSTALLER=brew must early-return None, not fall through"
        );
    }
    ...
    fn test_env_installer_explicit_empty_returns_none() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("QIDI_INSTALLER", "") };
        assert_eq!(env_installer(), None);
    }
    ...
    fn test_env_installer_managed_by_npm_any_value() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("QIDI_MANAGED_BY_NPM", "") };
        assert_eq!(env_installer(), Some("npm"));
    }
    ...
    fn test_env_installer_managed_by_internal() {
        let _g = InstallerEnvGuard::isolate();
        unsafe { std::env::set_var("QIDI_MANAGED_BY_INTERNAL", "1") };
        assert_eq!(env_installer(), Some("internal"));
    }
    ...
    fn test_env_installer_managed_by_npm_wins_over_npm_config_user_agent() {
        ...
        let _g = InstallerEnvGuard::isolate();
        unsafe {
            std::env::set_var("QIDI_MANAGED_BY_NPM", "1");
    ...
    fn test_env_installer_explicit_internal_wins_over_npm_managed() {
        // QIDI_INSTALLER=internal must override an inherited MANAGED_BY_NPM.
        let _g = InstallerEnvGuard::isolate();
        unsafe {
            std::env::set_var("QIDI_INSTALLER", "internal");
            std::env::set_var("QIDI_MANAGED_BY_NPM", "1");
        }
        assert_eq!(env_installer(), Some("internal"));
    }
