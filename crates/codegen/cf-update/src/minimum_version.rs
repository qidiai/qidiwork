...
//!
//! Set `QIDI_TEST_VERSION` to manually exercise either path without producing
//! a real out-of-date build.

use crate::auto_update::{get_installer, run_install_script};
use crate::version::{
    UpdateConfig, fetch_latest_version, get_installed_qidi_version, write_version_cache,
};
...
///
/// `QIDI_TEST_VERSION` lets devs override the running version to skip
...
mod tests {
    ...
    fn version_env_var_flows_through_to_decision() {
        let saved = std::env::var("QIDI_TEST_VERSION").ok();

        // SAFETY: #[serial] excludes other env-touching tests.
        unsafe { std::env::set_var("QIDI_TEST_VERSION", "0.1.50") };
        let decision =
            evaluate_minimum_version(&get_installed_qidi_version(), Some("0.1.100")).unwrap();
        assert!(matches!(
            decision,
            MinimumVersionDecision::BelowMinimum { .. }
        ));
        ...
        match saved {
            Some(v) => unsafe { std::env::set_var("QIDI_TEST_VERSION", v) },
            None => unsafe { std::env::remove_var("QIDI_TEST_VERSION") },
        }
    }
}
