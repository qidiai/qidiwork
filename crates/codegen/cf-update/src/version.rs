...
const NPM_PACKAGE: &str = "@qidi/qidiwork";
pub const GH_RELEASE_REPO: &str = "qidiai/qidiwork";

/// Primary CLI base URL: qidiai.ltd endpoint with edge caching
/// for binaries and origin-respecting no-cache for channel pointers.
pub(crate) const CLI_BASE_URL_PRIMARY: &str = "https://qidiwork.qidiai.ltd/cli";
...
pub struct UpdateConfig {
    /// Chat API proxy base URL (versioned `https://api.qidiai.ltd/v1` endpoint).
    pub proxy_base_url: String,
    /// Auth scope key for `~/.qidi/auth.json`.
    pub auth_scope: String,
    /// Enterprise deployment key (QIDI_DEPLOYMENT_KEY).
...
pub use cf_version::installed as get_installed_qidi_version;

/// Version of the managed grok binary currently on disk, read from the
/// `~/.qidi/bin/grok` symlink target (`../downloads/grok-<version>-<platform>`)
...
/// link whose target binary was deleted (e.g. manual `~/.qidi/downloads`
...
pub fn installed_on_disk_version() -> Option<String> {
    #[cfg(unix)]
    {
        let app = cf_shell::util::grok_home::grok_application();
...
/// Read the cached stable version from `~/.qidi/version.json` (sync, for display).
///
...
pub fn channel_name() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        derive_channel(cf_version::VERSION, &stable)
    })
}
...
/// `~/.qidi/version.json` (written by the auto-updater):
...
pub fn channel_label() -> &'static str {
    ...
    LABEL.get_or_init(|| {
        ...
        match derive_channel(cf_version::VERSION, &stable) {
            Some("alpha") => " [alpha]",
            Some(_) => " [stable]",
            None => "",
        }
