...
//! its own `OnceLock<QIDI_HOME>`. The helpers below ensure the per-binary
//! initialization is identical: same env-var set, same isolation guarantees,
//! same reset between tests.
//!
//! Mirrors the QIDI_HOME isolation pattern used in other integration tests.
//!
//! ## Usage
//!
//! ```ignore
...
//! #[serial_test::serial]
//! async fn my_test() {
//!     let _ = test_home();   // initializes QIDI_HOME once per binary
...
/// Returns a process-wide test `QIDI_HOME`, initialized exactly once per test
/// binary. Once initialized, `cf_config::grok_home()` will resolve to
...
pub fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe {
            ...
            std::env::remove_var("QIDI_INSTALLER");
            std::env::remove_var("QIDI_MANAGED_BY_NPM");
            std::env::remove_var("QIDI_MANAGED_BY_INTERNAL");
        }
        path
    })
}

/// Wipe state in `QIDI_HOME` between tests so each test sees a clean home.
...
/// Override the version reported by `get_installed_qidi_version()` for the
/// duration of the test (until [`reset_home`] or process exit).
pub fn set_test_version(v: &str) {
    // SAFETY: tests using this helper must be `#[serial]`.
    unsafe { std::env::set_var("QIDI_TEST_VERSION", v) };
}
...
/// Minimal [`cf_update::UpdateConfig`] for install tests.
pub fn make_update_config(channel: &str) -> cf_update::UpdateConfig {
    cf_update::UpdateConfig {
...
/// Backdate every file in `QIDI_HOME/downloads` by ~2 hours.
///
