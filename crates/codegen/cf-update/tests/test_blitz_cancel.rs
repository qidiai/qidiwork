...
//!
//! > `~/.qidi/bin/grok` resolves to a binary that passes the smoke-test, OR it
...
use cf_update::auto_update::install_internal_from_base;
...
async fn blitz_fuzz_stress() {
    ...
    let iterations: usize = std::env::var("QIDI_BLITZ_ITERS")
        .ok()
