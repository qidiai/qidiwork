...
use cf_update::auto_update::{ensure_latest_on_disk, install_internal_from_base, run_update};
use cf_update::version::installed_on_disk_version;

/// Assert the active `~/.qidi/bin/grok` resolves to the expected versioned
...
/// Lay down a managed-install layout in the test QIDI_HOME:
...
// installers that actually maintain the managed `~/.qidi/bin/grok` symlink
...
async fn disk_probe_rejects_dangling_symlink() {
    ...
    // ~/.qidi/downloads cleanup), the probe must report None — otherwise
