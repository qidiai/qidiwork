...
//!
//! Wires together a wiremock-mocked GCS bucket + an isolated `QIDI_HOME`
...
//! tests in this binary share a single `QIDI_HOME` and run serially via
//! `#[serial]`.
...
use cf_update::UpdateConfig;
use cf_update::auto_update::{install_internal_from_base, install_internal_from_bases};
...
async fn install_internal_cleans_up_stale_pager_symlink() {
    ...
    // update removes the stale symlink from ~/.qidi/bin/.
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
