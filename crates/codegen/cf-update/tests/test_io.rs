...
//!
//! These tests touch global process state — `QIDI_HOME` (a `OnceLock` in
//! `xai-grok-config`), `QIDI_TEST_VERSION`, and `NPM_TOKEN` — so they
//! must run serially. Once `QIDI_HOME` is initialized for a process, it can't
//! be changed; we set it from a single shared `OnceLock` and reset the
//! contents of the directory between tests.
//!
//! The patterns here mirror the QIDI_HOME isolation used in other
//! integration tests.
...
use cf_update::write_version_cache;
...
async fn cache_is_fresh() -> bool {
    // Mirror the implementation: look at version.json under QIDI_HOME,
...
// ─────────────────────────────────────────────────────────────────────────────
// get_installed_qidi_version env override
//
// The function honors `QIDI_TEST_VERSION` for testing. We exercise it
// via the public re-export only — no private items leaked.
// ─────────────────────────────────────────────────────────────────────────────
//
// Note: `get_installed_qidi_version` is not re-exported from `lib.rs`, but
...
async fn get_installed_version_uses_env_var_override() {
    ...
    unsafe {
        std::env::set_var("QIDI_TEST_VERSION", "9.9.9");
    }
    let v = cf_update::version::get_installed_qidi_version();
    assert_eq!(v, "9.9.9");
    unsafe {
        std::env::remove_var("QIDI_TEST_VERSION");
    }
}
...
async fn get_installed_version_with_env_var_takes_precedence() {
    ...
    let real = {
        ...
        cf_update::version::get_installed_qidi_version()
    };

    unsafe {
        std::env::set_var("QIDI_TEST_VERSION", "0.0.0-test");
    }
    let overridden = cf_update::version::get_installed_qidi_version();
...
async fn get_installed_version_handles_alpha_prerelease_in_env() {
    ...
    unsafe {
        std::env::set_var("QIDI_TEST_VERSION", "0.1.200-alpha.5");
    }
    let v = cf_update::version::get_installed_qidi_version();
...
async fn get_installed_version_does_not_validate_env_var_format() {
    ...
    unsafe {
        std::env::set_var("QIDI_TEST_VERSION", "not-a-version");
    }
    let v = cf_update::version::get_installed_qidi_version();
