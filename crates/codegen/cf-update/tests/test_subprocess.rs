...
use cf_update::auto_update::install_npm_for_test;
use cf_update::version::{
...
async fn fetch_npm_tag_passes_pkg_and_tag_to_npm() {
    ...
    assert!(args.contains("@qidi/qidiwork"), "args: {args}");
...
async fn fetch_npm_tag_alpha_appends_at_alpha_suffix() {
    ...
    let log = g.args_log();
    assert!(
        log[0].contains("@qidi/qidiwork@alpha"),
        "args: {}",
        log[0]
    );
}
...
async fn install_npm_calls_npm_with_version_arg() {
    ...
    assert!(args.contains("@qidi/qidiwork@0.1.181"), "args: {args}");
}
...
async fn install_npm_falls_back_to_dist_tag_on_no_target() {
    ...
    let log = g.args_log();
    assert!(
        log[0].contains("@qidi/qidiwork@latest"),
...
async fn fetch_gh_release_passes_repo_flag() {
    ...
    assert!(
        log[0].contains("qidiai/qidiwork"),
        "args: {}",
        log[0]
    );
}
