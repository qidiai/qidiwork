...
//! ```bash
//! export XAI_API_KEY=...
//! cargo run -p qidi-code --bin voice-probe -- --seconds 5
//! ```
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,cf_voice=debug".into()),
        )
        .init();
    ...
    if args.mic_only {
        ...
        {
            let (bytes, chunks) =
                cf_voice::run_mic_only_probe(config.sample_rate, args.seconds)?;
...
fn load_config(path: Option<&std::path::Path>) -> VoiceConfig {
    ...
    if let Ok(home) = std::env::var("QIDI_HOME")
...
Environment:
  ...
  RUST_LOG        optional (default info,cf_voice=debug)

Reads [voice] from ~/.qidi/config.toml unless --config PATH is set.
"#
    );
}
