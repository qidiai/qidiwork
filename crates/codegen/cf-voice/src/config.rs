...
/// `~/.qidi/config.toml` (STT URL pieces, language, sample rate, endpointing)
...
/// `QIDI_VOICE_MODE` override) — there is deliberately no local enable/disable
...
mod tests {
    ...
    /// voice gate — default on, remote kill switch / `QIDI_VOICE_MODE`).
    #[test]
