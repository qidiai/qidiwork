...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] cf_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `qidi-code`, which would defeat the lightweight
...
/// notably `std::io::Error` and `cf_tools::ToolError`) happens
