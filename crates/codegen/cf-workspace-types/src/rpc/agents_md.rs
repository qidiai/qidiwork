...
/// Claude.md / `.qidi/rules/*.md`) discovered from the workspace root up to
/// the git root, plus `~/.qidi` and compat dirs.
...
/// Mirrors the serde shape of `qidi-code`'s `AgentConfigFile`.
