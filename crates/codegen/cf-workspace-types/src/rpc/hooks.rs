...
//!
//! `cf_hooks` pulls in `git2`/`reqwest`/`qidi-code`, too heavy for
...
/// Wire mirror of `cf_hooks::discovery::HookRegistry`.
///
...
/// Wire mirror of `cf_hooks::config::HookSpec`.
///
...
/// `xai-grok-hooks`, so a server-side test (`qidi-code`'s
...
/// Wire mirror of `cf_hooks::event::HookEventName`.
///
