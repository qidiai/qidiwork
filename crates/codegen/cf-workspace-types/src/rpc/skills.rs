...
//! `qidi-code/src/implementations/skills/types.rs` (the type the
...
pub enum SkillScope {
    /// cwd/.qidi/skills
    Local,
    /// repo_root/.qidi/skills
    Repo,
    /// ~/.qidi/skills
    User,
    /// ~/.qidi/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    fn skill_info_deserializes_minimal_payload() {
        let raw = serde_json::json!({
            ...
            "path": "/workspace/.qidi/skills/my-skill/SKILL.md",
            "scope": "local",
        });
    ...
    // Fixture mirrored field-for-field from the qidi-code SkillInfo
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.qidi/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.qidi/skills"},
            "plugin_name": "infra-plugin",
            "plugin_version": "1.0.0",
            "plugin_root": "/root/.qidi/plugins/infra-plugin",
            "plugin_data": "/root/.qidi/plugin-data/infra-plugin",
            "allowed_tools": ["bash"],
            "model": "grok-4",
            "effort": "high",
            "user_invocable": true,
