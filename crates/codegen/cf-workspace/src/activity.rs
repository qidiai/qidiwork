...
use dashmap::DashMap;
use cf_file_utils::events::{Event, EventWriter, ToolOutcome};
use cf_file_utils::queue::UploadQueueStats;
use cf_tool_protocol::{ToolServerLifecycleStatus, ToolServerStatusPayload};
...
/// `QIDI_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`.
...
pub struct ActivityTracker {
    ...
    /// `QIDI_WORKSPACE_EVENTS_ENABLED` is off). Consumed by
...
impl ActivityTracker {
    ...
    /// idle-hold cap comes from `QIDI_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`
    ...
    pub fn snapshot_session(&self, session_id: &str) -> ToolServerStatusPayload {
        ...
        ToolServerStatusPayload {
            status,
            session_id: cf_tool_protocol::SessionId::new(session_id).ok(),
            connection_id: None,
...
/// The durability idle-hold cap from `QIDI_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`.
fn durability_idle_hold_max_from_env() -> u64 {
    durability_idle_hold_from_raw(std::env::var("QIDI_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS").ok())
}
