...
//!
//! Reads OIDC credentials from `~/.qidi/auth.json`, connects to a
//! server, exposes workspace tools, and refreshes tokens
//! automatically.
use clap::Parser;
use std::path::PathBuf;
use url::Url;
use cf_workspace::config::WorkspaceServerMetadata;
use cf_workspace::daemonize;
use cf_workspace::diag_server;
use cf_workspace::preview_supervisor::{self, PreviewArgs, PreviewVisibility};
...
fn server_id_startup_error(id: &str) -> Option<String> {
    id.parse::<cf_tool_protocol::ServerId>()
        .err()
...
struct Args {
    ...
    /// `QIDI_WORKSPACE_UPLOAD_QUEUE_ENABLED` env var to `false`) to fall back to
    /// the legacy inline path. Accepts `true`/`false`.
    #[arg(
        long,
        env = "QIDI_WORKSPACE_UPLOAD_QUEUE_ENABLED",
    ...
    /// Override with `QIDI_WORKSPACE_CONFINE_FS_TO_ROOT=false` (e.g. local dev).
    #[arg(
        long,
        env = "QIDI_WORKSPACE_CONFINE_FS_TO_ROOT",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
...
/// `/usr/local/bin/qidi-code-proxy` child (see `cli.rs` for the proxy's
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    let donating = cf_computer_hub_sdk::DonatingLogLayer::new_inert();
    tracing_subscriber::registry()
        .with(env_filter)
        .with(tracing_subscriber::fmt::layer())
        .with(donating.clone())
        .init();
    let direct_otlp = match std::env::var("QIDI_WORKSPACE_OTLP_ENDPOINT") {
        Ok(endpoint) if !endpoint.is_empty() => {
            match cf_tracing::init_fastrace(endpoint.clone(), SERVICE_NAME.to_owned(), None) {
                Ok(()) => {
    ...
    let url = Url::parse(&args.hub_url).map_err(|e| anyhow::anyhow!("invalid --hub-url: {e}"))?;
    {
        ...
        let profile = match std::env::var("QIDI_SANDBOX_PROFILE").ok() {
            Some(val) => {
                let parsed = val
                    .parse::<ProfileName>()
                ...
                if matches!(parsed, ProfileName::Custom(_)) {
                    tracing::warn!(
                        value = % val,
                        "Unrecognized QIDI_SANDBOX_PROFILE, defaulting to workspace"
                    );
            ...
            None if cf_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
        ...
        if profile == ProfileName::Off {
            tracing::info!(
                ...
                "Sandbox explicitly disabled via QIDI_SANDBOX_PROFILE=off"
            );
        } else {
            ...
            sandbox.install();
            let active = cf_sandbox::is_active();
            ...
            tracing::info!(
                ...
                cf_sandbox::should_restrict_child_network(), "{status_msg}"
            );
        }
    }
    let auth_provider = cf_workspace::hub_auth::provider(&url, args.auth_config.as_deref())?;
    tracing::info!(hub_url = % url, cwd = % cwd.display(), "Starting workspace server");
    let cwd_display = cwd.display().to_string();
    let session_id = std::env::var("QIDI_SESSION_ID").ok();
    ...
    let status_config = cf_workspace::StatusConfig::from_env();
    ...
    cf_workspace::init_metrics();
    let ws_handle = cf_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    let grace_budget = cf_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(
            grace_budget,
            cf_workspace::handle::DrainReason::Sigterm,
        )
        .await;
    ...
    cf_computer_hub_sdk::flush_log_layer();
