...
//! access token from `~/.qidi/auth.json` does (same identity).
...
use url::Url;
use uuid::Uuid;
use cf_computer_hub_sdk::pool::HubConnectionPool;
use cf_computer_hub_sdk::{AuthCredential, ToolHarnessBuilder};
...
async fn call_tool(
    harness: &cf_computer_hub_sdk::ToolHarness,
    name: &str,
    args: Value,
) -> anyhow::Result<Value> {
...
) -> anyhow::Result<cf_computer_hub_sdk::ToolHarness> {
