...
use std::sync::Arc;
use cf_tools::registry::types::{SessionContext, ToolRegistryBuilder, ToolServerConfig};
...
impl SessionTerminalBackend {
    ...
    /// cancel token.
    ///
    /// [`LocalTerminalBackend`]: cf_tools::computer::local::LocalTerminalBackend
    pub fn local(backend: cf_tools::computer::local::LocalTerminalBackend) -> Self {
    ...
    pub fn backend(&self) -> &Arc<dyn cf_tools::computer::types::TerminalBackend> {
        &self.backend
    }
...
pub struct WorkspaceBindConfig {
    ...
    pub viewer_ctx: Option<cf_tool_runtime::WorkspaceViewerContext>,
    ...
    pub tools: Option<Vec<cf_tools::types::ToolConfigEntry>>,
...
pub enum ResolvedToolset {
    ...
    InvalidToolConfig(cf_tools::registry::proto_convert::ToolConfigEntryError),
}
...
impl WorkspaceBindConfig {
    ...
    /// [`cf_tool_runtime::WorkspaceBindMetadata`] (same type the emitter
    /// serializes); `tool_config` is a consumer-only raw escape hatch read
    /// separately.
    pub fn from_metadata(metadata: &serde_json::Value) -> Self {
        let wire: cf_tool_runtime::WorkspaceBindMetadata =
        ...
        Self {
            ...
            tools: {
                let converted: Vec<cf_tools::types::ToolConfigEntry> = wire.tools.into_iter().map(|t| {
                    cf_tools::types::ToolConfigEntry {
                        id: t.id,
                        params_json: t.params_json,
                        name_override: t.name_override,
                        params_name_overrides: t.params_name_overrides,
                        behavior_version: t.behavior_version,
                        description_override: t.description_override,
                    }
                }).collect();
                Some(converted).filter(|tools| !tools.is_empty())
            },
    ...
    pub fn resolve(
        ...
        if let Some(cfg) = &self.tool_config {
            for (idx, tool) in cfg.tools.iter().enumerate() {
                if let Err(err) = cf_tools::types::config_validation::validate_name_override(
                    idx,
                    &tool.id.to_string(),
        ...
        if let Some(tools) = &self.tools {
            ...
            for (idx, entry) in tools.iter().enumerate() {
                ...
                match cf_tools::registry::proto_convert::tool_config_from_entry(
                    idx,
                    entry.clone(),
                ) {
            ...
            if !unserved_tool_ids.is_empty() {
                tracing::warn!(
                    ...
                    .manifest_version, running_version = cf_version::VERSION,
...
mod bind_config_tests {
    ...
    fn presets_are_never_resolved() {
        for preset in ["explore", "cf-tools-computer", "bogus"] {
...
pub struct WorkspaceConfig {
    ...
    /// Global hook sources (e.g. `~/.claude/settings.json`, `~/.qidi/hooks/`).
    pub hook_global_sources: Vec<HookSourceConfig>,
    /// Project-scoped hook sources (e.g. `<project>/.qidi/hooks/`).
    ...
    /// `<cwd>/.qidi/lsp.json`: `false` drops them at load, `true` keeps them. The
...
pub struct WorkspaceServerMetadata {
    ...
    /// Logical sandbox-service session UUID, from the `QIDI_SESSION_ID` env
...
impl WorkspaceServerMetadata {
    ...
    ///
    /// `env_session_id` is the raw `QIDI_SESSION_ID`; empty is normalized to
...
impl WorkspaceConfig {
    ...
    pub fn new_for_proxy(
        ...
        auth_provider: cf_computer_hub_sdk::SharedAuthProvider,
...
/// `*.json` hook files. Maps 1:1 to [`cf_hooks::discovery::HookSource`]
...
pub enum HookSourceConfig {
    ...
    /// A directory of `*.json` hook files (e.g. `~/.qidi/hooks/`).
    Directory(PathBuf),
}
