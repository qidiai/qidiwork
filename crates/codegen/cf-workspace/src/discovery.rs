...
//! `qidi-code` and `qidi-code` rather than duplicating them.
...
//!
//! Re-exports [`AgentsMdTracker`] from `qidi-code` for external
...
// workspace crate without a direct qidi-code dependency.
pub use cf_tools::types::agents_md_tracker::AgentsMdTracker;

// Re-export the config types that callers pass into WorkspaceConfig.
pub use cf_agent::plugins::discovery::DiscoveryConfig as PluginDiscoveryConfig;
pub use cf_agent::plugins::trust::TrustStore as PluginTrustStore;
pub use cf_agent::prompt::skills::SkillsConfig;
...
///
/// Delegates to [`cf_agent::prompt::skills::list_skills`] with
...
pub async fn discover_skills(root_cwd: &Path, config: &SkillsConfig) -> Vec<Value> {
    ...
    let skills = cf_agent::prompt::skills::list_skills(
        Some(&cwd_str),
        config,
...
pub async fn discover_agents_md(root_cwd: &Path) -> Vec<Value> {
    let cwd_str = root_cwd.to_string_lossy();
    let files = cf_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd_str,
...
///
/// Delegates to [`cf_agent::plugins::discover_plugins`] with
...
pub fn discover_plugins(
    ...
    let discovered = cf_agent::plugins::discover_plugins(
        Some(root_cwd),
        config,
        trust_store,
        project_trusted,
...
/// Load the project config from `<root_cwd>/.qidi/config.toml`.
///
...
pub fn load_project_config(root_cwd: &Path) -> Value {
    ...
    match cf_config::load_config_file(&config_path) {
...
mod tests {
    ...
    // `~/.qidi/skills/`, so on a developer machine the result may be
    ...
    fn agent_config_file_wire_matches_workspace_types_mirror() {
        // The RPC serializes cf-tools's AgentConfigFile and the remote
        ...
        // break discovery.
        let src = cf_agent::prompt::agents_md::AgentConfigFile {
            file_name: "AGENTS.md".to_string(),
            file_path: "/repo/AGENTS.md".to_string(),
            content: "# Instructions\n".to_string(),
        };
        let json = serde_json::to_value(&src).unwrap();
        let mirror: cf_workspace_types::rpc::agents_md::AgentConfigFile =
    ...
    // Discovery also scans the real `~/.qidi`, so fixtures use test-unique names.
    #[tokio::test]
    ...
    // from `~/.qidi/plugins/`, so tests check for specific plugins.
