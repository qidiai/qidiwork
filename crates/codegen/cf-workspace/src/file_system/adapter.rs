//! AcpFsAdapter: implements `qidi-code::AsyncFileSystem` using ACP gateway calls.
//!
//! This adapter enables file tool execution over ACP (remote filesystem).
//! It translates qidi-code' `AsyncFileSystem` trait into ACP protocol calls:
...
use cf_tools::computer::types::{AsyncFileSystem, ComputerError};

/// Wraps qidi-code's ACP gateway to satisfy qidi-code' AsyncFileSystem.
///
