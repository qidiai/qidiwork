...
//! [`crate::trust::TrustStore`] (`~/.qidi/trusted_folders.toml`). The
//! consume/gating half (the `DECISIONS` cache, `resolve_and_record`,
//! `project_scope_allowed`, the loader filters) lives in `qidi-code`.
//!
...
//! provisional and re-checked rather than cached — is a `qidi-code`
...
use cf_config::xai_grok_config_types::{BoolFlag, RemoteSettings};
...
pub fn decide_inputs_with_interactive(
    ...
    DecideInputs {
        ...
        // can't persist (Case 2: cwd IS $HOME, incl. the default `~/.qidi`).
...
/// this binary — true on a local/dev build (no `QIDI_VERSION` release stamp).
///
...
/// (`QIDI_VERSION` unset at compile time) — i.e. a local/dev build.
///
...
fn is_local_build() -> bool {
    // Runtime escape hatch: a pinned QIDI_TEST_VERSION simulates a release build,
    // so tests/CI (which run unstamped, i.e. local-looking) can exercise the gate.
    if std::env::var(cf_version::TEST_VERSION_ENV).is_ok() {
        return false;
    }
    option_env!("QIDI_VERSION").is_none()
}

/// Resolve whether the folder-trust gate is enabled.
///
/// On a local/dev build (no `QIDI_VERSION` release stamp) the feature is OFF
...
/// env `QIDI_FOLDER_TRUST` > `[folder_trust] enabled` (user) > managed >
...
fn feature_enabled_for_build(remote: Option<&RemoteSettings>, is_local_build: bool) -> bool {
    ...
    // shipped, release-stamped binaries. Even an explicit QIDI_FOLDER_TRUST/config
    ...
    let user = cf_config::load_from_disk().ok();
    let managed = cf_config::load_managed_config().ok();
    BoolFlag::env("QIDI_FOLDER_TRUST")
...
/// same `~/.qidi/trusted_folders.toml`. Best-effort; a write failure is logged,
/// not fatal.
...
/// `.claude` / `.qidi/agents` / subdir-launch gaps. NOT itself the trust gate.
...
fn collect_repo_config_kinds(cwd: &Path, first_only: bool) -> Vec<&'static str> {
    ...
    let chain = cf_agent::repo::RepoDirChain::resolve(cwd);
    ...
    // Project `.qidi/config.toml` declaring repo-controlled code-exec: a
    ...
    for path in crate::project_config::find_project_configs_in(&chain.dirs) {
        let Ok(root) = cf_config::load_config_file(&path) else {
            continue;
        };
    ...
    // must be gated — else a hooks-only clone (e.g. `.qidi/hooks/evil.json`) would
    ...
    // that must be gated — else a plugin clone (e.g. `.qidi/plugins/evil/`, even
    ...
    if !cf_agent::plugins::project_plugin_dirs_in(&chain.dirs).is_empty() {
        hit!("plugins");
    }
    // Project AGENT dirs (`.qidi/agents` / `.claude/agents`): a project agent
    ...
    if !cf_agent::discovery::project_agent_dirs_in(&chain.dirs).is_empty() {
        hit!("agents");
    }
...
pub fn prompt_for_trust(key: &Path) -> bool {
    ...
    let _ = writeln!(
        err,
        "This folder contains repo-local config (.mcp.json / .qidi/lsp.json / hooks) \
...
mod tests {
    ...
    fn repo_configs_present_detects_project_agents() {
        // A `.qidi/agents`-only clone must be gated: a project agent definition
    ...
    // QIDI_HOME-isolation idiom mirrored from this crate's `permission::claude_compat`
    ...
    fn simulate_release_build() -> EnvVarGuard {
        EnvVarGuard::set(cf_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"))
    }
    ...
    fn local_build_ignores_remote_rollout() {
        ...
        let _home = EnvVarGuard::set("QIDI_HOME", home.path());
        let _flag = EnvVarGuard::unset("QIDI_FOLDER_TRUST");
    ...
    fn release_build_keeps_gate_when_enabled() {
        ...
        // config nor an ambient env flag can override it: empty QIDI_HOME (no
        // config.toml/managed_config.toml) + QIDI_FOLDER_TRUST unset. nextest's
    ...
    fn local_build_ignores_explicit_env_optin() {
        ...
        // QIDI_FOLDER_TRUST=1 does NOT enable the feature (so a self-built grok
        // never prompts). QIDI_HOME isolated so on-disk config can't influence it.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("QIDI_HOME", home.path());
        let _flag = EnvVarGuard::set("QIDI_FOLDER_TRUST", Path::new("1"));
    ...
    fn release_build_defaults_on() {
        ...
        // the feature ON. Empty QIDI_HOME (no config.toml/managed config) +
        // QIDI_FOLDER_TRUST unset so only the default applies.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("QIDI_HOME", home.path());
        let _flag = EnvVarGuard::unset("QIDI_FOLDER_TRUST");
    ...
    fn is_local_build_honors_test_version_override() {
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        // A pinned QIDI_TEST_VERSION simulates a release build => not a local build.
        {
            let _sim = EnvVarGuard::set(cf_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"));
            assert!(!is_local_build());
        }
        // With it unset, an unstamped build (no QIDI_VERSION) is a local build.
        // Guard to the unstamped case so a release-stamped test binary (CI release)
        // doesn't spuriously fail this arm.
        let _unset = EnvVarGuard::unset(cf_version::TEST_VERSION_ENV);
        if option_env!("QIDI_VERSION").is_none() {
            assert!(is_local_build());
        }
    }
    ...
    fn store_io_is_noop_on_local_build() {
        ...
        // (mirrors `is_local_build_honors_test_version_override`); QIDI_HOME-isolated
        // and ENV_LOCK-serialized so toggling QIDI_TEST_VERSION is race-safe.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("QIDI_HOME", home.path());
        let _unset = EnvVarGuard::unset(cf_version::TEST_VERSION_ENV);
    ...
    fn revoke_folder_trust_store_persists_untrust_for_trusted_folder() {
        ...
        // QIDI_HOME-isolated so the seed/deny hit a temp store, not the real file.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("QIDI_HOME", home.path());
        let _sim = simulate_release_build();
        let tmp = repo_tmp();
        let key = workspace_key(tmp.path());
    ...
    fn revoke_folder_trust_store_writes_no_deny_for_never_trusted_folder() {
        ...
        // QIDI_HOME-isolated so the grant writes to a temp store.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("QIDI_HOME", home.path());
        let _sim = simulate_release_build();
