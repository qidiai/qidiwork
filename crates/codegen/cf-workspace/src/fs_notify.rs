...
//! FsNotify adapter functions bridging [`cf_fsnotify`] events to
...
pub(crate) fn fs_event_to_codebase_graph_event(
    paths: &[PathBuf],
    kind: FsEventKind,
) -> cf_codebase_graph::FileEvent {
    use cf_codebase_graph::{FileEvent, FileEventKind};
...
/// Convert [`cf_fsnotify::FsEventKind`] to the wire-type
/// [`cf_workspace_types::FsEventKind`].
///
...
pub(crate) fn to_workspace_event_kind(kind: FsEventKind) -> cf_workspace_types::FsEventKind {
    match kind {
        FsEventKind::Created => cf_workspace_types::FsEventKind::Created,
        FsEventKind::Modified => cf_workspace_types::FsEventKind::Modified,
        FsEventKind::Removed => cf_workspace_types::FsEventKind::Removed,
        FsEventKind::Renamed => cf_workspace_types::FsEventKind::Renamed,
        // `#[non_exhaustive]` fallback.
        _ => cf_workspace_types::FsEventKind::Modified,
    }
}
...
/// [`WorkspaceEvent::FsChanged`](cf_workspace_types::WorkspaceEvent::FsChanged)
...
pub(crate) fn spawn_fs_event_forwarder(
    ...
    events_tx: tokio::sync::broadcast::Sender<cf_workspace_types::WorkspaceEvent>,
    cwd: PathBuf,
    cancel: tokio_util::sync::CancellationToken,
    codebase_index: Option<std::sync::Arc<cf_codebase_graph::IndexManagerHandle>>,
) {
                        ...
                        Ok(FsEvent::FilesChanged { ref paths, kind }) => {
                            ...
                            for path in paths {
                                let _ = events_tx.send(
                                    cf_workspace_types::WorkspaceEvent::FsChanged {
                                        path: path.clone(),
                                        kind: ws_kind,
                                    },
                                );
...
fn parse_diff_name_status_line(
    line: &str,
    repo_root: &Path,
) -> Option<cf_codebase_graph::FileEvent> {
    let mut parts = line.splitn(3, '\t');
    let status = parts.next()?.trim();
    let path = parts.next()?;

    match status.chars().next()? {
        'A' => Some(cf_codebase_graph::FileEvent::created(repo_root.join(path))),
        'D' => Some(cf_codebase_graph::FileEvent::removed(repo_root.join(path))),
        'R' | 'C' => {
            let new_path = parts.next()?;
            Some(cf_codebase_graph::FileEvent::renamed(
                repo_root.join(path),
                repo_root.join(new_path),
            ))
        }
        _ => Some(cf_codebase_graph::FileEvent::modified(
            repo_root.join(path),
        )),
    }
}
...
pub(crate) async fn refresh_codebase_graph_after_head_change(
    idx: &cf_codebase_graph::IndexManagerHandle,
    repo_root: &Path,
    events_tx: &tokio::sync::broadcast::Sender<cf_workspace_types::WorkspaceEvent>,
) {
    ...
    cf_tools::util::detach_command(&mut diff_cmd);
    diff_cmd.envs(cf_tools::util::pager_env());
    ...
    if let Some(count) = files_updated {
        let _ = events_tx.send(
            cf_workspace_types::WorkspaceEvent::CodebaseIndexUpdated {
                files_indexed: count,
            },
        );
    }
...
pub(crate) fn ws_event_to_codebase_graph_event(
    path: &std::path::Path,
    kind: cf_workspace_types::FsEventKind,
) -> cf_codebase_graph::FileEvent {
    use cf_codebase_graph::{FileEvent, FileEventKind};
    let graph_kind = match kind {
        cf_workspace_types::FsEventKind::Created => FileEventKind::Created,
        cf_workspace_types::FsEventKind::Modified => FileEventKind::Modified,
        cf_workspace_types::FsEventKind::Removed => FileEventKind::Removed,
        cf_workspace_types::FsEventKind::Renamed => FileEventKind::Renamed,
    };
...
mod tests {
    ...
    fn hidden_dir_positive() {
        assert!(is_under_hidden_dir(
            &PathBuf::from("/workspace/.qidi/worktrees/abc/src/main.rs"),
    ...
    fn workspace_event_kind_round_trip() {
        use cf_workspace_types::FsEventKind as WsKind;
        assert_eq!(
    ...
    fn codebase_graph_event_mapping() {
        use cf_codebase_graph::FileEventKind;
