...
use std::sync::Arc;
use cf_hunk_tracker::{HunkTrackerActor, HunkTrackerHandle, TrackingMode};
use cf_tool_protocol::ToolServerStatusPayload;
use cf_tool_protocol::turn_hook::TurnHookOutcome;
/// Default SIGTERM drain budget (ms); override via
/// `QIDI_WORKSPACE_TERMINATION_GRACE_MS`. 45s fits under the K8s grace period.
const DEFAULT_TERMINATION_GRACE_MS: u64 = 45_000;
/// preStop-hook drain marker; override via `QIDI_WORKSPACE_DRAINING_FILE`.
...
use cf_file_utils::events::types::CancellationCategory;
use cf_file_utils::events::{Event, SessionRelationship, TurnOutcomeLabel};
use cf_file_utils::queue::EnqueueOutcome;
use cf_tool_protocol::turn_hook::{AfterTurnAckPayload, AfterTurnAckStatus};
...
impl WorkspaceHandle {
    ...
    pub async fn trace_donation_reporter(
        ...
        cf_computer_hub_sdk::HubDonatingReporter,
        cf_computer_hub_sdk::TraceDonationPump,
    )> {
        self.shared
            .hub_handle
            .lock()
    ...
    /// teardown.
    ///
    /// [`LogDonationSender`]: cf_computer_hub_sdk::LogDonationSender
    pub async fn log_donation_layer(
        &self,
        service_name: &str,
    ) -> Option<(
        cf_computer_hub_sdk::LogDonationSender,
        cf_computer_hub_sdk::LogDonationPump,
    )> {
        self.shared
            .hub_handle
            .lock()
    ...
    pub async fn metric_donation_reporter(
        &self,
        service_name: &str,
    ) -> Option<cf_computer_hub_sdk::MetricDonationPump> {
        self.shared
            .hub_handle
            .lock()
            .await
    ...
    /// Construct a handle with an explicit `$QIDI_WORKSPACE_HOME` and a
    /// pre-spawned [`UploadQueue`](cf_file_utils::queue::UploadQueue).
    ///
    ...
    pub(crate) fn new_with_data_collection(
        ...
        upload_queue: Arc<cf_file_utils::queue::UploadQueue>,
    ...
    fn build(
        config: WorkspaceConfig,
        workspace_home: std::path::PathBuf,
        upload_queue: Option<Arc<cf_file_utils::queue::UploadQueue>>,
        ...
        let local_registry = cf_computer_hub_sdk::LocalRegistry::new();
        ...
        let (hook_registry, hook_load_errors) = {
            use cf_hooks::discovery::{HookSource, load_hooks_from_sources};
        ...
        let lsp: Option<Arc<dyn cf_tools::implementations::lsp::LspBackend>> = {
            let sourced =
                cf_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
                    &config.root_cwd,
                    &[],
                    &[],
                    &[],
            ...
            let servers =
                cf_tools::implementations::lsp::config::filter_project_lsp_when_untrusted(
                    sourced,
            ...
            if servers.is_empty() {
                ...
                use cf_tools::implementations::lsp::{
        ...
        let session_event_writers: Arc<
            dashmap::DashMap<String, cf_file_utils::events::EventWriter>,
    ...
    /// The [`ToolServer`](cf_computer_hub_sdk::ToolServer) for this
    ...
    pub fn hub_server(&self) -> Option<cf_computer_hub_sdk::ToolServer> {
    ...
    pub async fn hub_server_blocking(&self) -> Option<cf_computer_hub_sdk::ToolServer> {
    ...
    pub fn create_session_with_config(
        ...
        session_id: impl Into<String>,
        cwd: Option<std::path::PathBuf>,
        tool_config: Option<cf_tools::registry::types::ToolServerConfig>,
        capability: CapabilityMode,
        viewer_ctx: Option<cf_tool_runtime::WorkspaceViewerContext>,
    ...
    pub fn create_session_with_tracker_and_viewer_ctx(
        ...
        let system_notify_channel = system_notifications
            .then(cf_tools::notification::types::ToolNotificationHandle::channel);
    ...
    pub(crate) async fn update_tool_config(
        ...
        session_id: &str,
        new_config: cf_tools::registry::types::ToolServerConfig,
    ...
    async fn resolve_and_swap_session_toolset_locked(
        ...
        let _ = self
            .shared
            .events
            .send(cf_workspace_types::WorkspaceEvent::ToolsChanged {
    ...
    pub(crate) async fn rebind_existing_hub_session(
        &self,
        session_id: &str,
        explicit_cfg: Option<cf_tools::registry::types::ToolServerConfig>,
    ...
    pub async fn on_before_turn(
        &self,
        session_id: &str,
        payload: &cf_tool_protocol::turn_hook::BeforeTurnPayload,
    ) {
    ...
    pub async fn on_after_turn(
        &self,
        session_id: &str,
        payload: &cf_tool_protocol::turn_hook::AfterTurnPayload,
    ) {
    ...
    pub async fn compute_turn_injections(
        &self,
        session_id: &str,
        request: &cf_tool_protocol::turn_hook::TurnHookRequest,
    ) -> cf_tool_protocol::turn_hook::HookReply {
        use cf_tool_protocol::turn_hook::{HookReply, TurnHookRequest};
        match request {
    ...
    /// `QIDI_WORKSPACE_TOOL_STATE_ENABLED` is off, opted out,
    ...
    ///
    /// No-op when the `QIDI_WORKSPACE_TOOL_DEFS_ENABLED` flag is off, no upload
    ...
    pub fn cancel_tool_call(&self, session_id: &str, call_id: &str) {
        self.shared.activity_tracker.tool_call_completed(
            call_id,
            Some(session_id),
            cf_file_utils::events::ToolOutcome::Cancelled,
        );
    ...
    pub fn hook_registry(&self) -> cf_hooks::discovery::HookRegistry {
    ...
    pub fn hook_load_errors(&self) -> &[cf_hooks::error::HookError] {
    ...
    pub fn get_or_create_codebase_index(
        &self,
        cwd: std::path::PathBuf,
    ) -> (Arc<cf_codebase_graph::IndexManagerHandle>, bool) {
        self.shared.codebase_indexes.lock().get_or_create(cwd)
    }
    pub fn get_codebase_index(
        &self,
        cwd: &std::path::Path,
    ) -> Option<Arc<cf_codebase_graph::IndexManagerHandle>> {
    ...
    fn spawn_codebase_index_event_forwarder(&self) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                match rx.recv().await {
                    Ok(cf_workspace_types::WorkspaceEvent::FsChanged { ref path, kind }) => {
                    ...
                    Ok(cf_workspace_types::WorkspaceEvent::GitHeadChanged { .. }) => {
    ...
    fn spawn_tool_definitions_event_forwarder(&self) -> Option<tokio::task::JoinHandle<()>> {
        ...
        Some(tokio::spawn(async move {
            ...
            loop {
                match rx.recv().await {
                    Ok(cf_workspace_types::WorkspaceEvent::ToolsChanged { session_id }) => {
    ...
    async fn emit_environment_artifact(
    ...
    ) -> Option<cf_file_utils::queue::EnqueueOutcome> {
        ...
        let env = {
            ...
            .in_span(
                fastrace::Span::root(
                    ...
                    trace_parent.unwrap_or_else(cf_tracing::local_or_random_span_ctx),
                )
                .with_properties(|| {
                    [
                        ("session_id", session_id.to_owned()),
        ...
        let outcome = upload_queue
            ...
            .await;
        match &outcome {
            cf_file_utils::queue::EnqueueOutcome::Failed { reason: _ } => {
                dc_log!(
    ...
    pub async fn start_session_mcp_servers(
        ...
        use cf_computer_hub_mcp_adapter::McpBridge;
        use cf_computer_hub_sdk::ToolServerHandler as _;
        use cf_mcp::servers::MCP_TOOL_NAME_DELIMITER;
        use cf_tool_protocol::SessionId;
        let tool_server = {
        ...
        let mcp_results: Vec<
            Result<cf_mcp::servers::McpClient, cf_mcp::servers::McpError>,
        > = tokio::task::spawn_blocking(move || {
            use std::collections::HashMap;
            use cf_mcp::oauth_config::McpOAuthConfigMap;
            use cf_mcp::servers::{McpClientTimeoutOverrides, McpMetaConfigMap};
            ...
            rt_handle.block_on(cf_mcp::servers::start_mcp_servers(
                configs,
                ...
                &oauth_config_map,
                &event_writer,
                cf_mcp::servers::OauthInteractivity::Interactive,
            ))
        })
        .await
        ...
        for result in mcp_results {
            match result {
                Ok(client) => {
                    ...
                    let transport: Arc<dyn cf_computer_hub_mcp_adapter::McpTransport> =
                        ...
                        Ok(handle) => {
                            for handler in handle.bridge.handlers() {
                                    ...
                                    .await
                                {
                                    ...
                                    cf_tool_protocol::ToolId::new(&qualified_name)
                                {
    ...
    pub async fn teardown_session_mcp(&self, session_id: &str) {
        ...
        let sid = match cf_tool_protocol::SessionId::new(session_id) {
            Ok(s) => s,
            Err(_) => return,
        };
        let mut tool_ids = session.mcp_tool_ids.lock().await;
    ...
    pub fn on_mcp_snapshot_changed(
        &self,
        new_snapshot: Vec<cf_tools::registry::types::ToolConfig>,
    ) -> usize {
    ...
    pub fn on_hub_tools_changed(
        &self,
        new_hub_tools: Vec<cf_tools::registry::types::ToolConfig>,
    ) -> usize {
        self.shared
            .hub_tools_snapshot
    ...
    pub(crate) fn session_bind_resolver(
        &self,
        catalog: Arc<Vec<Arc<dyn cf_computer_hub_sdk::ToolServerHandler>>>,
        rpc_tool_id: cf_tool_protocol::ToolId,
    ) -> cf_computer_hub_sdk::SessionHandlerResolver {
        let weak_shared = Arc::downgrade(&self.shared);
        Arc::new(
            move |sid: cf_tool_protocol::SessionId, params: Option<serde_json::Value>| {
                ...
                let bind_parent = params
                    ...
                    .unwrap_or_else(cf_tracing::local_or_random_span_ctx);
                ...
                async move {
                    let Some(shared) = weak_shared.upgrade() else {
                        WORKSPACE_BIND_FAILED_TOTAL
                            ...
                            .inc();
                        return Err(
                            cf_tool_runtime::ToolError::service_unavailable(
                    ...
                    let bind_config = params
                        ...
                        .unwrap_or_default();
                    let empty_toolset = || cf_tools::registry::types::ToolServerConfig {
                        tools: vec![],
                    ...
                    Ok(cf_computer_hub_sdk::ResolvedSessionHandlers {
                        handlers,
                        unserved_tool_ids,
                        resolve_error,
                    })
    ...
    pub async fn connect_hub(&self) -> WorkspaceResult<()> {
        ...
        let (template_handlers, rpc_tool_id) = {
            ...
            let tool_names: Vec<String> = handlers
                ...
                .map(|h| h.tool_id().as_str().to_owned())
                .collect();
            let rpc_handler: Arc<dyn cf_computer_hub_sdk::ToolServerHandler> =
        ...
        let catalog: Arc<Vec<Arc<dyn cf_computer_hub_sdk::ToolServerHandler>>> =
        ...
        let (activity_notify_handle, activity_notify_rx) =
            cf_tools::notification::types::ToolNotificationHandle::channel();
        ...
        let listener_task = tokio::spawn(async move {
            while let Some(notification) = notification_rx.recv().await {
                match notification {
                    cf_computer_hub_sdk::HubNotification::ToolsChanged {
                        added,
                        removed,
                        updated,
                        ..
            ...
            loop {
                match rx.recv().await {
                    Ok(event) => {
                        ...
                        let frame = cf_tool_protocol::ToolNotificationFrame::custom(
                            cf_tool_protocol::ToolId::new(
        ...
        let status_publisher_task = tokio::spawn(async move {
            ...
            async fn send_status(
                conn: &cf_computer_hub_sdk::HubConnection,
                ...
                let req = cf_tool_protocol::JsonRpcRequest {
                    jsonrpc: cf_tool_protocol::JsonRpcVersion,
                    id: cf_tool_protocol::JsonRpcId::from_request_id(&request_id),
                    session_id: None,
                    method: cf_tool_protocol::Method::ToolServerStatus
                        .as_wire_str()
                        .to_owned(),
                    params,
                };
...
fn build_session_routed_handlers(
    ...
    ws: &WorkspaceHandle,
) -> Vec<Arc<dyn cf_computer_hub_sdk::ToolServerHandler>> {
    ...
    for def in toolset.tool_definitions() {
        ...
        let mut desc = cf_tool_types::ToolDescription::new(
        ...
        match crate::hub::SessionRoutedToolHandler::new(
            ...
            Ok(handler) => {
                handlers.push(Arc::new(handler) as Arc<dyn cf_computer_hub_sdk::ToolServerHandler>)
            }
            Err(e) => {
                tracing::warn!(
...
pub(crate) fn apply_background_task_notification(
    ...
    notification: &cf_tools::notification::types::ToolNotification,
) {
    use cf_tools::notification::types::ToolNotification;
    match notification {
...
pub(crate) async fn run_activity_feed(
    ...
    mut rx: tokio::sync::mpsc::UnboundedReceiver<
        cf_tools::notification::types::ToolNotification,
    >,
) {
...
/// The SIGTERM drain budget from `QIDI_WORKSPACE_TERMINATION_GRACE_MS`
...
pub fn termination_grace_from_env() -> std::time::Duration {
    grace_budget_from_raw(std::env::var("QIDI_WORKSPACE_TERMINATION_GRACE_MS").ok())
}
...
/// Path of the preStop drain marker (`QIDI_WORKSPACE_DRAINING_FILE` or
/// [`DEFAULT_DRAINING_FILE`]).
fn draining_file_path() -> std::path::PathBuf {
    std::env::var("QIDI_WORKSPACE_DRAINING_FILE")
        .map(std::path::PathBuf::from)
...
/// sandbox; override via `QIDI_WORKSPACE_CONFINE_FS_TO_ROOT`); the CLI leader
/// passes `false`.
///
...
pub async fn connect_local_workspace(
    cwd: std::path::PathBuf,
    hub_url: url::Url,
    auth: cf_computer_hub_sdk::SharedAuthProvider,
    ...
    if let Ok(dir) = std::env::var("QIDI_WORKSPACE_SERVER_SKILLS_DIR")
        && !dir.is_empty()
    {
        ws_config.skills_config.server_skill_dirs = vec![dir];
    }
    if let Ok(dir) = std::env::var("QIDI_WORKSPACE_BUNDLED_SKILLS_DIR")
        && !dir.is_empty()
    {
        let allowlist = std::env::var("QIDI_WORKSPACE_BUNDLED_SKILLS_ALLOWLIST").ok();
        ws_config
            .skills_config
            .ignore
    ...
    let trace_source: Arc<dyn cf_file_utils::queue::TraceExportSource> = Arc::new(
        crate::upload::WorkspaceTraceExportSource::new(proxy_storage.clone()),
    );
    let upload_queue = Arc::new(cf_file_utils::queue::UploadQueue::spawn(
        &workspace_home,
        trace_source,
        cf_file_utils::queue::UploadRetryPolicy::default(),
    ));
    ...
    upload_queue.cleanup_orphans(cf_file_utils::queue::DEFAULT_MAX_AGE);
...
/// Resolve `$QIDI_WORKSPACE_HOME` — the workspace-owned on-disk state root.
///
/// Precedence:
/// 1. `$QIDI_WORKSPACE_HOME` (operator override).
/// 2. `<grok_home>/workspace`, where `<grok_home>` honours `$QIDI_HOME` and
///    otherwise falls back to `~/.qidi` (see [`cf_config::grok_home`]).
pub fn resolve_workspace_home() -> std::path::PathBuf {
    if let Ok(p) = std::env::var("QIDI_WORKSPACE_HOME") {
        if !p.trim().is_empty() {
            return std::path::PathBuf::from(p);
        }
    }
    cf_config::grok_home().join("workspace")
}
...
/// (`QIDI_WORKSPACE_EVENTS_ENABLED=true`). Any other value — including unset —
/// keeps the legacy behaviour: [`WorkspaceShared::session_event_writer`] hands
/// back [`EventWriter::noop()`](cf_file_utils::events::EventWriter::noop)
/// and no `events.jsonl` is ever opened.
fn events_enabled() -> bool {
    std::env::var("QIDI_WORKSPACE_EVENTS_ENABLED").as_deref() == Ok("true")
}
...
/// `QIDI_WORKSPACE_AFTER_TURN_WATCHDOG_MS` (malformed values fall back).
fn after_turn_watchdog() -> std::time::Duration {
    const DEFAULT_MS: u64 = 8_000;
    let ms = std::env::var("QIDI_WORKSPACE_AFTER_TURN_WATCHDOG_MS")
        .ok()
...
/// (`QIDI_WORKSPACE_TOOL_DEFS_ENABLED=true`; any other value keeps legacy
/// behaviour).
fn tool_defs_enabled() -> bool {
    std::env::var("QIDI_WORKSPACE_TOOL_DEFS_ENABLED").as_deref() == Ok("true")
}
...
async fn enqueue_workspace_tool_definitions(
    upload_queue: &cf_file_utils::queue::UploadQueue,
    session_id: &str,
    object_path: &str,
    bytes: &[u8],
) -> cf_file_utils::queue::EnqueueOutcome {
    use cf_file_utils::queue::EnqueueOutcome;
    let outcome = upload_queue
        .enqueue_bytes_blocking(
            bytes,
...
fn turn_outcome_label(outcome: cf_tool_protocol::turn_hook::TurnHookOutcome) -> TurnOutcomeLabel {
    use cf_tool_protocol::turn_hook::TurnHookOutcome;
    match outcome {
...
/// only [`connect_local_workspace`] resolves `$QIDI_WORKSPACE_HOME` — so the
...
/// Resolve `workspace_rewind_all_outcomes` from `QIDI_WORKSPACE_REWIND_ALL_OUTCOMES` (default off).
fn rewind_all_outcomes_from_env() -> bool {
    cf_config::env_bool("QIDI_WORKSPACE_REWIND_ALL_OUTCOMES").unwrap_or(false)
}
...
struct SessionToolHandle {
    tool_id: cf_tool_protocol::ToolId,
    desc: cf_tool_types::ToolDescription,
...
impl SessionToolHandle {
    fn new(
        ...
        workspace: WorkspaceHandle,
        session_id: String,
    ) -> Result<Self, cf_tool_protocol::IdError> {
        Ok(Self {
            tool_id: cf_tool_protocol::ToolId::new(tool_name)?,
            desc,
            workspace,
            session_id,
        })
...
impl cf_tool_runtime::ToolDyn for SessionToolHandle {
    ...
    ) -> cf_tool_runtime::ToolStream<cf_tool_runtime::TypedToolOutput> {
        use cf_tool_runtime::{ToolError, ToolErrorKind, ToolStreamItem, terminal_only};
...
impl WorkspaceHandle {
    ...
    pub fn create_local_harness(
        &self,
        session_id: &str,
    ) -> WorkspaceResult<cf_computer_hub_sdk::ToolHarness> {
        let session = self
            .session(session_id)
            .ok_or_else(|| WorkspaceError::SessionNotFound(session_id.to_string()))?;
        let toolset = session.toolset();
        let registry = cf_computer_hub_sdk::LocalRegistry::new();
        for def in toolset.tool_definitions() {
            let tool_name = def.function.name.clone();
            let desc = cf_tool_types::ToolDescription::new(
                tool_name.clone(),
                def.function.description.clone().unwrap_or_default(),
            );
            match SessionToolHandle::new(tool_name, desc, self.clone(), session_id.to_string()) {
                Ok(tool) => {
                    registry.register_dyn(Arc::new(tool) as Arc<dyn cf_tool_runtime::ToolDyn>);
                }
                Err(e) => {
                    tracing::warn!(
                        tool = % def.function.name, error = % e,
        ...
        let session_id = cf_tool_protocol::SessionId::new(session_id.to_string())
            .map_err(|e| WorkspaceError::HubError(format!("invalid session id: {e}")))?;
        Ok(cf_computer_hub_sdk::ToolHarness::local_only_with(
            registry,
            session_id,
            cf_tool_runtime::TypedExtensions::default(),
        ))
    }
}
impl WorkspaceHandle {
    ...
    pub fn new_minimal(
        ...
        let config = WorkspaceConfig {
            root_cwd: cwd,
            default_tool_config: cf_tools::registry::types::ToolServerConfig {
                tools: vec![],
                behavior_preset: None,
            },
...
pub(crate) mod tests {
    ...
    use std::sync::Arc;
    use cf_tools::registry::types::ToolServerConfig;
    use cf_tools::types::tool::ToolKind;
    use cf_workspace_types::WorkspaceEvent;
    ...
    fn make_handle_inner(
        ...
        let config = WorkspaceConfig {
            ...
            plugin_discovery_config: Default::default(),
            hub_config: None,
            auth_provider: Some(Arc::new(cf_computer_hub_sdk::AuthCredential::bearer(
                "test-token",
            ))),
            server_metadata: None,
            status_config,
    ...
    impl cf_tools::types::tool_metadata::ToolMetadata for BashCcoStub {
        fn kind(&self) -> ToolKind {
            ToolKind::Execute
        }
        fn tool_namespace(&self) -> cf_tools::types::tool::ToolNamespace {
            cf_tools::types::tool::ToolNamespace::MCP
        }
        fn description_template(&self) -> &str {
            "bash cco stub"
        }
    }
    impl cf_tool_runtime::Tool for BashCcoStub {
        type Args = serde_json::Value;
        type Output = cf_tools::types::output::ToolOutput;
        fn id(&self) -> cf_tool_protocol::ToolId {
            cf_tool_protocol::ToolId::new(BASH_CCO_STUB_NAME).expect("valid tool id")
        }
        fn description(
            &self,
            _ctx: &::cf_tool_runtime::ListToolsContext,
        ) -> cf_tool_types::ToolDescription {
            cf_tool_types::ToolDescription::new(BASH_CCO_STUB_NAME, "bash cco stub")
        }
        async fn run(
            &self,
            _ctx: cf_tool_runtime::ToolCallContext,
            _input: serde_json::Value,
        ) -> Result<cf_tools::types::output::ToolOutput, cf_tool_runtime::ToolError>
        {
            let output = BASH_CCO_STUB_STDOUT.as_bytes();
            Ok(cf_tools::types::output::ToolOutput::Bash(
                cf_tools::types::output::BashOutput {
                    output: output.to_vec(),
                    output_for_prompt:
                        cf_tools::types::output::BashOutput::make_output_for_prompt(
    ...
    pub(crate) fn assert_bash_cco_terminal(typed: &cf_tool_runtime::TypedToolOutput) {
        use cf_tool_runtime::ToolOutput as _;
        let resp = typed
    ...
    pub(crate) async fn drain_terminal_ok(
        mut stream: impl futures::Stream<
            Item = cf_tool_runtime::ToolStreamItem<cf_tool_runtime::TypedToolOutput>,
        > + Unpin,
    ) -> cf_tool_runtime::TypedToolOutput {
        use futures::StreamExt;
        use cf_tool_runtime::ToolStreamItem;
    ...
    async fn local_harness_preserves_bash_chat_completion_output() {
        use cf_tool_runtime::ToolCallContext;
        let handle = make_handle();
        register_bash_cco_stub(&handle);
        let harness = handle.create_local_harness("main").expect("local harness");
        let tool_id = cf_tool_protocol::ToolId::new(BASH_CCO_STUB_NAME).expect("valid tool id");
        let stream = harness
    ...
    /// ABSENT from that (cf-tools) catalog is still advertised. The old
    /// `catalog ∩ session-names` filter silently dropped exactly such tools
    /// (cf-tools renames → 6/11).
    #[tokio::test]
    ...
    async fn rebind_explicit_to_explicit_with_in_flight_call_defers_then_corrects() {
        use cf_file_utils::events::ToolOutcome;
    ...
    async fn toolset_terminal(
        toolset: &Arc<cf_tools::registry::types::FinalizedToolset>,
    ) -> Arc<dyn cf_tools::computer::types::TerminalBackend> {
        let res = toolset.resources.lock().await;
        res.get::<cf_tools::types::resources::Terminal>()
            .map(|t| t.0.clone())
    ...
    ///
    /// [`TerminalRunRequest`]: cf_tools::computer::types::TerminalRunRequest
    pub(crate) fn terminal_run_request(
        command: &str,
        out_dir: &std::path::Path,
        tool_call_id: &str,
    ) -> cf_tools::computer::types::TerminalRunRequest {
        cf_tools::computer::types::TerminalRunRequest {
            ...
            notification_handle: cf_tools::notification::ToolNotificationHandle::noop(),
    ...
    pub(crate) async fn start_background_sleep(
        ...
        out_dir: &std::path::Path,
        tool_call_id: &str,
    ) -> cf_tools::computer::types::BackgroundHandle {
        session
            .terminal_backend()
    ...
    impl crate::config::SessionContextFactory for PersistentShellFactory {
        ...
        fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
            crate::config::SessionTerminalBackend::local(
                cf_tools::computer::local::LocalTerminalBackend::with_persistent_shell(),
            )
        }
        fn registry_builder(&self) -> cf_tools::registry::types::ToolRegistryBuilder {
    ...
    async fn restarted_workspace_recreates_session_and_reports_lost_task() {
        ...
        let cf_tools::types::output::ToolOutput::TaskOutput(
            cf_tool_types::TaskOutputOutput::TaskNotFound(msg),
        ) = &result.output
        else {
    ...
    async fn events_jsonl_captures_turn_tool_toggle_and_mcp_variants() {
        use cf_file_utils::events::ToolOutcome;
        use cf_tool_protocol::turn_hook::{AfterTurnPayload, BeforeTurnPayload, TurnHookOutcome};
        ...
        handle.shared().session_event_writer(sid).emit(
            cf_file_utils::events::Event::McpToolCallStarted {
    ...
    async fn before_turn_hooks_sync_session_yolo_mode() {
        use cf_tool_protocol::turn_hook::{BeforeTurnPayload, TurnHookRequest};
        let handle = make_handle();
        ...
        assert_eq!(
            reply,
            cf_tool_protocol::turn_hook::HookReply::default(),
    ...
    async fn before_turn_yolo_transition_emits_yolo_toggled_event() {
        use cf_tool_protocol::turn_hook::BeforeTurnPayload;
        let (handle, home) = make_handle_with_events();
        let sid = "sess-yolo";
        let _session = handle
    ...
    fn turn_outcome_label_maps_every_variant() {
        use cf_file_utils::events::TurnOutcomeLabel;
        use cf_tool_protocol::turn_hook::TurnHookOutcome;
        assert!(matches!(
    ...
    impl cf_file_utils::queue::TraceExportSource for UnreachableSource {
        fn resolve(&self) -> cf_file_utils::TraceExportConfig {
            cf_file_utils::TraceExportConfig {
                bucket_url: None,
                service_account_key: None,
                upload_method: cf_file_utils::UploadMethod::Proxy {
    ...
    fn spawn_test_queue(home: &std::path::Path) -> Arc<cf_file_utils::queue::UploadQueue> {
        let policy = cf_file_utils::queue::UploadRetryPolicy {
            initial_delay: std::time::Duration::from_secs(3600),
            ..Default::default()
        };
        Arc::new(cf_file_utils::queue::UploadQueue::spawn(
            home,
    ...
    /// `$QIDI_WORKSPACE_HOME` — must NOT configure an upload queue, and must leave
    ...
    async fn new_defaults_to_ephemeral_home_and_inert_legacy_upload() {
        ...
        assert_ne!(
            ...
            "default construction must NOT use the real $QIDI_WORKSPACE_HOME"
        );
        assert!(
    ...
    async fn tool_state_upload_is_noop_when_flag_off() {
        ...
        unsafe { std::env::remove_var("QIDI_WORKSPACE_TOOL_STATE_ENABLED") };
        let factory = Arc::new(TestSessionContextFactory::new());
        let cwd = factory.temp.path().to_path_buf();
        let queue_home = tempfile::TempDir::new().unwrap();
        let queue = spawn_test_queue(queue_home.path());
    ...
    async fn tool_state_upload_is_noop_when_data_collection_disabled() {
        ...
        let _env = crate::session::tool_config::TOOL_STATE_ENV_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        unsafe { std::env::set_var("QIDI_WORKSPACE_TOOL_STATE_ENABLED", "true") };
        let factory = Arc::new(TestSessionContextFactory::new());
        let cwd = factory.temp.path().to_path_buf();
        let queue_home = tempfile::TempDir::new().unwrap();
        let queue = spawn_test_queue(queue_home.path());
    ...
    fn make_queue_backed_handle_with(
        ...
        let auth: cf_computer_hub_sdk::SharedAuthProvider = Arc::new(
            cf_computer_hub_sdk::auth::AuthCredential::bearer("test-token"),
        );
        ...
        let source: Arc<dyn cf_file_utils::queue::TraceExportSource> =
            Arc::new(crate::upload::WorkspaceTraceExportSource::new(proxy));
        let policy = cf_file_utils::queue::UploadRetryPolicy {
            max_attempts: 1,
            ..Default::default()
        };
        let queue = Arc::new(cf_file_utils::queue::UploadQueue::spawn(
            home.path(),
            source,
            policy,
        ));
    ...
    async fn environment_artifact_enqueued_when_queue_present() {
        ...
        assert!(
            matches!(
                outcome,
                Some(cf_file_utils::queue::EnqueueOutcome::Enqueued)
            ),
    ...
    async fn fork_session_cwd_override_used_when_set() {
        let handle = make_handle();
        let alt = std::env::temp_dir().join("qidi-code-test-cwd-override");
    ...
    async fn hook_registry_snapshot_is_disconnected() {
        ...
        {
            let spec = cf_hooks::config::HookSpec {
                name: "injected".into(),
                event: cf_hooks::event::HookEventName::SessionStart,
    ...
    fn workspace_shared_auth_provider_uses_workspace_config() {
        ...
        let service_auth: cf_computer_hub_sdk::SharedAuthProvider = Arc::new(
            cf_computer_hub_sdk::auth::AuthCredential::bearer("xai-service-token"),
        );
        let hub_auth: cf_computer_hub_sdk::SharedAuthProvider = Arc::new(
            cf_computer_hub_sdk::auth::AuthCredential::bearer("hub-token"),
        );
        let hub_cfg = crate::hub::HubConfig {
    ...
    async fn fork_session_inherits_viewer_ctx_from_parent() {
        ...
        let parent = handle
            .create_session_with_tracker_and_viewer_ctx(
                ...
                cf_hunk_tracker::HunkTrackerHandle::noop(),
                None,
                CapabilityMode::All,
                Some(cf_tool_runtime::WorkspaceViewerContext {
    ...
    fn handler_names(resolved: &cf_computer_hub_sdk::ResolvedSessionHandlers) -> Vec<String> {
        resolved
            .handlers
            .iter()
            .map(|h| h.tool_id().as_str().to_owned())
    ...
    async fn strict_bind_without_explicit_toolset_fails_closed_end_to_end() {
        ...
        let resolved = resolver(
            cf_tool_protocol::SessionId::new("bind-e2e-strict").unwrap(),
            Some(serde_json::json!(
                { "metadata" : { "preset" : "cf-tools-computer", "capability_mode" :
                "all" }, }
            )),
        )
        .await
        ...
        assert!(
            reason.contains(cf_version::VERSION),
    ...
    async fn strict_rpc_only_bind_fails_closed_with_resolve_error_end_to_end() {
        ...
        let resolver = bind_resolver_fixture(&handle);
        let resolved = resolver(
            cf_tool_protocol::SessionId::new("bind-e2e-rpc-only").unwrap(),
            Some(serde_json::json!(
    ...
    async fn lax_bind_without_metadata_uses_default_catalog_end_to_end() {
        ...
        let resolver = bind_resolver_fixture(&handle);
        let resolved = resolver(
            cf_tool_protocol::SessionId::new("bind-e2e-lax").unwrap(),
            None,
        )
        .await
        .expect("bind must succeed");
    ...
    async fn rejected_rebind_config_keeps_resolve_error_end_to_end() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = cf_tool_protocol::SessionId::new("bind-e2e-rejected").unwrap();
        let first = resolver(
            sid.clone(),
            Some(serde_json::json!(
                { "metadata" : { "tools" : [{ "id" : "cf_tools::read_file" }] },
    ...
    async fn explicit_empty_toolset_rebind_never_swaps_session_tools() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = cf_tool_protocol::SessionId::new("bind-e2e-rpc-only").unwrap();
        let first = resolver(
            sid.clone(),
            Some(serde_json::json!(
                { "metadata" : { "tools" : [{ "id" : "cf_tools::read_file" }] },
    ...
    async fn strict_rebind_with_corrected_toolset_heals_end_to_end() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = cf_tool_protocol::SessionId::new("bind-e2e-heal").unwrap();
        let first = resolver(
            sid.clone(),
            Some(serde_json::json!({ "metadata" : { "preset" : "cf-tools-computer" } })),
        )
        .await
    ...
    async fn owner_toolset_survives_concurrent_consumer_shaped_rebinds() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = cf_tool_protocol::SessionId::new("bind-e2e-consumer-storm").unwrap();
    ...
    async fn restored_server_first_bind_ordering_decides_capability_and_toolset() {
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = cf_tool_protocol::SessionId::new("bind-e2e-restore-read-first").unwrap();
        ...
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = cf_tool_protocol::SessionId::new("bind-e2e-restore-write-first").unwrap();
        resolver(
            sid.clone(),
            Some(serde_json::json!(
        ...
        let handle = make_strict_handle();
        let resolver = bind_resolver_fixture(&handle);
        let sid = cf_tool_protocol::SessionId::new("bind-e2e-restore-owner-first").unwrap();
    ...
    async fn compute_turn_injections_after_returns_skipped_ack_without_queue() {
        use cf_tool_protocol::turn_hook::{AfterTurnPayload, TurnHookOutcome, TurnHookRequest};
        let handle = make_handle();
    ...
    async fn compute_turn_injections_before_runs_turn_start_and_replies_noop() {
        use cf_tool_protocol::turn_hook::{BeforeTurnPayload, HookReply, TurnHookRequest};
        let handle = make_handle();
        let reply = handle
            .compute_turn_injections(
                "main",
    ...
    /// Unique skill names: discovery also reads the dev machine's `~/.qidi`.
    #[tokio::test]
    ...
    fn make_handle_with_queue_routing(
        ...
        WorkspaceHandle,
        Arc<cf_file_utils::queue::UploadQueue>,
        tempfile::TempDir,
    ) {
        use cf_computer_hub_sdk::auth::{AuthCredential, AuthProvider};
        let factory = Arc::new(TestSessionContextFactory::new());
        let cwd = factory.temp.path().to_path_buf();
        let config = WorkspaceConfig {
            root_cwd: cwd,
    ...
    async fn wait_enqueued(queue: &cf_file_utils::queue::UploadQueue, want: u64) {
    ...
    async fn enqueue_workspace_tool_definitions_reports_enqueued_at_session_root() {
        ...
        assert_eq!(outcome, cf_file_utils::queue::EnqueueOutcome::Enqueued);
        assert_eq!(
            queue
                .stats()
                .enqueued
    ...
    async fn two_phase_drain_no_queue_marks_draining_and_returns_zero() {
        ...
        assert_eq!(
            snap.status,
            cf_tool_protocol::ToolServerLifecycleStatus::Draining
        );
        assert!(
