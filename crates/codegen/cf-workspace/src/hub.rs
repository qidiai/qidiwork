...
impl HubHandle {
    ...
    pub(crate) async fn connect(
        ...
        session_handler_resolver: Option<cf_computer_hub_sdk::SessionHandlerResolver>,
        ...
        let mut server_builder = ToolServerBuilder::default()
            ...
            .binary_version(cf_version::VERSION)
...
struct CallCompletedGuard {
    ...
    outcome: cf_file_utils::events::ToolOutcome,
}
impl CallCompletedGuard {
    fn new(
        ...
        Self {
            ...
            call_id,
            session_id,
            outcome: cf_file_utils::events::ToolOutcome::Cancelled,
        }
    }
    fn set_outcome(&mut self, outcome: cf_file_utils::events::ToolOutcome) {
...
impl ToolServerHandler for SessionRoutedToolHandler {
    ...
    async fn handle_call(&self, ctx: ToolCallContext, args: Value) -> ToolStream<TypedToolOutput> {
        ...
        let hub_session = ctx
            .extensions
            .get::<cf_tool_runtime::SessionContext>()
            .map(|s| s.0.clone());
            ...
            match transport {
                ...
                None => {
                    tracing::warn!(
                        ...
                        "QIDI_HITL_PERMISSION_LIVE set but no hub ToolServer; rejecting guarded tool"
                    );
                    return terminal_only(Err(ToolError::new(
                        ToolErrorKind::PermissionDenied,
        ...
        Box::pin(async_stream::stream! {
            ...
            .set_outcome(cf_file_utils::events::ToolOutcome::Success); yield
            ...
            .set_outcome(cf_file_utils::events::ToolOutcome::Error); yield
...
fn parse_server_id(id: &str) -> Result<cf_tool_protocol::ServerId, ClientError> {
    cf_tool_protocol::ServerId::new(id)
...
mod tests {
    ...
    use cf_tool_runtime::{SessionContext, ToolCallId};
    ...
    async fn handle_call_terminal_matches_non_streaming_call() {
        ...
        match reference {
            Ok(run_result) => {
                ...
                assert_eq!(
                    typed.model_output,
                    vec![cf_tool_runtime::ContentBlock::Text {
    ...
    use cf_tools::types::tool_metadata::ToolMetadata as XaiToolMetadata;
    #[derive(Debug)]
    ...
    impl cf_tool_runtime::Tool for GateStreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> cf_tool_protocol::ToolId {
            cf_tool_protocol::ToolId::new("gate_streaming_stub").expect("valid tool id")
        }
        fn description(
            &self,
            _ctx: &::cf_tool_runtime::ListToolsContext,
        ) -> cf_tool_types::ToolDescription {
            cf_tool_types::ToolDescription::new("gate_streaming_stub", "gate streaming stub")
        }
        async fn run(
            &self,
        ...
        async fn execute(
            ...
            use cf_tool_runtime::{ToolProgress, ToolStreamItem};
    ...
    async fn drain_counts<T>(mut stream: cf_tool_runtime::ToolStream<T>) -> (usize, usize, bool) {
        let mut progress = 0;
        let mut terminal = 0;
    ...
    async fn wait_until(
        ...
        pred: impl Fn(&cf_tool_protocol::ToolServerStatusPayload) -> bool,
        timeout: Duration,
    ) -> cf_tool_protocol::ToolServerStatusPayload {
    ...
    fn bg_started_notif(task_id: &str) -> ToolNotification {
        use cf_tools::notification::types::{
    ...
    fn task_completed_notif(task_id: &str) -> ToolNotification {
        use cf_tools::computer::types::{TaskKind, TaskSnapshot};
