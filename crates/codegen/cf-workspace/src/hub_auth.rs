//! Hub [`AuthProvider`] from `~/.qidi/auth.json` for the standalone
...
fn default_auth_path() -> anyhow::Result<PathBuf> {
    let grok = cf_config::user_grok_home()
        .ok_or_else(|| anyhow::anyhow!("no user grok home (set $QIDI_HOME or $HOME)"))?;
...
/// the default credential path (`~/.qidi/auth.json`).
pub fn provider(
    hub_url: &Url,
