...
use cf_tool_protocol::{HookEvent, HookFrame, SessionId, ToolId, ToolServerEvictParams};
use cf_tool_runtime::{
...
) -> Vec<cf_workspace_types::rpc::workspace::BackgroundTaskSummaryWire> {
    use cf_tools::computer::types::TaskKind;
    use cf_tools::types::resources::Terminal;
    use cf_tools::types::tool::ToolKind;
    use cf_workspace_types::rpc::workspace::BackgroundTaskSummaryWire;
    let terminal = {
...
impl WorkspaceRpcHandler {
    ...
    async fn dispatch(
        ...
        use cf_workspace_types::rpc::git::{GitBranchInfoReq, GitMetadataReq};
        use cf_workspace_types::rpc::search::FuzzyStatusReq;
        use cf_workspace_types::rpc::skills::DiscoverPluginsReq;
        ...
        use cf_workspace_types::rpc::worktree::WorktreeCreateSyncReq;
...
impl ToolServerHandler for WorkspaceRpcHandler {
    ...
    async fn handle_call(&self, ctx: ToolCallContext, args: Value) -> ToolStream<TypedToolOutput> {
        ...
        let bound_session = ctx.extensions.get::<cf_tool_runtime::SessionContext>();
    ...
    async fn handle_hook(&self, session_id: SessionId, frame: HookFrame) {
        match frame.event {
            ...
            HookEvent::Custom { kind, payload } => {
                use cf_tool_protocol::turn_hook::{
    ...
    async fn handle_hook_request(&self, session_id: SessionId, frame: HookFrame) -> Option<Value> {
        use cf_tool_protocol::turn_hook::{self, TurnHookRequest};
...
mod tests {
    ...
    ) -> Option<cf_tool_runtime::ToolStreamItem<TypedToolOutput>> {
    ...
    async fn handle_evict_triggers_two_phase_drain() {
        use cf_tool_protocol::ToolServerLifecycleStatus;
        let handle = make_handle();
    ...
    async fn list_background_tasks_rpc_stays_truthful_across_rebinds() {
        ...
        use cf_workspace_types::rpc::workspace::ListBackgroundTasksResponse;
        let handle = make_handle();
    ...
    async fn handle_call_wraps_in_envelope_with_value() {
        ...
        ctx.extensions
            .insert(cf_tool_runtime::SessionContext("main".to_owned()));
        ...
        match item {
            cf_tool_runtime::ToolStreamItem::Terminal(Ok(typed)) => {
                let ok_val = typed
                    .value
                    .get("ok")
    ...
    async fn handle_hook_cancel_marks_call_completed() {
        use cf_tool_protocol::ToolCallId;
        let handle = make_handle();
        let handler = WorkspaceRpcHandler::new(handle.clone());
        let tracker = handle.activity_tracker();
