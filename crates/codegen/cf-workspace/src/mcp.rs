...
use cf_computer_hub_mcp_adapter::{
...
use cf_mcp::rmcp;
use cf_mcp::servers::McpClient;
use cf_tool_protocol::ToolId;
use cf_tool_runtime::{ToolCallContext, ToolStream, TypedToolOutput};
...
impl McpTransport for McpClientTransportAdapter {
    async fn initialize(&self) -> Result<McpServerInfo, cf_computer_hub_mcp_adapter::McpError> {
        let service = self
            .client
            .ensure_initialized()
            .await
            .map_err(|e| cf_computer_hub_mcp_adapter::McpError::Transport(e.to_string()))?;
        let info = service.peer_info().ok_or_else(|| {
            cf_computer_hub_mcp_adapter::McpError::Transport("no peer info after init".into())
        })?;
        Ok(McpServerInfo {
    ...
    async fn list_tools(
        &self,
    ) -> Result<Vec<McpToolDefinition>, cf_computer_hub_mcp_adapter::McpError> {
        let service = self
            .client
            .ensure_initialized()
            .await
    ...
    async fn call_tool(
        ...
        name: &str,
        arguments: Value,
    ) -> Result<McpCallResult, cf_computer_hub_mcp_adapter::McpError> {
        let service = self
            .client
            .ensure_initialized()
            .await
    ...
    async fn close(&self) -> Result<(), cf_computer_hub_mcp_adapter::McpError> {
...
/// Extract a server name from an [`McpError`](cf_mcp::servers::McpError),
/// falling back to `"unknown"`.
pub(crate) fn server_name_from_mcp_error(e: &cf_mcp::servers::McpError) -> &str {
    e.server_name().unwrap_or("unknown")
}

/// Bridge config factory for MCP bridge connections.
pub(crate) fn make_bridge_config(
    session_id: cf_tool_protocol::SessionId,
    server_name: &str,
) -> McpBridgeConfig {
    McpBridgeConfig {
        session_id,
