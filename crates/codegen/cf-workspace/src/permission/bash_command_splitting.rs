...
///
/// ```bash
/// XAI_API_KEY='xai-some-key' cargo run --bin qidi-code
/// ```
///
...
mod tests {
    ...
    fn test_parse_plain_commands_from_script() {
        ...
        let environment_key_command = "XAI_API_KEY='xai-some-key' cargo run --bin qidi-code";
        assert_eq!(
        ...
        let redirection_command = "cargo build --bin qidi-code 2>&1";
        assert_eq!(
        ...
        let another_long_command = "cargo test --package qidi-code --lib -- permission::bash_command_splitting::tests::test_parse_plain_commands_from_script --exact --nocapture 2>&1";
        assert_eq!(
