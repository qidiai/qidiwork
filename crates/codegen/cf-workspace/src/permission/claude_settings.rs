...
// =============================================================================
//
// `qidi-code::claude_import` writes the marker. We re-implement a small
...
pub fn is_claude_import_marked() -> bool {
    ...
    // lives in qidi-code (inaccessible from here at runtime). They also
    // set this env var so the workspace-resident gate honours the override
    // without a cross-crate dependency.
    if std::env::var("_QIDI_CLAUDE_MARKER_OVERRIDE").as_deref() == Ok("1") {
        return true;
    }
    let Some(config_path) = cf_config::user_grok_home().map(|g| g.join("config.toml")) else {
        return false;
    };
