...
use cf_computer_hub_sdk::harness::PERMISSION_REQUEST_KIND;
use cf_computer_hub_sdk::{ToolServer, WeakToolServer};
...
pub const HITL_PERMISSION_LIVE_ENV: &str = "QIDI_HITL_PERMISSION_LIVE";
...
pub fn access_kind_for_hub_tool(tool_name: &str, args: &Value) -> Option<AccessKind> {
    ...
    let name = name.strip_prefix("cf_tools:").unwrap_or(name);
    match name {
