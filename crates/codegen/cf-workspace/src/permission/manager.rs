...
fn permission_mode_artifact_str(mode: cf_telemetry::enums::PermissionMode) -> &'static str {
    use cf_telemetry::enums::PermissionMode;
    match mode {
...
fn spawn_permission_manager_with_pin(
    ...
    let _task = tokio::task::spawn_local(async move {
        ...
        // persist it to ~/.qidi/sessions/<cwd>/permission.toml. This caused
            ...
            match cmd {
                ...
                PermissionCommand::Request {
                    ...
                    if matches!(&access, AccessKind::Bash(_))
                        && cf_sandbox::should_auto_allow_bash()
