//! Permission resolution engine: merges native `.qidi/config.toml`,
...
/// user `~/.qidi` → `Requirements`, so [`is_admin_source`] trusts only the root tier.
fn load_requirements_permissions() -> Vec<Sourced<PermissionRule>> {
    cf_config::requirements_layers()
        .into_iter()
        .flat_map(|layer| {
...
/// Find every `<dir>/.qidi/config.toml` from `cwd` upward to the git repo
/// root (or just `<cwd>/.qidi/config.toml` when there is no git repo).
///
/// Returned paths are ordered from repo root (lowest priority) to `cwd`
/// (highest priority), matching `qidi-code::config::find_project_configs`.
...
///
///   * `~/.qidi/config.toml` (lowest priority)
///   * Each `.qidi/config.toml` from the git repo root down to `cwd`
...
fn load_config_toml_permissions(cwd: &Path) -> Vec<Sourced<PermissionRule>> {
    ...
    // Global `~/.qidi/config.toml` first (lowest priority within this layer).
    // Gated on user_grok_home() so a project's .qidi/config.toml is never read as
    // global permissions when neither QIDI_HOME nor a home dir resolves.
    if let Some(global_path) = cf_config::user_grok_home().map(|g| g.join("config.toml"))
        && global_path.is_file()
    {
        match cf_config::load_config_file(&global_path) {
    ...
    for path in find_project_grok_configs(cwd) {
        match cf_config::load_config_file(&path) {
            Ok(value) => rules.extend(extract_toml_permissions(&value, || {
...
fn managed_config_permissions(
    layers: &[cf_config::ManagedConfigLayer],
) -> Vec<Sourced<PermissionRule>> {
    layers
        .iter()
        .flat_map(|layer| {
    ...
    fn live() -> Self {
        Self {
            ...
            managed_config_rules: managed_config_permissions(
                &cf_config::managed_config_layers(),
            ),
        }
    }
}
...
/// [`cf_config::claude_managed_settings_path`] (platform path such as
...
fn load_managed_settings() -> ManagedSettings {
    let Some(path) = cf_config::claude_managed_settings_path() else {
...
pub fn yolo_disabled_by_policy() -> Option<&'static str> {
    let layers = cf_config::requirements_layers();
...
/// without `~/.qidi`); `path` only names the layer in a non-bool warning.
...
mod tests {
    ...
    // Shared so `QIDI_HOME`/`HOME` mutations here also serialize against the
    ...
    fn load_claude_env_merges_with_precedence() {
        // QIDI_HOME-isolate so the claude-import marker reads clean (an imported
        ...
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home_guard = EnvVarGuard::set("QIDI_HOME", home.path());
        let _marker_guard = EnvVarGuard::unset("_QIDI_CLAUDE_MARKER_OVERRIDE");
        let tmp = tempfile::tempdir().unwrap();
        let claude_dir = tmp.path().join(".claude");
        std::fs::create_dir_all(&claude_dir).unwrap();
    ...
    fn load_claude_env_with_project_drops_repo_env_when_untrusted() {
        ...
        // drop it. Isolate QIDI_HOME so the claude-import marker reads clean (an
    ...
    fn admin_source_trusts_only_root_owned_tiers() {
        ...
        // the user-writable `~/.qidi/requirements.toml` is not, despite its path.
        ...
        assert!(!is_admin_source(&RequirementSource::Requirements {
            path: "/home/u/.qidi/requirements.toml".into(),
        }));
    ...
    async fn managed_config_toml_rules_resolve_as_non_admin_defaults() {
        ...
        let layers =
            cf_config::managed_config_layers_at(Some(system.path()), Some(user.path()));
        ...
        .unwrap();
        assert_eq!(
            cf_config::managed_config_layers_at(Some(system.path()), Some(user.path())).len(),
            1
        );
