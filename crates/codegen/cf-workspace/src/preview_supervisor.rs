...
//! spawns the unchanged `/usr/local/bin/qidi-code-proxy` binary as a
...
pub const PREVIEW_PROXY_BIN_PATH: &str = "/usr/local/bin/qidi-code-proxy";
...
impl PreviewArgs {
    ...
    /// `qidi-code-proxy/src/cli.rs`). Absent options and a false
...
/// Build the unspawned proxy command. Secrets (`QIDI_SERVER_KEY` /
/// `QIDI_SESSION_ID`) reach the proxy by env inheritance — never argv.
fn build_preview_command(cfg: &PreviewArgs) -> io::Result<tokio::process::Command> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // Raw pre_exec, NOT cf_tty_utils::detach_command: the proxy must stay in
...
/// `qidi-code-proxy`'s `/__control/activity` route).
...
/// the default in `qidi-code-proxy/src/cli.rs`).
...
mod tests {
    ...
    fn to_argv_maps_every_flag_to_the_proxy_cli_names() {
        // Flag names must match qidi-code-proxy/src/cli.rs exactly.
        assert_eq!(
            sample_cfg().to_argv(),
            vec![
                "--preview-port",
    ...
    #[cfg(target_os = "linux")]
    const PDEATHSIG_HELPER_ENV: &str = "QIDI_PDEATHSIG_HELPER";
    #[cfg(target_os = "linux")]
