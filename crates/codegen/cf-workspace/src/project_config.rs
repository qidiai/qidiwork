...
//! `.qidi/config.toml` files by walking from `cwd` up to the git root.
//!
...
use cf_agent::repo::RepoDirChain;
...
/// True when `config_path` is `$QIDI_HOME/config.toml` (user tier, not project).
fn is_user_grok_config_file(config_path: &Path) -> bool {
    let Some(user_home) = cf_config::user_grok_home() else {
        return false;
    };
...
/// Find all `.qidi/config.toml` files from `cwd` upward to the git repo root.
...
///
/// If no git repo is found, only checks `cwd/.qidi/config.toml`. Excludes the
/// user-global config so `cwd == $HOME` does not treat `~/.qidi/config.toml` as
/// a project overlay.
...
/// `cwd == $HOME` does not treat `~/.qidi/config.toml` as a project overlay.
/// `pub(crate)` — the gate (`repo_configs_present`) reaches it within this crate.
