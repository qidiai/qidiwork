...
pub async fn run_startup_recovery(workspace_home: &Path, queue: &UploadQueue) -> RecoveryReport {
    ...
    for sidecar_path in sidecars {
        ...
        let actual_sha = cf_file_utils::sha256_hex(&bytes);
...
mod tests {
    ...
    use cf_file_utils::queue::{TraceExportSource, UploadRetryPolicy, sidecar_path_for};
    ...
    fn write_orphan_pair(
        ...
        let sidecar = QueueItemSidecar {
            ...
            enqueued_at,
            sha256: cf_file_utils::sha256_hex(content),
        };
