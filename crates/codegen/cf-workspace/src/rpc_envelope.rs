...
//! (the envelope types are canonical in `cf_workspace_types::rpc`).
//!
//! `error_code` uses a non-wildcard match so the compiler enforces
//! coverage of new `WorkspaceError` variants.

pub use cf_workspace_types::rpc::{RpcEnvelope, RpcError};
...
pub fn error_code(err: &WorkspaceError) -> &'static str {
    match err {
        ...
        WorkspaceError::TurnActive(_) => cf_workspace_types::rpc::envelope::TURN_ACTIVE,
...
pub fn rpc_error_to_workspace(err: RpcError) -> WorkspaceError {
    if let Some(kind) =
        cf_workspace_types::rpc::deploy::DeployError::from_wire_code(&err.code)
    {
...
mod tests {
    ...
    fn error_code_round_trip_all_variants() {
        ...
        variants.extend(
            cf_workspace_types::rpc::deploy::DeployError::ALL
                .into_iter()
