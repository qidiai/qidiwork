...
use std::sync::Arc;
use cf_computer_hub_mcp_adapter::McpBridgeHandle;
use cf_mcp::servers::McpState;
use cf_tools::notification::types::{ToolNotification, ToolNotificationHandle};
use cf_tools::registry::types::{FinalizedToolset, ToolConfig, ToolServerConfig};
use cf_hunk_tracker::HunkTrackerHandle;
use cf_tool_protocol::ToolId;
use cf_tool_runtime::WorkspaceViewerContext;
...
pub struct WorkspaceSession {
    ...
    pub(crate) hunk_checkpoints:
        Arc<tokio::sync::Mutex<HashMap<usize, cf_hunk_tracker::HunkTurnDelta>>>,
...
impl WorkspaceSession {
    ...
    pub(crate) async fn toolset_terminal_is_session_owned(&self) -> bool {
        ...
        match res.get::<cf_tools::types::resources::Terminal>() {
...
pub struct WorkspaceShared {
    ...
    pub(crate) events: tokio::sync::broadcast::Sender<cf_workspace_types::WorkspaceEvent>,
    pub(crate) respect_gitignore: bool,
    pub(crate) memory_config: Option<MemoryConfig>,
    pub(crate) hook_registry: Arc<parking_lot::RwLock<cf_hooks::discovery::HookRegistry>>,
    pub(crate) hook_load_errors: Vec<cf_hooks::error::HookError>,
    ...
    pub(crate) auth_provider: Option<cf_computer_hub_sdk::SharedAuthProvider>,
    ...
    pub(crate) activity_notify_handle:
        arc_swap::ArcSwap<Option<cf_tools::notification::types::ToolNotificationHandle>>,
    ...
    pub(crate) local_registry: cf_computer_hub_sdk::LocalRegistry,
    ...
    pub(crate) lsp: Option<std::sync::Arc<dyn cf_tools::implementations::lsp::LspBackend>>,
    ...
    /// (from `QIDI_WORKSPACE_REWIND_ALL_OUTCOMES`, default off).
    pub(crate) workspace_rewind_all_outcomes: bool,
    /// Resolved `$QIDI_WORKSPACE_HOME` — the workspace-owned on-disk state root
    /// (`<grok_home>/workspace` by default). The upload queue spills here.
    pub(crate) workspace_home: std::path::PathBuf,
    pub(crate) upload_queue: Option<std::sync::Arc<cf_file_utils::queue::UploadQueue>>,
    ...
    /// (`QIDI_WORKSPACE_EVENTS_ENABLED=true`). When `false`, every
    /// [`session_event_writer`](Self::session_event_writer) hands back an
    /// [`EventWriter::noop()`](cf_file_utils::events::EventWriter::noop) and
    ...
    /// enabled (`QIDI_WORKSPACE_TOOL_DEFS_ENABLED=true`).
    ...
    pub(crate) session_event_writers:
        Arc<dashmap::DashMap<String, cf_file_utils::events::EventWriter>>,
    ...
    pub(crate) inflight_enqueues: dashmap::DashMap<
        (String, u64),
        tokio::task::JoinHandle<cf_file_utils::queue::EnqueueOutcome>,
    >,
...
impl WorkspaceShared {
    ...
    pub fn upload_queue(&self) -> Option<&std::sync::Arc<cf_file_utils::queue::UploadQueue>> {
    ...
    /// [`EventWriter::noop()`](cf_file_utils::events::EventWriter::noop)
    ...
    pub(crate) fn session_event_writer(
        &self,
        session_id: &str,
    ) -> cf_file_utils::events::EventWriter {
    ...
    pub(crate) fn session_event_writer_cached(
        &self,
        session_id: &str,
    ) -> Option<cf_file_utils::events::EventWriter> {
    ...
    pub fn auth_provider(&self) -> Option<&cf_computer_hub_sdk::SharedAuthProvider> {
    ...
    ///
    /// Returns a clone of the [`ToolServer`](cf_computer_hub_sdk::ToolServer)
    ...
    pub fn subscribe_events(
        &self,
    ) -> tokio::sync::broadcast::Receiver<cf_workspace_types::WorkspaceEvent> {
...
pub(crate) fn get_or_open_session_writer(
    enabled: bool,
    writers: &dashmap::DashMap<String, cf_file_utils::events::EventWriter>,
    workspace_home: &Path,
    session_id: &str,
) -> cf_file_utils::events::EventWriter {
    use cf_file_utils::events::EventWriter;
    if !enabled {
...
mod tests {
    ...
    use dashmap::DashMap;
    use cf_file_utils::events::{Event, EventWriter};
