...
pub(crate) fn resolve_session_toolset(
    ...
    local_registry: Option<cf_computer_hub_sdk::LocalRegistry>,
    ...
    notification_handle: Option<cf_tools::notification::types::ToolNotificationHandle>,
) -> WorkspaceResult<(
    ToolServerConfig,
...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    viewer_ctx: Option<cf_tool_runtime::WorkspaceViewerContext>,
    notification_handle: Option<cf_tools::notification::types::ToolNotificationHandle>,
    terminal_backend: Arc<dyn cf_tools::computer::types::TerminalBackend>,
    ...
    let toolset = builder
        .finalize_with_trunc_config(
            finalize_config,
            ctx,
            cf_tools::types::context::TruncationConfig::default(),
            viewer_ctx,
        )
        .map_err(|errs| {
...
/// enabled (`QIDI_WORKSPACE_TOOL_STATE_ENABLED=true`; any other value keeps
/// legacy behavior).
pub fn tool_state_enabled() -> bool {
    std::env::var("QIDI_WORKSPACE_TOOL_STATE_ENABLED").as_deref() == Ok("true")
}
...
fn sanitize_session_id(session_id: &str) -> String {
    ...
    if modified {
        let digest = cf_file_utils::sha256_hex(session_id.as_bytes());
        safe.push('-');
...
/// `QIDI_WORKSPACE_TOOL_STATE_ENABLED`. Aliased to the crate-wide
...
pub struct WorkspaceSessionContextFactory {
    auth: Option<cf_computer_hub_sdk::SharedAuthProvider>,
    api_base_url: Option<String>,
    /// Resolved `$QIDI_WORKSPACE_HOME` when tool-state persistence is enabled;
...
impl WorkspaceSessionContextFactory {
    ...
    pub fn with_auth(auth: cf_computer_hub_sdk::SharedAuthProvider, api_base_url: String) -> Self {
        Self {
            auth: Some(auth),
    ...
    /// (`$QIDI_WORKSPACE_HOME`). Callers should only invoke this when
    ...
    fn build_session_context(
        ...
        let (image_gen_config, video_gen_config, web_search_config, app_builder_deployer_config) =
            if let (Some(auth), Some(url)) = (&self.auth, &self.api_base_url) {
                ...
                match cred {
                    cf_computer_hub_sdk::AuthCredential::Bearer { token, .. } => {
...
fn build_proxy_headers(base_url: &str) -> indexmap::IndexMap<String, String> {
    ...
    let version = cf_version::VERSION;
    headers.insert(
        "user-agent".to_string(),
        format!("qidi-code/{version}"),
    );
    ...
    if base_url.contains("cli-chat-proxy") || base_url.contains("chat-proxy") {
        headers.insert("X-XAI-Token-Auth".to_string(), "cli".to_string());
        headers.insert(
...
fn build_web_fetch_config() -> cf_tools::implementations::qidi_build::web_fetch::WebFetchConfig
{
    ...
    if let Ok(proxy) = std::env::var("QIDI_WEB_FETCH_PROXY") {
...
mod tests {
    ...
    fn tool_state_enabled_only_true_enables() {
        ...
        let var = "QIDI_WORKSPACE_TOOL_STATE_ENABLED";
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use cf_tools::types::resources::{State, WebCitationCounter};
