...
//!
//! Persists per-folder trust decisions to `~/.qidi/trusted_folders.toml`.
...
//!
//! The store is rooted at [`cf_config::user_grok_home`] — the **Option**
//! home that resolves to `None` (rather than a cwd-relative `./.grok`) when
//! neither `$QIDI_HOME` nor a home directory is set (e.g. a minimal container /
...
//! a `./.qidi/trusted_folders.toml` that self-trusts its own checkout (fail
//! closed).
...
/// Filename of the folder-trust store under `~/.qidi/`.
...
impl TrustStore {
    ...
    ///
    /// Resolves via [`cf_config::user_grok_home`], never
    /// [`cf_config::grok_home`], so it never falls back to a cwd-relative
    ...
    pub fn default_path() -> Option<PathBuf> {
        Self::default_path_in(cf_config::user_grok_home())
    }
...
/// ROOT (via the `~/.qidi/worktrees.db` registry), so every `grok -w` worktree
...
/// sharing `~/.qidi/`) across the whole read-modify-write so updates merge
...
///
/// The legacy `~/.qidi/trusted-hook-projects` file listed one canonical project
...
pub fn migrate_legacy_hook_trust() {
    ...
    MIGRATED.call_once(|| {
        let Some(legacy_file) = cf_hooks::trust::legacy_trust_file_path() else {
            return;
        };
...
fn migrate_legacy_hook_trust_in(legacy_file: &Path, store: &mut TrustStore) -> usize {
    ...
    let projects = match cf_hooks::trust::list_trusted_projects_with_file(legacy_file) {
        Ok(p) => p,
        Err(e) => {
            tracing::warn!(
...
mod tests {
    ...
    fn default_path_in_maps_home_and_preserves_no_home() {
        ...
        // how a cloned repo's own `<repo>/.qidi/trusted_folders.toml` could
        // masquerade as the user-global store and self-trust the checkout.
    ...
    fn default_path_sources_from_user_grok_home() {
        ...
        assert_eq!(
            ...
            cf_config::user_grok_home().map(|h| h.join(TRUST_FILE_NAME))
        );
    }
    ...
    /// Point `QIDI_HOME` at an isolated tempdir and register one grok-managed
    ...
    /// dir)`; the [`LockedTestEnv`] holds the lock and restores `QIDI_HOME` on
    ...
    fn register_grok_worktree(
        ...
        use cf_fast_worktree::{WorktreeDb, WorktreeKind, WorktreeRecord, WorktreeStatus};
