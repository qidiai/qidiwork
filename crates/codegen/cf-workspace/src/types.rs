...
//! `qidi-code`. Wire types live in
//! `cf_workspace_types::rpc`; this crate adds the connected-state
...
use cf_computer_hub_sdk::harness::ToolHarness;
use cf_workspace_types::rpc::agents_md::{AgentConfigFile, DiscoverAgentsMdReq};
use cf_workspace_types::rpc::code_nav::{
...
use cf_workspace_types::rpc::git::{
...
use cf_workspace_types::rpc::hunks::{
...
use cf_workspace_types::rpc::search::{
    ContentSearchData, ContentSearchRequest, FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq,
    FuzzyStatusReq,
};
use cf_workspace_types::rpc::session::{
    BeginPromptReq, EndPromptReq, FileRewindResponse, RewindToReq,
};
use cf_workspace_types::rpc::skills::{DiscoverPluginsReq, DiscoverSkillsReq, SkillInfo};
use cf_workspace_types::rpc::workspace::{
...
use cf_workspace_types::rpc::worktree::{
...
use cf_workspace_types::rpc::{RpcEnvelope, RpcError, WORKSPACE_RPC_TOOL_ID, WorkspaceRpc};
use cf_tool_runtime::{ToolCallContext, ToolStreamItem, TypedToolOutput};
...
pub async fn consume_stream_terminal(
    stream: &mut cf_tool_runtime::ToolStream<TypedToolOutput>,
) -> Result<TypedToolOutput, cf_tool_runtime::ToolError> {
    loop {
...
/// Check whether a [`ToolError`](cf_tool_runtime::ToolError) indicates
...
pub fn is_transport_fatal(err: &cf_tool_runtime::ToolError) -> bool {
    match err.kind {
        cf_tool_runtime::ToolErrorKind::NetworkError => true,
        cf_tool_runtime::ToolErrorKind::Custom => err
            .details
            .as_ref()
...
impl WorkspaceClient {
    ...
    pub async fn rpc_raw(
        ...
        let tool_id = cf_tool_protocol::ToolId::new(WORKSPACE_RPC_TOOL_ID)
...
mod tests {
    ...
    use cf_computer_hub_sdk::harness::LocalRegistry;
    use cf_workspace_types::rpc::skills::SkillScope;
    use cf_tool_protocol::{SessionId, ToolId};
    use cf_tool_runtime::{Tool, ToolError};
    ...
    impl cf_tool_runtime::ToolOutput for RawOut {}
    #[derive(Debug)]
    ...
    impl Tool for FakeWorkspaceRpc {
        ...
        fn description(&self, _ctx: &::cf_tool_runtime::ListToolsContext) -> ToolDescription {
        ...
        async fn run(&self, _ctx: ToolCallContext, args: Self::Args) -> Result<RawOut, ToolError> {
            ...
            match args.method.as_str() {
                ...
                "workspace.discover_skills" => ok(serde_json::json!(
                    ...
                    "path" : "/workspace/.qidi/skills/my-skill/SKILL.md", "scope"
                    : "local", }]
                )),
    ...
    async fn consume_stream_terminal_returns_ok() {
        ...
        let mut stream = cf_tool_runtime::terminal_only(Ok(typed));
        assert_eq!(
    ...
    async fn consume_stream_terminal_returns_err() {
        let mut stream: cf_tool_runtime::ToolStream<TypedToolOutput> =
            cf_tool_runtime::terminal_only(Err(ToolError::network_error("oops")));
    ...
    async fn consume_stream_terminal_exhausted_stream_is_network_error() {
        ...
        let mut stream = cf_tool_runtime::terminal_only::<TypedToolOutput>(Ok(typed));
