...
use cf_computer_hub_sdk::ToolHarness;
use cf_tools::types::output::ToolRunResult;
pub use cf_workspace_types::rpc::WorkspaceRpc;
pub use cf_workspace_types::rpc::agents_md::DiscoverAgentsMdReq;
pub use cf_workspace_types::rpc::code_nav::{
...
pub use cf_workspace_types::rpc::fs::{
...
pub use cf_workspace_types::rpc::git::{
...
pub use cf_workspace_types::rpc::hooks::{
    HookEventNameWire, HookRegistryReq, HookRegistryWire, HookSpecWire,
};
pub use cf_workspace_types::rpc::hunks::{
...
pub use cf_workspace_types::rpc::search::{FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq};
pub use cf_workspace_types::rpc::session::{BeginPromptReq, EndPromptReq, RewindToReq};
pub use cf_workspace_types::rpc::skills::DiscoverSkillsReq;
pub use cf_workspace_types::rpc::workspace::WorkspaceInfoReq;
pub use cf_workspace_types::rpc::worktree::{
...
fn hunk_line_info_to_wire(info: &cf_hunk_tracker::types::HunkLineInfo) -> HunkLineInfoWire {
    HunkLineInfoWire {
...
fn hunk_source_to_wire(source: cf_hunk_tracker::types::HunkSource) -> HunkSourceWire {
    use cf_hunk_tracker::types::HunkSource as S;
    match source {
...
fn hunk_to_wire(hunk: &cf_hunk_tracker::types::Hunk) -> HunkWire {
    HunkWire {
...
fn file_content_status_to_wire(
    status: cf_hunk_tracker::types::FileContentStatus,
) -> FileContentStatusWire {
    use cf_hunk_tracker::types::FileContentStatus as S;
    match status {
...
fn file_content_view_to_wire(
    view: cf_hunk_tracker::types::FileContentView,
) -> FileContentViewWire {
    FileContentViewWire {
...
fn file_content_entry_to_wire(entry: cf_hunk_tracker::FileContentEntry) -> FileContentEntryWire {
...
fn session_stats_to_wire(stats: &cf_hunk_tracker::types::SessionStats) -> SessionStatsWire {
    SessionStatsWire {
...
fn turn_summary_to_wire(turn: cf_hunk_tracker::types::TurnSummary) -> TurnSummaryWire {
    TurnSummaryWire {
...
fn session_summary_to_wire(summary: cf_hunk_tracker::SessionSummary) -> SessionSummaryWire {
    SessionSummaryWire {
...
fn tracker_action(kind: HunkActionKind) -> cf_hunk_tracker::types::HunkAction {
    match kind {
        HunkActionKind::Accept => cf_hunk_tracker::types::HunkAction::Accept,
        HunkActionKind::Reject => cf_hunk_tracker::types::HunkAction::Reject,
    }
}
...
fn session_tracker(
    ws: &WorkspaceHandle,
    session_id: Option<&str>,
) -> WorkspaceResult<cf_hunk_tracker::HunkTrackerHandle> {
    let sid = session_id
...
impl WorkspaceOp for HunkSingleActionReq {
    async fn execute(
        ...
        let hunk_id = cf_hunk_tracker::types::HunkId::from_string(self.action.hunk_id.clone());
    ...
    async fn execute(
        ...
        for h in &all_hunks {
            ...
            let is_agent = matches!(
                h.source,
                cf_hunk_tracker::types::HunkSource::AgentEdit { .. }
            );
...
fn hook_registry_to_wire(
    registry: &cf_hooks::discovery::HookRegistry,
...
) -> WorkspaceResult<cf_hooks::discovery::HookRegistry> {
...
fn resolve_index_for_workspace(
    ...
    std::sync::Arc<cf_codebase_graph::IndexManagerHandle>,
    std::path::PathBuf,
)> {
...
fn query_result_to_response(
    result: Result<cf_codebase_graph::QueryResult, cf_codebase_graph::QueryError>,
) -> CodeNavResponse {
    match result {
...
fn symbol_locations_to_response(
    locations: Vec<cf_codebase_graph::SymbolLocation>,
) -> CodeNavResponse {
    CodeNavResponse {
        locations: locations
            .into_iter()
...
///   session's [`FinalizedToolset`](cf_tools::registry::types::FinalizedToolset).
...
pub enum WorkspaceOps {
    ...
    Proxy { client: crate::types::WorkspaceClient },
}
impl WorkspaceOps {
    ...
    pub fn proxy(harness: Arc<ToolHarness>) -> Self {
        Self::Proxy {
            client: crate::types::WorkspaceClient::new((*harness).clone()),
        }
    }
    ...
    pub fn proxy_with_connected(harness: Arc<ToolHarness>, connected: Arc<AtomicBool>) -> Self {
        Self::Proxy {
            client: crate::types::WorkspaceClient::with_connected_flag((*harness).clone(), connected),
        }
    }
    ...
    pub fn client(&self) -> Option<&crate::types::WorkspaceClient> {
        match self {
    ...
    pub fn bind_local_session(
        ...
        hunk_tracker: cf_hunk_tracker::HunkTrackerHandle,
    ...
    pub async fn hook_registry(&self) -> WorkspaceResult<cf_hooks::discovery::HookRegistry> {
    ...
    ///   [`FinalizedToolset`](cf_tools::registry::types::FinalizedToolset)
        ...
        match self {
            ...
            Self::Proxy { client } => {
                if !client.is_connected() {
                    return Err(cf_tool_runtime::ToolError::network_error(
                        "The workspace server connection was lost. \
                         Please restart your session to reconnect.",
                    ));
                }
                let tool_id = cf_tool_protocol::ToolId::new(name).map_err(|e| {
                    cf_tool_runtime::ToolError::custom(
                        "hub_proxy_error",
                ...
                ctx.call_id =
                    cf_tool_protocol::ToolCallId::new(call_id.to_owned()).unwrap_or(ctx.call_id);
                let mut stream = client.harness().call(tool_id, args, ctx).await;
                let typed = crate::types::consume_stream_terminal(&mut stream)
                    .await
                    .inspect_err(|e| {
                        if crate::types::is_transport_fatal(e) {
...
mod tests {
    ...
    /// request types are defined in `qidi-code-types` (and re-exported
    ...
    async fn end_local_session_drops_bound_toolset() {
        ...
        let toolset = std::sync::Arc::new(
            cf_tools::registry::types::FinalizedToolset::empty_for_test(),
        );
    ...
    fn hunk_to_wire_serializes_identically() {
        use cf_hunk_tracker::types::{Hunk, HunkSource};
    ...
    fn file_content_entry_to_wire_serializes_identically() {
        use cf_hunk_tracker::FileContentEntry;
        use cf_hunk_tracker::types::FileContentView;
    ...
    fn session_summary_to_wire_serializes_identically() {
        use std::sync::Arc;
        use cf_hunk_tracker::SessionSummary;
        use cf_hunk_tracker::types::{Hunk, HunkSource, TurnSummary};
    ...
    fn hook_registry_wire_round_trip_both_directions() {
        let spec = cf_hooks::config::HookSpec {
            ...
            event: cf_hooks::event::HookEventName::PreToolUse,
            ...
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::path::PathBuf::from("/home/u/.qidi/hooks"),
            extra_env: std::collections::HashMap::from([("FOO".to_string(), "bar".to_string())]),
        };
        let mut registry = cf_hooks::discovery::HookRegistry::default();
    ...
    fn hook_event_name_wire_covers_all_upstream_variants() {
        use cf_hooks::event::HookEventName as E;
