//! cf-tools's full-replace compaction pass.
//!
//! cf-tools does not select a tail to keep; it summarizes the whole
...
//! ```
//!
//! Per-harness concerns stay in the product host (for example `qidi-code`): the triggers, the
...
pub struct FullReplaceContext<T> {
    ...
    /// turn). cf-tools keeps this; pass empty to drop it.
...
///
/// Returned by [`sample_full_replace_summary`] for harnesses (cf-tools's
...
/// Run cf-tools's full-replace compaction pass and return the rebuilt
...
///
/// This is the seam cf-tools's shell uses: it drives the verbatim → fitted →
    ...
    O: FullReplaceObserver + ?Sized,
{
    ...
    let prompt = CompactionPrompt {
        // cf-tools appends the summarization prompt as the final user
...
mod tests {
    ...
    /// returns a structured summary must produce cf-tools's exact compacted
