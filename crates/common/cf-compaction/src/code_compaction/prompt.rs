//! cf-tools's session-level summarization prompt.
//!
//! Split out of the crate-root `prompt` module so cf-tools's full-replace
...
/// Build cf-tools's session-level summarization prompt (no chat history).
///
...
/// `qidi-code::session::helpers::session_compact::build_compaction_prompt`
...
/// (mirrors `qidi-code`'s `SELF_SUMMARIZATION_PROMPT`). Framed
...
/// so the structured (cf-tools) and short self-summary prompts stay
/// shared instead of duplicated per harness.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum SummaryPromptKind {
    /// cf-tools's detailed, numbered-section summary prompt.
    #[default]
    Structured,
...
mod tests {
    ...
    fn kind_structured_matches_build_summary_prompt() {
        ...
        // so routing through the selector never changes cf-tools's prompt.
        assert_eq!(
