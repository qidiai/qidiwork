...
//!
//! The canonical `sample → classify → retry` loop, used by **both** cf-tools's
...
//! the winning summary (cf-tools cleans in its assembler; intra cleans via
...
pub enum SampleRetryError {
    ...
    Failure {
        ...
        /// signal the cf-tools host uses to step down its input size).
