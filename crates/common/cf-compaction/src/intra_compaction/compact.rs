...
// The `Shared` summarizer reuses cf-tools's full-replace summarization core
...
/// `FullReplace` strategy (default): cf-tools's full-replace — summarize the
...
/// preserves user intent itself, matching cf-tools. The reduction and
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 2. Summarize the whole conversation through cf-tools's shared core.
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    let summary_text = match policy.summarizer {
        ...
        // New (default): cf-tools's shared summarization core from
...
/// — cf-tools's summarization core (`build_summary_prompt` + bounded retry +
...
/// intra and cf-tools stay in lock-step. Outcome mapping:
...
where
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    // cf-tools appends the summarization prompt as the final user message;
    ...
    .await
    {
        // cf-tools returns the raw summary and cleans it in its assembler;
