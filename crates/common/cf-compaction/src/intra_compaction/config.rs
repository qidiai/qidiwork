...
///
/// - `FullReplace` (default): cf-tools's full-replace strategy — summarize
...
/// time (token threshold only, matching cf-tools) and also ignores
...
pub struct IntraCompactionConfig {
    ...
    /// as cf-tools full-replace auto-compact). Worthless early passes are
