...
//! cf-tools implements it for `cf_sampling_types::ConversationItem`.
//!
...
/// `ConversationItem` variants (cf-tools).
...
/// constructs the typed harness items that make up cf-tools's rebuilt
/// history.
///
...
/// that impl. They are also cf-tools-specific (Grok chat's tail-keep path
...
///
/// The cf-tools implementor (`ConversationItem`) maps each constructor to the
