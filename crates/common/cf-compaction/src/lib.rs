...
//! stay in each product host (for example `qidi-code`).
//!
...
//! cf-tools hosts through a small set of trait seams:
//!
...
//!
//! - [`code_compaction`] — cf-tools's whole-session **full-replace**
...
//! cf-tools; hosts still own snapshotting and host-only sections).
...
// cf-tools's full-replace subsystem now lives under `code_compaction`;
