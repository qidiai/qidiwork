...
async fn inner_dispatch_returns_not_found_when_tool_absent() {
    ...
    match item {
        ToolStreamItem::Terminal(Err(ref e))
            if e.kind == cf_tool_runtime::ToolErrorKind::NotFound =>
        {
            assert!(
...
async fn inner_dispatch_after_resolver_drop_returns_computer_hub_dropped() {
    ...
    match item {
        ToolStreamItem::Terminal(Err(ref e))
            if e.kind == cf_tool_runtime::ToolErrorKind::Custom =>
        {
            assert!(
...
async fn inner_dispatch_implements_object_safe_tool_dispatch() {
    ...
    let result = inner
        ...
        .await;
    assert!(matches!(result, Err(ref e) if e.kind == cf_tool_runtime::ToolErrorKind::NotFound));
}
