...
impl InMemRegistry {
    fn install(&self, session: SessionId, tool_id: ToolId, handle: Arc<dyn ToolHandle>) {
        let registration = ToolRegistration {
            ...
            server_id: None,
            description: handle.description(&cf_tool_runtime::ListToolsContext::default()),
            input_schema: None,
...
impl ToolRegistry for InMemRegistry {
    ...
    fn list_servers_for_user(
        &self,
        _user_id: &cf_tool_protocol::UserId,
    ) -> Vec<cf_computer_hub_core::registry::ServerRecord> {
        Vec::new()
    }
    ...
    fn get_server_record(
        &self,
        _connection_id: &ConnectionId,
    ) -> Option<cf_computer_hub_core::registry::ServerRecord> {
        None
    }
}
...
async fn collect(
    stream: &mut ToolStream<cf_tool_runtime::TypedToolOutput>,
) -> Vec<ToolStreamItem<cf_tool_runtime::TypedToolOutput>> {
...
async fn missing_tool_resolves_as_terminal_not_found() {
    ...
    assert_eq!(items.len(), 1);
    match &items[0] {
        ToolStreamItem::Terminal(Err(e)) if e.kind == cf_tool_runtime::ToolErrorKind::NotFound => {
            assert!(
...
async fn invalid_arguments_surface_as_terminal_error() {
    ...
    match &items[0] {
        ToolStreamItem::Terminal(Err(e))
            if e.kind == cf_tool_runtime::ToolErrorKind::InvalidArguments => {}
...
async fn authorize_returns_bound_principal_with_invoke_scope() {
    ...
    assert!(principal.has_scope(cf_computer_hub_core::LOCAL_INVOKE_SCOPE));
