...
use serde_json::json;
use cf_computer_hub_core::{error_from_envelope, is_workspace_unavailable, tool_error_from_wire};
...
use cf_tool_runtime::{ToolError, ToolErrorKind};
...
fn non_custom_decoded_error_is_not_recognized() {
    let wire = ToolErrorWire::ToolNotFound {
        tool_id: cf_tool_protocol::ToolId::new("ns:tool").unwrap(),
    };
    let err = error_from_envelope(envelope_for(&wire));
    assert_ne!(err.kind, ToolErrorKind::Custom);
    assert!(!is_workspace_unavailable(&err));
