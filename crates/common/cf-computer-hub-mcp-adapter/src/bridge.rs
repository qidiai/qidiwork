...
use cf_tool_protocol::{McpBlock, SessionId, ToolId, ToolOutputWire};
use cf_tool_runtime::{ToolCallContext, ToolError, ToolStream, TypedToolOutput, terminal_only};
...
/// [`cf_computer_hub_sdk::ToolServerBuilder`] to register them
/// with the hub.
///
...
impl McpBridge {
    ...
    /// Handlers to register with a [`cf_computer_hub_sdk::ToolServerBuilder`].
    ///
...
#[async_trait]
impl cf_computer_hub_sdk::ToolServerHandler for McpToolHandler {
...
mod tests {
    ...
    async fn bridge_handles_transport_error() {
        ...
        let item = stream.next().await.unwrap();
        match item {
            cf_tool_runtime::ToolStreamItem::Terminal(Err(ref e))
                if e.kind == cf_tool_runtime::ToolErrorKind::Execution =>
            {
                assert!(
