...
struct WaiterGuard<'a> {
    demux: &'a Demux,
    request_id: &'a cf_tool_protocol::RequestId,
}
...
pub struct ConnectionConfig {
    ...
    pub server_id: Option<cf_tool_protocol::ServerId>,
...
impl HubConnection {
    ...
    pub async fn call_request<P>(
        &self,
        request_id: cf_tool_protocol::RequestId,
    ...
    pub fn try_alloc_request_id(&self) -> Result<cf_tool_protocol::RequestId, ClientError> {
        let value = self
            .inner
            .next_request_id
            .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        cf_tool_protocol::RequestId::new(format!("c{value}")).map_err(ClientError::from)
    }
    ...
    pub async fn serve(
        ...
        params: cf_tool_protocol::ServeParams,
    ) -> Result<cf_tool_protocol::ServeResult, ClientError> {
...
async fn open_socket(
    ...
    cf_tracing::http_client::attach_trace_to_http_request(headers);
    ...
    if inner.kind == ConnectionKind::Harness {
        for sid in &sessions {
            let req = cf_tool_protocol::JsonRpcRequest {
                ...
                id: cf_tool_protocol::JsonRpcId::new_uuid_v7(),
...
mod tests {
    ...
    async fn serve_send_failure_fails_fast_without_retry() {
        ...
        let result = tokio::time::timeout(
            ...
            conn.serve(session, cf_tool_protocol::ServeParams { tools: vec![] }),
        )
        .await
        .expect("serve must fail bounded, not park");
        assert!(matches!(result, Err(ClientError::NetworkError(_))));
        let request_id = cf_tool_protocol::RequestId::new("c1").expect("valid");
        assert!(
    ...
    async fn serve_times_out_bounded_and_reclaims_every_attempt_waiter() {
        ...
        let result = conn
            .serve(session, cf_tool_protocol::ServeParams { tools: vec![] })
            .await;
        assert!(matches!(result, Err(ClientError::NetworkError(_))));
        for id in ["c1", "c2", "c3"] {
            ...
            let request_id = cf_tool_protocol::RequestId::new(id).expect("valid");
            assert!(
                demux.take_response_waiter(&request_id).is_none(),
