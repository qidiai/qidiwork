...
//!
//! Wire-level [`cf_tool_protocol::ToolErrorWire`] variants and JSON-RPC
...
use thiserror::Error;
use url::Url;
use cf_tool_protocol::{IdError, JsonRpcError, ToolCallId, ToolErrorWire};
...
pub enum ClientError {
    ...
    /// [`cf_tool_runtime::ToolCallContext::default`], which does so)
...
mod tests {
    ...
    fn sdk_reexported_recognizer_matches_decoded_error() {
        ...
        let err = cf_computer_hub_core::error_from_envelope(workspace_gone_envelope());
    ...
    fn sdk_reexported_recognizer_rejects_unrelated_custom_error() {
        ...
        let err = cf_computer_hub_core::error_from_envelope(env);
        assert!(!crate::is_workspace_unavailable(&err));
    }
}
