...
use url::Url;
use cf_computer_hub_core::{
    ErasedTool, ToolHandle, decode_call_result, error_from_envelope, progress_from_frame,
    tool_error_from_wire,
};
use cf_tool_protocol::notification_wire::{WireCustomNotification, WireToolNotification};
use cf_tool_protocol::session_event::{SessionEvent, ToolCallOutcome};
use cf_tool_protocol::{
...
use cf_tool_types::ToolDescription;
...
/// [`HookFrame`](cf_tool_protocol::HookFrame), answered via
/// [`ToolHarness::send_hook_reply`].
type HookRequestHandler = Arc<dyn Fn(cf_tool_protocol::HookFrame) + Send + Sync>;

/// Well-known [`HookEvent::Custom`](cf_tool_protocol::HookEvent::Custom) kind
/// for a server → harness permission request. Sibling of
/// [`cf_tool_protocol::turn_hook::TURN_HOOK_KIND`].
...
pub type ModelOutputExtractor =
    Arc<dyn Fn(&Value) -> Option<Vec<cf_tool_runtime::ContentBlock>> + Send + Sync>;

pub fn extractor_for<T>() -> ModelOutputExtractor
where
    T: cf_tool_runtime::ToolOutput + serde::de::DeserializeOwned + 'static,
{
    Arc::new(|value: &Value| {
...
impl LocalRegistry {
    ...
    pub fn register_dyn(
        &self,
        tool: Arc<dyn cf_tool_runtime::ToolDyn>,
    ...
    where
        T: Tool + std::fmt::Debug + 'static,
        T::Output: cf_tool_runtime::ToolOutput + serde::de::DeserializeOwned + 'static,
    {
        let id = tool.id();
        self.inner
            .extractors
    ...
    pub fn model_output(
        ...
        tool_id: &ToolId,
        output: &Value,
    ) -> Option<Vec<cf_tool_runtime::ContentBlock>> {
        self.inner
            .extractors
            .get(tool_id)
...
struct DynToolAdapter(Arc<dyn cf_tool_runtime::ToolDyn>);
...
impl ToolHandle for DynToolAdapter {
    ...
    async fn execute(
        ...
        ctx: ToolCallContext,
        args: Value,
    ) -> ToolStream<cf_tool_runtime::TypedToolOutput> {
...
pub struct ToolHarnessBuilder {
    ...
    default_extensions: Option<cf_tool_runtime::TypedExtensions>,
    ...
    resume: bool,
    last_seq: Option<cf_tool_protocol::LastSeq>,
}
...
impl ToolHarnessBuilder {
    ...
    pub fn default_extensions(mut self, extensions: cf_tool_runtime::TypedExtensions) -> Self {
    ...
    pub fn last_seq(mut self, last_seq: cf_tool_protocol::LastSeq) -> Self {
    ...
    pub async fn build(self) -> Result<ToolHarness, ClientError> {
        ...
        // but the initial connect must do it explicitly.
        {
            ...
            let req = cf_tool_protocol::JsonRpcRequest {
                ...
                method: cf_tool_protocol::Method::SessionOpen
                    .as_wire_str()
                    .to_owned(),
                params: cf_tool_protocol::SessionOpenParams {
                    resume: self.resume,
...
struct ToolHarnessInner {
    ...
    session: SessionId,
    /// Default extensions merged into every `ToolCallContext` before dispatch.
    default_extensions: cf_tool_runtime::TypedExtensions,
...
impl ToolHarnessInner {
    ...
    async fn refresh_remote_tools(&self) -> Result<Vec<ToolDescription>, ClientError> {
        ...
        let connection = borrow.connection();
        let request_id = connection.try_alloc_request_id()?;
        let params = cf_tool_protocol::ToolsListParams {
            session_id: self.session.clone(),
            mode: cf_tool_protocol::ToolDefinitionMode::Full,
        };
        ...
        match resp.outcome {
            ResponseOutcome::Result(value) => {
                let result: cf_tool_protocol::ToolsListResult =
...
impl ToolHarness {
    ...
    pub async fn list_servers(&self) -> Result<Vec<cf_tool_protocol::ServerInfo>, ClientError> {
        ...
        let req = JsonRpcRequest {
            ...
            method: cf_tool_protocol::Method::ServersList
                .as_wire_str()
                .to_owned(),
            params: cf_tool_protocol::ServersListParams {},
        };
        let resp = connection.call_request(request_id, &req).await?;
        match resp.outcome {
            ResponseOutcome::Result(value) => {
                let result: cf_tool_protocol::ServersListResult =
                    serde_json::from_value(value).map_err(|e| ClientError::Serde(e.to_string()))?;
                Ok(result.servers)
            }
            ResponseOutcome::Error(err) => Err(ClientError::from_jsonrpc_error(err)),
    ...
    pub async fn session_open_with(
        ...
        let result: Result<(), ClientError> = async {
            let connection = self.require_connection()?;
            let request_id = connection.try_alloc_request_id()?;
            let params = cf_tool_protocol::SessionOpenParams { resume, last_seq };
            let req = JsonRpcRequest {
                jsonrpc: JsonRpcVersion,
                id: JsonRpcId::from_request_id(&request_id),
                session_id: Some(self.inner.session.clone()),
    ...
    pub async fn session_bind_with_report(
        ...
        cwd: Option<&str>,
        metadata: Option<serde_json::Value>,
    ) -> Result<cf_tool_protocol::SessionBindServerResult, ClientError> {
        let start = std::time::Instant::now();
        let result: Result<cf_tool_protocol::SessionBindServerResult, ClientError> = async {
            let connection = self.require_connection()?;
            let request_id = connection.try_alloc_request_id()?;
            let parsed_server_id = cf_tool_protocol::ServerId::new(server_id)
                .map_err(|e| ClientError::InvalidConfig(format!("invalid server_id: {e}")))?;
            let params = cf_tool_protocol::SessionBindServerParams {
            ...
            match resp.outcome {
                ResponseOutcome::Result(value) => {
                    let bind_result: cf_tool_protocol::SessionBindServerResult =
    ...
    pub async fn session_attach(
        ...
        caller: &str,
    ) -> Result<cf_tool_protocol::SessionAttachServerResult, ClientError> {
        let start = std::time::Instant::now();
        let result: Result<cf_tool_protocol::SessionAttachServerResult, ClientError> = async {
            let connection = self.require_connection()?;
            let request_id = connection.try_alloc_request_id()?;
            let parsed_server_id = server_id
                .map(|s| {
                    cf_tool_protocol::ServerId::new(s)
                        .map_err(|e| ClientError::InvalidConfig(format!("invalid server_id: {e}")))
                })
                .transpose()?;
            let params = cf_tool_protocol::SessionAttachServerParams {
                server_id: parsed_server_id,
            ...
            match resp.outcome {
                ResponseOutcome::Result(value) => {
                    let attach_result: cf_tool_protocol::SessionAttachServerResult =
                        serde_json::from_value(value)
                            .map_err(|e| ClientError::Serde(e.to_string()))?;
                    self.inner
                        .remote_tools
    ...
    pub async fn session_unbind(&self, server_id: &str) -> Result<(), ClientError> {
        ...
        let parsed_server_id = cf_tool_protocol::ServerId::new(server_id)
            .map_err(|e| ClientError::InvalidConfig(format!("invalid server_id: {e}")))?;
        let params = cf_tool_protocol::SessionUnbindServerParams {
            server_id: parsed_server_id,
        };
        let req = JsonRpcRequest {
            jsonrpc: JsonRpcVersion,
    ...
    pub async fn session_close(&self) -> Result<(), ClientError> {
        let connection = self.require_connection()?;
        let request_id = connection.try_alloc_request_id()?;
        let params = cf_tool_protocol::SessionCloseParams { reason: None };
        let req = JsonRpcRequest {
            jsonrpc: JsonRpcVersion,
            id: JsonRpcId::from_request_id(&request_id),
            session_id: Some(self.inner.session.clone()),
    ...
    pub async fn call(
        ...
        let raw_stream: ToolStream<cf_tool_runtime::TypedToolOutput> =
    ...
    pub async fn send_notification(
        &self,
        notification: cf_tool_protocol::ToolNotificationFrame,
    ) -> Result<(), ClientError> {
    ...
    pub async fn send_hook(
        &self,
        mut hook: cf_tool_protocol::HookFrame,
    ) -> Result<(), ClientError> {
        ...
        let hook_type = match &hook.event {
            cf_tool_protocol::HookEvent::Cancel => "cancel",
            cf_tool_protocol::HookEvent::Pause => "pause",
            cf_tool_protocol::HookEvent::Resume => "resume",
            cf_tool_protocol::HookEvent::SessionEnded => "session_ended",
            cf_tool_protocol::HookEvent::Custom { .. } => "custom",
        };
    ...
    ///
    /// Sends the call-scoped `Cancel` [`HookFrame`](cf_tool_protocol::HookFrame)
    ...
    pub async fn cancel_call(
        ...
        let hook = cf_tool_protocol::HookFrame::cancel(
    ...
    pub async fn send_before_turn_hook(
        ...
        let hook = cf_tool_protocol::HookFrame::custom(
            self.session().clone(),
            cf_tool_protocol::turn_hook::BEFORE_TURN_KIND.to_owned(),
    ...
    pub async fn send_after_turn_hook(
        ...
        let hook = cf_tool_protocol::HookFrame::custom(
            self.session().clone(),
            cf_tool_protocol::turn_hook::AFTER_TURN_KIND.to_owned(),
            serde_json::to_value(&payload).map_err(|e| ClientError::Serde(e.to_string()))?,
        );
        self.send_hook(hook).await
    }
    ...
    ) -> Result<cf_tool_protocol::turn_hook::HookReply, ClientError> {
    ...
    pub async fn request_turn_hook_with_timeout(
        ...
        let hook_id = cf_tool_protocol::ToolCallId::new_v7().to_string();
        let hook = cf_tool_protocol::HookFrame::custom_request(
            self.inner.session.clone(),
            hook_id,
            cf_tool_protocol::turn_hook::TURN_HOOK_KIND.to_owned(),
            payload,
        )
        .with_trace_context(
            self.inner
    ...
    /// Answer a reverse-direction request/response [`HookFrame`](cf_tool_protocol::HookFrame)
    ...
    pub async fn send_hook_reply(
        &self,
        reply: cf_tool_protocol::HookReplyFrame,
    ) -> Result<(), ClientError> {
        let connection = self.require_connection()?;
    ...
    where
        F: Fn(cf_tool_protocol::HookFrame) + Send + Sync + 'static,
    {
...
/// hook, returning the decoded [`HookFrame`](cf_tool_protocol::HookFrame) or `None`.
fn parse_permission_request_hook(value: &Value) -> Option<cf_tool_protocol::HookFrame> {
    let method = value.get("method").and_then(Value::as_str)?;
    if method != Method::Hook.as_wire_str() {
        return None;
    }
    let hook =
        <cf_tool_protocol::HookFrame as serde::Deserialize>::deserialize(value.get("params")?)
            .ok()?;
    // Only request/response hooks (those with a reply leg) qualify.
    hook.hook_id.as_ref()?;
    match &hook.event {
        cf_tool_protocol::HookEvent::Custom { kind, .. } if kind == PERMISSION_REQUEST_KIND => {
            Some(hook)
        }
        _ => None,
    }
...
/// [`HookReplyFrame::hook_id`](cf_tool_protocol::HookReplyFrame::hook_id).
fn build_hook_reply_notification(
    session_id: &SessionId,
    mut reply: cf_tool_protocol::HookReplyFrame,
) -> JsonRpcNotification<cf_tool_protocol::HookReplyFrame> {
...
struct ObservedToolStream {
    inner: ToolStream<cf_tool_runtime::TypedToolOutput>,
...
impl Stream for ObservedToolStream {
    type Item = ToolStreamItem<cf_tool_runtime::TypedToolOutput>;
...
// `cf_computer_hub_core::remote`. Routing the harness through the same
...
mod tests {
    ...
    impl cf_tool_runtime::ToolOutput for EchoOut {}
    ...
    async fn local_only_emit_session_event_is_noop() {
        ...
        harness
            .emit_session_event(SessionEvent::PhaseChanged {
                phase: cf_tool_protocol::session_event::SessionPhase::Idle,
            })
            .await;
    }

    /// Wrap a [`HookFrame`](cf_tool_protocol::HookFrame) in the JSON-RPC `Request` envelope.
    fn inbound_hook_request_frame(hook: &cf_tool_protocol::HookFrame) -> Value {
        serde_json::json!({
            "jsonrpc": "2.0",
            "id": "h1",
    ...
    fn parse_permission_request_hook_matches_request_response_hook() {
        ...
        match parsed.event {
            cf_tool_protocol::HookEvent::Custom { kind, payload } => {
    ...
    fn parse_permission_request_hook_rejects_non_custom_event() {
        let hook = cf_tool_protocol::HookFrame {
            session_id: SessionId::new("s1").expect("valid"),
            tool_id: None,
            call_id: None,
            hook_id: Some("hook-7".to_owned()),
            event: cf_tool_protocol::HookEvent::Pause,
            trace_context: None,
        };
        assert!(parse_permission_request_hook(&inbound_hook_request_frame(&hook)).is_none());
    }
    ...
    async fn registered_handler_receives_matching_request_only() {
        ...
        let (tx, mut rx) = mpsc::channel::<cf_tool_protocol::HookFrame>(4);
        ...
        let other = cf_tool_protocol::HookFrame::custom_request(
            SessionId::new("s1").expect("valid"),
            "hook-0".to_owned(),
            cf_tool_protocol::turn_hook::TURN_HOOK_KIND.to_owned(),
            serde_json::json!({}),
        ...
        let perm = cf_tool_protocol::HookFrame::custom_request(
            SessionId::new("s1").expect("valid"),
            "hook-7".to_owned(),
            PERMISSION_REQUEST_KIND.to_owned(),
            serde_json::json!({ "tool_call_id": "call-1" }),
    ...
    fn hook_reply_notification_has_correct_wire_shape() {
        ...
        let reply = cf_tool_protocol::HookReplyFrame {
