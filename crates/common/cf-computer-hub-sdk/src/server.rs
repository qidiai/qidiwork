...
fn system_notify_ack_from_outcome(
    ...
    match outcome {
        ...
        ResponseOutcome::Error(err)
            if err.data.is_none()
                && cf_tool_protocol::error_codes::string_for(err.code)
...
pub struct ResolvedSessionHandlers {
    ...
    /// [`cf_tool_protocol::SessionBindResult::unserved_tool_ids`].
    pub unserved_tool_ids: Vec<String>,
    /// Human-readable reason the resolver failed the toolset closed;
    /// forwarded as [`cf_tool_protocol::SessionBindResult::resolve_error`].
...
/// already use [`cf_tool_runtime::Tool`] can adapt by calling the
...
pub struct ToolServerBuilder {
    ...
    server_id: Option<cf_tool_protocol::ServerId>,
...
impl ToolServerBuilder {
    ...
    pub fn server_id(mut self, id: cf_tool_protocol::ServerId) -> Self {
    ...
    /// [`cf_tool_protocol::SessionBindResult::binary_version`].
...
impl ToolServer {
    ...
    pub async fn serve(&self, session_id: SessionId) -> Result<(), ClientError> {
        ...
        let tools: Vec<cf_tool_protocol::ToolDescriptionWithSchema> = {
            let map = self.inner().session_handlers.read();
            let handlers = map.get(&session_id);
            handlers
                .map(|h| {
                    h.iter()
                        .map(|h| cf_tool_protocol::ToolDescriptionWithSchema {
        ...
        let params = cf_tool_protocol::ServeParams { tools };
    ...
    pub async fn send_system_notification(
        &self,
        session_id: SessionId,
        params: cf_tool_protocol::SystemNotifyParams,
    ) -> Result<SystemNotifyAck, ClientError> {
        ...
        if payload_len > cf_tool_protocol::MAX_SYSTEM_NOTIFY_PAYLOAD_BYTES {
            return Err(ClientError::ProtocolError(format!(
                "system.notify payload {payload_len} bytes exceeds {} byte cap",
                cf_tool_protocol::MAX_SYSTEM_NOTIFY_PAYLOAD_BYTES
            )));
        }
        let connection = self.inner().borrow.connection();
        let request_id = connection.try_alloc_request_id()?;
    ...
    pub async fn donate_traces(&self, otlp_request_b64: &str) -> Result<(), ClientError> {
        ...
        let connection = self.inner().borrow.connection();
        let notification = cf_tool_protocol::JsonRpcNotification {
            jsonrpc: JsonRpcVersion,
            session_id: Some(session),
            seq: None,
            ...
            loop {
                ...
                match method {
                    "session.bind" => {
                        ...
                        let Ok(sid) = cf_tool_protocol::SessionId::new(sid_str) else {
                            continue;
                        };
                        ...
                        tokio::spawn(async move {
                            ...
                            if let Some(id) = request_id {
                                let response = match result {
                                    Ok(()) => {
                                        let tools: Vec<cf_tool_types::ToolDescription> = server
                                            .handlers_for_session(&sid)
                                            .iter()
                                            .map(|h| h.description())
                                            .collect();
                                        let result = cf_tool_protocol::SessionBindResult {
                                            tools,
...
async fn push_disconnect_status(connection: &HubConnection, sessions: &[SessionId]) {
    use cf_tool_protocol::{JsonRpcRequest, ToolServerLifecycleStatus, ToolServerStatusPayload};
...
async fn execute_call(
    ...
    ctx.extensions.insert(cf_tool_runtime::SessionContext(
...
mod tests {
    use super::*;
    use cf_tool_runtime::ContentBlock;
